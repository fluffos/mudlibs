...
- **64 files across 39 libs — real, fixed**: same guard pattern as the reference implementation, adapted per file's own exit points (early-return, fight-back, alternate revive-room branches, etc.). Two of these 39 libs surfaced sub-notes worth flagging for future sweeps: (a) `bxsj`, `shujian2008`, `shujian3`, `sjecl`, `sjtx2` each carry a `death2.h` alongside a `death.h` in `d/death/npc/` — the live NPCs (`yanluo.lpc`/`mengpo.lpc`/`pusa.lpc`) `#include` **`death.h`**, which was already guarded from an earlier fix, while `death2.h` is dead/unreferenced; fixing `death2.h` in these 5 libs was a harmless no-op, not a real gap-close (only `bxsj1`'s pair had a genuinely unguarded live `death.h`); (b) `wdxtym`/`wxddym`'s `bai.lpc` has a pre-existing, unrelated compile error (`query("reborn_offer", ob)` — bad arg-2 type, int vs object — on a line this edit didn't touch) that leaves the file uncompilable regardless; the guard fix is textually correct and will take effect the moment that separate bug is fixed.
