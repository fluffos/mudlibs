...
**Confirmed and fixed in 12 libs across at least 5 independent lineages so far** (`dtsl`/`dtsl2`/`dtslmud` — byte-identical `yanluo.lpc`; `jym`/`xuanjianlu`/`xkm`/`shenzhou`/`xkx2001` — a second, unrelated lineage sharing "gargoyle" death-guide NPCs with per-file differences, not byte-identical, each hand-fixed; `haiyang2` — 13 files in one lib, including **plain ROOM objects with no NPC at all** (PK-arena staging rooms whose own `init()` spawned a bodyguard NPC); `bxsj` — a 4th unrelated lineage, plus a proactive hardening of one room upstream that wasn't confirmed vulnerable; `sj` — live-verified via a precisely-timed reconnect race; `yueyingqiyuan` — a 5th lineage, 4 psychopomp NPCs, whose fix covered two exit branches the `sj` reference implementation itself had left unguarded). Not yet swept corpus-wide, and given how many independent lineages have hit this, it's now a **standing checklist item for every round-three §10.7 test on any lib**: grep `call_out(` inside any `init()` function (not just death/reincarnation-themed directories — check ALL of them) and read each hit's specific chain shape before fixing (the guard/clear-point placement isn't as mechanically uniform as §7.111's single-line swap, so this needs per-file judgement, not a blind sweep).
