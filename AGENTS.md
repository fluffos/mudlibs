...
**Sweep in progress, mixed results — do NOT batch-apply blindly**:
`grep -rl "my_port = LOCAL_PORT() + MESSAGE_PORT"
...
- **Confirmed clean, no fix needed**: `aoxiangtianji`, `zjdywzb`,
  ...
  `xkxz2`/`xkxc98sj` (Xkxz MUD Wizard Group lineage, unrelated to
  "Doing"/hell) both lack a `MESSAGE_D` fallback in `goto.lpc`, so
  `tell <nonexistent-player>` was used as the trigger instead
  (routes through `cmds/std/tell.lpc` → `MESSAGE_D` → `messaged.lpc`'s
...
permission-tier issues like the one hit on `wxddym`) if `call` is
available (`(arch)`-tier grant, not guaranteed on every lib's seeded
admin); otherwise use `goto` if its `goto.lpc` has the
`MESSAGE_D->find_user()` fallback (grep `grep -n MESSAGE_D
work/cmds/wiz/goto.lpc`), or fall back to any command file referencing
`MESSAGE_D` (`grep -rln "MESSAGE_D" work --include='*.lpc'`). Remember
to also check the same lib's `versiond.lpc` for the sibling
...
```
This `foreach` has **no batching, no yield, and no cap** — every single entry in `closed_users` gets fully restored and `enter_world()`-ed in ONE heartbeat call. On a fresh boot, `find_body(u)` finds nobody for any of them, so EVERY account in the map gets a fresh `restore()` + `enter_world()` in that single call. `nt1`'s own `data/closed.o` had **138 accounts** queued this way.

**Root cause of the memory growth itself, empirically isolated (not just theorized)**: it is NOT the restored characters' own save data (a sampled account's save file was a perfectly ordinary 13KB, and the driver's own `mud_status(1)`/`status detail` command reported LPC-level "Total accounted" memory — the sum of every live Object, Mapping, Array, Prog block, and String the driver tracks — at only **17MB** while the OS-reported process RSS at that same instant was **752MB**, a >40x gap that kept widening (RSS climbed past 2.9GB over the next ~500s with the accounted-for LPC data staying essentially flat). This rules out "a leaked LPC object/mapping" as the mechanism. Controlled tests confirmed the trigger scales with account count and is caused by `enter_world()` moving each restored character into their saved `startroom`: this lazily compiles that character's entire home city/zone — its rooms, NPCs, quest content, and any procedurally-generated dungeon subtree (e.g. `d/llszhuang/dynamic/jiuqu*.lpc`) — none of which would otherwise get touched for a long time in normal gradual play. A boot with `closed_users` emptied to `0` compiled only the ~27-entry preload list and finished in ~2 minutes at a steady ~77MB. The IDENTICAL boot with just **10** of the 138 accounts restored kept compiling new, never-before-seen files (rooms/NPCs across `d/fuzhou`, `d/xiangyang`, `d/quanzhen`, `kungfu/skill/*`, etc.) continuously for 12+ minutes without plateauing, climbing past 2.9GB. The compiled program data itself is cheap (547 compiled programs totaled only 892KB of "Prog blocks" in the same `status detail` reading) — so the gigabytes are being consumed by the C-level driver's own **allocator** during the burst of compilation (parsing/AST/symbol-table churn for hundreds of files in rapid succession), which glibc's default allocator (ptmalloc) does not return to the OS even after the LPC-visible scratch memory is freed and reused — classic heap fragmentation from a large, tightly-clustered burst of small/medium transient allocations. With 138 accounts scattered across many different (and apparently large/densely-linked or procedurally-generated) zones, the cumulative reachable-file set balloons to a large fraction of the entire 24,649-file tree, and the accompanying allocator fragmentation compounds into the tens of GB.

**Why this differs from "just a big lib, boots slow"**: a large `.lpc` file count alone is a linear, bounded preload cost. The 138-account mass-restore-and-resume is a genuinely unbounded design gap: nothing caps how many accounts `load_all_users()` will process in one call, nothing paces `enter_world()` across multiple heartbeat ticks, and each account restored can drag in an entire zone's worth of previously-untouched files whose *compilation* cost (not their final resident size) is what balloons process RSS.
...
### 7.112 An NPC's `init()` unconditionally scheduling a `call_out()` chain gets a duplicate stacked on top of it every time the player it's tracking simply reconnects — because `enable_commands()` re-broadcasts `init()`, and every ordinary reconnect calls `enable_commands()` — corpus-wide sweep completed across two waves, ~270 libs fixed
...
**Discovery arc**: first found via `dtsl`'s round-three test (`yanluo.lpc`, fixed in `dtsl`/`dtsl2`/`dtslmud` — byte-identical files). Then independently rediscovered in a second, unrelated "gargoyle" death-guide NPC lineage (`jym`/`xuanjianlu`/`xkm`/`shenzhou`/`xkx2001`/`haiyang2`/`bxsj`/`sj`/`yueyingqiyuan`/`chidi`/`bixiecanyang`), confirming it as a general cross-lineage pattern rather than one archive's quirk — including one instance (`haiyang2`) in **plain ROOM objects with no NPC at all** (PK-arena staging rooms whose own `init()` spawned a bodyguard NPC), and at least one live-verified reconnect-race repro (`sj`) plus one that closed exit branches an earlier reference fix itself had missed (`yueyingqiyuan`). Then `aoxiangtianji`'s round-three test found ANOTHER instance under a completely different NPC filename (`d/death/npc/pang.lpc`, not `wgargoyle.lpc`/`bgargoyle.lpc`) — revealing that the corpus-wide "wave 1" filename-based sweep (below) had missed a whole second cluster of instances hiding under different NPC/room names.

**Corpus-wide sweep, wave 1 — the `wgargoyle.lpc`/`bgargoyle.lpc` lineage** (2026-08-18): a grep for `d/death/npc/wgargoyle.lpc` alone found it present in **154 libs**; 11 were already fixed via round-three discovery by that point, leaving 130 (plus one accidentally-omitted lib, `ntii`, caught by a verification pass) swept in 8 sequential batches of ~15-19 libs, dispatched to fresh subagents with the proven fix shape and reference implementations, each batch spot-verified via standalone `lpcc` compiles or live driver boots. Final count: 137+ libs fixed in this lineage, with 7 libs' `wgargoyle.lpc` intentionally left unguarded and individually documented as non-issues (already self-guard via `remove_call_out()`, the file is dead/unreferenced code with the real live NPC renamed and fixed elsewhere, or one has an unrelated pre-existing syntax error making it uncompilable regardless).

**Corpus-wide sweep, wave 2 — alternate NPC/room filenames** (2026-08-18, same day, following the `aoxiangtianji` finding): a behavioral grep (`call_out("death_stage"`, not filename-based) across every `.lpc` in the corpus found **296 more files across 142 libs** carrying the same vulnerable shape under DIFFERENT names — most commonly `d/shaolin/npc/yu-zu2.lpc` (a "prison warden" NPC, ~80 instances), `d/death/npc/pang.lpc`/`b.lpc` (judge/black-gargoyle NPCs under different filenames, ~70 instances combined), `d/bwdh/sjsz{,2,3}/{east,west}_xiangfang.lpc` (PK-arena staging ROOMs, ~30 instances), plus smaller clusters (`panguan.lpc`/`panguan2.lpc`, `mengpo.lpc`, `chacha.lpc`, `pusa.lpc`, `egate.lpc`/`wgate.lpc`, `leader.lpc`). Swept in 9 more sequential batches with the same subagent-dispatch method, each batch instructed to verify the actual code (not just the filename) before fixing, since many files sharing these short/generic names turned out to be unrelated NPCs (vendors, court officials, weapon items) or already self-guarded via `remove_call_out()`.

**Final verification and gap-closing**: a comprehensive final pass — re-checking EVERY lib in the corpus directly for all these filenames, not relying on either wave's batch-dispatch list — found a residual gap: 4 libs (`hymud`, `niaoren`, `sjcs`, `sanjieshenhua`) had been silently excluded from every wave-2 batch, because wave 2's batch lists were generated from wave 1's "still-unguarded `wgargoyle.lpc`" list, and these 4 libs' `wgargoyle.lpc` had already been fixed early in wave 1 — so they never appeared in wave 2's input at all, even though they independently had OTHER unfixed alternate-named files. Fixed directly (not via subagent, given the small remaining count) as a follow-up commit. **This is the actual final state**: a corpus-wide re-scan for the guard string across every candidate filename found exactly ONE remaining unguarded file, `libs/hy/work/海洋2002/hy3/d/shaolin/npc/yu-zu2.lpc` — a confirmed-dead, unreferenced embedded duplicate mudlib tree, intentionally left alone.

**Lessons for any future recurrence of this pattern on a newly-added lib**: (1) this is NOT mechanically uniform like §7.111's single-line swap — every file needs its own control-flow read, since `final_death_stage()` branches, extra item-drop/cloth-wearing logic, and dual live-copy directories (e.g. `d/xiyou/death/npc/`, `d/suburb/<x>/death/npc/`, `d/death22/npc/`) vary per lib; (2) always check for a renamed live NPC (`bai.lpc`/`hei.lpc`/`b.lpc`/`pang.lpc` instead of `wgargoyle.lpc`/`bgargoyle.lpc`) before concluding a lib has no vulnerable code — a single filename-based grep is NOT sufficient, this bug hides under many names; (3) when generating a "libs still needing fixing" batch list FROM a "libs where pattern X is still unguarded" query, remember that a lib whose pattern-X instance was already fixed will silently vanish from that list even if it has an unrelated pattern-Y instance still needing fixing — always do a final pass that checks every lib directly, not just libs implied by an earlier query's output; (4) `lpcc`'s standalone compile mode can trip an unrelated eval-cost/preload artifact on a sibling file having nothing to do with your edit (confirm via a real driver boot instead of trusting a non-zero `lpcc` exit code at face value); (5) this remains a **standing checklist item for every round-three §10.7 test on any lib not yet covered by this sweep**: grep `call_out(` inside any `init()` function, not just death/reincarnation-themed directories.

**Residual-gap closure, round two (2026-08-20)**: three SEPARATE round-four §10.7 deep tests (`hhsj`, `nt6nitan6win`, `jh2006`), run well after the "actual final state" above was declared, each independently rediscovered NEW unguarded instances under yet more filenames the prior sweep's seed lists never covered — confirming this bug hides under an effectively open-ended set of names, not a bounded list. A fresh behavioral corpus scan (`grep -rl 'call_out("death_stage"' libs/*/work --include='*.lpc' --include='*.h' | filter out files already containing the guard string`) found **85 candidate files across ~41 libs**. Per-file control-flow review (not just the filename) classified them as:
- **64 files across 39 libs — real, fixed**: same guard pattern as the reference implementation, adapted per file's own exit points (early-return, fight-back, alternate revive-room branches, etc.). Two of these 39 libs surfaced sub-notes worth flagging for future sweeps: (a) `bxsj`, `shujian2008`, `shujian3`, `sjecl`, `sjtx2` each carry a `death2.h` alongside a `death.h` in `d/death/npc/` — the live NPCs (`yanluo.lpc`/`mengpo.lpc`/`pusa.lpc`) `#include` **`death.h`**, which was already guarded from an earlier fix, while `death2.h` is dead/unreferenced; fixing `death2.h` in these 5 libs was a harmless no-op, not a real gap-close (only `bxsj1`'s pair had a genuinely unguarded live `death.h`); (b) `wdxtym`/`wxddym`'s `bai.lpc` had a pre-existing, unrelated compile error (`query("reborn_offer", ob)` — bad arg-2 type, int vs object — on a line this edit didn't touch) that left the file uncompilable regardless; the guard fix itself was textually correct all along. **Both now fixed (2026-08-21)**: `wdxtym` via its own round-four §10.7 test (live-verified through a real death dialogue), `wxddym` by porting the identical fix (`ob->query("reborn_offer")`) — `lpcc --batch` reports a FAIL on `wxddym` due to an unrelated missing-content issue in `create()` (`carry_object()` on a nonexistent `obj/` subdirectory), but this is a known lpcc-vs-live-driver artifact: the same missing directory exists on `wdxtym` too, whose live driver boot already proved this NPC loads and functions correctly despite it — re-confirmed directly on `wxddym` via a real native driver boot (clean, `Initializations complete`, no crash).
- **21 files — false positives, left alone**, matching the same exclusion categories the original sweep already established: already self-guarded via `remove_call_out("death_stage")` (`nitan170911`/`nitan6`/`nt6nitan6win`/`nt6`'s `chacha.lpc`, `xiakexing100`/`xkx100`'s `wgargoyle.lpc`/`bgargoyle.lpc`, 9 files total — this cancels-and-restarts rather than stacks, so no duplicate-revival risk even though it's a different mechanism than the temp-flag guard); already guarded under a DIFFERENT flag name (`dtsl`/`dtsl2`/`dtslmud`'s `yanluo.lpc` via `in_death_stage`, `dtxywzxzb`'s `pang.lpc` via `death_stage_running` — 4 files, a reminder that a literal `death_stage_active` grep alone will always under-count); dead code — `death_stage()` itself commented out (`hy3`/`syxjl`'s `wgargoyle.lpc`, `syxjl`'s `bgargoyle.lpc` — 3 files) or a confirmed-dead embedded duplicate tree (`hy`'s `d/shaolin/npc/yu-zu2.lpc`, previously documented above); pre-existing uncompilable syntax error unrelated to this bug (`xbtxiii`'s `wgargoyle.lpc` — missing `;` before an `if`, already broken independent of any guard); a different bug shape entirely, not the reincarnation-narrative pattern (`kxkj`/`kxkjii2`'s `open/tendo/room/tower/road3.lpc` — a PK-arena checkpoint room that self-perpetuates a 1-second poll forever with no `reincarnate()`/revival logic, so duplicate chains just mean redundant polling, not misrouted revival); and one narrower case where the vulnerable `call_out` is reachable only through the player's own repeated `say`/`bow` input, not through `init()`/reconnect at all (`shzs`'s `toilet.lpc`), so the reconnect-duplication mechanism this bug class depends on doesn't apply.
- One instance (`zitengzhan`'s `d/shengdoushi/7hai/nby/npc/haijiangjun.lpc`) had a structurally different shape — the `death_stage` chain is reached through an intermediate `greeting()`/`greeting2()` pair rather than directly from `init()`, and only the outer `greeting` scheduling had a (correct but insufficient) `remove_call_out()` self-guard — fixed with a `death_stage_active` flag spanning the whole outer-to-inner chain, following the same "read the actual control flow, don't template-paste" principle as every other file in this sweep.
...
Without an active `heart_beat`, a character stops receiving the periodic `heal_up()` ticks that regenerate limb health. Many combat/death systems in this project's libs gate reincarnation behind a health threshold (e.g. "your limbs must be at least 50% healed before you can `return` from the underworld") — if that gate is driven by `heal_up()`, and `heal_up()` is driven by `heart_beat`, then a character whose `heart_beat` never got restored after a netdead reconnect is **permanently stuck**: limb health frozen at whatever it was, the reincarnation gate never opens, no error anywhere, nothing crashes. This can affect ANY player who has ever had a single dropped connection, at ANY point after that reconnect, the next time they happen to die — an extremely wide, extremely delayed-onset blast radius for something this severe.

**Root cause note for whoever fixes this on another lib**: `set_heart_beat()`/`enable_commands()` are driver efuns with no cross-object form — they only affect `this_object()`. If the daemon handling the reconnect (`logind.lpc`) tries to call `set_heart_beat(1)` directly, it silently sets ITS OWN heart_beat, not the player character's, and appears to do nothing. The fix needs a small wrapper on the player-body object itself (e.g. `resume_heart_beat()` in `obj/user.lpc`) that the daemon calls via `->`.

**Verification technique**: the wizard `call` command's `query_heart_beat()` proved unreliable for testing this — `call_other()` to a function that doesn't exist on the target silently returns `0` rather than erroring, which can look identical to "heart_beat really is 0" and cost real debugging time. Use an in-game status command that reports the real heart_beat state (e.g. this project found `info`'s "心跳:N" field reliable) or an A/B `git stash`/re-test comparison instead of trusting a single `call` result.

**Corpus sweep in progress.** Refined candidate list: `find libs -iname logind.lpc | xargs grep -l net_dead` gives 62 libs whose `logind.lpc` has any netdead/reconnect awareness at all — like §7.112, each needs per-lib judgement (does that lib's ACTUAL reconnect path restore heart_beat somewhere, just not obviously?) rather than a blind mechanical sweep. `shzs` (the lib that found this bug) is already fixed. 2026-08-19 checked the next 15 alphabetically — **all 15 confirmed already clean, zero new fixes needed this batch**: `aoxiangtianji`, `dfgs2`, `dfgsiiv13b`, `dtsl2`, `dtxywzxzb`, `fyzfqyy`, `hell`, `hhsj`, `huoying`, `hxxtjqb`, `mhxy`, `mhxyqd`, `nitan170911`, `nitan6`, `nitan_ceshi`. In every one of these, `adm/daemons/logind.lpc`'s driver-invoked `reconnect(object ob, object user, int silent)` unconditionally calls `user->reconnect()` (verified live in the call graph, not dead code — reached from both the "same id already netdead" and "new connection matches netdead body" branches), and `obj/user.lpc`'s own `reconnect()` unconditionally does `set_heart_beat(1)` in the correct `this_object() == user` context. This is a distinct, consistently-correct lineage from `shzs`'s. 2026-08-19 batch 2 checked the remaining 45 (all of the prior "untested" list except `nt1`), front-loaded with the `sjsh`/`xiyouji`/`xlqy` neighborhood that seemed likeliest to share `shzs`'s bug: `sjsh`, `sjshv150`, `sjshv2578bb`, `sjshwzb`, `sjshwzjqb`, `sjcs`, `sanjieshenhua`, `xiyouji2003`, `xiyouji2006`, `xiaoyuxiyou`, `xixingzhanji`, `xlqy_early`, `xlqy_new2007`, `xlqyzdb`, `xianlvqiyuan`, `xyj2000`, `xyj2000f`, `xyj20032`, `xyj2006n`, `xyj2006zzzhx`, `nitan_san`, `nt6`, `nt6nitan6win`, `nte`, `ntii`, `rzrmud`, `shenmo`, `wxddym`, `xfbhh`, `xkm`, `xkxc98sj`, `xkxz2`, `yanhuangwuhun`, `yhwhpublicfi`, `yhyxs`, `yszz`, `yueyingqiyuan`, `zhonghua2`, `zhongjidiyu`, `zitengzhan`, `zjdy2008wzb`, `zjdyaryl`, `zjdywzb`, `zjdyzj`, `zzhj`. **All 45 confirmed already clean, zero new fixes needed this batch** — despite the expectation that the `sjsh`/`xiyouji`/`xlqy` neighborhood was likelier to share `shzs`'s bug, every one of them turned out to follow the same consistently-correct lineage as batch 1: the real `LOGIN_D`-defined `adm/daemons/logind.lpc`'s `reconnect(object ob, object user, int silent)` unconditionally calls `user->reconnect()` (confirmed as live, not dead code, by grepping the daemon's own call sites — every lib calls it from both the "already netdead, same id reconnecting" and "new connection matches a netdead body" branches), and the player body's own `reconnect()` (`obj/user.lpc` or `clone/user/user.lpc`, occasionally `feature/user.lpc`) unconditionally does `set_heart_beat(1)` (or an equivalent always-true tick constant) in the correct `this_object() == user` context. Verified via static call-graph trace only, per the batch-1-established rule that this is unambiguous — no live driver testing was needed. Caveats/gotchas hit along the way, for whoever does batch 3 on a new candidate list: (1) several of these libs have decoy `logind.lpc`/`user.lpc` copies scattered under wizard `/u/` home directories, old-version subfolders (`obj/old/`), or `adm/simul_efun/user.lpc` (an unrelated simul-efun helper, not the player class) — always confirm the REAL file via each lib's `LOGIN_D` `#define` in `include/globals.h` (or equivalent) before trusting a `find`/`grep` hit; (2) a couple of libs (`sjshwzb`, `sjshwzjqb`, `shenmo`, `zitengzhan`) have a second, non-canonical `logind.lpc` under `adm/daemons/network/` or a wizard sandbox that happens to contain byte-identical or look-alike code — harmless here since both copies were clean, but don't assume the first `find` hit is authoritative. This closes out the `find libs -iname logind.lpc | xargs grep -l net_dead` candidate list except `nt1`.

**§7.113 sweep CLOSED 2026-08-19.** `nt1` was the last candidate on the 62-lib list and, per its §7.110 OOM history, was checked with pure static reads only — no `lpcc`, no driver boot. Confirmed clean: `nt1`'s `LOGIN_D`-defined `adm/daemons/logind.lpc::reconnect()` calls `user->reconnect()`, and `clone/user/user.lpc::reconnect()` unconditionally does `set_heart_beat(1)` on the player body itself — same correct lineage as all other 60 checked libs. Final tally: **62 libs checked, 1 real bug found and fixed (`shzs`, the original discovery), 61 already correct.** No batch 3 needed unless a future full-corpus grep turns up additional candidates beyond this 62-lib list (e.g. libs whose `logind.lpc` restores heart_beat through a naming/architecture variant the `net_dead` grep didn't catch).
...
**Corpus sweep completed (2026-08-19).** The 4 candidates identified via direct code read (`zzhj`, `shzs`, `huoying`, `dfgsiiv13b` — each had `private void input_line(...)` in the `F_EDIT`-inherited mixin file) were all fixed (dropped `private`) and live-verified on a fresh driver boot with a fresh character: `zzhj` and `dfgsiiv13b`/`huoying` via a two-line board `post`/`to say` respectively, `shzs` via `to say` (no reachable board object or `chfn` in that archive). Each fix committed and pushed individually.

Broader search for the same shape under different names/locations, corpus-wide:
- A literal `private.*input_line` grep across every `.lpc`/`.c` file (excluding `work/data/`) turned up **zero** further hits beyond the 4 already fixed.
- All ~190 libs sharing this ES2/dfgs2-ancestry `feature/edit.lpc` or `feature/user/edit.lpc` mixin file (found via `find libs/*/work -path '*/feature/*edit*.lpc'`) were checked for a stray `private` anywhere in the file — clean except the 4 fixed instances. This confirms the bug was isolated to those 4 out of the ~190-lib lineage, not systemic to the mixin itself.
...
**Corpus-wide sweep completed (same session).** After a 4th independent instance (`mohuanshiji`), a full corpus grep/triage pass was run across every `bai.lpc`/`apprentice.lpc`/`recruit.lpc` in the corpus. 279 files matched the general shape; triaged down to 118 files across 74 libs with the exact dominant vulnerable form (`(string)me->query("family/family_name") != (string)ob->query("family/family_name")`, no preceding existence guard) — mechanically fixed identically to the 4 hand-verified instances, verified via paren-balance across all 118 plus `lpcc_check.sh` spot-checks on 3 unrelated lib families (all clean). 8 files were excluded from the mechanical pass and hand-reviewed separately: `fengyun434`/`fysjmb`/`xsfyssjb` (byte-identical siblings, a 3-clause `&&`-extended variant — the extra clause didn't actually protect the first-timer case either, since a fresh character's `family/title` is also unset; fixed the same way, 121 files/77 libs total for the whole §7.117 sweep) and `fy2005` (same 3-clause shape, but ALREADY correctly guarded elsewhere in that file — confirmed no fix needed). `xiakexing100`/`xkx100`'s `recruit.lpc` compares against a literal `"丐帮"` string gated behind an unrelated `ob->query("party")` check — a different mechanism, not this bug. `xo_final`'s `apprentice.lpc`/`recruit.lpc` already use a `stringp()` guard, equivalent protection, no fix needed. The other 161 of the original 279 candidates already had a correct guard and needed no change.
...
**A second, independent instance in `sunshadow` itself, found during its §10.7 round-two deep-test** (2026-08-27): `daemon/feat_d.lpc`'s `build_feat_list()` scans every category folder under the feats directory and applies the exact same stale arithmetic — `strsrch(files[y], ".lpc") != (strlen(files[y]) - 2)` — to decide whether a filename ends in `.lpc`, correct only for the original `.c` (2-char) extension. Since this check is a hard `continue` (skip) rather than a garbage-key corruption, the effect here was total exclusion rather than corruption: `__ALL_FEATS` ended up completely empty, `is_feat()` returned false for every feat in the game, and `add_feat()` silently no-op'd on every single `feats add`/`feats <type> <feat>` call while the UI still printed a fake "Congratulations, you have successfully added..." success message (the confirmation handler prints unconditionally, never checking `add_my_feat()`'s return value) — a `usable_feat()` gate elsewhere (`std/living.lpc`'s `cmd_hook()`, checking `strsrch(file, "/cmds/feats/") == 0`) then rejected every feat-gated command outright with the driver's own generic "Why?" fail message, indistinguishable from a nonexistent verb. This silently disabled the ENTIRE feat/combat-technique system for every class in the game (every martial/spellcraft/hybrid/arcana/divinebond/rage/pact/class/racial feat, including the automatic class-granted proficiencies every new character receives at creation) — confirmed live: a level-1 fighter's own automatically-granted "light/medium/heavy armor proficiency" etc. never appeared in `feats known`, and `feats martial sweepingblow` reported success but the `sweepingblow` command itself failed with the generic "Why?". Fixed identically to the established pattern: `strlen(files[y]) < 4 || files[y][strlen(files[y]) - 4..] != ".lpc"`. Fixing the scan then surfaced a second wave of ~25 previously-never-loaded individual feat files (`cmds/feats/**/*.lpc`) with their own independent, ordinary compile bugs (undefined `m_indices()` instead of `keys()`, `TYPE *a, b`-style declaration-binding typos, a misplaced-parenthesis `strsrch(x, "y" == -1)`, `object`-vs-`object*` mismatches, `varargs` arg-count-omitted calls, stray `return N;` in `void`-declared functions) that had never been reachable/compiled before because `build_feat_list()` always skipped every file in the directory — worth remembering generally: this bug class's "hard `continue`/skip" variant can mask an entire ADDITIONAL layer of unrelated bugs in the files it silently excludes, so fixing the scan itself is only step one; recompile everything the fixed scan now reaches.
...
### 7.121 A currency/economy function does real floating-point exchange-rate math but is declared `int` with no `to_int()` on its return — every buy/sell/exchange silently and permanently corrupts the player's money into a float

Found via a §10.7 deep-test playthrough of `dsIII` (Dead Souls 3.0): `secure/sefun/economy.lpc`'s `query_base_value()`/`query_value()`/`query_base_rate()`/`query_player_money()` compute a real floating-point exchange rate internally but are declared to return `int`, and never call `to_int()` before returning — unlike sibling functions in the SAME file (`currency_mass()`, `currency_value()`) that correctly do. Since `(int)` casts are compile-time-only on this driver (a distinct, already-documented gap — a declared `int` return type does not coerce a float value at runtime), every `buy`/`sell`/bank-`exchange` transaction silently converts the player's currency balance into a float that stays corrupted permanently in the save file (reproduced live: a shop purchase left `"silver":88.000000` in the raw save data, and an NPC's own balance-report message printed the float verbatim, "you only have 88.000000"). Two independent sibling call sites (`lib/teller.lpc`'s `eventExchange()`, `lib/props/value.lpc`'s `SetBaseCost()`) reintroduced the same corruption even after the root-cause function was fixed, since they also skip `to_int()` on the same underlying values — always check every call site that consumes a suspect function's return, not just the function itself.
...
Found on `sunshadow`'s §10.7 round-two deep-test: `adm/daemon/astronomy_d.lpc` declared `nosave mapping moons;` (plus `moonstate, moonphase, moonorbit`) then initialized all of them — plus two never-even-declared globals, `moonvisibility`/`moonillumination` — via bare top-level statements. The daemon never compiled, so `adm/simul_efun/total_light.lpc`'s `total_light()` (called from EVERY `look` in EVERY outdoor room, via `describe_current_room()`→`light_blind()`→`light_blind_remote()`) threw `*No program in object '/adm/daemon/astronomy_d'!` on its unconditional `ASTRONOMY_D->query_eclipse()` call, uncaught — silently blanking the ENTIRE room description (exits/items/mobs, everything past the failure point) for every `look` outdoors, game-wide, from the very first fresh boot. Three more independent instances found the same session, same shape: `cmds/mortal/_rumors.lpc` (`arealist`/`areas`, breaking the newbie-help-advertised `rumors` command entirely), `cmds/spells/e/_elemental_aura.lpc` and `_elemental_body_i.lpc` (`shortmap`/`colormap`/`elementmap`, breaking two mage/magus spells outright). **Fix**: merge the bare declaration and its later top-level assignment into a single proper declaration-with-initializer (`nosave mapping moons = ([...]);`), or — if the file already has a `create()` — move the assignment(s) into it; either is a mechanical, behavior-preserving rewrite. **How to detect across a corpus**: grep for a bare `^IDENT = (\[|\{|:` pattern at column-zero-ish indentation outside any function — most hits will be false positives (unindented code inside a function, or text inside a comment block), so hand-verify each by checking it isn't inside `{...}` before treating it as a real instance; a companion signal is a `nosave TYPE ident;` declaration with no initializer immediately followed (within a few lines, possibly across doc-comments) by a bare `ident = (...)` statement with no preceding type keyword.

### 7.122 A "class object" auto-reload mechanism (`compute_autoload_array()`/`destroy_autoload_obj()`/`load_autoload_obj()`) permanently duplicates every marker item a player carries, once per abrupt-disconnect-then-restore cycle, because the reload side never checks whether the item is already present

Found via a §10.7 deep-test playthrough of `mortremains` (TMI-2 1.4alpha). This mudlib's `std/user/save.lpc` header explicitly documents the intended design: `compute_autoload_array()` runs immediately before every `save_object()` to note which carried items (anything whose `query_auto_load()` returns truthy — class-membership markers, wizard tools, quest items, ~100+ files in this one archive) should be recreated fresh on next login, and `destroy_autoload_obj()` is supposed to strip those same items out of inventory at logout so they are never captured twice. The bug: `std/user.lpc`'s own quit/`remove()` paths call `save_data()` **before** `destroy_autoload_obj()` (one call site even has an explicit code comment confirming this order is deliberate — "First save the user's personal data. / Then destroy autoloading inventory."), and `net_dead()` (an abrupt-disconnect handler) calls `save_data()` and never calls `destroy_autoload_obj()` at all. On this driver, `save_object()` captures the carried item's data as ordinary inventory regardless, so the marker item ends up serialized into the save file in addition to being tracked in the separate `auto_load` list. On the player's next full restore, `restore_object()` recreates the item from the (redundant) inventory data, and `load_autoload_obj()` **unconditionally clones another one** from the `auto_load` list with no check for whether a matching item is already present. Each subsequent abrupt-disconnect/restore cycle adds one more copy — reproduced live on a real test character (`Bramwell`) that accumulated 5 copies of "A Pit Fighter's Scars" (a `prevent_drop`/`prevent_sell` class marker) purely from ordinary disconnect/reconnect testing, confirmed both via the raw save file and a live `i`.

This is a crash-free, boot-invisible, compile-clean bug — nothing about it shows up in `lpcc_check.sh`, a clean boot, or even a single continuous session; it only surfaces across multiple real disconnect/reconnect cycles, exactly the kind of thing `debug.log` never flags. **Fix applied** (targeting the reload side rather than reordering the save/destroy calls, since reordering risks losing the item entirely for a player who reconnects via a live/interactive path that skips `load_autoload_obj()`): `load_autoload_obj()` in `std/user/autoload.lpc` now scans the player's current inventory first and skips cloning any `auto_load` entry whose `base_name()` already matches an object already carried, making the reload idempotent regardless of how many times it fires or how the save file got into a redundant state. Verified live: after the fix, a further clean quit-and-relogin cycle on the already-corrupted `Bramwell` character held steady at 5 copies (did not grow to 6), confirming no further duplication going forward; the fix does not retroactively clean up saves already corrupted before it landed.

**How to apply generally**: this exact `auto_load`/`compute_autoload_array`/`destroy_autoload_obj`/`load_autoload_obj` mechanism is verbatim TMI-2 lineage code (this file's own header credits "Truilkan@TMI"), so any other TMI-2-descended mudlib in this corpus is a plausible sibling to check — grep for `query_auto_load` and `load_autoload_obj` and confirm the reload loop guards against an item already being present before cloning a fresh copy. More generally, on any archive with a "regenerate these items on login" mechanism separate from normal inventory restore, check whether the two mechanisms can ever both fire for the same item, and prefer fixing it with an idempotency guard on the regenerating side rather than reordering when the complementary cleanup runs, since reordering a save/cleanup pair can trade a duplication bug for a data-loss bug if any code path reaches the reload step without ever reaching the cleanup step first.
...
**Confirmed and fixed on `es1_win` (2026-08-28)**: exactly as predicted — byte-for-byte the same vulnerable `autoload.lpc` (same TMI header), same triggering ordering (`remove()`'s `save_me()`-before-`destroy_autoload_obj()`, `net_dead()` never calling it at all). Fixed by porting `es1`'s exact idempotency-guard remedy verbatim. Verified via a clean native driver boot only (not an independent live net_dead reproduction, since the remedy itself was already live-proven on the ancestor codebase) — see `libs/es1_win/NOTES.md` for detail.
...
**Sibling check closed (2026-08-28)**: a corpus-wide grep for the literal `__AutoLoad` marker found zero hits anywhere in `nightmare4`, `residuum`, `dsI`, `dsII`, `dsIII`, `ds386`, `dshakkard`, or `deadsouls_fluffos` — none of these 8 flagged siblings has this specific mechanism (by any filename) at all. `nightmare4` is a distinct-enough later generation of the Nightmare Object Library (its own separate example domain and object layer, per its own README) that it evidently never carried this particular reload idiom forward, and the Dead Souls lineage's own persistence design is architecturally unrelated. No fix needed anywhere; this closes the sibling list with an honest "not applicable" verdict rather than leaving it open indefinitely.
...
**Sibling sweep closed (2026-08-28)**: checked all 4 flagged libs directly. `yanhuangwuhun`, `yhyxs`, and `yhwhpublicfi` all carried the byte-identical bug (same clothing-handout code, same commented-out `//user->set("born",1)` line) and each has its own correct `d/register/npc/shuisheng.lpc` `do_decide()` already setting the flag properly — fixed all three by deleting the premature `enter_world()` line, verified via a clean native driver boot on each. `yhwhckdm` is `wasm_status: "not-convertible"` (a reference-code-only archive with no `work/` tree) — not applicable, correctly out of scope.

### 7.126 A coordinate-AREA's room-exit/area-exit doors are stored in save-file DATA as a macro-placeholder string (`"__DIR__foo.c"`) resolved at runtime by a custom `file_path()` helper — and that stored value still carries the pre-conversion `.c` extension, so `load_object()` silently fails on every such door after this project's own `.c`→`.lpc` rename

Found via a §10.7 round-two deep-test playthrough of `naruto` (ES2/Neolith lineage). This engine's coordinate-grid `AREA` objects (`std/area/map.lpc`) store each door tile's destination as `room_exit`/`area_exit` values inside the AREA's own persisted dbase (a `.o` save file, e.g. `world/area/wizard/wizard.o`) — but instead of a real path, the stored string is a macro-style placeholder (`"__DIR__transfer.c"`), meant to be expanded at *read time* by the AREA's own `file_path()` method (`replace_string(dir, "__DIR__", query("file_path"))`, plus similar substitutions for `_DIR_AREA_`/`_DIR_NPC_`/etc.) before being handed to `load_object()`. This was a legal, working design on the original MudOS-era server (whose extension resolution treated `.c`/`.lpc` interchangeably) but this driver resolves an explicit extension exactly — since these placeholder strings still end in the archive's *original* `.c` extension from before this project's own `.c`→`.lpc` conversion pass, `file_path()` faithfully resolves `"__DIR__transfer.c"` to `/world/area/wizard/transfer.c`, a file that no longer exists (it's `transfer.lpc` now), and `load_object()` silently returns 0. `valid_leave()`'s caller then reports a generic "this exit has a problem, notify the admins" message with no hint the real cause is a stale extension.

This is a distinct shape from §4.2 item 2 ("bare paths in plain-text data files") — that class is a literal, static full path written directly into a config-style listing file; this one is a macro placeholder *reconstructed* at runtime by a lib-specific substitution function, which is exactly why `convert_lib.sh`'s source-only `.c`-reference fixups never touched it (`.o` save data is out of scope for that sweep) and why it survives even a careful single-room registration/`look` test — only a door tile that's actually walked onto surfaces it, and on `naruto` the *specific* door that happened to break was the free `transfer` teleport room, the game's own documented sole "level 1 is free" route out of the tutorial hub, so the practical effect was that the ENTIRE hand-built game world beyond the tutorial area's plain-string room exits was unreachable from a fresh registration. A corpus grep across every `world/area/**/*.o` file in this one lib found **210 affected save files / ~969 stale `.c` references**.

**Fix** — patch the single choke point both `room_exit` and `area_exit` resolution flow through (the AREA's own `file_path()` helper), stripping a trailing bare `.c` after the macro substitutions:
```lpc
if( strlen(dir) > 2 && dir[strlen(dir)-2..] == ".c" )
    dir = dir[0..strlen(dir)-3];
```
This transparently fixes every affected `.o` file without touching any of them directly (a `.lpc`-suffixed value is never mismatched — its last two characters are `pc`, never `.c`). Verified live: `load_object()` on the resolved `transfer` room path now succeeds (confirmed via a direct `eval`), and a test character walking through a previously-broken mapping-shaped exit now correctly lands at the exact coordinate the source room's exit specifies, reaching a real village area and completing a village-join and safe-sparring-combat test that were previously unreachable.
...
**Sibling sweep closed (2026-08-28)**: checked all 6 flagged libs directly for the vulnerable `std/area/map.lpc`-style coordinate-grid framework. `zhyx`/`yanhuangwuhun`/`demonangel`/`es2` have no such framework at all (ordinary discrete rooms, per `haiyang2`'s own already-documented "not applicable" verdict, confirmed the same reasoning holds for these too). `huoying` DOES have it, with the identical unfixed `file_path()` shape — but a much smaller footprint than `naruto` (2 total `.o` save files under `world/area/`, 1 stale `__DIR__...c"` reference vs. `naruto`'s 210/~969). Fixed with the identical choke-point strip regardless of the small blast radius (the fix is a no-op for already-`.lpc`-suffixed values). This closes the sibling list — `naruto` and `huoying` are now the only two confirmed/fixed instances of this bug class in the corpus.
...
**Confirmed instance with an added nuance, `finalrealms`'s own §10.7 round-two deep functional test**: same exact shape (`global/player.lpc`'s `heart_beat()` had a stray `last_command = time() - query_idle(this_object());` left reachable inside the `!interactive(this_object())` branch after an old inline idle-kick `else` block was commented out around it, rather than after an if/else pair) — but guarding it with only `if (interactive(this_object())) ...` (the fix shape above) was NOT sufficient here and needed a second guard. This lib's own non-fighting `quit()` path calls `really_quit()` SYNCHRONOUSLY (immediate `dest_me()`, not a delayed `call_out()`), so the vulnerable line can be reached in the SAME `heart_beat()` tick as `this_object()`'s own destruction. Empirically, `this_object()` reads back as `int 0` (not a dangling/stale object reference) once the current object has actually been destructed earlier in the same call — and `interactive(0)` itself THROWS ("Bad argument 1 to interactive() Expected: object Got: 0") rather than returning false, reproducing the identical class of crash one line downstream of the original fix. **Fix**: add an `objectp()` guard ahead of the `interactive()` check: `if (objectp(this_object()) && interactive(this_object())) ...`. Verified live via a real abrupt-disconnect repro (raw socket close, no `quit`) left running across multiple heartbeat ticks: zero `query_idle`/`interactive` errors in `log/errors/*.err`, and the netdead body confirmed reaped (`who` no longer lists it) rather than lingering as an ever-crashing ghost. **How to apply generally, extending the note above**: when the non-interactive branch's cleanup can ALSO complete synchronously within the same tick (not just via a delayed `call_out()`), guard the vulnerable trailing code with `objectp(this_object())` in addition to `interactive(this_object())` — `this_object()` is not guaranteed to still evaluate to a real object reference after `dest_me()` has already run earlier in the same call.
...
**Confirmed sibling shape, unrelated file, `finalrealms`'s §10.7 round-two deep functional test**: `obj/handlers/align_tracker.lpc` declares `int *align_array = ({ });` (an empty array, not merely unset) and only ever appends to it inside a narrow, time-gated `if` block deep in `check_player_al()` — but the function's own final line, `return align_array[0];`, runs UNCONDITIONALLY regardless of whether that `if` block ever actually populated the array. On a fresh boot with no pre-existing `/save/aligntracker` save file (or any character too young for the timing gate to have fired yet — the common case for exactly the newly-registered characters a §10.7 pass tests), `align_array` stays empty and this throws "Array index out of bounds" on literally EVERY `adjust_align()` call, i.e. every alignment-affecting combat hit, project-wide from the moment of a fresh boot. **Fix**: `return sizeof(align_array) ? align_array[0] : 0;` — safe because the sole caller (`std/living/alignment.lpc`'s `adjust_align()`) already discards this function's return value entirely, calling it purely for its populate-the-array side effect. Verified live: the identical combat sequence that previously crashed (logged to `log/errors/obj.err`) now runs clean through several real hits with zero errors. **Distinguishing tell for this variant vs. the main entry above**: the base declaration already has an initializer (`({ })`, not a bare declaration) — the bug isn't a missing initializer, it's an unconditional final-line index into a collection that a narrow conditional block is the ONLY thing that ever grows past empty.
...
### 7.144 A generic, reusable NPC base object self-names in its own `setup()`/`create()`, permanently locking out every later per-instance rename attempt, because `set_name()` is a one-shot ("already named, ignore") setter — silently breaking `find_match()`/`find_living()` lookups for whatever identity callers tried to give it instead

Found via `finalrealms`'s §10.7 round-two deep functional test, in the death/resurrection flow. `baseobs/monsters/healer.lpc` is a shared, generic "priest" NPC base — `obj/monster.lpc`'s `set_name()` is a one-shot setter (`if (name && name != "object") return;`), and `healer.lpc`'s own `setup()` unconditionally calls `set_name("james")` at clone time. ~20 different per-race "raiseroom" objects clone this SAME base object once (as a persistent, named nurse/priest NPC, not the disposable per-transaction healer instance) and then try to give it their own room-specific identity via `helper->set_name("june")` (or "Alana", "Tara", "shalagha", etc.) inside their own `reset()` — but since `name` is already "james" by the time `reset()` runs, every one of these rename calls is a complete no-op: the object's real internal `name` (and its `set_living_name()` registration) stays "james" forever, even though a separate direct `set_short("June")` call cosmetically changes what `look`/room descriptions display. The result: `find_match(nurs, this_object())`/`find_living(name)` lookups keyed on the INTENDED identity ("june") permanently fail, even though the NPC is visibly standing right there under that display name. In this lib, that broke the entire "raise me" resurrection mechanic project-wide (12+ of ~20 race raiserooms use a name other than the base object's own default) with zero crash and zero `debug.log` signal — a `raise <target>`-shaped command just silently no-ops, `command()` returning 0 with the caller's own `notify_fail("...not here to help you...")` message never actually reaching the player (see the companion, not-fully-root-caused finding in `finalrealms`'s own NOTES.md about native `add_action`-registered failures going silent in that lib's custom command-queue architecture — a separate, compounding issue that made this one even harder to diagnose live).

**Fix**: don't self-name the shared base object in its own `setup()`/`create()` — let every real caller supply its own identity explicitly. `baseobs/monsters/healer.lpc`'s `setup()` no longer calls `set_name("james")` (kept `set_short("james")`, purely cosmetic); the ONE caller that used the object disposably without ever renaming it (a temporary priest clone inside the shop-adjacent `do_raise()` flow) now explicitly calls `priest->set_name("james")` itself, preserving that caller's original behavior. Every OTHER caller's own `reset()`-driven `helper->set_name(<real name>)` now succeeds on its very first call, since `name` is genuinely unset at that point.

**How to apply generally**: any shared/generic NPC/item base class meant to be cloned-and-customized per site (a "template" object, not a fixed unique character) is suspect if its own `setup()`/`create()` calls `set_name()` (or any other one-shot "first call wins" setter) with a hardcoded default AND at least one real call site later tries to override that same property on the clone — grep the base class for its own default-identity call, then grep every `add_clone`/`clone_object`/`new()` site cloning it for a subsequent rename attempt on the SAME property. A `set_short()`/`set_long()` cosmetic override succeeding while the underlying `name`/`set_living_name()` registration silently stays at the base class's own default is the exact tell — verify with a direct `find_living()`/`find_match()` lookup against the INTENDED name, not just a visual `look`.

### 7.145 A room's own custom `add_action` registration for a verb its inherited shop/vendor base class ALREADY implements is meant as a lightweight availability pre-check (with a real, dedicated integration point for exactly that purpose), but is wired as a direct verb handler instead — since a subclass's own `init()` registrations are tried before an inherited base class's same-verb registration, this completely shadows and swallows the real `buy()`/`sell()`, silently no-opping every transaction

Found via `finalrealms`'s §10.7 round-two deep functional test, live-testing a race-zone general store. `std/shop.lpc` (this lib's shared shop/vendor base class) implements real `buy()`/`sell()`/`list()`/`value()`/`browse()` via its own `add_action(...)` calls in its base `init()`, AND exposes a dedicated `set_open_condition(mixed)` API (consulted internally by every one of those functions via a shared `test_open()` gate) specifically so a subclass can add a cheap "is anyone here to serve you" precondition without touching the real transaction logic. At least 6 of this lib's ~20 race-zone shop rooms instead wrote a small `do_check()` helper matching that EXACT 0/1 contract (`if (!present(<shopkeeper>)) { notify_fail(...); return 0; } return 1;`) but wired it directly: `add_action("do_check", "buy"); add_action("do_check", "sell");` in their OWN `init()`, called AFTER the inherited `::init()`. Since this driver's `add_action()` tries the MOST RECENTLY registered sentence for a given verb first, `do_check()` intercepts "buy"/"sell" completely — and because it unconditionally `return(1)`s whenever the shopkeeper is merely present (with no notion of "and now actually run the transaction"), every `buy`/`sell` silently no-ops: `command()` reports success, nothing is exchanged, and no output is shown at all. The room's own source even has the CORRECT line sitting right there, commented out (`//set_open_condition("do_check");`), immediately above the broken `add_action` pair — a leftover from whichever refactor introduced the bug. One affected room compounded this with a second, independent bug: its `do_check()`'s `present(<Displayed Short Name>)` check used the NPC's capitalized display name ("Old woman") rather than its actual lowercase `id()`/`name` ("old woman") — `id()` on this driver is a plain case-sensitive `==` string compare, so even after correctly wiring `set_open_condition()`, the shop reported "not open" for every command (including the previously-unaffected `list`/`value`/`browse`, which don't go through `do_check` directly but DO check the shared `test_open()` gate) until the literal string was corrected to match the NPC's real `id()`.

**Fix**: delete the shadowing `add_action("do_check", verb)` pair from the subclass's own `init()`, and uncomment/restore the already-present `set_open_condition("do_check")` call in `setup()`/`create()` — a mechanical, behavior-preserving swap back onto the base class's own documented integration point. Where a room's `do_check()`-style check also uses a NPC name string, verify it against that NPC's actual `set_name()`/`id()` value (not its `set_short()` display text) before trusting it. Verified live end-to-end: `list`/`value`/`buy`/`sell` all correctly ran the real underlying transaction logic post-fix (item stock and player purse both updated correctly), where before every `buy`/`sell` was a silent, zero-effect no-op.

**How to apply generally**: any shop/vendor/interactive-service base class that both (a) registers its own real verb handlers via `add_action()` in a base `init()` and (b) exposes a separate, named "is available/open" hook meant to be set via a dedicated setter (`set_open_condition()`-shaped) is worth checking every subclass for a redundant `add_action` registration of the SAME verb names in the subclass's own `init()` — especially one whose handler function's own 0/1-return shape looks suspiciously like it was WRITTEN for the dedicated hook (a `notify_fail()`-then-`return 0` / bare `return 1` pattern with no actual transaction logic) but got wired as a direct verb override instead. A commented-out call to the correct dedicated setter sitting right next to the broken `add_action` pair, as seen here, is a strong tell that this is exactly what happened.

### 7.146 A single stray `/` where `/*` was clearly intended (the second star of the comment-open token dropped) is not a comment at all on this driver — it's a syntax error, but ONLY at the specific call site where nothing after it happens to parse as a harmless comma-expression, so the exact same typo shape is silently swallowed at some sites and hard-fails the whole file's compile at others

Found via `finalrealms`'s §10.7 round-two deep functional test. Several files in this archive have a same-line trailing "comment" of the form `some_call(...); / explanatory text here */` — clearly meant to be `/* explanatory text here */`, missing the opening `*`. Whether this compiles depends entirely on what the stray `/` and the following words parse AS, since LPC has no single-line `//`-vs-`/*` ambiguity here — a bare `/` starts a division expression, not a comment:
- `obj/misc/torch.lpc`'s `init()` had `/* Wow.. */\n  / Will try add_action \n  this_player()->add_command(...);\n  ...\n  */` — the stray `/` is immediately followed by an identifier-like word and then a full statement, which cannot parse as any valid expression: **hard compile error** ("syntax error, unexpected '/'"), taking down the whole file (and, transitively, everything that `inherit`s or `clone_object()`s it — in this case, every "starting room hands out a torch" flow project-wide, since `/baseobs/misc/torch.lpc` inherits this file).
- `obj/misc/book.lpc`'s `init()` had six lines shaped like `add_action ("open","open"); / open the book */` — same missing star, but here the stray `/` sits directly after a statement-ending `;`, so `/ open the book */` (three bare identifier-shaped tokens divided by nothing valid) also fails to parse as an expression: **hard compile error** on this file too.
- `d/heaven/heaven/meeting.lpc` and `room/admin/site_control.lpc` had the identical shape on a single `add_action(...); / comment */` line each — same hard compile error.

**How to detect**: grep for a bare `/` immediately followed by ordinary lowercase prose words (not `/=`, not `//`, not preceded by a numeric/variable operand that would make it a real division) — `grep -n '); / [a-z]' **/*.lpc` is a decent first pass, since the vast majality of real instances of this typo follow a `;`-terminated statement on the same line. Every confirmed instance in this archive was a same-line trailing annotation someone meant to wrap in `/* ... */`, dropped the star, on an ordinary `add_action(...)` call.

**Fix**: mechanical — restore the missing `*` (`/ text` → `/* text`, keeping the file's own already-correct closing `*/`). Zero behavioral change once it compiles; purely restores a pre-existing, never-actually-working comment annotation. Verified via `lpcc`/live boot: all 4 instances compile clean post-fix, and the previously-uncompilable `obj/misc/torch.lpc`/`obj/misc/book.lpc` are confirmed reachable again (torch cloning succeeds; `book.lpc` itself was otherwise unreferenced content in this archive, fixed for completeness since the same typo shape was already being swept).

**Caution — do not assume every instance of this shape fails to compile**: a stray `/` landing where the surrounding tokens DO happen to form a valid (if nonsensical) comma-expression or division will silently compile as dead/no-op code with no error at all, exactly like a normal comment would have looked to the author — this makes a batch `lpcc`-only sweep insufficient on its own; grep for the shape directly rather than relying purely on compile failures to find every instance.

### 7.147 A documented "returns 0 if the item can't be found" content-factory function is called in an unguarded `factory(...)->move(target)` chain at dozens of independent call sites across a codebase, so ANY one of them crashes outright the moment its requested item is missing from the archive — instead of degrading gracefully like the factory itself was written to allow

Found via `discworld`'s §10.7 round-two deep-test. `obj/handlers/armoury.lpc`'s `request_item(word, percent, area)` is a well-documented content factory (`@return` doc comment and inline example both present) that explicitly `return 0;` when the named weapon/armour/clothing/etc. isn't registered in any of its internal lists — a deliberate, safe-by-design contract for exactly the case where an archive ships without some content. But nothing enforces that CALLERS respect that contract: the idiomatic call shape used throughout this codebase is a direct chain, `ARMOURY->request_item("some item", N)->move(target);`, with no intermediate variable and no null check. When the archive genuinely lacks the named item (here: two entire missing content directories, `/obj/clothes/` and `/obj/armours/`, already a known gap in this lib), `request_item()` correctly returns `0` exactly as designed — and `0->move(target)` throws `*Bad argument 1 to call_other()`, uncaught, aborting the calling `create()`/`setup()` mid-execution. Live-reproduced on the archive's own advertised newbie combat-training dummy (`d/liaison/NEWBIE/dummy.lpc`'s `setup()`, requesting "dirty rags"): every attempt to enter the training booth crashed with a player-visible "A runtime error occurred", because the dummy's `setup()` never got past the missing-item request. Four more call sites with the identical unguarded shape were found by grepping the whole codebase for the same chain pattern and fixed proactively: the training room's own instructor NPC (`d/liaison/NEWBIE/trainer.lpc`), two real wandering "lost adventurer" NPCs (`obj/monster/godmother/{magrat,granny}.lpc`), and a shop NPC (`d/dist/pumpkin/rabbit/print_shop_office.lpc`) — two of the five (the godmother NPCs) were confirmed live-crashing during this same test session via an independent in-game trigger, not just grep-predicted.
...
Found via `discworld`'s §10.7 round-two deep-test, while single-file `lpcc`-checking an unrelated fix. `std/shops/print_shop.lpc` declares `protected int add_auto_load_info(string nosave, string dynamic);` (both the forward declaration and the real definition) — using `nosave` as an ordinary parameter name. On this driver, `nosave` and `static` are both real `L_TYPE_MODIFIER` tokens in the lexer (`~/src/fluffos/src/compiler/internal/lexer_utils.cc`: `{"nosave", L_TYPE_MODIFIER, DECL_NOSAVE}`, `{"static", L_TYPE_MODIFIER, DECL_NOSAVE | DECL_PROTECTED}`), not plain identifiers — so the parser sees a type-modifier keyword in parameter-name position and reports `error: syntax error, unexpected L_TYPE_MODIFIER`, hard-failing the whole file's compile. The function's own doc comment (`@param static_arg the nosave auto load information`) suggests the author's own intended name was `static_arg`, not the bare `nosave` actually typed — a plausible one-off typo rather than a deliberate choice, and unlike LDMud/MudOS-era drivers where these words are contextual and fine as identifiers outside a type position, this driver's lexer does not disambiguate by position. Because the failing file was a `std/` base class inherited by four real content rooms (a whole "print shop" zone), the failure was total and silent from the player's perspective — no player-facing symptom at all beyond that entire zone's rooms never having existed as playable content since this project's onboarding, an outcome indistinguishable from "this content was never finished," confirmed via `log/error-log.old` showing the identical parse error recorded on every boot since conversion, never previously investigated.

**Fix**: rename the offending parameter (here, `nosave` → `nosave_arg`) at every occurrence — the forward declaration, the real definition, and any lambda/closure capture referencing it by name (`$(nosave)` → `$(nosave_arg)`) — a mechanical, behavior-preserving rename. Verified via `lpcc <config> <file>` single-file compile: the base class and its real dependent both failed before the rename and both compiled clean after.
