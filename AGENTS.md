...
**Remaining work**: only `nt1` is left from the original candidate list, and it is deliberately unprocessed — §7.110 documents a live driver boot on `nt1` once consuming 22GB+ RAM and crashing the host, so it needs `lpcc`-only static verification or a memory-capped boot, not a normal live-driver check. No batch 3 is needed for §7.113 unless `nt1` is picked up under that special handling, or unless a future full-corpus grep turns up additional candidates beyond the original 62-lib list (e.g. libs whose `logind.lpc` restores heart_beat through a naming/architecture variant the `net_dead` grep didn't catch).
