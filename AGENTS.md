...
### 7.112 An NPC's `init()` unconditionally scheduling a `call_out()` chain gets a duplicate stacked on top of it every time the player it's tracking simply reconnects — because `enable_commands()` re-broadcasts `init()`, and every ordinary reconnect calls `enable_commands()` — corpus-wide sweep completed, 137+ libs fixed
...
**Corpus-wide sweep completed** (2026-08-18): once the pattern's scale became clear, a full grep for the `d/death/npc/wgargoyle.lpc` gargoyle lineage alone found it present in **154 libs**; 11 were already fixed via round-three discovery by that point, leaving 130 (later corrected to include one accidentally-omitted lib, `ntii`, caught by a final corpus-wide verification pass) that were swept in 8 sequential batches of ~15-19 libs each, dispatched to fresh subagents with the proven fix shape and reference implementations, each batch spot-verified via standalone `lpcc` compiles or live driver boots before committing. **Final state, verified by a script re-scanning every `wgargoyle.lpc` in the corpus for the guard string**: 137+ libs fixed (134 in the `wgargoyle.lpc`-named lineage alone, plus the original 3 `dtsl`-family `yanluo.lpc` libs), with exactly 7 libs' `wgargoyle.lpc` files intentionally left unguarded and individually documented as non-issues: `xkx100`/`xiakexing100` (already deduplicate via `remove_call_out()`), `yhwhpublicfi`/`zhonghua2`/`yszz`/`zitengzhan` (that specific file is dead/unreferenced code — the REAL live NPC is elsewhere, often renamed `bai.lpc`/`hei.lpc`, and was fixed separately), and `xbtxiii` (that file has an unrelated pre-existing syntax error that prevents it from ever compiling/loading at all, making the `call_out` unreachable regardless — flagged but not fixed, out of scope for this bug).

**Lessons for any future recurrence of this pattern on a newly-added lib**: (1) this is NOT mechanically uniform like §7.111's single-line swap — every file needs its own control-flow read, since `final_death_stage()` branches, extra item-drop/cloth-wearing logic, and dual live-copy directories (e.g. `d/xiyou/death/npc/`, `d/suburb/<x>/death/npc/`, `d/death22/npc/`) vary per lib; (2) always check for a renamed live NPC (`bai.lpc`/`hei.lpc` instead of `wgargoyle.lpc`/`bgargoyle.lpc`) before concluding a lib has no vulnerable code; (3) `lpcc`'s standalone compile mode needs paths relative to the lib's own `work/` directory (cd into `libs/<lib>` first) — a `work/`-prefixed path from the repo root produces a spurious universal "Fail to load object" error unrelated to any real bug; (4) this remains a **standing checklist item for every round-three §10.7 test on any lib not yet covered by this sweep**: grep `call_out(` inside any `init()` function, not just death/reincarnation-themed directories.
