...
| **B. Catalog overlay** | Stopgap only. Modern LDMud 3.x (`set_driver_hook`, `#'` / `unbound_lambda`, `virtual inherit`, MessageLib, XML cmds). **Does not load the real world.** User 2026-09-05: these games must become Strategy A (fully playable content), not stay as Void/workshop. Order: `ageofelements` first, then `sticklib`, `simud`, `morgengrauen`, `aoh`. `acme` has no world in the archive. | `acme` (framework only), and the 974–978 overlays until each is fully ported |
