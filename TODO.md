...
- **PROJECT COMPLETE: 91 archives done, 8 confirmed not-mudlib, 4
  intentionally deprioritized per the user's English-language policy --
  every one of the 103 numbered archives now has a terminal status.
  Zero "not started" rows remain.**
- **Done: 91 / 100** (shanhaizhanshen, xingzhanyingxiong,
  ...
  mohuanshiji [魔幻世纪, ES II/"神话西游" lineage -- THE FINAL ARCHIVE OF
  THE PROJECT])
...
| 4 | bxsj.rar | bxsj | 40004 | done | 书剑天下; found 3 new bugs incl. the this_player()-override footgun (§8c); RETROACTIVE FIX (2026-07-23, found via archive #90 jinyongwenzi -- the literal same codebase, processed much later after §15ae was discovered): this lib was completely command-dead after registration (private command_hook + a commandd.lpc sscanf "%s.c$" pattern that never matched anything post-rename, two independent causes of the same symptom) -- both fixed, re-verified with a fresh registration + look/score/quit all working, zero debug.log errors; see libs/bxsj/NOTES.md; 2026-07-23 rebuild+format+WASM pass: reformatted, native retest clean, WASM boots but login blocked by sited.lpc's query_ip_number() format check (the AGENTS.md example case) |
| 5 | bxsj1.rar | bxsj1 | 40005 | done | 书剑·经典, same lineage as #4; 3 known fixes applied proactively; RETROACTIVE FIX (2026-07-23, same as #4 above, identical two bugs in the shared codebase): private command_hook + commandd.lpc sscanf pattern both fixed, re-verified with a fresh registration + look/score/quit all working, zero debug.log errors; see libs/bxsj1/NOTES.md; 2026-07-23 rebuild+format+WASM pass: reformatted, native retest clean, WASM boots but login blocked by shared sited.lpc query_ip_number() limitation (same as bxsj) |
| 6 | chidi.rar | chidi | 40006 | done | 江湖I; found tail() missing-efun + include-angle-bracket bugs (§8e, convert_lib.sh fix); see libs/chidi/NOTES.md; 2026-07-23 rebuild+format+WASM pass: found+fixed a NEW bug (master.lpc's log_error() showed raw compiler diagnostics to any player, §15af pattern, missed until this pass's full retest), reformatted, native retest clean after fix, WASM boots but login blocked by the query_ip_number() limitation |
...
| 8 | dtsl.rar | dtsl | 40008 | done | 大唐双龙传, lib #1 lineage, booted with zero fixes; found F_UNIQUE macro gap + diamond-inherit issue; see libs/dtsl/NOTES.md; 2026-07-23 rebuild+format+WASM pass: reformatted, native retest clean, WASM fully works end-to-end |
...
| 18 | LLMUD(大唐双龙)v_0.11版.rar | llmud_datangshuanglong | 40015 | done | same lineage as dtsl(#8), zero boot fixes needed; 2026-07-23 rebuild+format+WASM pass: reformatted, native retest clean (§15s fix intact), WASM works (IP-check cosmetic-only here, not gating); see libs/llmud_datangshuanglong/NOTES.md |
...
| 26 | xlqy_new2007.rar | xlqy_new2007 | 40022 | done | 新仙侣情缘之飘渺纪元, ES II lineage; found + fixed an extract.sh bug (relative .rar paths silently failed) + the recurring convertd.lpc Greek-table backslash typo (§8h); 98.6% lpcc pass after fix; one non-fatal unexplained runtime error during login noted but not chased; see libs/xlqy_new2007/NOTES.md; 2026-07-23 rebuild+format+WASM pass: found+fixed 1 formatter-corrupted file (§: : bug), native retest clean after fix, WASM fully works |
| 27 | xlqy-解压看readme.rar | xlqy_early | 40076 | done | same 仙侣情缘 codebase/authorship as xlqy_new2007(#26) confirmed via diff (master.lpc/chinese.lpc are pure whitespace/brace reformatting of the same content, but logind.lpc/chinesed.lpc differ substantially) -- a genuinely earlier, rougher development snapshot matching the archive's own "very incomplete, for driver testing" disclaimer, not a duplicate; standard §15h fix (ported from #26) + §14/§8d-§15o(was entirely absent)/§15w/§8h/§15t fixes + eventd.lpc rename-width bug + uppercase .C rename (3 files) + missing save-data directories created (data/{login,user,npc,fabao,gold,pet}/{a-z}/ + log/nosave/, none existed and save_object() does a bare fopen() with no mkdir) + NEW __FILE__-in-#include-not-inherit bug (workroom.h expanded to its own path not the includer's, 100+ runtime errors/session) + NEW same-file forward-reference silently binding to an inherited function instead of the local override (a bare forward decl was insufficient, had to move the real body earlier) + 3 undeclared-variable bugs in cndd.lpc; full registration + post-login-command flow verified in 4 independent sessions with real names 秦风/秦岭/秦天/秦水, each confirmed via score+quit; notable: this archive's manmade_npcd spawns 100 autonomous NPCs lazily compiling the whole ~9,177-file map on boot, driving CPU to 75-85% for minutes -- a genuine intentional feature matching the readme, not a bug; 98.8% lpcc pass (9070/9177); see libs/xlqy_early/NOTES.md; 2026-07-23 rebuild+format+WASM pass: reformatted, native retest clean, WASM fully works (cosmetic IP-display glitch only) |
| 28 | xo.zip | xo | 40023 | done | 笑傲江湖迷你版, TMI-2/ES2 lineage (Falcon), secure/daemon/ layout; found a case-sensitivity #include bug (Action.h vs action.h, new §15g) that was causing 209/1395 lpcc failures, fixed to 72; applied §4's security-daemon reentrancy guard proactively; see libs/xo/NOTES.md; 2026-07-23 rebuild+format+WASM pass: reformatted, native retest achieved a full playthrough for the first time (秦风丁), WASM reaches gender prompt then world-entry hangs (reproducible, not the IP-check issue, doesn't repro on sibling xo_final -- flagged, not patched) |
| 29 | xo最终版1.2.rar | xo_final | 40024 | done | same TMI-2/ES2 lineage as xo(#28), full "final" build (~7,174 files vs xo's 1395); found the "comment eats next line" typo (2x) + ~8 lossy-conversion corrupted string literals; full registration flow verified incl. real Chinese name "赵云" reaching the password prompt; see libs/xo_final/NOTES.md; 2026-07-23 rebuild+format+WASM pass: found+fixed 1 formatter-corrupted file, native retest clean (full reg to look/score/quit), WASM fully works |
...
| 33 | 东方故事二.rar | dongfanggushi2 | 40027 | done | dup: 东方故事二 (1).rar; "东方故事Ⅱ之天朝帝国", ES II lineage but a distinct/smaller codebase than es1_win/esI despite same adm/obj/ layout; found a NEW driver-crash bug (master.lpc destructing SIMUL_EFUN_OB segfaults on boot, §15l) + §4 fix on both valid_write/valid_read + §15h fix incl. a new sliding-window sub-variant; unusually long registration chain (id→confirm→password→email→race→gender→Chinese name) verified end-to-end through a real Chinese name "萧峰" all the way into an actual game room; 85.9% lpcc pass; see libs/dongfanggushi2/NOTES.md; 2026-07-23 rebuild+format+WASM pass: reformatted, native retest clean, WASM fully works end-to-end |
...
| 97 | 风云III修订版 .rar | fengyun3xiuding | 40089 | done | dup: "...  (1).rar"; readme calls it "星星修订的风云3" (a themed community revision); confirmed 风云3 engine core same family as fengyun434/fy2/fy2005/zzfy(#30)/moniHuafu(#57) via md5sum (master.c content-identical to zzfy's, chinese.c byte-identical across this archive/zzfy/moniHuafu), closest to zzfy's snapshot with distinct site content/branding; standard §15h fix + §15p/§15u(moniHuafu's phone-home self-destruct pattern recurred, neutralized, confirmed reachable only via an unreferenced dead relay)/§14/§15s fixes + a new moniHuafu-style static/nosave string-literal collision (8 files reverted) + a genuine undeclared-variable typo in gangster2.lpc; full registration + post-login-command flow verified across 2 sessions with real name 秦风 reaching 凤求凰客栈, look/score/quit all working, zero errors either time; 99.6%+ lpcc pass (3700/3714, up from 3698); see libs/fengyun3xiuding/NOTES.md |
| 98 | 风云III典藏版.rar | fengyun3dianzang | 40090 | done | dup: "...(1).rar"; same 风云3 engine lineage as zzfy(#30)/moniHuafu(#57)/fengyun3xiuding(#97) -- master.c/simul_efun.c/securityd.c/chinese.c all byte-identical to #97's raw archive (main session cross-verified master.c), only logind.c differs; standard §15h fix + §15p/§15u(genuinely reachable phone-home license check via gtell.lpc)/§15s/§8e fixes + moniHuafu-style static/nosave collision recurred in 3 files (master.lpc/securityd.lpc/suicide.lpc) + 2 pre-existing typos; full registration + post-login-command flow verified across 4 sessions with real names 秦风/秋月/令狐冲/任我行, look/score/quit all working, zero errors across all 4 boots; 99.6% lpcc pass (3750/3765, up from 3747); see libs/fengyun3dianzang/NOTES.md |
...
| 100 | 魔幻世纪.rar | mohuanshiji | 40092 | done | THE FINAL ARCHIVE OF THE ENTIRE ~100-ARCHIVE PROJECT; ES II-derived "神话西游"(SJXY)/xyj-descended build (readme credits 小宝/西瓜太郎/桃心太郎, hosted by 星月传奇), genuinely distinct build, not a rebrand of any shipped sibling; standard §15h fix + §8h convertd.lpc (43 occurrences, crashed simul_efun's compile) + NEW case-sensitivity bug (CHANNEL_D macro="/adm/daemons/channeld" but file was CHANNELD.lpc post uppercase-.C rename, broke ftpd and other CHANNEL_D callers, renamed to lowercase) + NEW finding: 15 uppercase-.C files were never GBK->UTF-8 converted at all (convert_lib.sh's forced-text check is case-sensitive, file(1) misclassified them as binary), converted properly + stripped stray DOS Ctrl-Z bytes from 11 files + §14/§15p/§15w/§15ab/§15b/§8e/§15ac/§15t fixes + 3 pre-existing typos; confirmed §15ae/§15ar/§15s all already fine in this lib's own source (author already fixed command_hook, convert_lib.sh auto-fixed the sscanf pattern, driver's f_message() tolerates a raw int 0 exclude arg); full registration + post-login-command flow verified twice with real names 秦风四(male)/秦风五(female), both reaching 南城客栈 with correctly-gendered output, look/score/quit all working, debug.log completely clean both times; 97.4% lpcc pass (8316/8535); see libs/mohuanshiji/NOTES.md |
