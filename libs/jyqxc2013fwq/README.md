...
- 和 `jyqxc`/`jyqxc2` 同一架构家族的又一个存档（2013 年服务器版）。
  §10.7 深度测试用 `diff -rq` 逐文件核对确认：这**不是**字节级重复
  压缩包（不同于 `jyqxc2` 相对 `jyqxc` 的关系）——`work/` 目录树文件
  数量就不同（本档案 3942 个 vs `jyqxc`/`jyqxc2` 各约 3693/3694 个），
  绝大多数 `.lpc` 源文件内容也不同，只是共享同一套整体目录结构、地
  图布局和 bug 面，是一份真正独立、点位不同的存档快照。
