...
/*
	Added by Clode@RW 2001/10/8 05:37下午
	用來中止一個互動物件的input_to狀態
*/

#ifdef F_REMOVE_INPUT_TO
void
f_remove_input_to PROT((void))
{
    object_t *ob;
...
f_store_variable PROT((void)) {
    ...
    if( st_num_arg==3 ) {
        ob=sp->u.ob;
        pop_stack();
        idx = find_global_variable(ob->prog, (sp-1)->u.string, &type, 2);
    } else {
        ob= current_object;
        idx = find_global_variable(ob->prog, (sp-1)->u.string, &type, 0);
    }
...
f_fetch_variable PROT((void)) {
    ...
    if( st_num_arg==2 ) {
        ob=sp->u.ob;
        pop_stack();
        idx = find_global_variable(ob->prog, sp->u.string, &type, 2);
    } else {
        ob= current_object;
...
/* 
	Written by Clode@Revival World 2002/3/15 07:29下午
	將 simul_efun base_name 改成真的 efun
*/
#ifdef F_BASE_NAME
void
f_base_name PROT((void)) {
	...
	free_object(sp->u.ob, "f_base_name");
    	...
    	if( (tmp = strrchr(res, '#')) != 0 )
    	{
        	*tmp = '\0';
        	
    		tmp = new_string(strlen(res), "f_base_name");
...
/* 
	Written by Clode@Revival World 2001/7/10 05:31下午 
	去除字串兩端的空白字元
*/

#ifdef F_REMOVE_FRINGE_BLANKS
void
f_remove_fringe_blanks PROT((void)) {
	int len;
...
nosave void deep_copy_svalue PROT((svalue_t *, svalue_t *));

nosave array_t *deep_copy_array P1( array_t *, arg ) {
    array_t *vec;
    int i;
...
nosave array_t *deep_copy_class P1(array_t *, arg) {
    array_t *vec;
    int i;
...
nosave int doCopy P3( mapping_t *, map, mapping_node_t *, elt, mapping_t *, dest) {
    svalue_t *sv;
...
nosave mapping_t *deep_copy_mapping P1( mapping_t *, arg ) {
    mapping_t *map;
...
nosave void deep_copy_svalue P2(svalue_t *, from, svalue_t *, to) {
    switch (from->type) {
    case T_ARRAY:
	depth++;
...
/* Beek */
#ifdef F_VARIABLES
nosave void fv_recurse P5(array_t *, arr, int *, idx, program_t *, prog, int, type, int, flag) {
    int i;
    array_t *subarr;
    char buf[256];
    char *end = EndOf(buf);
...
nosave int at_end(int i, int imax, int z, int *lens) {
    if (z + 1 != lens[i])
	return 0;
    for (i++; i < imax; i++) {
	if (lens[i] > 0)
...
nosave char *pluralize P1(char *, str) {
    char *pre, *rel, *end;
    char *p, *of_buf;
 ...
 */
nosave int file_length P1(char *, file)
{
  struct stat st;
  FILE *f;
  int ret = 0;
...
nosave int query_ip_port P1(object_t *, ob)
{
...
nosave int memory_share PROT((svalue_t *));

nosave int node_share P3(mapping_t *, m, mapping_node_t *, elt, void *, tp) {
    int *t = (int *)tp;
...
nosave int memory_share P1(svalue_t *, sv) {
 ...
 */
#ifdef F_MEMORY_SUMMARY
nosave void fms_recurse P4(mapping_t *, map, object_t *, ob, 
