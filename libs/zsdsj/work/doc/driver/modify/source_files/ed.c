...
nosave int version = 601;

nosave int EdErr = 0;

nosave int append PROT((int, int));
nosave int more_append PROT((char *));
nosave int ckglob PROT((void));
nosave int deflt PROT((int, int));
nosave int del PROT((int, int));
nosave int dolst PROT((int, int));
/* This seems completely unused; -Beek
nosave int esc PROT((char **));
*/
nosave int doprnt PROT((int, int));
nosave void prntln PROT((char *, int, int));
nosave int egets PROT((char *, int, FILE *));
nosave int doread PROT((int, char *));
nosave int dowrite PROT((int, int, char *, int));
nosave int find PROT((regexp *, int));
nosave char *getfn PROT((int));
nosave int getnum PROT((int));
nosave int getone PROT((void));
nosave int getlst PROT((void));
nosave ed_line_t *getptr PROT((int));
nosave int getrhs PROT((char *));
nosave int ins PROT((char *));
nosave int join PROT((int, int));
nosave int move PROT((int));
nosave int transfer PROT((int));
nosave regexp *optpat PROT((void));
nosave int set PROT((void));
nosave void set_ed_buf PROT((void));
nosave int subst PROT((regexp *, char *, int, int));
nosave int docmd PROT((int));
nosave int doglob PROT((void));
nosave void free_ed_buffer PROT((object_t *));
nosave void shift PROT((char *));
nosave void indent PROT((char *));
nosave int indent_code PROT((void));
nosave void report_status PROT((int));

#ifndef OLD_ED
nosave char *object_ed_results PROT((void));
nosave ed_buffer_t *add_ed_buffer PROT((object_t *));
nosave void object_free_ed_buffer PROT((void));
#endif

nosave void print_help PROT((int arg));
nosave void print_help2 PROT((void));
nosave void count_blanks PROT((int line));
nosave void _count_blanks PROT((char *str, int blanks));

nosave ed_buffer_t *current_ed_buffer;
nosave object_t *current_editor;        /* the object responsible */
...
nosave char inlin[ED_MAXLINE];
nosave char *inptr;		/* tty input buffer */
...
nosave int append P2(int, line, int, glob)
{
    if (glob)
	return SYNTAX_ERROR;
    setCurLn(line);
...
nosave int more_append P1(char *, str)
{
    if (str[0] == '.' && str[1] == '\0') {
	P_APPENDING = 0;
#ifdef OLD_ED
...
nosave void count_blanks P1(int, line)
{
    _count_blanks(gettxtl(getptr(line)), 0);
}

nosave void _count_blanks P2(char *, str, int, blanks)
{
    for (; *str; str++) {
	if (*str == ' ')
	    blanks++;
...
nosave int deflt P2(int, def1, int, def2)
{
    if (P_NLINES == 0) {
	P_LINE1 = def1;
	P_LINE2 = def2;
...
nosave int del P2(int, from, int, to)
{
...
nosave int dolst P2(int, line1, int, line2)
{
...
/*	doprnt.c	*/
nosave int doprnt P2(int, from, int, to)
{
    from = (from < 1) ? 1 : from;
...
nosave void free_ed_buffer P1(object_t *, who)
{
    clrbuf();
#ifdef OLD_ED
...
nosave void prntln P3(char *, str, int, vflg, int, len)
{
	...
	if (*str >= 0 && *str < ' ') {
	//if (*str < ' ' || *str >= DEL) { mark by Clode@RW
	    switch (*str) {
	    case '\t':
...
nosave int egets P3(char *, str, int, size, FILE *, stream)
{
    int c = 0, count;
    char *cp;
...
nosave int doread P2(int, lin, char *, fname)
{
    FILE *fp;
    int err;
    unsigned int bytes;
    unsigned int lines;
    nosave char str[ED_MAXLINE];
...
nosave int dowrite P4(int, from, int, to, char *, fname, int, apflg)
{
    FILE *fp;
    int lin, err;
    unsigned int lines;
...
nosave int find P2(regexp *, pat, int, dir)
{
    int i, num;
    ed_line_t *lin;
...
nosave char *getfn P1(int, writeflg)
{
    nosave char file[MAXFNAME];
    char *cp;
    char *file2;
    svalue_t *ret;
...
nosave int getnum P1(int, first)
{
    regexp *srchpat;
    int num;
    char c;
...
nosave ed_line_t *getptr P1(int, num)
{
    ed_line_t *ptr;
    int j;
...
nosave int join P2(int, first, int, last)
{
    char buf[ED_MAXLINE];
    char *cp = buf, *str;
    ed_line_t *lin;
...
nosave int transfer P1(int, num)
{
    int mid, lin, ntrans;
...
nosave int subst P4(regexp *, pat, char *, sub, int, gflg, int, pflag)
{
    int nchngd = 0;
    char *txtptr;
...
#define bool char
nosave int lineno, errs;
nosave int shi;			/* the current shift (negative for left
				 * shift) */
 ...
 *              environments.
 */
nosave void shift P1(register char *, text)
{
...
nosave char *stack, *stackbot;	/* token stack */
nosave int *ind, *indbot;	/* indent stack */
nosave char quote;		/* ' or " */
nosave bool in_ppcontrol, after_keyword_t, in_mblock;	/* status */
int in_comment;
nosave char last_term[ED_MAXLINE+5];
nosave int last_term_len;
 ...
 *              indentation.
 */
nosave void indent P1(char *, buf)
{
    nosave char f[] =
...
nosave int docmd P1(int, glob)
{
    nosave char rhs[MAXPAT];
    regexp *subpat;
    int c, err, line3;
...
nosave void report_status P1(int, status) {
    switch (status) {
    case EOF:
...
nosave void print_help P1(int, arg)
{
    switch (arg) {
    case 'I':
...
nosave ed_buffer_t *ed_buffer_list;

nosave ed_buffer_t *add_ed_buffer P1(object_t *, ob) {
    ed_buffer_t *ret;
...
nosave void object_free_ed_buffer() {
