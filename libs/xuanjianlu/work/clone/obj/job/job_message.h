...
string get_family_master_massage(string command_mode,string menpai,string state)
{
	...
	switch(menpai)
				...
				case "少林派" :
					...
					break;
				case "古墓派" :
					talk_msg+=(string)get_msg_map("job_tell_mode",command_mode,state,gumu_message);
					break;
				case "灵鹫宫" :
					talk_msg+=(string)get_msg_map("job_tell_mode",command_mode,state,lingjiu_message);
					break;
				case "姑苏慕容" :
					talk_msg+=(string)get_msg_map("job_tell_mode",command_mode,state,murong_message);
					break;
				default :
