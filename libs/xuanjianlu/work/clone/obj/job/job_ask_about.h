...
void tell_player_family_job_open(object player,object master)
{
	...
	message_vision(CYN "$n对$N说道：好，希望你能为我"+family_name+"做出贡献。\n"NOR, player, master);
	player->set_temp("accept_family_job",1);
}
void tell_player_family_job_close(object player,object master)
{
	...
	message_vision(CYN "$n对$N说道：你要继续努力，争取在下次"+family_name+"大会上得到最佳评定。\n"NOR, player, master);
	player->delete_temp("accept_family_job");
}