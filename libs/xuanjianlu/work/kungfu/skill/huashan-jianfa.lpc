...
// qfy July 9, 1996
// Modified by Xuanyuan 2002.5.16

#include <ansi.h>
inherit SKILL;

mapping *action = ({
([	"action":"$N横剑当胸，左手捏了个剑诀，似是执笔写字一般，正是华山剑法的「诗剑会友」",
	"force" : 130,
        "dodge" : 5,
	"parry" : 5,
	"damage": 10,
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	if ( me->query_temp("zixia") ) {  
		...
		if( damage_bonus/2 > victim->query_str() ) {
			victim->receive_wound("qi", (level - 100) / 2 );
			return MAG ""+ me->name()+"长剑挥出，这一下使上了“紫霞功”，"+ victim->name()+"只觉全身一热，腾腾腾连退三步，一交坐倒。\n" NOR;
		}
	}
}
