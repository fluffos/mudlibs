...
int practice_skill(object me)
{
        ...
        if ( me->query_temp("weapon") )
                return notify_fail("练习五毒心法必须空手方可。\n");
...
mixed hit_by(object me, object victim, int damage, int damage_bonus, int factor)
{
	...
	if ((int)victim->query_temp("wudu/shield")!=1) return damage;
	
	dp = (int)victim->query_skill("wudu-xinfa",1);
        if (objectp(weapon2) ){
                dp += victim->query_skill("jueqing-bian",1);
        } else if (!objectp(weapon2) ) {
                if ( victim->query_skill_prepared("strike") != "chilian-shenzhang"  || victim->query_skill_prepared("cuff") != "meinu-quan")
                        return damage;
                dp += victim->query_skill("chilian-shenzhang",1);
        } else return damage;
        ...
        if ( random(ap+dp) > ap ) {
                if ( objectp(weapon2) && objectp(weapon1)) {
                        ...
                        {                 
							...
							result = YEL"$n舞动软刃幻出千万条银丝缠住$N的兵刃，$N的招数只进到一半便走不下去了。\n"NOR;
                        }
                        else {
					result = YEL"只见$n手中拂尘变幻多端，$N心下迟疑，本来已经递出去的招数又生生的收了回来。\n"NOR;
					damage_bonus=-random(4000);
					}
                }
					...
					{
					damage_bonus=-random(4000);
			        result = YEL"$n周身剧毒，$N只觉得一股腐朽之气扑面而来，本来已经递出去的招数又生生的收了回来。\n"NOR;                      
					}
                	else {
					result = YEL"只见$n嘴角微微冷笑，$N只觉得一股腐朽之气扑面而来，即便不敢妄进，将进招改为守招。\n"NOR;
					damage_bonus=-random(4000);
					}
				}
        		else if( !objectp(weapon2) && objectp(weapon1) ) {
                        ...
                        {                 
							damage_bonus=-random(4000);
							result = YEL"$n将身形一转，双手劲力把$N的兵刃一带，$N一个趔趄打偏了方向。\n"NOR;
                        }
                        else {
					result = YEL"只见$n嘿嘿冷笑，突然伸爪抓向$N，$N心中一惊：对方掌上有毒！本来已经递出去的招数又生生的收了回来。\n"NOR;
					damage_bonus=-random(4000);
					}
                }
                else{
                        ...
                        {                 
							damage_bonus=-random(4000);
							result = YEL"$n双掌互擦，登时腥臭弥漫，中人欲呕。$N突然心中一惊：对方掌上有毒！本来已经递出去的招数又生生的收了回来。\n"NOR;
                        }
                        else {
					result = YEL"只见$n嘿嘿冷笑，突然一掌劈向$N，$N心中一惊：对方掌上有毒！本来已经递出去的招数又生生的收了回来。\n"NOR;
					damage_bonus=-random(4000);
				}
			}
		}
