...
//				return notify_fail(target->name() + "目前正自顾不暇，使用此招有违江湖道义。\n");
        ...
        if( (int)me->query_skill("yiyang-zhi", 1) < 240 ||
        ...
        if( (int)me->query("neili") < (int)me->query_skill("yiyang-zhi", 1)/2 + me->query_skill("force",1)*2/3 + jiali*3 )
		...
		damage = (me->query("jiali") + me->query("jiajin")) * (int)me->query_skill("force")/50 + 
					(random((int)me->query_skill("yiyang-zhi", 1)) + (int)me->query_skill("force", 1))*time + 
		...
		if ( ap + random(ap) > dp + dp / 3 ){
				...
				me->add("neili", -(int)me->query_skill("yiyang-zhi", 1) - jiali );
 ...
 //               if(me->query_skill("yiyang-zhi", 1) > 199)
		...
		if ( ap + random(ap) > dp + dp / 2 ){
				...
				message_vision(HIG"$n当即伸出中指往$N食指上一弹，使的正是黄药师所授“弹指神通”功夫。\n\n" NOR, me, target);
					if (random((int)me->query_skill("yiyang-zhi", 1)) > random((int)target->query_skill("tanzhi-shentong", 1)))
					{
					...
					me->add("neili", -(int)me->query_skill("yiyang-zhi", 1)/3 - jiali );
...
门、通谷，下至中注、四满，直抵横骨、会阴，尽处于这一指威力之下。\n\n" NOR, me, target);
		if ( ap + random(ap) > dp + dp / 2 ){      
				...
				me->add("neili", -(int)me->query_skill("yiyang-zhi", 1)/3);
				me->add("jingli", -(int)me->query_skill("kurong-changong", 1)/4);
  //              if((int)me->query_skill("yiyang-zhi", 1) > 239) 
                call_out("perform3", 4, me, target);    
				return 1;
				}
                message_vision(HIR "结果一指点中，$n只觉得全身气脉通道阻塞，真气立受干挠！\n" NOR, me, target);
                target->start_busy(me->query_skill("force",1)/30+random(3));
                ...
                me->start_busy(2);
				me->add("neili", -(int)me->query_skill("yiyang-zhi", 1)/2);
...
int perform3(object me, object target)
{
		...
		if ( ap + random(ap) > dp + dp / 2 ){               
				...
				me->add("neili", -(int)me->query_skill("yiyang-zhi", 1));
