...
void init()
{
	...
	wheres = where[random(sizeof(where))];
...
"官府的捕头吧？！大家都是武林中人，今天能否高抬贵手？我必有重谢！你好好斟酌一下，是否愿意？\n",ob,me);
	...
	switch(random(4)) {
		...
		case 3:
			...
			call_out("kill_ob", 1, ob);
			break;
write(WHT"「"+ob->query_temp("guanfu_target")+"」好象往「"+wheres["chinese"]+"」的方向逃去了。\n"NOR);
           me->move(wheres["weizhi"]);
           return; 
		}         
	}
...
int do_yes()
{
        ...
        me->add_money("gold",30+random(40));
        command("give "+ob->query("id")+" gold");
        ob->add("shen",-1000);
        message_vision("$N嘿嘿一笑，转身离开，一眨眼就消失不见了。\n",me);
        destruct(me);
        ...
        me->add_money("gold",10+random(10));
        command("give "+ob->query("id")+" gold");
        message_vision("$N嘿嘿奸笑两声，悄悄塞给了$n一棵人参。\n",me,ob);
        r_shen=new("renshen");
        r_shen->move(ob);
