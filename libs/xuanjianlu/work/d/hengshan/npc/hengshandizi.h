...
int do_say()
{
	...
	if (ob->query("hengshan/job") && !me->query("songshan"))
	{
...
string getnpcname(string npc)
{
	...
	if (!objectp(npcob) ) 
		npcob=load_object("/d/hengshan/npc/"+npc);
    	if (!objectp(npcob)) 
    	{
...
string getroomname(string room)
{
	...
	if (!objectp(roomob) ) 
		roomob=load_object("/d/quanzhou/"+room);
    	if (!objectp(roomob)) {
...
void unconcious()
{
	...
	if (objectp(ob = me->query_temp("last_damage_from")) && ob->query_temp("marks/arrestee") == me->query("name") && me->query("bang")!=2 ) 
	{
		...
		me->set("bang",2);
		ob->set("songshan/"+query("id")+"/zhua",1);
		ob->set_temp("bang",me);
...
void die()
{
	...
	if (objectp(ob = me->query_temp("last_damage_from")) && ob->query_temp("marks/arrestee") == me->query("name") && me->query("bang")!=2 ) 
	{
		...
		me->set("neili", me->query("max_neili"));
		me->set("bang",2);
		ob->set("songshan/npc"+query("npcid")+"/zhua",1);
		ob->set_temp("bang",me);
		ob->delete_temp("marks/arrestee");
		ob->remove_all_killer();
		me->remove_all_killer();
