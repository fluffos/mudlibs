...
string tell_job(object player) {
    ...
    // 50: bonus 300%  Remember it's effective death, so if one die 50
    // 100: bonus 400% times, it means that he actually lost 250K exp

    sacrifice = (int)player->query("futou_bang/sacrifice");
    if (sacrifice > 100) sacrifice = 100;

    if (sacrifice < 10) sacrifice *= 2;
    else if (sacrifice < 30) sacrifice = sacrifice*4-20;
    else if (sacrifice < 50) sacrifice = sacrifice*10-200;
    else sacrifice = sacrifice*3+150;
