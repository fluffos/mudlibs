...
void my_destruct(object obj)
{
		  ...
		  if (obj && objectp(obj) ) {
								...
								while(i--)
								{
										  ...
										  {
											 call_out("move_me",1+random(4),inv[i],"/d/bwdh/sjsz/kantai_w1.lpc");
					 ...
					 if( userp(obj) )
					 {
						 call_out("move_me",1+random(4),obj,"/d/bwdh/sjsz/kantai_w1.lpc");
...
void finish(object target) {
		...
		ob->query("sjsz/team_name") != target->query("sjsz/team_name") )
	{
	 ...
	 if ( target->query("sjsz/white") )
		  call_out("move_me",1+random(2),target,__DIR__"east_xiangfang");
	 else if ( target->query("sjsz/red") )
		  call_out("move_me",1+random(2),target,__DIR__"west_xiangfang");

	 if( objectp(corpse = CHAR_D->make_corpse(target, ob)) )
	 {
		 call_out("move_me",1+random(2),corpse,environment() );
	 }
