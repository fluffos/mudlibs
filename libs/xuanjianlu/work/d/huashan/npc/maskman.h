...
int do_filter(string arg)
{
	...
	if ( objectp(me) && me->query("kill_player") == victim->query("id") ) {
		message_vision( "$n对着$N大吼：还想跑？纳命来吧！\n", victim, me );
		return 1;
	}
...
int checking2(object me, object victim)
{
	...
	if(wizardp(victim)) 
		command("tell xuanyuan 你在"+room->query("short")+"，我在"+environment(me)->query("short")+"。");
	...
	&& !victim->is_ghost() ) {
		if(wizardp(victim))
			command("tell xuanyuan 该前往"+room->query("short")+"。");
