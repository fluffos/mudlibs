...
void create()
 {
       set_name(HIC"VCD"HIM"遥控"HIG"器"NOR,({"controler","vcd","yaokong"}) );
    set_weight(200);
    if( clonep() )
           set_default_object(__FILE__);
    else {
        set("unit", "台");
            set("long", "一个先进的遥控播放器，点播请用 play vcd。\n");
         set("no_get",1);
    }
    setup();
}
...
int continue_kala(int release, object me)
{
        string *strs=({
                "\n\n                   "YEL"盛  夏  的  果  实\n\n"NOR,
                CYN"$N："+HIR+"也许放弃  才能靠近你\n"NOR,
                CYN"$N: "+HIG+"不再见你  你才会把我记起 \n"NOR,
                CYN"$N: "+HIY+"时间累积  这盛夏的果实 \n"NOR,
                CYN"$N: "+HIB+"回忆里寂寞的香气 \n"NOR,
                CYN"$N: "+HIM+"我要试着离开你  不要再想你 \n"NOR,
                CYN"$N: "+HIC+"虽然这并不是我本意 \n"NOR,
                CYN"$N: "+HIW+"你曾说过   会永远爱我 \n"NOR,
                CYN"$N: "+HIR+"。。。。\n"NOR,
                CYN"$N: "+HIG+"也许承诺   不过因为没把握 \n"NOR,
                CYN"$N: "+HIY+"别用沉默   再去掩饰甚么 \n"NOR,
                CYN"$N: "+HIB+"当结果是那么赤裸裸 \n"NOR,
                CYN"$N: "+HIM+"以为你会说甚么 才会离开我 \n"NOR,
                CYN"$N: "+HIC+"你只是转过头   不看我 \n"NOR,
                CYN"$N: "+HIW+"。。。\n"NOR,
                CYN"$N: "+HIR+"不要刻意说   你还爱我 \n"NOR,
                CYN"$N: "+HIG+"当看尽潮起潮落 只要你记得我 \n"NOR,
                CYN"$N: "+HIY+"。。。 \n"NOR,
                CYN"$N: "+HIB+"你曾说过 会永远爱我 \n"NOR,
                CYN"$N: "+HIM+"也许承诺 不过证明没把握  \n"NOR,
                CYN"$N: "+HIC+"不用难过 不用掩饰甚么 \n"NOR,
                CYN"$N: "+HIW+"当结果是那么赤裸裸 \n"NOR,
                CYN"$N: "+HIR+"其实不必说甚么  才能离开我 \n"NOR,
                CYN"$N: "+HIG+"起码那些经过    属于我 \n"NOR,
                CYN"$N: "+HIY+"。。。\n"NOR,
                CYN"$N: "+HIB+"也许放弃 才能靠近你\n"NOR,
                CYN"$N: "+HIM+"不再见你 你才会把我记起 \n"NOR,
                CYN"$N: "+HIC+"时间累积 这剩下的果实 \n"NOR,
                CYN"$N: "+HIW+"回忆里爱情的香气 \n"NOR,
                CYN"$N: "+HIR+"。。。\n"NOR,
                CYN"$N: "+HIG+"我以为不露痕迹  思念却满溢 \n"NOR,
                CYN"$N: "+HIY+"或许这代表我的心 \n"NOR,
                CYN"$N: "+HIM+"。。。\n"NOR,
                CYN"$N: "+HIC+"如果你会梦见我 请你再抱紧我…… \n"NOR,
                CYN"$N: "+HIW+"..............\n"NOR,
