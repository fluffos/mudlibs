...
// Let's see if this player still carries an un-expired task
	...
	tell_object(me, "康亲王想了想道：听说"+CHINESE_D->chinese(party)+"的『" + sprintf("%s(%s)",cname,id)+ "』蓄谋已久，欲对我大清不利，
那你就在" + time + "内干掉此人，提头来见吧！\n" NOR);
	
#ifdef debug lj
	tell_object(me, "That is test version time , the real run time is 2 times of now!\n");
#endif	
	command("whisper "+me->query("id")+" 听说他出没在"+CHINESE_D->chinese(party)+"附近，你可以去那里寻找！");
	command("push "+me->query("id"));

	me->set("mission/kangqw", missiondata);
...
int accept_object(object who, object ob)
{
	...
	command("say 你想要功绩(1)、神(2)还是金钱(3)呢？想好了就告诉本王。");
	input_to("select_gift",who,mbonus);
	call_out("destory", 1,ob);
	return 1;
}

void select_gift(string arg,object who,mapping mbonus)
{
        int i;
