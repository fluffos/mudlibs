...
int exert(object me, object target)
{
        ...
        if (target != me)
                return notify_fail("你只能用九阴真经来提升自己的战斗力。\n");
        ...
        message_combatd(HIW "$N" HIW "$N眼中射出两道寒芒，刹那间将自己的功力提升至颠峰！\n" NOR, me);
        //打通任督二脉的效果
...
void remove_effect(object me, int amount)
{
        if ((int)me->query_temp("powerup"))
        {
                if (me->query("breakup"))
                {
                        ...
                        me->add_temp("apply/defense", - (amount * 2 / 3));
                }
                else
                {
                ...
                tell_object(me, "你的九阴真经运行完毕，将内力收回丹田。\n");
        }
