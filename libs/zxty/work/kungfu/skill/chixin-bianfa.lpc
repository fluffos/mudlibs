...
mapping *action = ({
        ([ "name":   ""MAG"黯然"WHT"销魂"NOR"",
           "action": "$N美眉紧锁，一招「"MAG"黯然"WHT"销魂"NOR"」手中$w向$n的$l舞去。",           "force":  100,
           "dodge":  -10,
           "damage": 30,
           "lvl"    : 0,
           "damage_type":  "割伤"
...
mapping *action2 = ({
     ...
     "force" : this_player()->query_skill("chixin-bianfa",1)/2,
     ...
     "parry" : this_player()->query_skill("parry",1)/2,
        ...
        ([ "name":   ""HIB"致死"HIR"不渝"NOR"",
           "action": "$N手中$w对空一指，化鞭为剑，一招「"HIB"致死"HIR"不渝"NOR"」好似不故一切
攻向$n。",
     "force" : this_player()->query_skill("chixin-bianfa",1)/2,
     "dodge" : this_player()->query_skill("dodge",1)/4,
     "damage" : this_player()->query("zjb_dj/dj")*100,
     "parry" : this_player()->query_skill("parry",1)/2,
        ...
        ([ "name":   ""HIG"室迩"MAG"人远"NOR"",
           "action": "$N忽然后跳三步，$n正诧异间，$N一式「"WHT"室迩"MAG"人远"NOR"」，手中$w陡
然刺向$n的$l。",
     "force" : this_player()->query_skill("chixin-bianfa",1)/2,
     "dodge" : this_player()->query_skill("dodge",1)/4,
     "damage" : this_player()->query("zjb_dj/dj")*100,
     "parry" : this_player()->query_skill("parry",1)/2,
        ...
        ([ "name":   ""HIR"痴"HIG"心"HIM"不"HIB"悔"NOR"",
           "action": "$N默念口诀，一招「"HIR"痴"HIG"心"HIM"不"HIB"悔"NOR"」横生挥向$n的$l,$n慌忙招架，不
料$N的$w突然打了个弯，重重的缠向$n的$l。",
     "force" : this_player()->query_skill("chixin-bianfa",1)/2,
     "dodge" : this_player()->query_skill("dodge",1)/4,
     "damage" : this_player()->query("zjb_dj/dj")*100,
     "parry" : this_player()->query_skill("parry",1)/2,
...
int valid_learn(object me)
{
        ...
        if ((me->query("family/master_id")!="shangguan feifeng")&&
            ...
            (me->query("family/master_id")!="shangguan yunqian")&&                  (me->query("family/master_id")!="long yujiao"))
...
mapping query_action(object me, object weapon)
{
        int i, level;
          mapping a_action;
        level   = (int) me->query_skill("chixin-bianfa",1);
          me = this_player();
        if (me->query_skill("chixin-bianfa",1)<501)  
    return action[random(sizeof(action))];
 
        if (me->query_skill("chixin-bianfa",1)>500
...
int practice_skill(object me)
{
       ...
       if ((me->query("family/master_id")!="shangguan feifeng")&&
           (me->query("family/master_id")!="meng shashaonv")&&  
           (me->query("family/master_id")!="shangguan yunqian")&&           (me->query("family/master_id")!="long yujiao"))                return notify_fail("非本门亲授弟子无法练习。\n");
...
return HIM "你只听到 "HIY"撕啦"HIM" 一声轻响，竟然是$n被长鞭在身上留下了一条血痕！！\n" NOR;
  }
}
