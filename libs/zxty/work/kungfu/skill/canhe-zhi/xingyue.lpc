...
int perform(object me, object target)
{
 ...
 ob = me->select_opponent();
    ...
    if( !me->is_fighting() )
        return notify_fail("「星月连环」只能在战斗时使用。\n");
     if (me->query_temp("perform")>time()) return notify_fail("你上一招未使完！\n");
    if( me->query_temp("weapon"))
        return notify_fail("你先放下手中的武器再说吧？！\n");
        if( (int)me->query("neili") < 200 )
        return notify_fail("你的真气不够！\n");
       if( (int)me->query_skill("canhe-zhi") < 500 ||
        me->query_skill_mapped("unarmed") != "canhe-zhi")
        return notify_fail("你的「参合指」还不够精纯，无法使用「星月连环」！\n");
  msg = HIM "$N催动神元功内劲，一指化三击，连环施展三记重手，正是参合指法之「星月连环」。\n" NOR;
    ...
    for(i = 0; i < 3; i++)
        if (me->is_fighting(ob) && ob->is_fighting(me) && ob->query("eff_qi")>0){
              me->set_temp("action_msg",HIB"$N内力疾吐，眨眼间又是一招击出。\n"NOR);
