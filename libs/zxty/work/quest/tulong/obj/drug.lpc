...
// add by Zjb@TY 
          if ( this_object()->query("zhuren") != ob->query("id") )
                return notify_fail(HIW"这个龙元不是属于你的，你想干什么?\n"NOR);
...
int move(mixed dest)
{
if (userp(dest)&&!this_object()->query("zhuren"))
this_object()->set("zhuren",""+dest->query("id")+"");
return ::move(dest);
}
