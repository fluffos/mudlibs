...
message("channel:chat", HBMAG"【精灵神界】"+who->query("name")+"成功的解开了第五封印.\n"NOR,users());
        me->set_temp("m_success/琼草",1);  
...
int do_eat(string arg)
{
   ...
   if( !me->query("eatable")||who->query_temp("tulong/eat")){
    ...
    neili_add=random(1000);
   
  message_vision("$N将一颗$n轻轻嚼着咽下了肚，脸上泛起阵红晕。\n", who);
   tell_object(who, "你觉得一阵热气直通七窍，浑身上下好象有使不完的力气。\n");

  who->set_temp("tulong/eat",1);
   
if ( who->query("ty_dan/drug1") < time() )
{
   this_player()->set("ty_dan/shc",time()+10000);
