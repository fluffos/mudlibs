...
void create()
{
	...
	set("inquiry", ([
		...
		"价钱" : "花轿加轿夫，五十两金子。要是加吹鼓手，另加五十两。现钱交易，先付钱。",
	]) );
	setup();
}
...
int do_look(string arg)
{
	if ((arg=="jiaozi"
	 ...
	 || arg=="jiao")
	 && query("on_way")) {
		tell_object(this_player(), "这是一顶漂亮的花轿，大红的缎子布包着，四角扎着大红花，四根杆子都用彩带绕着。\n"
...
int do_kill(string arg)
{
	if ((arg=="jiaofu tou"
	 ...
	 || arg=="shou"
	 || arg==query("groom"))
	 && environment()!=find_object("/d/city/yltw")) {
		write("呔！坏人美事，定遭天谴，会五雷轰顶的！\n");
		return 1; 
	}
	return 0;
}
...
int do_say(string arg)
{
	...
	if ((arg=="到了" || arg=="here") && getuid(me)==query("groom") && query("on_way")) {
		...
		if (present("jiaofu tou", find_object("/d/city/yltw"))) {
		...
		if (objectp(tou=present("jiaofu tou",environment(me)))) {
			...
			if (objectp(jiao=present("jiaozi",tou)))
				jiao->move("/d/city/yltw");
		}
		if (objectp(worker1=present("jiaofu",environment(me)))) {
			worker1->move("/d/city/yltw");
			worker1->set_leader(0);
			worker1->delete("location");
		}
		if (objectp(worker1=present("luogu shou",environment(me)))) {
			worker1->move("/d/city/yltw");
			worker1->set_leader(0);
...
int accept_object(object who, object ob)
{
	...
	if (ob->value()>=500000 && ob->value()<1000000) {
	...
	if (ob->value()>=1000000) {
		...
		command("say 好的好的，多谢您老的金子，花轿这就备好，请新娘子上轿。");
		...
		worker2->set("location", 1);
		worker3->set_leader(worker1);
		worker3->set("location", 1);
		message_vision("轿夫头把轿帘撩了起来，新娘可以进去了。\n",who);
