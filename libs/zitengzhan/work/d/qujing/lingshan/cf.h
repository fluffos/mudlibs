...
if(extra_bonus > 900) extra_bonus = 900;
        potential_bonus = random(extra_bonus) + level_bonus + kind_bonus;
        combat_exp_bonus = extra_bonus*2 + (random(level_bonus) + random(kind_bonus))*2;
        daoxing_bonus = extra_bonus*2 + (random(level_bonus) + kind_bonus/2+random(kind_bonus/2))*2;
...
//        if ( level > 1 ) CHANNEL_D->do_channel(me, "rumor",str);
 message("channel:rumor", HIM"【长安夜话】某人："+HIM""+str+"\n"NOR,users());
