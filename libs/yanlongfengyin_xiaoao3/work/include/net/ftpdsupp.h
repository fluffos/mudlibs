...
mixed xyzx_ftpuser_login(string str, int type)
{
	string file_arg, *ftpuser, name, pass;
	mapping userlist;
	int i;

	if ( file_size(CONFIG_DIR + "ftp_users") < 0 )
		return 0;

	file_arg = read_file(CONFIG_DIR + "ftp_users");
	ftpuser = explode(file_arg, "\n");
	userlist = allocate_mapping( sizeof(ftpuser) );
	for ( i = 0; i < sizeof(ftpuser); i++)
	{
		if ( sscanf(ftpuser[i], "%s:%s", name, pass) )
		{
				userlist[name] = pass;
		}
	}
	...
	if ( type == 1 )
	{
		ftpuser = keys(userlist);
		return ftpuser;
	}

	// 验证用户密码，用户名存在则返回这个用户的密码
	if ( type == 2 )
	{
		return userlist[str];
	}
...
int check_access(string name)
{
	...
	if ( file_size(CONFIG_DIR + "ftp_users") > 0 )
	{
		if ( !arrayp(user_name = xyzx_ftpuser_login(name, 1)) )
			return 0;
		if ( !sizeof(user_name) )
			return 0;

		if ( member_array(name, user_name) != -1 )
		{
			if ( file_size(HOME_DIR(name)) != -2 )
				mkdir(HOME_DIR(name));
			return 1;
		}
...
{
    string fancytext, cpass, user_name, user_pass;
    object login_ob;
	...
	if ( file_size(CONFIG_DIR + "ftp_users") > 0 )
	{
		...
		if ( !stringp(user_pass = xyzx_ftpuser_login(name, 2)) )
			return 0;
		if ( user_pass == "" )
			return 0;
		
		if ( member_array(name, user_name) != -1 && plaintext == user_pass )
			return 1;
    ...
    login_ob->restore();
    fancytext = (string)login_ob->query("passwordold");
#endif
