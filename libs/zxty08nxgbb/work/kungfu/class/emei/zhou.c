...
"她是一位容貌美丽，端庄可爱的姑娘。虽然年纪很轻，但在峨嵋派中是\n出类拔萃的人物。\n");
        ...
        set_skill("linji-zhuang",2000);
        set_skill("jinding-zhang",2000);
        set_skill("huifeng-jian",2000);
        set_skill("zhutian-bu",2000);
        set_skill("yugalism", 2000);
        set_skill("dushi-jiren",2000);
        set_skill("unarmed", 2000);
        set_skill("zhuyan-shu", 2000);
        set_skill("jiuyin-baiguzhao", 2000);
       ...
       carry_object("/clone/misc/zhujingchan");
  }
...
void attempt_apprentice(object ob)
 {
    ...
    if ((int)ob->query("per") < 30 ) {
        ...
        if ((int)ob->query("max_neili") < 500000) {
                command("shake");
                command("say "+RANK_D->query_respect(ob)+"的内力虽然深厚，但是要想学九阴真经上的武功还嫌稍弱。");
                return;
                  } 
