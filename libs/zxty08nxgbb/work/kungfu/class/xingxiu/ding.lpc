...
string ask_me()
{
    object me,ob;
    ob=this_player(); 
      if ((string)ob->query("family/master_id") != "ding chunqiu")
        return "你是何人？";  
    if (query("biao_count") < 1)
        return "你来晚了，那柄宝器我已经赠送给别入了。";   
    add("biao_count", -1);                  
    message_vision(HIC"丁春秋从一个红木锦盒里捧出一根沉甸甸的钢杖交给了$N \n\n"NOR,ob);
    me=new("d/xingxiu/obj/thunder");
    me->move(ob);
    return "这就是天山老人当年的兵器，你就拿它为本门扬威天下吧。\n";
}
