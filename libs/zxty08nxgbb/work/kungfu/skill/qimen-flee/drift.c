...
int conjure(object me, object target)
{
  ...
  if (present("bao jing",me)) return notify_fail("你正在做task，无法使用神通！\n");
...
void select_target(object me, string name)
{
	...
	if (me->query("quest/quest_type")=="杀"&& (string)me->query("quest/quest")==ob->short()&& me->query("quest/task_time")<time())
	{
		me->start_busy(2); 
		write("你无法感受到这个人的精力 ....\n");
		return;
	...
	if(ob->query("id")=="salt seller")
	{
		...
		if(ob->query("id")==me->query("id")+ " xixia")
		{
			...
			if( !environment(ob) ||environment(ob)->query("no_magic") || environment(ob)->query("no_fight") || random(me->query_skill("qimen-flee",1)) < 30 ) {
		...
		if ( domain_file(base_name(environment(ob)))=="shaolin"
			...
			|| domain_file(base_name(environment(ob)))=="meizhuang" 
		...
		if ( domain_file(base_name(environment(ob)))=="heimuya" || domain_file(base_name(environment(ob)))=="gumu" || domain_file(base_name(environment(ob)))=="xixia"){
			me->start_busy(2); 
			write("那里太遥远了。\n");
			message("vision",HIY+me->name()+"一脸气急败坏，灰头灰脸地从洞里又钻了回来。\n" NOR, environment(me),({me}));
			return;
