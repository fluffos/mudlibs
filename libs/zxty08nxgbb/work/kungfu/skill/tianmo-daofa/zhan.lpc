...
int perform(object me, object target)
{
        ...
        if( (int)me->query_skill("tianmo-daofa", 1) < 200 )
                return notify_fail("你的天魔刀法不够娴熟，无法施展出斩字诀。\n");

        if ((int)me->query_skill("tianmo-shengong", 1) < 200)
                return notify_fail("你的天魔神功火候不够，无法施展出斩字诀。\n");
        ...
        if( random(ap + dp ) > dp ) {
                ...
                target->apply_condition("broken_arm",lvl); 
                me->add("neili", -100);
        }
