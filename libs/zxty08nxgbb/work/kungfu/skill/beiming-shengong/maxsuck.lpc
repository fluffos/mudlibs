...
int exert(object me, object target)
{
 ...
 if(  skill < 50 || me->query("max_neili") > skill*1200 )
    ...
    if( (int)target->query("max_neili") < 60  || !living(target))
        return notify_fail( target->name() + "已经内力涣散，你已经无法从他体内吸取任何内力了！\n");
              if (target->query("no_suck"))   return notify_fail(target->name()+"没有任何内力!\n"); 
...
if (userp(target))
{
  me->set("combat_zjb",""+target->query("id")+"");
   ...
   if( wiz_level(target)>0 || (int)target->query("max_neili") <= 0 || (int)target->query("no_suck")|| (int)target->query("max_neili") >210000)
   ...
   max_suck=(int)target->query("max_neili") - (int)me->query("max_neili")/2;
        ...
        max_suck/=20;
if (max_suck*3/2 > 180 && skill<1001)
 return notify_fail(HIR""+target->name()+"的内力太雄厚了,你吸了后可能承受不了,还是等你内力高点了再来吸吧!\n"NOR);
