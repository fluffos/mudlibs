// By KinGSo@舞林 2006年4月25日 早上

#define CANG "「" HIW "八方藏刀势" NOR "」"

#include <ansi.h>

inherit F_SSERVER;
#include "/kungfu/skill/eff_msg.h";
 
int perform(object me, object target)
{
	object weapon;
        string msg;
        int damage;
         int ap, dp;
        int count;
        int i;

       

        if (! target)
        {
	        me->clean_up_enemy();
	        target = me->select_opponent();
        }

	if (! target || ! me->is_fighting(target))
		return notify_fail(CANG "只能对战斗中的对手使用。\n");
 
	if (! objectp(weapon = me->query_temp("weapon")) ||
	    (string)weapon->query("skill_type") != "blade")
                return notify_fail("你使用的武器不对，难以施展" CANG "。\n");

	if ((int)me->query_skill("force") < 200)
		return notify_fail("你的内功火候不够，难以施展" CANG "。\n");

	if ((int)me->query_skill("hujia-daofa", 1) < 200)
		return notify_fail("你的胡家刀法还不到家，难以施展" CANG "。\n");

        if (me->query_skill_mapped("blade") != "hujia-daofa")
                return notify_fail("你没有激发胡家刀法，难以施展" CANG "。\n");

	if ((int)me->query("neili") < 1000)
		return notify_fail("你的真气不够，难以施展" CANG "。\n");

        if (! living(target))
                return notify_fail("对方都已经这样了，用不着这么费力吧？\n");

	message_vision(HIW "$N" HIW "轻舒猿臂，施出「战八方藏刀式」，手中的" + weapon->name() +
              HIW "吞吞吐吐，变化莫测，笼罩了$n" HIW "周身要害！\n" NOR, me, target);
        damage = (int)me->query_skill("hujia-daofa", 1);
       ap = me->query_skill("blade");

        if (random(me->query_skill("blade")) > target->query_skill("parry") / 2)
        {
                message_vision(HIY "$n" HIY "见来招实在是变幻莫测，不由得心"
                       "生惧意，招式登时出了破绽！\n" NOR, me, target);

               me->add_temp("apply/attack", ap/10);
             //   me->add_temp("apply/damage", damage/2);
                COMBAT_D->do_attack(me, target, me->query_temp("weapon"),0);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),3);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),3);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),3);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),3);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),3);
              

              

              me->add_temp("apply/attack", -ap/10);
          //      me->add_temp("apply/damage", -damage/2);
         

        } else
        {
                message_vision(HIC "$n" HIC "心底微微一惊，打起精神小心接招。\n" NOR, me, target);
            //    me->add_temp("apply/damage", damage/3);

                COMBAT_D->do_attack(me, target, me->query_temp("weapon"),1);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),0);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),1);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),2);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),3);
               if (me->is_fighting(target)) COMBAT_D->do_attack(me, target, me->query_temp("weapon"),3);
            //     me->add_temp("apply/damage", -damage/3);

               

               
             
        }

	
	me->add("neili", -220);
       

	me->start_busy(1 + random(2));
     me->start_perform(5,"「八方藏刀势」");
	return 1;
}
