...
// query_temp("lqsg/lq") 是冷泉内功yun lengquan之后的标志
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
      ...
      weap1 = me->query_temp("weapon");
           ...
           if( (me->query_skill("hujia-daofa") > 450) && (me->query("neili") > 10000)&& (me->query_skill("miaojia-jianfa") > 450)&& (me->query_skill("lengquan-shengong") > 450)
       && (victim->query_temp("weapon")) 
       && random(10) > 7){
       message_vision(HIY "只见$N手中" + weap1->name() + ""HIY"高举，一股夹着寒气的剑气对着对方的" + weap->name() +""HIR"刺去！\n" NOR, me ); 
       victim->start_busy(1);
       if ((me->query_skill("force")) > victim->query_skill("force") ){
       message_vision(HIW "\n潜运全身真劲全力一剑刺去下，只听 乒! 的一声巨响，" + weap->name()
  + "被"+weap1->name()+""HIW"从$N"HIR"手中直刺成两段，$N"HIW"情急之下只好空手应战。\n" NOR, victim );
 ...
 //  第六等级的伤气
      if( me->query_skill("hujia-daofa",1)>4000
  && (me->query("neili")*6) > victim->query("max_neili") && me->query_skill("miaojia-jianfa") > 4000&& me->query_skill("lengquan-shengong") > 4000 ){
              victim->receive_wound("qi", (damage_bonus ) * 3 , me);
return HIW "$N的刀剑归真以到"BLINK HIC"最高等级"NOR HIR"，随意一剑带出千阵寒气扑向$n！！！！！\n" NOR;
  }      

 //  第五等级的伤气
      if( me->query_skill("hujia-daofa",1)>3000
  && (me->query("neili")*5) > victim->query("max_neili") && me->query_skill("miaojia-jianfa") > 3000&& me->query_skill("lengquan-shengong") > 3000) {
              victim->receive_wound("qi", (damage_bonus )*2 , me);
return HIW "$N的刀剑归真以到"BLINK HIW"第五等级"NOR HIR"，随意一剑带出数百阵寒气扑向$n！！！！！\n" NOR;
  }      

 //  第四等级的伤气
      if( me->query_skill("hujia-daofa",1)>2000
      && (me->query("neili")*4 > victim->query("max_neili"))&& me->query_skill("miaojia-jianfa") > 2000&& me->query_skill("lengquan-shengong") > 2000) {
           victim->receive_wound("qi", damage_bonus  , me);
return HIW "$N的刀剑归真以到"BLINK HIY"第四等级"NOR HIR"，随意一剑带出一股寒气扑向$n！！！！！\n" NOR;
  }      

      if( damage_bonus < 100 ) return 0;
 // 第二等级的伤气
        if ( me->query_skill("hujia-daofa",1)>400
         && me->query_skill("hujia-daofa",1)<1001
&& (me->query("neili")*3) > victim->query("max_neili") && me->query_skill("miaojia-jianfa",1)>400
         && me->query_skill("miaojia-jianfa") < 1001&&me->query_skill("lengquan-shengong",1)>400
         &&  me->query_skill("lengquan-shengong") < 1001) {
             victim->receive_wound("qi", (damage_bonus - 100) / 8 , me);
return HIW "$N的刀剑归真已初有小成,带出一小阵"HIW"寒气"HIW"扑向$n！！！！！\n" NOR;
}

 //  第三等级的伤气
      if( me->query_skill("dugu-jiujian",1)>1000
  && me->query("zjb_dj/dj")>=5
  && (me->query("neili")*3) > victim->query("max_neili")&& me->query_skill("miaojia-jianfa",1)>1000
         && me->query_skill("lengquan-shengong",1)>1000 ){
                victim->receive_wound("qi", (damage_bonus - 100) / 3 , me);
return HIW "$N的刀剑归真已入返璞归真境界，随意一剑带出白色寒气扑向$n！！！！！\n" NOR;
  }      
