...
int perform(object me, object target)
{
    ...
    if((int)me->query_skill("unarmed") < 120 || me->query_skill("nianhua-zhi",1) < 100 )
        return 
...
if( random(me->query_skill("nianhua-zhi",1)+me->query_skill("unarmed",1)) > (int)target->query_skill("unarmed",1) )
    {
        tell_object(target, HIW "你突然间觉得浑身上下不听使唤，动弹不得！\n" NOR);
        message("vision", CYN "只见"+target->name()+"突然间神色大变，动弹不得！\n\n" NOR, environment(me), ({target}) );
        busy_time=random( (int)me->query_skill("nianhua-zhi",1) / 50 )+1;
