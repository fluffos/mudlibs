...
int heal_myself(object me)
{
    ...
    message("vision",
        ...
        environment(me), me);
    me->receive_curing("jing", this_player()->query("max_jing")/5); 
    me->add("neili", -50);
    me->set("jiali", 0);
        me->start_busy(1);
        me->improve_skill(force_skill, random(me->query_skill(force_skill, 
1)),0);
    return 1;
}
int heal_target(object me,object target)
{
    ...
    if (!stringp(force_skill)) return 
notify_fail("你所学的内功中没有这项技能。\n");
...
n" NOR,
        me, target );
    target->receive_curing("jing", this_player()->query("max_jing")/3 );
    me->add("neili", -150);
    me->set("jiali", 0);
    target->set("jiali", 0);
    me->start_busy(5);
