...
int heal_myself(object me)
{
    ...
    if( me->query_temp("heal_busy") > time() )
        return notify_fail("你刚刚疗伤完毕，正在回气中!!\n");
    if( me->is_fighting() )
        return notify_fail("战斗中运功疗伤？找死吗？\n");
    if ((int)me->query_skill(force_skill, 1) < 50)
        return notify_fail("你的"+to_chinese(force_skill)+"修为还不够。\n");
    if( (int)me->query("neili") < me->query("max_neili")/10 )
    ...
    message("vision",
        ...
        environment(me), me);
        me->receive_curing("qi", this_player()->query("max_qi")/5);
        me->add("neili", -200);
        me->improve_skill(force_skill, random(me->query_skill(force_skill, 1)),0);
        me->set_temp("heal_busy",time()+random(2));
    return 1;
}
