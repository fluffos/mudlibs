...
int perform(object me, object target)
{
    ...
    me->start_busy(1);
  if( random( (int)me->query_skill("liuyang-zhang",1)+me->query_skill("dodge",1)+me->query_skill("force",1))
          > random(target->query_skill("dodge") ) ) {
        msg += CYN " 结果$p被$P攻了个正着！\n" NOR;
        target->receive_damage("qi",(int)me->query_skill("bahuang-gong",1),me);
        target->receive_wound("qi",me->query_skill("liuyang-zhang",1) + random(me->query_skill("force",1)),me);
        target->receive_wound("jing",me->query_skill("liuyang-zhang",1) + random(me->query_skill("force",1)),me);
