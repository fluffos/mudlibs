...
//    mapping my_fam  = me->query("family");
    ...
    if( me->is_fighting() )
        return notify_fail("「唱字决」不能在战斗中演练。\n");
...
if ( (string)me->query("family/family_name") != "桃花岛") 
    ...
    if( !skill || (skill < 60))
        return notify_fail("你的玉箫剑法等级不够！\n");
    if( me->query("neili") < 50 )
        return notify_fail("你的内力不够！\n");
    if (me->query_skill("sword",1)<= skill)
        return notify_fail("你的基本剑法不够，无法通过演练「唱字决」提高玉箫剑法的技巧！\n");
    if( me->query("jing") < -jing_cost )
        return notify_fail("你现在太累了，无法集中精神演练「唱字决」！\n");
 write(HIW "你使出玉箫剑法之「唱字决」，将手中" + weapon->name() + HIW "挥舞了几下，玉箫剑法似乎更深进了一些。\n" + NOR);
