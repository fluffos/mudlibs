...
void create()
{
        ...
        if (mp["class"]) set("class",mp["class"]);
         if (mp["family"]) set("family/family_name2",mp["family"]); 
        set("title",HIC"反清头领"NOR);

        ratio=80+random(me->query_temp("shan_quest/count")*3+1);
...
*/
         skill = (int)(me->query_skill("dodge",1)/2)+me->query("zjb_dj/dj") *10; 
     if(skill>10000)    skill=10000;
        ...
        set("chat_msg_combat",chat_chance_combat_here);
             set("str",(me->query("zjb_dj/dj")*2)); 
             set("con",(me->query("zjb_dj/dj")*2)); 
             set("dex",(me->query("zjb_dj/dj")*2)); 
    ...
    dengji = me->query("zjb_dj/dj");
     ...
     set("zjb_dj/dj",dengji);
        ...
        set("max_neili",me->query("zjb_dj/dj")*100000);
        set("neili",me->query("zjb_dj/dj")*100000);
       ...
       set("killer",me->query("id"));
        if (me->query("zjb_dj/dj") > 30 ){         set("max_qi",query("max_neili")/2);}
        if (me->query("zjb_dj/dj") > 80 ){         set("max_qi",query("max_neili"));}
        if (me->query("zjb_dj/dj") < 31){          set("max_qi",query("max_neili")/4);}
//        set("max_qi",me->query("zjb_dj/dj")*80000);
        set("qi",query("max_qi"));
        set("eff_qi",query("max_qi"));
        set("max_jing",me->query("zjb_dj/dj")*50000);
...
//        if(random(10)>8) carry_object(__DIR__"obj/tiejia")->wear();
//        else carry_object(__DIR__"obj/armor")->wear(); 
}
...
void init()
{
        ...
        message_vision(
        HIB"$n一看到$N, 立刻撒出一包迷香，接着转身就走！\n"
        NOR,ob,killer); 
//      place_room(ob,killer);
        ob->set_temp("shan_quest/inquiry",1);
        } else
        if (ob->query_temp("shan_quest/being_chased")
        && !killer->query("baixing"))   
        {
        message_vision(
        CYN"$n一看到$N，冷冷的道：“原来是朝廷鹰犬，那就来吧！”"
           +" \n"NOR,ob,killer);
        ...
        {
        message_vision(
        CYN"$n不屑地朝$N说道: “原来就是你一直在妨碍我们反清大业，我们就来个了断吧！” \n"NOR,ob,killer);
...
// message_vision("$N狞笑了几声：“今天心情好，本大爷就放过你，不陪你玩了。”说罢，一个筋斗跳出战圈，不知去向。\n",ob);
...
void die()
{
      ...
      int zjb;
        ...
        if((int)killer->query_temp("shan_quest/count") <
         ...
         reward_exp=killer->query_int()*5+killer->query("kar")*20+count*600;  
         ...
         if(me->query("baixing"))
{
         ...
         if(killer->query("money") > 520000000){
                   killer->add("money",-500000000);
                   killer->add("zjb_money",5);
tell_object(this_player(),HIR"系统自动把你的存款换成星空币了！\n"NOR);
}

         if(killer->query("zjb_dj/dj") < 11)
           reward_exp -=reward_exp/5;
      if(reward_exp > 7800) reward_exp=7509+random(200);
...
if (reward_pot > 1300) reward_pot = 1088+random(200);
          if (reward_pot<0) reward_pot=0;
reward_exp = reward_exp - random(600);
if(!killer->query_temp("shan_quest/being_chased")){
          reward_exp = reward_exp/10;
          reward_pot=reward_pot/10; 
          tell_object(killer,
          HIM"由于你没回答门派，奖励大大减少了！\n"NOR);

}

//调用奖励文件 add by Amis@XO2    
   MIS_REW->give_reward_questkiller(killer,reward_exp,reward_pot);              
...
int get_level(int exp)
{
int level=this_player()->query("zjb_dj/dj")*150;
...
int bandit_send(object ob)
{
   ...
   if(ob->is_ghost() && !ob->query_temp("linji/youming") )
   {
    remove_call_out("escape_me");
    remove_call_out("bandit_send");
