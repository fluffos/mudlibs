...
string ask_for_zigong()
{
	object me;
	me = this_player();
             if(me->query("mud_age") < 350000)
                  return "你的年纪太小，等18岁再来吧？\n";
...
string ask_zhuanxing() 
{
     ...
     return "您是当今罚恶使者，来这里升级莫非想用BUG？\n";

        if( me->query("id")==shangshan->query("winner") )
     return "您是当今赏善使者，来这里升级莫非想用BUG？\n";

        if( me->query("id")==mengzhu->query("winner") )
return "您是当今武林盟主，来这里升级莫非想用BUG？\n";

        if( me->query("id")==xinxiu->query("winner") )
return "您是当今新秀使者，来这里升级莫非想用BUG？\n";

     if( me->query("gender") != "无性" )
 return "你又不是公公想来干什么?";
        if( exp < 20000000 && dj>1) {
  me->add("zjb_dj/dj",-1);
me->delete_skill("pixie-jian"); 
 me->delete("class");
  me->add("max_neili",-(me->query("max_neili")/4));
  me->add("max_jingli",-(me->query("max_jingli")/4));  me->set("gender","男性");

  if( me->query_skill("kuihua-xinfa"))
me->delete_skill("kuihua-xinfa"); 

        command("chat 恭喜"+me->name()+"由公公转为男性,重新做人!");
return "您顺利转性为男性,您为了转性减了1级,减了"+(me->query("max_neili")/4)+"点内力？\n
以及"+(me->query("max_jingli")/4)+"点精力和放弃了辟邪剑法等武功!\n";
}
  me->add("combat_exp",-(exp/3));  me->delete("pixie-jian"); 
  me->add("max_neili",-(me->query("max_neili")/4));
  me->add("max_jingli",-(me->query("max_jingli")/4));     
  me->set("gender","男性");
 me->delete("class");
me->delete_skill("pixie-jian"); 
...
me->delete_skill("kuihua-xinfa"); 
        ...
        command("chat 恭喜"+me->name()+"由公公转为男性,重新做人!");
return "您顺利转性为男性,您为了转性减了"+(exp/3)+"点经验,减了"+(me->query("max_neili")/4)+"点内力？\n
以及"+(me->query("max_jingli")/4)+"点精力和放弃了辟邪剑法等武功!\n";
}
