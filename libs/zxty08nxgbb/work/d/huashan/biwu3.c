...
",
    ...
    set("exits", ([ /* sizeof() == 2 */
    "west":__DIR__"houzhiwangyu",
        ]));
    set("no_fight",1);
    set("fight_room", __DIR__"houzhiwangyu");
    set("objects",([
       "/clone/misc/drum3":1,
    ]));
    setup();
...
string look_notice()
{
        ...
        for(i=0;i<sizeof(name_list);i++) {
                if(name_list[i] == "seperator" ) {
                        j++;
                        k=0;
                        printf("\n第"+chinese_number(j)+"届(成年组)华山论剑结果：\n");
                        continue;
                }
                k++;
...
void relay_message(string msg)
{
   ...
   string *msgs;
    int i;
    if (!look_fight_room=find_object(__DIR__"houzhiwangyu"))
    look_fight_room=load_object(__DIR__"houzhiwangyu");
   msgs=explode(msg, "\n");
