...
void create()
{
    set_name("张无忌", ({ "zhang wuji", "zhang", "wuji"}));
        ...
        set_skill("taiji-jian",1550);
        set_skill("qiankundanuoyi", 1650);
        set_skill("sword",1600);
        set_skill("force", 1500);
        set_skill("shenghuo-shengong", 1650);
        set_skill("dodge", 1600);
        set_skill("tiyunzong", 1550);
        set_skill("unarmed", 1600);
        set_skill("jiuyang-shengong", 1080);
        set_skill("parry", 1600);
        set_skill("literate", 1500);
        set_skill("shenghuo-lingfa",1220);
        set_skill("taoism",680);
        set_skill("taiji-quan",1650);
        ...
        carry_object("/clone/misc/cloth")->wear();
         carry_object("quest/biao/obj/shenghuoling")->wield();
         carry_object("quest/biao/obj/shenghuoling")->wield();
        carry_object("d/mingjiao/obj/tulong-dao");
        set("chat_chance", 1);
        set("chat_msg", ({
...
void attempt_apprentice(object ob)
{
        ...
        if ((int)ob->query("max_neili", 1) < 5000) {
                command("shake");
                command("say 九阳真气是需要深厚的内力！");
                command("say 你的内力不足，现在我没什么可以教你的！");
                return;
        }
        if ((int)ob->query_skill("parry", 1) < 190) {
                command("shake");
...
{
        if( skill == "jiuyang-shengong" && me->query_skill("jiuyang-shengong", 1) < 200)
