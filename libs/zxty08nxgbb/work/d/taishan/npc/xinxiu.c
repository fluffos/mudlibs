...
void create()
{
    seteuid(getuid());
    if (!restore()) {
      set_name("李云天", ({ "xinxiu shizhe", "xinxiu", "shizhe" }) );
     set("title", "新秀使者" );
    set("gender", "男性" );
    set("age", 35);
      set("long","一位笑容可掬的中年人，他就是乐善好施，爱贫如子的武林新秀使者。\n");
    ...
    carry_object("/d/city/obj/cloth")->wear();        
          ...
          set("id", "xinxiu");
          set_name(query("name"), ({ "xinxiu shizhe", "xinxiu", "shizhe" }));
   set("no_get",1);
         set("startroom","/d/taishan/yuhuang");
...
if(ob->query("daoxin"))
{
  tell_object(ob, "你只不过是个残疾人,也配做当今新秀使者?\n");
return 0;
}
...
int accept_fight(object ob)
{
    ...
    me  = this_object();
                 if( this_player()->query("mud_age") > 500000 ){
           command("say 你太老了吧.去打别的像吧！这个是专门为新手定做的.\n");
     return 0;
  }
    ...
    if( this_player()->query("id")==name1 ){
        command("say 盟主大人，在下新秀使者怎么是您的对手？！\n");
        return 0;
    }
...
int do_copy(object me, object ob)
{
    ...
    ob->delete("party");
       ...
       me->set("short", HIC+"第" + chinese_number(me->query("generation")) + "代新秀使者"+ NOR+" " + me->query("name") + "(Xinxiu shizhe)");
    me->delete("title");
 ...
 me->set("zjb_dj/dj",ob->query("zjb_dj/dj"));
  ...
  save();
    tell_object(ob, "状态储存完毕。\n");
        newob = new("/d/taishan/npc/xinxiu");
      newob->move("/d/taishan/yuhuang");
    destruct(me);
    return 1;
}
...
int do_recover()
{
    ...
    if(me->query("max_neili",1)>ob->query("max_neili",1)+1000
     ||me->query("max_jingli",1)>ob->query("max_jingli",1)+1000)
...
int do_gengxin()
{
    ...
    command("chat 我退出江湖后，新秀使者之位将由李云天少侠暂时接任。\n");
    rm(query_save_file()+__SAVE_EXTENSION__);
    room=new("/d/taishan/npc/xinxiu");
    room->move("/d/taishan/yuhuang");
    destruct(this_object());
    return 1;
}
