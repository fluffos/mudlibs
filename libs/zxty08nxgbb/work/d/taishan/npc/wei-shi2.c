...
void init()
{
    ...
    if( this_player()->query("id") == mengzhu ) {
        message_vision(HIY+query("name")+HIY+"举起长剑，向$N"+HIY+"齐声大喊：盟主万岁，万岁，万万岁！\n" NOR, this_player());
    }   
    else if( this_player()->query("id") == shangshan ) {
        message_vision(HIY+query("name")+HIY+"举起长剑，向$N"+HIY+"齐声大喊：赏善使者好！\n" NOR, this_player());
    }   
    else if( this_player()->query("id") == fae ) {
        message_vision(HIY+query("name")+HIY+"举起长剑，向$N"+HIY+"齐声大喊：罚恶使者好！\n" NOR, this_player());
    }   
}
