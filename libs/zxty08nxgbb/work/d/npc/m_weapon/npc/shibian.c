...
int do_zhu(string arg)
{
    ...
    if( !arg )
        return notify_fail("侍鞭认真的说：想好剑的名称后在对我说。\n");
    sscanf(arg ,"%s" ,w_name );
    id="my whip";
    if(!w_name||!id)
    ...
    message_vision(HIR "只听咯的一声轻响，一条神鞭从炉中倏然跃起。化作一道青电猛的向$N的前胸刺来！！\n" NOR,me );
    ...
    message_vision("神鞭又飞了起来，飞到半空，当的一声落回到地上。\n炉中的火灭了。一室的劲气化于无形，一切又归于沉寂。\n",me );
    me->set("qi",0);
    me->set("jing",0);
    me->set("neili",0);
    ...
    me->set("weapon/rate",rate); //武器材料的等级  
