...
int do_jia()
{
    ...
    string j_id;
    me = this_player();
    j_id = me->query("jia/id");
    if( !(me->query("jia/make")) )
        return notify_fail("你造过防具吗?\n");
    if( objectp(obj = present(j_id, me)) ) {
        say("欧冶子脸色一沉：您的防具不就在你手边么？还来干什么！\n");
        return 1;
    }
    if( !(me->query_temp("conditionj_ok")) ) {
        message_vision(HIY "这位"+RANK_D->query_respect(me)+"想要从新得到防具?这不难.不过......\n" NOR, me);
        message_vision(HIY "欧冶子沉吟了一下说:你得找到我要的东西.\n" NOR, me);
        switch( random(5) ) {
            case 0:
                message_vision("拿到菩提子,再来吧!\n",me);
        ...
        me->set_temp("conditionj",1);
        }
    else    {
        me->delete_temp("conditionj_ok");
        jia = creat_jia();
        jia->move( me );
    }
    return 1;
...
{
    object me,jia;
    string mask;    //the verible that record the type of jia
    me = this_player();
    mask = me->query("jia/type");
    switch(mask)    {
        case "甲":
            jia = new("/d/npc/m_weapon/npc/jia",1);
            return jia;
    }
}