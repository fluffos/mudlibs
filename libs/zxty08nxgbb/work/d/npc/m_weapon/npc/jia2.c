...
void init()
{
    ...
    add_action("do_hui","huihuai");
    add_action("do_rename","gaiming");
     add_action("do_qie","xiang");
    add_action("do_clean","qingchu");
}
 ...
 */      
    if ( me->query("jia/int") ) 
    set("weapon_prop/intelligence",me->query("jia/int"));
    if ( me->query("jia/dex") ) 
    set("weapon_prop/dexerity",me->query("jia/dex"));
    if ( me->query("jia/con") ) 
    set("weapon_prop/constitution",me->query("jia/con"));
    if ( me->query("jia/str") ) 
    set("weapon_prop/strength",me->query("jia/str"));
...
{   
    ...
    me=this_player();
      if( !arg || arg!="kuangshi")
        return notify_fail("请输入qingchu kuangshi清除?\n");
...
int do_xiang(object me,object obj)
{
    ...
    if ( ob->query("armor") )
    me->add("jia/armor",ob->query("armor"));
    me->add("jia/qie",1);
...
//mixed ob_hit(object me, object victim, int damage, int factor) //攻击和挨打反了 疑问中
