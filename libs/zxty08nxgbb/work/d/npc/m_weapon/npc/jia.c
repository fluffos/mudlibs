...
{   
    ...
    me=this_player();
      if( !arg || arg!="metal")
        return notify_fail("请输入clean metal清除?\n");
...
if ( me->query("jia/qie") && me->query("jia/qie")==me->query("jiaqie")+1 ) 
   return notify_fail("这件定做已经镶锲了终极金属了，需要重新镶嵌请先去除原金属(clean metal)!\n");
