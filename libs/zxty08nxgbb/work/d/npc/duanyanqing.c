...
void create()
{
    set_name("段延庆", ({"duan yanqing", "yanqing", "qing","duan"}));
    set("gender", "男性");
...
void init()
{
        ...
        ::init();
//        if (interactive(ob = this_player()) && !is_killing(ob->query("id")) )
        if ( !is_killing(ob->query("id")) && living(this_object()))
    {
