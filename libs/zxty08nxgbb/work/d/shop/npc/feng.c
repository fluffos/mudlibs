...
int stop_auction()
{
        ...
        } else            
        {
                ...
                if (! objectp(the_owner)
                   || (int)the_owner->query("money") < (int)(now_price * 10000))
                {
                	// 取消该玩家的拍卖资格
                        do_ban(temp_owner + " 1");
                ...
                } else
                {
                        ...
                        the_owner->add("money", - now_price * 10000);
                        the_owner->save();
                        owner += ({ temp_owner });
                        ob->set("owner", owner);
...
int do_jupai(string arg)
{
	...
	//  手工拍卖，检查执行巫师是否还在，如果不在则进程转为自动
        ...
        if ((int)buyer->query("money") < (money + now_price) * 10000)
