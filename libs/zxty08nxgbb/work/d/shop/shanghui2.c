...
void create()
{
        set("short",BYEL+HIW"总商会 二楼"NOR);        
        set("long", @LONG
总商会二楼，常来这里的人，都是有能耐有来头的。商会
里围着黑压压的一群人，南方有一扇紧紧关闭的小门。只有在
每月一次的店铺拍卖时「南霸天」凤天南才会将它打开，在那
里会见各处的商贾。小门旁挂着的一块红木牌子(paizi)上标
写着各家店铺拍卖的底价。
LONG);
...
int do_open()
{
        ...
        message_vision(HIY "$N" HIY "轻轻一推，将通向总商会的"
                       "小门开启了。\n" NOR, me);

        shout(HIY "【商界传闻】" NOR + WHT + me->name() + WHT "[" +
              me->query("id") + WHT "]开启了扬州总商会的店铺拍"
              "卖会的大门。\n" NOR);
