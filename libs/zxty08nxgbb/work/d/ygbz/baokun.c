...
"xiang" : "箱子里刻着一些字符，好象是一样武功，你不由想仔细看(think)一下!\n",          
   "jia" : "兵器架上其中有一把刀特别吸引人，你要拿(pick)出来看看吗?\n",      
...
if(query_temp("nasword")){ 
   message_vision("$N想去拿宝刀结果发现已经给人拿了\n", this_player());
     return 1;
 }
                                message_vision(
HIY"$N在兵器架上拿下了一把宝刀!\n"NOR, this_player());
                    sword = new(__DIR__"obj/nhun-dao");
                        sword->move(me);   
set_temp("nasword",1);
 message("channel:chat", HBRED HIW"【天涯】"+me->query("name")+"在杨公宝库里发现了凝魂宝刀了!\n"NOR,users()); 
return 1;
                }
        return 1;
}
...
int do_push(string arg)
{
  ...
  message_vision(HIG"$N轻轻推开墙壁，发现有道暗门!\n"NOR,me);
   set("exits/north", "/d/changan/ca24"); 
