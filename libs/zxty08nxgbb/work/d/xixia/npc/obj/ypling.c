...
int do_behead(string arg)
{
  ...
  if (!arg || arg != "corpse" || !(obj = present(arg, environment(me))))
              return notify_fail("你要behead啥?!\n");
  if (obj->query_temp("must_killby") != me->query("id") )           return notify_fail("这里用不了令!\n");

  if (obj->query("beheaded", 1))           return notify_fail("这里!\n");
...
/*
  if (query("victim") == obj->query("id")
       && query("owner") == obj->query_temp("last_damage_from")) {
*/
//if (obj->query_temp("must2_killby")== me->query("id") && me->query_temp("job2_given") ){
    set("done", 1);
