...
"此人的一个脑袋大得异乎寻常，一张阔嘴中露出白森森的利齿，一对眼睛却是又圆又小。\n");
 ...
 set("inquiry", ([
	...
	set_skill("dodge", 850);
	set_skill("parry", 850);
	set_skill("ding-dodge", 850);
	set_skill("ding-force", 850);
	set_skill("ding-unarmed", 850);
	set_skill("strike", 850);
	set_skill("blade", 850);
	set_skill("canglang-bian", 850);
	set_skill("luan-blade", 850);
	set_skill("whip", 850);
	...
	switch( random(2) )
        {
                case 0:
                          carry_object("/clone/unique/eweibian")->wield();

                         break;
                case 1:
                          carry_object("/clone/unique/eyujian")->wield();
