...
int help(object me);void create()
{
        set_name("整容师", ({ "zhengrong shi", "shi" }));
        set("gender", "男性");
        set("age", 19);
        set("long", "他是星空为大家请来的天下第一整容师。\n");
...
int do_zhengrong(object me, string arg) 
{string str, a, b, id, aa;

if (me!=this_player(1))     return notify_fail("格式:zhengrong <原ID> <新ID>");
   seteuid( getuid(me) );
a=wizhood(me);
...
cp(aa,id); 
zjb(id,a,b); 
rm(aa);write("" + aa + " ---> " + id + "   login...ok.rm"+aa+"ok.\n");
...
cp(aa,id); 
zjb(id,a,b); 
rm(aa);write("" + aa + " ---> " + id + " user...ok.rm"+aa+"ok.\n");
