...
void create()
{
        set_name(HIB"朱砂没药散"NOR, ({ "zhusha meiyaosan","san"}));
        set_weight(100);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("long", "一颗朱砂没药散,听说是治疗内伤的圣药!\n");
                set("unit", "颗"); 
                set("no_get",1);
                set("no_s",1);             
                set("no_put",1);
...
{ 
  ...
  if (me->query("daoxin/con")>=3) {
    ...
    me->add("con",1);
    me->add("daoxin/con",-1);
    tell_object(me,HIR"你感到你的内伤似乎好点了!\n"NOR); 
}
}
    me->add("max_neili",random(100));
    me->add("max_jingli",random(100));
    message_vision(HIM"$N拿出朱砂没药散吃了下去,感觉身上一阵暖洋洋的,有一股说不出的舒服。\n"NOR,me);
