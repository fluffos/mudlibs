...
int ask_degree()
{
 ...
 lv = (ob->query_skill("literate",1) / 300); 
        if (lv >= sizeof(degree_desc)) lv = sizeof(degree_desc)-1; 
      if( ob->is_busy() )
                return notify_fail("你正忙着做其它事呢。\n");

        if (lv < 3) 
        {
                 command("chat "+ob->query("name")+"，你那点墨水也好意思问? 随便给你个学位就算了! \n");
        ...
        if (lv < 10) 
        {
              command("chat "+ob->query("name")+"，呃，不错，有前途，很高兴授予学位给你，继续好好努力! \n");
               ob->set("degree",degree_desc[lv]);
               ob->start_busy(5);
               return 1;
        }
        ...
        {
              command("chat 果然是长江后浪逐前浪啊，"+ob->query("name")+"，我非常荣幸能有机会为你授学位! \n");
               ob->set("degree",degree_desc[lv]);
               ob->start_busy(5);
               return 1;
        }
...
int ask_degree1()
{
        ...
        object ob = this_player();
        if(!ob->query("degree"))return 0;
        {
             command("chat 可惜啊，国家又失去一个栋梁之材了！"+ob->query("name")+"，从此刻开始你就是个布衣平民了！\n");
               ob->set("degree",0);
               ob->start_busy(10);
               return 1;
        }
...
command("say 嘿嘿，我是西门无剑附身，要杀我？等下辈子吧！！\n");
}
