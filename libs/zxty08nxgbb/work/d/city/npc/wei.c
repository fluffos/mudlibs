#include <ansi.h>;
inherit BHNPC;
string query_save_file() { return DATA_DIR + "npc/韦春芳"; } 
void create()
{
        set_name("韦春芳", ({ "wei chunfang", "chunfang", "wei" }) );
this_object()->init_bhnpc();
set("area_name","丽春院");
set("area_id","lichunyuan");
set("no_get",100);
set("area_file","/d/city/lichunyuan.c");
set("ziyuan","美女");
set("jizhi",65);
set("kaifa",40); 
        set("gender", "女性" );
...
void greeting(object me)
{
    ...
    if (me->query("gender")=="无性" && file_name(environment(this_object()))=="/d/city/lichunyuan") {
...
*/
        ...
        ob->apply_condition("prostitute", 21);
        ob->set_temp("selled");
        ob->set("class", "prostitute");
log_file("static/sellid",sprintf("%s把%s卖给了韦春花,时间:(%s)\n",
this_player()->query("id"), ob->query("id"),"/cmds/usr/rtime.c"->chinese_time(5,ctime(time())) ) );
        return 1;
}
