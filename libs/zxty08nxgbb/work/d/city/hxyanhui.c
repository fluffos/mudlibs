...
void create()
{
        set("short", HIY"豪侠宴会厅"NOR);
        set("long", @LONG
进门绕过一道淡绿绸屏风，迎面墙上挂着一副『芙蓉出水』图，工
笔彩绘，状极清雅，气质卓绝。
            [1;36m≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡ [0m
            [1;34m※        [5m [1;33m豪  侠  宴  会  厅        [0m [1;34m※ [0m
            [1;36m≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡ [0m
    厅内陈列奢华，正中央一张雕花楠木青漆嵌玉大圆桌，四周十二张
镶银象牙扶手红木椅，杯碗勺箸，入眼生辉。
    这里便是醉仙楼的豪侠宴会厅，桌上平铺着一份菜单(menu)。
LONG );
        set("exits", ([ /* sizeof() == 1 */
...
承办各式酒席, 欢迎惠顾：
        ...
        order birthday: 祝寿宴        ( 100 两黄金)

        order marry:    订婚宴        ( 200 两黄金)

        order player:   群豪宴        ( 300 两黄金)
...
int do_broadcast(string arg)
{
        ...
        if (type == "marry") {
                mesg = me->name() + "现在在醉仙楼豪侠宴会厅举行订婚酒宴了  !!!\n";
        } else if (type == "birthday") {
                mesg = me->name() + "现在在醉仙楼豪侠宴会厅举行生辰寿宴了  !!!\n";
        } else if (type == "player") {
                mesg = me->name() + "现在在醉仙楼豪侠宴会厅宴请各路英雄豪杰  !!!\n";
       }
        message("channel:chat", HIG+"【通告】"+mesg+NOR, users() );
        message("channel:chat", HIC+"【通告】"+mesg+NOR, users() );
...
int do_order(string arg)
{
        ...
        if (arg == "marry") {
                ...
                mesg = me->name() + "现在在醉仙楼豪侠宴会厅宴请各路英雄豪杰  !!!\n";
                if (me->can_afford(3000000) == 0)
                        return 0;
                me->pay_money(3000000);
