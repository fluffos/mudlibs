...
int do_move(string arg) 
{
        ...
        if( me->name(1) != ob->query("name1") && me->name(1) != ob->query("name2") )
                return notify_fail("你还是先投骰子吧！\n");

        if( !ob->query("name2") )
                return notify_fail("还要有一个人才能下棋！\n");

        if( ob->query("round") == 0 && ob->query("first") != me->name(1) )
                return notify_fail("这个回合不该你走棋！\n");

        if( ob->query("round") == 1 && ob->query("first") == me->name(1) )
                return notify_fail("这个回合不该你走棋！\n");
...
rche+=TABLE[table[tROW]][tCOL][1];
        ...
        if ( TABLE[table[tROW]][tCOL][2] == 5 ){
                ...
                ob->set("over",this_player()->name(1));
        }
...
ob->query("name1") == me->name(1)?ob->query("name2"):
ob->query("name1")) ); 
    ob->set("over", me->name(1)==ob->query("name1")?ob->query("name2"):ob->query("name1"));  
}
}
...
int do_toss() 
{
    ...
    if (!ob->query("over") )
     ...
     if (me->name(1) == ob->query("name1") )
        ...
        if( ob->query("name1") ){
                ob->query("num")>num?ob->set("first",ob->query("name1")):ob->set("first",me->name(1));
                message_vision( ob->query("first")+"使用红色棋子先行。\n",me );
                init_table();
        }

        ob->set("num",num);
        ob->query("name1")?ob->set("name2",me->name(1)): ob->set("name1",me->name(1));
...
int do_deploy()
{
        ...
        if( ob->query("id1") )
                if( (present(ob->query("id1"),environment(ob))) || 
                (present(ob->query("id2"),environment(ob))) )
                        if( me->name(1) != ob->query("name1") && me->name(1) != ob->query("name2") && !ob->query("over") )
