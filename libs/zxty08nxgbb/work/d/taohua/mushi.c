...
周围则堆放了无数的，奇珍异玩。一张供桌摆在玉棺前，桌上放着一个香炉及一
个盘子(panzi)。墓室对面挂这一幅青年少妇的画像，画像上的少妇巧笑嫣然,和
惨白的玉棺及碧油油的珠光形成了一幅奇诡的画面。忽然一阵阴风吹过，你不禁
打了一个寒颤。
LONG);
    set("item_desc", ([
        "panzi" : @TEXT
...
int do_qiao(string arg)
  {
          object sword, book, pker, me=this_player();
  
          if( !arg || arg=="" ) return 0; 

          if( arg=="玉棺" )
                               {  
  if(query_temp("nobook")){ 
     message_vision("$N撬了个空，定神一看，发现已有人早一步盗空了这里。\n", this_player());
     return 1;
   }
                                  message_vision(
 HIC"$N‘叮’的一声撬开玉棺，只见棺盖通体透明，一看就知道是无价之宝！！\n"NOR, this_player()); 

                set_temp("nobook",100);
                me->add("max_neili",-200); 
                me->start_busy(100);
                book = new("/d/pipip/jiuyin2");
                book->move(me);
       pker = new (__DIR__"npc/pker") ;
       message_vision("\n突然墓室中打开了一道暗门，从里面走出一个守墓家奴。\n"
       "对$N喝道：贼子敢尔！连夫人的东西你也敢动！\n", me);
       pker->move(environment(me));
       pker->kill_ob(me);
  message("channel:chat", HBRED HIG"〖江湖通缉令〗"+me->query("name")+"在桃花岛墓地盗取了一批陪葬品，希望各位好汉将其诸杀。\n"NOR,users());  
  return 1;
                  }
          return 1;
  }
