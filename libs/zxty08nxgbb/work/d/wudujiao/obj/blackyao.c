...
//     object *inv;
	...
	if( !victim || victim==me) return notify_fail("没有你想下毒的人哦。\n");
       ...
       if ((int)me->query_skill("duji",1) < 70)    
	...
	if( ob->query("max_liquid"))
       {
		...
		if( random(myskill) < skill/2 )
			message("vision", "你看到" + me->name() + "鬼鬼祟祟地在"
				+ victim->name() + "身上的一" + ob->query("unit")
				+ ob->name() + "里洒入了一些"+name()
...
{
	this_player()->apply_condition("chanchu_poison",
		(int)this_player()->query_condition("chanchu_poison") 
