...
"凤舞地书着『"+RED+"少林寺"+NOR+"』三个大字。寺前三三两两的站着几个僧人。\n"
    );
    set("exits", ([
...
int valid_leave(object me, string dir)
{
    ...
    while(ob){
        if (ob->is_character() && !ob->is_corpse() )
            return notify_fail("你身上背负着的"+ob->name()+"，不能与你一起进寺！\n");
        ob=next_inventory(ob);
    }
    if ( (me->query("family/family_name") == "少林派") && me->query("gender")=="男性")
    {
...
string look_gate()
{
    return "一道三丈来高的朱红杉木包铜大门。经常有些香客和少林弟子敲(knock)门进出。\n";
}
