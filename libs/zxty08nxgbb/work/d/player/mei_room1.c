...
void create()
{
        set("short", "卧室");
        set("long", @LONG
屋子布置的精致富丽，当中一张青玉案，案上两只白玉瓶，瓶里插着几
十枝玫瑰。玉瓶旁铺着几张信筏，放着些笔墨砚石，未入此处，清新的花香
已沁人心裨。南边有扇窗子(window)，景色宜人。一张绣榻放在墙角，两只
金钩轻搭纱帐。房间虽不大，但是给人清新舒适的感觉。
LONG
        );
        set("exits", ([ 
  "down" : "/u/mei/tianying/elou",
  "north" : "/u/mei/workroom",
]));
    set("item_desc", ([
        "window" : 
...
int do_jump(string arg)
{
    ...
    if (arg != "window" && arg != "out") return command("jump "+arg);
        ...
        message_vision("$N提一口气，姿态潇洒地飘落在下面的大榕树上！\n", me);
     me->move("/d/city/chatroom");
    return 1;
}
...
{
    message_vision( 
     HIM "    就在两人阴阳交融的瞬间中, 几年来积存种种相思的情欲火山爆发似地
     奔放出来, $N不同於刚才的温存，像是要把$n完全融化似地, 只见金钩轻摇，
     被翻红浪, $n以不间断地低吟著更激起$N的欲望, $n觉得此时此刻最美, 最
     令人陶醉, 令人....... \n" NOR,me,obj);
}
void update_user(object user)
{
