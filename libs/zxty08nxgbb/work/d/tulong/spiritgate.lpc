...
{ 
   ...
   member = this_player()->query_team();
     ...
     if( this_player()->query("combat_exp")<20000) return notify_fail("你这样的能力是进不了精灵界的。\n");
    if( this_player()->query_temp("m_success/孽龙")&& 
    this_player()->query("m_success/wait")>time())
     return notify_fail("别急,你现在进不了。\n");
   if( sizeof(member) < 2 )
     return notify_fail(HIG"如果队中成员数目不为二或二以上，精灵之门不会开启。\n"NOR);
