...
{
        object who,ob;
        int money,clone_number,paimai_count,i,value,sale_price,zjb;
        string id,file_name;
        mapping fpaimai,vpaimai,ipaimai,npaimai,lpaimai,opaimai,spaimai;
        string* terms;
        
      if ( !this_object()->query("rm")||this_object()->query("rm")<time() )
        this_object()->rm_ob(this_object());
        ...
        who=this_player();
        zjb=who->query("zjb_money");
        
        //add by Amis@XO2
        if( this_object()->query("startbid") )
                return notify_fail("拍卖师不耐烦地道：没见我正忙着拍卖吗，请等一会吧！\n");
        
        
        if( who->is_busy() )
         return notify_fail("你的动作还没有完成，不能提交物品进行拍卖。\n");
    if( !arg || sscanf(arg, "%s with %d",id, money)!=2 )
                return notify_fail("指令格式：paimai <物品id> with <底价(星空币为单位)> \n");
        if( !ob=present(id, who))
                return notify_fail("你身上没有这样东西。\n");
        if( ob->query("money_id") )     
                return notify_fail("你要拍卖「钱」？\n");
        if( ob->query("ty_gift") )     
              return notify_fail("此物只因天上有！快走快走！\n");

/*
        if (ob->query("value")<10 || money==0)
                return notify_fail("拍卖师说道：这件东西不值钱，你就别卖了！\n");
*/
        if(!wizardp(who)&&who->query_temp("paimai_time")&& time() < who->query_temp("paimai_time")+30)
                 return notify_fail("你刚刚拍卖完东西，目前不能参与拍卖。\n");
        if (ob->query("age")||ob->query("combat_exp"))
           return notify_fail("拍卖师说道：你想贩卖人口啊！\n");
        if( ob->query("nopaimai") )
                return notify_fail("拍卖师说道的说：这是孤品啊，我们这里可不敢承接这个买卖了！\n");
        if(money < 1)
                return notify_fail("拍卖师说道：您想做赔本生意，嘿嘿，您财大气粗无所谓，小店的声誉可是要受损的。\n");
        if(money > who->query("zjb_money")*4)
           return notify_fail("您的户头上的钱连拍卖起价的1/4也没，想捣乱啊???\n");
   
        command("sys "+who->query("name")+"("+who->query("id")+")"+NOR+ob->query("name")+HIC+"进行拍卖。\n"NOR);
        command("chat "+"有人提交"+NOR+ob->query("name")+HIC+"进行拍卖，请使用"+HIG"【和气生财】"NOR+HIC"频道（ＳＡＬＥ）了解具体情况。"NOR);
        delete("paimai_id");
        set("startbid",1);
        who->set_temp("paimai_time",time());
        who->set_temp("on_paimai",1);//记录提交拍卖，不允许退出quit
        ...
        this_object()->set("fpaimai/"+ sprintf("%d",query("total_number")),file_name);
        ...
        set("jpaimai/"+sprintf("%d",query("total_number")), ob->query("bh"));
...
log_file("static/paimai_move1",who->query("name")+"("+who->query("id")+")"+"开始拍卖物品"+ob->name()+",时间是"+"/cmds/usr/rtime.c"->chinese_time(5,ctime(time()))+"\n");
        if(!ob->query_unique())
                destruct(ob); 
        else
        {
...
int begin_paimai(string name,int money,int num,int m)
{
        ...
        if (m==1)
        {
                command("sale "+"现在开始拍卖 "NOR+name+HIG+" 的所有权。"+NOR);
        }
        set("on_paimai",1);
        mon = to_ch(money);
        command("sale "+NOR+name+HIG+"底价"+mon+"第"+chinese_number(m)+"次");
        command("sale "+name+HIG"是本店第『"+chinese_number(num)+"』号拍卖品，有意者请使用ＢＩＤ命令进行竞买。\n"NOR);
...
int finish_paimai(int num,int sign)
{
        ...
        if (sign==0)
        {
                command("sale "+query("npaimai/" + sprintf("%d", num))+HIG+" 的所有权拍卖流标了。"+NOR);
                if(owner)
{
//          owner->delete_temp("on_paimai");//delete提交拍卖记录
        ...
        {
                command("sale  成交!"+buyer->query("id")+"获得了 "+NOR+query("npaimai/" + sprintf("%d", num))+HIG+" 的所有权。"+NOR);
        }
        else
        {
                command("sale 成交! 有人获得了 "+NOR+query("npaimai/" + sprintf("%d", num))+HIW+" 的所有权。"+NOR);
                command("sys  成交!"+buyer->query("id")+"获得了 "+NOR+query("npaimai/" + sprintf("%d", num))+HIG+" 的所有权。"+NOR);
        buyer->delete_temp("on_paimai");//delete提交出价记录

        }
                if (buyer)
{
                        tell_object(buyer,HIR"你获得了 "+NOR+query("npaimai/" + sprintf("%d", num))+HIR+" 的所有权，请在十分钟内来扬州中央广场取回你买的东西。\n"NOR);
        buyer->delete_temp("on_paimai");//delete提交拍卖记录

}
                if (owner)
                        tell_object(owner,HIR"你的"+NOR+query("npaimai/" + sprintf("%d", num))+HIR+"卖了"
                        +chinese_number(query("vpaimai/" + sprintf("%d", num)))+"枚星空币，已经加入你的户头了。\n"NOR);     
       owner->delete_temp("on_paimai");//delete提交拍卖记录

//          owner->add("zjb_money",query("vpaimai/" + sprintf("%d",num)));
        }
    if(buyer)
        ...
        if (query("pnum"))
            pay_back(owner_id,query("vpaimai/" + sprintf("%d", num)));
...
int report_paimai(object buyer,int money,int num)
{
   ...
   {
                pay_back(query("ipaimai/" + sprintf("%d", num)),query("vpaimai/" + sprintf("%d", num)));//归还前一个竞买者的钱?
               add("pnum",1);
      }
      buyer->add("zjb_money",-money);
      buyer->set_temp("on_paimai",1);//设置bid记录，防止quit
     ...
     mon =to_ch(money);
          if(!buyer->query_temp("invisibility_buyer"))
      {
          if (!query("pnum"))
              add("pnum",1);
                command("sale  "+buyer->query("id")+" 出价 "+mon+"购入"+NOR+query("npaimai/" + sprintf("%d", num))+HIG"的所有权。");
     ...
     else
     {
       ...
       command("sale  有人出价 "+chinese_number(money)+"枚星空币购入"+NOR+query("npaimai/" + sprintf("%d", num))+HIG"的所有权。");
...
int pay_back(string id, int amount)
{
    ...
    if(who)
        {
                who->add("zjb_money",amount);
      who->delete_temp("on_paimai");//删除bid记录

tell_object(who,HIR"你刚才参加拍卖的钱现在还给您！\n"NOR);
                if(!who->save(1)) 
                {
...
int go_paimai(string id,string name,int money,int num,int m)
{
        ...
        m+=1;
        mon = to_ch(money);
        command("sale "+id+" 出价 "+mon+"购入 "+NOR+name+HIG+" 的所有权，第"+chinese_number(m)+"次");
        command("sale "+name+HIG"是本店第『"+chinese_number(num)+"』号拍卖品，有意者请使用ＢＩＤ命令进行竞买。\n"NOR);
        if (!(m==3))
...
int do_yao()
{
        mapping fpaimai,vpaimai,ipaimai,npaimai,lpaimai,spaimai,jpaimai;
        string* terms;
        string id;
        ...
        if( mapp(fpaimai) &&  sizeof(fpaimai) )
        {
                ...
                {
                        if (ipaimai[terms[i]]==me->query("id") && lpaimai[terms[i]]==0)
                        {
                                ob=new(fpaimai[terms[i]]);
                                if (spaimai[terms[i]]>0)
                                        ob->set_temp("sale_price",spaimai[terms[i]]);
                                if (jpaimai[terms[i]]) {
                                ob->panduan(jpaimai[terms[i]]);
                                ob->create();
                                }
                                ob->move(me);
log_file("static/paimai_move2",me->query("name")+"("+me->query("id")+")"+"得到拍卖物品"+ob->name()+",时间是"+"/cmds/usr/rtime.c"->chinese_time(5,ctime(time()))+"\n");
               ...
               me->delete_temp("on_paimai");//delete提交拍卖记录
                         return 1;

                        }
                }
        }
        tell_object(me,"这里没有属于你的物品。\n");
        return 1;
}
...
int rm_ob(object me)
{
      if ( this_object()->query("rm")<time() ) {
      rm("/data/paimai.o");
      me->set("rm",time()+72000);
      me->save();
      command("sys 拍卖系统更新完毕！");
      }
}
void unconcious()
{
