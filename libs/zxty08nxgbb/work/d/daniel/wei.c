...
*/
        ...
        if(lp->query("bangzhu_id")=="???")      {
              message_vision("$N提醒$n道：“帮主已失踪多日，快去找天界之神商量商量吧。\n",this_object(),ob);
                return;
        }
...
int do_withdraw(string arg)
{
   ...
   if (what!="gold")  {
    return 
notify_fail(query("name")+"道：只能取黄金！\n");
}
        if (amount==1 && what=="coin")  {

                return notify_fail(query("name")+"怒道：都象你这样，岂不要俺吃西北风去？！\n");
        }
      what = "/clone/money/gold";

   if (amount>2000)
{
return notify_fail(query("name")+"怒道：你取那么多钱去陪葬呀？！\n");
}
