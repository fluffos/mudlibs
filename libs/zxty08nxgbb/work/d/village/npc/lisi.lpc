...
int do_name(string target)
{
   ...
   if (base_name(environment())!=query("startroom")){
        return notify_fail("李四说道：这种事在这里说不太妥当吧！\n");
     }
       if( this_player()->query("age")<19 ) 
        return notify_fail("李四说道：你这小孩，没准是在耍我，不去！\n");
    if( !target || target==" ") 
    ...
    CHANNEL_D->do_channel(me,"sys",sprintf("%s雇佣李四杀%s",this_player()->query("name"),dest->query("name")));
    ob=new(__DIR__"obj/paper");
       ob->set("long",sprintf("你打开草纸一看，上面字迹模糊地写着：\nXX%sXXXXX雇佣***李XX杀X**%sXXXXXX\n\n\n",this_player()->query("name"),dest->query("name")));
    ob->move(this_object());
...
int accept_object(object who, object ob)
{
        ...
        && !this_object()->query("waiting_target")) 
    {
        ...
        if (i>2500) i=2500;
		if (who->query("age")<19){
		   tell_object(who,"李四把钱小心收好，笑嘻嘻地说：小孩，北京王府井大街卖冰糖葫芦的老头\n是俺干爹，你给我钱了，俺和他说声，日后你想吃冰糖葫芦了，保证不收你\n.....一个子！\n");
		   return 1;
		}
