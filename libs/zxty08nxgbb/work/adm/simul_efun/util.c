...
int exec( object me, object ob )
{
       if ( previous_object() ) {
if ( base_name(previous_object()) != "/adm/daemons/logind" 
          && ((int)wiz_level(me) > (int)wiz_level(ob) || geteuid(me)=="zmud" )
          && base_name(previous_object()) != "/adm/daemons/wzd_log")  {
       if ((int)wiz_level(me) > 0 )
       log_file( "login/exec",sprintf("%s从%s登录企图使用exec()
       Efun进入%s身体,来自原文件%s,时间发生与(%s)。\n", 
       ob->query("name")+"("+ob->query("id")+")", 
       query_ip_number(ob),
       me->query("id"),
       base_name(previous_object()),
       ctime(time()) )); 
       write(BLINK HIR"你想越权吗？\n"NOR);
       return 1;
       }
       }
     return efun::exec(me,ob);
}
