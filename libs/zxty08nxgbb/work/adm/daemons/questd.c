...
int quest_reward(object me, object who,object quest_item)
{
      ...
      exp =450+random(1300)+((me->query("int"))*random(80));
        pot = exp/8;
        me->add("combat_exp",exp);
        me->add("potential",pot);
  tell_object(me, sprintf("Äã»ñµÃÁË"HIR"%s"NOR"µÄ¾­ÑéÖµ ,"HIY"%s"NOR"µãÇ±ÄÜ£¡\n"NOR, chinese_number(exp),chinese_number(pot)) );
CHANNEL_D->do_channel(this_object(),"rumor",sprintf(HIG+me->name()+NOR"½«Ñ°µ½µÄ"HIW+quest_item->name()+NOR"ËÍ¸ø"+who->name()+"»ñµÃ"HIR+chinese_number(exp)+NOR"µÄ¾­Ñé£¬"HIY+chinese_number(pot)+NOR"µãÇ±ÄÜ\n"));
         score=me->query_temp("dynamic_find");        
                        if(score+1==10){
            me->delete_temp("dynamic_find");
            me->add("combat_exp",1000);
            tell_object(me,"ÄãÕæÐÐ£¬ÄãµÃµ½ÁË"+chinese_number(1000) + "µãÎäÑ§¾­ÑéµÄ¸½¼Ó½±Àø£¡\n");        
        }
