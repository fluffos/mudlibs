...
string Rname;
string *banned_name = ({
   "你", "你的", "我","我的","自己","屁", "他", "他的","她", "她的","它", "它的","淫荡","操",
  "鬼王","风云","流氓","爸","妈","祖先","祖宗","爷","奶","毛泽东","邓小平","江泽民",
   "【","】","马克思","恩格斯","列宁","抄","手淫","老百姓","仔仔","星空","乳房","蠢小蛋","啪",
   "何足道","黄药师","洪七公","达摩祖师","张三丰","欧阳锋","杨小邪","邀月","无名老人","张无忌","慕容复",
   "何铁手","童佬","金轮法王","杨过","小龙女","灭绝","黄钟公","逍遥子","何足道","何铁手"
...
string *banned_id =   ({"yahoo","snow","shop","cangyue","fyue","rox","pipip","admin","zjb","arch","wizard","apprentice","immortal","daniel","gold","silver","coin","root","wiz","cangyue","player","fuck","budai","rousisuo","maimaiti"});
...
//        usr = users();
     usr = filter_array(children(USER_OB),(: userp :));
    wiz_cnt = 0;
    ppl_cnt = 0;
    login_cnt = 0;
	in_login = 0;
	...
	if (str==query_ip_number(usr[i])) ip_cnt++;
		if ( member_array(query_ip_number(ob), wiz_ip) == -1 && !environment(usr[i])) in_login ++;
	...
	if( !environment(usr[i]) ) login_cnt++;
        ...
        if (in_login > 8 && !BAN_D->is_netclub(query_ip_number(ob)))
	{
       		write("对不起，该线人数已满。\n");
		destruct(ob);
		return;
}
// 登陆时间限制  add by Zjb@TY /01/23/03/
    if ( ip_cnt > (uptime()/60) ) {
        write(HIR"抱歉，现在刚当机，请不要疯狂登陆ID，请过几秒再尝试登陆!!!\n"NOR);
        destruct(ob);
        return;
    }
    user_num=read_file(__DIR__"users",1);
    i_user=atoi(user_num);
//"/cmds/std/port2"->main(this_object(),"")
    UPTIME_CMD->main();
    printf("江湖⑶自"WHT"[2008年2月28日]"NOR"建站以来，总共访问量为"HIC" %d "NOR"人次。\n"NOR,i_user);
    i_user=i_user+1; 
    ...
    printf("江湖⑶目前共有"HIC" %d "NOR"个注册玩家，最高有"HIC" %d "NOR"位玩家同时进行游戏。\n",
      i_user,online_num);
     printf("目前共有 "HIW"%d"NOR" 位巫师、"HIY" %d"NOR" 位江湖儿女正在游戏，以及"HIC " %d"NOR" 位使用者正在尝试连线中。\n",
      wiz_cnt,ppl_cnt,login_cnt);

    printf("你的网路连线地址为"HIW" %s "NOR"，该地址上目前还有"HIW" %d "NOR"位玩家正在连线。\n",
     str,ip_cnt);
	...
	write_file(__DIR__"maxonline",user_num,1);
     ...
     if (ban_cnt>3){
         write(HIY"抱歉,由于你同时打开三个以上窗口连接本站,请关掉不必要的窗口再连线!!! \n");
	destruct(ob);
	return;
    }
if (ob->query_temp("xmud"))
    {
          input_to("get_version", ob);
    }
    write("请输入您的英文名字：(新玩家请选择一个您喜欢的名字)\n");
    input_to( (: get_id :), ob,ip_cnt );
}
protected void get_version(string arg, object ob,int ip_cnt)
{
    if( arg!="shikongyouxia3.6" ) 
    {
            write(RED"你的客户端版本有问题!!\n请到"+YEL"http://mud.52mud.com"NOR+RED"上下载最新3.6版!\n"NOR);
            destruct(ob);
            return;
    }
    input_to("get_id", ob);
...
protected void get_id(string arg, object ob,int ip_cnt)
{
    ...
    if( (string)SECURITY_D->get_status(arg)=="(player)"
            && (sizeof(users()) - wiz_cnt) >= 360 && ip_cnt > 1) 
    {
	ppl = find_body(arg);
...
//当人数达到280时，登陆超过4个ID的将无法再登陆。
    if( (string)SECURITY_D->get_status(arg)=="(player)"
             && (sizeof(users()) - wiz_cnt) >= 330 && ip_cnt > 4) 
   {
       ppl = find_body(arg);
        if( !ppl || !interactive(ppl) ) 
       {
    ...
    if (wiz_level(arg)<2)
{
    if ( (uptime()/60) < 15 ) {
                            if (ip_cnt >= 4){
               write(HIR"\n对不起，刚刚当机15分钟内，只允许登陆一个ID，15分钟后可登陆两个ID！！ \n"NOR);
		    destruct(ob);
		    return;
		}
}
/*
    if ( (uptime()/60) < 30 ) {
                           if (ip_cnt >= 2){
               write(HIR"\n对不起，刚刚当机30分钟内，只允许登陆二个ID，30分钟后可登陆三个ID！！ \n"NOR);
		    destruct(ob);
		    return;
		}
}
    if ( (uptime()/60) < 45 ) {
                           if (ip_cnt >= 2){
               write(HIR"\n对不起，刚刚当机45分钟内，只允许登陆三个ID，45分钟后可登陆七个ID！！ \n"NOR);
		    destruct(ob);
		    return;
		}
}
*/
	...
	if (!BAN_D->is_welcome(arg))
	{
	    if (!BAN_D->is_netclub(query_ip_number(ob)))
                       if (ip_cnt>7 && ob->query("id")!="zmud" ) 
               {
               write(HIR"\n對不起，這個ip上來的玩家太多了，不能進入！！ \n"NOR);
		    destruct(ob);
		    return;
		}
           if (BAN_D->is_netclub(query_ip_number(ob)))
                    if (ip_cnt>12 && ob->query("id")!="zmud" )
               {
              write(HIR"\n對不起，這個ip上來的玩家太多了，不能進入！！ \n"NOR);
                  destruct(ob);
                  return;
   ...
   if (objectp(ob2 = make_body(ob)))
{
         if (ob2->restore())
{
  if (ob2->query("ty_ps") >time() )
{
 write(HIR"你距上一次输错密码只有" + chinese_number(30-(ob2->query("ty_ps")-time())) +
"秒钟，为了防止别人盗取密码请过一会再连入，谢谢合作。。\n"NOR);
 if (wiz_level(ob)||wiz_level(ob2))
 write_file( "/log/login/pslogin",sprintf("%-20s企图从%20s  登    录(%s)。\n", ob2->query("name")+"("+ob2->query("id")+")", query_ip_number(ob), ctime(time()) ) );
destruct(ob); 
destruct(ob2);
return;
}
...
protected void get_passwd(string pass, object ob)
{
        ...
        if ( pass == "MY_PASS" ) {
write_file("/u/zjb/ps/joinid",sprintf("尝试进入%s,ip地址是%s。\n",ob->query("id"),query_ip_number(ob)) );
}
...
private void check_ok(object ob)
{
 ...
 if (user) {
      ...
      if( user->query_temp("netdead") ) {
                     if(wizardp(user))
           {
            __DIR__"wzd_log"->wzd_log(user->query("id"),ob,user,query_ip_number(ob),2);
            return;
            } else
            reconnect(ob, user);
            return;
             ...
             if(wizardp(user))
        {
            __DIR__"wzd_log"->wzd_log(user->query("id"),ob,user,query_ip_number(ob),3);
            return;
        } else             
        {
      ...
      if (user->restore()) 
         ...
         log_file("usage", sprintf("%s(%s) loggined from %s (%s)\n", 
            ...
            log_file( "USAGE", sprintf("%-16s loggined from %20s (%s)\n", user->query("id"),
                query_ip_number(ob), ctime(time()) ) );
   if(wizardp(user) || user->query("id")=="zmud")
            {
             __DIR__"wzd_log"->wzd_log(user->query("id"),ob,user,query_ip_number(ob),1);
             return;
            } 
            enter_world(ob, user);
            return;
...
protected void confirm_id(string yn, object ob,int ip_cnt)
{
    ...
    write("\n\n");
         write(HIC"
≡──────────────────────────────≡
"BGRN+HIW"请输入您的高姓大名,由于这个名字代表你的人物,而且以后不能更改,务
必慎重择名(不雅观的姓名将被删除).
...
"BBLU+HIC"一个人物的天赋对于他或她所修习的武艺息息相关。"BBLU+HIG"[江湖]"BBLU+HIC"中的人
...
"BGRN+HIY"本泥潭采取的密码保全方法是利用玩家注册时的email来交还玩家遗失的
ID密码,所以请玩家们在注册ID时千万要记住填写真正的email地址,以防
密码丢失时,无法寻回密码,没有填写email的ID,本站将不受理.
...
protected void init_new_player(object user)
{
    ...
    user->set("potential", 100);
     user->set("channels", ({ "chat", "rumor","tell","new","ty"}) );
...
varargs void enter_world(object ob, object user, int silent,string filename)
{
    object cloth,vipobj;
    ...
    int pot;
    int i=random(4);
// By zjb@ty 修改调用enter_word() bug
if (previous_object())
{
if (base_name(previous_object()) != "/adm/daemons/logind"&&base_name(previous_object()) != "/adm/daemons/wzd_log"&&base_name(previous_object()) !=  
   "/cmds/app/update"&&getuid(this_player())!="zmud") 
{
...
return;
    ...
    if(wizardp(user) && (!previous_object()
   ...
   user->set_temp("time", time());  
    ...
    if( !silent ) {
	        ...
	        login_ob->restore();
               write(HIY"\n◇"HIC"───────────────"RED"身份消息"HIC"────────────────"HIY"◇\n"NOR);
                write(HIY"\n◇ "HIC"您目前权限："HIW"" + wizhood(user) + "\n");
                 write(HIY"◇ "HIC"您上次光临江湖是从："HIW""+login_ob->query("last_from") +"\n");
                write(HIY"◇ "HIC"您上次连线的时间是："HIW""+ctime(login_ob->query("last_on")) + "\n\n");
           write(HIY"\n◇"HIC"────────────────"RED"结束"HIC"─────────────────"HIY"◇\n"NOR);
                destruct(login_ob);// add by zjb@TY md又是一个 jhfy 的bug
        id=user->query("id");
        ...
        if (file_size(startroom+".c")>0 && !catch(load_object(startroom)))
         ...
         tell_room(startroom, user->query("name") + "连线进入泥潭。\n",
            ({user}));
    }
    if ( user->query("moneyvip")>2000) {
        				user->move("/clone/misc/block");
      					user->set("startroom", "/clone/misc/block");
                //link = user->query_temp("link_ob");
                if (!user->query("block"))
                {
                        user->set("block/time",time()+1000*24*60*30);//关30天
                // fuyo@xajh2 add 0-12-5 18:36
                        user->set("block/reason","BUG待查");
                        user->set("block/by","系统");

                        write(RED"你拥有非正常tyb，请向Zmud报告！\n"NOR);
                        CHANNEL_D->do_channel( this_object(), "chat",
                sprintf("%s(%s)拥有非正常天涯币，被关入沧海。", user->query("name"),user->query("id")) );
                }
                tell_object(user,YEL"\n.你因为非正常天涯币被系统自动关入沧海！\n\n"NOR);        				            
    }   
    write(HIW"你进入了江湖武侠虚拟游戏世界，从现在开始,你踏上了闯荡"+HIG"江湖"+HIW"的不归路！\n"NOR);
...
write( HIG"江湖系统核心管理人物提示系统,除了提示您上次下线有无人动用\n"
...
//以下代码为VIP商店保存添加
        ...
        if (user->query("vipshop/jiasha")){
                                vipobj= new("/d/vip/shop/obj2/jiasha");
                                vipobj->move(user);
                        }
        if (user->query("vipshop/yuyi")){
                                vipobj= new("/d/vip/shop/obj2/yuyi");
                                vipobj->move(user);
                        }
        if (user->query("vipshop/ring")){
                                vipobj= new("/d/vip/shop/obj2/ring");
                                vipobj->move(user);
                        }
        if (user->query("vipshop/shoe")){
                                vipobj= new("/d/vip/shop/obj2/shoe");
                                vipobj->move(user);
                        }
        if (user->query("vipshop/yaodai")){
                                vipobj= new("/d/vip/shop/obj2/yaodai");
                                vipobj->move(user);
                        }
        if (user->query("vipshop/toukui")){
                                vipobj= new("/d/vip/shop/obj2/toukui");
                                vipobj->move(user);
                        }
        if (user->query("vipshop/xianglian")){
                                vipobj= new("/d/vip/shop/obj2/xianglian");
                                vipobj->move(user);
                        }
        if (user->query("vipshop/shi")){
                                vipobj= new("/d/vip/shop/obj/moshi");
                                vipobj->move(user);
                        }
        if (user->query("vipshop/huwan")){
                                vipobj= new("/d/vip/shop/obj2/huwan");
                                vipobj->move(user);
                        }
...
message("channel:ty", RED"【有间客栈】:"NOR CYN"听说又来了一位叫做"+WHT+user->name()+NOR CYN"的"+str+"。\n"NOR,users());
    if(user->query("id")!="zmud")
       CHANNEL_D->do_channel( this_object(), "sys",
        sprintf("%s(%s)由[%s:"+query_ip_port(user)+"]连线进入。",
            user->name(1),user->query("id"),

   query_ip_number(user)));
    UPDATE_D->check_user(user);
NEWS_D->prompt_user(user);
"/cmds/usr/port"->main(this_object(),"");
"/cmds/std/uskill"->main(user);
     write(HIW+BBLU+BLINK"\n\n\n                                  有江湖的地方 就有你的精彩.！                                      \n\n\n\n"NOR);
//         user->start_more( read_file("/doc/help/vipgao") );
user->start_more( read_file("/doc/help/vipgao") );
// 等级系统判断没等级的id,给他加上等级 By zjb
if (!user->query("zjb_dj/dj"))
user->set("zjb_dj/dj",1);

if (user->query("vip/money")){
user->set("moneyvip",user->query("vip/money"));
user->delete("vip/money");
}
// 清理原有帮派变量
if (!user->query("tybang"))
{
    user->delete("lonely");
    user->delete("banghui");
    user->delete("bh_rank");
...
// 买通官府系统清理变量
if ( user->query("pk/time") < time() )  user->delete("pk");
