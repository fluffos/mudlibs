...
/* global vars */
// 定义我们mud的端口和数据传输套接的id。
private int my_port, socket_id;

// 这个mapping包含了我们mud里面的一些信息。
private mapping muds;

// 这个变量保存了每个域名里的mud的入口，以及关于协议支持的服务的信息。
...
string *GEN_SERVICE=({"gtell", "gchannel", "mudlist_q", "mudlist_a", "ping_q", "ping_a", "gfinger_a", "gfinger_q" }); 
...
int startup_udp()
{
        ...
        CHANNEL_D->do_channel(this_object(),"debug",
                "无法绑定socket到UDP服务上!error="+err_no+"。\n");
#endif
...
// 这个函数被udp服务使用来发送packets
void send_udp(string host, int port, string msg)
{
        int sock;

        if (!ACCESS_CHECK(previous_object())//AUX_PATH在/include/net/dns.h里定义
       ...
       if (args["NAME"]) {
        ...
        while( i-- ) {
           ...
           if( bootsrv[0] == addr && bootsrv[1] == args["PORTUDP"] )
                                flag = 1;
           if( args["name"] == Mud_name() ) flag=1;
        }
        // we now execute the function we have received      

       if( flag > 0 ) 
       if (file_size(AUX_PATH + func + ".c") > 0 && member_array(func,GEN_SERVICE) != -1)
       {
//               CHANNEL_D->do_channel(this_object(), "sys",args["NAME"]+" "+
//                    args["HOSTADDRESS"]+" "+args["PORT"]+" apply to "+func+"  connect!");

                (AUX_PATH + func + ".c" )->incoming_request(args);

/*
                for( i=0; i<sizeof(keys(args)); i++)
                log_file("dns_log",keys(args)[i]+":"+values(args)[i]+" ");
                log_file("dns_log","\n***"+func+"***\n");
*/
...
void send_shutdown()
{
        ...
        socket_close(socket_id);
        CHANNEL_D->do_channel(this_object(), "sys", "MUD互联已经被终止。 \n");
}
...
string start_message()
{
		...
		sizeof(ty_users()), TCP_SERVICE_LEVEL);
}
...
void init_database()
{
        ...
        CHANNEL_D->do_channel(this_object(), "sys", "MUD互联完成。\n");
        return;
       }
        ...
        while( i-- ) {
                ...
                PING_Q->send_ping_q(bootsrv[0], bootsrv[1]); 
                MUDLIST_Q->send_mudlist_q(bootsrv[0], bootsrv[1]);
        }
        remove_call_out("init_database");
...
void refresh_database()
{
        ...
        if(find_call_out("refresh_database") != -1) 
        return;
...
/*
        if ( !undefinedp(junk["USERS"]) || junk["USERS"]<1 )
        {
          zap_mud_info(name, junk); 
          return;    
        }
*/
      ...
      //if this is a new mud or has lost connection.
          ...
          junk["MUDNAME"]+"("+junk["NAME"]+")\n IP:"+junk["HOSTADDRESS"]+" "+junk["PORT"]+"加入互联！"+"人数:"+junk["USERS"]+"人！\n");
...
//      if (ACCESS_CHECK(previous_object()))
		...
		(["USERS":sprintf("%d",sizeof(ty_users())) ])
        ]);
}
...
/*
        ...
        if (!euid || !member_group(euid, "zhuguan") && !member_group(euid, "socket"))
                return;
*/
        monitor = ob;
}
