...
void ready_to_start()
{
        ...
        call_out("vipjob", 1);//首次调用
        
        call_out("start_story", 1100 + random(210));//间隔时间
...
void vipjob()
{
	...
	local = localtime(time());
        ...
        if( local[LT_HOUR] > 18 && local[LT_HOUR] < 21 )        
				{
                                        h = 21 - local[LT_HOUR];
					m = 60 - local[LT_MIN];
					s = 60 - local[LT_SEC];
        	...
        	time += chinese_number(s) + "秒";*/
					message("system", HIC"\n\n【天涯公告】现在是VIP用户双倍任务奖励时间 ！\n\n"NOR, users());
					message("system", HIC"\n\n【天涯公告】距离结束时间还剩下 "+h+ "小时"+m+ " 分"+s+ " 秒"+"。\n\n"NOR, users());
				}
				
				remove_call_out("vipjob");        
        call_out("vipjob", 1800);//每次调用间隔时间
}
...
void start_story()
{
        ...
        running_story = ob;
        running_story->set("story_name",name1);
