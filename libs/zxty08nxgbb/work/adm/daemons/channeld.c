...
mapping channels = ([
   ...
   "sys":  ([  "msg_speak": HIR "【系统】%s：%s\n" NOR,
    ...
    "adm":  ([  "msg_speak": NOR+HIW+"【天神】"+HIW+"%s：%s\n" NOR,
    ...
    "wiz":  ([  "msg_speak": NOR+HIY+"【巫师议事】"+HIY+"%s：%s\n" NOR,
           "msg_emote": NOR+HIY+"【巫师议事】"+HIY+"%s" NOR,
            "msg_color": NOR+HIY,           
            "wiz_only": 1,
            "intermud": GCHANNEL,
            "intermud_emote": 1, 
...
//                      "filter":1,
    ...
    "debug":([  "msg_speak": HIW "【调试】%s：%s\n" NOR,
     ...
     "chat": ([  "msg_speak": HIC+"【"+HIC"笑谈天涯"NOR+HIC"】%s：%s\n" NOR,
              "msg_emote": HIC+"【"+NOR+WHT"笑谈天涯"NOR+HIC"】"+"%s" NOR,
            "msg_color": NOR+HIC,
//          "intermud": GCHANNEL,
//           "intermud_emote": 1,
          "channel": "chat",
...
/*
          "new": ([  "msg_speak": HIG+"【"+HIW+"初出茅芦"+HIG+"】%s：%s\n" NOR,
      "msg_emote": CYN+"【"+HIG+"初出茅芦"+CYN+"】"+HIG+"%s" NOR,
            "msg_color": NOR+HIG,
                        
        ]),
*/
   "rumor":    ([  "msg_speak": YEL "【天外飞语】"HIM"%s：%s\n" NOR,
      "msg_emote": YEL "【天外飞语】"HIM"%s" NOR,
            "msg_color": NOR+HIM	,
            "anonymous": "某人",
        ]),
  ...
  "es":   ([  "msg_speak": HBK+"【天涯传说】%s：%s\n" NOR,
   "msg_emote": HBK"【天涯传说】%s" NOR,
            "msg_color": NOR+HBK,
            "intermud": GCHANNEL,
            "intermud_emote": 1,
            "wiz_only": 1,
      ...
      "ty":   ([     "msg_speak": HIG "【浪迹天涯】"+NOR+HIG+"%s：%s\n" NOR,
       "msg_emote": YEL "【天涯浪迹】"+NOR+HIG+"%s" NOR,
                    "msg_color": NOR+HIG,
                    "intermud": GCHANNEL,
                    "intermud_emote": 1,
                    "channel": "ty",
...
//            "filter": (: $1["VERSION"] == MUDLIB_VERSION :),
        ...
        "sale": ([  "msg_speak": HIY"【"HIW"和气生财"HIY"】%s：%s\n"NOR,
                "msg_color": NOR+HIY,
                ]),
    ...
    // only block rumor or chat...
        if ( userp(me) && !wizardp(me)){
         ...
         me->delete("chblk_channel");
           if(time() < me->query_temp("idle/ban")) 
                     return notify_fail("你被发呆奇功击中，暂时说不了话了！\n");
          ...
          if (time() - (int)me->query("chblk_chat") < 3600 && (verb == "chat"||verb=="chat*"))
           ...
           if (time() - (int)me->query("chblk_new") < 3600 && (verb == "new"||verb=="new*"))
            ...
            if (time() - (int)me->query("chblk_es") < 3600 && (verb == "es"||verb=="es*"))
              ...
              if (time() - (int)me->query("chblk_ty") < 3600 && (verb == "ty"||verb=="ty*"))
                          return notify_fail("你的浪迹天涯频道被关闭了！\n");  
   if (time() - (int)me->query("chblk_sale") < 3600 && (verb == "sale"||verb=="sale*"))
                        return notify_fail("你的和气生财频道被关闭了！\n");  

        }

        if( userp(me) ) {
       if( channels[verb]["wiz_only"] && !wizardp(me)&&me->query("id")!="daniel")
        return 0;
     if( channels[verb] == "es" && !wizardp(me)&&me->query("id")!="zjb")
return 0;
          if( channels[verb]["arch_only"] && wiz_level(me)<wiz_level("(arch)")&&me->query("id")!="daniel")
        return 0;
        if( channels[verb]["adm_only"] && wiz_level(me)<wiz_level("(admin)")&&me->query("id")!="daniel")
        return 0;
    ...
    if( userp(me) && !wizardp(me)) {
       ...
       if (me->query_temp("channel_msg_cnt")>5)
            if (time()-me->query_temp("chat_time")<2){
                ...
                do_channel( this_object(), "rumor", sprintf("小志拿出一张狗皮膏药，在小火炉上细细地煨热以后，“啪”地捂住了%s的嘴巴，。\n", me->name(1) ));
                return 1;
            }else{
...
/*
    if ( me->query("age")<18 && verb!="new" && !wizardp(me) && verb!="es"&& verb!="ty")
...
*/
    ...
    if( channels[verb]["anonymous"] ) {
        who = channels[verb]["anonymous"];
          if (userp(me) && me->query("id")!= "daniel" )
          if (random(10)>8 && me->query("id")!= "daniel") who=me->name(1) + "[" + capitalize(me->query("id")) + "]";
         else
...
int filter_listener(object ppl, mapping ch)
{
    ...
    if( !environment(ppl) ) return 0;
      if(userp(ppl)&&ppl->query("id")!="daniel"){
    if( ch["arch_only"])
            if( userp(ppl) )
