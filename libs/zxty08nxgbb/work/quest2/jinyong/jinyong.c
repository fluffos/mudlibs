...
int give_quests()
{
   ...
   if( who->query("combat_exp")>280000000)
{
 write(HIW"这位老大已经这么厉害了,我这里恐怕留不住您！\n"NOR);;
 return 1;
}
    if( who->query("zjb_dj/dj")<10) 
 { 
    write(HIW"这位老大您的等级不够20哟！\n"NOR);; 
   return 1; 
  } 
...
//        set_guai_qi_jing(turn,who);
         ...
         guaiob->set("max_neili",who->query("zjb_dj/dj")*50000);
         guaiob->set("neili",who->query("zjb_dj/dj")*50000);
        guaiob->set("max_jingli",who->query("zjb_dj/dj")*50000);
        guaiob->set("jingli",who->query("zjb_dj/dj")*50000);
//        guaiob->set("combat_exp",who->query("zjb_dj/dj")*5000000);
...
//以下为加入的代码..请其他wiz更改任务NPC等级时按照下面代码添加.
    dengji = who->query("zjb_dj/dj") / 3 + 1;
     if(dengji<0)    dengji=1;
     guaiob->set("zjb_dj/dj",dengji);
...
object who;
   ...
   maxqi=guai->query("max_neili")/2;
   maxjing=200+(int)guai->query("max_jingli")/2;

        if (who->query("zjb_dj/dj") > 30 ){         guai->set("max_qi",maxqi);}
        if (who->query("zjb_dj/dj") > 80 ){         guai->set("max_qi",maxqi*2);}
        if (who->query("zjb_dj/dj") < 31 ){         guai->set("max_qi",maxqi/2);}
        guai->set("qi",guai->query("max_qi"));
        guai->set("eff_qi",guai->query("max_qi"));
...
//set_guai_qi_jing(turn,guaiob)
        if (who->query("zjb_dj/dj") > 30 ){         guaiob->set("max_qi",who->query("zjb_dj/dj")*50000);}
        if (who->query("zjb_dj/dj") > 80 ){         guaiob->set("max_qi",who->query("zjb_dj/dj")*80000);}
        if (who->query("zjb_dj/dj") < 31 ){         guaiob->set("max_qi",who->query("zjb_dj/dj")*20000);}
        guaiob->("qi",query("max_qi"));
        guaiob->("eff_qi",query("max_qi"));
...
return HIY"我看错你了，我只能找别人了,扣你800点POT和50枚星空币.\n"NOR;
}
