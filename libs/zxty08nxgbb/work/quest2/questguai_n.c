...
void die ()
{
        ...
        message_vision("$N惨叫一声，死了。\n",this_object());
if ( questob )
exp = (questob->query_int()*10) + (questob->query("zjb_dj/dj")*25) + random(2000);
if (exp > 10000) exp = 10000;
 pot = (questob->query_int()*5) + random(500);
 if (pot > 2600) pot = 2444+random(200);
        if(objectp(questob)){
