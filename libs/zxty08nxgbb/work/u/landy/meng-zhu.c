...
// Update by landy  Destruct the tow npc. 2008.4.13
...
void create()
{
    ...
    if(sizeof(children("/d/taishan/npc/meng-zhu")) > 2)
    {
    	Time = localtime(time());
    	log_file("Dest_MengZhu.txt",sprintf("Error: Tow loding meng-zhu   %d月%d日   %d:%d\n",Time[LT_MON],Time[LT_MDAY],Time[LT_HOUR],Time[LT_MIN]));
    	destruct(npc);
    }
