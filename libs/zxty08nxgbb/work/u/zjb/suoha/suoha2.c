...
int do_sit()
{
    ...
    if(me->query_temp("duchang_chouma")<1000)
        return notify_fail("你的筹码太少了！\n");
    i=sizeof(player);
        if (i<2)//还有空位
        {
        me->delete_temp("suoha");
        me->set_temp("suoha/being",1);
                player+=({me});
               me->add_temp("duchang_chouma",-20);//押５个
               yazhu_chouma+=20;//筹码
        i=sizeof(player);
      message_vision("$N拿出20个筹码押出去，坐上了座位！\n",me);
        if (i == 2)//满2人自动开始
                {
...
int do_yazhu(string arg)
{
    ...
    if(!me->query_temp("suoha/shuohua"))//是否为说话的方向
        return notify_fail("现在不是你说话！\n");
    if(!arg||sscanf(arg,"%d",i)!=1||i>50||i<1)
        return notify_fail("你要押多少筹码？最多不能押超过50个筹码。\n");
    remove_call_out("check_player");//清除延时呼叫
    me->add_temp("duchang_chouma",-i);
    yazhu_chouma=i;
    message_vision("$N哼了一声，推上去一些筹码说：“我押 "+i+" 个！敢跟吗？”\n",me);
    ob=gen_ob(me);
tell_object(ob,"你要跟(gen)的话，就是再上"+i+"个筹码，不押(buya)就是认输！要么也可以大(da)过对方一定的筹码!\n");
    me->delete_temp("suoha/shuohua");
    ob->set_temp("suoha/da",1);
...
int do_gen()
{
    ...
    da=me->query_temp("suoha/da2");
    if(me->query_temp("suoha/da2")) {
    me->add_temp("duchang_chouma",-da);
    me->delete_temp("suoha/da2");
    }else
    me->add_temp("duchang_chouma",-i);
...
int do_da(string arg)
{
    ...
    object me,ob;
    me=this_player();
    if(!yazhu) return notify_fail("现在不是大过对方的时候！\n");
    if(!me->query_temp("suoha/da"))//是否为说话的方向
      return notify_fail("现在比是大过对方的时候！\n");
    if(!arg||sscanf(arg,"%d",i)!=1||i>50 || i<1 )
        return notify_fail("你要大过对方多少筹码？最多不能超过50个筹码!\n");
    remove_call_out("check_player");//清除延时呼叫
  me->add_temp("duchang_chouma",-(i+yazhu_chouma));

message_vision("$N哼了一声，推上去一些筹码说：“我不但跟，还大你 "+i+" 个！敢跟吗？”\n",me);
yazhu_chouma+=i;
    ob=gen_ob(me);
...
int do_buya()
{
    ...
    me=this_player();
    if(!yazhu) return notify_fail("现在不是跟注的时候！\n");
    if(!me->query_temp("suoha/shuohua")&&!me->query_temp("suoha/genhua"))//是否为说话的方向
        return notify_fail("现在不是你说话！\n");
    remove_call_out("check_player");//清除延时呼叫
...
int do_leave()
{
    ...
    {
        me->add_temp("duchang_chouma",yazhu_chouma);//将刚刚押的返还
        yazhu=0;
        remove_call_out("check_player");//清除延时呼叫
        message_vision(HIG"$N不想再玩了！\n"NOR,me);
        chouma+=yazhu_chouma;//加进前面人押下的筹码数
       call_out("game_over",1);//结束
    }
    return 1;
}
...
int do_quansuo()//全梭
{
    ...
    remove_call_out("check_player");//清除延时呼叫
    quansuo=1;
    if(me->query_temp("suoha/shuohua"))//说话方
    {
    i=(1000-chouma/2);//最高是200,台面上有一半
    me->add_temp("duchang_chouma",-i);
        me->delete_temp("suoha/shuohua");
    yazhu_chouma=i;
    }
    else if(me->query_temp("suoha/genhua"))//跟话方
    {
...
"┌───┐┌───┐┌───┐┌───┐┌───┐\n"
"│※※※││　　　││　　　││　　　││　　　│\n"
"│%s│",name1,(player[0]->query_temp("suoha/shuohua")?HIW"说话"NOR:""),
    (player[0]->query_temp("suoha/genhua")?HIW"跟话"NOR:""),an1);
    for(k=0;k<4;k++)
    {
        if(k<i)
...
"┌───┐┌───┐┌───┐┌───┐┌───┐\n"
"│※※※││　　　││　　　││　　　││　　　│\n"
"│%s│",name2,(player[1]->query_temp("suoha/shuohua")?HIW"说话"NOR:""),
    (player[1]->query_temp("suoha/genhua")?HIW"跟话"NOR:""),an2);
    for(k=0;k<4;k++)
    {
        if(k<j)
