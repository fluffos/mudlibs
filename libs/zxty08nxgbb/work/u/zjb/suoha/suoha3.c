...
int do_sit()
{
    ...
    if(me->query_temp("duchang_chouma")<500)
    ...
    i=sizeof(player);
        ...
        me->set_temp("suoha/being",1);
                player+=({me});
                me->add_temp("duchang_chouma",-10);//押５个
                yazhu_chouma+=10;//筹码
        i=sizeof(player);
        message_vision("$N拿出10个筹码押出去，坐上了座位！\n",me);
        if (i == 2)//满2人自动开始
                {
...
int do_yazhu(string arg)
{
    ...
    if(!arg||sscanf(arg,"%d",i)!=1||i>25||i<1)
        return notify_fail("你要押多少筹码？最多不能押超过25个筹码。\n");
...
int do_da(string arg)
{
   ...
   if(!me->query_temp("suoha/da"))//是否为说话的方向
    ...
    if(!arg||sscanf(arg,"%d",i)!=1||i>25 || i<1 )
        return notify_fail("你要大过对方多少筹码？最多不能超过25个筹码!\n");
    remove_call_out("check_player");//清除延时呼叫
