...
*/
    ...
    {      // Does the victim dodge this hit?
        result += dodge_msg;
    if( ap+cost > pp && random(your["jing"]*100/your["max_jing"] + victim->query_int()/2) > 30&&me_exp > vic_exp) 
        {
 ...
 else 
  {
   ...
   {
            result += parry_msg;
   if( ap+cost > dp && random(your["jing"]*100/your["max_jing"] + victim->query_int()) > 60&&me_exp > vic_exp)
            {   
                if(vic_exp>150000000)
                   {
...
// npc的特殊防御
     ...
     if( vic_exp+500000 > me_exp && (random(my["jing"]*100/my["max_jing"] + me->query_int()) > 50) ) 
            {
          if(me_exp>300000000)
