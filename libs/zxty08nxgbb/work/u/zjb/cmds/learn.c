...
int main(object me, string arg)
{
        ...
        object ob;
int master_skill, my_skill, jing_cost, grade, times, pertimes, my_combat_exp;
int dj = this_player()->query("zjb_dj/dj");
        ...
        && me->query("couple/couple_id")!=ob->query("id")&&ob->query("couple/couple_id")!=me->query("id")) {
...
my_combat_exp = ((int)me->query("combat_exp")/10);
  ...
  grade = 1;
     ...
     if( (int)me->query("jing") > jing_cost * times )
        {
     if (my_skill * my_skill * (my_skill/10)<0)
     {

    if ((string)SKILL_D(skill)->type()!="knowledge" && my_skill * (my_skill/10) * (my_skill/11) > my_combat_exp)
     return notify_fail("也许是缺乏实战经验，你对"+ob->name()+"的回答总是无法领会。\n");
     }
        if ( my_skill > 20000 && SKILL_D(skill)->type()!="knowledge" )
     return notify_fail("目前非读书写字类的武功上限定为20000级，请联系巫师增加上限。\n");

          if( (string)SKILL_D(skill)->type()!="knowledge"
     && my_skill * (my_skill/10) * (my_skill/10) > my_combat_exp )
                {
                        return notify_fail("也许是缺乏实战经验，你对"+ob->name()+"的回答总是无法领会。\n");
     }
     else
