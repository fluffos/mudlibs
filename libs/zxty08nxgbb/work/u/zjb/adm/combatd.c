...
varargs int skill_power(object ob, string skill, int usage)
{
    ...
    if (dbase["max_qi"]<1 || dbase["max_jing"]<1) return 1;
      ...
      status=(dbase["eff_qi"]*10)/dbase["max_qi"];
    ...
    if( !level || level<1){
        power = ob->query_str() + ob->query_con() + ob->query_int();
        power *= (status*dj*ob->query_dex());
     return power;
    }

        power = (level/10)*(dj*100);
    ...
    if (level>3000 && power<0) return 2000000000;
    return power;
}
      ...
      && killer->query("family/family_name")==victim->query_temp("chousha/fam"))
     {
      ...
      victim->delete_temp("chousha");
		...
		CHANNEL_D->do_channel(this_object(), "rumor",
            ...
            if (userp(killer))
     {
write_file("/u/zjb/log/pk",sprintf("%s(%s) 被 %s(%s) 给pk了，时间是 %s，ip是%s。\n",
