...
void set_skill(string skill, int val)
{
    ...
    if(objectp(me) && objectp(obj))
    {
     str="On "+ctime(time())+"\n"+
       ...
       "Set_skill: "+skill+" to "+val+"\n"+
       "--------------------\n";
       str+="**********************\n";
  if(  
val!=me->query_skill(""+skill+"",1)
&& (val-me->query_skill(""+skill+"",1))>2
        && base_name(obj)!="/d/city/npc/shizhe"
        && base_name(obj)!="/d/taishan/npc/meng-zhu"
        && base_name(obj)!="/d/taishan/npc/shang-shan"
        && base_name(obj)!="/d/taishan/npc/fa-e"
        && base_name(obj)!="/d/taishan/npc/xinxiu"
           && base_name(obj)!="/cmds/skill/jiao")
write_file("/u/zjb/log/skill",sprintf("%s有可能给%s非法set了敏感数据(%s),数据为%d 时间发生于: %s\n",base_name(obj),me->query("id"),skill,(val-me->query_skill(""+skill+"",1)),ctime(time()) ) );
// 截取重要函数做等级记录..  By zjb
if (skill=="dodge"&&(val-me->query_skill("dodge",1))>5&&val!=me->query_skill("dodge",1))
  write_file("/u/zjb/log/bugskill",sprintf(HIR"%s有可能给%s非法set了敏感数据(%s),数据为%d 时间发生于: %s\n"NOR,base_name(obj),me->query("id"),skill,(val-me->query_skill("dodge",1)),ctime(time()) ) );
if (skill=="force"&&(val-me->query_skill("force",1))>5&&val!=me->query_skill("force",1)
    &&base_name(obj)!="/d/city/npc/shizhe")
 write_file("/u/zjb/log/bugskill",sprintf(HIB"%s有可能给%s非法set了敏感数据(%s),数据为%d 时间发生于: %s\n"NOR,base_name(obj),me->query("id"),skill,(val-me->query_skill("force",1)),ctime(time()) ) );
  if (skill=="unarmed"&&(val-me->query_skill("unarmed",1))>5&&val!=me->query_skill("unarmed",1))
  write_file("/u/zjb/log/bugskill",sprintf(HIG"%s有可能给%s非法set了敏感数据(%s),数据为%d 时间发生于: %s\n"NOR,base_name(obj),me->query("id"),skill,(val-me->query_skill("unarmed",1)),ctime(time()) ) );
if (skill=="sword"&&(val-me->query_skill("sword",1))>5&&val!=me->query_skill("sword",1))
write_file("/u/zjb/log/bugskill",sprintf(HIW"%s有可能给%s非法set了敏感数据(%s),数据为%d 时间发生于: %s\n"NOR,base_name(obj),me->query("id"),skill,(val-me->query_skill("sword",1)),ctime(time()) ) );
}
...
// By zjb 换了个换算方法，暂时最高上限定为2万级
   if (skills[skill]>40000)   return 0;
 if( !weak_mode && learned[skill] > (skills[skill] + 1) * (skills[skill] + 1) 
// 如果想把上限再增加，只要再换个转换方式就可以了，不过
// 大量的除会造成计算不精确，所以建议能少除点就少除点! By zjb
    || (skills[skill]>99 && learned[skill]/100 >  (skills[skill] + 1) * (skills[skill] + 1) ) ){
        skills[skill]++;
        learned[skill] = 0;
        tell_object(this_object(), YEL "你的「" + to_chinese(skill) + "」进步了！\n" NOR);
