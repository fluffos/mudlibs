...
//2004,11,26
    ...
    if( !level || level<1){
     ...
     return power;
        ...
        power = (level/30)*(dj*30);

    if (usage == SKILL_USAGE_ATTACK)
        power *= (dbase["str"] + (temp ? temp["str"] : 0));    
    else
        power*=(dbase["dex"] + (temp ? temp["dex"] : 0));
...
pkexp = killer->query("combat_exp")/300;
if ( pkexp > victim->query("combat_exp")/300 )
pkexp = victim->query("combat_exp")/300;
victim->delete("hecheng");
      if (!killer)     return;
...
killer->delete("combat_zjb");
victim->delete("combat_zjb");
        victim->add("combat_exp", -(int)victim->query("combat_exp") / 120);
      }
    } else {
        killer->add("MKS", 1);
        bls = 1;
