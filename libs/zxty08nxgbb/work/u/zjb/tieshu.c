...
static string *c_random = ({"","零","一","二","","三","","四","五","","六","","七","八","","九","十","百","千","万","亿","兆"});
...
int c = me->query_temp("xixia/guess_c");
  ...
  sscanf(arg,"%d",str);
   ...
   if( !arg ) {
       tell_object(me,HIC"\n ----------------答题……〖^~"+ chinese_ty(a) +"〗加上〖"+ chinese_ty(b) +HIC+"〗，等于多少数值----------------\n"NOR);
...
int a = random(9999)+100;
int b = random(9999)+100;
int c = a+b;
int d = random(8);
...
/*
  if (time() - who->query_temp("xixia/guess") > 80){
...
//      if (who->query_temp("xixia/guess") < who->query_temp("xixia/guess2")){
         gain = who->query_condition("ypjob")*2500+100000; 
        pot = who->query_condition("ypjob")*500+10000;
//  }
