...
..-.-.-   _o\ \\\     '::'   (o_ '-.-' |__\'-.-;~   
     ...
     :::''':::::^)__\:::::::::::::::::'''''''-._ \\n"); 
  if (me->query("driver"))
    tell_object (who,"  镖头："+me->query("driver")->query("name"));  
...
int do_speedup (string arg)
{
  ...
  else if (s < 90)
  {
    s += 8+random(16);
    tell_object (who,"你用力抽了一鞭。\n");  
    message_vision (me->query("name")+"嗖地一声加快了速度！\n",me);
  }
  else 
  {
    s += 16+random(32);
...
int do_stop (string arg)
{
  ...
  else 
  {
    ...
    message_vision (me->query("name")+"突然急剧地减速！\n",me);
    message_vision (me->query("name")+"可怕地震动着，马匹口角溢出血来！\n",me);
...
void check_speed ()
{
  ...
  {
    me->set("speed",0);
    message_vision (me->query("name")+"没有镖头了。\n",me);
