...
"点潜能，好好干，哈哈哈哈！\n"NOR;
 ...
 if( me->query_temp("kill_quest/count") > 14
     && (time()-me->query("kill_quest/time"))<20)
   return 
     CYN"你已经累了一天了，老夫吩咐你的任务都完成了，先回去歇息吧。\n"NOR;

if ( me->query_temp("dugu_wait") > time())
return HIG"你刚刚才取消任务,等一下再来接任务吧。\n"NOR;
/*
 if((time()-me->query_temp("kill_quest/wait_start")) < 60)
   return 
     CYN"干得不错，老夫很满意，你先去休息一下，过会还有任务呢。\n"NOR;
*/
...
{       
        ...
        money = me->query("zjb_money");
         ...
         command("say 我看错你了，我只能找别人了,扣你800点POT和50枚星空币.");
         me->add("zjb_money",-50);
         me->add("potential",-800);
         me->delete("kill_quest");
         me->delete_temp("kill_quest");
        me->set_temp("dugu_wait",time()+15);
