...
return 1;
    ...
    message_vision("$N拿起" + name() + "咕噜噜地喝了几口" + query("liquid/name")
        + "。\n", this_player());
    if (!query("liquid/water"))
        this_player()->add("water", 30);
    else
        this_player()->add("water", query("liquid/water"));
...
int do_fill(string arg)
{
    ...
    if (query("liquid/type")=="water" && query("liquid/remaining")==query("max_liquid"))
        return notify_fail(name()+"已经装满清水了！\n");
    if( query("liquid/remaining") )
