...
#include <dbase.h>
#include <room.h>
int check_team_member(object ob);
nosave int weight = 0;
...
//           call_other(dest, "???"); //change by lonely
//                  ob = find_object(dest);
// the end
           ob = load_object(dest);
        ...
        if (!env && sizeof(all_inventory(ob))>70 && userp(ob)){
                if( ob==this_player() ){
...
//如果目的物件不是this_object的环境并且如果在放入此物就会超重,则执行下面这段代码
// 目前是否有ob=this_object和ob=this_player()分不清而导致能超重的bug???????
        ...
        if( interactive(this_object())      // are we linkdead?
            ...
            &&  !silently ) {
              ...
              tell_object(ob, HIG"〖"HIW+environment()->query("short") +HIG"〗"NOR+ "\n");
       ...
       tell_object(ob, HIG"〖"HIW+environment()->query("short") +HIG"〗"HIW+str+NOR+ "\n");
                        }else{
       ...
       tell_object(ob, HIG"〖"HIW+environment()->query("short") +HIG"〗"HIW+str+NOR+ "\n");
        ...
        if(this_object()->query_team())
        {
                this_object()->delete_temp("pending");//簿笆ぇ碞埃硂夹癘
                call_out("check_team_member",3,this_object());//浪琩钉琌┬丁       
        }    
        return 1;
}
int check_team_member(object ob)
{
        object *member;
        int i;
        remove_call_out("check_team_member");
        ...
        member[i]->dismiss_team();
                if(member[i]!=ob)
                {
                        if(environment(member[i])
                          &&environment(member[i])!=environment(ob))
                        {
                                  member[i]->dismiss_team();
                                  write("你离队了.\n");
                        }
