       ...
       { 
                   ...
                   set("item_desc", ([ 
                          "asm"  :    "机器屏幕上显示：" HIM + BLINK" 请插入(insert)你的会员卡 " NOR 
                                      "的字样，机器上\n布满着各种奇怪的按扭。购买结束后请记得取卡(pull card)\n",
                   ]));
...
int do_insert(string arg)
{
        ...
        if (arg == "card" || arg == "tianya card")
        {
                if (! ob = present("tianya card", me)) return 0;
                ...
                write(HIY "天涯自动售货机正在读取你卡上的数据，请稍后...\n" NOR);
...
{  
   ...
   write(HIG "数据读取成功，请输入(list)显示可以购买的物品，输入(want)购买物品。取卡(pull card)\n" NOR);
...
int do_list()
{
  ...
  msg +=HIG "离火玄冰玉(lihuo yu)                   高级武器材料            100%                   ￥10.00\n";
  msg +=HIG "乾坤石(qiankun shi)                    高级武器材料            100%                   ￥15.00\n";
  msg +=HIG "天山玄铁(tianshan xuantie)             高级武器材料            100%                   ￥20.00\n";
  ...
  msg +=HIR "寰宇天晶(tian jing)                    终极武器材料            100%                   ￥ 50.00\n";
  msg +=HIR "合成武器超级材料(hecheng cailiao)      合成武器材料            100%                   ￥ 30.00\n";
  msg +=HIR "星空币(zjb bi)                         游戏货币一千            100%                   ￥ 10.00\n";  
...
int do_pull(string arg)
{
        ...
        {      
                rmb = me->query("rmb");
                tell_object(me, HIC "你目前的存款为" + chinese_number(rmb) + "圆人民币。\n" NOR);
                write(HIW "你将卡片从天涯自动售货机中取出。\n" NOR);
                ob = new("/clone/gift/tycard.c");
                ob->move(me);
...
int do_buy(string arg)
{
    ...
    else  if (arg == "hecheng cailiao")
    {
          ...
          ob = new("/d/zjb/weapon/jue");
          ob->move(me);
          me->add("rmb", -30);  
          write(HIG "你买下一块合成武器材料！\n" NOR);
    }    
        ...
        else if (arg == "lihuo yu")
    {
          ...
          ob = new("/clone/fam/item/lihuo");
          ob->move(me);
          me->add("rmb", -10);  
          write(HIG "你买下一块离火玄冰玉！\n" NOR);
    }    
            else if (arg == "qiankun shi")
    {
          if (rmb < 15)
          { 
              write("对不起,你的卡上余额已不足,请尽快充值!\n" NOR);
          ...
          ob = new("/clone/fam/item/qiankun");
          ob->move(me);
          me->add("rmb", -15);  
          write(HIG "你买下一块乾坤石！\n" NOR);
    }  
                else if (arg == "tianshan xuantie")
    {
          if (rmb < 20)
          { 
              write("对不起,你的卡上余额已不足,请尽快充值!\n" NOR);
