...
int do_get(object me, object obj,object env,int amount)
{
    ...
    object *zjbobj;
    object *inv = deep_inventory(obj);    
    zjbobj = filter_array(inv,(:get_object:)); 
      if(sizeof(zjbobj)&&!userp(obj)){
    write (HIB"这样东西里面有个东西是不能get的,你没办法get!\n"NOR); 
return 1;
}        if( !obj ) return 0;
     if( obj->query("no_get") && !wizardp(me)||obj->query("area_name"))
                return notify_fail("这个东西拿不起来。\n");
       if( obj->query("id") == "wuwum" )
              return notify_fail("这个东西拿不起来。\n");
        if( obj->is_character() ){
                if( living(obj) && wizhood(me)!="(admin)")  return notify_fail(obj->query("name")+"不用你背。\n");
        }
        ...
        foreach (object ob in all_inventory(env)){
                ...
                if( sizeof(all_inventory(me)) > 150 ) {
                        tell_object(me, "你身上的东西太多了！\n");
                        continue;
                }
                if( ob->is_character() && living(ob) && wizhood(me)!="(admin)"){
                      write(ob->name(1)+"不用你背。\n");
                      continue;
                }
               if ( obj->query("id") == "mu ren") {
                        tell_object(me, "木人不需要你背！\n");
                        continue;
                }
               if ( obj->query("id") == "tong ren") {
                        tell_object(me, "铜人不需要你背！\n");
                        continue;
                }
               if ( obj->query("id") == "corpse") {
                                             inv = all_inventory(obj);
                              for(b=0; b<sizeof(inv); b++) inv[b]->move(environment(me));
             }
               if ( obj->query_temp("must_killby")) {
                                                inv = all_inventory(obj);
                                for(b=0; b<sizeof(inv); b++) inv[b]->move(environment(me));
