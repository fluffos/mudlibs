...
/*
        if(!wizardp(me)&&me->query_temp("bid_time")&& time() < me->query_temp("bid_time")+30)
                 return notify_fail("你目前不能参与拍卖。\n");
*/
         if( me->query("doing") ) return notify_fail("闭关中不能出价！\n");
           if (wizardp(me)) return notify_fail("巫师不能参与拍卖!\n"); 
        if( !arg )
...
//拍卖师放置的路径
        ...
        else
        {
                ...
                if( obj->query("paimai_id") == me->query("id")) 
                        return notify_fail("你刚出价了。\n");
...
//    if(obj->query("ipaimai/"+sprintf("%d",number))=="paimai shi")
    ...
    if(argg < 0 )
        ...
        obj->set("paimai_id",me->query("id"));
        me->start_busy(2);
      me->set_temp("on_paimai",1);//设置bid记录，防止quit;
      me->set_temp("bid_time",time());//设置bid记录，防止quit;
