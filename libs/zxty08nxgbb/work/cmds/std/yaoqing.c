...
int main(object me, string arg)
{
        ...
        object lingpai;
        mapping data;
        string *what,stuffid,stuffname,myid;
        string name;
        myid = me->query("id");
        ...
        if(me->is_fighting()|| me->is_busy()&&!me->query("doing"))
                return notify_fail("你正忙着呢。\n");
                
        if(ob->is_fighting()|| (ob->is_busy() &&  !ob->query("doing") ) )
        ...
        if (lingpai->query("bangzhu_id")!=myid && !lingpai->query("yaoqing/"+myid)&&userp(ob)) {
                destruct(lingpai);
                return notify_fail("您没有使用这个指令的权利。\n");
        }
        destruct(lingpai);

        message_vision("$N邀请$n参加帮会「"+trans_color(banghui,2)+NOR"」。\n",me,ob);
        if(! userp(ob) )        {
                if(!ob->query("zhengzhao"))
                        return notify_fail("看样子"+ob->query("name")+
                                "对你的帮会没有兴趣。\n");
                switch(ob->query("zhengzhao"))  {
                case 1: if((!(banghui=ob->query("banghui"))|| ob->query("zhongcheng")<me->query_per())&&ob->query("qi")*100>ob->query("max_qi")*80) {
                        message_vision("$N对$n说道：“即是如此，你我不如切磋(qiecuo)一下武功如何？”\n",ob,me);
                      ob->set_temp("tybang/target",me);
                        return 1;
                        }
                ...
                case 2: if(!(banghui=ob->query("banghui")) ||
                        ...
                        chinese_number(money/1000)+"两黄金，少一个子也不行。　盶n",ob,me);
                       ob->set_temp("tybang/target",me);
                       ob->set_temp("tybang/money",money);
                        return 1;
                        }
                        message_vision("$N冲着$n笑道：“我可不敢高攀贵帮啊。”\n",ob,me);
                        return 1;
                case 3: if(!(banghui=ob->query("banghui")) ||
                        ob->query("zhongcheng")<me->query_per())    {
                       data=ob->query_temp("tybng/target");
                        if(! mapp(data))        data=([]);
                        target=keys(data);
                        ...
                        ob->set_temp("tybang/target",data);
...
void delete_target(object npc,string name)
{
        ...
        data=npc->query_temp("tybang/target");
        if(! mapp(data))        return;
        map_delete(data,name);
        npc->set_temp("tybang/target",data);
}
