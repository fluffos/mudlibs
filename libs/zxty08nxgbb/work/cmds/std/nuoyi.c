...
int main(object me, string arg)
{
	...
	if( form == 1)
	{
        ...
        me->add_temp("apply/attack", count * r_lvl);
		me->set_temp("nuoyi", 1);
                me->start_call_out( (: call_other, __FILE__, "remove_effect_nuoyi", me, count * r_lvl :), skill);
		me->add("neili", -300);
		me->start_busy(3);
		return 1;
	}
	...
	me->start_busy(3);
        if (random(me->query("neili",1)) > 2*target->query("neili",1)/3 )
        {
            ...
            if (me->query("neili")<damage) damage=me->query("neili");
                me->add("neili", -damage);
...
void remove_effect_nuoyi(object me, int amount)
{
   if ( (int)me->query_temp("nuoyi") ) {
