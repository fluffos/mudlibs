...
int main(object me, string arg)
{
        ...
        if(me->query("id")==lingpai->query("bangzhu_id"))       {
                msg="你是帮会「"+trans_color(me->query("banghui"),2)+"」的帮主，脱离前最好指定新任帮主！\n";
        }
        else    msg="你是帮会「"+trans_color(me->query("banghui"),2)+NOR"」的帮众。\n";
        msg +="你真的要脱离吗(y/n)？";
        write(msg);
        destruct(lingpai);
...
int yes_or_no(string str,object me,string banghui)
{
        ...
        if(str=="y"||str=="yes")        {
         ...
         message("channel:rumor",YEL"【天涯帮会】某人："+me->query("name")+"脱离"+
                "帮会「"+trans_color(banghui,2)+"」！\n"NOR,users());
        me->delete("banghui");
        me->delete("tybang");
        ...
        if(! sizeof(lingpai->query("player")) && 
                ...
                rm("/data/guild/"+banghui+".o");
                 message("channel:rumor",YEL"【天涯帮会】某人：帮会「"+trans_color(banghui,2)+
                        "」土崩瓦解了！\n"NOR,users());
        }
        destruct(lingpai);
        return 1;
