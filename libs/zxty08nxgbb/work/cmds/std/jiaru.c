...
int main(object me, string arg)
{
        ...
        if((object)me->query_temp("tybang/target")!=ob)
        ...
        if (me->query("joinbp") > 3 )
                return notify_fail("象你这种背信弃义的垃圾还想加入帮派吗？.\n");
        ...
        me->set("tybang/banghui",ob->query("tybang/banghui"));   
        me->set("tybang/zjb",1);
        me->add("joinbp", 1);
        me->delete_temp("tybang/target");
        me->save();
        destruct(lingpai);
                message("channel:rumor", 
 HIM"【天涯帮会】"+ "某人："+me->query("name")+"加入帮会「"+trans_color(ob->query("banghui"),2)+HIM"」！\n"NOR,users());
        return 1;
        }
        else    {
        destruct(lingpai);
