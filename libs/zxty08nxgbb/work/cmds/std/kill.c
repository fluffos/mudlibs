...
/*
  if( me->query_condition("guanfu_task") &&  !living(obj) )
    return notify_fail("太没人性了吧?！\n");
*/
    if (wizardp(obj))
        return notify_fail(obj->query("name")+"瞪了你一眼，你突然吓得两腿发抖，几乎跪了下来。\n");
     if((int)obj->query("age") <= 35 && userp(obj))
        return notify_fail("为了世界更美好，放过小孩子吧.\n");
      if((int)obj->query("mud_age") <= 1728000 && userp(obj))
       return notify_fail("为了世界更美好，放过小孩子吧.\n");
    ...
    message_vision("\n$N对著$n喝道：「" 
        + callname + "！今日不是你死就是我活！」\n\n", me, obj);
//            message("vision",HIB"〖天外飞语〗"YEL+":"+HIR" 听说" + me->short() + "对" + obj->name() + "下了杀手,真是黑心!\n"NOR,me,obj);
    me->kill_ob(obj);
    ...
    if( !userp(obj))
        ...
        obj->fight_ob(me);
me->set("combat_zjb",""+obj->query("id")+"");
        tell_object(obj, HIR "如果你要和" + me->name() 
            + "性命相搏，请你也对这个人下一次 kill 指令。\n" NOR);
// message("channel:ty", HIB"〖天外飞语〗"YEL+":"+HIR" 听说" + me->short() + HIR"对" + obj->name() + "下了杀手,真是黑心!\n"NOR,users());
    }
...
//同门互助（玩家可不参与）
//           if ( (string)ob->query("family/family_name")==callname && !ob->is_killing(me) && !userp(ob) && !ob->query("notongmen") ){
