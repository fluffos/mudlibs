...
//显示装备属性
      ...
      if (obj->query("damage"))
		...
		str += sprintf(HIM"         武器等级：" YEL "+ " NOR"%d 级\n" NOR, me->query("weapon/lv"));
          if (obj->query("rate"))
...
int look_living(object me, object obj)
{
    ...
    if (!objectp(present("fire", me))  && environment(me)->query("outdoors") && NATURE_D->outdoor_room() && !wizardp(me)&&me->query("id")!="snow" ){
...
// edit by Zjb@TY 
    ...
    if ( obj->query("neili")/3 > me->query("neili")/2 )
    str += HIB"此人内力十分深厚,你看不出他的深浅,还是小心为妙!\n"NOR;
    else
    if ( obj->query("neili") > me->query("neili") )
    str += HIM"此人内力虽然比你深厚,但是距离也不大,不过交起手来还是有危险的!\n"NOR;
    else
    if ( obj->query("neili")*2 < me->query("neili") )
    str += HIG"此人内力修为十分差,你一眼便看透了他的虚实!!\n"NOR;
    else
    if ( obj->query("neili")/2 < me->query("neili")/3 )
