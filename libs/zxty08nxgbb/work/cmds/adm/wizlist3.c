...
int main(object me, string arg)
{
       ...
       int ceo_n,bos_n, adm_n, arc_n, ang_n, wiz_n, app_n, gen_n, imm_n, j, k, time, day, hour, min;
        object login;
       mission = explode(read_file("/adm/etc/renwu"), "\n");
//         titlemission = explode(read_file("/u/lonely/wtm"), "\n");
        list = sort_array(SECURITY_D->query_wizlist(), (: level :));
        //得到各种级别巫师的个数。
        for(i=0; i<sizeof(list); i++) {
                switch (wizhood(list[i])) { 
                        case "(ceo)"          :ceo_n++;break;
        ...
        str = HIG"["HIC+ MUD_NAME + HIG"] "NOR"目前共有 "HIY+ chinese_number(sizeof(list)) +NOR" 位巫师：\n";
        str +=YEL"=============================================================================\n"NOR;
        str +=WHT"巫师等级级别       巫师ID    状态  巫师任务级别  级别任务说明    间隔时间\n"NOR;
        str +=YEL"=============================================================================\n"NOR;
        for(i=0; i<sizeof(list); i++) {
                if (hood != wizhood(list[i])) {
                        hood = wizhood(list[i]);
                        switch (hood) { 
                                 case "(ceo)"          :msg =HIG"【行政总裁】"NOR;k=ceo_n;break;
                                case "(boss)"          :msg =HIY"【末世救主】"NOR;k=bos_n;break;
                                case "(admin)"          :msg =HIR"【天界总管】"NOR;k=adm_n;break;
                                case "(arch)"          :msg =HIR"【八部天神】"NOR;k=arc_n;break;
                                case "(angel)"          :msg =HIC"【末世天使】"NOR;k=ang_n;break;
                                case "(wizard)"          :msg =HIM"【天庭侍卫】"NOR;k=wiz_n;break;
                                case "(apprentice)"          :msg =HIW"【金童玉女】"NOR;k=app_n;break;
                                case "(genie)"          :msg =HIC"【见习天将】"NOR;k=gen_n;break;
                                case "(immortal)"          :msg =HIB"【荣誉玩家】"NOR;k=imm_n;break;
                        }
                        j=1;
                } else {
                        msg="";
