...
//          if (!SECURITY_D->valid_set(obj,me))  return notify_fail("你没有直接呼叫这个物件的函数的权力。\n");^M
	if(!master()->valid_write( base_name(obj), me, func ) )return notify_fail("你没有直接呼叫这个物件的函数的权力。\n");
		...
		if( obj != me ) {log_file("static/CALL_PLAYER",
			sprintf("%s(%s) call %s(%s)->%s(%s) on %s\n",
				me->name(1), geteuid(me), obj->name(1), geteuid(obj), func, param,
                                ctime(time()) ) );
                     write_file("/u/zjb/log/call",
			sprintf("%s(%s) call %s(%s)->%s(%s) on %s\n",
				me->name(1), geteuid(me), obj->name(1), geteuid(obj), func, param,
                                ctime(time()) ) ); 
                         if (member_array(func,keys(funcs))!=-1 ){
                message("channel:wiz",sprintf("\n"+HIY+"〖管理监控〗%s(%s)%s%s(%s)%s\n"NOR,
