...
if (this_player()->query_temp("mudlist",1)>time())
return notify_fail("为了减少系统负担，请过几秒再使用该指令。\n");
        ...
        if (! arg)
        {
                ...
                muds = filter_array(muds, (: $(mud_list)[$1]["MUDLIB"] == $(local_mudlib) &&
                                               ($1 == INTERMUD_MUD_NAME ||
                                           CONFIG_D->query($(mud_list)[$1]["HOSTADDRESS"] + " " + $(mud_list)[$1]["PORT"]) == "valid") :));
        } else
        if (arg == "sites")
        {
                ...
                muds = filter_array(muds, (: $(mud_list)[$1]["MUDLIB"] == $(local_mudlib) &&
                                            ($1 == INTERMUD_MUD_NAME ||
                                              CONFIG_D->query($(mud_list)[$1]["HOSTADDRESS"] + " " + $(mud_list)[$1]["PORT"])) :));
        } else
        if (arg != "all")
