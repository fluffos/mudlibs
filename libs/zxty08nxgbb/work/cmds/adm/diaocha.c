...
int main(object me, string arg)
{
        object *ob;
        int i, m, num, b, str, c;
        string msg, *file, str1, str2, argg;

        msg = "";
        file = ({ });
        
        if ( !arg ) return 1;        
        sscanf(arg, "%s %d", argg, str);
        switch (argg) 
        {
                case "user"  :
                        ob = users();
                ...
                case "living":
                        ...
                        write_file("/u/zjb/log/liv.log", msg, 1);
                        break;
                case "char"  :
                ...
                case "room"  :
                        ...
                        write_file("/u/zjb/log/room.log", msg, 1);
                ...
                case "object":
                        ...
                        {
                                if ( i>1 )
                                if ( base_name(ob[i]) == base_name(ob[i-1]) )
                                {
//                                b++;
//                                if ( b > str/2)
                                me->add_temp("obj/"+base_name(ob[i]),1);
                                 }
  else { if( me->query_temp("obj/"+base_name(ob[i-1])) > str ) {
            me->delete_temp("obj/"+base_name(ob[i-1]));
         write(base_name(ob[i-1])+"\n"); b++;} }
                        }
                        me->delete_temp("obj");
                       break;
                default:
