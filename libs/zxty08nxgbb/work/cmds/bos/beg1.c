...
int main(object me, string arg)
{
    ...
    if(me->is_busy())
        return notify_fail("你现在正忙，没法要东西。\n");
        ...
        if ( ob->query("equipped") || ob->query("no_drop") )
         ...
         sp = (int)me->query_skill("begging") + (int)me->query_kar() * 5
                - (int)me->query("begger");
        if( sp < 1 ) sp = 1;
        dp = (int)victim->query("jing") * 15 +victim->query("combat_exp")-victim->query("shen")*10;
