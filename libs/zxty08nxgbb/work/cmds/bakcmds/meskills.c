...
if(!arg)
    return notify_fail("指令格式：meskills 武功基本技能 武功英文名字 武功中文名字 第几招\n");
...
if((int)me->query("combat_exp") < (skill_num-1)*200000+20000000)
...
write("\n想象一下你和老猫的打斗，(高手指南:必须要有你和老猫字样出现!)\n");
write("\n你前腿踢出，后腿脚尖点地，一式「横空出世」，二掌直出，攻向老猫的上中下三路\n");
write("你纵身一跃，手中武器一招「金光泻地」对准老猫的某部位斜斜刺出一剑\n");
...
{
        write(@HELP
  指令格式：meskills 武功基本技能 武功英文名字 武功中文名字 第几招
  例如：meskills unarmed mytest 我的掌法 1
 
这是用来创建自己风格武功的指令，随着经验值的增长，所可以实现的招式
越来越多，招式的威力也越来越大。
玩家增加招式的熟练程度的方式是使用两个专用指令：
  lian,用practice也可以，但是难度会加大。
  qlian 类似于 learn
...
招式描述中的：
老猫，你，某部位，武器带有普遍性，将来可以用来对付各种不同敌人。
