...
int checking(int i)
{
        ...
        obb=FINGER_D->acquire_login_ob(ob->query("id"));
                                if (ob->query("id")!=name)
                                        log_file("bad_data",name+"\tisn't\t"+ob->query("id")+"\n");
                                if (ob->query("zjb_money")>=10000 &&
 (time()-obb->query("last_on"))/86400<1)
log_file("check_users",sprintf("%10s(%s) 星空币:%d,上线ip是:%s\n",ob->name(),name,ob->query("zjb_money"),ob->query("last_from")));
