                ...
                for(j=0;j<2*X+1;j++)
                {
                        ...
                        else
                        {
                                ...
                                else
                                {
                                        ...
                                        //if(!(i%2)&&!(j%2))
                                        if(m[j][i]!="─")
                                                for(int d=8-sizeof(clean_color(m
[
j][i]));d>0;d--)
                                                        tmp+=" ";
                                }
...
int draw_path(int x,int y,string direc)
{
        ...
        if(sscanf(direc,"%*swest%*s"))
                xx--;
/*
    if(sscanf(direc,"%*senter%*s"))
             yy--;
   if(sscanf(direc,"%*sout%*s"))
            yy++;
...
。
理论上讲，该命令应该能完全正确地反应地图，但是由于巫师设计地图不规范等
多
种原因（比如迷宫、有不规范出口比如enter之类的，房间之间无法双向关联等）
，
导致地图可能在显示时与行走方向上有一定区别，甚至会出现走一步路，地图就
完
全变样的情况。比如，房间A可以从south走到B房间，但是B房间走north却走到C
房
间，这时在A用该命令与在B用该命令效果完全不一样！
此外，连接标志中箭头所指方向地势更高，有不规范出口的房间以黄色标志，你
所
处的房间以闪烁来标志！
