...
//modifu by Amis@xajh2 2006-2-6 12:58
	...
	if(sscanf(buff,"mission obj data file  --%*s"))
	{
		...
		while((buff=read_file(file,++line,1))!="End data"&&buff)
		{
			...
			if(sscanf(buff,"%s-%s-%d-%d-%d-%s-(%d)\n",
				...
				return notify_fail("format wrong:data file line "+line+" format error!\n");
			odata=([]);
			odata["id"]=id;
			odata["cname"]=cname;
			odata["value"]=value;
			...
			if ( sscanf(filename,"/d/%s/%s",party,temp) !=2 )
				return notify_fail("filename wrong:data file line "+line+" format error!\n");
	...
	if(sscanf(buff,"mission char data file  --%*s"))
	{
		...
		while((buff=read_file(file,++line,1))!="End data"&&buff)
		{
			if(sscanf(buff,"%s**%s**%d**%d**%d**%d**%s-(%d)\n",
				...
				return notify_fail("format error:data file line "+line+" format error!\n");
			odata=([]);
			odata["id"]=id;
			odata["cname"]=cname;
			odata["exp"]=exp;
...
int get_mission_data_from_file(string dir)
{
	...
	for ( int i=0 ; i < sizeof(files) ; i++ )
	{
		...
		if ( stringp(obj->query("id")))
		{
			...
			log_file(char_file,sprintf("%s**%s**%d**%d**%d**%d**%s-(%d)\n",
				obj->query("id"),
				obj->query("name"),
