...
if (this_player()->query_temp("mudlist",1)>time() && !wizardp(me))
return notify_fail("为了减少系统负担，请过几秒再使用该指令。\n");
        if(!mud_list)
        ...
        output = 
HIG BBLU"Mud                    中文名称              国际网络位址     端口  人数\n"NOR+
"────────────────────────────────────\n";
...
mud_list[muds[loop]]["HOSTADDRESS"]?mud_list[muds[loop]]["HOSTADDRESS"]:"219.153.47.181",
                                mud_list[muds[loop]]["PORT"]);
               
                if(!undefinedp(mud_list[muds[loop]]["USERS"]))
                                output+=sprintf("  %-4d"        , atoi(mud_list[muds[loop]]["USERS"]));
                else
...
output+="────────────────────────────────────\n";
           output += sprintf("共有 %d 个 Mud 与"+"天涯"+"连线。\n",total==0?0:total-1);
        this_player()->start_more( output );
     this_player()->set_temp("mudlist",time()+5);
