...
object find_wiz(string x)
{
    ...
    if( str[i]->query("env/debug"))     tell_object(str[i],x+"\n");
         }
    return 0;
}
int help(object me);
...
int main(object me, string arg)
{
    ...
    if( userp(me) && !wizardp(me)) {
       ...
       if (me->query_temp("tell_msg_cnt")>5)
            if (time()-me->query_temp("tell_time")<2)
                ...
                if(me->is_busy()&&me->query_temp("tell_busy")) {
                        me->add("jing",-5);
                        write("你现在喉咙嘶哑,半句话也说不出了!\n");
                        return 1;
                        }
         if( time()-me->query_temp("tell1")<1&&!wizardp(me) ) {me->start_busy(8);
                me->set_temp("tell_busy",1);
                remove_call_out("delete_busy");
                call_out("delete_busy",8,me);
                me->add("jing",-5);
                write("别急，慢慢说！\n");
               return 1;
}
    ...
    if( sscanf(target, "%s@%s", target, mud)==2 ) {
        ...
        write(BOLD "网路讯息已送出，可能要稍候才能得到回应。\n" NOR);
        return 1;
    }
    obj = find_player(target);
    if (obj && !userp(me)){
        tell_object(obj, sprintf( BOLD "%s千里传音你：%s\n" NOR,
   ...
   if (!me->visible(obj)) return notify_fail("没有这个人....。\n");
    ...
    if (obj==me) return notify_fail("你在自言自语的不知道在说什么...\n");
    debug(sprintf( WHT "%s(%s)告诉%s(%s)：%s" NOR,me->name(1),me->query("id"),obj->name(1),obj->query("id"), msg));
    if ( member_array("tell", me->query("channels"))==-1)
                        me->set("channels", me->query("channels") + ({ "tell" }) );
    write( WHT "【私语】"HIG "你千里传音"+ obj->name(1) + "：" + msg + "\n" NOR);
    ...
    tell_object(obj, sprintf( WHT "【私语】" HIG "%s千里传音你：%s\n" NOR,
        me->name(1)+"("+me->query("id")+")", msg));
   ...
   if (in_edit(obj)) return notify_fail("但是，对方好像正在编辑档案！\n");
    ...
    me->set_temp("tell1",time());
    return 1;
}
void delete_busy(object me)
{
        if(!me || !environment(me)) return;
        me->delete_temp("tell_busy");
}
string remote_tell(string cname, string from, string mud, string to, string msg)
{
    ...
    if( ob = find_player(to) ) {
        if( cname )
       tell_object(ob, sprintf(WHT "【私语】" HIG "%s(%s@%s)千里传音你：%s\n" NOR,
                cname, capitalize(from), mud, msg ));
        else
              tell_object(ob, sprintf(WHT "【私语】" HIG "%s@%s 千里传音你：%s\n" NOR,
