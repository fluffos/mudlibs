...
int main(object me, string victim_name)
{
	object victim;
          ...
          if (((int)me->query("age")<15) && !wizardp(me))
	{
        ...
        if ((int)me->query("vote/zjb_luantou")>me->query("mud_age"))
	{
    ...
    if (vote(me, victim) <= 0)
	{
		if ((int)me->query("vote/abuse")> 50)
		{
            ...
            me->set("vote/zjb_luantou",me->query("mud_age")+1800);
...
if ((!wizardp(me)&&reason = (int)victim->query("vote/reason"))>0 && (reason!=V_UNCHBLK))
  {
  	return notify_fail("要把当前的表决完成以后才可以提新的动议。\n");
  }

 if ( victim->query_temp("openchat") + 300 > time() )
        return notify_fail("他的频道刚刚被打开了没超过5分钟，还是等一会再尝试打开他的频道吧。\n");
 if(victim->query("chblk_jh"))
  {
	channel = "江湖同盟";
   ...
   else if (victim->query("chblk_es")) 
  {
   ...
   else if (victim->query("chblk_new")) 
  {
  	channel = "新手";
  }else
if (victim->query("chblk_channel"))
      channel = "公众";
else
return notify_fail(victim->name()+"的频道已经是打开的了。\n");
 ...
 // vv = (int) valid_voters(me)/10;
  ...
  if (df>0)
  {
        shout( HIG "【投票】"+my_name+"投票打开"+v_name+"("+capitalize(victim->query("id"))+")"+"的"+channel+"频道，还差"
			+chinese_number(df)+"票。\n" NOR);
	write( HIG "【投票】"+my_name+"投票打开" +v_name +"的"+channel+"频道，还差"
			+chinese_number(df)+"票。\n" NOR);
  ...
  } else 
  {
  	if (me != victim)
  	{
        shout( HIG "【投票】"+my_name+"投票打开"+v_name+"("+capitalize(victim->query("id"))+")"+"的"+channel+"频道。"
		+v_name+"的"+channel+"频道被打开了！\n" NOR);
	write( HIG "【投票】"+my_name+"投票打开" +v_name+"的"+channel+"频道。"
		+v_name+"的"+channel+"频道被打开了！\n" NOR);
	} else 
	{
	shout( HIG "【投票】"+my_name+"投票打开自己的"+channel+"频道。"
		+my_name+"的"+channel+"频道被打开了！\n" NOR);
	write( HIG "【投票】"+my_name+"投票打开自己的"+channel+"频道。"
		+my_name+"的"+channel+"频道被打开了！\n" NOR);
	}		
	...
	victim->delete("chblk_new");
	victim->delete("chblk_es");
	victim->delete("chblk_jh");
        victim->delete("chblk_channel");
       victim->set_temp("openchat",time());
  } 
