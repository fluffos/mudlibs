...
{
  string line,zjb,daniel,my_str,my_dex,my_con,my_int,my_kar,my_per,skill_type,master,couple,killer;
         mapping my,pmap;
         object ob, weapon;
...
//         seteuid(getuid(me));
        ...
        if(attack_points > 2000000000) attack_points = 2000000000;
        if(parry_points > 2000000000) attack_points = 2000000000;
        if(dodge_points > 2000000000) attack_points = 2000000000;
        ...
        if (userp(ob))
         ...
         line += sprintf("%s"HIW"%-19s"NOR"%s%-46s┃\n",
                 "┃攻 击 力 ：   ",
                  attack_points/500+1+" (+"+ob->query_temp("apply/damage")+")",
         ...
         line += sprintf("%s"HIW"%-19s"NOR"%s%-46s┃\n",
                 "┃防 御 力 ：   ",
                 (dodge_points/10 + (weapon? parry_points/10: (parry_points/10)))/50 + 1+" (+"+ob->query_temp("apply/armor")+")",
...
//   " ("+(1 + (int)ob->query("learned_points")) * 100 / (1 + (int)ob->query("potential"))+"%)",
 ...
 += sprintf("%s"RED"%-13s"NOR"%s"HIM"%-13s"NOR"%s"HIY"%-12s"NOR"┃\n",
         ...
         if ((int)ob->query("vip/vip") > 0)
daniel = HIY " 你是VIP会员，目前总共有" + 
  chinese_number(ob->query("moneyvip")) + "枚天涯币。\n" NOR;
    else
 daniel = HIY " 你还未成为VIP会员，目前总共有" +  
  chinese_number(ob->query("moneyvip")) + "枚天涯币。\n" NOR; 
 line += sprintf("%s",daniel);
        if ((int)ob->query("zjb_money") > 0)
zjb = HIB " 你目前总共有" + 
chinese_number(ob->query("zjb_money")) + "枚星空币。\n" NOR;
    else
    zjb = HIB " 你还没有星空币。\n" NOR;

 line += sprintf(WHT+" %s在"+HIG+CHINESE_MUD_NAME+NOR+"里的游戏时间是:"+BLINK+HIY"%s"+NOR+HIW"||"NOR"%s"+NOR,ob==me?"你":ob->name(1),FINGER_D->age_string( (int)ob->query("mud_age")),zjb);
