...
void say_answer(object who)
{
   ...
   command("say 对了，沉默才是答案！这本来是没有答案的问题，一切要视当时的情形而定。你过关了，去吧。\n");   
     this_object()->delete_temp("check_answer");     this_player()->move("d/tulong/spirit5");
     return ;
}
...
int do_answer(string arg)
{
   ...
   message_vision("$N答道：我会先救“" + arg + "”。\n", this_player());
       ...
       {
        switch (random(5)) {
         case 0:      message_vision("$N侧身让出一条路。\n", this_object());   
     this_object()->delete_temp("check_answer");
    this_player()->move("u/wenhua/tlong/huoshan");
     remove_call_out("say_answer"); 
          break;
         case 1: 
    message_vision("$N侧身让出一条路。\n", this_object());   
     this_object()->delete_temp("check_answer"); 
     this_player()->move("u/wenhua/tlong/huoshan1");  remove_call_out("say_answer");
         break;
         case 2: 
    message_vision("$N侧身让出一条路。\n", this_object());   
     this_object()->delete_temp("check_answer"); 
     this_player()->move("u/wenhua/tlong/huoshan3");
  remove_call_out("say_answer");
         break;
         case 3: 
    message_vision("$N侧身让出一条路。\n", this_object());   
     this_object()->delete_temp("check_answer"); 
     this_player()->move("u/wenhua/tlong/huoshan2");
  remove_call_out("say_answer");
         break;
         case 4: 
    message_vision("$N侧身让出一条路。\n", this_object());   
     this_object()->delete_temp("check_answer"); 
     this_player()->move("u/wenhua/tlong/huoshan4");
  remove_call_out("say_answer");
         break;
             } 
     return 1;
