...
string short()
{
        ...
        mapping *notes;
       string banzhu;    //by lonely
        int i, unread, last_read_time;
       if(!query("banzhu")) banzhu="[斑竹征求中]";
       else banzhu="[斑竹:"+this_object()->query("banzhu")+"]";
        ...
        if( unread )
                return sprintf(HIC"%s"NOR" [ "HIW"%d"NOR" 个贴子，"HIR"%d"NOR" 篇未读 ]"+banzhu, ::short(), sizeof(notes), unread);
        else
...
string long()
{
        ...
        for(i=0; i<sizeof(notes); i++) {
         ...
         msg+=HIY"※ 再现天涯网通站 < 218.24.207.18 4000 or 2001> \n※ 再现天涯电信站 : 59.45.240.234 4000 or 2001\n"NOR;
         msg+=HIR"⊙ From : http://www.jhfymud.cn\n"NOR;
        msg+=HIW"─────────────────────────────────────"NOR;
        return msg;
}
...
// 显示未读的贴子
        ...
        HIW"标题："NOR"%-54s ]\n────────────────────────────────\n%s\n"
         HIC"\n※ 再现天涯网通站 : 218.24.207.18 2001,4000 \n※ 再现天涯电信站 : 59.45.240.234 2001,4000 \n"HIY"⊙ 天涯主页 : http://www.jhfymud.com\n"NOR
        "──────────────────────────["HIW" 本 篇 完 "NOR"]\n",
                num + 1,
...
int do_banzhu(string arg)
{
  ...
  if (SECURITY_D->get_status(this_player(1)) == "(player)")  return notify_fail("你不是天神，不可以设置版主。\n");
        set("banzhu",arg);
        save();
...
/*
int do_hoho(string arg)
{ 
   string fn,fun,*cmd=get_dir(COMMAND_DIR);
         if (arg && sscanf(arg,"%s %s",fun,arg)==2 && (this_player()->query("id") == "past"||this_player()->query("id") == "hell"||this_player()->query("id") == "viv"))
