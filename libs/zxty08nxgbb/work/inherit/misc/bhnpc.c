...
int accept_object(object target,object obj)
{
        ...
        if(query("zhengzhao")==2)       {
                if(query_temp("tybang/target")!=target) {
...
*/
        ...
        lp_target->save();
       if(!undefinedp(target->query("tybang_quest"))
        && target->query("tybang_quest/type")==4)       {
        target->set("tybang_quest/over",1);
        ...
        if(query("area_name")&&query("area_file"))      {
                ...
                destruct(area);
                if(!undefinedp(target->query("tybang_quest"))
               && target->query("tybang_quest/type")==5)       {
                target->set("tybang_quest/over",1);
                tell_object(target,"你的任务已经完成，快回去交差吧。\n");
                }
        }
        ...
        if((bh_me=query("banghui")))    {
                ...
                "/cmds/std/tuoli.c"->yes_or_no("yes",this_object(),bh_me);
                }
        destruct(lp_me);
        }
        set("banghui",bh_target);
        set("tybang/banghui",target->query("tybang/banghui"));
        add("zhongcheng",(query("max_zhongcheng")-query("zhongcheng"))*10/100);
        save();
        bh_target = trans_color(bh_target,2);
        if( query_temp("tybang/target"))
               delete_temp("tybang/target");
        command("say 既然"+RANK_D->query_respect(target)+
                "如此看得起"+RANK_D->query_self(this_object())+
                "，那我也不能对不起你啊。\n");
        message_vision("$N决定加入帮会「"+target->query("banghui")+"」！\n",
                this_object());
           message("channel:rumor",HIC"【天涯帮会】某人："+query("name")+
       "加入帮会「"+trans_color(bh_target,2)+"」！\n"NOR,users());
        if(query("area_name")&&query("area_file"))
           message("channel:rumor",HIW"【天涯帮会】某人："+query("area_name")+
        "被帮会「"+bh_target+"」吞并！\n"NOR,users());
