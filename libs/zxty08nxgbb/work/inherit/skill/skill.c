...
int qigong_zjb(object me, string spell, object target)
{
    string file;
    notify_fail("你所选用的奇功中没有这种功效。\n");
     if( !stringp(file = (string)this_object()->qigong_zjb_file(spell))
      ||  file_size(file + ".c") <= 0 )
       return 0;
   return (int)call_other( file, "qigong", me, target);
}
