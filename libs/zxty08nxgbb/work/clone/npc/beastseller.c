...
void create()
{        
        set_name("宠物商人", ({ "pet shangren", "pet", "shangren" }) );
        set("gender", "男性" );
        set("age", 21);
        set("long",@LONG
宠物商人来自遥远的天竺，最近突然出现在扬州城，并出售人们
从没有见过的宠物蛋，传说这个宠物蛋一经孵化将具有神奇的功
能，你如果喜欢的话，不妨也选(choose)一只。
LONG );
        // set("title",HIW"魔幻兽商人"NOR);
        set("combat_exp", 5);
        
        set("inquiry", ([ 
                "宠物蛋"     : "只要给我一百两黄金，你就能拥有宠物蛋了！", 
        ]));
...
void greeting(object ob)
{
        ...
        switch( random(5) ) {
                case 0:
                        say( "宠物商人说道：这位" + RANK_D->query_respect(ob)
                                + "，要宠物么？\n");
                        break;
                case 1:
                        say( "宠物商人说道：这位" + RANK_D->query_respect(ob)
...
int do_choose()
{
        ...
        if (me->query_skill("training", 1) < 30)
                return notify_fail("你的驭兽术太低了，即使养了宠物，也会背弃你而去。\n");
        
        if (! me->query_temp("beast/money"))
        {
                command("say 这位" + RANK_D->query_respect(me) + "，每只宠物一百两黄金，你必须先给钱！");
                return 1;
        }
...
void get_subtype(string arg, object ob)
{
        ...
        ob->set("beast/beast_type", n);

        write("\n");
        write("请设定宠物的性别(雄性：1  雌性：0)：");
...
void get_gender(string arg, object ob)
{
        ...
        ob->set("beast/beast_gender", n ? "男性" : "女性");
          
        write("\n");
        write(sort_string(CYN "狂风使者淡淡说道：“既然如此，下面将进行宠物"
              "择主仪式，你不需要有任何的心理负担，放松全身即可，这是个短"
              "暂的仪式！”接着又道：“仪式开始！” \n“宠物，择主”，随着一"
              "声沉喝，狂风使者身上闪起一道光柱，只见从光柱中飞舞无数宠物"
              "蛋，此时狂风使者收回光柱，紧接着挥出一道红光打在宠物蛋"
              "上。宠物蛋在红光的照射下开始出现强烈的骚动，紧接着便纷纷"
              "漂浮起来，飞向半空。这时你才发见原来在表面一层宠物蛋下面"
              "还有着更多的宠物蛋，宠物蛋不断漂浮而起，停在空中散发着"
              "微弱的光芒，照在你的脸上，仿佛在寻找主人，宠物蛋刹那间开"
              "始在空中乱舞，一只宠物蛋向你手中飞来。你本能的伸出双手去"
              "接飞到身前的宠物蛋。\n" NOR, 64)); 
        write(CYN "\n只听狂风使者说道：“仪式结束，恭喜你！”\n" NOR); 
        write(sort_string(CYN "\n只听狂风使者又道：“鉴于你目前功力尚潜，不足"
                "以于宠物蛋进行心灵沟通，我就帮你一次吧。”\n" NOR, 64)); 
        write(sort_string(CYN "你按照狂风使者的指点开始飞快的念道：“在天的见"
                "证之下，集勇气、智慧、与美丽于一身的强大生物，神兽呀！请"
                "你以最深的灵性，聆听我的倾诉，我－" + ob->query("name") +
                "－将与你缔结永生的血之盟约，终此生惟有你与我为终生之盟友"
                "，契。”\n" NOR, 64));        

        write(sort_string(WHT "只见你手中的宠物蛋却传来一阵异样，合在掌中的宠"
                "物蛋温度异常的升高，升到几乎烫手的高温，而且里面隐隐有东"
                "西活动的感觉。$N满心期待中，原本光华的外壳开是出现裂痕，然"
                "后裂痕加大，一股奇特，说不上香或臭的味道开始散出，充斥整个"
                "空间。接着，一颗龙眼大小的头伸了出来，看起来有点像" + 
                beast_type_name[ob->query("beast/beast_type") - 1] +  "的样子，接着整个身子全露"
                "了出来，看来只有巴掌不到的大小，一身应该是银色的，但因为沾"
                "满了湿滑黏液而泛出珍珠色泽，小神兽看了$N一眼，伸出小舌头在"
...
void get_id(string arg, object ob)
{
        ...
        ob->set("beast/beast_id", arg);
        ob->set("beast/beast_unit", beast_unit_name[ob->query("beast/beast_type") - 1]);
        
        write("\n");
        write("请设定宠物的中文名：(可加颜色)");
...
void build_beast(object ob)
{
        ...
        beast_id = ob->query("beast/beast_id");
        beast_name = ob->query("beast/beast_name");
        beast_name += NOR;
        beast_unit = ob->query("beast/beast_unit");
        beast_desc = ob->query("beast/beast_desc");
        beast_desc += NOR;
        beast_gender = ob->query("beast/beast_gender");
       ...
       VERSION_D->append_sn(fn + ".c"); // 给物品增加识别码
        ...
        command("say 你可以吹声口哨召唤你的宠物！<whistle " + beast_id +">\n");
...
int accept_object(object me, object ob)
{
        ...
        if (me->query_skill("training", 1) < 30)
        {
                command("say 你的驭兽术不够，即使养了宠物，也会离你而去！");
                return 0;
        }
        ...
        if (ob->query("money_id"))
        {
                if (ob->value() < 1000000)
                {
                        command("say 这位给的未免少了点！每只宠物蛋要一百两黄金！");
                        return 0;
                }
                else
                {
...
int check_legal_name(string name, int max_len)
{
        ...
        {
                write(sprintf("对不起，宠物中文字必须是 1 到 %d 个中文字。\n",
                      max_len / 2));
                return 0;
        }
