...
#define MIRROR        "/quest/task/mirror"
int ask_mirror();
void create()
{
...
{
str=sprintf("%s的任务是：%s内%s%s，",2>1?"你":me->query("name"),time_period(job["time"]),job["quest_type"],job["quest"]);
        ...
        else
{
 ...
 if (sscanf(base_name(ob[i]),"/d/wudang/npc/%*s" )) continue;
