...
/************************************************************
 * 神兽生命周期则分为：卵期、幼生期、成长期、变态期和成熟期。
 * 幼生期：认主，定下神兽属性（金木水火土），时间一个月；
...
void create()
{
        ...
        set("nopaimai", "这你也要卖？你忘记当初与它的血之盟约吗？");    
        set("no_vipcun", "这你也要存？");              
        set("owner", "zmud");
        set("owner_name", "小刀");
        ...
        set("max_neili", 1000);    
        set("jingli", 1000);
        set("max_jingli", 1000);    让宠物自己根据技能长气
        */               
        set("shen_type", 0);
...
string long()
{
        ...
        {
                result += HIW "它目前正处于幼生期，丝毫看不出来它的战力属性。\n" NOR;
                return result;
        } else
        {
                if (! query("magic/growup"))
                        result += HIW "它目前正处于成长期，它的各项战力属性为：\n" NOR;
                else
                if (! query("magic/become"))
                        result += HIW "它目前已处于变态期，它的各项战力属性为：\n" NOR;
                else
                if (! query("magic/beast"))
                        result += HIW "它目前已处于成熟期，它的各项战力属性为：\n" NOR;
                else
                        result += HIW "它目前已是上古神兽，它的各项战力属性为：\n" NOR;
        ...
        result += "-------------------------------------\n";
        result += HIW "神兽等级：" + level + "\n" NOR;
        result += HIW "防御能力：" + query("magic/armor") + "\n" NOR;
        result += HIW "战力属性：" + chinese_s(query("magic/type")) + "\n" NOR;
        result += HIW "灵力数值：" + query("magic/power")  + "\n" NOR;
...
int heal_up()
{
        ...
        if (query("mud_age") >= 3600 && ! query("magic/owner"))
        {
                ...
                if (environment(ob) != environment())
                        tell_object(ob, HIW "你内心深处隐隐约约的感应到你的神兽(" + name() + HIW ")目前急于认主。\n" NOR);
                else
                {
        ...
        if (query("mud_age") > 3600 && query("mud_age") <= 39600)
        {
	        ...
	        if (! query_condition("need_upgrade"))
                ...
                */
                ...
                if (random(10) == 1)   
                tell_object(ob, HIW "你内心深处隐隐约约的感应到你的神兽(" + name() + HIW 
...
int do_teach(string arg)
{
        ...
        {
              //  if (!query_skill("arg") || query_skill("arg",1)<1) set_skill(arg,1);
...
int do_upgrade(string arg)
{
        ...
        if (! query("magic/owner"))
                return notify_fail("你的神兽暂时还不需要你的能量。\n");
                
        if (environment(me) != environment())
                return notify_fail("你的神兽并不和你处于一个房间。\n");
        ...
        if (me->query_skill("force") < 1000)
                return notify_fail("你的内功根基不够扎实，不能贸然传输给神兽。\n");
        ...
        if (me->query("zjb_dj/dj") < query("zjb_dj/dj")) 
                return notify_fail("你现在积累的实战体会还太少，无法把能量灌输神兽！\n");
                
        message_sort(HIR "$N集中精神，手指神兽脊，同时运转丹田内力，经"
                "由奇经八脉源源由体内流出，注入神兽($n" HIR ")体内。\n" NOR, 
                me, this_object());

        if (me->query("max_neili") < me->query("max_neili")-1000)
        {
                if (random(2) == 1)
                {
                        // 内力未满警告
        ...
        if (query("magic/owner") && ! query("magic/growup"))
        {
                ...
                add("magic/blood", 1);
                add("magic/armor", 2);
                message_vision(RED "$N的神兽($n" RED ")灵性提高了！\n" NOR, me, this_object());
                save();
                return 1;
        } else  
        ...
        if (query("magic/become"))
        {
                ...
                if (b_en >= (b_lv * 2))
                {
                        ...
                        message_vision(HIY "神兽身忽的一亮，一道金光隐入$N的身体，不见了！\n" NOR, me);
                        message_vision(HIG "$N的神兽的升阶了！\n" NOR, me);
                        ...
                        if (b_level <= 9 && b_lv % 9 == 0)
                        {
                                ...
                                set("magic/lvl", 1);
                                add("zjb_dj/dj", 1);                                
                                message_vision(HIY "神兽身忽的泛起红光，无数道紫光隐入$N的身体，不见了！\n" NOR, me);
                                message_vision(HIG "$N的神兽的升级了！\n" NOR, me);
                        }                       
                        ...
                        if (b_level > 9 && ! query("magic/beast"))
                        {
                                ...
                                add("magic/armor", 800);     
                                add("zjb_dj/dj", 10);                                    
                                message_vision(HIY "突然天空出现一道玄光与神兽身泛起的红光相接，神兽全体通红！\n" NOR, me);  
        ...
        if (env == environment(me))
        {
                ...
                if(sizeof(skl)) {//如果存在SKILL
                	...
                	buf = "#include \"/clone/npc/magic-beast.h\"\n";
									...
									me->query("beast/beast_name"), me->query("beast/beast_id"));
									buf += sprintf("\t\tset(\"gender\",\"%s\");\n", me->query("beast/beast_gender"));
									buf += sprintf("\t\tset(\"unit\", \"%s\");\n", me->query("beast/beast_unit"));									
									buf += sprintf("\t\tset(\"long\", \"%s\");\n", me->query("beast/beast_desc")+
									"\n它是" + me->query("name") + "的宠物。\n");
									buf += sprintf("\tset(\"owner\",\"%s\");\n", me->query("id"));
									buf += sprintf("\tset(\"owner_name\",\"%s\");\n", me->query("name"));
									buf +="\tlast_age_set = 0;\n";		

        					//fc += sprintf("\t\tset_skill(\"long\", \"%s\");\n", desc);
                	//数组化skill
                	sname  = sort_array( keys(skl), (: strcmp :) );

                	for(int i=0; i<sizeof(skl); i++) {
                		//设置SKILL
                		buf += sprintf("\tset_skill(\""+sname[i]+"\",%d);\n", skl[sname[i]]);
                		//设置learns
                		buf += sprintf("\tset_learned(\""+sname[i]+"\",%d);\n", lrn[sname[i]]);
                		//判断是否需要JIFA
                		if (sizeof(map) > 0 && map[sname[i]]) 
                		mapskill += sprintf("\tmap_skill(\""+sname[i]+"\",%s);\n", "\""+map[sname[i]]+"\"");
                	}
                	buf += mapskill+"\n";
                	buf +=  "\t::setup();\n}\n\n";
                	write("buf="+buf);
                	...
                	if (file_size(file) > 0) {
                		write("FILE存在！\n");
                		t=rm(file);
                		 //t="/cmds/app/rm"->remove_file(file);
                		 write("t= "+t+"\n");
                	}else write("FILE不存在！\n");
                	//创建新文件
                	assure_file(file);
        					write_file(file, buf); // 写入文件
                }
                //save(); 
                destruct(this_object());
                return 1;
...
int receive_change(object me)
{
        ...
        set_weight(1); 
        move(me);
        set("no_put", "神兽已经被铠化，无法分开。");
        set("no_give", "神兽已经被铠化，无法分开。");
        set("no_drop", "神兽已经被铠化，无法分开。");
        set("no_steal", "神兽已经被铠化，无法分开。");
        set("no_get", "神兽已经被铠化，无法分开。");        
...
int receive_recover(object me)
{
        ...
        message_sort(HIM "$N身上的神兽细胞层层脱落，由身上分离下来的那一"
