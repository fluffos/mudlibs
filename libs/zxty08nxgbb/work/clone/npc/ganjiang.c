...
int recustomizeok(object me)
{
   ...
   log_file("static/dingzuo", sprintf("%s %s 重新打造了 %s(%s) from %s\n",
...
void waiting(object me, object ob, object moye)
{
	...
	if (add_temp("item/waited", -1) <= 0) {
		if (objectp(moye))
                        moye->delete_temp("item");
		log_file("static/dingzuo", sprintf("%s %s do timeout when make item of %s from %s\n",
				 log_time(), log_id(me),
...
int do_name(string arg)
{
	...
	if ((sid = converts(sid, 8, NOCOLOR)) == ILLEGAL_STR)
        ...
        if (file_size(ITEM_DIR + me->query("id")[0..0] + "/" + me->query("id") + "-" + sid + ".c") != -1) {
...
void confirm_make(string arg, object ob)
{
	...
	if (item_filename = create_file(ob)) {
		...
		ob = find_object(item_filename);
         ...
         log_file("static/dingzuo", sprintf("%s %s created  %s(%s) from %s\n",
...
int do_discard(string arg)
{
        ...
        message_vision("$N拿出一些" + money->name() + "交给"
                       "$n。\n", this_object(), me);
        log_file("static/dingzuo", sprintf("%s %s discard  %s(%s) from %s\n",
				log_time(), log_id(me),
...
int do_recustomize(string arg) //重新再CLONE一次
{
    ...
    path = ITEM_DIR + me->query("id")[0..0] + "/" + me->query("id") + "-" + arg + ".c";
    //command("say path= "+path);
    //if (file_size(ITEM_DIR + me->query("id")[0..0] + "/" + me->query("id") + "-" + arg + ".c") != -1) {
