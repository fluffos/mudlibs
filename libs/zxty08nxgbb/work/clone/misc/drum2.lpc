...
#define lunjian2 "/d/huashan/lunjian2"
    ...
    if(sizeof(fighters)<8){
        if (query("loop")){
            ...
            if (stringp(query("drum_knocked"))){
                log_file("/fight/lunjian2", file);
                if (no1){
                   add_title(no1,"本月中年组第一");
               no1->set_skill("dodge",no1->query_skill("dodge",1)+15);
               no1->set_skill("literate",no1->query_skill("literate",1)+15);
               no1->add("zjb_money",100);
         no1->add("potential",70000); 
                    if (no1->query("PKS")>10){    
                        no1->add("PKS",-10);
                    }else no1->delete("PKS"); 
tell_room(room, HIW"恭喜"+no1->name()+"获得本月中年组第一的宝座，同时也得到了因得的奖励!\n");
                }
                if (no2){
                    add_title(no2,"本月中年组第二");
         no2->set_skill("dodge",no2->query_skill("dodge",1)+10);
            no2->add("zjb_money",80);
            no2->add("potential",30000); 

                    if (no2->query("PKS")>7){   
                        no2->add("PKS",-7);
                    }else no2->delete("PKS");
 tell_room(room, HIY"恭喜"+no2->name()+"获得本月中年组第二的宝座，同时也得到了因得的奖励!\n");
                }
                if (no3){
                    add_title(no3,"本月中年组第三");
     no3->set_skill("literate",no3->query_skill("literate",1)+10);
   no3->add("zjb_money",50);
            no3->add("potential",10000); 
                    if (no3->query("PKS")>5){    
                        no3->add("PKS",-5);
                    }else no3->delete("PKS");
 tell_room(room, HIG"恭喜"+no3->name()+"获得本月中年组第三的宝座，同时也得到了因得的奖励!\n");
                }
            }
...
void start_fight()
{
    ...
    if (mapp(fighters=query("group"+round))){
        ...
        if (!room=find_object(lunjian2))
             room=load_object(lunjian2);
        recover(ob1);
        recover(ob2);
        message_vision(HIW+"\n$N和$n施展轻功,双双跃上擂台。\n\n"+NOR,ob1,ob2);
        ob1->move(room);
...
{
     if (!arg) return "";
    if( (strsrch(arg, "本月中年组第一") >= 0)
      ||(strsrch(arg, "本月中年组第二") >= 0)
      ||(strsrch(arg, "本月中年组第三") >= 0))
        {
        arg = replace_string(arg, HIY+"本月中年组第一"+NOR,"");
        arg = replace_string(arg, HIY+"本月中年组第二"+NOR,"");
        arg = replace_string(arg, HIY+"本月中年组第三"+NOR,"");
        }
    return arg;
}
