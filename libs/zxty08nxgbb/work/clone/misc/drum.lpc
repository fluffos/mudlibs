    ...
    if(sizeof(fighters)<8){
        if (query("loop")){
            ...
            if (stringp(query("drum_knocked"))){
                ...
                if (no1){
                    add_title(no1,"本月天下第一");
                    no1->add("potential",5000);     
                        new("/clone/money/thousand-gold",500)->move(no1);
                    if (no1->query("PKS")>10) 
                         no1->add("PKS",-10);
                    else no1->delete("PKS");
                }
                if (no2){
                    add_title(no2,"本月天下第二");
                        new("/clone/money/thousand-gold",300)->move(no2);
                    no2->add("potential",3000);     
                    if (no2->query("PKS")>7)                     no2->add("PKS",-7);
                    else no2->delete("PKS");
                }
                if (no3){
                    add_title(no3,"本月天下第三");
                    no3->add("potential",2000);     
                        new("/clone/money/thousand-gold",200)->move(no3);
                    if (no3->query("PKS")>5)                     no3->add("PKS",-5);
                    else no3->delete("PKS");
                }
            }
