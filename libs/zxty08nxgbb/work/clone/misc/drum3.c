...
#define lunjian3 "/d/huashan/lunjian3"
    ...
    if(sizeof(fighters)<8){
        if (query("loop")){
            ...
            if (stringp(query("drum_knocked"))){
                log_file("/fight/lunjian3", file);
                if (no1){
                    add_title(no1,"本月老年组第一");
              no1->set_skill("dodge",no1->query_skill("dodge",1)+15);
              no1->set_skill("force",no1->query_skill("force",1)+15);
              no1->add("zjb_money",150);
              no1->add("potential",70000);
 message_vision(HIW"恭喜"+no1->name()+"获得本月老年组第一的宝座，同时也得到了因得的奖励!\n",no1);
                ...
                if (no2){
                    add_title(no2,"本月老年组第二");
              no2->set_skill("dodge",no2->query_skill("dodge",1)+10);
              no2->add("zjb_money",100);
              no2->add("potential",30000);
 message_vision(HIY"恭喜"+no2->name()+"获得本月老年组第二的宝座，同时也得到了因得的奖励!\n",no2);
                ...
                if (no3){
                    add_title(no3,"本月老年组第三");
              no3->set_skill("dodge",no3->query_skill("dodge",1)+7);
              no3->add("zjb_money",50);
              no3->add("potential",10000);
message_vision(HIG"恭喜"+no3->name()+"获得本月老年组第三的宝座，同时也得到了因得的奖励!\n",no3);
...
void start_fight()
{
    ...
    if (mapp(fighters=query("group"+round))){
        ...
        if (!room=find_object(lunjian3))
             room=load_object(lunjian3);
        recover(ob1);
        recover(ob2);
        message_vision(HIW+"\n$N和$n施展轻功,双双跃上擂台。\n\n"+NOR,ob1,ob2);
        ob1->move(room);
...
{
     if (!arg) return "";
    if( (strsrch(arg, "本月老年组第一") >= 0)
      ||(strsrch(arg, "本月老年组第二") >= 0)
      ||(strsrch(arg, "本月老年组第三") >= 0))
        {
        arg = replace_string(arg, HIY+"本月老年组第一"+NOR,"");
        arg = replace_string(arg, HIY+"本月老年组第二"+NOR,"");
        arg = replace_string(arg, HIY+"本月老年组第三"+NOR,"");
        }
    return arg;
}
