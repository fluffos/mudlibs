...
int do_stock(string arg)
{
        ...
        if (goods->is_character())
              ...
              if (goods->query("age")||goods->query("combat_exp")) 
                return notify_fail("你想贩卖人口啊！\n"); 
            if( goods->query("nopaimai") ) 
                   return notify_fail("这是孤品啊，我们这里可不敢承接这个买卖了！\n"); 
          if( goods->query("ty_gift") )      
