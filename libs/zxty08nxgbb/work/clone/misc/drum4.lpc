...
#define lunjian4 "/d/huashan/lunjian4"
...
int sort_user(object ob1,object ob2)
{
     if (ob1->query("zjb_dj/dj") < ob2->query("zjb_dj/dj"))
            return 1;
      else return -1;
}
...
int sort_winner(object ob1,object ob2)
{
      ...
      if (!ob1) return 1;
      if (!ob2) return -1;
      if ((arena[ob1] == arena[ob2]) && (ob1->query("zjb_dj/dj") > ob2->query("zjb_dj/dj")))
            return -1;
    ...
    if(sizeof(fighters)<8){
        if (query("loop")){
            ...
            if (stringp(query("drum_knocked"))){
                log_file("/fight/lunjian4", file);
                if (no1){
                    add_title(no1,"本月寿星组第一");
              no1->set_skill("dodge",no1->query_skill("dodge",1)+30);
              no1->set_skill("force",no1->query_skill("force",1)+30);
              no1->add("zjb_money",300);
              no1->add("potential",100000);
 message_vision(HIW"恭喜"+no1->name()+"获得本月寿星组第一的宝座，同时也得到了因得的奖励!\n",no1);
                ...
                if (no2){
                    add_title(no2,"本月寿星组第二");
              no2->set_skill("dodge",no2->query_skill("dodge",1)+20);
              no2->set_skill("force",no2->query_skill("force",1)+20);
              no2->add("zjb_money",200);
              no2->add("potential",50000);
 message_vision(HIY"恭喜"+no2->name()+"获得本月寿星组第二的宝座，同时也得到了因得的奖励!\n",no2);
                    if (no2->query("PKS")>7){   
                        no2->add("PKS",-7);
                    }else no2->delete("PKS");
                }
                if (no3){
                    add_title(no3,"本月寿星组第三");
              no3->set_skill("dodge",no3->query_skill("dodge",1)+10);
              no3->set_skill("force",no3->query_skill("force",1)+10);
              no3->add("zjb_money",100);
              no3->add("potential",10000);
message_vision(HIG"恭喜"+no3->name()+"获得本月寿星组第三的宝座，同时也得到了因得的奖励!\n",no3);
                    if (no3->query("PKS")>5){    
                        no3->add("PKS",-5);
                    }else no3->delete("PKS");
                }
            ...
            delete("drum_knocked");
            delete("lunshu");
         environment()->set("exits",([ "west":"/d/huashan/houzhiwangyu",]));
...
void start_fight()
{
    ...
    if (mapp(fighters=query("group"+round))){
        ...
        if (!room=find_object(lunjian4))
             room=load_object(lunjian4);
        recover(ob1);
        recover(ob2);
        message_vision(HIW+"\n$N和$n施展轻功,双双跃上擂台。\n\n"+NOR,ob1,ob2);
        ob1->move(room);
...
{
     if (!arg) return "";
    if( (strsrch(arg, "本月寿星组第一") >= 0)
      ||(strsrch(arg, "本月寿星组第二") >= 0)
      ||(strsrch(arg, "本月寿星组第三") >= 0))
        {
        arg = replace_string(arg, HIY+"本月寿星组第一"+NOR,"");
        arg = replace_string(arg, HIY+"本月寿星组第二"+NOR,"");
        arg = replace_string(arg, HIY+"本月寿星组第三"+NOR,"");
        }
    return arg;
}
...
int give_money(object *players,object *fighters)
{
    if (query("drum_knocked")){
        ...
        foreach (object user in players){
                if (user && !wizardp(user) && ( environment()==environment(user) ) && user->query("mud_age") > 86400*2 ){
