...
// By zjb,笨蛋daniel,如果该room没出口，你go个头
// tmd还出了debug,还好我修改及时..
...
void create()
{
     ...
     id = this_player()->query("id");
          ...
          set_name(name, ({ id+" shashou", "shashou" }) );
...
*/
        skill = (me->query_skill("dodge",1)+me->query_skill("force",1))/3+me->query("zjb_dj/dj") *10;
...
//以下开始添加任务NPC等级计算..
   dengji = me->query("zjb_dj/dj") / 2;
     ...
     set("zjb_dj/dj",dengji);
        ...
        set("max_neili",me->query("zjb_dj/dj")*50000);
        set("neili",me->query("zjb_dj/dj")*50000);
        set("max_jingli",me->query("zjb_dj/dj")*50000);
...
void die()
{
        ...
        if(killer && killer->query("id")==me->query("killer"))
        {
                   reward_exp=random(1000)+
                         ((int)killer->query_int()*5)+((int)me->query("kar")*20);
/*
                   if(killer->query("zjb_dj/dj") < 5 )
                           reward_exp -=reward_exp/6;
              if(killer->query("zjb_dj/dj") > 10 )
...
*/
          ...
          reward_pot=reward_exp/10;
           ...
           {
                   reward_exp +=500+random(1000); 
                   reward_pot +=250+random(500);
           }
 if (reward_exp > 10000 ) reward_exp = 9804+random(200);
//       reward_pot = random(reward_pot);
          reward_pot=reward_exp/10;
  if (reward_pot > 3100) reward_pot = 2988+random(200);
           if (reward_pot<0) reward_pot=0;

                   MIS_REW->give_reward_questkiller(killer,reward_exp,reward_pot);      
                   
//                   killer->move("/d/xiangyang/guangchang",1);
