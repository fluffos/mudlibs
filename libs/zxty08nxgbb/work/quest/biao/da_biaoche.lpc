...
{       
        seteuid(getuid());
        set_name(HIG"长风大镖车"NOR, ({ "da biaoche", "biaoche", "che"}) );
        set_weight(500000);
        set("no_get", 1);
        set("unit", "辆");
        set("value", 0);
        set("long", 
          HIY"一辆长风镖局的大镖车，上面结结实实盖着蓬布，一群镖头赶(gan)着车，牢牢地护在一边。"NOR); 
        set("owner","xingyun");
        set("parter","zjb");
        set("owner_name","星空");
        set("no_clean_up",1);
...
int destroy2(object ob)
{
    ...
    if(objectp(ob))
    {
    message("channel:ty",MAG"【天涯传闻】"+HIB+"由于"+query("owner_name")+
     "率领的镖队未能如期抵达，长风镖局已派人前去接管镖队！\n"NOR,users());
     ...
     if(objectp(parter))
     {
     parter->set_temp("quest_yunbiao/have_task",0);
...
{        
   ...
   if(!objectp(me)||!objectp(parter)) 
        ...
        {
        if(objectp(killer[i]) 
          && (killer[i]->query("killer1")==me->query("id")
          || killer[i]->query("killer2")==me->query("id")))
         flag=1;
        }
       ...
       if(file_name(obj)!=ob->query("file"))
       {
         ...
         if (random(10)<2 && !environment(ob)->query("no_fight"))
         {
          ...
          robber->set("killer1",me->query("id"));
          robber->set("killer2",parter->query("id")); 
       ...
       {
        reward_exp1=(int)me->query("combat_exp")/4000;
        reward_exp1=reward_exp1+(int)me->query("quest_yunbiao/bonus")*25;
       ...
       if(reward_exp1>20000) reward_exp1=20000;
        ...
        reward_pot1=reward_exp1/4;
        reward_score1=reward_exp1/40;
        reward_exp2=(int)parter->query("combat_exp")/5000;
        reward_exp2=reward_exp2+(int)parter->query("quest_yunbiao/bonus")*30;

        reward_exp2=reward_exp2+(int)parter->query_temp("quest_yunbiao/bandit_killed")*200;        
        if(reward_exp2>12000) reward_exp2=12000;
        reward_exp2=reward_exp2/2+random(reward_exp2/2);
        reward_pot2=reward_exp1/4;
...
if (parter->query("more_money")!=0)
{
parter->add("money",this_player()->query("more_money")*100000000);
parter->set("more_money",0);
}
...
if (parter->query("money",1)>100000000)
{
parter->add("zjb_money",parter->query("money",1)/100000000);
parter->set("money",100);
tell_object(parter,HIR"你在钱庄的钱已达到一万两黄金！\n"NOR);
tell_object(parter,HIR"星空已经帮你把钱庄里的黄金转换成了"HIB"星空币"HIR"存在了钱庄!\n"NOR);
}
...
int destroy1(object ob)
{
    ...
    if(objectp(ob) && !present(ob->query("owner"),environment(ob)) && !present(query("parter"),environment(ob)) )
    {
     me=find_player(ob->query("owner"));
     if(!me) me=find_living(ob->query("owner"));
     parter=find_player(ob->query("parter"));
     if(!me) me=find_living(ob->query("parter"));     
     if(objectp(me))

    message("channel:ty",HIB"【镖局传闻】"+query("owner_name")
     +"率领的镖队在半路让人给劫了！\n"NOR,users());
     remove_call_out("destroy1");
     remove_call_out("destroy2");
     destruct(ob);
