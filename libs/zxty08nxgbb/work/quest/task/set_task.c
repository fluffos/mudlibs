...
#define ZIXU          "/clone/npc/halfgod"      //task发布宣布者
#define TASK_CARRIER  "/quest/task/npc/task_carrier" //task 携带者
#define TASK_OBJECT   "/quest/task/set_task/"            //task物件目录
...
void set_task()
{
    ...
    CHANNEL_D->do_channel(zixu, "chat", HIY"TASK任务重新分布完毕。\n"NOR);
...
int do_return(object ob, object me, string arg)
{
 ...
 if ( count <= 10 ) count = 10;
    if ( count >= 50 ) count += random(count);

    exp = 5000 * (count + random(count) * 2) + 50;
    pot = 2000 * (count + random(count) * 2) + 50;
...
string set_item(object me)
{
        ...
        if (me->query("mirror_count") == 400)
        {
              ...
              log_file("static/mirror", sprintf("%s(%s) 获得终极矿石 at %s.\n",
              me->name(1), me->query("id"), ctime(time())));
        ...
        if (me->query("mirror_count") == 500)
        {
              ...
              log_file("static/mirror", sprintf("%s(%s) 获得宝珠 at %s.\n",
              me->name(1), me->query("id"), ctime(time())));
