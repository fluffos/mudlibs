...
#include <ansi.h>
#define   TASK_OBJ_PATH  "/quest/task/set_task/"       //task物件目录
...
{ 
        ...
        string msg, *dirs, dir_name, dir_total;
        mapping dir;
    ...
    foreach(dir_name in keys(dir))
       ...
       //add by Amis@XO2                  
       if ( ! npc->is_character() ) env = npc;
       else env = environment(npc);        
       ...
       if( (int)env->query_door(dirs[i], "status") )
            ...
            dirs -= ({ 0 });
       if( sizeof(dirs)==0 )
                        dir_total = "    这里没有任何明显的出路。\n";
       else if( sizeof(dirs)==1 )
                        dir_total = "    这里唯一的出口是 " + BOLD + dirs[0] + NOR + "。\n";
       else
                        dir_total = sprintf("    这里明显的出口是 " + BOLD + "%s" + NOR + " 和 " + BOLD + "%s" + NOR + "。\n",
