...
int do_deposit(string arg)
{
    ...
    if (amount >20000)
        return notify_fail("一次最多只能存2万个数量。\n");
    if (money_ob->query("base_value")*amount < 0 ||
        this_player()->query("money",1)+(money_ob->query("base_value")*amount) <0
      ||this_player()->query("money",1)+(money_ob->query("base_value")*amount) >     2100000000)
        return notify_fail("你一次存这么多钱会暴的，存少点吧。\n");
    ...
    message_vision( sprintf("$N从身上取出%s%s%s，存入钱庄。\n",
      chinese_number(amount),money_ob->query("base_unit"),
money_ob->query("name")),
        this_player() );

    this_player()->add("money",money_ob->query("base_value")*amount);
...
int do_withdraw(string arg)
{
    ...
    money_ob = "/clone/money/"+money;
    if (money_ob->query("base_value")>10000)
        return notify_fail("你只能取黄金、银子或铜板！\n");
//add by daniel move时没有释放内存bug
         if (sizeof(all_inventory(this_player()))>70 && userp(this_player()))
                        return notify_fail( "你身上东西太多了。\n");
    ...
    if (value_money*101/100 >this_player()->query("money") 
        && this_player()->query("zjb_money")<1)
...
tell_object(this_player(),HIR"你的存款不够，系统自动把你的星空币换成存款了！\n"NOR);
    ...
    MONEY_D->pay_player(this_player(), value_money);
