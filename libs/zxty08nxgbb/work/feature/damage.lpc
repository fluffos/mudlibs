...
void unconcious()
{
    ...
    //擂台比武不死
     if( objectp(room=environment(me))      &&stringp(fightroom=room->query("fight_room")) 
     &&!downroom=find_object(fightroom))
...
message("channel:rumor",NOR"〖"HIY"武林争霸"NOR"〗"+HIY+me->name()+"被"+killer->name()+"一脚踢下了擂台！\n"NOR,users());
                       unconcious();
                       return;
               }
...
void die()
{
    ...
    if( wizardp(this_object()) && query("env/immortal") ) return;
/*
  if(objectp(killer = query_temp("last_damage_from"))&&environment(this_object())->query("no_fight")&&userp(this_object()) ) {
       this_object()->remove_all_enemy();
               unconcious();
     return; 
     }
*/
    if (objectp(previous_object())&&!SECURITY_D->valid_set( this_object(), previous_object()) )
                        return ;

    //擂台比武不死
    if( objectp(room=environment(me)) 
    &&stringp(fightroom=room->query("fight_room")) )
    {
        ...
        if( !objectp(killer = me->query_temp("last_damage_from")) )
                ...
                message("channel:rumor",NOR"〖"HIY"武林争霸"NOR"〗"+HIY+me->name()+"被"+killer_name+"一脚踢下了擂台！\n"NOR,users());
                unconcious();
                return;
                }
    }        
    ...
    if (objectp(killer)) corpse->set("killedby",killer->query("id"));
    if( userp(this_object()) ) {
...
int heal_up()
{
    ...
    if (environment()&&environment()->query("freeze")) return -1;
    update_flag = 0;
