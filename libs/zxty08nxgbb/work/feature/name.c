...
varargs string short(int raw)
{
    ...
    if( stringp(query("banghui")) ) 
          str=sprintf("%s%s"+NOR"%s%s",query("tybang/banghui"),
        (string)query("tybang/bh_rank")? query("tybang/bh_rank"):"帮众"," ",str);
    ...
    if (!raw) {
        ...
        if ((int)query_temp("pending/exercise") != 0)
            ...
            return name() + "正坐在地下吐纳炼精。"; else
        if(interactive(this_object())
        && query_idle( this_object() ) > 120)
{
/*
                 fadai = (int)this_object()->query("int")/10;
        	if (fadai<0) fadai = 0;
                if (fadai>1000) fadai = 800;
  this_object()->improve_skill("fadai-qigong",fadai);
*/
       str += HIM+" <发呆"+chinese_number(query_idle(this_object())/60)+"分钟>"+NOR; }else
        if(this_object()->is_ghost() ) str = HIB "(幽魂) " NOR + str; else
        if(in_input() ) str += HIC " <输入文字中>" NOR; else
        if(query_temp("netdead")) {
        if( query("doing") ) str += HIR " <离线闭关中>" NOR;
        else str += HIG " <断线中>" NOR;
        } else
        if((string)this_object()->query("doing"))
        str += HIY " <闭关中>" NOR; else
        if((string)this_object()->is_fighting())   str += HIR " <搏斗中>" NOR;
        else
        if(in_edit() ) str += HIY " <编辑档案中>" NOR; else
        if(query("env/invisibility")) str +="<隐身中>"NOR;
           if(query("vip")) str += HIY  "<VIP>"NOR;
