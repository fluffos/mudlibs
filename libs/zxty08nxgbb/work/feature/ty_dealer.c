...
string value_string(int value)
{
        if( value < 1 ) value = 1;
                 ...
                 return chinese_number(value) + "天涯币";
        else    if( value < 10000 )
...
int do_list()
{
        ...
        int i,j,vip,amount;
        who=this_player();
        vip=who->query("vip/vip");
        inv=all_inventory(this_object());
        if (!sizeof(inv) && !arrayp(goods = query("vendor_goods")))
                return notify_fail("目前没有可以卖的东西。\n");
        if (arrayp(goods = query("vendor_goods"))){
                printf("这是%s提供给你的物品价目表：\n非VIP玩家购买金额比VIP玩家贵3TYB\n", query("name"));
                for (i = 0; i < sizeof(goods); i++)
                printf("%-s："HIG"%20s\n"NOR"\t%s", 
goods[i]->short()+fill_blank(remove_ansi(goods[i]->short()), 30) ,
vip?(value_string(goods[i]->query("tianya_money"))):value_string(goods[i]->query("tianya_money")+3),

//value_string(goods[i]->query("tianya_money")), 
goods[i]->query("long"));
        if(!vip){
                printf(HIR"\n你不是VIP玩家，所看到的价格比VIP玩家贵3TYB\n"NOR,);}
        ...
        printf("这是%s提供给你的物品价目表：\n", query("name"));
                ...
                for (i=0;i<sizeof(my);i++)
                        printf("%-s：%20s\n20s\n", goods[i]->short()+fill_blank(remove_ansi(goods[i]->short()), 30) ,
//                        value_string(goods[i]->query("tianya_money"))),goods[i]->short();
                        vip?(value_string(goods[i]->query("tianya_money")+3)):value_string(goods[i]->query("tianya_money"))),goods[i]->short();
...
int do_buy(string arg)
{
    ...
    if (inv && sizeof(inv)>=50) {
        ...
        if(amount>20) return notify_fail("你一次不能买这么多。\n");
        if (items == ""){
                if (obj->query("equipped")||obj->query("money_id"))
                        return notify_fail(owner->query("name")+"气冲冲地说：身上的东西，不卖！\n");
        price=obj->query("tianya_money");
        }else{
        price=items->query("tianya_money");
                if (amount>9)
      ...
      if ( !who->query("vip") ){ price=price+3;}

       if( who->query("moneyvip")+1>price*amount ) {
                for (i=0;i<amount;i++){
                        if (items == ""){
                ...
                amount=i;
                who->add("moneyvip",-price*amount);
                 who->add("vip/buy",price*amount);

                if (items == "")
              message_vision("$N向$n买下" +CHINESE_D->chinese_number(amount)+obj->query("unit") + obj->query("name") + "。\n\n"HIR"请尽快使用kaiguang xxx为你的装备做记号，使用之外该装备下线才不会失落！\n如未使用kaiguang而造成的装备下线丢失，本站不负任何责任！。"NOR,
                                who, owner );
                else
                message_vision("$N向$n买下" +CHINESE_D->chinese_number(amount)+items->query("unit") + items->query("name") + "。\n\n"HIR"请尽快使用kaiguang xxx为你的装备做记号，使用之外该装备下线才不会失落！\n如未使用kaiguang而造成的装备下线丢失，本站不负任何责任！\n"NOR,
                                who, owner );
log_file("login/vip",sprintf("%s买了%d个%s,(%s)\n",who->query("id"), amount,items->query("name"),
