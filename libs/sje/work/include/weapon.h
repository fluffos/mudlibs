// weapon.h

#include "/inherit/weapon/poisoned_weapon.lpc"

#ifndef __WEAPON__
#define __WEAPON__

#define DEFAULT_WEAPON_LIMB "右手"

#define TWO_HANDED      1
#define SECONDARY       2
#define EDGED           4
#define POINTED         8
#define LONG            16

#define AXE             "/inherit/weapon/axe"           // 斧
#define BLADE           "/inherit/weapon/blade"         // 刀
#define DAGGER          "/inherit/weapon/dagger"        // 矛
#define FORK            "/inherit/weapon/fork"          // 叉
#define HAMMER          "/inherit/weapon/hammer"        // 锤
#define HOOK            "/inherit/weapon/hook"          // 钩
#define SWORD           "/inherit/weapon/sword"         // 剑
#define STAFF           "/inherit/weapon/staff"         // 杖
#define STICK           "/inherit/weapon/stick"         // 棒
#define CLUB            "/inherit/weapon/club"          // 棍
#define THROWING        "/inherit/weapon/throwing"      // 暗器
#define TWOWEAPON	"/inherit/weapon/twoweapon"	// 双器
#define WHIP            "/inherit/weapon/whip"          // 鞭
#define BRUSH           "/inherit/weapon/brush"         // 笔
#define BOW		"/inherit/weapon/bow"		// 弓
#define ARROW		"/inherit/weapon/arrow"		// 箭
#define SPEAR            "/inherit/weapon/spear"          // 枪

#define WEAPON_COMBO    "/inherit/weapon/weapon_combo"  //组合

#define F_AXE           "/inherit/weapon/_axe"
#define F_BLADE         "/inherit/weapon/_blade"
#define F_DAGGER        "/inherit/weapon/_dagger"
#define F_FORK          "/inherit/weapon/_fork"
#define F_HAMMER        "/inherit/weapon/_hammer"
#define F_HOOK           "/inherit/weapon/_hook"
#define F_SWORD         "/inherit/weapon/_sword"
#define F_STAFF         "/inherit/weapon/_staff"
#define F_BRUSH         "/inherit/weapon/_brush"
#define F_THROWING      "/inherit/weapon/_throwing"
#define F_WHIP          "/inherit/weapon/_whip"
#define F_SPEAR          "/inherit/weapon/_spear"

#endif
