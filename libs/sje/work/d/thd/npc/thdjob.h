...
int ask_job() {
  ...
  if ((int)me->query("combat_exp") > 500000) {
    tell_room(environment(ob), "黄蓉看了看" + ob->query("name") + ",说道: 你武功太好了，出去做大侠吧！\n", ({ ob }));
    tell_object(ob, "黄蓉看了看你,说道：你武功太好了，出去做大侠吧！\n");
    ob->delete_temp("thd/job_s");
    return 1;
  }
  ...
  // thd_job 主要对新手开放，高手实在太简单了，决定大于200 times,不能做thd job
  if (ob->query("thd_job") > 200) {
    command("say 难得你有这份心意，但是这些事情还是留给低辈弟子去干吧。");
    return 1;
  }
  ...
  // 设置 busy
  ob->apply_condition("thd_job", 15 + random(20));
...
int accept_object(object who, object ob) {
  ...
  if (ob->query("id") == "lv feicui"
    ...
    exp = 30 + (int)who->query(
      "thd_job",
      1
    ) / 20 + random(who->query(
      "thd_job",
      1
    ) / 20) + random((int)who->query(
      "thd_job",
      1
    ) / 20) + random((int)who->query(
      "age",
      1
    )) + random((int)who->query("int", 1)) - job_busy * job_busy;
    if (exp < 100) exp = 50 + random(30);
    if (exp > 300) exp = 280 + random(50);
    ...
    if ((int)who->query("potential", 1) > ((int)who->query("max_pot", 1) * 3 / 2))
      who->set("potential", who->query("max_pot", 1) * 3 / 2);
