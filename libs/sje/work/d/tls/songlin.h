int valid_leave(object me, string dir) {
  ...
  if (dir == "north")
    me->add_temp("mark/steps", -1);
  if (me->query_temp("mark/steps") == me->query_temp("xdt/steps")
    && me->query_temp("mark/step") == me->query_temp("xdt/step")) {
    me->move(__DIR__ "xdt");
    me->delete_temp("mark/steps");
    me->delete_temp("mark/step");
    return notify_fail("你正走着，透过树枝，隐约发现前面有片空地，你拨开树枝钻了过去。\n");
  }
  if (me->query_temp("mark/steps") == -5 && me->query_temp("mark/step") == 8) {
    me->move(__DIR__ "lsy");
    me->delete_temp("mark/steps");
    me->delete_temp("mark/step");
