...
int main(object me, string arg)
{
	...
	if(sscanf(item, "%d %s", amount, item)==2) {
		if( !objectp(obj = present(item, me)) )	
          ...
          if( !wizardp(me) && obj->query("no_give") )
...
int do_give(object me, object obj, object who)
{
    ...
    if (userp(me) && who->query("env/no_accept") && wiz_level(me)<=wiz_level(who)) return 0;
	if( !interactive(who) && !who->accept_object(me, obj) )
		return 0;
		
	if(obj->query("dynamic_quest"))
        if(QUEST_D->quest_reward(me,who,obj))
        return 1;
	...
	if( obj->move(who) ) {
		...
		message("vision", sprintf("%s给你%s%s。\n", me->name(),
			unit, obj->name()), who );
		message("vision", sprintf("%s给%s%s%s。\n", me->name(), who->name(),
