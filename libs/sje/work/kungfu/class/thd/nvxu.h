...
int ask_test1() {
  ...
  if (!present("biyu xiao", me))
    if (obj = carry_object("/clone/weapon/biyuxiao")) obj->wield();
  else
    command("wield xiao");
  if (!present(
    ob,
    environment(me)
  ) || ob->query("qi") <= 0 || ob->query("jing") <= 0 || ob->query("jingli") <= 0) {
    command("sneer");
...
int do_answer(string arg) {
  ...
  if (ob->query_temp("thd_marry") == 2) {
    ...
    if (arg == "羽") {
      if (!present("biyu xiao", this_object()))
        if (obj = carry_object("/clone/weapon/biyuxiao")) obj->wield();
      ...
      return 1;
    } else if (arg == "我不知道" || arg == "不知道" || arg == "dunno") {
      if (!present("biyu xiao", this_object()))
        if (obj = carry_object("/clone/weapon/biyuxiao")) obj->wield();
      command("sneer " + ob->query("id"));
      command("say 这都答不出？！\n");
...
道：好吧，这第二关就算是个平手。\n", ({ ob }));
  ...
  if (ob->query_temp("thd_marry") == 3) {
    ...
    if (arg == "虚胜实") {
      ...
      new(__THD__ "obj/shoufa.lpc")->move(ob);
