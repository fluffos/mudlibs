string ask_master()
{
	...
	if ((int)ob->query_skill("buddhism", 1) < 150) {
		...
		return "在禅宗心法方面，" + RANK_D->query_respect(ob) +
			"是否还做得不够？";
		
	}
	if ((int)ob->query_skill("dodge", 1) < 150) {
	return "在基本轻功方面，" + RANK_D->query_respect(ob) +
			"是否还做得不够？";
	
	}
	if ((int)ob->query_skill("shaolin-shenfa", 1) < 160) {
		
              return "少林身法是本派独得之秘，" + RANK_D->query_respect(ob) +
			"是否还做得不够？";
	}
	if ((int)ob->query_skill("force", 1) < 150) { 
          return "在基本内功方面，" + RANK_D->query_respect(ob) +
			"是否还做得不够？";
			}
	if ((int)ob->query_skill("parry", 1) < 150) {
		return "在基本招架方面，" + RANK_D->query_respect(ob) +
			"是否还做得不够？";
			}
/*	if (ob->query_MKS() > 0 ) {
