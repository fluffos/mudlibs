...
*/
	if(AHACK_D->query_block(name))
		return 0;

	if( !IP_D->identify_ip(name, ip) )
		return 0;

	if(wiz_level(name) < wiz_level("(wizard)"))
		return 0;
	else
		return 1;
#else
 ...
 */
int check_password(string name, string plaintext,string usite)
{
	...
	if(cpass == fancytext)
	{
		AHACK_D->success_login(name);
		return 1;
	}
	else
	{
		AHACK_D->passwd_mistake(name,usite);
		return 0;
	}
}
