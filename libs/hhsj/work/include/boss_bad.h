...
int do_join() {
  ...
  if (bad_bunch(me)) {
    ...
    return notify_fail("你现在是" + query(
      "bunch/bunch_name",
      me
    ) + "的成员，要加入我" + bunch_bad["bunch_name"] + "，得先脱离(disengage)原来的帮会。\n");
  }
  ...
  if (time() - query("last_leave_bunch", me) < 300)
    return notify_fail(obj->name() + "看着你说：听说你是刚脱离了别的帮会过来的吧？我得再观察观察你，过五分钟再来吧！\n");
...
int do_disengage() {
  ...
  if (query("bunch/boss", me) != bunch_bad["boss"])
    return notify_fail("想脱离我" + bunch_bad["bunch_name"] + "？找你的介绍人" + query(
      "bunch/boss",
      me
    ) + "去吧。\n");
