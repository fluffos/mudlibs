  ...
  switch (job["type"]) {
    ...
    case "截镖":
      ...
      if (sizeof(obj) < 10) {
        ...
        return "去踩盘的弟兄们回来讲，" + query("nickname", ob) + "的镖车将经过" +  //region_names[region] +
        to_chinese("region") + query("short", dest) + "。";
      } else {
        ...
        return "去踩盘的弟兄们回来讲，" + biaoju + "的镖车将经过" +  //region_names[region] + 
        to_chinese("region") + query("short", dest) + "。";
      }
      break;
