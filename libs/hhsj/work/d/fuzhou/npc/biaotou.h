...
int ask_jiaobiao() {
  ...
  if (query_temp("fuwei_destm", me) != query("id", ob)) {
    message_vision(
      HIR "$N问$n道：“搞错了！搞错了！你怎么把给" + query_temp("fuwei_dest", me) + "分局的镖车送到我这来呢？”\n" NOR,
      ob,
      me
    );
    return 1;
  ...
  message_vision(
    YEL "$N和" + query_temp("fuwei_dest", me) + "分局的趟子手们一阵粗言秽语、喊爹骂娘地勾肩搭背一拥出门去了。\n" NOR,
    biaotou
  );
  ...
  tell_object(
    ...
    sprintf(
      HIR + query(
        ...
        "name",
        me
      ) + "称许道：“你被奖励了！得到%s点经验和%s点潜能和%s点江湖阅历。镖局发给你四十两黄金做你的薪金和补偿你的押金。”\n" NOR,
      chinese_number(give_exp),
      chinese_number(give_pot),
      chinese_number(give_score)
    ) + NOR
  );
