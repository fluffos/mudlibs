    ...
    for (i = 0; i < sizeof(target); i++) {
      ...
      if (times / 2 * 2 != times && query(
        ...
        if (query("race", target[i]) != "人类" || times / 2 * 2 == times)
          ...
          replace_string(fail, "$f", to_chinese(map_force)) + "\n" NOR,
          me,
          target[i]
        );
        ...
        damage = force + music / 3 + random(music / 2) * flag + random(tag * 10) + (-query(
          "max_neili",
          target[i]
        ) / 30) + -random(force2 / 2) + -force2 / 3;
        ...
        if (damage > 0) {
          ...
          if (!target[i]->is_killing(query(
            "id",
            me
          )) || !target[i]->is_fighting(me) || !me->is_fighting(target[i]))
            target[i]->kill_ob(me);
          ...
          if ((query("jing", target[i]) < 0 || query("jingli", target[i]) < 0)
            ...
            "jing",
            target[i]
          ) < 0 || query("jingli", target[i]) < 0) && query("race", target[i]) != "人类")
