...
mapping can_use_pfm(string pfm_id, object me, object target) {
  ...
  if (pfm_id == "dh") {
    ...
    if (target->query_busy() > 3)
      return ([ "err_msg": query("name", target) + "还忙得很呢，还是趁机攻击吧" ]);
    ...
    if (query_temp("hj_special/sh", target)) {
      ...
      tell_object(
        me,
        HIR + query("name", target) + HIR "被拍个正中，突然两眼发直，整个人呆住了似的。(造成忙时" + target_busy + "秒)\n\n" NOR
      );
      tell_room(
        environment(target),
        HIR + query("name", target) + HIR "被拍个正中，突然两眼发直，整个人呆住了似的。\n\n" NOR,
        ({ me, target })
      );
