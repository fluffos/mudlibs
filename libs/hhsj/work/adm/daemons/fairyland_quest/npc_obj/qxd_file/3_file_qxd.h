...
void checking() {
  ...
  if (!hp || hp < 1) {
    ...
    CHANNEL_D->channel_broadcast(
      "rumor",
      sprintf("幻境: 听说%s[%s]探险失败，离开了幻境。", query("name", me), query("id", me))
    );
    return;
  }
  ...
  if (wait_over == "yes" && time > DENG_TIME / 5) {
    ...
    CHANNEL_D->channel_broadcast(
      "rumor",
      sprintf("幻境: 听说%s[%s]探险成功，离开了幻境。", query("name", me), query("id", me))
    );
    return;
  }
...
void enter_hj() {
  ...
  if (query("huanjing2003/last_power_times", me)) {
    if (!query("huanjing2003/last_power", me)) {
      tell_object(
        me,
        sprintf(
          "你还可以使用%d次累积力量，但是现在你的累积力量比你能得到的基本力量还低，并未生效。\n",
  ...
  if (query("huanjing2003/last_skills_times", me)) {
    if (!query("huanjing2003/last_skills_lv", me)) {
      tell_object(
        me,
        sprintf("你还可以使用%d次累积技能，但是现在你没有任何累积技能，并未生效。\n", query("huanjing2003/last_skills_times", me))
      );
  ...
  if (query("huanjing2003/gift", me)) {
    tell_object(
      me,
      HIW "你曾是幻境游戏的" + query("huanjing2003/gift", me) + "者！\n你将得到增多300点游戏内最大气息、力量增强10点的奖励！\n" NOR
    );
...
*/
  ...
  CHANNEL_D->channel_broadcast(
    "rumor",
    sprintf("幻境: 听说%s[%s]进入了幻境，开始探险。", query("name", me), query("id", me))
  );
  remove_call_out("checking");
  call_out("checking", 1);
