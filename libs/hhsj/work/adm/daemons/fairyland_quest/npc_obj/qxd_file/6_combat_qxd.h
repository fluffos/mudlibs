...
int do_gk(string arg) {
  ...
  if (!query_temp("hj_hp", target) || query_temp("hj_hp", target) < 1)
    return errs(query("name", target) + "没有丝毫幻境里的气息，不必攻击了吧。\n");
...
// 不允许杀得分低的玩家
    if( query_temp("hj_score", target)<200 && !query("hj_game/npc", target) )
        return errs(query("name", target)+"的游戏得分太低，这样做好象不太好吧？\n");
*/
...
void do_attack(object me, object target, int damages) {
  ...
  addn_temp("hj_hp", -damages, target);  //给目标扣血
  // 显示健康状态
...
void fighting() {
  ...
  if (!inv_target || inv_target != environment(me) || !query("room_mark", inv_target))
    return hj_halt(HIW "\n$N" HIW "见" + query("name", target) + HIW "已不在此处，摆了一个收式，罢下手来。\n\n" NOR);

  // 目标携带神人令
  if ((srl = present("shenren ling", target))) {
    message_vision(
      CYN "$N" NOR + CYN "身上的" + query("name", srl) + NOR + CYN "突然发出一阵光芒！\n" NOR,
      target
    );
  ...
  if (figh_type == "hand") {
    ...
    tell_room(
      ...
      CYN "\n" + query(
        "name",
        me
      ) + NOR + CYN "奋力向" + query("name", target) + NOR + CYN + temp + "！\n" NOR,
      ({ me, target })
    );
    tell_object(
      target,
      CYN "\n" + query(
        "name",
        me
      ) + NOR + CYN "奋力向你" + temp + "，造成了" HIR "" + damages + "" NOR + CYN "点的伤害！\n" NOR
    );
    tell_object(
      me,
      CYN "\n你奋力向" + query(
        "name",
        target
      ) + NOR + CYN + temp + "，造成了" HIG "" + damages + "" NOR + CYN "点的伤害！\n"
    );
  ...
  if (figh_type == "tools") {
    ...
    if (query("hj_game/obj", use_tools) == "sword") {
      // 先确定攻击力、忙时等
      damages = (me_power * 3 / 4 + random(me_power / 4)) + (query(
        "power",
        use_tools
      ) * 3 / 4) + random(query("power", use_tools) / 4);
      ...
      tell_room(
        ...
        CYN "\n" + query(
          ...
          "name",
          use_tools
        ) + NOR + CYN "向" + query("name", target) + NOR + CYN + temp + "去！\n" NOR,
        ({ me, target })
      );
      tell_object(
        target,
        CYN "\n" + query(
          ...
          "name",
          use_tools
        ) + NOR + CYN "向你" + temp + "来，造成了" HIR + damages + NOR + CYN "点的伤害！\n" NOR
      );
      tell_object(
        me,
        CYN "\n你握起" + query(
          ...
          "name",
          target
        ) + NOR + CYN + temp + "去，造成了" HIG + damages + NOR + CYN "点的伤害！\n"
      );
    }
    // 否则是魔法水晶类
    else {
      ...
      tell_room(
        ...
        color_name + "只见" + query(
          ...
          "name",
          target
        ) + NOR + color_name + "攻去，造成了" HIG + damages + NOR + color_name + "点的伤害！\n" NOR,
        target
      );
      tell_object(
        target,
        color_name + "只见" + query(
          "att_msg",
          use_tools
        ) + "向你攻来，造成了" HIR + damages + NOR + color_name + "点的伤害！\n" NOR
      );
    }
  }
  ...
  if (figh_type == "skills") {
    ...
    damages = me_power / 4 + random(me_power / 4) + (query_temp(
      "hj_game_damages/" + use_skills,
      me
    ) * 3 / 4) + random(query_temp("hj_game_damages/" + use_skills, me) / 4);
    ...
    tell_room(
      ...
      color_name + "\n" + query(
        "name",
        me
      ) + NOR + color_name + "运起「" + skills_name + "」向" + query(
        "name",
        target
      ) + NOR + color_name + "攻去！\n" NOR,
      ({ me, target })
    ...
    tell_object(
      ...
      color_name + "\n" + query(
        "name",
        me
      ) + NOR + color_name + "运起「" + skills_name + "」向你攻来，造成了" HIR + damages + NOR + color_name + "点的伤害！\n" NOR
    );
    tell_object(
      me,
      color_name + "\n你运起「" + skills_name + "」向" + query(
        "name",
        target
      ) + NOR + color_name + "攻去，造成了" HIG + damages + NOR + color_name + "点的伤害！\n" NOR
    );
  ...
  if (figh_type == "skills" && skills_name == "夺魂之技") {
    ...
    if (!query_temp("hj_special/sh", target) && !random(10)
      ...
      tell_room(
        environment(me),
        sprintf(HIR "%s" HIR "突然两眼发直，整个人呆住了似的。\n" NOR, target->name(1)),
        ({ me, target })
      );
      tell_object(target, HIR "你突然两眼发直，整个人呆住了似的。 (造成忙时 " + target_busy + " 秒)\n" NOR);
      tell_object(
        me,
        HIR + target->name(1) + HIR + "突然两眼发直，整个人呆住了似的。 (造成忙时 " + target_busy + " 秒)\n" NOR
      );
    }
  }
  ...
  if (random(8) == 2 && query_temp("hj_game_find", me) == "dian") {
    message_vision(
      GRN "忽然不知从哪里引出一道" BLINK + HIG "闪电" NOR + GRN "劈向$N" NOR + GRN "，打个正中！\n" NOR,
      target
    );
