...
void full_all(object me) {
  if (!me)
    return;
  set("jingli", query("max_jingli", me), me);  //只要加些精力给走路时消耗，以及保持饿不死就行了。
  set("food", 50, me);
  set("water", 50, me);
}
...
int valid_leave(object me, string dir) {
  ...
  if (!query("hj_game/npc", me)) {
    if (query_temp("hj_game_find", me) == "feng")
      addn_temp("hj_move_busy", (150 + random(100)), me);  //再增强
      else addn_temp("hj_move_busy", (300 + random(200)), me);
  }

  set_temp("hj_the_orention", dir, me);  //记录这一步的方向
...
void games_random_move(object me) {
  ...
  tell_room(environment(me), HIR + query("name", me) + HIR "突然凭空消失不见！\n" NOR, ({ me }));
