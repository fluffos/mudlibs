...
#define        award_data        ([ "gxd": ([ "score?":30,"me?":1,"dt-n?":"family/gongji", "c-name":"幻境贡献","all->0?":"yes" ]),"exp": ([ "score?":1,  "me?":3000,"dt-n?":"huanjing2003/exp","c-name":"幻境经验","all->0?":"yes" ]),"pot": ([ "score?":1,  "me?":1000,"dt-n?":"huanjing2003/pot","c-name":"幻境潜能","all->0?":"yes" ]),"mar": ([ "score?":1,  "me?":300,"dt-n?":"huanjing2003/mar","c-name":"幻境体会","all->0?":"yes" ]), ])
...
" HIC "=====================================================================\n" NOR,
  ...
  if (arg == "last") {
    ...
    if (!temp_map || !sizeof(temp_map))
      ...
      msg += "你的技能记录如下(还有" + query("huanjing2003/last_skills_times", me) + "次使用的机会)：\n";
      key = keys(temp_map);
      for (i = 0; i < sizeof(key); i++) {
        msg += sprintf("%s\t%s\t\t%d\n",
          key[i],
