...
string ask_me() {
  ...
  if (sizeof(skl) <= 1)
    return RANK_D->query_respect(fighter) + "功力不够，不够资格闯光明圣火阵。";
  sname = keys(skl);
  ...
  if (query("guangming_winner", fighter))
    return RANK_D->query_respect(fighter) + "已然闯过光明圣火阵，可不要拿杨逍开这等玩笑。";

  if (query("guilty", fighter) > 0)
    return RANK_D->query_respect(fighter) + "你累犯数戒，身带重罪，我如何能准许你闯光明圣火阵！";

  if (query("assigned_fighter", me))
    return RANK_D->query_respect(fighter) + "，今日已有人挑战光明圣火阵，你过一段时间再来吧。";
