...
int do_beidouzhen(string arg) {
  ...
  power = me->query_skill("force") + ob1->query_skill("force") + ob2->query_skill("force") + ob3->query_skill("force") + ob4->query_skill("force") + ob5->query_skill("force") + ob6->query_skill("force");
  power += me->query_skill("sword") + ob1->query_skill("sword") + ob2->query_skill("sword") + ob3->query_skill("sword") + ob4->query_skill("sword") + ob5->query_skill("sword") + ob6->query_skill("sword");
  power += query(
    "str",
    me
  ) + query(
    ...
    "str",
    ob2
  ) + query("str", ob3) + query("str", ob4) + query("str", ob5) + query("str", ob6);
  power += query(
    "dex",
    me
  ) + query(
    ...
    "dex",
    ob2
  ) + query("dex", ob3) + query("dex", ob4) + query("dex", ob5) + query("dex", ob6);
  power = power / 140;
...
int beidouzhen(object me) {
  ...
  bonus = random((query(
    "int",
    me
  ) + query("con", me) + 25) / 30) + (query("int", me) + query("con", me) + 25) / 30;
  if (bonus > 4) bonus = 4;
  ...
  if (random(14) == 1) {
    ...
    if (objectp(enemy) && objectp(present(
      query("id", enemy),
      environment(me)
    )) && !enemy->is_busy())
