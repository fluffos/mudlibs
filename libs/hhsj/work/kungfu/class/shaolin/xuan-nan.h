...
string ask_me() {
  ...
  if (my_fam && my_fam["family_name"] != "少林派")
    return RANK_D->query_respect(fighter) + "又不是我们少林寺的人，打听罗汉阵做什么？";
...
int waiting(object me) {
  ...
  if (!objectp(fighter = present(query("assigned_fighter", me), environment(me)))) {
    wait_time++;
    call_out("waiting", 1, me);
  } else if (!present("xuanku dashi", environment(me)) || !query_temp("xuanku-asked", fighter)) {
    if (random(10) == 0)
...
int do_recruit(object me, object fighter) {
  ...
  if (!(ft_fam = query("family", fighter)) || ft_fam["family_name"] != "少林派") {
    ...
    say("\n玄难说道：好，好，好！" + query("name", fighter) + "，你愿意拜我等为师吗？\n");
