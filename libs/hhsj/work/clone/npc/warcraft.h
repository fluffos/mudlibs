...
void heart_beat() {
  ...
  if (env != owner && env != env2)
    ...
    if ((query(
      "max_qi",
      owner
    ) > query("qi", owner) || query("max_qi", owner) > query("eff_qi", owner))
      && query("race_type") == "麒麟") {
      if (random(15) < query("level") / 10) {
        ...
        if (query(
          "eff_qi",
          owner
        ) > query("max_qi", owner)) set("eff_qi", query("max_qi", owner), owner);
    ...
    if (query("race_type") == "青龙" && owner->is_fighting()) {
      target = offensive_target(owner);
      if (objectp(target) && random(15) < query("level") / 10 && !query_temp(
        "hiding_kuihua",
        owner
      )) {
...
int do_shape(string arg) {
  ...
  str += HIC "内力：" + sprintf(
    "%d",
    query("neili")
  ) + "/" + sprintf("%d", query("max_neili")) + "\n" NOR;
  str += HIC "精气：" + sprintf(
    "%d",
    query("jing")
  ) + "/" + sprintf("%d", query("eff_jing")) + "\n" NOR;
  str += HIC "精力：" + sprintf(
    "%d",
    query("jingli")
  ) + "/" + sprintf("%d", query("max_jingli")) + "\n" NOR;
