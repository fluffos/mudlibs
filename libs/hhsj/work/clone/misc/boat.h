...
int turnning(object me, string dir) {
  ...
  switch (dir) {
    case "东":
      if (stepe >= 300) over(ob);
        ...
        ob->delete_temp("stepw") addn_temp("stepe", random(3),; ob);
      }
    ...
    case "西":
      ...
      if (stepe > 0 && !stepw) addn_temp("stepe", -random(3), ob);
        ...
        ob->delete_temp("stepe") addn_temp("stepw", random(3),; ob);
      }
    ...
    case "北":
      ...
      if (steps > 0 && !stepn) addn_temp("steps", -random(3), ob);
        ...
        ob->delete_temp("steps") addn_temp("stepn", random(3),; ob);
      }
    ...
    case "南":
      ...
      if (stepn > 0 && !steps) addn_temp("stepn", -random(3), ob);
        ...
        ob->delete_temp("stepn") addn_temp("steps", random(3),; ob);
      }
  ...
  set_temp("dir", dir);
  if (random(10) > 7)
    random(2) ? message_vision(
      "$N驾着" + name() + "，正破浪向" + dir + "面驶去。\n",
      me
  ...
  //冰火岛在 e200,n950,
    ...
    for (i = 0; i < sizeof(obj); i++) {
      if (userp(obj[i])) {
        ...
        message(
          "vision",
          "只听嘣地一声，一个" + obj[i]->name() + "从触礁的海船里被抛了出来！\n",
          environment(obj[i]),
          ({ obj[i] })
        );
      }
    ...
    "combat_exp",
    me
  ) * 2) == query("combat_exp", me) && random(3) == 1) over(ob);
  else {
    remove_call_out("turnning");
...
void over(object ob) {
  ...
  if (obj) {
    ...
    for (i = 0; i < sizeof(obj); i++) {
      ...
      message(
        "vision",
        "突然一阵大风刮过，将一个" + obj[i]->name() + "从海里吹上岸来！\n",
        environment(obj[i]),
        ({ obj[i] })
      );
    }
