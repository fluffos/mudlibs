    ...
    if (sizeof(in_data) > 0 && !undefinedp(in_data["enchase"])) {
      ...
      if (!undefinedp(en_data["apply_prop"])) {
        ...
        foreach (string key in keys(en_data["apply_prop"]))
          in_desc += HIK + filter_color(
            EQUIPMENT_D->chinese(key, en_data["apply_prop"][key]),
            1
          ) + NOR + ",";
      }
      if (!undefinedp(en_data["weapon_prop"])) {
        ...
        foreach (string key in keys(en_data["weapon_prop"]))
          in_desc += HIK + filter_color(
            EQUIPMENT_D->chinese(key, en_data["weapon_prop"][key]),
            1
          ) + NOR + ",";
      }
      if (!undefinedp(en_data["armor_prop"])) {
        ...
        foreach (string key in keys(en_data["armor_prop"]))
          in_desc += HIK + filter_color(
            EQUIPMENT_D->chinese(key, en_data["armor_prop"][key]),
            1
          ) + NOR + ",";
      }
      if (!undefinedp(en_data["rings_prop"])) {
        ...
        foreach (string key in keys(en_data["rings_prop"]))
          in_desc += HIK + filter_color(
            EQUIPMENT_D->chinese(key, en_data["rings_prop"][key]),
            1
          ) + NOR + ",";
      }
      in_desc = in_desc[0..<2];
    }
    msg += sprintf("[%2d]  %-" + (20 + ob_name_real_len) + "s      %-11d%s\n",
...
int store_item(object me, object ob, int amount) {
  ...
  if (!ob->query_amount() && (query(
    "set_data",
    ob
  ) || !mapping_eqv(temp_data, obj->query_entire_temp_dbase())
  ...
  for (i = 0; i < n; i++) {
    if (all[i]->file == file &&
      ...
      all[i]->amount += amount;
      save();
      message_vision("$N存入" + chinese_number(amount) + (query(
        "base_unit",
        ob
      ) ? query("base_unit", ob) : query("unit", ob)) +
        query("name", ob) + "到如意乾坤袋里。\n", me);
      destruct(ob);
      return 1;
    }
