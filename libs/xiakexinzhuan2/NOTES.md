...
20-25 minutes reported for `xianjianchuanqi`/`shiji`, plausibly because
...
in a short time, similar in spirit to `xianjianchuanqi`'s "ambient NPC
...
(`xianjianchuanqi`'s `.../d/xingxiu/silk6`, `shiji`'s
...
happened to touch it via `free_svalue()`. Unlike the `xianjianchuanqi`
