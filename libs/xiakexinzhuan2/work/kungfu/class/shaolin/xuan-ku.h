...
string ask_me()
{
	...
	say("\n玄苦说道：好吧，我来召集罗汉堂长老们于西练武场集合，我稍候在中央练武场上等你。\n");
...
//	monk->move("/d/shaolin/houdian");
	...
	if(!( room = find_object("/d/shaolin/hguangz3")) )
	room = load_object("/d/shaolin/hguangz3");
//	monk = present("mu ren", room);
//	monk->move("/d/shaolin/houdian");
	...
	fighter->set_temp("fighting", 1);
        ...
        call_out("fighting",  3, me, fighter, 0);
}
...
void fighting(object me, object fighter, int count)
{
	...
	if(!( room1 = find_object("/d/shaolin/hguangz1")) )
	room1 = load_object("/d/shaolin/hguangz1");
...
void do_recruit(object me, object fighter)
{
	...
	{
		command("bow");
		say("\n玄苦说道：" + RANK_D->query_respect(fighter) + "此番过阵，当对本身修行大有助益，百尺竿头，更进一步，老衲就此别过！\n");
		call_out("do_back", 5, me );
	}
	else
	{
...
void do_back(object me)
{
	...
	message("vision", "\n玄苦大师挥了挥手， 罗汉堂长老们随即鱼贯离开练武场。\n", environment(me) );
        ...
        if (monk) monk->move("/d/shaolin/hguangz2");
...
//      monk->move("/d/shaolin/wuchang1");
	...
	for(i=1; i<10; i++) 
	{
        ...
        me->set("startroom", "/d/shaolin/lhtang");
	me->move("/d/shaolin/lhtang");
}
