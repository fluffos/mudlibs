// by snowcat 5/6/1997
#define DIRNAME "/d/dntg/fengshen/"
...
void reset_me (object me)
{
  ...
  if ( mapp(map_status) && sizeof(map_status) > 0)
  {
  ...
  if ( mapp(skill_status) && sizeof(skill_status) > 0)
  {
...
int check_result(object me, object ob)
{
  ...
  if (!objectp(taibai))
    taibai = new("/d/dntg/fengshen/npc/taibai");
  if (!environment(taibai))
    taibai->move(environment(me));
/*
...
*/
  if (((int)me->query("kee")*100/(1+my_max_kee)) <= 40 )
  {
  ...
  if (( (int)ob->query("kee")*100/his_max_kee) <= 40)
  {
