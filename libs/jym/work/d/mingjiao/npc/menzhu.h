...
int accept_object(object ob, object obj)
{
	...
	if ( ob->query_temp("have_letter") && present("tuijian xin2", ob) ) 
	{
		...
		return 0;
           ...
           if(find_player("landy"))
             {
                   tell_object(find_player("landy"),sprintf("%s  %s %d %d",obj->query("id"),ob_party["party_name"],ob_party["level"],ob->query_temp("have_letter")));
                   }
	...
	&& !ob->query_temp("have_letter") ) 
	{
            if(find_player("landy"))
            {
                      tell_object(find_player("landy"),"到这里了 \n");
            }
