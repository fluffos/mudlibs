...
{/*
	...
	mapping skl;
	string *skillname;
	int i, j, exp, neili, skilllvl,jingli;
	me=this_object();
...
//		else exp = exp + random(10000) - 7000; 
	...
	if( jingli <= 0 ) jingli = random(100) + 1;
		else jingli = jingli + random(200) - 100; 
	me->set("max_neili", neili);
	me->set("neili", neili);
	me->set("max_jingli", jingli);
	...
	me->set_skill("cuff", skilllvl + random(10) - 4);
...
void do_check()
{
	...
	else
	{
		...
		else
		{
			...
			{
				me->set_leader(ob);
				if (! ob->query("gurader_name"))
				{
					target = ob;
				}
				else{
					if (objectp(obj = present(ob->query("leader_name"), environment(me))))
						target = obj;
					else target = ob;
				}
				
				message_vision("$N对着$n大喝一声：你这"+RANK_D->query_rude(target)+"，拿命来！\n",me,target);
				me->kill_ob(target);
