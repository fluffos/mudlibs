...
int do_search(string arg)
{
                  ...
                  if( me->query_temp("herc/got",1) )
                                         return notify_fail("这里已经找过了，再找也找不出什么了！\n");

                  if( !present("yao ba", me) )
                                         return notify_fail("你没有工具，无法拨开灌木山草寻找草药！\n");

                  if( (int)me->query("jingli") < 30 )
                                         return notify_fail("你太累了，先歇息一会儿吧。\n");

                  me->add_temp("herc/times", 1);
                  me->add("jingli", -10);
                  me->start_busy(1+random(2));

                  if( query("countc") < 1 && (int)me->query_temp("herc/times") > 30 )
                                         return notify_fail("你把四周的灌木杂草都翻遍了，可是没发现什么，看来这里没指望了！\n");

                  message_vision("$N用药锄拨动着四周的灌木杂草，仔细地看有没有草药！\n", me);

				 // power_num = (to_int(random( me->query("kar")+me->query("int") + me->query("str") )/3));

                  power = (int)me->query_temp("herc/times");
                  //power *= query("countc");

                  //if( power/2 + random(power/2) > 250 ) {
				     if( random(power) > 3) {
                                         me->set_temp("herc/found", 1);
...
void setup_ob(object me,object victim)
{
        ...
        if ( !me->query_temp("copiedc") ) {
                ...
                me->set("combat_exp", victim->query("combat_exp",1)/2);
...
int do_dig(string arg)
{
                  ...
                  me->delete_temp("herc/found");
                  me->set_temp("herc/got", 1);
                  return 1;
}
