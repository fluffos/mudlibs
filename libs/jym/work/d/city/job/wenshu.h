...
//        me->apply_condition("gf_busy",10+random(3));
        ...
        me->delete_temp("mark");
		exp=200;
        if ((int)me->query("gf_job")<50) exp += random(exp); 
		if ((int)me->query("gf_job")<300 && (int)me->query("gf_job")>=50) exp=exp*2 +  random(exp); 
		if ((int)me->query("gf_job")>300) exp=exp*4  +  random(exp); 
        pot= exp/4 + random(exp/4);
		gfjob_times = 1;
		JOB_GIFTD->job_gift(me,(["combat_exp": exp,"potential": pot,"gf_job": gfjob_times,]),
			(["job_name":"gf_jobs","job_name_cn":"缉拿江洋大盗","job_time_limit":30,]));
