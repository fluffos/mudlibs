...
if icon="1" then 
icon="<img src=images/sign067.gif border=0> "
...
%>
		document.write("<tr>");
			...
			%>
			document.write("<td align=center style='font-size:<%=font%>pt;line-height:150%'><a href=<%=path%>news_view.asp?newsid=<%=id%> target='<%=open%>' title='<%=title%>'><img hspace=2 vspace=2 border=0 width=<%=w%> height=<%=h%> src=<%=pic%>><font style='font-size:<%=font%>pt'><%=ttt%></font></a></td>");
			<%
			rs.movenext
			if rs.eof then exit for
			next
