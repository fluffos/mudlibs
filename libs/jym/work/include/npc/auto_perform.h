...
int quanzhen_perform()
{
	...
	if ( objectp(weapon) && weapon->query("skill_type") == "sword")
	{
		  ...
		  else if( me->query("neili") < 150 && me->query("eff_qi",1) < me->query("max_qi")/2 && me->query_skill("sword") >= 135 )
...
int shenlong_perform()
{
        ...
        if( !me->query_temp("powerup") && random(3) ==1)
                command("exert powerup");
        else if( !me->query_temp("hgmz_powerup") && random(3) ==1)
        ...
        if ( objectp(weapon) ) {
                if ( weapon->query("skill_type") == "hook" )
                        return perform_action("hook.wucai");
        }
        else{
			if( random(2) ==0)
				return perform_action("strike.huagu");
			else return perform_action("strike.huanxue");
		}
...
int auto_perform()
{
	...
	if (me->query("auto_pfm") == "no")  return 0;
