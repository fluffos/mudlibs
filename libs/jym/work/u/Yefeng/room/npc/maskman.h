...
void init()
{
	...
	if ( me->query("kill_player") == victim->query("id") && present(victim, environment(me))) {
        ...
        if ( strsrch(file_name(environment(me)), "/u/yefeng/room/hsforest") == -1 && me->query("kill_player") != victim->query("id") ) {
		remove_call_out("checking");
		remove_call_out("destruct_me");
	...
	if ( !me->query_temp("copied") ) {
	   if ( mapp(skill_status = victim->query_skills()) ) {
		...
		for(i=0; i<sizeof(skill_status); i++) {
			if ( skill_status[sname[i]] > max && SKILL_D(sname[i])->type() !="knowledge" ) max = skill_status[sname[i]];
//			if ( skill_status[sname[i]] > max ) max = skill_status[sname[i]];
		}
		
		max1 = (int)max*4/5 + 1;
//		max1 = max1 * 90/100; //muder add
		max = (int)max*4/5 + 1 + random((int)max/2);
//		max = max * 90/100; //muder add
