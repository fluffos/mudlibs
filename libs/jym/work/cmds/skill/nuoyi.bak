...
//        if( !me->query_skill("qiankundanuoyi", 1) )
	...
	if( me->is_busy() )
        ...
        if( (int)me->query_skill("qiankun-danuoyi", 1) < 350 )
	...
	if( form == 1)
	{
		...
		me->add_temp("apply/strength", count * r_lvl);
		me->set_temp("nuoyi", 1);
	...
	me->start_busy(3);
        ...
        if ((random(me->query("neili",1))+me->query("combat_exp")) > (target->query("neili",1)+target->query("combat_exp")))
        {
	...
	me->start_busy(3);
        ...
        if ( (random(me->query("neili",1))+me->query("combat_exp")) > (target->query("neili",1)+target->query("combat_exp"))&& ( r_lvl > random(5) ) )
        {
                write( CYN"哈哈，成功了！\n"NOR);
