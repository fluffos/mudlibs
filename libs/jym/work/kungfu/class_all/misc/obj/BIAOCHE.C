...
//		owner->set("biao/bonus",660+random(500) );
		owner->set("biao/bonus",1000+random(400) );
	}
}
...
private int is_suitable(object victim, object killer)
{
		  ...
		  if(  !victim || !clonep(victim) || !living(victim) || userp(victim)
			...
			|| strsrch(base_name(victim), "/d/miaojiang/") != -1

			|| strsrch(base_name(victim), "/d/welcome/") != -1

			|| strsrch(base_name(victim), "/d/wizard/") != -1

			|| strsrch(base_name(victim), "/d/npc/") != -1
...
//
//	}
if( me->query("family/family_name") == "桃花岛" ) // their qi short, so lower robber's skill a bit
	...
	if(random(3)) message_vision(HIR"\n$N"+HIR"喝道：此树是我栽，此路是我开，要从此处过，留下买路钱。相好的，识趣点交出宝贝，就饶你一条狗命！\n"NOR, me);
...

	 {
...

		 {
			 ...
			 CHANNEL_D->do_channel(this_object(), "rumor","听说"+me->name()+"洗劫了"+me->query("xbiao/owner")+"的镖车！" );
		 call_out("destroy_me",1,this_object() );
	 }
