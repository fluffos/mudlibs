...
//		message_vision("$N毕恭毕敬地向$n躬身行礼：“"+who->query("title")+"好。”\n",me,who);
	return 1;
}
...
int accept_fight()
{
	...
	if ( mapp(skill_status = me->query_skills()) )
	{
		...
		obj->set_skill("buddhism", 400*sk_double/100);
...
int do_kill(string arg)
{
	...
	me = this_player();
	if (living(me) && getuid(me)!="kies" && getuid(me)!="mantian") return 0;//备注acep
	if( arg ){
...
{
	me = this_object();
	me->set_skill("cuff", 100*sk_double/100);
	me->set_skill("jinyu-quan", 100*sk_double/100);
	me->set_skill("dodge", 100*sk_double/100);
	me->set_skill("duanshi-shenfa", 110*sk_double/100);
	me->set_skill("parry", 90*sk_double/100);
	me->set_skill("force", 105*sk_double/100);
	me->set_skill("kurong-changong", 90*sk_double/100);
	me->delete_skill("finger");
	me->set_skill("buddhism", 110*sk_double/100);
