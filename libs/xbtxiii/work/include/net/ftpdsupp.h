...
int check_access(string name)
{
    ...
    return SECURITY_D->get_status(name) != "(player)" ;
#else
...
#endif
*/
//密码改用文件存储方式，因此验证密码的方式必须更改
/*
//    pdfile= sprintf(DATA_DIR "password/%c/%s", name[0], name+".pas");
//    if(file_size(pdfile)!=-1)  fancytext=read_file(pdfile);
//
...
*/     
     return PASSWORD_D->main(name,"check",plaintext) ;
}
