...
void attempt_apprentice(object ob)
{
        ...
        if (!(ob_fam = ob->query("family")) || ob_fam~ != "明教")
        {
                command("say " + RANK_D->query_respect(ob) + "与本教素无来往，不知此话从何谈起？");
                return;
        }
        if ( (string)ob->query("class")!="bonze" && ob_fam~ == "明教")
        {
        ...
        if ( ob_fam~ == (my_fam~ + 1) )
        {
                command("say " + ob_fam~ + "的徒弟怎麽跑到我这儿来了，哈哈哈 !");
        ...
        if ( ob_fam~ == (my_fam~ + 2) )
        {
                if ( ob->query_temp("apprentice_ok") )
                {
                        ...
                        command("say 是" + ob_fam~ + "叫你来找我的吧，哈哈哈 !");
...
#include "/d/mingjiao/npc/mingjiao.h"
*/
