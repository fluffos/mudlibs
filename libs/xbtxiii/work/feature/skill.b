...
//                return;
	...
	if( weak_mode==0 &&userp(this_object())	&&
		learned[skill] > (skills[skill] + 1) * (skills[skill] + 1) ) {
			skills[skill]++;
			learned[skill] = 0;
			tell_object(this_object(), HIC "你的「" + to_chinese(skill) + "」进步了！\n" NOR);
			SKILL_D(skill)->skill_improved(this_object());
    }else if( weak_mode==1 && userp(this_object())&&
                (learned[skill]-amount) > (skills[skill] + 1) * (skills[skill] + 1) ) {
                        learned[skill] -= amount ;
            tell_object(this_object(), YEL "你的「" + to_chinese(skill) + "」无法再进步了！\n" NOR);
    }else if( weak_mode==2 && userp(this_object())&&
