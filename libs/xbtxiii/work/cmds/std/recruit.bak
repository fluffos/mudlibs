...
int main(object me, string arg)
{
	...
	// follow modified by elon 09-10-95 to fix a bug in 1st time recruit
        /*follow modified by cocacola on 1997-2-20 for fixing the bug 
about apprentice in the the family*/
        if(mapp( ob->query("family"))&&me->query("family/family_name")!=ob->query("family/family_name"))
            {
			message_vision(
                "$n决定背叛师门，改投入$N门下！！\n\n"
			...
			ob->add("betrayer", 1);
            ob->set("fealty", -100);
            ob->add("fambet/"+ ob->query("family/family_name"), 1);
        } else if((string)me->query("family/master_name") != (string)ob->query("name")
        &&me->query("family/family_name")==ob->query("family/family_name"))
           {
            me->set("score", 0);
            me->set("fealty", me->query("fealty")-20);
                        message_vision(
                "$n决定改拜$N为师！！\n\n"+
                "$n跪了下来向$N恭恭敬敬地磕了四个响头，叫道：「师父！」\n\n",
                me, ob);
