...
int give_job() {
  ...
  if (me->query("combat_exp") < 100000) {
    command("hmm");
    message_vision(
      CYN "$N对$n说道：做镖师这行是刀口上舔血，随时都有性命之忧，" + RANK_D->query_respect(me) + "的本事差了一些，恐不合适做这个，还是请回吧。\n" NOR,
      this_object(),
      me
    );
    return 1;
  ...
  message_vision(
    CYN "$N说道：这是" + job["target_name"] + "所重托的红镖，非同小可。你千万不可大意，\n务必交到" + job["npc_title"] + job["job"] + "手里。\n" NOR,
    this_object()
  );
...
int dealy_message(object me, mapping job) {
  ...
  short = HIR + "远东镖局镖头" + NOR + " " + me->query("title") + " " + me->query("name") + "(" + capitalize(me->query("id")) + ")";
...
void init() {
  ...
  if (job) {
    if (me->query("end_job")) {
      exp = 2000 + random(100) + job["exp_bonus"] * 2;
      pot = exp / 2;
      ...
      me->delete("end_job");
      me->set("end_array", time() + random(50) + 50);
      ...
      message_vision(CYN "$N对着$n吼道：速速离开这里，远东镖局不能录用你这样无能之人。\n" NOR, this_object(), me);
      me->delete("fjob");
      me->delete("fjob_time");
      me->delete_temp("apply/short");
      //  me->set("no_array",time()+random(400)+300);
      me->set("no_array", time() + random(50) + 100);
      if (biaoshi = present("biao shi", environment(me)))
        destruct(biaoshi);
    }
  }
