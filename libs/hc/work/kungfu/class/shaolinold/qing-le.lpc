...
string ask_me_1()
{
    ...
    string *sname;
    int i;
        if (time()-this_player()->query("qingle/letter")<300)
        return RANK_D->query_respect(this_player()) + "不是已经领过手谕了么？\n";
    ...
    ob->move(this_player());
    this_player()->set("qingle/letter",time());
...
string ask_me_2()
{
    ...
    if (!(fam = this_player()->query("family")) || fam["family_name"] != "少林派")
        return RANK_D->query_respect(this_player()) + 
        "与本派素无来往，不知此话从何谈起？";
        if (time()-this_player()->query("qingle/ling")<300)
        return RANK_D->query_respect(this_player()) + "不是已经领过达摩令了么？\n";
    skl = this_player()->query_skills();
    if (mapp(skl)){
        sname  = sort_array( keys(skl), (: strcmp :) );
        for(i=0; i<sizeof(skl); i++) {
    ...
    ob->move(this_player());
    this_player()->set("qingle/ling",time());
