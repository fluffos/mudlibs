...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
    ...
    me->add("force", - factor );
    damage = (int)me->query("force")/10 + factor
   ...
   &&      random(victim->query_skill("force")) > me->query_skill("force")/2 ) {
      ...
      if( damage < 100 ) return "$N受到$n的内力反震闷哼一声。\n";
      else if( damage < 200 ) return "$N被$n以内力反震「嘿」地一声退了两步。\n";
      else if( damage < 400 ) return "$N被$N以内力一震胸囗有如受到一记重锤连退了五六步。\n";
