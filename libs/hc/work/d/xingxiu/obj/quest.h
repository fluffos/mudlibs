// modified 2000,09,09,by ahxia, just changed from /feature/quest.c
// to /d/xingxiu/quest.h
// thanks to snowman, sorry all. :)
//========= origial quest.c below ==========
// QUEST.C for randomly get target npcs.
// 15.03.99 by snowman@jyqxz
...
mapping get_quest(object me, int shen, string type)
{
        ...
        for (i=0;i < sizeof(living);i++){       // check for the 1st time, random.
        	...
        	if(check(living[j], exp, me->query_temp("quest/last_id"), 
        	   !intp(shen)?0:shen>0?1:-1, me->query("family/family_name"))){
        		 target = living[j];
        		 break;
        	         }
        	}
