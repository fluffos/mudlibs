...
int do_join()
{
       ...
       if (me->query("bang_bad"))
       {
               if (me->query("bang_bad/name") == bang_bad["name"])
               {
                       ...
                       return notify_fail("你已经是我" + me->query("bang_bad/name") + "的成员了，昏头了？\n");
               }
               return notify_fail("你现在是" + me->query("bang_bad/name") + "的成员，要加入我" + bang_bad["name"] + "，得先脱离(disengage)原来的帮会。\n");
       }
...
int do_disengage()
{
       ...
       if (me->query("bang_bad/boss") != bang_bad["boss"])
               return notify_fail("想脱离我"+bang_bad["name"]+"？找你的介绍人" + me->query("bang_bad/boss") + "去吧。\n");
...
int accept_object(object me,object ob)
{
       ...
       if (bang_quest["id"] != "xc" || me->query("bang_bad/boss") != who->name())
       {       
               msg = who->name() + "朝你点了点头，没有说什么话。\n";
               tell_object(me,msg);
               return 1;
       ...
       bonus = ([ "exp" : exp, 
                  ...
                  "score" : score, 
                  "weiwang" : weiwang, 
                  "prompt": "在巡"+bang_bad["place_name"]+"的过程中，经过锻炼" 
               ]);
