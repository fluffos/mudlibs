...
"海洋"系列的另一支世界，社区自称"海洋V"，世界观为武侠向。经核心档案比对确认，`hy5` 实际上与本项目里的 `haiyang2`（海洋II／星月传奇）血缘更近：`adm/obj/master.lpc` 逐字节完全相同，抽样域文件也约有 90% 字节相同，本质上是同一个基础世界，而不只是"海洋"家族的又一条独立分支。`hy5` 独立重写了安全精灵（`securd.lpc`），并在 `haiyang2` 约 141 个地图分区的基础上再扩展约 50 个新区域——安南（越南）、高丽（朝鲜）、日本（神户/京都）、西藏、新疆、楼兰、台湾等，使总地图规模成为本项目最大之一，这个扩展部分才是本档案真正独有的内容。
