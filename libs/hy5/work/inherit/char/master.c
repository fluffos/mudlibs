...
if (shen > 80000) shen=80000;
if (shen < -80000) shen=-80000;

                if (me->query_skill(skill, 1) >= 200 && shen < 0 && me->query("shen") > shen && !query("pubmaster"))
{
...
return 1;
		...
		if( me->query_skill(skill, 1) >= 250 && ((int)me->query("mpgx")*5)+200<= me->query_skill(skill, 1) ) {
...
{
		if( me->query_skill(skill, 1) >= 100 && ((int)me->query("mpgx"))+10<= me->query_skill(skill, 1) ) {
			message_vision("$N说到,你为" + this_object()->query("family/family_name") +
                       "作出的贡献不够，这后面的武功暂时还不能传你。\n",
				this_object(), me );
			return 1;
