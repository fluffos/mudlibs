...
//if (type=="jing") typezl="精";
//if (type=="qi") typezl="气";
//if (type=="jingli") typezl="精力";
//if (type=="neili") typezl="内力";
//message_vision("$N的"+typezl+"受到"+(damage*-1)+"点伤害！\n", this_object());
...
//message_vision("$N最大对像:"+(this_object()->query_max_hatred()->query("name"))+"！\n", who);
//message_vision("$N的仇恨值:"+(this_object()->query_hatred(who))+"！\n", who);
...
//message_vision("$N的"+typezl+"受到"+(damage*-1)+"点创伤！\n", this_object());
if( objectp(who) && damage>100 ) {
                if( !userp(this_object()) ) {
if (this_object()->query_hatred(who) < 500000000)
                        add_hatred(who, damage/100);
...
message("channel:chat", HIC"【屠人战况】"HIR + " "+this_object()->query("name")+"被"+killer->query("name")+"打出了屠人大会！\n"NOR,users() );
	...
	//玩家保护
        if (userp(this_object()) && !this_object()->query_temp("isdeads") && !this_object()->query_condition("killer"))
        {
        	...
        	{
		message_vision(HIR "$N眼看就要昏倒，$N咬破舌尖 吐出一口鲜血 结果$N一下子就清醒了过来。\n" NOR, this_object());
...
{
	message("channel:rumor", HIM"【江湖谣言】"+"听说"+this_object()->name()+
    "死了 死因不明。\n"NOR, users());
message_vision(HIW "$N本来死定了，但一阵白光包围了$N，$N消失在了白光中! \n" NOR, this_object());
...
//被挑战的人不死
        if(userp(this_object()) && this_object()->query_temp("hdjoblb")=="挑战"
        && !this_object()->query_condition("killer")        )
        ...
        )
{
	...
	message("channel:rumor", HIM"【江湖谣言】"+"听说"+this_object()->name()+
    "死了 死因不明。\n"NOR, users());
message_vision(RED "$N本来死定了，但替身娃娃发出一阵红光包围了$N，$N消失在了白光中! \n" NOR, this_object());
		this_object()->set("jing", this_object()->query("max_jing")+100);
		this_object()->set("eff_jing", this_object()->query("max_jing")+100);
		this_object()->set("qi", this_object()->query("max_qi")+100);
		this_object()->set("eff_qi", this_object()->query("max_qi")+100);
...
log_file("log", "死亡人物" + victim->query_condition("psgc") + victim->query("name")+"\n", 1);
	victim->skill_expell_penalty();
		...
		sprintf("%s由于判师被门派追杀而死!", victim->name(), victim->name()));
}
}
