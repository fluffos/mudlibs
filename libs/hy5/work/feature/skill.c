...
int skill_death_penalty()
{
	...
	if( !mapp(learned) )
		for(i = 0; i<sizeof(sk); i++) {
				if( skills[sk[i]] > 100 ) skills[sk[i]]--;
		}
	else
		for(i = 0; i<sizeof(sk); i++) {
...
//		if( undefinedp(skills[skill]) ) skills[skill] = 0;
	...
	//i=(int)learned[skill] + amount;
	//dj=i/((skills[skill] + 1) * (skills[skill] + 1));

	   if((!weak_mode || !userp(this_object()))
	        && learned[skill] > (skills[skill] + 1) * (skills[skill] + 1) ) {
        	while (learned[skill] > (skills[skill] + 1) * (skills[skill] + 1)) {
	        		(int)learned[skill] -= (skills[skill] + 1) * (skills[skill] + 1);
        ...
        else if(learned[skill] > (skills[skill] + 1) * (skills[skill] + 1)) {
	        	(int)learned[skill] = (skills[skill] + 1) * (skills[skill] + 1) +1;
        }
