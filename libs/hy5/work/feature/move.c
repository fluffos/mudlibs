...
//if(!environment() && query("equipped") && (query("makeritem")||query("systemmake")) )
//{
//delete("equipped");
//}
...
//	if (query("ownmake") && query("no_get") && userp(ob))
//return notify_fail( "这个东西对" + ob->name() + "而言太重了。\n");
	...
	if( interactive(me)		// are we linkdead?
		...
		if( query("env/brief") || query_temp("follow"))
		{
			...
			{
			                if (this_object()->query_temp("xmud"))  command("xkan");
else			 command("look");
}
	}
