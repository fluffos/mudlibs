...
&& gem != "windjade" && gem != "firejade" && gem != "icejade" && gem != "thunderjade" && gem != "earthjade")
		return 0;
	if (level < 1 || level > 8)
		return 0;
...
object new_gem(string gem, int lev,int lb)
{
	object ob;
	...
	ob->set_name(query_gem_name(gem, lev)+HIY" 原石"NOR, ({query_gem_id(gem, lev)}));
