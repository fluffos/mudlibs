...
/*	for(i=0; i<sizeof(ob); i++){
		...
		//call_out("auto_reboot_start", 86400+random(100));// 1天自动重启
//                call_out("auto_reboot_start", 172800+random(60));// 2天自动重启
                  call_out("auto_reboot_start", 259200);// 3天自动重启
//                    call_out("auto_reboot_start", 432000);// 5天自动重启
...
void auto_reboot_start()
{
	...
	message("system",RED"【重启精灵】"BLU"现在开始海洋V-星月传奇 自动重启倒计时，还有5分钟！\n" NOR,users());
...
private void count_time(int min)
{
	min--;
	if( min ) {
		message("system",RED"【重启精灵】"BLU"各位玩家，离海洋V-星月传奇 自动重启还有" + chinese_number(min) +"分钟的时间！\n"NOR,users() );
		call_out("count_time", 60, min); // 告诉大家还有几分钟时间
	} else {
		message("system",RED"【重启精灵】"BLU"各位玩家，海洋V-星月传奇 现在自动重启！！！\n"NOR,users() );
