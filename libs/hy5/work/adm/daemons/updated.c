...
//	ob->set("channels", ({ "chat", "rumor", "party", "xkx", "sing" }) );
	...
	if (ob->query("age")==14) money_limit = 1000;
	if (ob->query("age")>14) money_limit = 1000 + (ob->query("age")-15) * 20;
	if (ob->query("age")>20) money_limit = 2000 + (ob->query("age")-20) * 20;
	if (ob->query("age")>30) money_limit = 4000 + (ob->query("age")-30) * 30;
	if (ob->query("age")>40) money_limit = 8000 + (ob->query("age")-40) * 40;
