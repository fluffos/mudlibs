...
mapping channels = ([
    ...
    "hy": ([  "msg_speak": NOR+HIC+"【海洋IV】"+HIW+"%s：%s\n" NOR,
            "msg_emote": NOR+HIC+"【海洋IV】"+HIW+"%s" NOR,
            "intermud": GCHANNEL,
            "intermud_emote": 1,
            "channel": "hy",
            "msg_color": NOR+HIY,
    ...
    "gb": ([  "msg_speak": HIC+"【简体中文】%s：%s" NOR,
            "msg_emote": HIC+"【简体中文】"+CYN+"%s" NOR,
            "msg_color": NOR+HIC,
                          "channel": "gb",
                          "omit_address": 0, 
    ...
    "bg": ([  "msg_speak": HIC+"【繁体中文】%s：%s" NOR,
            "msg_emote": HIC+"【繁体中文】"+CYN+"%s" NOR,
            "msg_color": NOR+HIC,
                          "channel": "bg",
                          "omit_address": 0, 
    ...
    "chat": ([  "msg_speak": HIC+"【论道江湖】%s：%s" NOR,
  ...
  "new": ([  "msg_speak": HIC+"【"+HIW+"初入江湖"+HIC+"】%s：%s" NOR,
  "msg_emote": CYN+"【"+HIY+"初入江湖"+CYN+"】"+HIC+"%s" NOR,
    ...
    "rumor":    ([  "msg_speak": HIM "【江湖谣言】%s：%s" NOR,
            "msg_emote": HIM "【江湖谣言】%s" NOR,
            "msg_color": NOR+HIR,
            "anonymous": "某人",
        ]),

    "tasks":    ([  "msg_speak": HIM "【传闻】%s：%s" NOR,
            "msg_emote": HIM "【传闻】%s" NOR,
            "msg_color": NOR+HIR,
//"anonymous": "",
//"wiz_only": 1,
        ]),

    "wizrumor":  ([  "msg_speak": NOR+HIM+"【谣言】"+HIM+"%s：%s\n" NOR,
            "msg_emote": NOR+HIM+"【谣言】"+HIY+"%s" NOR,
            "intermud": GCHANNEL,
            "intermud_emote": 1,
            "anonymous": "某人",
            "msg_color": NOR+HIR,           
...
//if (verb="gb" && userp(me) ) arg=LANGUAGE_D->Big52GB(arg); 
//if (verb="bg" && userp(me) ) arg=LANGUAGE_D->GB2Big5(arg); 
//if (me->query_temp("big5")) arg=LANGUAGE_D->Big52GB(arg); 
...
mname=me->name(1);
if (me->query_temp("big5") && userp(me) && !emote && mname!="" ) mname=mname+"("+LANGUAGE_D->Big52GB(mname)+")"; 
    ...
    if (verb=="gb" && userp(me) && arg!="" ) arg=LANGUAGE_D->Big52GB(arg)+"("+arg+")"; 
if (verb=="bg" && userp(me) && arg!=""  ) arg=LANGUAGE_D->GB2Big5(arg)+"("+arg+")"; 
    if (me->query_temp("big5") && !emote && userp(me) && arg!="" ) arg=arg+"("+LANGUAGE_D->Big52GB(arg)+")"; 
    ...
    if( channels[verb]["anonymous"] ) {
        ...
        if (userp(me))
         if (random(10)>8 && !wizardp(me)) who=mname + "(" + capitalize(me->query("id")) + ")";
         else
                do_channel( this_object(), "sys", sprintf("%s躲了起来造谣。", mname));
    }
    else 

    if(me && (userp(me) || !stringp(who = me->query("channel_id")) ))
    {
        if (me->name()!=mname && emote)
                do_channel( this_object(), "sys", sprintf(HIW"%s(%s)"+WHT+"扮%s!", mname,me->query("id"),me->name()));
        if (!mname)
        {
        mname=me->name(1);
}
        ...
        who = mname + "(" + capitalize(me->query("id")) + ")";
    }
    ...
    if( !undefinedp(channels[verb]["intermud"]) && me->is_character()) {
        ...
        channels[verb]["channel"], me->query("id"), mname, arg, emote,
