...
void create(){ 
    ...
    set("prep", "on");
    set("unit", "个");
    set("long", "的表面被磨得十分光滑，看来经常有人坐在这里（ｓｉｔｄｏｗｎ）。\n");
    set("value", 10);
    }
...
int do_look()
{
        ...
        {
                set("long", "蒲团的表面被磨得十分光滑，看来经常有人坐在这里（ｓｉｔｄｏｗｎ）。\n");
        }
        return 0;
} 
int do_sitdown(){
        ...
        laoni = present("lao ni", environment(this_object()));
        if ( me->query("class") == "shenshui" && objectp(laoni)) 
...
void move_shenshui(object player) {
         if ( player->query_temp("state/sit") 
                ...
                player->move("/d/qingping/shenshui/huoyan");
