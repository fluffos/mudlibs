        ...
        if (present("ying",environment(me)))
   {
        ...
        if (bankowner) 
        {
                ...
                if (type == "biaoyin")
                {
                        ...
                        bukuai1 = new("/d/tieflag/fy/npc/bukuai");
                        bukuai2 = new("/d/tieflag/fy/npc/bukuai");
                        bukuai1->move(environment(me));
                        bukuai2->move(environment(me));
                        message_vision(HIR"突然从暗中跳出来几个捕快对$N嚷道：胆敢劫镖银！我们埋伏在这里好久了！\n"NOR, me);
                        message_vision(HIR"$n对$N狞笑道：如今人赃俱获，乖乖跟我们走吧！ \n"NOR,me,bukuai1);
                        message_vision(HIR"$n将$N押向大牢。 \n"NOR,me,bukuai1);
                        destruct(bukuai1);
                        destruct(bukuai2);
