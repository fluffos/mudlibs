...
void ctake()
{
    ...
    if(sum==0)
    {
        ...
        else
        {
            ...
            command("give silver to "+player->query("id"));
            in_game=0;
            return;
        }
    }
...
int do_answer(string arg){
        ...
        if (player->query_temp("pianzi_yes_no")) {
                if (arg == "yes") {
                        ...
                        message_vision("$N说道：”这位" + RANK_D->query_respect(player) + "果然有见地。”\n", 
                                        this_object(), player);
   ...
   if (!player->query("marks/买幽冥紫灵剑")) {
        ...
        if (ob->query("money_id") == "gold" ) {
                if (ob->query_amount() < 100) {
                        ...
                        message_vision("$N小心地从怀中取出一把宝剑交给$n。嘀咕道：“你可莫辜负了灵剑。” \n", 
                                        this_object(), player); 
                        sword = new(__DIR__"obj/purple_sword");
                        sword->set("owner", player->name());
                        sword->move(player);
