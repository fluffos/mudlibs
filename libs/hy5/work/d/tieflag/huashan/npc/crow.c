//我把华山crow修改了一下，在单机上测试了一下，发觉可以follow，但是一直到处兜圈子，总是走不到悬崖，
//你看看我的第二处修改的地方，那里有个__DIR__"crowway"，但是我在文件里找不到crowway.c这个文件啊，是不是少了？？
//你帮忙看看好么？
...
{
        object room;
        room = find_object(__DIR__"qipan");
        if (room != environment())
        {
                return;
        }
...
似乎要$n跟它去什么地方。\n",this_object(),player );
                ...
                add_action("do_follow","follow");                   //我打开了这里，原文件把这里屏蔽了
        }
} 
...
void go(object me)
{
    ...
    if (way[i]["delay_time"]<0)
    {
                ...
                this_object()->move(__DIR__"duanshan");
