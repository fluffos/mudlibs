...
int wangyou(object me){
        ...
        if(interactive(me) && environment(me) == environment(this_object())) {
                ...
                if(!me->query("marks/3years_force")){
                        message_vision("$N接着道：“你还是把它交给我吧。我也不会白要你的，帮你长三年的功力，\n", this_object()); 
                        message_vision("你看怎么样？”（ａｃｃｅｐｔ　ｙｅｓ／ｎｏ）\n", me); 
                } else {
...
int do_accept(string arg){
   ...
   me = this_player();
        ...
        destruct(grass);
                if(!me->query("marks/3years_force")){
                message("vision", "\n" + this_object()->name() + "袍袖鼓起拂向" + me->name() + "胸前。\n", 
                                environment(me), me);
                tell_object(me, HIY"\n金鹰老人袍袖一拂，一股热浪扑来，你觉得胸口一热，似乎有股力量注入了你体内。\n"NOR);
