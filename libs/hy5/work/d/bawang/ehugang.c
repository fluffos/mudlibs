...
{ 
   ...
   int wearing; 
        ...
        { 
                if((string)inv[i]->query("name") == "布衣"&& (int)inv[i]->query("equipped") ) 
                { 
                        wearing = 1; 
                } 
        } 
        ...
        { 
                ...
                { 
if( !me->query_temp("yuehu") ) 
                        { 
                                return notify_fail("大喽罗咽了咽口水笑道：给咱们寨主送烧鸡？不留下两只，\n对兄弟们可交代不过去呦......\n"); 
                        } 
                        else 
                                return 1;                
                } 
                ...
                { 
                      if( !me->query_temp("yuehu") ) 
                        { 
                                return notify_fail("小喽罗咽了咽口水笑道：给咱们寨主送烧鸡？不留下两只，\n对兄弟们可交代不过去呦......\n"); 
                        } 
                        else 
                           return 1;                
                } 
