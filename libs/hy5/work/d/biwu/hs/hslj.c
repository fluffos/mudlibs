...
int  do_levelup(string arg)
{
	...
	if(!wizardp(me) && (int)me->query_skill("jiuyin-zhengong", 1) && (int)me->query("jinyong/book5")!=9)
	return notify_fail("ÄãµÄÔÓÑ§Ì«¶à£¬»ªÉ½ÂÛ½£Ö»ÔÊÐíÄÜ´ú±íÒ»·½ÃÅÅÉµÄÐÂÐã²Î¼Ó£¡\n");
	
	if(me->query("guard/ok") && (int)me->query("jinyong/book5")!=9 )
...
{
if((int)me->query("jinyong/book5")==9)
{
		i = 990+random(3000);
		who->set("jinyong/book5",10);
		who->add("combat_exp",i+19000);
		who->add("potential",i*28);
		who->add("mpgx",20);
		who->add("expmax",2);
		//command("chat "+this_player()->query("name")+"µ¨É«¹ýÈË£¬Òå³ÍÇà³Ç¶ñ°Ô¡£");
		tell_object (who,"ÄãÓ®µÃÁË"+chinese_number(i+19000)+"µã¾­Ñé"+
		           chinese_number(i*28)+"µãÇ±ÄÜ"+"¶þÊ®µãÃÅÅÉ¹±Ï× ¶þµã³É³¤ÉÏÏÞ£¡£¡\n");
		message("channel:chat", HIB"¡¾º£ÑóIII¡¿ÌýËµ"+who->name() + "Íê³ÉÁËÉäµñÓ¢ÐÛ´«µÄËùÓÐÈÎÎñ!"NOR"\n", users());
		who->save();
}		
if (!me->query_skill("jiuyin-zhengong",1)) me->set("guard/ok",1);
me->save();
