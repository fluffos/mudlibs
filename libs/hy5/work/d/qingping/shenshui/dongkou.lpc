...
void create()
{
        set("short", "洞口");
        set("long", @LONG
你站在一个山坡上，背后是爬满了各式各样的藤萝的峭壁，有些藤萝还
开着鲜艳的花朵，就象是一幅巨大的挂毯把峭壁遮在了后面。往远处看，只
见群山叠起，云雾缭绕，山峰都隐没在一片白茫茫的雾海之中。
LONG
        );
   set("exits", ([ 
      "north" : __DIR__"gudi",
...
void init()
{
   add_action("do_push", "push");
       if(!this_player()->query("m_success/老周"))
        {
                this_player()->set("m_success/老周",1);
...
int do_push(string arg)
{
   ...
   if (flvl >= 30) {  
      if (!query("exits/enter")) {
         ...
         message_vision("$N大石慢慢向旁边滑去, 露出了一个洞口。 \n", me);
