...
int ask_me2()
{
	...
	where = environment(who);
	  	if(((int)me->query("jinyong/book4")!=2))
	{
		message_vision("$N对$n说道： 段誉给四大恶人抓走了，你去救他了吗? \n",this_object(),me);
                return 1;	
	}

      if(!query("iskillok"))
      {
      message_vision("突然从门外突然杀进一个和尚 说道： 把六脉神剑谱交出来吧!\n",this_object(),who);	
      	fang = load_object("/kungfu/class/xueshan/jiumozhi");
	...
	fang->kill_ob(who);
     ...
     set("iskillok",1);
        ...
        fang = present("jiumo zhi",where);
	if(fang)
	{
		who->start_busy(3);
		message_vision("$N对$n说道： 这位"+ RANK_D->query_respect(who) +"先帮我战败鸠摩智吧。\n",this_object(),who);
		return 1;
	}

this_object()->set("book_give",1);
this_object()->delete("assigned_fighter");
this_object()->set("iskillok",0);
who->set_temp("killtlbb3",1);
		message_vision("$N对$n说道： 要知道段誉的下落，先过六脉剑阵吧。\n",this_object(),who);
		return 1;
...
string ask_six()
{
	...
	if (query("book_give") < 1)
        return"你来晚了，那本《六脉神剑谱》我已经赠送给别入了。";   



if (fighter->query_temp("killtlbb3"))
{
command("say "+fighter->query("name")+" 多谢出手帮忙。\n");
}
...
if (this_player()->query_temp("killtlbb3"))
{
command("say "+this_player()->query("name")+" 段誉的下落，你去问本门枯竹大师吧。\n");
	this_player()->set_temp("killtlbb4",1);
}	
