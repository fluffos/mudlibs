...
add_action("do_shedinggood", "shedinggood");

add_action("do_songgood", "songgood");
}
...
int do_songgood()
{
	...
	write(present->query("name")+"("+present->query("id")+")给随机一个人，确定吗？[Y/N]\n");
	input_to ((: yyygood :));
	return 1;
}
...
void yyy(string arg)
{
	...
	int i;
          ...
          log_file("liwu",sprintf("%s  随机礼物%s，%s\n",ctime(time()),file
		,this_player()->query("id") ));
	if (arg == "y" || arg == "Y")
	{
...
int do_shedinggood(string arg)
{
	object who=this_player();
	object present;
	if ( wiz_level(who)<wiz_level("(arch)") && who->query("id")!="xyanhu" ) 
		return notify_fail("要wizard及以上巫师执行。\n");
