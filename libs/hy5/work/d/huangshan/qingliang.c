...
void sun_rise(int sunrise, int sunrise_stage) {
    ...
    if(sunrise_stage == 0 && random(10) > 2) {
        ...
        switch(sunrise_stage) {
            case 0:
                message("vision", RED"\n一弯红色的光芒渐渐从山峦上漫开，初时不很耀眼，但有很强的渲染力。\n"NOR, watchers);
                message("vision", RED"山峦的顶部都被染成红色。 \n\n"NOR, watchers);
                        call_out("sun_rise", 5, 0, 1);
                break;
            case 1:
                message("vision", HIR"\n桔红色的太阳缓缓的从山峦后升起，山谷忽然亮了起来。\n"NOR, watchers);
                message("vision", HIR"晨风中喧闹的山鸟止了叫声，一群群飞向远方，衬在朝阳里，留下一点点灵动的远影。\n"NOR, watchers);
                call_out("sun_rise", 5, 0, 2);
                break;
            case 2:
                message("vision", HIR"\n一轮红日映着天边的彩霞缓缓地从东方升起。\n"NOR, this_object(), watchers);
                message("vision", HIY"\n太阳终于跃起在峰峦之上，万物都被渡上了一层淡淡的金色。 \n"NOR, watchers);
                j = random(12);
                if (j == 1) {
                    message("vision", HIR"\n满山火红的杜鹃花也渡上了淡淡的金色，看似霞光流动，变幻着五彩的颜色，\n"NOR, watchers);
                    message("vision", HIR"你迷失在这五彩的世界里。\n"NOR, watchers);
                    for(i=0; i<sizeof(watchers); i++) {
                        if (!watchers[i]->query("宿命B/黄山_日出")) {
                                watchers[i]->set("宿命B/黄山_日出",1+random(10));
                        }
        ...
        switch(sunrise_stage) {
            ...
            case 2:
                message("vision", HIR"\n一轮红日映着天边的彩霞缓缓地从东方升起。\n"NOR, this_object(), watchers);
                message("vision", HIR"\n一轮红日浮动在流光灿烂的云海上，云涛渐渐平息，而不时变换着深红，浅红，桔红的色彩。\n"NOR, watchers);
                message("vision", HIR"晨雾渐渐散去，化做流霞点缀在天际。\n"NOR, watchers);
                j = random(12);
                if (j == 4) {
