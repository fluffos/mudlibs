...
void attempt_apprentice(object ob)
{
	...
	{
		command("say 贫僧不收低辈弟子。阿弥陀佛。\n");
		return;
	}
	ob->set_temp("wait_nod", 1);
	command("say 依照南少林寺历代规矩，"+RANK_D->query_respect(ob)+"你必须接下我十二招，我才能收你为徒。你愿意么？那就点个头吧(nod)。");
}
int do_nod()
{
...
void check(object ob)
{
	...
	else
	{
		...
		if( ob->query("class") == "bonze")
		{
			...
			command("say 从今以后你的法名叫做"+new_name+ "，恭喜你荣升为我南少林派天字辈圣僧！\n");
		}
		else ob->set("title", "南少林俗家第十八弟子");
	}
	return;
}
