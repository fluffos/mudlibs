...
map_skill("spells", "magic-water");
        set("no_get",1);
b=1210000+random(66000);
a=1210000+random(68000);
d=30+random(60);
set("str",d);
set("dex",30+d);
set("int",d);
...
if (!me) return;        
              me->add("score",1);
        message_vision(HIC"$N,倒地而亡，化为了一滩冰水。\n"NOR,ob,me);
if (!me->query("migong/lev16"))
{
exp = 480000+random(88000);
pot = exp/2;
score = random(300)+950;
 me->set_temp("magicov1",1);
 me->set("migong/lev16",1);
...
i=random(8);
	...
	me->add("per",1);tell_object(me,HIM"以及一点容貌。\n"NOR);
        ...
        if((int)me->query("combat_exp",1) < ob->query("combat_exp",1))   {
        message("vision", me->name() + HIG"成功消灭"+ob->query("title")+HIG",完成了火之世界的任务。\n"NOR, users());
...
{
corpse=new("/clone/magic/obj/firehat");
corpse->move(me);
