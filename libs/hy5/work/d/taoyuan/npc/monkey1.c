...
你也许可以教小猴子一些简单的动作。（ｔｅａｃｈ）\n");
    ...
    set("verbs", ({ "bite","claw" }) );
    setup();
        //carry_object(__DIR__"obj/monkeybook");
} 
void init() {
...
int do_kill(string arg)
{
        ...
        }else
        {
                ...
                {
                        message_vision("$N从地上抓起一把烂泥向$n扔一仍，做了一个鬼脸，飞奔而去。\n",this_object(),me);
...
//    write(sprintf("action is %s \ndest is %s\n act is %s\n", action, dest, act)); 
        result = me->command(sprintf("%s %s", action, dest? dest : ""));
        ...
        if(result) {
            //if(time() - query("time") > 10) 
                ...
                if( act == "wield" || act == "unwield" ) 
                    ...
                    command(sprintf("%s zhu pian", act));
                    set("time", time());
...
\n", this_object());
                    ...
                    command(sprintf("%s %s", action, dest? me->query("id") : ""));
                    set("time", time());
                }
            } else {
...
int accept_object(object who,object ob)
{
    ...
    while (i--) {
        ...
        if (obj)
        {
                message_vision("$N从怀中摸出了一个$n，指了指，一脸得益之色。\n",this_object(),obj);
                return 0;
        }
...
{
        object book;
        book = new(__DIR__"obj/monkeybook.c");
        
        //book->move(this_object());
        book-> move(environment(this_object()));
        message_vision("好象有什么从$N身上调了下来。\n",this_object());
        ::die();        
}    
