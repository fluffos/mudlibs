...
LONG
  ...
  "north" : __DIR__"baiyang",
        ...
        set("item_desc", ([
                "冰川": "
你围着冰川看了又看，发现冰川里似乎有着古怪！击(ｂｒｅａｋ)开它！或用水灌(guan)它。\n",
                "ice": "
你围着冰川看了又看，发现冰川里似乎有着古怪！击(ｂｒｅａｋ)开它！或用水灌(guan)它。\n",
        ]) );
        set("frozen",10);
...
int do_pour(string arg)
{
        ...
        if(con->query("liquid/name") == "热水" || con->query("liquid/name") == "清水")
        tell_object(me,"寒冰刚刚化开了一点儿就又冻上了！！\n");
        if(con->query("liquid/name") == "盐水" || con->query("liquid/name") == "米酒")
        {
