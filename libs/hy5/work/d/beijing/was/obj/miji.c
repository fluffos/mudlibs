...
int do_du(string arg)
{
        ...
        pxlevel = me->query_skill("xianglong-zhang", 1);
        if( (int)me->query("combat_exp") < 100000 ) {
                write("你的实战经验不足，无法看懂降龙十八掌秘笈中的内容。\n");
                return 1;
        }
        if( (int)me->query("xianglong-zhang") < (int)pxlevel*pxlevel*pxlevel/10 ) {
                write("你的实战经验不足，无法看懂降龙十八掌秘笈中的内容。\n");
                return 1;
        }
        if( me->query_skill("xianglong-zhang", 1) > 249){
                write("你研读了一会儿，但是发现秘笈中的内容对你而言都太浅了，没有学到任何东西。\n");
                return 1;
        }
        if( me->query_skill("xianglong-zhang", 1) < 80){
                write("你研读了一会儿，但是发现秘笈中的内容深奥无比，没有学到任何东西。\n");
                return 1;
        }
        me->receive_damage("qi", 50);
        me->receive_damage("jing", 50);
        me->improve_skill("xianglong-zhang", (int)me->query_skill("literate", 1)/5+1);
