...
个人都那么遥远。他仿佛已将自己完全隔绝红尘外，没有欢乐，没有享受。\n");
    ...
    set("resistance/kee",66);
    set("resistance/sen",66);
    set("resistance/gin",66);
...
等，迟早总会等得到机会的！”\n\n"NOR,ob);       
        ...
        if (environment(ob)->query("wanma_discussion") && me->query("wanma/准备杀主脑_ma")
                ...
                message_vision(CYN"马空群摸着手上的伤痕，“泪可以抹干，但血，还是要血来洗的！”\n\n\n",ob);
                
                if(!me->query("wanma/终局马空群")){
...
就不能在背后给他一刀。”\n"NOR,me);
                        ...
                        me->delete_temp("wanma/answer_ma");
                        me->set("wanma/上楼",1);
                        message_vision(YEL"\n$N不置可否地一笑：“此处耳目众多，你可到我书房等候，我自有安排。”\n"NOR,ob);
                        message_vision(YEL"$N转身离去，不再说话。\n"NOR,ob);
                        destruct(this_object()); 
                        return 1;
                }       
                tell_object(me,"你只能回答ｙｅｓ或ｎｏ。\n");
   ...
   int i;
        ...
        if(!objectp(weapon)){
                if(!objectp(obj)){
                        ...
                        message_vision(HIR"\n血与泪一滴一滴掉在$N的" + obj->name() + HIR"上又滑落到地上。\n", me);
                }
        }else {
                message_vision(HIR"\n血与泪一滴一滴掉落在$N的" + weapon->name() + HIR"上，不一会便凝固了。。 \n"NOR,
                                me);
                message_vision(HIR"血泪在" + weapon->name() + HIR"上弥漫着血腥的味道，但转瞬间又蒸发无痕。\n"NOR, me);
             ...
             if(random(me->query("宿命B/泰山_日出"))< 9 ) {
                                weapon->add(prop+"courage", 50);
                                weapon->add(prop+"strength", 20);
                                weapon->add(prop+"damage", 100);
                                weapon->add("base_damage", 100);
                        } else {
                weapon->add(prop+"karma", 3);
                                weapon->add(prop+"courage", 3);
                                weapon->add(prop+"strength", 10);
                        }
                   weapon->wield();
                } else {
             if(random(me->query("宿命B/泰山_日出")) < 9 ) {
