...
{
object weapon; if (!objectp(weapon = me->query_temp("weapon")) || (string)weapon->query("skill_type") != SKILL_BASE_NAME) return notify_fail("你使用的武器不对。\n");	if( (int)me->query("qi") < 25 )
...
return HIG"$N使用了无上妙仙决!!$N全身的伤好了很多!\n"NOR;
        ...
        if (random(damage) > victim->query_str() && random(3)==0) 
        {
                ...
                return  HIW "你听到「喀嚓」一声轻响，已被$N的兵器所发杀气挫伤，$n顿时血冒三丈！！！\n" NOR;
        }
...
if (result) message_vision(result,me,ob);
            return damage;
}
