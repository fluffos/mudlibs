...
addnl(who,"exp",exp);
        ...
        who->apply_condition("guojob2_busy",13);
        who->apply_condition("menpai_busy",13);      
        who->apply_condition("zzz_busy",5+random(3));      
...
if ((int)me->query("combat_exp",1) >100)
{
         me->add("combat_exp",-100);
}
if ((int)me->query("potential",1) >100)
{
...
{  object obj,*ob;
   ...
   { call_out("begin_kill",30,me,target);
	...
	{obj->do_copy(me,1);
	 ...
	 obj->set_name("黑衣人", ({me->query("id")+" heiyi","heiyi ren","ren"}));
