...
if (me->query("shen")<0)
    ...
    if ( (int)me->query("combat_exp") < 10000000 && me->query("jobtime")!=5)
...
me->apply_condition("menpai_busy",16);
}

int job_over(string arg)
{
        int job_pot,job_exp,i;
        object me = this_player();
	object badguy;
	object room;
	...
	if(me->query_temp("help_killed")>0)
	{
	        ...
	        job_exp=((int)me->query_temp("killed_mgb"))*120;
                  job_pot=job_exp*2/3;
                  job_exp=job_exp;
if (job_exp >3500) job_exp=3500;
job_pot=job_exp*2/3;
if (job_pot >3200) job_pot=3200;

if ((i = (me->query("combat_exp") - me->query_temp("temp_exp")) * 60
/ (me->query("mud_age") - me->query_temp("mud_age"))) > 100)
i = i*60;

if (i > 35900)
{
job_exp=job_exp/3+10;
job_pot=job_pot/3+10;
}

job_exp=job_exp*2/3+200;
addnl(me,"exp",job_exp);
me->add_temp("mpjobn6",1);
        ...
        me->start_busy(3);
                me->apply_condition("guojob2_busy",16);
me->apply_condition("menpai_busy",16);
       ...
       if(me->query_temp("killed_mgb")<2)
       {
        ...
        job_exp=job_exp;
if (job_exp >3500) job_exp=3500;
if (job_pot >3500) job_pot=3500;
me->apply_condition("menpai_busy",15);
...
string ask_newover()
{
         object me;
         me = this_player();
          if(!me->query_condition("guojob2_busy") && !me->query_condition("jobshadi_limit") && !me->query_temp("mark/job_shadi"))
         return "你没有领任务,跑这里瞎嚷嚷什麽?";
         me->clear_condition("jobshadi_limit");
         if (me->query("combat_exp",1) > 10000) me->add("combat_exp",-100+random(20));
