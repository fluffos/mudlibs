...
int ckiller=4;
#include <ansi.h>
#include "/quest/menpai/givegift2.c"
mapping *wheres=({
...
([ "quest_name": "玉虚子", "exp_bonus":random(10)+30,"pot_bonus": random(8)+10,"quest_city":"楚州" ]), 	
([ "quest_name": "云清子", "exp_bonus":random(10)+30,"pot_bonus": random(8)+10,"quest_city":"楚州" ]), 	
([ "quest_name": "张言成", "exp_bonus":random(10)+30,"pot_bonus": random(8)+10,"quest_city":"楚州" ]), 	
...
(["quest_name": "校尉", "exp_bonus":random(15)+30,"pot_bonus": random(8)+10 ,"quest_city":"楚州" ]), 	
(["quest_name": "鱼贩子", "exp_bonus":random(15)+30,"pot_bonus": random(8)+10 ,"quest_city":"楚州" ]), 	
(["quest_name": "莫飞云", "exp_bonus":random(15)+30,"pot_bonus": random(8)+10 ,"quest_city":"楚州" ]), 	
...
tell_object(me,"目前为止,你的门派贡献度为『"HIC+chinese_number(me->query("mpgx"))+NOR"』。\n" NOR);
             ...
             quest["score"]=50;
        me->set("quest", quest);
        me->set("task_time", (int) time()+(int) quest["time"]+380);
...
tell_object(who,"你坏我门派大事。下次小心了！\n");
}
//who->apply_condition("menpai_busy",9+random(9));
...
tell_object(who,"任务使说道，好吧，这次就算了，下不为例。\n");
                ...
                who->delete_temp("roomjob");
                   who->apply_condition("menpai_busy",9+random(9));who->delete_temp("menpaijob");
                   who->delete_temp("menpaijob");
                   who->set("quest",0);
                   who->delete("songxin");
...
if (!who->query("zhuanshen") && who->query("combat_exp",1)<=8000000 ) exp=exp*6;
if (!who->query("zhuanshen") && who->query("combat_exp",1)<=8000000 ) pot=pot*6;
             ...
             bonus += score;
             who->set("score", bonus);
             tell_object(who,"恭喜你又完成一个任务！\n");
             tell_object(who,"你被奖励了：" + chinese_number(exp)
...
{
tell_object(me,"只有"+this_object()->query("mp")+"的弟子才能做此任务!\n" NOR);
      return  "你是哪里来的奸细?";
} 
...
if(count>1)
{
minexp=ob->query("combat_exp",1);
	  maxexp=minexp;
      
	  for(i=1;i<count;i++)
      { if(team[i]!=0)
	  { if(team[i]->query("combat_exp",1)<minexp)
	    minexp=team[i]->query("combat_exp",1);
		if(team[i]->query("combat_exp",1)>maxexp)
        maxexp=team[i]->query("combat_exp",1);
	  }
	  }
	  if ((maxexp-minexp)>2000000)
{
if (wizardp(me)) tell_object(me,"队伍"+maxexp+":"+minexp+"。\n" NOR);
...
me->set("quest/quest", "武林高手到"+room);	
me->set("quest_name", "护送:"+room);	
...
if (rd==0) 
{
                ...
                obhs->move(room2);
                obhs->set_name(obhs->query("name"),({ me->query("id")+" "+obhs->query("id")}));
                ...
                me->set_temp("quest/locationname", room2->query("short"));
}	
...
if (!me->query("zhuanshen") && me->query("combat_exp",1) <=6000000)
tell_object(me,"据探子得知的情报，敌人最有可能在:"+me->query_temp("quest/locationname")+"附近。\n" NOR);

if (wizardp(me))
tell_object(me,"位置:"+me->query("quest/location")+":"+obhs->query("id")+"。\n" NOR);

me->delete_temp("quest/locationname");
	  return "去把门派密件找到吧。";
