...
{	
	...
	who->add("mpgx",10);
	who->add("expmax",2);
message_vision("\n$N说道：“我还联系了西藏的活佛去抓行痴这老和尚，吴三桂也要起兵了，还有关外的老毛子，我不会败的....。\n", this_object());
...
int ask_me()
{
	...
	{
		message_vision("$N对$n说道： 有事找胖头陀去说去吧！\n",this_object(),who);
				return 1;	
	}

	  	if(!(int)who->query_temp("book752"))
	{
		message_vision("$N对$n说道： 有事找瘦头陀去说去吧！\n",this_object(),who);
				return 1;	
	}
	
	  	if(!(int)who->query_temp("book753"))
	{
		message_vision("$N对$n说道： 有事找夫人去说去吧！\n",this_object(),who);
				return 1;	
	}
	
	command("kick " + who->query("id"));
	
	message_vision("$N对$n说道： 我教的事即然你知道，你就去死吧 。\n",this_object(),who);
        kill_ob(who);	            
