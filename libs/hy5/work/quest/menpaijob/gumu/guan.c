...
void create()
{
        set_name(HIR"蒙古元帅"NOR, ({ "yuanshuai"}));
       set("long",
                "一名蒙古元帅。\n");
...
int do_rob(string arg)
{
   ...
   stage = 0;
   	...
   	if (!arg )
   	return notify_fail("你要做什么！请输入 shayuanshuai 你的id！\n");
   	
   	if (arg!=me->query("id"))
   	return notify_fail("你要做什么任务 请输入 shayuanshuai 你的id！\n");
   	
   	if(me->query("id") !=ob->query_temp("jobname"))
		return notify_fail("这是别人的任务！\n");
