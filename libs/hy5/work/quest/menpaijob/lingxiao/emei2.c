...
if (random(10)>8)
{
	...
	ob->set("max_qi",  me->query("qi")/2); 
        ...
        ob->set("max_neili",me->query("max_neili"));
        ob->set("jing",me->query("max_jing")/3);
        ob->set("eff_jing",me->query("max_jing")/3);
        ob->set("max_jing",me->query("max_jing")/3);
}
...
int i,pot,exp;
	...
	return;
	if(objectp(killer = query_temp("last_damage_from"))
	&& killer->query_temp("lunzhi")
&& this_object()->query("usename")==killer->query("name"))

if (killer && killer->query_temp("lunzhi")
&& this_object()->query("usename")==killer->query("name"))
{
