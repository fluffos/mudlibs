...
int do_volunteer()
{
        ...
        if(!me->query_skill("kuihua-xinfa",1) && !me->query_skill("tmdafa",1))
           return notify_fail("左护法对你说：你先学点本门内功再来吧。\n");
      ...
      if (me->query_condition("riyue_guard"))
           return notify_fail("左护法对你说：你专心守好平定州，别想打两份工。\n");
      if (!me->query_condition("riyue_guard") && me->query_temp("in_guard"))
{
me->apply_condition("menpai_busy",12);
me->delete("guardry");
me->delete_temp("in_guard");
           return notify_fail("左护法对你说：任务失败了？ 你过一段时间再来吧。\n");
}
...
if (me->is_fighting())
      ...
      if (!me->query_condition("riyue_guard") && !me->query_temp("in_guard") && me->query("guardry"))
{
me->apply_condition("menpai_busy",12);
me->delete("guardry");
me->delete_temp("in_guard");
