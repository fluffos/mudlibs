...
int ask_job()
{
      ...
      if( !wizardp(me) && (int)me->query_skill("longxiang",1) < 50)
        ...
        if( !wizardp(me) && me->query("family/family_name") != "雪山寺")
           {
    ...
    if (!wizardp(me) && (int)me->query_condition("guojob2_busy"))
        ...
        if (!wizardp(me) && me->query("quest") ) 
      	{
command("say "+me->query("name")+"身上还有别的任务，先去完成了再来吧？");
return 1;
}
...
if(((int)this_player()->query("jinyong/book3")==2))
{
	...
	{
		message_vision("$N对$n说道： 你先帮我宰了花铁干吧。\n",this_object(),who);
		return 1;	
	}

		  	if((!(int)who->query_temp("killsx3")))
	{
		message_vision("$N对$n说道： 你先帮我宰了刘承风吧。\n",this_object(),who);
		return 1;	
	}

		  	if((!(int)who->query_temp("killsx1")))
	{
		message_vision("$N对$n说道： 你先帮我宰了陆天抒吧。\n",this_object(),who);
		return 1;	
	}

		  	if((!(int)who->query_temp("killsx4")))
	{
		message_vision("$N对$n说道： 你先帮我宰了水岱吧。\n",this_object(),who);
		return 1;	
	}		


command("thank " + who->query("id"));
	message_vision("$N对$n说道： $n干的不错。\n",this_object(),who);
	message_vision("$N对$n说道： 老祖绝不会亏待你的$n。\n",this_object(),who);
	...
	tell_object (who,"你赢得了"+chinese_number(i+10880)+"点经验"+
	           chinese_number(i*13)+"点潜能"+"十点门派贡献 二点成长上限！！\n");
	who->save();
return 1;
}	
...
//::die();	
        	...
        	message_vision("$N说道：这不可能，我的刀法天下无双！\n" NOR,this_object());
        	message_vision("$N说道：江陵古庙 礼拜诸佛......\n" NOR,this_object());
		i = 100+random(2000);
