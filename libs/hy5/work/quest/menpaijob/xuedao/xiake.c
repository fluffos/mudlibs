...
void create()
{
  ...
  set_name(names[i]+nm2[random(sizeof(nm2))]+nm3[random(sizeof(nm3))], ({"xiake"}));
...
int do_copy(object me)
{
      ...
      if(!me->query_temp("xx_rob")){
        ...
        ob->set("max_jingli",me->query("max_jingli") /2);
        ob->set("eff_jingli",me->query("max_jingli") /2);
        ob->set("jingli",me->query("max_jing") /2);
...
if (killer->query("kill_xueshan") == 1)
{
killer->add_temp("xsjob2",1);	
}
