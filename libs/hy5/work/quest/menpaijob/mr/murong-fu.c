...
if ((int)me->query("jinyong/book4")!=3)
      	return RANK_D->query_respect(this_player()) + 
		"想要做什么呢？";
	...
	this_player()->set_temp("mrjob",1);

	message_vision("$N想知段誉的消息，先帮我找大燕传国玉玺吧。\n",this_player());
	return "快去吧。";
       ...
       if(ob->is_busy())
{
   ...
   if(obj->query("id") == "mr yuxi" && me->query("kill_mr") == 1 && !ob->is_busy())
   {
   	ob->start_busy(2);
