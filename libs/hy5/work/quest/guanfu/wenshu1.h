...
{
        call_out("dest", 1800);
	add_action("do_wancheng2", ({"wancheng2","over"}));
...
// object ob;
	object me, target;
	int exp,pot,gfjob_times,now_time,i;
	...
	if(target->query("userp")) return notify_fail("嘿嘿，想作弊？！\n");
...
//      me->set("potential", me->query("max_pot"));
	...
	if ((int)me->query("combat_exp")<120000){
         pot=200+random(me->query("max_pot")*3/2);
...
gfjob_times=gfjob_times/10;
if (gfjob_times>200) gfjob_times=200;
	exp=exp+gfjob_times;
exp=exp+8;
addnl(me,"exp",exp);
me->add_temp("mpjobn9",1);
         destruct(this_object());
         return 1;
	}
	if ((int)me->query("combat_exp")>120000 && 
	me->query("combat_exp")<380000){
         pot=200+random(me->query("max_pot"));
...
// object ob;
    ...
    message_vision(HIW"$N冷笑一声，看来完不成任务了，从怀中掏出文书看了看，微运内劲将文书向空中飞射出去。"
"\n但见文书化做片片雪片，飞落在周围。\n"NOR, me);
	...
	me->apply_condition("gf_busy", 18);
	if (!me->query_condition("menpai_busy")) me->apply_condition("menpai_busy", 3);
	destruct(this_object());
	return 1;
}
