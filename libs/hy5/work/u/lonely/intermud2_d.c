...
void receive_ping_request(mapping info)
{
        ...
        NCH_CHANNEL("收到 PING 要求 , 送出回应到 "+info["HOSTADDRESS"]+":"+port+" .");
}
...
void receive_mudlist_request(mapping info)
{
        ...
        NCH_CHANNEL("收到 "+info["HOSTADDRESS"]+":"+info["PORTUDP"]+" 的 Mudlist Request 并回应之.");
}
...
void receive_gtell(mapping info)
{
        ...
        if( undefinedp(info["CNAME"]) )
                tell_object(ob, sprintf(GRN "%s@%s(%s) 告诉你：%s"NOR,capitalize(info["WIZFROM"]),info["NAME"],name,info["MSG"] ));
        else    tell_object(ob, sprintf(GRN "%s(%s@%s[%s]) 告诉你：%s"NOR,info["CNAME"],capitalize(info["WIZFROM"]),info["NAME"],name,info["MSG"] ));
