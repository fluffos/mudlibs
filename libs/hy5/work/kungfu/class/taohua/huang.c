...
int ask_marry()
{
        ...
        if ((int)ob->query("jinyong/book5")==4)
        {
...
int ask_test2()
{
        ...
        if (ob->query_temp("thd_marry") > 2) {
                ...
                if ((int)ob->query("jinyong/book5")==4 && !query("iskillok"))
        {
        command("say 这关你还要和欧阳世侄比一下武功。");
      	fang = load_object(__DIR__"ouyangke");
	fang->move(environment(ob));	
	fang->kill_ob(ob);
     this_object()->set("iskillok",1);
     set("iskillok",1);
ob->set_temp("book5511", 1);
return 1;
}

                if (!ob->query_temp("book5511"))
        {
        command("say 你打败欧阳世侄了吗。");
return 1;
}
...
int ask_test3()
{
	object fang;
        ...
        if (ob->query_temp("thd_marry") < 3) {
                ...
                if ((int)ob->query("jinyong/book5")==4 && !query("iskillok2"))
        {
        command("say 这关你还要和欧阳兄比一下武功。");
      	fang = load_object(__DIR__"ouyangfeng");
	fang->move(environment(ob));	
	fang->kill_ob(ob);
     this_object()->set("iskillok2",1);
     set("iskillok2",1);
ob->set_temp("book5521", 1);
return 1;
}

                if (!ob->query_temp("book5521"))
        {
        command("say 你打败欧阳兄了吗。");
return 1;
}
...
int do_answer(string arg)
{
    ...
    int payed;
who=ob;
if (ob->query_temp("big5") && userp(ob) && arg!="" ) arg=LANGUAGE_D->Big52GB(arg); 

        if (ob->query_temp("thd_marry") == 2) {
                
                if (!ob->query_temp("book5511"))
               {
               command("say 好小子，你要做什么?\n");
                        return 1;
                }
                                if (!ob->query_temp("book551") && (int)ob->query("jinyong/book5")==4)
               {
               command("say 好小子，你打赢了吗?\n");
                        return 1;
                }
...
command("wield yuxiao");
        ...
        if (ob->query_temp("thd_marry") == 3)  {
               ...
               command("say 好小子，你要做什么?\n");
                        return 1;
                                ...
                                if (!ob->query_temp("book552") && (int)ob->query("jinyong/book5")==4)
               {
               command("say 好小子，你打赢了吗?\n");
                        return 1;
                }
		...
		write("黄药师说道：我徒弟陆冠英处有敌人来偷袭，你有空去帮帮他吧。\n");
		who->save();
               }
