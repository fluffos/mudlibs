...
//	}
	...
	if (((int)ob->query_skill("sword",1) < 80) || ((int)ob->query_skill("huashan-sword",1) < 180))
	{
		command("say 我看" + RANK_D->query_respect(ob) + "的华山剑法还没学到家吧。");
		return;
	}
	if (((int)ob->query_skill("force",1) < 80) || ((int)ob->query_skill("zixia-shengong",1) < 180))
	{
		command("say 我看" + RANK_D->query_respect(ob) + "的紫霞神功还没学到家吧。");
		return;
	}
	if (((int)ob->query_skill("sword",1) < 80) || ((int)ob->query_skill("kuangfeng-jian",1) < 180))
	{
		command("say 我看" + RANK_D->query_respect(ob) + "的狂风快剑还没学到家吧。");
		return;
	}
	if (((int)ob->query_skill("force",1) < 80) || ((int)ob->query_skill("huashan-neigong",1) < 180))
	{
		command("say 我看" + RANK_D->query_respect(ob) + "的剑宗华山内功还没学到家吧。");
		return;
	}
	if (((int)ob->query_skill("dodge",1) < 80) || ((int)ob->query_skill("feiyan-huixiang",1) < 180))
	{
