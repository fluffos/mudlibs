...
int ask_job()
{
        ...
        if( !wizardp(me) && me->query("family/family_name") != "星宿派")
           {
                      message_vision("$N对着$n说道。你是哪里来的奸细？\n", this_object(), me);
                      return 1;
            }

      if( !wizardp(me) && (int)me->query_skill("huagong-dafa",1) < 50)
            {
...
return 1;
        ...
        if (!wizardp(me) && me->query("kill_xingxiu") == 1) {
                ...
                ob->set_name(HIM"大富商"NOR, ({ me->query("id")+" fushang"}));
