...
exp=exp/3+1;
addnl(ob,"exp",exp);
ob->apply_condition("menpai_busy",3);
                                ob->start_busy(3);
...
message_vision(HIR"\n$N不知道哪里找了个菜鸟煞有介事地来超度，看来是白忙了。\n"NOR, ob);
				...
				ob->add("qi", -(int)ob->query("qi")/8);
                                ...
                                ob->delete_temp("作法事");
		command("say 法事已经做完。");
	        ob->apply_condition("gelunbu",6+random(8));
...
int maxpot;	   
	...
	if ( ob->query_skill("longxiang",1) < 30 && ob->query_skill("xiaowuxiang",1) < 30)
		return "你的本门内功还早得很呢，没法让你开坛做法事。";
         if(ob->query_condition("menpai_busy")>0)
         return "你刚刚忙过,还跑这里瞎嚷嚷什麽?";
...
{
if (random(60)==1 && me->query("combat_exp") > 1500000)
{
