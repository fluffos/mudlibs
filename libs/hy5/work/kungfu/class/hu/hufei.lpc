...
int ask_me2()
{
	...
	{
		message_vision("$N对$n说道： 要知道宝藏的消息 先帮我把阎基抓来吧！\n",this_object(),who);
		return 1;	
	}
	...
	if ( !present("po tiehe", who))
        {
		message_vision("$N对$n说道： 没有藏宝铁盒我无法帮你！\n",this_object(),who);
		return 1;	
	}
	...
	{
		who->start_busy(3);
		message_vision("$N对$n说道： 这位"+ RANK_D->query_respect(who) +"先帮我战败田青文吧。\n",this_object(),who);
		return 1;
	}
	...
	{
		who->start_busy(3);
		message_vision("$N对$n说道： 这位"+ RANK_D->query_respect(who) +"先帮我战败殷吉吧。\n",this_object(),who);
		return 1;
	}

      if(!query("iskillok"))
      {
      message_vision("突然从门外突然杀进两个杀手 对$n说道： 把铁盒交出来吧!\n",this_object(),who);	
	...
	{
		message_vision("$N对$n说道： 你帮我对付了田青文了吗?\n",this_object(),who);
		return 1;	
	}
	
		  	if((!(int)who->query_temp("killtian1")))
	{
		message_vision("$N对$n说道： 你帮我对付了殷吉了吗?\n",this_object(),who);
		return 1;	
	}


	command("thank " + who->query("id"));
	message_vision("$N对$n说道： 多谢 保护藏宝铁盒。\n",this_object(),who);
	message_vision("$N对$n说道： 闯王宝藏就在玉笔峰附近。\n",this_object(),who);
	message_vision("$N对$n说道： 再请帮个忙，把苗大侠的女儿救出来吧。\n",this_object(),who);
