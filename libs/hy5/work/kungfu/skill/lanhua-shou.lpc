...
//me->delete_temp("autopfm_lhs");
	level   = (int) me->query_skill("lanhua-shou",1);
        if (random(3)==0 && random(level) > 160 && me->query_skill("force") > 250 && me->query("neili") >= 600) {
		me->add("neili", -10);
        ...
        if (random(3)==0 && random(level) > 130 && me->query_skill("force") > 250 && me->query("neili") >= 600) {
		me->add("neili", -10);
		me->set_temp("autopfm_lhs", 1);
		return ([
        ...
        if (random(3)==0 && random(level) > 80 && me->query_skill("force") > 250 && me->query("neili") >= 600) {
		me->add("neili", -10);
		me->set_temp("autopfm_lhs", 1);
		return ([
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	int hurt, lvl = me->query_skill("lanhua-shou", 1);
       ...
       int j, k;
 int level;
       j = me->query_skill("lanhua-shou", 1);
...
if (random(6)==0 && level>=300 && me->query_skill_mapped("force") == "bibo-shengong")
        {
victim->add("qi",-(250+random(j)));
victim->add("eff_qi",-(50+random(j)));
