...
int perform(object me, object target)
{
        ...
        if (me->query("max_neili") / 2 + random(me->query("max_neili") / 2) > target->query("max_neili"))
        {
                ...
                damage = 850+me->query_skill("unarmed")+(me->query_skill("longxiang",1)*3);
  if (damage > target->query("qi",1)/2) damage=target->query("qi",1)/2;
                ...
                if (userp(me) && userp(target) && random(3)==0)
                {
                target->unconcious();
              }
              else
              	{
              		             		 damage = 350+me->query_skill("unarmed")+(me->query_skill("longxiang")*3);
