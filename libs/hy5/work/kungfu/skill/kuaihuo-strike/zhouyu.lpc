...
int perform(object me, object target)
{
	...
	int flag;
	string fskill,sskill,bskill,pfname,pskill,j;
	...
	if( random(ap + dp) > dp )
	{
		damage = (int)me->query_skill("kuaihuo-strike",1)*10;
		...
		if( damage > 0 )
		{
			...
			damage +=(int)me->query_skill("strike",1)*2;
			if (damage >20000) damage=20000;
				if (userp(target)&& damage >target->query("jing",1)) damage=target->query("jing",1);
				if (userp(target)&& damage >target->query("qi",1)) damage=target->query("qi",1);	
