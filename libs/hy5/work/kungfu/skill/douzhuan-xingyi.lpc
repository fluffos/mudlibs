...
{
		ob->receive_damage("qi", damage2,me);
	...
	if ( damage > 0 ) {
		...
		&& me->query_skill_mapped("force") == "shenyuan-gong"
			...
			if (me->query_skill("douzhuan-xingyi", 1) > 150 && me->query("max_neili") > 2500
				...
				damage = damage + (damage* 2 / 3);
if (ob->query("qi") > damage)
				rob[i]->receive_wound("qi", damage,me);
			...
			if ( (random(me->query("combat_exp")) > ob->query("combat_exp")/3)
			     && (random(me->query("str", 1)) > ob->query("str", 1)/3)
			     && (random(me->query("dex", 1)) > ob->query("dex", 1)/3)
...
if (me->query_skill("shenyuan-gong",1) < 50) return;
	damage = 100+((int)me->query_skill("douzhuan-xingyi", 1)*3);
