...
victim->receive_damage("qi", flvl,me);
victim->receive_wound("qi", flvl,me);     
...
if (!level2) return;
if (!jiali) jiali=10;
	if( damage_bonus > 50 && random(level2)>100 && random(3)==0) {
	victim->receive_wound("qi", level2+random(50));
