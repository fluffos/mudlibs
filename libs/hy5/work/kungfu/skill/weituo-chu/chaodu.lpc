...
int perform(object me, object target)
{
	...
	if((int)me->query("con")+(int)me->query_skill("force",1)/10<50)
        ...
        message_vision(BLU"\n突然$N面露佛光：「超度极乐」，身形暴涨，如同韦陀尊者再世一般，右拳突出击在"+weapon->name()+BLU"当中，“哄”的一声，"+weapon->name()+"化为千颗流星飞向$n！\n" NOR, me,target);
//        weapon->move(environment(me));
//        weapon->set("name", weapon->query("name")+"的碎片");
//        weapon->unequip();
//        weapon->set("value", 0);
//        weapon->set("weapon_prop", 0);
        damage = (int)me->query_skill("club") + (int)me->query_skill("buddhism",1)+ ((int)me->query_skill("weituo-chu",1)*2);
...
if (random(6)==0 && target->query("int") > 26 && target->query("qi") < 800)
{
           	message_vision(HIY"\n$n脑中一片悬晕！一片空白！\n" NOR, me,target);
target->add("int",-1);
}
else if (random(6)==0 && target->query("str") > 26 && target->query("qi") < 800)
{
           	message_vision(HIY"\n$n全身一阵麻木！身体一松！\n" NOR, me,target);
target->add("str",-1);
}
