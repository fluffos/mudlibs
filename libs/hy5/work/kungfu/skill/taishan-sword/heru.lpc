...
int perform(object me,object target)
{
	...
	if( me->query("neili") < 250 )
		return notify_fail("你的内力不够，无法运用岱宗何如！\n");

	if( me->query("max_neili") < 500 )
		return notify_fail("你的内力修为不够，无法运用岱宗何如！\n");
...
void hit_him(object me, object target)
{
		...
		environment(me) == environment(target))
	{
		...
		target->receive_damage("qi",300+skill*2+me->query_skill("force",1));
		target->receive_wound("qi",300+skill*2+me->query_skill("force",1));
	}
	return ;
}
