...
if (random(2)==0) target->start_busy(3);
  ...
  if( random( me->query("combat_exp") ) > random( target->query("combat_exp")/3 ) || !living(target) || random(3)==0)
  {
			//damage = 500+random(  me->query_skill("meinv-quan",1) + me->query_skill("yunv-xinfa",1) + me->query("jiali") )*( 1 + random(me->query("jiali")/5) ) ;
  ...
  if( random( me->query("combat_exp") ) > random( target->query("combat_exp")/3 ) || !living(target) || random(3)==0)
  {
	//damage = 500+random(  me->query_skill("force",1) + me->query_skill("cuff",1) + me->query("jiali") )*( 1 + random(me->query("jiali")/5) ) ;
	damage =me->query_skill("yunv-jian",1);
		//damage = (int)me->query_skill("yunv-xinfa", 1);
