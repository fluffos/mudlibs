...
([	"action": "$N双腿一缩，似欲跪拜，一招"+(order[random(13)])+"「鲁达拨柳」"NOR"左手抓向$n右脚足踝，右手$w直击$n+(order[random(13)])+"「,
	"dodge": 180,
	"damage" : 180,
	"parry": 110,
	"force": 400,
...
if (random(6)==0 && level>=300 && me->query_skill_mapped("force") == "shenlong-xinfa")
{
...
if (!victim->is_busy()) victim->start_busy(2);	
 return HIR "$N高声狂叫：战无不胜，攻无不克，无坚不摧，无敌不破！$n头一晕,鲜血喷出!\n";
        ...
        if (random(6)==0  && me->query_skill_mapped("force") == "shenlong-xinfa")
        {
if (victim->query("qi") > 500)
victim->add("qi",-((random(lvl)) / 2+1));
victim->add("eff_qi",-(1+random(lvl)/2));
                victim->apply_condition("shenlong_poison",10);
