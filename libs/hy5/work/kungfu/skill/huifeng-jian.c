...
int ob_hit(object ob, object me, int damage)
{
        ...
        nskill =(int) me->query_skill("huifeng-jian",1);
     ...
     if(  wa && wa->query("skill_type") == "sword"
        && random(skill) > ((int)ob->query_skill("parry", 1)/3)
         ...
         && userp(me))
         {
               ...
               ob->receive_damage("qi", nskill+10);
...
nskill =(int) me->query_skill("huifeng-jian",1);
        ...
        if( random(2)==0 && me->query_temp("huifeng/jue")) {
                victim->add("qi", -(random(nskill)+10) / 2);
                victim->receive_wound("qi", (random(nskill)+10) / 2 );
