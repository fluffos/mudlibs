...
{                                                                                                                
	int level, jiali, time,level2,damage,lvl;
	object weapon;
	        string name;
        name = xue_name[random(sizeof(xue_name))];
	weapon = me->query_temp("weapon");
        lvl = (int) me->query_skill("qidao",1);
        level = (int) me->query_skill("xuantian-wuji",1);
        level2= (int) me->query_skill("xuantian-wuji",1);

if (random(4)==0 && level>=200 && me->query_skill_mapped("force") == "xuantian-wuji")
{

        if (!victim->is_busy())
        victim->start_busy(2);
//        me->add("neili", -50);
        victim->receive_damage("qi", random(lvl)/2  + damage_bonus / 3);
        victim->receive_wound("qi", random(lvl)/2  + damage_bonus / 5);
        if (random(2)==0)
...
int ob_hit(object ob, object me, int damage)
{
     ...
     int busy_time;
      ...
      if (random(6)==0 && !me->is_busy() && me->query_skill_mapped("force") == "xuantian-wuji") {
                message_vision(HIG"$N心神合一，琴弈相通，弈道的"+(order[random(13)])+""+(order[random(13)])+"「得算篇」"NOR""NOR"历历在目，身形骤然轻巧起来。\n"NOR, me);       
if (me->query("qi")<= me->query("max_qi") )
{
                me->add("qi",(me->query_skill("art",1)*2));
 if (me->query("eff_qi")<= me->query("max_qi") )               me->add("eff_qi",(me->query_skill("art",1)*2));
}
                me->start_busy(1);
        }
