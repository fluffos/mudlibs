...
level2= (int) me->query_skill("luoying-shenjian",1);             
	if (random(5)==1 && me->query_skill("luoying-shenjian",1)>100)
	{
	if (!victim->is_busy())
	victim->start_busy(2);
	return HIR"$N脚走五行,$n二眼一花不知所知!!\n"NOR;
	}
	if( random(3)==0 && random(level2)>80) {
	victim->receive_wound("qi", (random(level2)+10));
...
int ob_hit(object ob, object me, int damage)
{
	object weapon = me->query_temp("weapon");
        ...
        level = me->query_skill("luoying-shenjian", 1);
level2 = me->query_skill("luoying-shenjian", 1);
        if (  level2 > 100
        && random(5)==1){
