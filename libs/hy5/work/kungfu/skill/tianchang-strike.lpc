...
int valid_learn(object me)
{
        if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
                return notify_fail("练天偿掌法必须空手。\n");

        if ((int)me->query_skill("force") < 120)
                return notify_fail("你的内功火候不够，无法练天偿掌法。\n");
        
        if ((int)me->query("max_neili") < 750)
                return notify_fail("你的内力太弱，无法练天偿掌法。\n");

        if ((int)me->query_skill("strike", 1) < 80)
                return notify_fail("你的基本掌法火候不够，无法练习天偿掌法。\n");

        if ((int)me->query_skill("strike", 1) < (int)me->query_skill("tianchang-strike", 1))
                return notify_fail("你的基本掌法水平有限，无法领会更高深天偿掌法。\n");
...
{
        int i, level;
        level = (int)me->query_skill("tianchang-strike", 1);
...
int practice_skill(object me)
{
        ...
        if ((int)me->query("neili") < 70)
                return notify_fail("你的内力不够练天偿掌法。\n");
...
{
        return __DIR__"tianchang-strike/" + action;
}
