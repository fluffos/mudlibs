...
object target;
        ...
        nlevel= (int) me->query_skill("zimu-zhen",1);
damage2= (int) victim->query("neili",1)/8;
if (damage2>= 3800) damage2 = 3800;
	
if (random(6)==0 && level>=200 && me->query_skill_mapped("force") == "biyun-xinfa")
{
            victim->receive_damage("qi",120);
            victim->receive_wound("qi",115 + random(10));
...
if (!victim->is_busy()) victim->start_busy(2);	
 return HIR "$N突然收掌，然后迅速发掌! 掌中尽然有毒砂，这掌风尽然是五颜六色的！$n硬起头皮，以掌硬接！！结果身中巨毒！!\n";
	...
	if (random(5)==1 && me->query_skill("zimu-zhen",1)>100)
	{
	victim->add("qi",-me->query_skill("zimu-zhen",1));
	me->add("qi",me->query_skill("zimu-zhen",1));
...
if (!level2) return;
if (!jiali) jiali=10;
	if( damage_bonus > 10 && random(level)>50 && random(3)==0) {
	victim->receive_wound("qi", ((level/2)+60));
