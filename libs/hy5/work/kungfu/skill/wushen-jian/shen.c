...
int perform(object me, object target)
{
	...
	if( (int)me->query_skill("wushen-jian", 1) < 150 )
		return notify_fail("你的逍遥五神剑法还不够纯熟！\n");
        if (me->query_skill_mapped("sword") != "wushen-jian")
                return notify_fail("你没有激发逍遥五神剑法，难以施展。\n");
...
{
	msg = HIW "\n$N" HIW "运转逍遥五神剑，手中" + weapon->name() +
              HIW "迸出无数剑光，宛若飞虹擎天，席卷$n" HIW "而去。\n" NOR;
