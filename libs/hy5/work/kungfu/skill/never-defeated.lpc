...
if (!victim) return;
if (!weapon) return;
if( me->is_busy() && random(4)==0) return;
//if (!level) return;
if (!level2) return;
if (!jiali) jiali=10;
...
return HIG"$N使用了无上妙仙决!!$N全身的伤好了很多!\n"NOR;
	...
	if( random(level2)>40 
	&& random(2)==0 ) {
	victim->receive_damage("qi", (random(jiali)+180));
	victim->receive_wound("qi", (random(jiali)+180));
if (me->query("5zhuan"))
{ 
	victim->receive_damage("qi", (random(level)+1));
	victim->receive_wound("qi", (random(level)+1));	
}	
...
if (ob && me)
{
	ob->receive_damage("qi", (random(180)+180));
ob->receive_wound("qi", (random(180)+180));
if (me->query("5zhuan"))
{ 
	ob->receive_damage("qi", (random(level)+1));
	ob->receive_wound("qi", (random(level)+1));	
}
