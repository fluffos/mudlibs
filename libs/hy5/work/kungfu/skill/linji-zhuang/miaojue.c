...
lvl=(int)me->query_skill("linji-zhuang",1);
        if ((!target))
                return notify_fail("你要用真气为谁使用妙伤决？\n");
        ...
        if( !wizardp(me) && userp(me) && me->query("family/family_name") != "峨嵋派")
                return notify_fail("此绝技只有峨嵋派门人体悟佛心才能使用。\n");
...
//                return notify_fail("不能对自己使用。\n");
        ...
        if( target->query_condition("qihuifu"))
                return notify_fail("对方正在被妙伤决回复。\n");

        if( me->query_condition("gong_busy"))
                return notify_fail("你才对人使用过妙伤决 你的真气还没有恢复。\n");
        ...
        message_combatd(
                HIG "$N运起临济十二庄，将手掌贴在$n背心，一瞬间 将十二道真气输入$n体内....\n"
                "阿弥陀佛，善哉！善哉！ $N额头上冒出豆大的汗珠，$n吐出一口瘀血，脸色看起来红润多了。\n" NOR,
                me, target );
