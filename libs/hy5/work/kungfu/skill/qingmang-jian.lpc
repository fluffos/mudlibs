...
mapping query_action(object me, object weapon)
{
    ...
    if (random(3)==0 && !userp(me) || (int)me->query_skill("qingmang-jian", 1) > 10)
{
	

	if( random(8)==1 && random(level) > 100)
	{
		        if (me && target) 
        {
        if (!target->is_busy() && random(2)==0) target->start_busy(2);	
        }
		return ([
...
if (!weapon) return;
if (!jiali) jiali=10;
	if( damage_bonus > 30 && random(level2)>30 ) {
