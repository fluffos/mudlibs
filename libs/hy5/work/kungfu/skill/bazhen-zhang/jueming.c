...
skill = me->query_skill("bazhen-zhang",1);
        ...
        if( (int)me->query("neili") < 800 )
                ...
                if (random(me->query_skill("parry")) > dp || random(3)==0)
                {
                       me->start_busy(1);
                       target->start_busy(1);
                       me->add("neili", -200);
...
skill = me->query_skill("bazhen-zhang",1);
        ...
        if (random(me->query_skill("dodge")) > target->query_skill("dodge") / 2 || random(3)==0)
              {
...
if (skill> 350)
{
      msg = HIY "最后只见$N" HIY "聚力于掌，平平推出，顿时掌风澎湃，掌力"
        ...
        if (random(me->query_skill("strike")) > target->query_skill("dodge") / 2 || random(3)==0)
        {
                me->start_busy(3);
