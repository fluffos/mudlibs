...
mapping query_action(object me, object weapon)
{
        ...
        if (me->query_temp("tzzf") && me->query("jiali",1) <= 50 )
        me->set("jiali",50);
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
        int lev,damage, i;
        object weap;
        int level, jiali, time,level2,nlvl;
        object weapon;
        int lvl;
        int flvl;
string msg;
        lvl  = me->query_skill("guiyuan-tunafa", 1);
        nlvl  = me->query_skill("tiezhang-zhangfa", 1);
        flvl = me->query("jiali");

if (me->query_skill_mapped("force") == "guiyuan-tunafa" && lvl > 300 && random(6)==0)
{
...
{
		victim->receive_wound("qi", (nlvl) +10 );
        ...
        if( (me->query("neili") > 1000)&& (victim->query_temp("weapon"))
                 ...
                 {      
                        ...
                        message_vision(HIW "$N运掌如刀，连击三十六下，只听见"+(order[random(13)])+"「啪」"NOR"地一声，$n手中的" + weap->name()
                                + "已经断为两截！\n" NOR, me, victim );
if (!weap->query("systemmake") && !weap->query("ownmake"))
{
