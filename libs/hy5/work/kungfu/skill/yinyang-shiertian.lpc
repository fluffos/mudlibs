...
if (weapon) return;
if( me->is_busy() && random(2)==0) return;

if (damage_bonus>300) damage_bonus=300;
        victim->receive_wound("qi", (200+(damage_bonus * 2)), me);
...
if (me->query("zhuanfinal/1") ==1 && random(3)==0)
{
	...
	victim->receive_wound("qi", victim->query("qi")/8);
        if (!userp(victim))
         victim->receive_wound("qi", 888,me);
...
else        if (victim->query("neili") < 600)
                victim->set("neili", 0);
        else
