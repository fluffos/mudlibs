...
if (!target) return;
if (!me) return;
        targin=target->query("qi");
        tarsen=target->query("jing");
        damgin = targin/2;
        damsen = targin/2;
        if (damgin>=20000) damgin=20000;
        if (damsen>=20000) damsen=20000;
        cps = (int)target->query("dex");
        ...
        environment(me) == environment(target))
        {
			...
			//target->set_temp("last_damage_from",me);							
                	//target->die();
                	target->receive_damage("jing",damsen,me);
                	target->receive_damage("qi",damgin,me); 
                target->start_busy(8);
                }
                else{
