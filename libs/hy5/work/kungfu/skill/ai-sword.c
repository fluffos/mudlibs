...
void two(object me,object victim,object weapon,int damage)
{
     ...
     sk= me->query_skill("ai-sword", 1)*3;
...
//	&& me->query_skill_mapped("parry") == "ai-sword"  
	&& damage>0)
 	{
...
void lovekill(object me,object victim,object weapon,int damage)
{
 ...
 skillb = (int)me->query_skill("ai-sword",1)*3;
...
"HIR"                                       剑意纵横，分影倏忽，仿佛斩出了岁月惊心\n"NOR,me,victim);
             victim->receive_damage("qi",skillb+random(1000),me);
             COMBAT_D->report_status(victim);
...
"HIR"                                       剑光一道，夺人心魄，仿佛照出了岁月光华\n"NOR,me,victim);
             victim->receive_damage("qi",2*skillb+random(1000),me);
...
void lockman(object me,object victim,object weapon,int damage)
{
        ...
        int sk = me->query_skill("ai-sword",1)*3;
        if (!userp(previous_object())) sk=120;
        mk = (int)me->query("MKS",1);
               ...
               if( random(mecor) >= random(victimcor))
                {
                   ...
                   victim->receive_damage("qi",3*sk+random(1000),me);
 //                  victim->receive_damage("jing",1*(victim->query("max_jing")/10));
                   victim->receive_damage("jing",1*(victim->query("max_jing")/8),me);
                   if (damage <= 0)
...
void four_kill(object me,object victim,object weapon,int damage)
{
 ...
 bell = me->query("bellicosity");
 mk = (int)me->query("MKS",1);
 damage= (int)(me->query_skill("ai-sword", 1))*5;
...
message_vision("\n
                             "HIG"§情真爱又真．快乐是人生§"NOR"\n"NOR,me,victim);
             victim->receive_damage("qi",1*damage+random(1000),me);
             COMBAT_D->report_status(victim);
 ...
 if(random(2)==0)
  {
   ...
   message_vision(HIR"              血花耀眼又灿烂，灿烂的婉如流星，流星一现即逝，血花也很快的洒下\n"NOR, me);
        ...
        victim->receive_damage("qi",1*(victim->query("max_qi")/5),me);
//        victim->receive_damage("jing",1*(victim->query("max_jing")/5));
        victim->receive_damage("jing",1*(victim->query("max_jing")/5),me);
        victim->start_busy(2);
   ...
   else
  {
   message_vision(HIR"              血花耀眼又灿烂，灿烂的婉如流星，流星一现即逝，血花也很快的洒下\n"NOR, me);
        ...
        victim->receive_damage("qi",1*(victim->query("max_qi")/8),me);
        victim->receive_damage("jing",1*(victim->query("max_jing")/8),me);
        victim->start_busy(1);
        me->add("neili",-1*(me->query("neili")/10));
  }
...
void power_slash(object me,object victim, object weapon,int damage)
{
 ...
 victim=enemy[j];
    ...
    {
     victim->receive_wound("qi", damage+random(5000),me);
     switch(random(4)+1)
    {
     case 1 :
...
message_vision(sprintf(HIC"$n被$N的七色剑意所困，全身上下伤痕累累！\n"NOR),me,victim);
          victim->receive_wound("qi",da+random(3000),me);
          COMBAT_D->report_status(victim,1);
     }
     else
     {
...
void dream(object me,object victim,object weapon,int damage)
{
     ...
     if(mk < 10000)  {damagea = mk/10;  }     //mk<10000  ,威力约在800-990
                                              ...
                                              //杀人数越少,威力才会强
     damage1= random(damagea)+damagea+random(3000)*3;
     bell = me->query("bellicosity",1);
    ...
    if(random(100)>50)
     {
       message_vision(HIW"\n
        ...
        "HIW"天地间飘曳起一道幽幽蓝光，似真似幻，如沉静的月光投入深邃的大海 \n
              ...
              victim->receive_damage("qi",1*damage1,me);
...
message_vision(sprintf(MAG"结果被$N确确实实的命中要害，$n身上立即多了一道无情的伤痕！\n"NOR),me,victim);
          victim->receive_wound("qi",5900+random(5000),me);
       ...
       for(i=1;i<=3;i++)
       {
         switch(i)
         {
           ...
           if( me->query_skill("ai-sword",1) > random(60)+100)
           {
                  ...
                  victim->receive_wound("qi",800+random(3000),me);
                  me->add("neili",-50);
                  if(i != 3)
                  COMBAT_D->report_status(victim,1);
                  else if( damage <=0 )
...
NOR"            "HBBLU"剑光剑意已侵入$n神志，粉碎了$n的斗志，让$n尽往回忆的伤悲处走\n"NOR, me, victim);
                ...
                victim->receive_wound("jing",(me_jing+3000),me);
                victim->receive_wound("qi",(me_qi+3000),me);
