...

{
   ...
   set_name(order[random(13)]+"皇家狮鹫"NOR, ({ "griffin"}));
if (random(2)==0)
	set("magicgift",5);
...
b=119600+random(99000);
a=119600+random(99000);
d=95+random(80);
set("str",d);
set("dex",30+d);
set("int",d);
...
set("eff_qi",a);
 ...
 set("chat_chance_combat", 90);
        set("chat_msg_combat", ({
  (: perform_action, "spells.moonlight griffin" :),
 }) );
     set("chat_chance", 30);
        set("chat_msg", ({
		(: command("get coin") :),
        ...
        if(random((int)me->query("combat_exp")) > ob->query("combat_exp")/3)   
{
           ...
           ob->start_busy(2);
           msg = RED"$N"RED"飞上天空猛冲下来，$n"RED"结果被扫中了!\n"NOR;
