...

{
   ...
   set_name(order[random(13)]+"贝尔菲高尔"NOR, ({ "ying"}));
set("magicgift",1+random(6));
	set("magicset",1);
       set("title",HIB"欲情天使"NOR);
       set("zhuanbest",1);
       set("long",
"一位可怕的坠天使。\n");
...
set("eff_qi",a);
	...
	set_temp("apply/armor",8800);
	set_temp("apply/damage",8600);
     map_skill("spells", "magic-light");
        set("chat_chance_combat", 99);
        set("chat_msg_combat", ({
               (: perform_action, "spells.cure yqh" :),
               (: perform_action, "spells.delete" :),
               (: perform_action, "spells.holyword" :),
               (: perform_action, "spells.moonlight yqh" :),
