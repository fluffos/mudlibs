#include <ansi.h>
inherit BOSS;
string *order = ({""HIY"", ""HIG"", ""RED"", ""MAG"", ""YEL"", ""HIC"", ""HIB"", ""HIR"",""HIB"", ""CYN"",""WHT"",""HIM"",""BLU""});
...

{
   ...
   set_name(order[random(13)]+"食尸鬼"NOR, ({ "Ghoul"}));
...
b=105600+random(86000);
a=105600+random(85000);
d=105+random(60);
set("str",d);
set("dex",30+d);
set("int",d);
...
{
string msg;
        if(random((int)me->query("combat_exp")) > ob->query("combat_exp")/32)   
{
             ob->add("qi",-ob->query("qi")/30);
             ob->add("eff_qi",-ob->query("eff_qi")/30);
           ob->start_busy(2);
           ob->apply_condition("xx_condition",10);
           msg = RED"$N"RED"突然跳起一爪!，$n"RED"被打中了!\n"NOR;
