...
void create()
{
	...
	//迷宫房间里的BOSS。
        set_exit_npcs(__DIR__"npc/boss203");
	...
	//入口房间短描述
	set_entry_short(HIM"混沌的迷宫"NOR);

	//入口房间描述
	set_entry_desc(RED @LONG
...
LONG
	...
	//BOSS房间短描述
	set_exit_short(HIM"混沌的迷宫的尽头"NOR);

	//BOSS房间描述
	set_exit_desc(RED @LONG
这里是混沌的迷宫,似乎是用来避难的地方。然而地
上到处是一堆堆的骨骸与乾掉的血迹。更让人觉得气氛
显得十分的诡异。
LONG
	...
	//迷宫房间的短描述
	set_maze_room_short(HIM"混沌的迷宫"NOR);

	//迷宫房间的描述，如果有多条描述，制造每个房
	//间的时候会从中随机选择一个。
	set_maze_room_desc(RED @LONG
这里是混沌的迷宫,似乎是用来避难的地方。然而地
上到处是一堆堆的骨骸与乾掉的血迹。更让人觉得气氛
显得十分的诡异。。
LONG
