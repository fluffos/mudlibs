...

{
   ...
   set_name(WHT"真.千年冰龙王"NOR, ({ "bing long" , "long" , "dragon" }));
                set("race", "野兽");
       set("long",
"一条巨大的龙，嘴里喷着冰，看到了你，向你扑了过来。
...
b=2800000+random(660090);
a=3000000+random(680900);
d=50+random(60);
set("str",d);
set("dex",30+d);
set("int",d);
...
int big_blowing()
{
        message_vision(WHT "$N" WHT "从冰海里深深地吸入一口冰气，全身转为雪白的晶色，"
                       "整个龙腹胀大了几倍！！\n" NOR, this_object());
...
int hurting()
{
        ...
        message_vision(HIB "$N" HIB "吐出一股冰冷的冷焰．．．．．．整个天地似乎都被冻结！！！\n" NOR,
                       this_object());
...
if (ob!=me && random(2)==0)
{
	...
	ob->apply_condition("wugong_poison", 80);
        ob->apply_condition("chanchu_poison", 80);
        ob->apply_condition("xiezi_poison", 80);
        ob->apply_condition("snake_poison", 80);
        ob->apply_condition("zhizhu_poison", 80);
        tell_object(ob, HIG "你觉得被咬中的地方一阵麻痒！\n" NOR );
}

else
{        
        damage = 1300 + random(2900);
        damage -= ob->query_temp("apply/reduce_fire");
        if (damage < 0) damage = 0;
        ob->receive_wound("qi", damage);
        me->set("neili", me->query("max_neili"));
        return WHT "$N" WHT "“吼”的一声吐出一团冰焰，登时令$n"
               WHT "犹如进入寒冰地狱。\n" NOR;
