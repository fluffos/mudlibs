	...
	if(killer && userp(killer)) {
		...
		//第6个任务，守卫丁一，杀二十个亡灵
                if(mazeobj->query("quest/quest_index")==6 && is_undead() &&
                   environment() && environment()->query("short")==HIY"聚灵法阵"NOR) {
				count = mazeobj->query("quest/to_kill/亡灵");
				if(count>0) {
					finished = mazeobj->query("quest/killed/亡灵");
					finished++;
					if(finished>count)
						finished = count;
					mazeobj->set("quest/killed/亡灵",finished);
