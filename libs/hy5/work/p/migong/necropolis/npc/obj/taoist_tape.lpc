	...
	switch (index)  {
		...
		default: 
			...
			if(mazeobj) {
				...
				mazeobj->set("quest/quest_name","证明忠诚");
				mazeobj->set("quest/quest_desc","杀八个骷髅，八个幽灵，八个僵尸");
				mazeobj->set("quest/quest_branch","taoist");
				mazeobj->set("quest/to_kill/骷髅",8);
				mazeobj->set("quest/to_kill/幽灵",8);
				mazeobj->set("quest/to_kill/僵尸",8);
			}
			speaker->delete_temp("in_speech");
			destruct(this_object());
			return;
	...
	switch (index)  {
		...
		default: 
			...
			if(mazeobj) {
				...
				mazeobj->set("quest/quest_name","证明实力");
				mazeobj->set("quest/quest_desc","杀三个骷髅武士，三个骷髅法师，三个幽冥之眼，三个幽冥之火，三个血僵尸，三个尸煞");
				mazeobj->set("quest/quest_branch","taoist");
				mazeobj->set("quest/to_kill/骷髅武士",3);
				mazeobj->set("quest/to_kill/骷髅法师",3);
				mazeobj->set("quest/to_kill/幽冥之眼",3);
				mazeobj->set("quest/to_kill/幽冥之火",3);
				mazeobj->set("quest/to_kill/血僵尸",3);
				mazeobj->set("quest/to_kill/尸煞",3);
			}
			speaker->delete_temp("in_speech");
			destruct(this_object());
			return;
	...
	switch (index)  {
		...
		default: 
			...
			if(mazeobj) {
				...
				mazeobj->set("quest/quest_name","排除异己");
				mazeobj->set("quest/quest_desc","杀少林和尚，取得法器");
				mazeobj->set("quest/quest_branch","taoist");
				mazeobj->set("quest/to_kill/心武",1);
				mazeobj->set("quest/to_find/聚灵塔",1);
			}
			speaker->delete_temp("in_speech");
			destruct(this_object());
			return;
	...
	switch (index)  {
		...
		default: 
			...
			if(mazeobj) {
				...
				mazeobj->set("quest/quest_name","寻找法引");
				mazeobj->set("quest/quest_desc","找到巫妖的骨杖，幽冥魔的幽冥之火，僵尸王的僵尸血各一样");
				mazeobj->set("quest/quest_branch","taoist");
				mazeobj->set("quest/to_find/骨杖",1);
				mazeobj->set("quest/to_find/幽冥之火",1);
				mazeobj->set("quest/to_find/僵尸血",1);
			}
			speaker->delete_temp("in_speech");
			destruct(this_object());
			return;
	...
	switch (index)  {
		...
		default: 
			...
			if(mazeobj) {
				...
				mazeobj->set("quest/quest_name","清理法阵");
				mazeobj->set("quest/quest_desc","把丁一带到聚灵法阵，并清理那里的血魔手下");
				mazeobj->set("quest/quest_branch","taoist");
	...
	switch (index)  {
		...
		default: 
			...
			if(mazeobj) {
				...
				mazeobj->set("quest/quest_name","超度亡灵");
				mazeobj->set("quest/quest_desc","护卫丁一，同时在聚灵法阵处杀死十个亡灵");
				mazeobj->set("quest/quest_branch","taoist");
				mazeobj->set("quest/to_kill/亡灵",10);
...
void play_sound_7(object speaker,int index) {
	...
	switch (index)  {
		...
		case(5):
			...
			environment()->set("short","聚灵法阵");
			objs = all_inventory(environment());
			foreach(obj in objs) 
					if(obj && userp(obj))
							obj->ccommand("look");
			environment()->set("short",HIY"聚灵法阵"NOR);
			moon->hurt();
			break;
		case(6):
			tell_room(environment(),HIR"\n\n丁一说道：朋友们，现在是我报答你们的时候了！就是把你们变成血魔的一部分！哈哈！\n\n"NOR);
			speaker->set_temp("任务",7);
			//mazeobj = MAZE_D->query_maze_mainobj(speaker);
			if(mazeobj) {
				mazeobj->set("quest/maze_code","necropolis");
				mazeobj->set("quest/quest_index",7);
				mazeobj->set("quest/quest_name","终结血魔");
				mazeobj->set("quest/quest_desc","杀死成为血魔的丁一");
				mazeobj->set("quest/quest_branch","taoist");
				mazeobj->set("quest/to_kill/丁一",1);
			}
