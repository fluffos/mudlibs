...
void kicking(object ob)
{
	...
	message("vision", "只听“乒”地一声，" + ob->query("name") + "被人从小店里一脚踢了出来，狼狈万状的逃开了。\n", environment(ob), ob);
}
...
int ask_me()
{
	...
	{
		message_vision("$N对$n说道： 小尼姑？ 就在楼上，还有几分姿色，不过跟她一起的就不怎么样了。\n",this_object(),who);		
		return 1;
	}
	else
	{
		message_vision("$N对$n说道： 小尼姑？ 就在楼上，似乎是被人绑架来的，客官想打抱不平可要小心啊。\n",this_object(),who);
		who->move("/p/jinyong/book8/henshan/huiyan");
		return 1;
	}
}