...
$n笑道：“好，十年了，我终于可以重出江湖了，多谢这位"+ RANK_D->query_respect(me)+"\n", me, ob );
		...
		if(!huang)
			new("/p/jinyong/book8/meizhuang/npc/huangzhong-gong")->move(where);
		if(!sheng)
			new("/p/jinyong/book8/meizhuang/npc/danqing-sheng")->move(where);
		if(!heibai)
			new("/p/jinyong/book8/meizhuang/npc/heibai-zi")->move(where);
		if(!tubi)
			new("/p/jinyong/book8/meizhuang/npc/tubi-weng")->move(where);
