...
{
			me->start_busy(2);
			return notify_fail("附近全是道路，让你心神一乱。\n");
}
...
"$N爬上自己的宏物，大喝一声“去吧”，只见宏物向外飞跑而去。。。\n\n"NOR, me);
          ...
          if(arg=="gc") pla = "/d/city/majiu";
     else if(arg=="tam") pla = "/d/beijing/majiu";
     else if(arg=="qz") pla = "/d/quanzhou/zhongxin";
     else if(arg=="gm") pla = "/d/gumu/mumen";
     else if(arg=="gyz") pla = "/d/guiyun/taihu";
     else if(arg=="xy") pla = "/d/xiangyang/guangchang";
...
else if(arg=="hmy") pla = "/d/heimuya/road1";	
else if(arg=="tzf") pla = "/d/tiezhang/shanjiao";	
else if(arg=="mz") pla = "/d/meizhuang/gate";				
	
else if(arg=="th") pla = "/d/mr/matou";
else if(arg=="wls") pla = "/d/dali/wuliang/xiaolu1";
else if(arg=="ts") pla = "/d/xingxiu/tianroad2";
else if(arg=="lxc") pla = "/d/lingxiao/shiya";
else if(arg=="xss") pla = "/d/xueshan/shanmen";
else if(arg=="gmd") pla = "/d/mingjiao/shanmen";						

else if(arg=="zt") pla = "/d/huashan/jzroad3";
else if(arg=="cbs") pla = "/d/guanwai/baihe";
else if(arg=="zns") pla = "/d/quanzhen/shanjiao";
else if(arg=="hes") pla = "/d/hengshan/jinlongxia";
else if(arg=="hens") pla = "/d/henshan/lingxingmen";
else if(arg=="hs") pla = "/d/huashan/shaluo";	

else if(arg=="tas") pla = "/d/taishan/daizong";
else if(arg=="sons") pla = "/d/songshan/taishique";
else if(arg=="qcs") pla = "/d/qingcheng/jianfugong";
else if(arg=="kls") pla = "/d/kunlun/shanmen";
else if(arg=="ems") pla = "/d/emei/bgs";
else if(arg=="wds") pla = "/d/wudang/jiejianyan";	

else if(arg=="nsl") pla = "/d/nanshaolin/ting";
else if(arg=="bsl") pla = "/d/shaolin/shanmen";
else if(arg=="wdj") pla = "/d/wudujiao/xiao3";
else if(arg=="tm") pla = "/d/tangmen/zhulin";
...
"宏物跑了半天,没找到要去的地方！\n"NOR, me);
               message_vision(HIR
"目前你可以去的地方有如下:
北京(tam)   大理(dl)   泉州(qz) 马厩(gc)  塘沽(tg)   归云庄(gyz)
襄阳(xy)    武功镇(wg) 长安(ca) 兰州(laz) 关外(gw)   景德镇(jdz)
永登城(yd)  伊犁镇(yl) 佛山(fs) 扬州(yz)  延平(yp)   江陵(jl)
成都城(cd)  无锡城(wx) 灵州(lz) 金陵(jinl)苏州城(sz) 杭州城(hz)
福州城(fz)  沧州(cz)   盛京(yj) 昆明(km)  荆州(jz)   洛阳(ly)
岳阳(yy)    宜兴(yx)   武庙(wm) 黑木崖(hmy) 铁掌峰(tzf) 梅庄(mz)
太湖(th)   无量山(wls) 天山(ts) 凌霄城(lxc) 雪山寺(xss) 光明顶(gmd)
中条山(zt)  长白山(cbs) 终南山(zns) 恒山(hes) 衡山(hens) 华山(hs)
泰山(tas)   嵩山(sons) 青城山(qcs)昆仑山(kls) 峨眉山(ems)武当山(wds)
南少林(nsl) 北少林(bsl) 五毒教(wdj) 唐门(tm)  太原(ty)  新疆(xj)
\n"NOR, me);
