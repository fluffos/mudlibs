...
void create()
{
	...
	int i;
        ...
        set_name(HIY"「九阴真经」上卷"NOR, ({ "jiuyin shangjuan", "shangjuan" }));
        set_weight(50);
        if( clonep() )
		...
		for (i=0; i<sizeof(skills); i++)
			desc += sprintf("\t%s (%s)\n", to_chinese(skills[i]["name"]), skills[i]["name"]);
                set("long",HIC"这是九阴真经的上卷，里面记载着深奥的道家练起法门。你可以研读以下的技能：\n\n"HIW+desc+"\n"NOR);
                set("treasure", 1);
                set("value", 0);
                set("material", "skin");
                set("no_put", 1);
...
int do_yandu(string arg)
{
	...
	if( me->query("int") < 30 || me->query_int() < 50 )
