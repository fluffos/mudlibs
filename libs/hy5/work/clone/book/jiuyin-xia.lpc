...
mapping *skills = ({
	(["name": "jiuyin-baiguzhua", "max": 200]),
	(["name": "cuixin-zhang", "max": 200]),
...
void create()
{
	...
	int i;
        ...
        set_name(HIY"「九阴真经」下卷"NOR, ({ "jiuyin xiajuan", "xiajuan" }));
        set_weight(50);
        if( clonep() )
		...
		for (i=0; i<sizeof(skills); i++)
			desc += sprintf("\t%s (%s)\n", to_chinese(skills[i]["name"]), skills[i]["name"]);
                set("long",HIC"这是九阴真经的下卷，里面记载着各种高深的武学。你可以研读以下的技能：\n\n"HIW+desc+"\n"NOR);
                set("treasure", 1);
                set("value", 0);
                set("material", "skin");
                set("no_put", 1);
