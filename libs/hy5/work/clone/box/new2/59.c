...
void create()
{
        set_name(HIC"天兵符"NOR, ({ "tianbing fu", "fu" }) );
        set_weight(500);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("long", "这是一个可以使用的神符。可以使用(use)\n") ;
...
if (userp(me) && userp(target) && target->query("combat_exp",1) < me->query("combat_exp",1)) 
	return notify_fail("对方比你弱小，没必要使用！\n");
        ...
        if( !objectp(ob = present("tianbing fu", me)) )
...
return notify_fail("现在正是用攻击敌人的时候啊！\n");
                me->start_busy(3);
        message_vision(HIC"\n$n"HIC"突然祭出一张 天兵符 !!  \n"NOR,this_object(),me);
         msg = HIC"$N"HIC"对着$n"HIC"祭出一张 天兵符！\n"NOR;
        msg += HIC"$n"HIC"大吃一惊!!\n"NOR;

	exp = (int)me->query("combat_exp");
	me->receive_damage("jing",100);
	dizi = new("/d/wuyi/npc/npcz1.c");
	dizi->move(environment(me));
        dizi->invocation(me, (exp));
        dizi->set("possessed", me);


	dizi1 = new("/d/wuyi/npc/npcz2.c");
