...
if (this_player()->query("str") <18)
{
this_player()->set("str",18);
message_vision(HIB"$N的先天臂力增加了！\n"NOR, this_player());
}
if (this_player()->query("int") <18)
{
this_player()->set("int",18);
message_vision(HIC"$N的先天悟性增加了！\n"NOR, this_player());
}
if (this_player()->query("con") <18)
{
this_player()->set("con",18);
message_vision(HIY"$N的先天根骨增加了！\n"NOR, this_player());
}
if (this_player()->query("dex") <18)
{
this_player()->set("dex",18);
message_vision(HIG"$N的先天身法增加了！\n"NOR, this_player());
}
if (this_player()->query("kar") <25)
{
this_player()->set("kar",25);
message_vision(HIC"$N的先天福缘增加了！\n"NOR, this_player());
}
if (this_player()->query("per") <22)
{
this_player()->set("per",22);
