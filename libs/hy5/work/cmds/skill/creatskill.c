...
if((int)me->query("skillmaxim")!=0
        && sizeof(keys(me->query("skillmaxim/"))) >2 )
    return notify_fail("你最多只能创建二种高级功夫。\n");
...
if((int)me->query("combat_exp") < (skill_num-1)*1000000+1000000)
       return notify_fail("你的经验值还不够。\n");


if(skill_num>= 30  && (int)me->query("combat_exp") < (skill_num-1)*10000000+1000000)
       return notify_fail("等级超过三十，你的经验值还不够。\n");


if(skill_num>= 50)
       return notify_fail("一个武功最多创建五十招。\n");
...
分类为招名 出招 中招 未中 四种 分别对应绝招出招 中招 未中 时的描述。
描述中不能出现控制符，不然将删除!
