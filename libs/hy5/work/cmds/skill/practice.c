...
//          return notify_fail(HIY"\n不要影响别人，还是找一个室内的地方吧。\n"NOR);
	...
	if ( times < 1 || times > 300)
		return notify_fail("练习次数必须大于一次，但不能超过三百次。\n");
	...
	if (skill/10*skill*skill > (int)me->query("combat_exp")) 
//	{
...
if ((!me->query("zhuanshen") && !me->query("guard/flag") && !me->query("zhuanbest") && !me->query("4zhuan") && !me->query("5zhuan")) || me->query("hyvip")==4)
{
			me->improve_skill(skillname, ((skill_basic/5 +1)*times));
			//me->improve_skill(skillname, ((skill_basic/5 +1)*times));
}


if(present("learn emblem",me)) me->improve_skill(skillname, ((skill_basic/5 +1)*times));
if(present("study emblem",me)) me->improve_skill(skillname, ((skill_basic/5 +1)*times));
