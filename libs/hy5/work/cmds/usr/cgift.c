...
if (me->query("cgiftok",1)>5)
return notify_fail("玩家只能修改五次天赋!\n");
...
if (!gift_desc[from] )
return notify_fail("你要调整什么属性? 六项先天属性代号如下:膂力:str,根骨:con,身法:dex,悟性:int,福缘:kar,容貌:per。\n");
...
write("六项先天属性代号如下:膂力:str,根骨:con,身法:dex,悟性:int,福缘:kar,容貌:per,\n");
write("请慎重使用。一个玩家只能修改五次先天属性。\n");
return 1;
}
