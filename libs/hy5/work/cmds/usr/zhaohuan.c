...
int main(object me, string arg)
{
	...
	if (!wizardp(me) && me->query_temp("zhshinv",1) > 7)
		return notify_fail("本次登陆 你已经召唤过8次。\n");

	if (me->query("jing") < 100)
		return notify_fail("你的精气太低了，无法发出长啸。\n");
		
        if (!wizardp(me) && (int)me->query_condition("zdizi_busy"))  
        return notify_fail("你刚召唤过她.\n");
...
object clone_guard(object me)
{
	...
	ob->set("str", (me->query("str") > 90 ? 90 : me->query("str")));
	ob->set("int", (me->query("int") > 90 ? 90 : me->query("int")));
	ob->set("con", (me->query("con") > 90 ? 90 : me->query("con")));
	ob->set("dex", (me->query("dex") > 90 ? 90 : me->query("dex")));
	ob->set("kar", (me->query("kar") > 90 ? 90 : me->query("kar")));
...
ob->set("hyvipok",1);
	...
	ob->set("max_neili", (int)me->query("max_neili") * 3);
	...
	if (me->query("hyvip",1)==4) 
{
	...
	if (stringp(ob->query_skill_mapped("throwing"))) {
		...
		weapon = new("/clone/weapon/lianzi");
		weapon->move(ob);
		if (!i) weapon->wield();
		i = 1;
	}
