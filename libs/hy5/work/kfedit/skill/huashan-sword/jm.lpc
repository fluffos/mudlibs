...
if ( (int)me->query_skill("huashan-sword", 1) >200)
{
	...
	me->add_temp("apply/damage", -300);	
        ...
        if (random(me->query_skill("dodge")) > target->query_skill("dodge") / 3 || random(3)==0)
        {                
        ...
        if (!target)
{
 ...
 return notify_fail("敌人已经不在了。\n");
        ...
        if (me->query("combat_exp")> target->query("combat_exp")/3  || random(3)==0)
        {
                me->start_busy(1);
                target->start_busy(1);
                me->delete_temp("perform");
                damage = (int)me->query_skill("huashan-sword", 1)+200;
