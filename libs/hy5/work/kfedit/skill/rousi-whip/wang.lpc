...
CYN"$N手中$W"CYN"挥动，驱柔丝乐鞭散发出根根"RED"细丝"CYN"，要将$n裹起来！\n"NOR,
...
HIW"结果柔丝乐鞭散发出的细丝越积越多，似是积成了一团团丝棉，将$n紧紧裹了起来！\n"NOR,
HIW"结果，柔丝乐鞭似是积成了一团团丝棉，将$n紧紧裹了起来！\n"NOR,
HIW"$n越斗越是害怕，一个疏忽，被柔丝乐鞭紧紧裹了起来！\n"NOR,
});
...
//              string msg;
        ...
        int cost = me->query_skill("rousi-whip", 1)/2;
        if( !target ) target = offensive_target(me);

        if( !target
        ||      !target->is_character()
        ||      !me->is_fighting(target) )
                return notify_fail("天罗地网只能对战斗中的对手使用。\n");
        ...
        if (me->query_skill_mapped("whip") != "rousi-whip")
        ...
        message_vision(CYN"\n$N默运柔丝鞭法,但见手中一根根银丝从"+weapon->name()+CYN"慢慢飘起,在空中着一圈圈卷着，
...
int check_fight(object me, object target, object weapon)
{
        ...
        if(wep != weapon){
           ...
           me->start_busy(1);
           if(target->is_busy() 
              && random(me->query_skill("rousi-whip", 1)) < (int)target->query_skill("dodge", 1)/2){
              target->start_busy(2);
              me->delete_temp("YSJL/chan");
              return 1;
              }
       ...
       if (userp(me) && (me->query_skill_mapped("whip") != "rousi-whip" || me->query_skill_mapped("parry") != "rousi-whip")){
           message_vision(HIY"$N突然变招，划破"+HIW"「天罗地网」"+HIY"，使出和柔丝乐鞭完全不同的招数来！\n"NOR,me);
       ...
       if( me->query("force") <= me->query_skill("rousi-whip", 1)/2 ){
...
int checking(object me, object target, object weapon)
{
        ...
        cost = 20 * target->query_temp("YSJL/chan"+me->query("id")) + me->query_skill("rousi-whip", 1)/2;
        busy_time = (int)me->query_skill("rousi-whip", 1)/60;
        ap = me->query("combat_exp")/1000 * me->query_skill("rousi-whip",1) * me->query_int();
        dp = target->query("combat_exp")/1000 * target->query_skill("dodge",1) * target->query_int();
        if (ap < 1) ap = 1;
        if (dp < 1) dp = 1;
        me->add("neili", -cost);
if (busy_time> 6) busy_time=6;
        if(random(ap) > dp/2){          
