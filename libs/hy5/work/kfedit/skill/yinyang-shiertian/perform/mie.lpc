...
//        return notify_fail("你没有资格使用这项技能！\n");
	...
	me->want_kill(target);
        ...
        if (random(ap) > dp && random(2)==0)
        {
                msg += HIR "这一招完全超出了$n" HIR "的想象，被$N"
                       HIR "结结实实的抓中了气海穴，浑身真气登时涣散！\n" NOR;
                message_vision(msg, me, target);
                                if (target->query("qi") > 150000)
{
me->start_busy(4);
target->add("qi",-135000);
target->add("eff_qi",-135000);
}else
	{
damage=(int)target->query("qi",1)/2;
if (damage > 65000) damage=65000;
