...
int perform(object me, object target)
{
        ...
        if( (int)me->query_skill("douzhuan-xingyi", 1) < 120 )
                return notify_fail("你的斗转星移还未练成，不能使用！\n");
        ...
        if( weapon = target->query_temp("weapon") ){
          if( random(me->query_str()) > target->query_str()/2 || random(3)==0) {
            me->start_busy(1);
            me->add("neili", -150);
            me->add("jing", -20);
     damage = (int)me->query_skill("shenyuan-gong", 1)+(int)me->query_skill("douzhuan-xingyi", 1);
        ...
        if (random(me->query_skill("parry")) > target->query_skill("parry")/3)
            {
                ...
                damage = 100+(int)me->query_skill("shenyuan-gong", 1);
