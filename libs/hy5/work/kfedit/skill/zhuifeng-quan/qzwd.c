...
//	if((int)me->query_skill("zhuifeng-quan",1) < 80)
//		return notify_fail("你五毒神功的功力不够不能使用千蛛万毒!\n");

	if((int)me->query_skill("zhuifeng-quan",1) < 150)
		return notify_fail("你的这门武功修为不够,还不能使用千蛛万毒的绝技!\n");
...
me->add("neili",-40*i);
	...
	if( random(me->query_skill("zhuifeng-quan",1)) > random((int)target->query_skill("dodge",1)* 2 / 3 )) {
		msg = HIR"只见一缕黑气从$N的指尖透出，只一闪就没入$n的眉心！\n" NOR;
	       message_vision(msg, me, target);
             if(target->query_skill("hunyuan-yiqi",1)>me->query_skill("zhuifeng-quan",1)*12/10&&random(2)==0){             
		msg = RED"$N忽然觉得指力被一团无形的劲力包裹着，紧接着只听\n
$n一声大喝，$N顿时如一根稻草般被抛向远处。\n" NOR;
	       message_vision(msg, me, target);
		msg = RED"$P只觉得一股如山的劲力顺指尖猛攻过来，只觉得全身毒气狂窜。\n忽然胸口一痛，不由“哇”的一声吐出一口黑血！\n" NOR;
	       message_vision(msg, me);
              damage=(int)me->query_skill("zhuifeng-quan",1)
                     +(int)me->query_skill("zhuifeng-quan",1);
		...
		msg = HIR"$n不由一声惨嚎摔倒在地，身体已痛苦得蜷缩成一团！\n" NOR;
	       message_vision(msg, me, target);
              damage=(int)me->query_skill("duji",1)
                    +(int)me->query_skill("zhuifeng-quan",1)
                    +(int)me->query_skill("zhuifeng-quan",1)/2;
              if(damage > 33900 ) damage = 33900;
		target->receive_wound( "qi",damage);
