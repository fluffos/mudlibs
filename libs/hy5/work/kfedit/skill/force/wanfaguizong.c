...
//		return notify_fail("你的杂学太多，无法对本门武功贯通使用！\n");
	...
	if( (int)me->query("neili") < me->query("max_neili")/10+200 ) 
		return notify_fail("你的内力不够。\n");
...
int check_fight(object me)
{
	if (!me || !(int)me->query_temp("hslj/powerup")) return 0;
            ...
            && me->query_skill_mapped("force") != "jiuyin-zhengong")
        {
			...
			//if (me->query("eff_qi") > me->query("max_qi")*3) me->set("eff_qi", me->query("max_qi"));
			//if (me->query("qi") > me->query("eff_qi")*3) me->set("qi", me->query("eff_qi"));
                        }
        }
