...
int perform(object me, object target)
{
    ...
    if(ap*2/3+random(ap/3)>dp/2+random(dp/2))
    {
		...
		target->receive_damage("qi",damage*(2+random(3)),me);
		me->receive_damage("neili",cost*2,me);
		...
		if(ap*2/3+random(ap/3)>dp/2+random(dp/2))
		{
			...
			target->receive_wound("qi",100+damage,me);
			me->receive_damage("neili",100+cost,me);
			message_vision(msg,me,target);
			COMBAT_D->report_status(target);
