...
if( !target ) target = offensive_target(me);
    ...
    if( (string)me->query_skill_mapped("force")!= "jinghai-force")
     return notify_fail("必须靖海神功才行。\n");
      ...
      if( (int)me->query_skill("jinghai-force", 1) < 60 )
        ...
        if((int)me->query_skill("dragonfight",1)<370
        ||(int)me->query_skill("jinghai-force",1)<370) 
 ...
 if(random(3)==0 || (random(me->query("combat_exp",1)) >= target->query("combat_exp")/3))
        {
                damage=100+damage_bonus/2+random(damage_bonus/2);
                if(wizardp(me))tell_object(me, sprintf(GRN "damage: %d\n" NOR, damage));
                wound=damage;
                msg +=HIR "$n躲闪不及，被劲风带倒在地。 \n";
 ...
 if(random(3)==0 || (random(me->query("combat_exp",1)) >= target->query("combat_exp")/3))
        {
                damage_bonus=(damage_bonus/40000+random(300))/8;
                damage=200+damage_bonus+random(damage_bonus);
                
                if (damage > 36000) damage=36000;
                if(wizardp(me))tell_object(me, sprintf(GRN "damage: %d\n" NOR, damage));
                wound=damage;
                msg +=HIR  "$n被$N一掌震飞一丈多远。 \n";
                target->receive_damage("qi", damage,me);
                if(wound<=0) wound=random(100);
                if(wound>0) target->receive_wound("qi", wound);
                msg+=COMBAT_D->damage_msg(damage, "内伤");
        ...
        {
                damage=300+damage_bonus/2+random(damage_bonus/2);
                if(wizardp(me))tell_object(me, sprintf(GRN "damage: %d\n" NOR, damage));
                wound=damage-(int)target->query_temp("apply/armor")/2;
                msg +=HIR  "$n空手功夫不行，显然吃了大亏。 \n";
                target->receive_damage("qi", damage,me);
                if(wound<=0) wound=random(100);
                if(wound>0) target->receive_wound("qi", wound);
                msg+=COMBAT_D->damage_msg(damage, "内伤");
