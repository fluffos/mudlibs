...
//		return notify_fail("你的杂学太多，无法对本门武功贯通使用！\n");
	...
	if (random(me->query("combat_exp")) > target->query("combat_exp")/3 || random(3)==0){
		message_vision(HIR"结果$n躲闪不及被$N攻了正着，" + weapon->name() + HIR"上的"HIC"电劲"HIR"乘机钻入$n体内。\n"NOR,me,target);
              p = 2+random(4);
	target->receive_damage("qi",(int)me->query_skill("bahuang-gong",1)*8);
	target->receive_wound("qi",(int)me->query_skill("bahuang-gong",1)*8);
...
target->receive_wound("qi",(int)me->query_skill("bahuang-gong",1));
...
//  give bonus to NPC kill
    ...
    if( random((int)me->query("combat_exp"))> (int)target->query("combat_exp")/4 || random(3)==0) {
        msg += RED"\n\n$N"RED"用不可置信的眼光看着自己心口的血洞，身子一晃，倒在地上。 \n" NOR;
	target->receive_damage("qi",(int)me->query_skill("tianyu-qijian",1)*8);
	target->receive_wound("qi",(int)me->query_skill("tianyu-qijian",1)*8);
    }
