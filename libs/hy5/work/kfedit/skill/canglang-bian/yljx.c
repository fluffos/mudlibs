...
int perform(object me, object target)
{
    ...
    skill = me->query_skill("canglang-bian",1);   
        ...
        if (me->query_skill("bibo-shengong", 1) < 250)  
                return notify_fail("你的碧波神功修为不够，无法施出「云龙九现」！\n");  
    ...
    celepower = (int)me->query_skill("whip",1); 
    
    exp_bonus = (me->query("combat_exp",1)-3000000)/6000000*celepower;
    ...
    enemyexp = target->query("combat_exp",1);
...
damage2=damage1+ (int)me->query_skill("whip",1)+10; 
//                              total= damage1+total;
//                              message_vision(HIW"\t\tdamage is "+(string)(total)+" .\n"NOR,me);

if (userp(target)) damage1=damage1/2;
if (target->query("qi")>=damage1)
{
                                target->receive_damage("qi", damage1);
                                target->receive_wound("qi",damage1);
}
if (target->query("jing")>=damage1)
{

                                target->receive_damage("jing", random(damage1/2));
                                target->receive_wound("jing",1 + random(celepower/2));
}
