        ...
        if( random(me->query("combat_exp")) > (int)target->query("combat_exp")/4 || random(3)==0) 
{
	...
	target->receive_damage("qi",(int)me->query_skill("shenlong-xinfa",1)*8);
	target->receive_wound("qi",(int)me->query_skill("shenlong-xinfa",1)*8);
...
if ((random(me->query("combat_exp"))+extra*800) >(int)target->query("combat_exp")/3 || random(3)==0)
{
	target->receive_damage("qi",(int)me->query_skill("shenlong-xinfa",1)*8);
	target->receive_wound("qi",(int)me->query_skill("shenlong-xinfa",1)*8);
