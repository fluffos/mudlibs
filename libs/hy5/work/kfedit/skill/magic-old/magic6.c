...
//		damage = (31 - spi)*(spells/4)*(random(31-kar)+1)+100;
		...
		if(damage > 19500) damage = 19500;
		if(damage != 0) {
			message_vision(HIB "$N"HIB"的全身都被毁灭之光笼罩了！\n" NOR, obj);
  		            obj->receive_damage("qi", obj->query("eff_qi")/4,me);
               		    obj->receive_damage("jing", obj->query("eff_jing")/6,me);
                            obj->receive_wound("qi", obj->query("eff_qi")/4,me);               		    
//                            obj->start_busy(2);
...
me->receive_heal("qi", random(4000)+4000);
me->receive_heal("jing", random(4000)+4000);
}
