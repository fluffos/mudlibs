...
int perform(object me, object target)
{
	...
	if( (int)me->query_skill("wushen-jian", 1) < 250 )
		return notify_fail("你的逍遥五神剑法不够娴熟，不会使用「神龙剑决」。\n");
	...
	extra = me->query_skill("wushen-jian",1) / 15;
	extra += me->query_skill("wushen-jian",1) /15;
	...
	{
		damage = me->query_skill("sword")+me->query_skill("wushen-jian",1);
...
me->delete_temp("perform_wushenjian/qian");
	...
	me->add_temp("apply/attack", -100);
	me->add_temp("apply/damage", -1000);
if ((int)me->query_skill("wushen-jian", 1)> 350)
{
       wname = weapon->name();

        damage = (int)me->query_skill("wushen-jian", 1) / 2;
        damage += random(damage / 3)+(int)me->query_skill("wushen-jian", 1);

        ap = me->query_skill("sword");
        dp = target->query_skill("dodge");
        msg = HIG "$N" HIG "施出逍遥五神剑法「清流剑」绝技，手中" + wname +
