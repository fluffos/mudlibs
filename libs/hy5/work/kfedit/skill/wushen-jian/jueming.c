...
weapon = me->query_temp("weapon"); 
	...
	if( (int)me->query("neili") < me->query("max_neili")/7+200 ) 
       ...
       if( userp(me) && !wizardp(me) &&(int)me->query_skill("yunlong-jian", 1) < 500 )
		return notify_fail("你的本门外功不够!不能贯通使用！\n");

       if( userp(me) && !wizardp(me) &&(int)me->query_skill("yunlong-zhua", 1) < 500 )
		return notify_fail("你的本门外功不够!不能贯通使用！\n");

	if(userp(me) && !wizardp(me) && (int)me->query_skill("yunlong-shengong", 1) < 500 )
		return notify_fail("你的本门内功不够高!不能贯通使用！\n");

//	if(!wizardp(me) && (int)me->query_skill("jiuyin-zhengong", 1) )
//		return notify_fail("你的杂学太多，无法对本门武功贯通使用！\n");

        if (userp(me) && !wizardp(me) && !me->query("guard/flag"))
		return notify_fail("你的江湖经验还不够，无法对本门武功贯通使用！\n");

        if (userp(me) && !wizardp(me) && !me->query("guard/ok"))
		return notify_fail("你还没有通过华山论剑! 无法对本门武功贯通使用！\n");

        if ( userp(me) && !wizardp(me) && me->query_skill_mapped("force") != "yunlong-shengong")
