...
int perform(object me, object target)
{
        ...
        {
                i = me->query_skill("bibo-sword", 1) / 4;
                j = me->query_skill("sword", 1) / 4;
        }
        else 
        {
                i = me->query_skill("bibo-sword", 1) / 5;
        ...
        if (! target || ! me->is_fighting(target))
                return notify_fail("碧波剑法的「海潮汹涌」只能在战斗中对对手使用。\n");

        if (! weapon || weapon->query("skill_type") != "sword")
                return notify_fail("你必须使用武器才能使用「海潮汹涌」！\n");

        if ((int)me->query_skill("bibo-sword", 1) < 130)
                return notify_fail("你的碧波剑法还不够娴熟，使不出「海潮汹涌」绝技。\n");
        ...
        if (me->query_skill_mapped("sword") != "bibo-sword" && userp(me))
                return notify_fail("你现在无法使用「海潮汹涌」！\n");

        if (me->query_skill_mapped("parry") != "bibo-sword" && userp(me))
                return notify_fail("你现在无法使用「海潮汹涌」！\n");
        ...
        if (ap / 2 + random(ap) > dp)
        {
                ...
                if (present(target, environment(me)) && me->is_fighting(target))
                        ...
                        COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 1);
                        COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 1);
                if (me->query_skill("bibo-sword", 1) > 180)
                        call_out("remove_effect", 1, me, target);
                        
                if ((int)me->query_skill("bibo-sword", 1) > 150
...
void check_fight(object me,object target,object weapon, int p)
{
        ...
        if( ! me 
         ...
         || ! present(target,environment(me))
         || me->query_skill_mapped("sword") != "bibo-sword"){
                target->add_temp("apply/attack", p/2);
                target->add_temp("apply/defense",p/2);
