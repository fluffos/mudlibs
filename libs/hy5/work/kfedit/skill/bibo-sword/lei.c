...
int perform(object me, object target)
{
        ...
        */
        ...
        j = me->query_skill("bibo-sword", 1);
        z = me->query_skill("bibo-sword", 1);
       ...
       if (!weapon || weapon->query("skill_type") != "sword")
        ...
        if (me->query_skill_mapped("parry") != "bibo-sword" && userp(me))
                return notify_fail("你的基本招架必须是碧波剑法时才能使出「迅雷」！\n");
/*        || me->query_skill_mapped("sword") != "bibo-sword"
               || me->query_skill_mapped("parry") != "bibo-sword")
                return notify_fail("你必须在使用剑时才能使出「迅雷」！\n");
*/

        if(me->query_skill("bibo-sword", 1) < 160 )
                return notify_fail("你的碧波剑法还不够娴熟，使不出「迅雷」。\n");
