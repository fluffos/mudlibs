...
int perform(object me, string arg)
{
    ...
    if(!weapon || weapon->query("skill_type") != "sword"
      || me->query_skill_mapped("sword") != "yunv-jian"
        ...
        return notify_fail("你要把玉女剑做为剑法才能用「双剑合璧」。\n");

    if(me->query_skill_mapped("force") != "yunv-xinfa" )
        return notify_fail("你要把玉女心法做为内功才能用「双剑合璧」。\n");
    if(me->query_skill_mapped("force") != "yunv-xinfa" )
        return notify_fail("你要把玉女心法做为内功才能用「双剑合璧」。\n");
...
if (me->query("gender")=="女性" && target->query("gender")=="女性")
    ...
    if(!weapon1 || weapon1->query("skill_type") != "sword"
      || target->query_skill_mapped("sword") != "yunv-jian"
        ...
        return notify_fail(target->name()+"要把玉女剑做为剑法才能用「双剑合璧」。\n");
...
message_combatd(HIC "$N"HIC"和$n" HIC "两人眼神一交，心意已"
                        ...
                        me, target);
        //target->start_busy(5);
        message_vision(HIG"$N使出"HIW"玉女剑法!"HIG" 左边刺一剑！\n" NOR,me,target);
        COMBAT_D->do_attack(me, victim, me->query_temp("weapon"));
        message_vision(HIC"$n使出"HIR"全真剑法!"HIC" 右边刺一剑！！\n" NOR,me,target);
        COMBAT_D->do_attack(target, victim, target->query_temp("weapon"));
        message_vision(HIG"$N使出"HIW"玉女剑法!"HIG" 上边刺一剑！\n" NOR,me,target);
        COMBAT_D->do_attack(me, victim, me->query_temp("weapon"));
        message_vision(HIC"$n使出"HIR"全真剑法!"HIC" 下边刺一剑！！\n" NOR,me,target);
        COMBAT_D->do_attack(target, victim, target->query_temp("weapon"));
        message_vision(HIG"$N使出"HIW"玉女剑法!"HIG" 高处劈出一剑！\n" NOR,me,target);
        COMBAT_D->do_attack(me, victim, me->query_temp("weapon"));
...
target->add("eff_qi", target->query("qi")/3);
}
}
    ...
    if(me->query_temp("gumu_hebi")) 
{
   ...
   me->add_temp("apply/attack", -100);
   if (target) target->add_temp("apply/attack", -100);
