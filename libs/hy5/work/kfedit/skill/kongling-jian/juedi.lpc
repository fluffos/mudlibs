...
int perform(object me, object target)
{
    ...
    if( (int)me->query_skill("kongling-jian",1)<180)
    return notify_fail("你的空灵剑法不够娴熟，不会使用「绝地逢生」。\n");
     if( (int)me->query_skill("kongdong-xinfa",1)<80)
        return notify_fail("你的本门内功不够深厚，不会使用「绝地逢生」。\n"); 
                        
    if( (int)me->query("neili",1)<500)
    return notify_fail("你现在内力太弱，不能使用「绝地逢生」。\n");
 if (random(2)==0) target->start_busy(2);
    if( (int)me->query_skill("kongdong-xinfa", 1) > 220 )
    {
 ...
 msg=HIY "$N衣衫无风自动，使出崆峒派空灵剑法中的震山绝技「绝地逢生」。\n"
        ...
        if (random(me->query("neili")) > target->query("neili")/3 )
{
    ...
    damage= (int)me->query_skill("kongling-jian",1);
