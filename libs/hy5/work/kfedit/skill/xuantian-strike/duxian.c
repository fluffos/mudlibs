...
if (random(3)==0) target->start_busy(3);
	...
	msg =  HIC  "^^^^^^^^^^^天山雪崩！^^^^^^^^^^^ " NOR;
	 ...
	 COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
        //me->add("neili", -150);
        if (random(me->query("combat_exp")) >target->query("combat_exp")/3 || random(2)==0)
        {
       	 msg =  HIR  "「三笑逍遥散」" NOR;
	 COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
        message_vision(HIR"$N突然怪异的一笑，$n尽然跟着笑了笑!\n"NOR,me,target);
            target->receive_damage("qi", p*8,me);  
            target->receive_wound("qi", p*8,me);  
