...
int cut_abandon_jl(string job_kind)
{
	...
	switch (job_kind)
	{
        ...
        if (this_player()->query("family/gongji"))
                this_player()->add("family/gongji", -cut_cc);
        if (this_player()->query("family/gongji") < 0)
                this_player()->delete("family/gongji");
...
//        tell_object(me,HIG"你的潜能上升了" + atoi(count_sub(me->query("potential"), old_pot)) < 0  ? 0 :
//                atoi(count_sub(me->query("potential"), old_pot)) + "点。\n"NOR);
