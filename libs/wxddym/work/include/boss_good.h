...
int do_join()
{
        ...
        if (this_player()->query("bunch_bad") || me->query("shen") < 0)
        {
...
*/
        if (time() - (int)me->query("last_leave_bunch") < 120)
                return notify_fail(ob->name() + "看着你说：听说你是刚脱离了别的帮会过来的吧？我得再观察观察你，过两分钟再来吧！\n");
...
int do_disengage()
{
        ...
        if (me->query("bunch/bunch_name") != bunch_good["bunch_name"])
        {
        ...
        if (me->query("bunch/boss") != bunch_good["boss"])
                return notify_fail("想脱离我"+bunch_good["bunch_name"]+"？找你的介绍人" + me->query("bunch/boss") + "去吧。\n");
...
int do_choose(string bh)
{
       ...
       //如果是黑道的，就赶走
        if (this_player()->query("bunch_bad"))
        {
                message_vision("$N上上下下的仔细打量着$n，脸色越来越难看。\n", obj, me);
                command("say " + "我们" + bunch_good["bunch_name"] + "都是正义之士，岂能容得了你这种邪恶之徒？");
