       ...
       if (bad_bunch(me))
       {
               if (me->query("bunch/bunch_name") == bunch_bad["bunch_name"])
               {
                       if (me->query("shen") > 0)
                       {
                               ...
                               msg = "听说" + me->name(1) + "被" + obj->name(1) + "逐出了" + bunch_bad["bunch_name"]+"。";
                               CHANNEL_D->do_channel(this_object(), "rumor", msg);
                               return notify_fail("你被" + obj->name() + "开除出了" + bunch_bad["bunch_name"] + "。\n");
                       }
                       return notify_fail("你已经是我" + me->query("bunch/bunch_name") + "的成员了，昏头了？\n");
               }
               return notify_fail("你现在是" + me->query("bunch/bunch_name") + "的成员，要加入我" + bunch_bad["bunch_name"] + "，得先脱离(disengage)原来的帮会。\n");
       }

       if (this_player()->query("bunch_good") || me->query("shen") > 0)
       {
...
*/
       if (time() - (int)me->query("last_leave_bunch") < 120)
              return notify_fail(obj->name() + "看着你说：听说你是刚脱离了别的帮会过来的吧？我得再观察观察你，过两分钟再来吧！\n");
       ...
       tell_object(me,"恭喜你成为" + bunch_bad["bunch_name"] + "的成员！\n");
       msg = "听说" + me->name(1) + "拜" + obj->name(1) + "为老大，加入了" + bunch_bad["bunch_name"]+"。";
       CHANNEL_D->do_channel(this_object(), "rumor", msg);
       return 1;
}
...
int do_disengage()
{
       ...
       if (!this_player()->query("bunch_bad") || me->query("bunch/bunch_name") != bunch_bad["bunch_name"])
       {
               command("yi " + me->query("id"));
               command("say 这位" + RANK_D->query_respect(me) + "，你并非我" + bunch_bad["bunch_name"] + "的成员啊！");
               return notify_fail("看来" + obj->name() + "给你搞的满头雾水。\n");
       }

       if (me->query("bunch/boss") != bunch_bad["boss"])
               return notify_fail("想脱离我"+bunch_bad["bunch_name"]+"？找你的介绍人" + me->query("bunch/boss") + "去吧。\n");
       ...
       {
               message_vision("$N对着$n勃然大怒：好你个兔崽子，没事偏偏要去冒充什么正人君子！\n",obj,me);
               tell_object(me,"你被" + obj->name() + "开除出了" + bunch_bad["bunch_name"] + "。\n");
               msg = "听说" + me->name(1) + "被" + obj->name(1) + "逐出了" + bunch_bad["bunch_name"]+"。";
               CHANNEL_D->do_channel(this_object(), "rumor", msg);
       } else
       {
               message_vision("$N对着$n叹气道：早就知道你不会在这种小地方混一辈子的，也罢，你自己保重！\n",obj,me);
               tell_object(me,"你决定脱离" + bunch_bad["bunch_name"] + "。\n");
       }
       return 1;
}
...
int do_choose(string bh)
{
       ...
       if (me->query("shen") > 0)
       {
               ...
               if (me->query("bunch/boss") == bunch_bad["boss"])
               {
                       message_vision("$N对着$n怒喝道：留你在我" + bunch_bad["bunch_name"] + "岂非叫人笑话！\n",obj,me);
...
int do_giveup()
{
       ...
       //如果是白道的，就赶走
       if (this_player()->query("bunch_good"))
       {
               message_vision("$N上上下下的仔细打量着$n，脸色越来越难看。\n",obj,me);
               command("say " + "你这种自命正派人士的到我这里来干什么？想找茬子？");
               return notify_fail("看来" + obj->name() + "并不欢迎你！\n");
...
int accept_object(object me,object ob)
{
       ...
       bonus = ([ "exp" : exp,
                  ...
                  "score" : score,
                  "weiwang" : weiwang,
                  "prompt": "在巡"+bunch_bad["place_name"]+"的过程中，经过锻炼"
               ]);
