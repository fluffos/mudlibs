...
inherit F_SSERVER;
string query_name() { return "威慑"ZJBR"刀"; }
int perform(object me, object target)
{
        ...
        if (! target || ! me->is_fighting(target))
                return notify_fail("「威慑刀」只能对战斗中的对手使用。\n");
        ...
        if ((level = me->query_skill("tiandaodao", 1)) < 100)
                return notify_fail("你的天道刀不够娴熟，不会使用「威慑刀」。\n");

        if (me->query("neili") < 100)
                return notify_fail("你现在真气不够，无法使用「威慑刀」。\n");

        msg = HIM "$N" HIM "使出天道刀「威慑刀」，默运口诀，将手中"
