...
void del_protect(object me)
{
        ...
        me->delete_temp("protect_xuanwu");

        me->add_temp("reduce_damage", -90);
...
void heart_beat()
{
        ...
        // 如果主人下线则自动保存后离开
        if( !(env1=environment()) || !objectp(owner = find_player(ob->query("owner" ))) ) {
                save();
        ...
        if( living(ob) && (owner->query_temp("comedby") == ob || ob->query("actions" )) &&
            env1 != env2 && env1 != owner && !env2->query("no_magic") ) {
                if( is_fighting() ) remove_all_enemy(1);
                ob->move(env2);
        ...
        // 麒麟--治疗
        if( (owner->query("max_qi" ) > owner->query("qi") || owner->query("max_qi") > owner->query("eff_qi"))
            && query("race_type") == "麒麟" ) {
                if( random(15) < query("level")/10 ) {
                        tell_object(owner, HIY + ob->name() + HIY "全身闪出火红般的光芒照向你，你感觉气血有所恢复。\n" NOR);
                        owner->add("qi", 500 + random(query("magic/power"))*200);
                        owner->add("eff_qi", 200 + random(query("magic/power"))*80);
                        if( owner->query("eff_qi" ) > owner->query("max_qi" ) ) owner->set("eff_qi", owner->query("max_qi" ) );
                        if( owner->query("qi" ) > owner->query("max_qi" ) ) owner->set("qi", owner->query("max_qi" ) );
                }
        }
        ...
        if( query("race_type") == "青龙" && owner->is_fighting() ) {
                ...
                if( objectp(target) && random(15) < query("level")/10 && !query_temp("hiding_kuihua", owner) ) {
                        ...
                        owner->set_temp("hiding_kuihua", 1 );
        ...
        if( query("race_type") == "玄武" && owner->is_fighting() ) {
                if( random(15) < query("level")/10 && !owner->query_temp("protect_xuanwu" ) ) {
                        tell_object(owner, HIM + ob->name() + HIM "发出一道耀眼的光芒，将你全身笼罩在内，你感觉到全身表面有层龟壳般保护。\n" NOR);
                        owner->add_temp("reduce_damage", 90 );
                        owner->set_temp("protect_xuanwu", 1 );
                        remove_call_out("del_protect");
                        owner->call_out("del_protect", 3 + random(5) );
                }
        }
        }
...
int do_modify(string arg)
{
        ...
        if( item == "actions" ) {
                ...
                set("actions", msg);
                me->set_temp("comedby", this_object());
                write("ＯＫ\n");
                save();
                return 1;
        }
        ...
        if( idx == "combat_exp" && para ) {
                if( para > query("combat_exp") ) {
                        ...
                        if( lvl > level ) {
                                ...
                                add("max_jingli", 1000*j);
                                add("max_neili", 2000*j);
                                add("max_jing", 500*j);
                                add("max_qi", 300*j);
                                add("int", j);
                                add("str", j);
                ...
                if( env )
                {
                        ...
                        if( interactive(env = environment()) )
                        {
                                ...
                                tell_object(env, HIM + name() +
                                        HIM "忽然离你而去了！\n" NOR);
                                // 让所有的人也下马
                                if (objectp(follower = this_object()->query_temp("is_rided_follow")))
                                {
...
varargs void die(object killer)
{
        ...
        owner->set("warcraft/status", "died");
...
mixed hit_ob(object me, object ob, int damage_bouns)
{
        ...
        ob->receive_wound("qi", 200 + me->query("magic/power"));
