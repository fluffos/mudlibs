...
以金庸小说背景为世界观的武侠 MUD（游戏内明确要求玩家不要使用小说中人
物的本名），玩家社区自称"情义盟"。角色拥有膂力、悟性、根骨、身法四
项天赋，创建时可选择其一或全部随机产生。新手历程完整可玩：从客店、
西大街到扬州武馆的引导流程，中央广场的切磋战斗，死亡后经历鬼门关与
白无常的五段对话，再到武庙复活重生。地图上的昼夜变化是真实生效的游
戏机制——入夜后裸方向词（如"north"）会失效并提示"天色太黑了"，须
改用"go <方向>"，这是有意设计而非缺陷。虽然与本项目里的
`hy2000`/`hy2002`/`hy5` 同属"海洋"命名，但比对 master 文件哈希与
`securd.lpc`/`feature/skill.lpc` 后并未发现血统关联，是各自独立开发
的同名之作。
