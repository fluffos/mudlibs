...
{
        return HIW "【 三界传说 】"HIG"人"HIR"·"HIY"神"HIR"·"HIB"鬼"HIW" 三界尊神"NOR;
}
...
void reset_me (object me)
{
	...
	for(i=0; i<sizeof(inv); i++)
	{
        ...
        me->set_name(get_honor_str(), ({ "numberone","zun shen",  "zunshen" }) );
...
int accept_fight(object ob)
{
	...
	{
                command ("say 本尊神怎可与自己较量！\n");
                return notify_fail("你已经是三界尊神了。\n");
	}
	if (me->is_fighting())
	{
		command ("say 请稍候！\n");
                return notify_fail("尊神正在忙着，无法顾及你。\n");
	}
	if (! interactive(ob))
	{
		command ("say 不！\n");
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 ) {
		...
		find(id);
		zhang->move(ob);
                command ("chat 从现在开始"+ob->query("name")+"就是我们新的三界尊神了！");
                message_vision("尊神给了$N一根神杖。\n",ob);
		if( !ob->query("master/rank") )
			ob->add("score", 200);
		else
			ob->add("score", 5);
                ob->set("master/rank",HIG"人"HIR"·"HIY"神"HIR"·"HIB"鬼"HIW" 三界尊神"NOR);
