...
int do_leave()
{
	...
	message_vision(HIW"\n$N深叹了一口气："+RANK_D->query_self(me)+"技不如人，情愿认输！\n\n"NOR, me);
		...
		log_file("leitai",sprintf("%s(%s) 于擂台上击败 %s(%s)。", me->name(1), getuid(me), ob->name(1), getuid(ob)));
		do_back(me);
	        do_back(ob);
	        }
	else do_back(me);
...
void observe(object ob1,object ob2)
{
	...
	if(!ob1 && !ob2) return;
		...
		log_file("leitai",sprintf("%s(%s) 于擂台上击败 %s(%s)。", ob2->name(1), getuid(ob2), ob1->name(1), getuid(ob1)));
		do_back(ob1);
		do_back(ob2);
		this_object()->reset();
		return;
		...
		log_file("leitai",sprintf("%s(%s) 于擂台上击败 %s(%s)。", ob1->name(1), getuid(ob1), ob2->name(1), getuid(ob2)));
		do_back(ob1);
		do_back(ob2);
		this_object()->reset();
		return;
