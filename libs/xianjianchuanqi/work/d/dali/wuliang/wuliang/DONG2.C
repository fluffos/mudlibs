...
int do_ketou(string arg)
{
	...
	if( arg=="yuxiang" ) {
        ...
        if (!me->query("dali/meet_dy1")) me->add_temp("ketou_trigger", 1);
          }
             if( me->query_temp("ketou_trigger")>15&&me->query_temp("ketou_trigger")<18 ) {
		message_vision("$N看见蒲团上的薄草早已破裂，不由伸手进去，里面什么也没有，好象已经被取走了。\n", 
			me);
	 me->delete_temp("lbwb1");
        if (!me->query("dali/meet_dy1")) me->set_temp("duanyu/find1",1);
        me->add("ketou_trigger", 1);
            }
	        return 1;
