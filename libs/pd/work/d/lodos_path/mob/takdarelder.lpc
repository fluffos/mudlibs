...
void create() {
 ::create();
   ...
   set_id(({"takdar elder", "tak'dar elder", "tak'dar", "takdar", "elder"}));
   ...
   set_body_type("human");
 ran = random(100);
    if (ran < 20) set_level(19); else
    if (ran < 40) set_level(20); else
    if (ran < 60) set_level(21); else
    if (ran < 80) set_level(22); else
    if (ran < 101) set_level(23);
}
