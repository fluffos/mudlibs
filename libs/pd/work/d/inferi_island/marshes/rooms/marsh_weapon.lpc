...
void create() {
    ...
    set_long(
      ...
      "%^BOLD%^%^BLACK%^Welcome to the Marsh Weaponry! This is a place for passerby's to sell their goods for a price, and to buy them for an even bigger price. The walls are riddled with goods, but they are not for selling. The shopkeeper is keeping busy by cleaning shelves and cleaning the floor, even though it is not doing much help; this area has obviously been abandoned for quite some time now. Zeon, the shopkeeper, has a list posted on the wall for all to read.%^RESET%^");
