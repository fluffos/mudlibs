...
void create() {
    ...
    set_short("%^BOLD%^%^BLACK%^Evil %^RED%^P%^YELLOW%^y%^RED%^r%^YELLOW%^o%^RED%^m%^YELLOW%^a%^RED%^n"
      "%^YELLOW%^c%^RED%^e%^YELLOW%^r%^RESET%^");
    set_long("This man was sent on the journey to Roston with the drows "
      "just in case they failed to take over the lands of Roston, he would "
      "be able to destroy the town with a lift of his finger. He is well "
      ...
      "and lonely man, now he does the work of armies in a matter of seconds "
      "some believe its because of the power the helmet he wears, is unbelievable. "); 
    ...
    new(OBJ "drink")->move(this_object());
    new(OBJ "drink")->move(this_object()); 
    new(OBJ "drink")->move(this_object());
    new(OBJ "drink")->move(this_object());
    new(OBJ "drink")->move(this_object());
