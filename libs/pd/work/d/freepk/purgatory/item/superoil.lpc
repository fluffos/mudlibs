...
// customized to fit monkey's paws and into superoil by Wraith
#include <std.h>
#include <locations.h>
...
int do_oil(object ob)
{
 ...
 string *st;
  if (!ob) return 0;
 ob->add_hp(125 + random(230) );
 ob->add_sp(57 + random(57) + random(57) + random(57));
 st = ob->query_limbs();
 i = sizeof(st);
 while(i--)
 ob->heal_limb(st[i], random(29)+29 );
 if(_FFirst)
