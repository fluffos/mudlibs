...
void create() {
    ::create();
     set_name("Upper part of the northern gatehouse");
     set_short("Upper part of the northern gatehouse");
...
"%^BOLD%^%^BLACK%^into the window.");
 ...
 set_exits( (["down" : ROOMS"gatehousen"]) );

    set_items( ([
        "controls"  : "%^BOLD%^%^WHITE%^A large wheel is set on a hige woodem block that has all sorts of ropes coming out of it. These are the controls for the gates.",
        ...
        "window" : "Staring out of the window, there is a large castle off in the distance."
      ]) );
