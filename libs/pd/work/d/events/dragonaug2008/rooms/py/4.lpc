...
int throw_vine_at_branch(string str) {
  ...
  if((vineOb = get_vine(this_player())) != present(vineStr, this_player())) {
    vineOb = present(vineStr, this_player());
    if(vineOb && file_name(vineOb)[0..sizeof(DRAG_ITEMS"vine")-1] == DRAG_ITEMS"vine")
      write("You try to throw the vine, but it merely blows about in the wind.\nYou pull the vine back in.");
    else
