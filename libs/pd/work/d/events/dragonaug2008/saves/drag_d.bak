...
void set_data_no_save(mixed key, mixed value) {
...
void init_data() {
  ...
  set_data_no_save("canProveCourage", 1);
  load_object(DRAG_ROOMS"instance_d")->
    destruct_instances_with_instance_id("Event Test Of Courage Instance");

  set_data_no_save("denyEntranceToPillarRoomViaDoor", 1);
  DOOR_D->set_closed("Ogre King Event's Door", 1);

  set_data_no_save("cankillgemogre", 1);
  set_data_no_save("canloadgemogre", 1);
  set_data_no_save("canchargegem", 1);
  set_data_no_save("cangathergems", 1);
  set_data_no_save("canloadcrystaldragon", 1);

  set_data_no_save("canfindportalpiece", 1);
  set_data_no_save("canfindogredagger", 1);

  set_data_no_save("speechguidedragon", -1);
  set_data_no_save("speechdragonelder", -1);
  set_data_no_save("speechdragonmage", -1);
  set_data_no_save("speechfinaldragon", -1);

  set_data_no_save("prisonbeams", 8);

  set_data_no_save("denyOldCaveEntrance", 1);
  // this is just a reminder to turn on later, eventstarted || allowdragonstofly is checked
  set_data_no_save("allowdragonstofly", 0);
}
...
void ogre_king_event_start() {
  myData["canKillOgreKing"] = 1;
  myData["denyOgreKingThroneEntrance"] = 0;
  myData["OgKiPlayers"] = ({ });
  load_object(DRAG_ROOMS"instance_d")->destruct_instances_with_instance_id("Ogre King Throne Instance");
}
