...
object atk;
     ...
     if(random(100) < 20) {write("The %^BOLD%^dr%^GREEN%^ago%^RESET%^%^GREEN%^nsfu%^BOLD%^%^WHITE%^ry%^RESET%^ pulsates with energy and strikes hard.");;
...
result=(random(50))+((this_player()->query_level())*4);;
return result;;
}
}
int query_auto_load() {
