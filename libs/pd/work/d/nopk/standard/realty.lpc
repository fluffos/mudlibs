...
void create() {
  ::create();
   ...
   set("short", "Carnage Realty");
   set("long",
        "Welcome to Carnage Realty!\n"+
...
void reset() {
  ::reset();
   if(present("pacix")) return;
    new(MOB"pacix")->move(this_object());
}
...
*/
	default:
	    notify_fail("%^CYAN%^Pacix says: %^RESET%^You must consult Inferno.\n");
	    return 1;
    }
...
int read(string str) {
	...
	"--------------------------------------------------------------------\n"+
"house: a new simple one room house                      100000 gold\n"+
"\n"+
