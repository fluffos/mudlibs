...
int throw(string str) {
   ...
   if(!this_player()->kill_ob(ob)) {
      notify_fail("You cannot attack them yet.");
      return 0;
   }
   tell_room(environment(this_player()), this_player()->query_cap_name()+
     " throws a huge boulder at "+ob->query_cap_name()+"!", ({ this_player(), ob }));
   tell_object(ob, this_player()->query_cap_name()+" throws a huge boulder "
     "at you!");
   write("You throw a huge boulder at "+ob->query_cap_name()+"!");
   ob->damage(70);
   if (this_object()->query_wielded()) this_player()->force_me("unwield boulder");
   this_object()->remove();
   return 1;
}
