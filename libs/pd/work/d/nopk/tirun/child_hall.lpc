// Keep this hall -- it allows those who have not picked a class
// to be able to use cost
#include <std.h>
#include <tirun.h>
#include <config.h>
#include <council.h>
...
void create() {
    ...
    set("short", "The Child Hall By default");
    set_long("Welcome to the child hall. If you are not an immortal, "
       "you really shouldn't be here... If you are mortal, please post "
       "here, inform inferno and never come back again. You can't do "
       "jack crap in here, because you're not supposed to even be here. "
       "Go away, or pay the price, you bloody newbie.");
    set_smell("default", "It smells like newbie!");
    set_exits( 
	      (["up" : ROOMS"square",
		"east" : ROOMS"child_vote"]));
    ob = new("std/bboard");
    ob->set_name("board");
    ob->set_id( ({ "board", "child board" }) );
    ob->set_board_id("child_board");
    ob->set_max_posts(50);
    ob->set_edit_ok("inferno");
    ob->set_location(ROOMS"child_hall");
    ob->set("short", "%^RESET%^%^GREEN%^The adventurers board");
    ob->set("long", "A small, tack board.");
}
