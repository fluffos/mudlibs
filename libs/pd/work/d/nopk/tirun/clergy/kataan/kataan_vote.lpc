...
void create() {
  ::create();
    set("short", "Kataan voting room");
    set_property("light", 2);
    set_property("indoors", 1);
    if(VOTING_D->election_day()) {
      set("long",
          "This is the voting room of the Kataan Hall.  Kataans come "
          "here during election time to vote for their leaders.  It "
          "is currently election time, so a list of all candidates "
          "is posted on the wall.");
    }
    else {
      set("long",
          "Kataans come here to the voting room of the Kataan Hall "
          "during election time to elect their leaders.  The next "
          "elections will are to be held for a period of four days "
          "beginning: "+NEXT_ELECTION+".");
    }
    set_items(
        (["list" : "During election time, it lists all candidates."]) );
    set_exits( 
              (["west" : ROOMS "kataan_hall"]) );
}
void init() {
  ::init();
  if(wizardp(this_player())) return;
  if((int)this_player()->query_level() == 1) return;
