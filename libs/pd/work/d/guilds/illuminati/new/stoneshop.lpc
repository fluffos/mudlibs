...
::create();
  ...
  set_long("%^RESET%^%^ORANGE%^This strange room has all sorts of odd figurines placed all around on shelves. "
  ...
  set_exits(([
     "northeast" : ROOMS"illuminati/new/floor2",
  ]));
}
void reset() {
   ::reset();
   if(!present("rand"))
   new(ROOMS"illuminati/new/rand")->move(this_object());
