...
void create() {
    ...
    set_long("%^BOLD%^%^WHITE%^As you enter the room you notice you are in the "
      ...
      "variety of scattered objects; such as, %^YELLOW%^b%^WHITE%^a%^YELLOW%^l"
      "ls %^WHITE%^o%^YELLOW%^f l%^WHITE%^i%^YELLOW%^ght%^WHITE%^, %^BLACK%^"
      "orbs of darkness%^WHITE%^, bags and %^RESET%^%^ORANGE%^lem%^WHITE%^bas "
      "br%^ORANGE%^ead %^BOLD%^%^WHITE%^put in their areas for keeping; not many "
      "things are in this room as it is for the Elvin %^YELLOW%^Goddess %^WHITE%^"
      "to rest her head.  However, the simplicity of this room adds a distinct "
      "%^GREEN%^ch%^WHITE%^a%^GREEN%^rm %^WHITE%^and %^CYAN%^h%^WHITE%^a%^CYAN%^"
      "rm%^WHITE%^o%^CYAN%^ny%^WHITE%^.");
    set_items( (["ceiling" : "%^BOLD%^%^WHITE%^Looking up you notice that"
      ...
      " eat it." ]) );
set_exits( (["down" : "/d/guilds/illuminati/rooms/mageh",
"main" : "/d/guilds/illuminati/rooms/mainroom"]) );
}
