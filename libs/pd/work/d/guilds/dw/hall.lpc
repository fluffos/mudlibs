...
void create() {
    ...
    board->set_edit_ok(({"daboura", "nightshade", "stormbringer", "whit", "nesoo", "ironman", "cortoth" }));
    board->set_max_posts(100);
    board->set_board_name("Dragon Warrior");
}

int fbuy(string str) {
    return notify_fail("This item can no longer be purchased.");
...
int fread(string str) {
    ...
    write("The sign reads: \n"
      "-----------------------\n"
      "dragon eggs can no longer be purchased.");
    return 1;
}
