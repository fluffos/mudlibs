...
void create() {
    set_name("pole");
    set_id( ({ "wooden fishing pole", "fishing pole", "wooden pole", "pole" }) );
    set_curr_value("copper", 15 + random(5));
    set_weight(8+ random(1));
    broken = 0;
    bait = 0;
}
...
string query_short() {
    if(!broken) {
        if(!bait) return "A wooden fishing pole.";
        else return "A wooden fishing pole (baited).";
    }
    else return "A wooden fishing cold (broken)";
}
...
int bait_it(string str) {
    ...
    if(pole != "pole" && pole != "hook") return 0;
    ...
    if(!bait) {
        bait_item = present(the_bait, this_player());
