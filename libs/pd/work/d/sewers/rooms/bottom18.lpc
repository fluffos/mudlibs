...
void create() {
    ...
    set_long("%^RESET%^%^RED%^A stifiling heat emanates from the pit.\n%^RESET%^"
      ...
      " of the pit, meeting the awaiting angels.  There are a set of stairs "
      "leading down into the pit, although only the first step or two is there.");
    set_exits( (["east" : ROOMS+"bottom9"
	,"west" : ROOMS+"bottom10"
      ]) );
	...
	,"stairs" : "It may have decended into the pit, but now only one or two steps remain."
      ]) );
}
