...
void create() {
    ::create();
    set_name("Jack, keeper of Ships");
    set_short("Jack, sitting on a chair");
    set_long("This is the ship sellers brother.  Unlike John, which only "
      "repairs and sells ships, Jack allows you to upgrade your ship.  Just "
      "<upgrade list> to see a list of availible upgrades, as well as "
      "<upgrade [option]> to upgrade an option.");
    set_id(({ "jack", "keeper", "ship keeper" }));
    set_race("human");
    set_gender("male");
...
int view(string s) {
    ...
    if(!ob->query_owner()) ob->set_owner(this_player()->query_name());
    ob->load();
    write("Your ship has "+(ob->query_max_hp() - ob->query_hp())+" out of "+ob->query_max_hp()+" his points.  ("+to_int(percent(ob->query_hp(), ob->query_max_hp()))+"% Damaged)");
    return 1;
}
