...
-----------------------------------------
 ...
 [ Orb ]  ( 20 gold pieces )
     A magic orb that will help regenerate magic.
     You can <buy '#' orb> to buy a specific number of orbs.
 [ Scroll ] ( 30 gold pieces )
      Enchanted with the Arcane magics of the sorcerers,
      this scroll can be used to add magical armour.
...
int fBuy(string str) {
 ...
 case "acid":
    ...
    if(numba) {
      ...
      say("Shengar hands "+this_player()->query_cap_name()+" "+(string)numba+"
acid orbs.");
      while(numba--)
...
blue flames.");
    ...
    say("Shengar hands "+this_player()->query_cap_name()+" a ball of blue
fire.");
...
break;
 ...
 case "scrolls":
    ...
    if(numba) {
      ...
      this_player()->add_money("gold", -30*numba);
      write("Shengar hands you "+(string)numba+" scrolls.");
say("Shengar hands "+this_player()->query_cap_name() +" "+(string)numba+" scrolls.");
while (numba--)
new("/d/guilds/forsaken/obj/scroll")->move(this_player());
      return 1;
    }
    this_player()->add_money("gold", -30);
write("Shengar hands you a scroll.");
say("Shengar hands "+this_player()->query_cap_name()+" a scroll.");
new("/d/guilds/forsaken/obj/scroll")->move(this_player());
    return 1;
    break;
