...
varargs void caught_theft(string type, int amt) {
  ...
  if (file_name(environment(to))[0..9] == "/d/freepk/")
    return;
...
int query_steal_exp(object thief, object victim) {
  ...
  if (victim->is_player())
    ...
    exp = thief->query_modified_exp(thief, victim)/5;
...
varargs void move_player(mixed dest, string msg) {
    ...
    if (horse = query_riding()) {
      if (userp(query_riding())) {
        ...
        ridingmsg = userp(horse) ? horse->query_cap_name() : a_or_an(horse->query_name())+" "+horse->query_name();
...
void restart_heart() {
    ...
    if(net_died_here && (find_object(net_died_here) || load_object(net_died_here))) {
...
varargs nomask void die(mixed killer) {
    ...
    if (atck) {
      if (userp(to) && userp(atck)) {
        ...
        else if (!strsrch(file_name(env), "/d/freepk/") ||
