...
int query_steal_exp(object thief, object victim) {
  int exp = 0;
  ...
  if (victim->is_player())
    ...
    exp = ADVANCE_D->query_exp_for_kill(thief, victim)/5;
...
int quit(string str) {
	...
	CHAT_D->do_raw_chat("system", "Logout <system> "+msg);
    }
    remove();
    return 1;
}
...
//	CHAT_D->do_raw_chat("driver", "Enter <driver> "+msg);
  	msg = capitalize((string)query_name())+" from "+query_ip()+"";
  	CHAT_D->do_raw_chat("system", "Login <system> "+msg);
...
void heart_beat() {
  ...
  if (player_age > autosave && !autosave_off)  {
    if (!in_input()) message("info", "Autosaving.", this_object());
    save_player(query_name());
...
void net_dead() {
    ...
    save_player(this_object()->query_name());
    if (!this_object()->query_invis()) {
	CHAT_D->do_buddylist_line(this_object()->query_name(), capitalize(this_object()->query_name())+" has gone link-dead.");
	CHAT_D->do_enemylist_line(this_object()->query_name(), capitalize(this_object()->query_name())+" has gone link-dead.");
    }
    this_object()->set_rest_type(0);
    if (!this_object()->query_invis()) {
	msg = capitalize((string)query_name())+" from "+query_ip()+"";
	CHAT_D->do_raw_chat("system", "Linkdead <system> "+msg);

    } 
    if (!this_object()->query_invis() && environment(this_object())) {
...
varargs nomask void die(mixed killer) {
    ...
    if (objectp(killer))
      atck = killer;
    else if (killer = to->query_current_attacker())
      atck = killer;
    else if (objectp(killer = to->query_last_damage()) && killer->is_living() && (time() - to->query_last_damage_time()) <= 2)
      atck = killer;
    else if (arrayp(tmp_arr = to->query_property("last damage")) && objectp(killer = tmp_arr[0]) && intp(tmp_arr[1]) && time() - tmp_arr[1] <= 2)
      atck = killer;

    if (!objectp(atck)) atck = 0;

    to->dismount_all(to->query_cap_name()+" falls from your back.", to->query_cap_name()+" bucks you off.");
  ...
  */
    ...
    // if you don't start in tirun, you can get stuck with no way to pray
    // changed recall command to take ghosts back to monastery
    //to->set_primary_start("/d/nopk/tirun/square.c");
    ...
    if (atck) {
      if (userp(to) && userp(atck)) {
        ...
        else if (env->query_zone() == "freepk" ||
            (myrace == "archangel" && hisrace == "demon") ||
            (myrace == "demon" && hisrace == "archangel") ||
            myrace == "vampire")
              pk = 0;
        else if (sizeof(tmp_arr) == 3 && !strsrch(""+tmp_arr[2], "/d/freepk/"))
          pk = 0;
        else
          pk = 1;
      }
...
void write_messages() {
  ...
  */
    ...
    if (tmp != 100) message("login", "\n       >>> %^YELLOW%^Experience point bonus active: "+tmp+"%!%^RESET%^ <<<", this_object() );
    if( !message ) return;
...
int id(string str) {
  ...
  return member_array(str, query_id()) != -1 || ::id(str);
}
string query_short() {
  object to = this_object();
  string name, tmp;
  ...
  if (!interactive(to))
    ...
    if (objectp(tmpob = query_riding())) {
      if (userp(tmpob))
        ...
        tmp = a_or_an(tmpob->query_name())+" "+tmpob->query_name();
      post += " (riding "+tmp+")";
    }
    if ("/d/paintball/start"->query_paintball() && tmpob = present("gun", to)) {
      tmp = tmpob->query_team();
      post += " %^"+upper_case(tmp)+"%^[ "+capitalize(tmp)+" ] %^RESET%^";
    }
  }
