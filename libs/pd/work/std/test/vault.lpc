...
int GoThroughDoor() {
    ...
    if(!doors) return 1;
    verb = query_verb();
    if(member_array(verb, this_object()->query_exits()) == -1)
        return 0;
    ind = keys(doors);
...
// query_lock added by ::Inferno(9/28/00);
string query_lock(string str) {
   if (!doors[str]["key"]) return 0;
   return doors[str]["key"];
}
