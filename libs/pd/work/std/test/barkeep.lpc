...
int buy(string str) {
    ...
    if(!str) {
	message("info", "%^CYAN%^"+query_cap_name()+" says:%^RESET%^ What is it you would like?", environment(this_object()));
	return 1;
    }
    str = lower_case(str);
    ...
    if (!this_object()->no_pay())
    if(!(cost = price(this_player(), menu[str]["strength"]))) {
	write(query_cap_name()+" says: You do not have enough "+currency+" for that!");
	say(query_cap_name()+" turns away "+this_player()->query_cap_name()+" for lack of money.", this_player());
	return 1;
    }
    write("You pay "+cost+" "+currency+" coins for a "+str+".");
    say(query_cap_name()+" collects some "+currency +" from "+this_player()->query_cap_name()+".", this_player());
    ...
    ob->set_weight(menu[str]["strength"] / 10);
    if (ob->query_weight() < 1) ob->set_weight(1);
    i = menu[str]["strength"];
   ...
   ob->set_destroy();
	...
	say(this_player()->query_cap_name()+" drops a "+str+".", this_player());
