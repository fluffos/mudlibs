...
varargs int fly(string s, object p) {
    int *c = ({ coords[0], coords[1], coords[2] }), sp;
    string op;
    object o;
    if(!s || s == "") s=query_verb();
    ...
    o=SKY_D->get_room(c);
    if(!o) fail("You can't fly that way.\n");
    ...
    this_player()->move(o);
    if(this_player()->query_mounted()) this_player()->query_mounted()->move(o);
...
int land(string s) {
    string sym=SKY_D->get_xy(coords), room;
...
string get_map() {
    ...
    if(!c) return "Your not in the sky.";
    if(c[2] > 0) alt = c[2]*2;
    c[0]=c[0]+4+(alt/2);
    c[1]=c[1]-2-(alt/2);
    for(i=0; i<(5+alt); i++) {
