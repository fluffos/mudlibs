...
create() {
::create();
set_id(({"dagger", "ivory dagger", "knife", "bloodstone", "bloodstone
dagger"}));
set_name("dagger");
...
if(random(100) > 85) {
write("%^BOLD%^%^RED%^The %^BLACK%^bl%^RED%^oo%^YELLOW%^d%^RED%^sto%^BLACK%^ne%^RED%^ glistens as you stab "+this_player()->query_current_attacker()->query_cap_name()+" deep in the chest!%^RESET%^");
