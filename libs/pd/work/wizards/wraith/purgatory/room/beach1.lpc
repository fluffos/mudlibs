...
void create() {
    ::create();
     ...
     set_long("The %^BOLD%^%^BLACK%^black%^RESET%^ sand stretches to the north and " 
"south, animal tracks lay scattered all around. Some of the smaller white "
"rocks can clearly be made out now, they look like headstones! The path to the "
"west leads into a strange looking graveyard.");
...
void reset() {
 ::reset();
  if (!present("crab", this_object()))
    new(MOBS"crab" )->move(this_object());
  if (!present("crab 2", this_object()))
    new(MOBS"crab" )->move(this_object());
}
