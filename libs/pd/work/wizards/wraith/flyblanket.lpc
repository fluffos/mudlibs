...
::create();
    ...
    set_id(({"fly-blanket", "armour", "flies"}));
    set_ac(12);            
    set_mass(25);
    set_type("body armor");  
    set_wear( (: extra_worn :) );
...
int extra_worn() {
    if((int)this_player()->query_level() < 35) {
        ...
        return notify_fail("You are not big enough to gather %^ORANGE%^f%^BOLD%^%^BLACK%^l%^RESET%^%^ORANGE%^i%^BOLD%^%^BLACK%^e%^RESET%^%^ORANGE%^s%^RESET%^!");
