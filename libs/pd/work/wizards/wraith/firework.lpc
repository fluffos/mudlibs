...
int throw(string str) {
   ...
   tell_room(environment(this_player()), this_player()->query_cap_name()+" throws a %^BOLD%^%^RED%^f%^BOLD%^%^BLACK%^i%^RED%^r%^BLACK%^e%^RED%^w%^BLACK%^o%^RED%^r%^BLACK%^k%^RESET%^ at "+ob->query_cap_name()+"!", ({ this_player() }));
...
void hit(object ob, object tp) {
   ...
   tell_room(environment(ob), "%^BOLD%^%^CYAN%^A %^BOLD%^%^RED%^f%^BOLD%^%^BLACK%^i%^RED%^r%^BLACK%^e%^RED%^w%^BLACK%^o%^RED%^r%^BLACK%^k%^RESET%^%^BOLD%^%^CYAN%^ flys in and hits "+ob->query_cap_name()+" %^BOLD%^%^CYAN%^engulfing "+ob->query_cap_name()+" in an %^BOLD%^%^YELLOW%^exp%^WHITE%^lo%^YELLOW%^s%^RED%^ion%^RESET%^%^BOLD%^%^GREEN%^!", ({ ob }));
   this_object()->remove();
}
void init() {
 ::init();
