...
void create() {
 ::create();
   ...
   set_id(({"%^RESET%^%^RED%^D%^BOLD%^%^BLACK%^e%^RESET%^%^RED%^a%^BOLD%^%^BLACK%^t%^RESET%^%^RED%^h"}));
...
new("/wizards/wraith/room/mobs/Death/scythe")->move(this_object());
force_me("wield scythe");

new("/wizards/wraith/room/mobs/Death/dcloak")->move(this_object());
force_me("wear cloak");
}
int scythe() {
    object ob;
