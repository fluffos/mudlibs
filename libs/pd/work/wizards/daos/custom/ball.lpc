...
int show_commands(string str) {
	...
	break;
    case 5:
	write("%^BOLD%^YELLOW%^This will make the Phoenix follow me. (Carina).%^RESET%^");
	break;
    case 6:
...
int pull_phoenix_feather(string str) {
	...
	call_out("shoot_firework", 60);
	return 1;
    }
}

void shoot_firework() {
    message("info", "%^BOLD%^%^WHITE%^Carina pulls a %^RED%^bright red feather %^WHITE%^from the %^RESET%^%^MAGENTA%^back %^WHITE%^of the %^YELLOW%^Ph%^MAGENTA%^oe%^RED%^ni%^RESET%^%^MAGENTA%^x %^BOLD%^%^WHITE%^causing it to shoot off a %^RED%^Fi%^WHITE%^rewo%^RED%^rk%^WHITE%^.", environment(this_player()), ({this_player()}) );
...
int stroke_phoenix(string str) {
	...
	call_out("shoot_second_firework", 60);
	return 1;
    }
}

void shoot_second_firework() {
    message("info", "%^BOLD%^%^WHITE%^Carina strokes the %^YELLOW%^head %^WHITE%^of the %^YELLOW%^Ph%^MAGENTA%^oe%^RED%^ni%^RESET%^%^MAGENTA%^x %^BOLD%^%^WHITE%^causing it to shoot off a %^YELLOW%^Fi%^WHITE%^rewo%^YELLOW%^rk%^WHITE%^.", environment(this_player()), ({this_player()}));
...
int warn_phoenix(string str) {
    if (str == "phoenix") {
        ...
        this_player()->set_mp(0);
	write("%^BOLD%^%^WHITE%^You %^RESET%^%^RED%^warn %^BOLD%^%^WHITE%^the %^YELLOW%^Ph%^MAGENTA%^oe%^RED%^ni%^RESET%^%^MAGENTA%^x%^BOLD%^%^WHITE%^ of %^RED%^danger%^WHITE%^ causing it to shoot off a %^BLACK%^fi%^RED%^rewo%^BLACK%^rk%^WHITE%^.");
	call_out("shoot_third_firework", 60);
	return 1;
    }
}

void shoot_third_firework() {
    message("info", "%^BOLD%^%^WHITE%^Carina %^RESET%^%^RED%^warns %^BOLD%^%^WHITE%^the %^YELLOW%^Ph%^MAGENTA%^oe%^RED%^ni%^RESET%^%^MAGENTA%^x%^BOLD%^%^WHITE%^ of %^RED%^danger%^WHITE%^ causing it to shoot off a %^BLACK%^fi%^RED%^rewo%^BLACK%^rk%^WHITE%^.", environment(this_player()), ({this_player()}) );
    message("info", "%^BOLD%^%^BLACK%^A %^RESET%^%^GREEN%^green mark %^BOLD%^%^BLACK%^of a sk%^WHITE%^u%^BLACK%^ll, with a %^RESET%^%^GREEN%^s%^BOLD%^%^GREEN%^e%^RESET%^%^GREEN%^rp%^BOLD%^%^GREEN%^e%^RESET%^%^GREEN%^nt %^BOLD%^%^BLACK%^protruding from the mouth, is shot into the %^CYAN%^sky%^BLACK%^ as a %^RED%^warning%^BLACK%^.", users());
}  
...
int follow_allow_phoenix(string str) {
	...
	following = 1;
	tp = this_player();
	message("info", "%^BOLD%^%^GREEN%^Fawkes %^YELLOW%^fl%^WHITE%^ie%^YELLOW%^s %^WHITE%^onto Carina's back and sings %^CYAN%^h%^WHITE%^a%^CYAN%^rm%^WHITE%^o%^CYAN%^n%^WHITE%^iou%^CYAN%^sly%^WHITE%^.", environment(tp));
