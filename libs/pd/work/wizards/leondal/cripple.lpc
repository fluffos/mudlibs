...
int cmd_cripple(string str) {
  ...
  weps = filter(tp->query_wielded(),
    (: member_array($1->query_type(), ({ "blunt", "knif
" }) ) != -1 :)
  );
  if (sizeof(weps)) {
  ...
  if (tgt->query_skill("perception") + tgt->query_stats("wisdom")/2 +
tgt->query_stats("intelligence")/2 >
  ...
  tp->set_paralyzed(2, "You are concentrating on crippling "+
                       tgt->query_cap_name()+"!");
...
void do_dmg(mixed *args) {
  ...
  string limb=args[3];
  object wep=args[4];
  object oenv=args[5];   //Compare original room with current - Leondal
  object env;
  string skill;
...
interactive(tgt))) {
  ...
  if (env != oenv) {     //Added by Leondal 09/2009
    message("info", "You moved and messed up the crippling process.",tp);
    return;
  }
  ...
  switch (skill) {
    case "melee":
    case "blunt":
      mymsg = "Your blow "+COL2+"smashes"+RES+" into
"+tgt->query_cap_name()+"'s "+
    ...
    break;
    case "knife":
      mymsg = "Your knife "+COL2+"rips"+RES+" through
"+tgt->query_cap_name()+"'s "+
              limb+"!";
