...
int cmd_force2(string str) {
    ...
    if(!(ob = find_living(who = lower_case(who)))) {
        ...
        if (!ob) {
        notify_fail("Could not find anything living called "+who+".\n");
        return 0;
      }
    }
    seteuid(UID_FORCE);
    ...
    seteuid(UID_LOG);
    log_file("force", geteuid(previous_object())+" "+(x ? "forced" : "tried to force")+" "+who+" to "+act+"\n"+ctime(time())+"\n");
    seteuid(getuid());
