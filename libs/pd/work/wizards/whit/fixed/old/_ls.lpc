...
varargs int do_ls(mixed path, object act_ob, int show_dots, string real_path)
{
   string *files;
   string *Files,*Dirs;
   int index,j,x,y,rows,cols,l,length,num,size,i, colors;
   string *wcfiles;
   string string_path;
...
// End code by Peter
   ...
   for (y = 0; y < rows; y++) {
	 ...
	 switch(size) {
	    case -2:
	      message("Nsystem","%^BLUE%^%^BOLD%^     " + files[index] + "%^RESET%^/", this_player());
	      tdirs++;
	      l++;
	      break;
	    case -1:
	      message("Nsystem","   - %^RED%^%^BOLD%^" + files[index]+"%^RESET%^", this_player());
	      break;
	    default:
		 ...
		 message("Nsystem", " ", this_player());
	      message("Nsystem", size + " "+colour+files[index]+"%^RESET%^", this_player());
	      break;
	   }
	 if (length - l > 0)
