...
mapping read_languages()
{
  ...
  langs = ([]);
  lines = read_database(LANGUAGES_DB);
  lnsz = sizeof(lines);
  tell_object(find_player("stranger"),"lang: "+lnsz+"\n");
  for(i = 0; i < lnsz; i++) {
    line = explode(lines[i], ":");
    langs[line[0]] = line[1];
    tell_object(find_player("stranger"),"lang: "+line[0]+" : "+line[1]+"\n");
  }
  return langs;
}
...
string *testit()
{
    return read_database(LANGUAGES_DB);
}
