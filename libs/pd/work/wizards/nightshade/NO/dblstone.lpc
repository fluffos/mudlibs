...
// Simple change to double strength for elites and guilds.
// Base cost should be over 100gp.
...
int rub(string str)
{
    if (environment(this_object())!=this_player())
        return 0;
    if (!str) return 0;
    if (!(member_array(str,this_object()->query_id())+1)) return notify_fail(capitalize(_Action)+" what?\n");
