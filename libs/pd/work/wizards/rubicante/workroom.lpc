...
room::create();
  ...
  "type 'h', this will display a list of commands for the built in editor."
   ...
   ({"wall", "the wall"}):
    ...
    ob = new(BBOARD);
    ob->set_name("rubicante");
    ob->set_location("/wizards/rubicante/workroom");
    ob->set_id( ({ "board", "rubicante board" }) );
    ob->set_board_id("rubicante_idea");
    ob->set_max_posts(99);
    ob->set_short("Rubicante's Board");
    ob->set_long("Rubicante's Board.");
