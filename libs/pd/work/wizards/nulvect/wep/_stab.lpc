...
int cmd_stab(string str) {
  ...
  if (dmg < 15) {
    message("combat","You stab at "+tgt->query_cap_name()+
                     " with a knife, but miss.", tp);

    message("combat",tp->query_cap_name()+" stabs at "+tgt->query_cap_name()+
                     " with a knife, but misses.", environment(tp), ({ tp, tgt }) );
    
    message("combat",tp->query_cap_name()+" stabs at you"
                     " with a knife, but misses.", tgt);
  }
  else {
    wep = weps[0]->query_name();
    message("combat","You "+COL+"stab"+RES+" "+tgt->query_cap_name()+" with "+wep+"!", tp);
    
    message("combat",tp->query_cap_name()+" "+COL+"stabs"+RES+" you with "+wep+"!", tgt);

    message("combat",tp->query_cap_name()+" "+COL+"stabs"+RES+" "+tgt->query_cap_name()+
