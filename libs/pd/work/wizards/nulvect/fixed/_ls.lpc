...
// End code by Peter
   ...
   for (y = 0; y < rows; y++) {
      for (x = 0; x < cols; x++) {
         ...
         switch(size) {
            case -2:
              message("system","     %^BLUE%^%^BOLD%^" + files[index] + "/", this_player());
              tdirs++;
              l++;
              break;
            case -1:
              message("system","   - %^RED%^%^BOLD%^" + files[index]+"", this_player());
              break;
            default:
              ...
              size /= 1024;
              if (size == 0) size = 1;
              if (size < 10) message("system", " ", this_player());
              if (size < 100) message("system", " ", this_player());
              if(find_object(path+files[index]))
                 message("system", "*", this_player());    /* Plura 930119  */
              else
                 message("system", " ", this_player());
              message("system", size + " "+colour+files[index]+"", this_player());
              break;
           }
         if (length - l > 0)
            message("system", spaces[1 .. (length - l)], this_player());
        }
      message("system", "\n", this_player());
   }
#if 0
