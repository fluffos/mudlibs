...
int cmd_mug(string str) {
  ...
  rank = 1+tp->query_level()/13;
  if (rank > 4) rank = 4;

  steal = BALANCE3_D->get_steal(tp, tgt, rank,
    ({ "stealing", "stealing", "stealing", "stealing", "murder" }),
    ({ "strength", "strength", "strength", "strength", "strength", "dexterity" }) );

  steal = steal*3/2;

  // they're already holding on to it...
  if (item && item->query_wielded())
    steal -= tgt->query_stats("strength")/2;

  if (item) steal -= item->query_weight();
  else steal -= 6;
...
int do_stealing(object tp, object tgt, object item) {
  ...
  tp->add_exp(tp->query_steal_exp(tp, tgt)*3/2);
  ...
  // money/purse follows
    ...
    int broke=1;
    foreach (string curr in HARD_CURRENCIES) {
      int m, r;
