...
#define MARK_PROP "mark_abil_info"
#define MARK_OB "/cmds/skills/obj/mark_ob"
// 10 minutes
#define MARK_TIME 60*10
...
int can_cast(object tp, string name) {
  ...
  if (sizeof(marks) && sizeof(marks[name]))
      FAIL("You have already marked something with that name.");

  if (name != "list" && sizeof(filter(marks, (:
      ( environment($2["ob"]) == environment(this_player()) ) :) )) )
        FAIL("You have already marked this area.");
...
int cmd_mark(string str) {
  ...
  marks = (mapping)tp->query(MARK_PROP) || ([]);

  if (name == "list") {
    if (!sizeof(marks))
      message("info", "You have not marked any trails.", tp);
    else {
      message("info", "You have marked these places:", tp);
  ...
  ob->set_id( ob->query_id() + ({"mark_ob", name}) );
  ob->set_owner(tp);
  ob->set_name(name);
  ...
  tp->set(MARK_PROP,
    marks + ([
      name : ([
        "time" : time()+to_int(MARK_TIME*perc),
        "ob" : ob,
      ])
    ])
  );
