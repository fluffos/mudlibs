...
//  if (str) tgt = present(str, env);
  ...
  if (caught)
    message("combat", COL+tp->query_cap_name()+" attempts to "
                      "sneak behind you unnoticed!"+RES, tgt);

  message("combat", COL+"You sneak around behind "+
...
void finish_backstab(mixed *args) {
  ...
  if (dmg >= tgt->query_max_hp())
    dmg = tgt->query_max_hp()*3/4 + random(dmg/5);
