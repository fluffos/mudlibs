...
void help() {
  message("help",
    ...
    "taxing and can only be done on occaison.",
    this_player() );
}
...
int can_cast(object tp, object tgt, object corpse) {
  ...
  // every 2 levels of maxed faith shaves off an hour on casting time
  if ((int)tp->query_property("resurrect_spell")+HOW_OFTEN-
      ((tp->query_skill("faith")-(50*6))/(2*6))*60*60 > time())
    FAIL("You pray to the gods.\n"
         "The gods refuse to answer.\n");
  
  if (member_array(tgt->query_race(), unressable) != -1)
    FAIL(capitalize(pluralize(tgt->query_race()))+" cannot be resurrected.\n");

  if (member_array(corpse->query_old_race(), unressable) != -1)
