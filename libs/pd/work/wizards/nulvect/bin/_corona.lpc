...
void help() {
  message("help",
    ...
    "the coronal edge of the elemental plane of fire. Once there, "
    "he must travel to the place that corresponds with the location on "
    "the physical plane where he wishes to arrive.\n\n"
...
int cmd_corona(string str) {
    ...
    if (str=="list")
    {
        if (!gat) { write("You have not marked any locations."); return 1; }
        i = sizeof(gat);
        key = keys(gat);
    ...
    if (!(gat[wh]) || (fail && !wizardp(this_player())))    
    {
        write("You do not know of such a place.");
        return 1;
    }
