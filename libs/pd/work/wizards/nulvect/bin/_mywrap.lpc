  ...
  switch (str) {
    case "on":
      ...
      wrap->misc = ({
        ...
        5,
        tp->query_skill("tail")/20 + random(2) + 1,
        ({ (tp->query_limbs() -
           ({ "left horn", "right horn", "left wing", "right wing",
              "tail", "head"
           }) )
        }),
      });
      ...
      foreach (string l in wrap->misc[4])
        tp->add_magic_protection( ({ l, wrap->misc[3], 0 }) );
