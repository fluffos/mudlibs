...
void return_to_death_site() {
  ...
  if (s = sizeof(RecentDeaths) && d=RecentDeaths[s-1] && d->room &&
...
varargs void die(mixed source) {
  ...
  (to->query_alignment() * -1) > random(6500)) {
    ...
    mymsg =
      ...
      "and is pulled into the Abyss.%^RESET%^"
    ;
    roommsg =
      "%^BOLD%^%^BLACK%^"+to->query_cap_name()+"'s body is engulfed in "
      "deep shadow and is pulled into the Abyss.%^RESET%^"
    ;
    allmsg =
      "%^MAGENTA%^An unearthly howl splits the air as "
      ""+to->query_cap_name()+" is pulled into the Abyss.%^RESET%^"
    ;
  ...
  message("death", allmsg, users(), env);
