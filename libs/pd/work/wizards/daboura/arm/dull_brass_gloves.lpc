...
void toggle_brass(object tp) {
  ...
  set_id(({ "gloves", "brass gloves", "dull brass gloves", DULL_BRASS_ID }));
  set_short("dull brass gloves");
  set_long("Gloves made of brass, they are very dull and old.");
  set_ac(1);
  
  if (state == "crystal") {
    message("info", COL+"Your crystal gloves transform into brass!"+RES, tp);
    message("info", COL+tp->query_cap_name()+"'s crystal gloves transform "
...
void toggle_crystal(object tp) {
  ...
  set_name("crystal gloves");
  set_short(COL+"crystal gloves"+RES);
  set_long("Gloves made of solid crystal, they reflect any light that shines on them.");
  set_id(({ "gloves", "crystal gloves", CRYSTAL_ID }));
  set_ac(5);

  if (state == "brass") {
    message("info", COL+"Your brass gloves transform into crystal!"+RES, tp);
    message("info", COL+tp->query_cap_name()+"'s brass gloves transform "
