...
int cmd_rootgrasp(string str) {
    object ob, tp;
    int time, rounds, time, i, which, skip, rand, bonus, intt;
	object *inv;
    string what;
    ...
    if(!living(ob))
        return return notify_fail("You spell fails.\n");
	...
	message("room", "%^GREEN%^Large %^ORANGE%^roots%^GREEN%^ sprout out from the ground an %^RED%^grab%^GREEN%^ "
		+ob->query_cap_name()+ "!"
		, environment(tp), ob);
