...
{
    ::init();
    add_action("map", "map"); // function, command
}
    ...
    if(sizeof(ply))
        {
            ...
            {
                //if(random(2) == 1) continue;   
                if(!sizeof(ply[i]->query_attackers())) continue;
                if(ply[i]->is_player())
                {
                    passive(ply[i]);  
                }
                ...
                {
                    if(ply[i]->query("behavior") == 3)
                    {
                        defensive(ply[i]);
                        continue;
                    }
                    if(ply[i]->query("behavior") == 2)
                    {
                        passive(ply[i]);
                        continue;
                    } 
                    if(ply[i]->query("behavior") == 1)
                    {
                        aggressive(ply[i]);
                        continue;
                    } 
                    if(ply[i]->query("aggressive") <= 10)
                        defensive(ply[i]);   
                    else if(ply[i]->query("aggressive") >= 35)
                        aggressive(ply[i]);
                    else
                        passive(ply[i]);
