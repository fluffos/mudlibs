...
void create()
{
 ...
 set_prevent_get("The box spins out of your reach.");
}
     ...
     while(i--)
      {
       ...
       if ((int)BBOARD_D->get_num_posts(tmp)-read > 0)
         {
         number = 1 + (int)BBOARD_D->get_new_post(tmp, (string)this_player()->query_name());
             msg = (mapping)BBOARD_D->get_message(tmp, number-1);
    if(!msg) {
        notify_fail("Invalid message number.\n");
        return 0;
    }
    lines = ({ "%^BOLD%^%^BLUE%^"+tmp +"%^GREEN%^ board post #"+number+"%^RESET%^ by "+capitalize(msg["owner"])+", subject: "+msg["title"] });
...
int __list(string tt)
{
 ...
 int read, unread, i;
if (tt) return 0;
write("%^BOLD%^%^WHITE%^Here are your current unread posts:"); 
 if (!i = sizeof(str = get_dir(BBOARD_DIR)))
 ...
 while(i--)
  {
   ...
   out = arrange_string(arrange_string(tmp, 17)+": %^BLUE%^%^BOLD%^["+"%^GREEN%^"+unread+"%^BLUE%^]%^RESET%^", 25); 
...
read =(int)BBOARD_D->get_num_read(tmp,(string)this_player()->query_name());
   ...
   out +=
arrange_string(arrange_string(tmp,17)+": %^BOLD%^%^BLUE%^[%^GREEN%^"+(BBOARD_D->get_num_posts(tmp)-read)+"%^BLUE%^]%^RESET%^",25);
    i--;
    }
    if ((i-2) > 0)
    {
...
read =(int)BBOARD_D->get_num_read(tmp,(string)this_player()->query_name());
   ...
   out +=
arrange_string(tmp,17)+": %^BLUE%^%^BOLD%^[%^GREEN%^"+(BBOARD_D->get_num_posts(tmp)-read)+"%^BLUE%^]%^RESET%^";
    i--;
    }
 write(out);
}
