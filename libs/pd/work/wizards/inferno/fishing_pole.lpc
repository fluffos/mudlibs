...
void create() {
    set_name("stick");
    set_id( ({ "stick", "fishing pole", "hook", "string", "pole" }) );
    set("value", 20);
    set("weight", 28);
    broken = 0;
    bait = 0;
...
string query_long(string str) {
    if(str == "hook") {
        ...
        return ("A lightweight aluminum championship fishing pole! I rule.");
    }
}
 
string query_short() {
    if(!broken) {
        if(!bait) return "A championship fishing pole";
        else return "A championship fishing pole (baited)";
    }
    else return "A championship fishing pole (ruined, you fool!)";
}
