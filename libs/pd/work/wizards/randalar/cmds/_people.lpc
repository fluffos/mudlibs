// Code credit is gven to all below.  I wanted the command, however, to
// show the room people are in.  As such, I've modified it slightly...
// --Randalar
// P.S. Found that the code exists, someone just commented it out. WTF?
...
#else
      if (sscanf(field, "/wizards/%s/%s", str2, str3) == 2)
...
#else
      line += sprintf("%-28s", field[0..34]);
#endif
#if 0 // Not too worried about this information --R
      xyzpdq = (string) list[i] -> query_race();
      field = ""+xyzpdq+"";
      if ( xyzpdq = 0 ) field = "-";
      if (!field) field = "-";
      line += sprintf("%-11s", field);
      xyzpdq = (string) list[i] -> query_subclass();
      field = ""+xyzpdq+"";
      xyzpdq = (string) list[i]->query_class();
...
check_invis(object ob)
{
   string *pos;
   pos = POSITIONS;
   if((int)ob->query_invis() && get_pos(this_player()) < get_pos(ob) && wizardp(ob))
     return 0;
   return 1;
}
int
...
{
   string p;
     return (wizardp(ob) && ((p = (string)ob->query_position()) == "Archon Avatar" ||
