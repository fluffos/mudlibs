...
void do_dmg(object ob, object tp) {
   ...
   while (i--) {
     if (!all[i]) continue;
     if (environment(all[i]) != environment(tp)) continue; 
     if (!all[i]){
  ...
  if (ob) {
   ...
   if (dmg < 30) {
     ...
     tell_object(ob, "Flame passes over your body, not harming you at all.");
   } else
   ob->damage(dmg, "holy");
  }
...
you can attack single targets with holyfire.
ENDTEXT);
}
int spell() {
   object o;
