...
int cmd_fly(string str) {
    ...
    if(environment(this_player())->query_property("no fly")) {
       notify_fail("The winds are to fearce.\n");
       return 0;
    }
    ...
    switch(str) {
        ...
        case "hall": if (this_player()->query_race()== "archangel") dest = "/wizards/inferno/angel/main"; else
                     if (this_player()->query_race()== "demon") dest = "/d/merkkerri/demonquest/dquest115"; else
                     dest = "/d/tirun/dragon_hall"; break;
        case "elven village": if(skill >= 20) dest = "/d/standard/rain_forest/square"; break;
        case "arlon": if(skill >= 25) dest = "/d/arlon/room/square"; break;
        ...
        case "merkkerri": if(skill >= 85) dest = "/d/merkkerri/jungle/merkkirri/mjungle1"; break;
    ...
    call_out("go_sky", 3, this_player());
    return 1;
}
int go_sky(string dest) {
        write("Seeing "+capatalize(dest)+" you swoop down to land!");
