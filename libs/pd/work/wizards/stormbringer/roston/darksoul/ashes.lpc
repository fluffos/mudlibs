...
void create()
{
 ...
 set_id( ({ "ashes", "black ashes", "ashcheck" }) );

    MANA = random(2500)+2500;
...
void init()
{
    ...
    ::init();
    if (owner)
	if (present("ashcheck", owner)==this_object()) 
	    set_heart_beat(1);
    add_action("rub", "rub");
}
...
void heart_beat()
{
    ...
    ob2 = present("ashcheck 2", owner);
    ob3 = present("ashcheck 3", owner);
...
void do_mana(int quickly)
{
    ...
    if (MANA <= 0)
    {
	message("info", "The ashes turn to dust.", owner); 
	this_object()->remove();
	return;
    }
    if (random(100) > 40 )
...
int move(mixed dest)
{
    ...
    if (present("ashcheck 2", owner) &&
      owner->is_living())
	present("ashcheck 2", owner )->set_heart();
    return ::move(dest);
}
