...
int becomeSubclass(string str)
{
if(str!="sorceror" && str!="wizard" && str!="pyromancer" && str!="necromancer") {
 ...
 write("%^BOLD%^You have chosen to follow the path of the %^RESET%^%^RED%^ "
   +str+"%^WHITE%^%^BOLD%^.");
 return 1;
}
int goSorceror()
...
int choose(string what) {
   ...
   if (what != "necromancer" && what != "pyromancer" && what != "wizard" && what != "sorceror") {
