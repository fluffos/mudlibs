...
#define LONG_DESC "%^BOLD%^%^WHITE%^The %^BLUE%^Belt of %^YELLOW%^C%^BLACK%^a%^YELLOW%^ndles %^WHITE%^is made from a soft wax that is abnormally strong but somehow flexible. There are many %^YELLOW%^golden stars%^WHITE%^, placed around the belt, that are used used to magically hold waxes.%^RESET%^"
...
void create() {
    ...
    set_limbs( ({ "torso" }) );
    set_ac(2);
    set_max_internal_encumbrance(40); // should allow 5 waxes; they weigh 4 each
