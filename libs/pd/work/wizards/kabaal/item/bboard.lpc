...
int remove_message(string str) {
 ...
 else if(x == ILLEGAL_ACCESS) write("You may only remove your own
posts.\n");
...
int read_message(string str) {
 ...
 if(!str)
 number = 1 + (int)BBOARD_D->get_new_post(board_id,
(string)this_player()->query_name());
 ...
 lines = ({ "Message "+number+" by "+capitalize(msg["owner"])+":
"+msg["title"] });
 lines += explode(msg["message"], "\n");
 this_player()->more(lines);
 BBOARD_D->set_read(board_id, (string)this_player()->query_name(),
number-1);
 return 1;
...
posts = (int)BBOARD_D->get_num_read(board_id,
(string)this_player()->query_name());
if (posts==-1)
 {
 tot = 0;
 ...
 seeker = ::query_short();
 act = (tot-posts);
 seeker += " %^RED%^< "+act+" %^BLUE%^Unread, %^RED%^"+tot+" %^BLUE%^Posts
>%^MAGENTA%^";
...
[#]>,\n";
 borg += "and <remove [#]>.\n";
 borg += "There are "+(int)BBOARD_D->get_num_posts(board_id)+" messages
posted.\n";
