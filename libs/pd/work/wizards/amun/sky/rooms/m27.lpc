...
void create() {
    ...
    set_exits( ([ "south" : SKYROOM "m19" ]) );
}
void reset() {
    ::reset();
    if (!present("gold fish", this_object()))
	new(SKYMOB "gold_fish")->move(this_object());
    if (!present("angel fish", this_object()))
	new(SKYMOB "angel_fish")->move(this_object());
}
