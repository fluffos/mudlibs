...
void create() {
    ...
    set("long","%^BOLD%^%^WHITE%^The clouds begin to shift slightly as the wind grows stronger. To the north, %^CYAN%^lights %^WHITE%^can be seen flickering under the small crack at the bottom of the door. %^GREEN%^Pl%^RESET%^%^ORANGE%^a%^BOLD%^%^GREEN%^nts %^WHITE%^magically grow through a tuft of clouds towards the south. Small sounds that sound like incantations can be heard wafting through the area.%^RESET%^");
    ...
    set_exits( ([ "north" : SKYROOM "clergy_hall2", "south" : SKYROOM "rs_3" ]) );
