...
void create() {
    ...
    set("long","%^RESET%^%^ORANGE%^Dimmer %^BOLD%^%^WHITE%^and %^RESET%^%^ORANGE%^dimmer %^BOLD%^%^WHITE%^the light becomes as the clouds grow more fluffy, they look as if they are beginning to form something. %^GREEN%^Green v%^RESET%^%^ORANGE%^i%^BOLD%^%^GREEN%^nes %^WHITE%^begin to reach up from under the clouds and wrap themselves around the forming clouds. The sound of singing and cheerful laughter can be heard coming from the southwest. %^YELLOW%^Light %^WHITE%^can be seen from the south.%^RESET%^");
    ...
    set_exits( ([ "north" : SKYROOM "lsup_6", "south" : SKYROOM "lsup_8" ]) );
}
