...
int stroke_otis(string str) {
	...
	write("%^BOLD%^%^BLACK%^You stroke the head of the T%^WHITE%^i%^BLACK%^g%^WHITE%^e%^BLACK%^r causing it to shoot off a %^YELLOW%^Fi%^WHITE%^rewo%^YELLOW%^rk%^WHITE%^.%^RESET%^");
...
void shoot_second_firework() {
    message("info", "%^BOLD%^%^BLACK%^Artemis strokes the head of the t%^WHITE%^i%^BLACK%^g%^WHITE%^e%^BLACK%^r, causing it to shoot off a %^YELLOW%^Fi%^WHITE%^rewo%^YELLOW%^rk%^WHITE%^.%^RESET%^", environment(this_player()), ({this_player()}));
