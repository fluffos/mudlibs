...
int enter_mist(string str) {
    object tp = this_player();
    if (tp->query_level() >= 50 && tp->query_level() <= 55 && present("fang", tp) && present("mirror", tp) && present("daywalker cloak", tp) && str && str == "mist") {
	if (!str || str != "mist") return notify_fail("Enter what?\n");
	write("%^BOLD%^%^BLACK%^You force your body forward through the mist and are pulled into a new area.%^RESET%^");
	this_player()->move_player(VR"haunted_BP_1");
	return 1;
    }
    return 0;
}
