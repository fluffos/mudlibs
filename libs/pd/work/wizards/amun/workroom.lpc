...
void create() {
    ...
    set_exits( ([ "tirun" : "/d/nopk/tirun/square", "lounge" : "/d/nopk/standard/adv_inner", "north" : "/wizards/amun/bedroom", "blus" : "/wizards/ironman/workroom" ]) );
    bb = new("std/bboard");
    bb->set_name("Board of Greatness");
    bb->set_id( ({ "board", "board of", "of greatness", "board of greatness", "greatness" }) );
    bb->set_max_posts(200);
    bb->set_edit_ok( ({ "amun" }) );
    bb->set_location("/wizards/amun/workroom");bb->set_short("%^BOLD%^%^WHITE%^B%^RESET%^o%^BOLD%^%^WHITE%^a%^RESET%^r%^BOLD%^%^WHITE%^d %^RESET%^o%^BOLD%^%^WHITE%^f %^RESET%^G%^BOLD%^%^WHITE%^r%^RESET%^e%^BOLD%^%^WHITE%^a%^RESET%^t%^BOLD%^%^WHITE%^n%^RESET%^e%^BOLD%^%^WHITE%^s%^RESET%^s");
