...
"\n\n%^BOLD%^%^RED%^!IMPORANT!%^RESET%^ - Please be sure to read %^BOLD%^%^CYAN%^<help rules>%^RESET%^ and %^BOLD%^%^CYAN%^<help name policy>%^RESET%^ before proceeding!");
    skip_obvious();
    set_items(
...
int pick(string str) {
    ...
    if (str == "dragon" || str == "demon" || str == "god" ||
      str == "lich" || str == "archangel" || str == "vampire") {
