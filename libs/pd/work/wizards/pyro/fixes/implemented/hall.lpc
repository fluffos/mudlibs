...
void advance_verify(string str) {
  str = strip_color(lower_case(str));
  if(str != "y" && str != "yes") {
    write("You have decided not to advance.");
    return;
  }
  //currently ADVANCE_D prints its own error messages, but it only ever returns 1
  if(!(int)ADVANCE_D->advance())
    write("You cannot advance at this time.");
}

int advance(string str) {
  if(str) return notify_fail("Syntax: <advance>\n");
  if(!this_player()->getenv("ASK_ADVANCE"))
    return (int)ADVANCE_D->advance();
  write("Are you sure you wish to advance to the next level? (y/n)");
  input_to("advance_verify");
  return 1;
}
