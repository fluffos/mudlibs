...
int cmd_score(string str) {
    ...
    sb +=  do_bar(this_player()->query_intox(), this_player()->query_max_intox());
    write(sb);
sb = arrange_string("%^BOLD%^%^MAGENTA%^Stamina Points : %^RESET%^"+sp+"%^BOLD%^%^BLUE%^/%^RESET%^"+max_sp, 35);
    ...
    sb +=  do_bar(this_player()->query_quenched(), this_player()->query_max_quenched());
    write(sb);
...
void check_outlaw() {
    ...
    if(drug)
      write("    Drug offenses: "+(string)drug);
    if(oth1)
      write("    Minor offenses: "+(string)oth1);
    if(oth2)
      write("    Major offenses: "+(string)oth2);
    if(theft)
      write("    Theft offenses: "+(string)theft);
    if(pk)
      write("    Murder offenses: "+(string)pk);
}
void show_punishments() {
  object tp = this_player();
  string out;
  int gtime, gagged;
  gagged = tp->query_property("gagged");
  gtime = tp->query_property("gagtime");

  if (!gagged) return;
  write("%^BLUE%^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
  out = "You are gagged.";
  if (gtime) out += " ("+gtime+" seconds left)";
  write("%^RESET%^"+out);
}
void help() {
    write("Syntax: <score>\n\n"
