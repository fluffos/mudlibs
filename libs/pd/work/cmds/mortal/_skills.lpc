 ...
 *attack = ({ "attack", "melee", "defense", "double wielding", "rage",
             "charging", "riding"}),
 *weapons = ({ "knife", "blade", "blunt", "ranged", "projectile", "axe",
              "flail" }),
 *decep = ({ "locks", "stealth", "murder", "stealing", "bargaining",
            "agility", "euphony", "perception", "valor",
            "discipline", "ferocity", "entertainment", "flying", "crafting"  }),
 *magic = ({ "faith", "healing", "conjuring", "magic attack", "magic defense",
            "summoning", "psionics", "wizardry", "illusion", "nature",
            "belief", "necromancy", "pyromancy", "clairvoyance", "death", "aquamancy",
...
int magic_class()
{
 ...
 if (tp->query_class()=="fighter" && (tp->query_subclass()!="paladin" &&
tp->query_subclass()!="antipaladin")) return 0;
 return 1;
}
...
// string *attack, *magic, *deception, *other, *weapon;
  string *attack, *magic, *deception, *weapon;
 string seeker;
  int i, a;
if(this_player()->query_race() == "")
return notify_fail("Your body has not yet formed.\n");
 if(this_player()->query_race() == "dragon") return "/daemon/_dskills"->dskills(str);
...
write(
"%^BLUE%^=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+="
"\n   %^GREEN%^Deception/Other  %^CYAN%^Points       Max        %^MAGENTA%^Weapons    "     
"%^CYAN%^      Points      Max\n"
"%^BLUE%^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
);
