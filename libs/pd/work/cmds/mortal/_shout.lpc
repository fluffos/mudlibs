...
int cmd_shout(string str) {
	...
	if(member_array(lang, (string *)(LANG_D->all_languages())) == -1)
	    this_player()->remove_language(lang);
...
void shout_out(string lang, string str, int prof)
{
    ...
    seteuid(UID_LOG);
    log_file("shouts", geteuid(previous_object())+" (shout)(" + lang + "): " +
      str + "\n");
    seteuid(getuid());
...
void help() {
    message("help",
      ...
      "Sends a shout in whatever language to the entire mud. Misuse\n"
      "of this power may result in action from law.",
      this_player()
    );
}
