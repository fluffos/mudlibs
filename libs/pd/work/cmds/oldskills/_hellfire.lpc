...
int cmd_hellfire(string str) {
    ...
    i = this_player()->query_skill("summoning");
    i += this_player()->query_stats("intelligence")*3/5;
	...
	if (!per[x]->query_ghost() && per[x] != this_player() && !wizardp(per[x])) {
	    ...
	    per[x]->damage(i+(random(i)/2) - per[x]->query_skill("magic defense")*2/3 - per[x]->query_stats("wisdom")*2/3 , "physical");
