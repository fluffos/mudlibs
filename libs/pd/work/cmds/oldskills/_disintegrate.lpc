...
int cmd_disintegrate(string str) {
   ...
   dmg = tp->query_skill("psionics") + tp->query_stats("wisdom");
   dmg -= ob->query_stats("wisdom")*3/4 + ob->query_skill("magic defense")/3;
   dmg *= 3/2;
   if (dmg < 10) {
   ...
   tp->add_mp(-dmg/7);
   tp->set_casting(dmg + random(tp->query_level()/4));
   tp->set_disable();
   return 1;
}
