...
inherit DAEMON;
int spell();
string *unressable = ({ "demon", "lich", "dragon", "archangel", "bastet", "vampire" });
int cmd_resurrect(string str)
{
    ...
    {
        ob->set_stats("strength",(int)ob->query_stats("strength")/2);
        ob->set_stats("constitution",(int)ob->query_stats("constitution")/2);
        ob->set_stats("dexterity",(int)ob->query_stats("dexterity")/2);
        ob->set_stats("charisma",(int)ob->query_stats("charisma")/2);
        ob->set_stats("intelligence",(int)ob->query_stats("intelligence")/2);
        ob->set_stats("wisdom",(int)ob->query_stats("wisdom")/2);
    } else {
...
int spell() {
    ...
    if (o->query_skill("belief") < 70) return 0;
    return 1;
}
void help() {
