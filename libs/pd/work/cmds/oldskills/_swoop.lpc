...
void finish_him(object *stuff) {
   ...
   dmg += tp->query_stats("strength")*3/4 + tp->query_stats("dexterity")*3/4;
   dmg -= ob->query_stats("constitution")/2 + ob->query_stats("dexterity");
   ...
   ob->damage(dmg + random(this_player()->query_level()));
    this_player()->add_skill_points("flying", dmg/40);
   return;
}
