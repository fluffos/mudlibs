...
int cmd_blast (string str) {
   ...
   if(environment(this_player())->query_property("no magic")) {
    ...
    if((mag = (int)this_player()->query_skill("magic attack")) < 50) {
   ...
   write("%^BLUE%^%^BOLD%^You summon up the forces of your life "
    ...
    points = ((int)this_player()->query_mp())*3/4;
    points += mag - ((int)ob->query_skill("magic defense"))*2;
    if(points < 0) points = 1;
    temps = random(200-mag)/10;
    if(temps <7) temps =6;
    this_player()->set_paralyzed(random(temps/2)+(temps/2));
    this_player()->add_skill_points("magic attack", temps/5);
