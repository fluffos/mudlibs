...
int cmd_psistorm(string str)
{
    ...
    i = this_player()->query_skill("psionics");
    i += this_player()->query_stats("intelligence")*3/2;
	...
	if (!per[x]->query_ghost() && per[x] != this_player() &&
	  per[x]->query_level() > 5 && !wizardp(per[x])) {
	    if(per[x]->query_skill("magic defense") + per[x]->query_stats("wisdom")/2 > i) {
	    ...
	    per[x]->damage(i + (random(i)/3) - per[x]->query_skill("magic defense")/2 - per[x]->query_stats("wisdom")/2 , "physical");
