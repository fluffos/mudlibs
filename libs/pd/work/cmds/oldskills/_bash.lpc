...
int cmd_bash(string str) {
    ...
    write("%^BLUE%^You bash "+victim->query_cap_name()+" on the back of "
      "the head! "+victim->query_cap_name()+" falls to the ground!");
    ...
    /****( Damage: )***********************************************/
    j += this_player()->query_skill("murder");
    j += this_player()->query_stats("strength");
    j += this_player()->query_stats("dexterity");
    j -= random(victim->query_skill("defense")/4);
    j -= victim->query_stats("dexterity")/3;
    j -= victim->query_stats("strength")/3;
    j *= 3/2;
    /**************************************************************/
