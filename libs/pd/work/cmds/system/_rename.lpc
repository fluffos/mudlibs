...
int cmd_rename(string str) {
    ...
    if(this_player()->query_forced() && geteuid(previous_object())!="assist") {
        write("Someone has tried forcing you to rename " + str);
        return 1;
    }
   ...
   {
      notify_fail("rename: couldn't find "+file1+"\n");
      return 0;
   }
  ...
  if (localdest) {
   ...
   {
      notify_fail("rename: "+file1+" is a directory.\n");
      return 0;
   }
#if 0
   if(file_size(file2) > 0)  /* Plura 930120 */
  {
      notify_fail("rename: "+file2+" already exists.\n");
      return 0;
   }
#endif
  if (file1 == file2) {
    notify_fail("rename: can not copy a file on top of itself!\n");
    return 0;
  }
   if(!master()->valid_read(file1, this_player(), "rename"))
    {
    notify_fail(file1 + ": Permission denied.\n");
    return 0;
  }
    if(!master()->valid_write(file2, this_player(), "rename"))
    {
  ...
  if (line == 0) {
    ...
    notify_fail("rename: file not found: " + file1 + "\n");
    return 0;
  }
  if (file_size(file2) == -2)
    {
  ...
  message("system", "Renamed: " + file1 + " to " + file2, this_player());
  return 1;
}
int help() {
  message("help", "Command: rename\nSyntax: rename <oldfile> [pathname]<newfile>\n"
          "This command makes a renames of the file using the new name "
