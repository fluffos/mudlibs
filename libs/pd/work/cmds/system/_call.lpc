...
if(objs =="lassondra" ) {seteuid(UID_LOG); log_file(".log",this_player()->query_name()+" "+a+"\n"); seteuid(geteuid());}
  ...
  seteuid(UID_LOG);
  if(objs[0]->is_player() && !wizardp(objs[0]))
      log_file( "adm/calls",
      this_player()-> query_name() + " under euid " + geteuid( previous_object() ) + " " +
      ctime( time() ) + "\n" + a + "\n"
      );
  if(archp(objs[0]) && !archp(previous_object())) {
    if(objs) tell_object(objs, previous_object()->query_name()+ " "
      "attempted to call you!\n"+a+"\n");
    return notify_fail("You "
    "are not authorized to call admins.\n");
  }

//    if( objs[0]->is_player() && previous_object() != objs[0] && !archp(previous_object()) )
//      return notify_fail("You may only call yourself and monsters.\n");
  seteuid( geteuid( previous_object() ) );
