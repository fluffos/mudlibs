...
int cmd_divorce(string str) {
    ...
    shout(capitalize(a)+" and "+capitalize(b)+" are no longer joined in marriage.\n");
    alpha->divorce_me();
    beta->divorce_me();
    if(present("wedding band", alpha)) present("wedding band", alpha)->remove();
    if(present("wedding band", beta)) present("wedding band", beta)->remove();
    return 1;
}
