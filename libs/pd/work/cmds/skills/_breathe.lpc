...
int cmd_breathe(string str) {
    ...
    cost = tp->query_skill("breath")/8;
    if (sizeof(tgts) > 1)
      cost += (cost*1/2)*(sizeof(tgts)-1);
    ...
    foreach (object ob in tgts) {
      ...
      ob->do_damage("target", dam, get_damage_type(tp), DAMAGE_NO_SEVER, tp);
...
int get_damage(object tp, object ob) {
  ...
  if (level >= 50) {
    dam = BALANCE3_D->get_damage( tp, ob, 4,
            ({ "breath", "breath", "breath", "attack" }),
            ({ "strength" }),
            ({ "intelligence", "wisdom" }) );
  }
  else if (level >= 25) {
    dam = BALANCE3_D->get_damage( tp, ob, 3,
            ({ "breath", "breath", "breath", "attack" }),
            ({ "strength" }),
            ({ "intelligence", "wisdom" }) );
  }
  else {
    dam = BALANCE3_D->get_damage( tp, ob, 2,
            ({ "breath", "breath", "breath", "attack" }),
            ({ "strength" }) );
  }
  dam = dam*7/6 + random(dam/5);
...
void do_extra_white(object tp, object ob, int dam, int stronghit) {
  if (!tp || !ob) return;
  ob->add_poisoning(2+dam/25);
  if (stronghit && random(100) < 30)
...
int get_damage_type(object tp) {
  switch (tp->query_subclass()) {
    case "red": return DAMAGE_PHYSICAL | DAMAGE_FIRE;
    case "white": return DAMAGE_PHYSICAL | DAMAGE_POISON;
    case "black": return DAMAGE_PHYSICAL | DAMAGE_LIGHTNING;
    case "blue": return DAMAGE_PHYSICAL | DAMAGE_ICE;
    case "green": return DAMAGE_PHYSICAL | DAMAGE_ACID;
  }
