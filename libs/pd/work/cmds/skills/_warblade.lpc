...
int cmd_warblade(string str) {
  ...
  */
  ...
  if (tp->query_stats("dexterity")*(80+random(15))/100+tp->query_skill("discipline")/6+random(20) < tgt->query_stats("dexterity")) {
    message("combat","You miss "+tgt->query_cap_name()+" with your war slash.", tp);

    message("combat",tp->query_cap_name()+" misses "+tgt->query_cap_name()+
    ...
    if (rank >= 3) {
      // for every 25 dex over 75, give another hit
      for (int i=(tp->query_stats("dexterity")-75+random(30))/25; i>0; i--) {
        ...
        message("combat", tp->query_cap_name()+" swings around and "+COL+
                          ...
                          environment(tp), ({ tp, tgt}) );
        tgt->do_damage(tgt->return_target_limb(), dmg/7, DAMAGE_PHYSICAL | DAMAGE_SLASH, DAMAGE_BYPASS_ARMOUR, tp);
        tp->add_sp(-((3+cost/10)));
      }
    }
