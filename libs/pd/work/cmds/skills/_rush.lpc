...
//  cost = dmg/10.5;
  ...
  if (tgt->query_stats("dexterity")*(9+random(4))/10 > tp->query_stats("dexterity"))
    miss=1;
  ...
  tp->set_disable( (userp(tp) ? 1 : 2) );
  tp->add_sp(-(cost/2));
  tp->set(RUSH_PROP, time());

  call_out("rush_hit", (userp(tp)?1:3), ({ tp, tgt, type, miss, dmg, cost, (sizeof(weps) ? weps[0] : 0) }) );
...
void rush_hit(mixed *args) {
  ...
  if (type == "melee") {
    what = "body";
    dtype |= tp->query_melee_damage_type();
  }
  else {
    if (!wep) {
  ...
  if (miss) {  
    ...
    tgt->do_damage(tgt->return_target_limb(), dmg, dtype, 0, tp);
    tp->add_sp(-(cost/2));
  }
