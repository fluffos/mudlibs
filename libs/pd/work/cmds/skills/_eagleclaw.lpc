...
int can_cast(object tp, object tgt) {
  ...
  if (!tgt->query_is_limb("head") && !tgt->query_is_limb("torso"))
    FAIL("That has neither heart nor throat!");
...
int cmd_eagleclaw(string str) {
  ...
  cost = BALANCE3_D->get_cost(dmg, 4, "sp") * 3 + random(50);
  ...
  tgt->do_damage(limb, dmg, tp->query_unarmed_damage_type() | DAMAGE_PIERCE, tp->query_unarmed_damage_flags(), tp);
