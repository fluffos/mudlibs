...
#define POCKET_DIR DIR_CLASS_CMDS+"/obj/pocket/"
...
int cmd_pocketgag(string str) {
  ...
  ob->set_id( (ob->query_id() || ({})) + ({ POCKET_ID }) );
  ...
  if (ob->is_living()) {
    if (sizeof(tp->query_pets()) >= 5) {
      message("info", capitalize(definite_article(ob->query_short()))+
        " bites you and runs away!", tp);
      message("info", capitalize(definite_article(ob->query_short()))+
        " bites "+tp->query_cap_name()+" and runs away!", env, tp);
      tp->do_damage("none", 5+random(20), DAMAGE_PHYSICAL | DAMAGE_PIERCE,
         DAMAGE_NONLETHAL | DAMAGE_HP_ONLY | DAMAGE_BYPASS_ARMOUR | DAMAGE_BYPASS_STRENGTHS | DAMAGE_NO_PLAYER_REDUCE, ob);
      ob->remove();
    }
    else {
      ob->move(env);
