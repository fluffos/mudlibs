...
int can_cast(object tp, object tgt, object *weps, string limb) {
  ...
  if (strlen(limb) && !tgt->query_is_limb(limb))
...
//  if (str) tgt = present(str, env);
  ...
  perc += tgt->query_skill("agility")/3;
  perc += tgt->query_stats("intelligence");
  perc += tgt->query_stats("wisdom");
  perc += tgt->query_stats("dexterity")*75/100;

  stealth = tp->query_skill("stealth");
  stealth += tp->query_skill("agility")*4/9;
  stealth += tp->query_skill("murder")/4;
  stealth += tp->query_stats("dexterity")*7/10;
  if (tp->query_invis())
    stealth = stealth * 130 / 100;
...
void finish_backstab(mixed *args) {
  ...
  if (caught &&
      ...
      random(tgt->query_stats("dexterity")/3+caught*2) >
  ...
  if (tp->query_level() < 20)
    ...
    if (userp(tp) && userp(tgt))
      ...
      l = 6;
    */
    tmp = tp->query_stats("dexterity")*4/3*(1+rank);
    ...
    */
    mod = tmp / (tgt->query_stats("dexterity")*6.0);
    if (mod < 1.3) mod = 1.3;
    if (mod > 1.5) mod = 1.5 + (mod-1.5)/2.5;
    if (mod > 1.9) mod = 1.9 + (mod-1.9)/3.0;
  }

  dmg += wc*20;
  dmg *= mod;

  if ((int)tp->query(VANISH_PROP)+4 >= time()) dmg *= 0.85;
