...
int do_exit(string str) {
  ...
  if (exit == "out")
    ...
    if (this_player()) {
      ...
      this_player()->do_damage("none", (amt+random(amt/5)-random(amt/3)), DAMAGE_MAGIC | DAMAGE_FIRE, 0, "fire plane");
    }
...
void do_burn() {
  ...
  foreach (object what in all_inventory()) {
    if (what->is_living()) {
      ...
      what->do_damage("none", (amt+random(amt/5)-random(amt/3)), DAMAGE_MAGIC | DAMAGE_FIRE, 0, "fire plane");
    }
    else {
