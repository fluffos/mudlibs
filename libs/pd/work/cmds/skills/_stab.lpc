...
#define SKMORE ({ "murder", "discipline", "entertainment" })
#define DWMOD 3
...
void extra_hit(object tp, object tgt, int rank, int dmg, object w, string limb, int focus) {
  ...
  bodydam = dmg * (60 + focus*25) / 100;
  div = 15 - random(focus*6);

  tgt->do_damage(limb, bodydam, DAMAGE_PHYSICAL | DAMAGE_PIERCE, DAMAGE_NO_HP, tp);

  if (focus || (rank >= 3 && tp->query_alignment() < -1*random(2500))) {
    message("combat", "You twist the "+wep+" hard!", tp);
    message("combat", tp->query_cap_name()+" twists the "+wep+" hard!",
      environment(tp), tp);
    tgt->do_damage(limb, dmg/div, DAMAGE_PHYSICAL | DAMAGE_PIERCE, DAMAGE_NO_SEVER, tp);
  }
