...
void help() {
  message("help",
    ...
    "doing more damage and hitting all of his opponents.\n"
...
int can_cast(object tp, object ob, object drink) {
  ...
  if (objectp(drink) && drink->query_type() != "alcoholic")
...
//  cost = dmg/2.9;
  cost = BALANCE3_D->get_cost(dmg, 2, "mp") * 250 / 100;
  ...
  if (dmg < 10) {
    ...
    tgt->do_damage("target", dmg, DAMAGE_MAGIC | DAMAGE_FIRE, 0, tp);
  }
  ...
  if (drinkpower) {
    foreach (object t in tgts) {
      ...
      t->do_damage("random", d, DAMAGE_MAGIC | DAMAGE_FIRE, DAMAGE_NO_SEVER, tp);
    }
    drink->remove();
  }
