...
//  cost = dmg/12 - 30;
  cost = BALANCE3_D->get_cost(dmg, 4, "sp") * 130 / 100 - 30;
  if (cost < 20) cost=20;
  ...
  if (tp->query_sp() < cost) {
    ...
    tgt->do_damage("random", 10+random(25), DAMAGE_PHYSICAL | DAMAGE_IMPACT, DAMAGE_NO_SEVER, tp);
    return;
  }
  
  tp->set_disable();
  tp->set_magic_round(); // to stop idle hits too
  tp->add_sp(-cost);
  ...
  if (othertgt) {
    ...
    if (tp->kill_ob(othertgt)) {
      if (dodge > hit+random(20))
        ...
        otherdmg = BALANCE3_D->get_damage(tp, othertgt, 3,
          ({ "attack", "melee" }),
          ({ "strength", "strength", "dexterity" }),
          ({ "strength", "dexterity" }) );
      }
  ...
  if (otherdmg) {
    ...
    othertgt->do_damage("random", otherdmg, DAMAGE_PHYSICAL | DAMAGE_IMPACT, DAMAGE_NO_SEVER, tp);
    othertgt->set_rest_type(LAY);
  }
  
  tgt->do_damage("random", dmg, DAMAGE_PHYSICAL | DAMAGE_IMPACT, DAMAGE_NO_SEVER, tp);
