...
int cmd_boot(string str) {
  ...
  cost = BALANCE3_D->get_cost(dmg, rank, "sp") * 175 / 100;
  if (cost < 15) cost = 15;
  ...
  tgt->do_damage("torso", dmg, DAMAGE_PHYSICAL | DAMAGE_UNARMED | DAMAGE_IMPACT, DAMAGE_NO_SEVER | DAMAGE_NONLETHAL, tp);
  ...
  if (tp->query_stats("strength") > tgt->query_stats("constitution")*175/100) {
    ...
    boot->misc = ob;
    boot->endfunc = function(class Effect boot) {
