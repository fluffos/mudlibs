...
int spell() {
  ...
  if (tp && tp->query_class() == "mage" && tp->query_skill("conjuring") >= 90)
    return 1;
  if (tp && tp->query_class() == "vampire" && tp->query_level() >= 10)
    return 1;
  return 0;
}
...
int cast_mist(object env, object *all) {
  int count = 0, i= sizeof(all);
  while(i--) {
    if (!all[i] || all[i] == this_object())
      continue;
