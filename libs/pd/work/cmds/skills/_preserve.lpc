...
int cmd_preserve(string str) {
   ...
   if (ob->query_preserved())
      return notify_fail("It has already been preserved.\n");

   if (tp->query_mp() < 20)
      return notify_fail("Your spell fails, due to lack of magic.\n");

   if (tp->query_ghost())
      return notify_fail("Your words are hollow.\n");
...
/*   if (random(tp->query_skill("necromancy")) < random(50))
   ...
   ob->set_long(ob->query_long()+" %^GREEN%^(preserved)%^RESET%^ ");
   tp->add_mp(-random(15));
   return 1;
}
...
This spell preserves a corpse or body part from
decaying completely.
...
int spell() {
   ...
   o = this_player();
   if (o)
     if (o->query_class() == "vampire" || o->query_subclass() == "kataan" || o->query_subclass() == "necromancer")
      if (o->query_level() >= 12)
          return 1;
}
