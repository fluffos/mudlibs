...
//    amount = random((int)this_player()->query_skill("necromancy")/5)+5;
    ob = present("remains", environment(this_player()));
    amount = 5+ob->query_mp()/600+random(ob->query_mp()/300);
    this_player()->add_mp(amount);
    amount = 5+ob->query_hp()/650+random(ob->query_hp()/400);
    this_player()->add_hp(amount);
    this_player()->add_alignment(-2);
  ob->loot(this_player());
  /*
