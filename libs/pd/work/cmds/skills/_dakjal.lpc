...
int cmd_dakjal(string str) {
  ...
  pcost = BALANCE3_D->get_cost(pdmg, 4, "sp") * 150 / 100;
  ...
  mcost = BALANCE3_D->get_cost(mdmg, 4, "sp") * 75 / 100;
  ...
  if (tp->query_rest_type()) {
    ...
    tgt->do_damage("target", pdmg, DAMAGE_PHYSICAL | DAMAGE_SLASH, 0, tp);
  }
  ...
  if (tp->query_mp() < mcost) {
    ...
    tgt->do_damage("random", mdmg, DAMAGE_FAITH | DAMAGE_UNHOLY, DAMAGE_BYPASS_ARMOUR, tp);
  }
