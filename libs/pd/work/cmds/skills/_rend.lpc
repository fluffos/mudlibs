...
int cmd_rend(string str) {
  ...
  foreach (string l in tgt->query_limbs()) {
    ...
    if ((fatal && perc >= 70) || (!fatal && perc >= 40)) {
      tgt->do_damage(l, tgt->query_max_dam(l), DAMAGE_FAITH | DAMAGE_UNHOLY, DAMAGE_NO_HP, tp);
      ok++;
    }
    else {
      // 30-40 percent of max damage
      int dmg = tgt->query_max_dam(l) * (30+random(11)) / 100;
      tgt->do_damage(l, dmg, DAMAGE_FAITH, DAMAGE_NO_HP, tp);
    }
  }

  if (!ok) {
    tgt->do_damage("none", 300+random(100), DAMAGE_FAITH | DAMAGE_UNHOLY, DAMAGE_HP_ONLY, tp);
  }
