...
#define FEAR_PROP "fear_spell_time"
...
int can_cast(object tp, object tgt) {
  ...
  if ((int)tgt->query(FEAR_PROP) >= time())
    FAIL("You cannot try to strike fear into "+tgt->query_objective()+" again so soon!");
...
int cmd_fear(string str) {
  ...
  cost = dmg*2/3;
  if (cost > 300) cost = 300 + (cost-300)/2;
  if (cost > 500) cost = 500 + (cost-500)/2;

  if (tp->query_mp() < cost)
    FAIL("You need more magic.");

  if (userp(tgt))
    dmg -= tgt->query_skill("valor")/2;
