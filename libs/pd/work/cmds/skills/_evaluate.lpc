...
string query_output(object tgt);
...
int cmd_evaluate(string str) {
  ...
  message("info", "You look "+COL+"over"+RES+" "+tgt->query_cap_name()+
                  " and try to "+COL+"evaluate"+RES+" "+
                  tgt->query_possessive()+" armouring.", tp);
  ...
  message("info", query_output(tgt), tp);
...
string query_examine(object tp, object tgt) {
  return strip_color(query_output(tgt));
}
...
string query_output(object tgt) {
  ...
  rep = COL+"You see that "+COL2+rep+COL+" vulnerable."+RES;
