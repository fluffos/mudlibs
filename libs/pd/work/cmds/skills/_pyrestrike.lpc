...
int cmd_pyrestrike(string str) {
  ...
  if (tp->query_level() >= 40) {
    dmg=BALANCE3_D->get_damage(tp, tgt, 3,
          ...
          ({"strength"}) );
    bdmg=BALANCE3_D->get_damage(tp, tgt, 3,
    ...
    dmg=BALANCE3_D->get_damage(tp, tgt, 2,
          ({"melee","melee","attack","claw"}),
          ({"strength", "strength", "dexterity"}) );
    bdmg=BALANCE3_D->get_damage(tp, tgt, 2,
          ({"breath","breath","attack"}),
          ({"strength", "strength", "intelligence"}) );
  }
  ...
  cost = BALANCE3_D->get_cost(dmg, 3, "sp")*60/100;
  ...
  tgt->do_damage("target", dmg, DAMAGE_PHYSICAL | DAMAGE_UNARMED | DAMAGE_IMPACT, 0, tp);
...
void roast(mixed *args) {
  ...
  cost = BALANCE3_D->get_cost(bdmg, 3, "sp");
  ...
  tgt->do_damage("all", bdmg, DAMAGE_PHYSICAL | DAMAGE_FIRE, DAMAGE_NO_SEVER, tp);
