...
int cmd_siphon(string str) {
  ...
  mod = 0.35 - (0.5 * ((sizeof(tgts)-1)/2));
  if (mod < 0.15) mod = 0.15;
  ...
  foreach (object tgt in tgts) {
    ...
    cost = BALANCE3_D->get_cost(dmg, 2, "mp")*85/100;
  ...
  foreach (mixed info in hit) {
    object tgt=info[0];
    int dmg=info[1];
    tgt->do_damage("none", dmg, DAMAGE_MAGIC, DAMAGE_NO_SEVER | DAMAGE_BYPASS_ARMOUR, tp);
    tp->add_hp(to_int(dmg*0.4));
  }
