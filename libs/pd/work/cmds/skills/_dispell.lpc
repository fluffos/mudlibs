...
int cmd_dispell(string str) {
  ...
  if (all)
    tgts = filter(all_inventory(env),
      (: living($1) && strlen($1->query_long()) && $1 != this_player() && can_cast(this_player(), $1) :) );
  else if (str && str != "")
...
//    dmg = dmg*(1.15+to_float(tp->query_stats("wisdom")/40)*0.1);
    if (al <= -500)
      dmg = dmg*(1.1+((absolute_value(al)-500)/100)*0.07);
    else if (al > -100)
      dmg = dmg*3/5;

//    cost = dmg/12 + 10;
    cost = BALANCE3_D->get_cost(dmg, rank, "mp") * 9/10;
    ...
    who->do_damage("none", dmg, DAMAGE_FAITH | DAMAGE_HOLY, DAMAGE_HP_ONLY, tp);
