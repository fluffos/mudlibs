...
int cmd_parry() {
   ...
   if (!abil()) {
    ...
    tp = this_player();
    if (tp->query_subclass() == "red") red=1;
    if(!red && !sizeof((object *)tp->query_wielded())) {
...
int abil() {
  string *allowed = ({ "fighter", "swashbuckler", "red" });
  object tp = this_player();
  if (tp && tp->query_level() >= 2 &&
    sizeof( ({ tp->query_class(), tp->query_subclass() }) & allowed) )
      return 1;
  return 0;
}
