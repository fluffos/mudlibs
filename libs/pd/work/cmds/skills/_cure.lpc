...
int cmd_cure(string str) {
    ...
    healing = (int)tp->query_skill("healing")*3/4;
    healing += tp->query_stats("wisdom")*2/3;
...
void send_messages(object tp, object ob, string clas, string limb) {
    ...
    if(tp == ob) {
        tell_object(tp, COL+"You cure your own wounds through "+thing+"."+RES);
        tell_room(environment(tp), COL+tp->query_cap_name() + " mutters "+what+"."+RES, ({tp}));
    }
    else {
        tell_object(tp, COL+"You cure "+ob->query_cap_name()+"'s wounds through "+thing+"."+RES);
        tell_object(ob, COL+tp->query_cap_name()+" cures your wounds through "+thing+"."+RES);
