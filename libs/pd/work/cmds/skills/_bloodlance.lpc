...
int cmd_bloodlance(string str) {
  ...
  spcost = BALANCE3_D->get_cost(pdmg, 2, "sp") * 110 / 100;
  ...
  mpcost = BALANCE3_D->get_cost(mdmg, 3, "mp") * 115 / 100;
  ...
  tgt->do_damage("torso", pdmg, DAMAGE_PHYSICAL | DAMAGE_PIERCE, 0, tp);
  ...
  if (alignment_ok(tp)) {
    ...
    tgt->do_damage(0, mdmg, DAMAGE_FAITH | DAMAGE_UNHOLY, DAMAGE_HP_ONLY | DAMAGE_BYPASS_ARMOUR, tp);
    tp->add_hp(absorb);
  }
