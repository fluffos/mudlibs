...
int abil() {
  ...
  if (tp->query_class() == "fighter" && tp->query_skill("melee") >= 30)
    return 1;
  if (tp->query_class() == "vampire" && tp->query_skill("melee") >= 30)
    return 1;
  if (tp->query_subclass() == "monk" && tp->query_level() >= 6)
    return 1;
  if (member_array(tp->query_subclass(), ({"thug","thief","druid",}) ) != -1 &&
...
int can_cast(object tp, object tgt) {
  ...
  if (!tp->query_is_limb("head"))
    FAIL("You can't headbutt without a head.");
...
//  cost = dmg/11.2;
  ...
  myhit = tp->query_stats("dexterity")*2;
  myhit += tp->query_skill("attack")*3/2;
  ...
  if (myhit+random(15) < hisdodge) {
    ...
    if (tp->query_subclass() == "monk") {
      perc = COMBAT_D->combo(tp, "headbutt");
      dmg = dmg * perc / 100;
      if (perc > 100) {
        mystr *= 2;
        extra = COLC+"powerfully"+RES+" ";
      }
    }
    ...
    message("combat", "You bring your "+
      ...
      " and "+extra+COL2+"slam"+RES+" your forehead into "+tgt->query_cap_name()+
      "'s "+limb+"!", tp);
    message("combat", tp->query_cap_name()+" brings "+tp->query_possessive()+" "+
      COL+"h"+COL2+"e"+COL+"a"+COL2+"d"+COL+" b"+COL2+"a"+COL+"c"+COL2+"k"+RES+
      " and "+extra+COL2+"slams"+RES+" "+tp->query_possessive()+" forehead into "
      "your "+limb+"!", tgt);
    message("combat", tp->query_cap_name()+" brings "+tp->query_possessive()+" "+
      COL+"h"+COL2+"e"+COL+"a"+COL2+"d"+COL+" b"+COL2+"a"+COL+"c"+COL2+"k"+RES+
      " and "+extra+COL2+"slams"+RES+" "+tp->query_possessive()+" forehead into "+
    ...
    if (mystr+random(25) < hisstr) {
      ...
      message("combat", tp->query_cap_name()+" looks pained from the blow.",
        env, tp);
      tp->do_damage("head", dmg/6, DAMAGE_PHYSICAL | DAMAGE_IMPACT, DAMAGE_NO_SEVER, tp);
      tgt->do_damage(limb, dmg/2, DAMAGE_PHYSICAL | DAMAGE_IMPACT, DAMAGE_NO_SEVER, tp);
    }

    else {
      tgt->do_damage(limb, dmg, DAMAGE_PHYSICAL | DAMAGE_IMPACT, DAMAGE_NO_SEVER, tp);
    }
