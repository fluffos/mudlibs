...
int cmd_flyingkick(string str) {
    ...
    if(!str)
        return notify_fail("Flyingkick whom?\n");
    ...
    */
    ...
    call_out("do_jump", 1, ({victim, tp, rm, dir, startroom}) );
...
int do_jump(mixed *args)
{
  ...
  object startroom=args[4];
	...
	if(!tp)
		return 1;
	if(!victim)
                return notify_fail("That seems to be gone!\n");
	if(!rm)
	...
	message("room", "%^BOLD%^"+tp->query_cap_name()+
		...
		, environment(tp), tp);
	call_out("do_dmg", 1, ({victim, tp, rm, startroom}) );
}
...
int do_dmg(mixed *args) {
  ...
  object tp=args[1];
  object rm=args[2];
  object startroom=args[3];

    if(!rm)
    	return notify_fail("Umm..... the room is gone.... tell an imm.");
    	
  if (environment(tp) != startroom) {
    message("room","Your jump kick was interrupted.",tp);
    return 1;
  }
    tp->move(rm);
