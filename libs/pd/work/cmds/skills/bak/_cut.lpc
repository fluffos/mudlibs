...
int cmd_cut(string str) {
  ...
  if (dmg < 15) {
    message("combat","You try to cut "+tgt->query_cap_name()+
                     ", but miss.", tp);

    message("combat",tp->query_cap_name()+" tries to cut "+tgt->query_cap_name()+
                     ", but misses.", environment(tp), ({ tp, tgt }) );
    
    message("combat",tp->query_cap_name()+" tries to cut you, but misses.", tgt);
  }
  else {
    wep = weps[0]->query_name();
    message("combat","You "+COL+"cut"+RES+" "+tgt->query_cap_name()+" with "+wep+"!", tp);
    
    message("combat",tp->query_cap_name()+" "+COL+"cuts"+RES+" you with "+wep+"!", tgt);

    message("combat",tp->query_cap_name()+" "+COL+"cuts"+RES+" "+tgt->query_cap_name()+
