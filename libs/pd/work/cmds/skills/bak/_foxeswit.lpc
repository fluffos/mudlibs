...
#define FOXESWIT_PROP "foxeswit_spell_time"
...
int query_waiting(object ob) { return ob->query(FOXESWIT_PROP) > time(); }
...
int cmd_foxeswit(string str) {
	...
	if(ob != tp)
    {
        write("%^GREEN%^You call upon the primal nature of the %^BLUE%^witty fox%^RESET%^%^GREEN%^!");
        message("target", "%^GREEN%^You feel the power of the %^BLUE%^witty fox%^RESET%^%^GREEN%^ enter your body!", ob);
        message("room", "%^GREEN%^"+ob->query_cap_name()+
        	" seems to look wittier.", environment(tp), ob);  
    }
    else
    {
       write("%^GREEN%^You call the primal nature of the %^BLUE%^witty fox%^RESET%^%^GREEN%^ on yourself!");
       message("room", "%^GREEN%^"+ob->query_cap_name()+
        	" seems to look wittier.", environment(tp), ob);  
    ...
    ob->set(FOXESWIT_PROP, time()+time);

	amount = 5;	

    ob->add_skill_bonus("nature", tp->query_base_skill("nature")/5, time);
    ob->add_stat_bonus("intelligence", tp->query_base_stats("wisdom")/4, time);
...
void debuff_off(object ob) {
        if(!ob) return;
        message("target", "%^GREEN%^You feel the nature of the witty fox leave you!", ob);
        message("room", "%^GREEN%^"+ob->query_cap_name()+" seems less witty."
			, environment(ob), ob);
        
}
void help() {
        write("Syntax: <foxeswit [(living)]>\n"
        "Druids can call on the primal nature of the "
		"witty fox, and bestow its gift on the target.\n");
}
int spell() {
   	object tp;
   	tp = this_player();
   	...
   	if (tp->query_skill("nature") < 150) return 0;
