...
int cmd_chop(string str) {
  ...
  if (dmg < 15) {
    message("combat","You chop at "+tgt->query_cap_name()+
                     " with an axe, but miss.", tp);

    message("combat",tp->query_cap_name()+" chops at "+tgt->query_cap_name()+
                     " with an axe, but misses.", environment(tp), ({ tp, tgt }) );
    
    message("combat",tp->query_cap_name()+" chops at you with an axe, but misses.", tgt);
  }
  else {
    wep = weps[0]->query_name();
    message("combat","You "+COL+"chop"+RES+" "+tgt->query_cap_name()+" with "+wep+"!", tp);
    
    message("combat",tp->query_cap_name()+" "+COL+"chops"+RES+" you with "+wep+"!", tgt);

    message("combat",tp->query_cap_name()+" "+COL+"chops"+RES+" "+tgt->query_cap_name()+
