...
int cmd_detect(string str) {
   ...
   if (!spell()) {
    ...
    if((string)tp->query_subclass() != "monk" && (string)tp->query_subclass() != "cleric") {
    ...
    if(tp->query_magic_round()) return 1;
...
int check_alignment(object tp, string clas) {
   if(clas == "monk" && (int)tp->query_alignment() < 200) return 0;
   else if(clas == "cleric" && (int)tp->query_alignment() <0) return 0;
    return 1;
}
...
int spell() {
   ...
   if (tp->query_subclass() != "monk" && tp->query_subclass() != "cleric")
     return 0;
   if (tp->query_skill("faith") < 12) return 0;
   return 1;
}
