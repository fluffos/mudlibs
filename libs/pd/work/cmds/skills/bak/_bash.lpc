...
int cmd_bash(string str) {
  ...
  if (dmg < 15) {
    message("combat","You try to bash "+tgt->query_cap_name()+
                     ", but miss.", tp);

    message("combat",tp->query_cap_name()+" tries to bash "+tgt->query_cap_name()+
                     ", but misses.", environment(tp), ({ tp, tgt }) );
    
    message("combat",tp->query_cap_name()+" tries to bash you, but misses.", tgt);
  }
  else {
    wep = weps[0]->query_name();
    message("combat","You "+COL+"bash"+RES+" "+tgt->query_cap_name()+" with "+wep+"!", tp);
    
    message("combat",tp->query_cap_name()+" "+COL+"bashes"+RES+" you with "+wep+"!", tgt);

    message("combat",tp->query_cap_name()+" "+COL+"bashes"+RES+" "+tgt->query_cap_name()+
...
void help() {
  message("help","\n"
    "Syntax: <bash [living]>\n\n"
    "A basic attack with a blunt weapon.\n"
