...
if((string)tp->query_subclass() != "pyromancer" && (string)tp->query_subclass() != "sorceror") {
    ...
    attack = BALANCE2_D->get_Mdamage( tp, ob, 1, ({ "magic attack" }),
                                   ({ "wisdom" }) );
    ...
    write("You attempt to burn your opponent!");
    tell_room(environment(tp), ""+(string)ob->query_cap_name()+
...
int spell() {
    if(wizardp(this_player()) && this_player()->query_subclass() == "pyromancer") return 1;
    return 0;
}
