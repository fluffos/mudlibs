...
#define BEARSRAGE_PROP "bearsrage_spell_time"
...
int query_waiting(object ob) { return ob->query(BEARSRAGE_PROP) > time(); }
...
int cmd_bearsrage(string str) {
	...
	if(ob != tp)
    {
        write("%^GREEN%^You call upon the primal nature of the %^RED%^raging bear%^RESET%^%^GREEN%^!");
        message("target", "%^GREEN%^You feel the power of the %^RED%^raging bear%^RESET%^%^GREEN%^ enter your body!", ob);
        message("room", "%^GREEN%^"+ob->query_cap_name()+
        	" seems to look stronger.", environment(tp), ob);  
    }
    else
    {
       write("%^GREEN%^You call the primal nature of the %^RED%^raging bear%^RESET%^%^GREEN%^ on yourself!");
       message("room", "%^GREEN%^"+ob->query_cap_name()+
        	" seems to look stronger.", environment(tp), ob);  
    ...
    ob->set(BEARSRAGE_PROP, time()+time);

	amount = 5;	

    ob->add_skill_bonus("attack", tp->query_base_skill("nature")/5, time);
    ob->add_skill_bonus("melee", tp->query_base_skill("nature")/5, time);
    ob->add_stat_bonus("strength", tp->query_base_stats("wisdom")/4, time);
...
void debuff_off(object ob) {
        if(!ob) return;
        message("target", "%^GREEN%^You feel the nature of the raging bear leave you!", ob);
        message("room", "%^GREEN%^"+ob->query_cap_name()+" seems less strong."
			, environment(ob), ob);
        
        ob->set(BEARSRAGE_PROP, 0);
}
void help() {
        write("Syntax: <bearsrage [(living)]>\n"
        "Druids can call on the primal nature of the "
		"raging bear, and bestow its gift on the target.\n");
}
int spell() {
   	object tp;
   	tp = this_player();
