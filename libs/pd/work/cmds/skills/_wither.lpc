...
int cmd_wither(string str) {
  ...
  dmg_sp = dmg * tgt->query_max_sp() / tgt->query_max_hp() / 4;
  if (dmg_sp > tgt->query_max_sp()/4) dmg_sp = tgt->query_max_sp()/4 + random(tp->query_level());
  ...
  tgt->do_damage("random", dmg,
    DAMAGE_PHYSICAL | DAMAGE_FAITH | DAMAGE_UNHOLY | wep->query_damage_type(),
    wep->query_damage_flags(), tp);
  tgt->add_sp(-dmg_sp);
