...
int cmd_eagledive(string str) {
  ...
  // try find_object first in case it's a cloned room, THEN try load_object
  if (strlen(dir) && strlen(tmp = env->query_exit(dir)) &&
      tmp != ROOM_VOID &&
      objectp(tgtenv = (find_object(tmp) || load_object(tmp))) && strlen(tgtname))
...
void do_hit(mixed *args) {
  ...
  string *skills, *limbs;
  string limb;
  int rank, dmg, cost, hit, miss=0, tmp, dtype;
  ...
  if (!newenv || environment(tp) != oldenv)
    RETFAIL("Something interrupts your attack!");

  if (tp->move(newenv))
    RETFAIL("Something knocks you out of the air!");
  ...
  cost = BALANCE3_D->get_cost(dmg, rank, "sp") - 10;
  if (cost < 20) cost = 19+random(7);
  tp->add_sp(-cost);

  limbs = ({ tgt->query_torso() }) + regexp(tgt->query_limbs(), "head|arm");
  limb = limbs[random(sizeof(limbs))];

  dtype = DAMAGE_PHYSICAL | DAMAGE_UNARMED | DAMAGE_IMPACT;
