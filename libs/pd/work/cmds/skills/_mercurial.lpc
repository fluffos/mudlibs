...
int cmd_mercurial(string str) {
    ...
    if (!str) return notify_fail("Cast mercurial upon whom?\n");

    else if (str == "me") ob = tp;

    else ob = present(str, environment(tp));

    if (!ob || !ob->is_living()) return notify_fail("Cast mercurial upon whom?\n");
    
    if (waiting[ob]){
        write("Your spell bounces off into the sky.\n");
        return 1;
    }
