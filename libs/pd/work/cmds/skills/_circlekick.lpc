...
int cmd_circlekick(string str) {
  ...
  foreach (object atk in atkrs) {
    ...
    if (dodge > hit + random(25) - random(25)) {
      ...
      atk->do_damage("random", dmg, tp->query_unarmed_damage_type(), tp->query_unarmed_damage_flags(), tp);
    }
  ...
  if (perc > 100) {
    ...
    foreach (object atk in atkrs) {
      ...
      atk->do_damage("random", dmg, tp->query_unarmed_melee_type(), tp->query_unarmed_damage_flags(), tp);
    }
