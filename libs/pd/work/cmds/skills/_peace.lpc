...
int cmd_peace(string str) {
    ...
    if (!wizardp(this_player())) {
        if (this_player()->query_subclass() == "cleric") {
            ...
            if(random(def)<2) {
                write("Your belief does not bring peace, this time.");
            ...
            if(random(def)<2) {
                write("Your faith does not bring peace, this time.");
                this_player()->set_disable();
                this_player()->set_magic_round();
                return 1;
            }
    ...
    if ((this_player()->query_class("cleric"))) {
        this_player()->add_skill_points("belief", 5+def);
    }
    if ((this_player()->query_class("monk"))) {
        this_player()->add_skill_points("faith", 5+def);
    }
    if (!wizardp(this_player()))
      this_player()->add_alignment(2+random(4));
