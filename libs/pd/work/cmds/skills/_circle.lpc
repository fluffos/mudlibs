...
int can_cast(object tp, object tgt, object *weps) {
  ...
  if (tp->query_riding())
    FAIL("You can't circle until you dismount!");

  if (sizeof(tp->query_riders()))
    FAIL("You can't move fast enough with people riding on your back.");
...
int cmd_circle(string str) {
  ...
  cost = BALANCE3_D->get_cost(dmg, 4, "sp") * 110 / 100 + 10;
  ...
  def = tgt->query_stats("dexterity")*5/4;
  def += random(tgt->query_stats("intelligence")/2);
  def += tgt->query_skill("agility");
  def += tgt->query_skill("defense")/2;
  def += tgt->query_skill("perception")*4/5;
  ...
  agil = tp->query_stats("dexterity")*4/3;
  agil += tp->query_skill("agility");
  agil += tp->query_skill("attack")/2;
  agil += tp->query_skill("stealth")*3/4;
  ...
  if ((sizeof(tmp = (mixed *)tp->query(BACKSTAB_PROP)) == 2 &&
      ...
      (sizeof(tmp = (mixed *)tp->query(AMBUSH_TIME)) == 2 &&
      tmp[0] == time() && tmp[1] == tgt) ) {
