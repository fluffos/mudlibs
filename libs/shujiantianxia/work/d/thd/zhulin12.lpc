...
LONG
        ...
        set("exits", ([
                "south" :  __DIR__"zhulin"+(random(10)+10),
                "north" : __DIR__"zhulin"+(random(10)+10),
                "west" :  __DIR__"zhulin13",
                "east" : __DIR__"zhulin"+(random(10)+10),
        ]));
...
void init()
{
        ...
        if ((ob->query("family/family_name") == "桃花岛" && ob->query_skill("qimen-bagua", 1) > 80) ||
                ob->query_skill("qimen-bagua", 1) > 150) {
                write("由于你平时看惯了这些简易的阵法，所以不用细想，信步就走出了竹林！\n");
                ob->move(__DIR__"hetang", 1);
