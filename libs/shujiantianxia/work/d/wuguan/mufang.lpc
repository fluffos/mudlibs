...
int do_ju(string arg)
{
	...
	if( !weapon || weapon->query("id") != "ju zi")
		return notify_fail("你想用什么来锯木头，用手掰吗？("HIY HBCYN"wield ju zi"NOR")\n");
             
	costj = random( me->query("con")/3);
	costq = random( me->query("str")/3);

	if ( me->query("jingli") < costj || me->query("jing") < costq){
	...
	if( me->query_temp("mark/锯") > 15 + random(5) || me->query_temp("mark/完了")) {
		...
		write(CYN"木房管事说道：「干的不错，好了，你可以去大师兄鲁坤那覆命("HIY HBCYN"task ok"CYN")了！」\n"NOR);
		return 1;
	}
