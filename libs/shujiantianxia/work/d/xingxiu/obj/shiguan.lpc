...
int do_liandu()
{
        ...
        if( me->query_skill("poison") > 200 )
                return notify_fail("你的毒技已经太高，无法通过练毒来提升了。\n");
...
int start_liandu(object me)
{
        ...
        string poison_type = this_object()->query_temp("poison_type");
...
上角，以屈下颊至；又从耳後入耳中，出走耳前，交颊。三路合于一处！\n"NOR);
        ...
        if((int)me->query("neili") < 0 || (int)me->query("jingli") < 0){
                ...
                me->set("neili", 0);
                me->apply_condition("xx_liandu", 10 + random(40));
        ...
        if(period==times){
                write(HIW"\n猛地，你感觉到一股至"+poison_type+"的毒气顺手臂袭来，竟然透过你的内劲直抵丹田！\n"NOR);
                write(HIW"这一瞬间，你发觉那至"+poison_type+"的毒气和自己体内原本的毒气相互融合，一同归纳进了气海！\n"NOR);
                tell_room(environment(me), HIY+me->name()+"缓缓收回双掌，笼罩着全身的青气也随之消散了。\n"NOR, ({ me }));
                if(me->query_skill("poison") < 200)
                        me->improve_skill("poison", times * neili_lost * (this_object()->query_temp("liandu")+1));
                switch(poison_type){
                        case "阴": if(me->query("gender") != "男性")
                                        me->improve_skill("poison", me->query_skill("poison") * 20);
                                   break;
                        case "阳": if(me->query("gender") == "男性")
                                        me->improve_skill("poison", me->query_skill("poison") * 20);
                                   break;
                        default: break;
                }                
                ...
                me->add("shen", -3000);  
                me->improve_skill("huagong-dafa", me->query_con() * 10);
                me->apply_condition("xx_liandu", 35);
