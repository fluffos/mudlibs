...
// 判断 player        
        ...
        &&  !ob->query("hostage/passed_x"))
        {
                command("look " + ob->query("id"));
                command("say " + RANK_D->query_rude(ob) + "，想救叛党，哼，先过了我这关！");
...
int do_kill(object me,object ob)
{
        ...
        k_neili = (int)ob->query("max_neili") - 1500;
        j = (int)ob->query("dex");
        k = (int)ob->query("str");

        killerskill1 = skill2[random(sizeof(skill2))];
        number = (int)(number - random(20) - 20);
// 随机 skill set
        number_t = number - 20;
...
//        int i,j;
        ...
        {
        	ob->revive(1);
            	environment(ob)->set_temp("killed",1);
...
*/
		if (!me->query_temp("hostage/job1_killed"))
			me->set_temp("hostage/job1_killed",1);
		else
			me->add_temp("hostage/job1_killed",1);
       		destruct(ob);
       		return;
	}
