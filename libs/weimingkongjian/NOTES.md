...
`mudlib directory : ./world` directives) — the slug `weimingkongjian` is
...
shujian2008/shujiantianxia) and its body text is actually about a
...
(archives #46 `xiyangzaixian_fengkuang` and #47
`xiyangzaixian_fengyun2`). Confirmed via md5sum: `adm/simul_efun/chinese.c`
is **byte-identical** to `xiyangzaixian_fengkuang`'s copy
...
6. **AGENTS.md §8b** (new mudlib-bug instance, same shape as the
   `xingzhanyingxiong` finding): `message_combatd(msg, me, victim)` is
...
directory` points at `libs/weimingkongjian/work` (absolute path). All
