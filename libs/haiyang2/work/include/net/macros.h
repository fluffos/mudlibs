...
// §15p fix (new manifestation): Mud_name() used to lazily load_object the
// dns_master intermud daemon on ANY call (channeld.lpc/logind.lpc/etc all
// call it unconditionally, well outside of preload) -- its create() does
// real resolve()/socket_create() work against an unreachable "boot server"
// in this environment (see AGENTS.md §15p). DNS_MASTER's own
// query_mud_name() just returns the constant INTERMUD_MUD_NAME (see
// adm/daemons/network/dns_master.lpc) -- return that directly, byte-for-
// byte equivalent, without ever touching dns_master.
