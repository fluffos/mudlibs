...
`ffxymud`（非凡夕阳MUD/时空游侠录）、`jhfy2`（江湖风云2）和
`xysylmhb`（夕阳三-炎龙美化版，标题带"夕阳再现"但地图其实是这个
家族）——这五者的 `d/city/sj.lpc`（"世界之巅"跳崖场景）逐字节完全
相同，`ffxymud`/`xysylmhb` 连登入横幅/标题都完全不像同一批档案，
如果不比对源码很难发现它们同源；`wlhd` 这个房间的
