...
int perform(object me, object target)
{
        ...
        if( !target || !target->is_character()
           || !me->is_fighting(target) || !living(target) )
                return notify_fail("醉里乾坤只能对战斗中的对手使用。\n");

         if( (int)me->query_skill("pomo-jianfa", 1) < 300 )
                return notify_fail("你的泼墨披麻剑法还不够熟练，不能使用醉里乾坤对敌！\n");
       
        if (!objectp(weapon = me->query_temp("weapon"))
        || (string)weapon->query("skill_type") != "sword")
                return notify_fail("你使用的武器不对。\n");

             if (me->query_skill_mapped("sword") != "pomo-jianfa")
                return notify_fail("你的剑法中无此功夫。\n");
       
             if( (int)me->query_skill("literate", 1) < 900 )
                return notify_fail("你的读写太低，领悟不到醉里乾坤闪奥义。\n");
/*
             if (!(me->query("meizhuang/zuili_perform") & 1))
              return notify_fail("你虽然听说过“醉里乾坤”这一招，可是却未获传授。\n");
*/
        if( (int)me->query("max_neili", 1) < 5500 )
                return notify_fail("你现在内力不足，使不出最高绝学醉里乾坤！\n");     

        if( (int)me->query("neili", 1) < 5000 )
                return notify_fail("你现在真气不足，使不出醉里乾坤！\n");     

        if( (int)me->query_skill("sword", 1) < 180 )
                return notify_fail("你的剑法不够娴熟，使不出醉里乾坤的绝招！\n");

        if (me->query_dex() < 32 )
                return notify_fail("你的速度太差，使不出醉里乾坤！\n");

        msg = WHT "\n$N将兵器收了回来，凝神闭气,突然左脚踏前闪电般向$n劈去。正是最高剑决[醉里乾坤]！\n"NOR;        
        ...
        {                     
                ...
                target->receive_wound("qi", damage/2);
                  ...
                  p = (int)target->query("qi")*50/(int)target->query("max_qi");
...
if  (! target ) 
     return notify_fail("对方已经不在了。\n");
            ...
            me->query_skill("pomo-jianfa", 1) > 149 )
        {
                ...
                p = (int)target->query("qi")*80/(int)target->query("max_qi");
