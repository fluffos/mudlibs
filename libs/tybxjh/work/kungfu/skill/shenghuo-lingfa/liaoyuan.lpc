...
int perform(object me, object target)
{
 ...
 ob = me->select_opponent();  
  if ( (string)me->query("family/family_name") != "明教") 
      return notify_fail("只有明教弟子才能使用该PFM。\n");
    if( !me->is_fighting() )
        return notify_fail("「圣火燎原」只能在战斗时使用。\n");
    ...
    if( (int)me->query_skill("sword") < 100 ||
        me->query_skill_mapped("sword") != "shenghuo-lingfa")
        return notify_fail("你的「圣火令法」还不够精纯，无法使用「圣火燎原」！\n");
msg = HIR "$N脚踏乾坤五行，手中剑器一卷，突然放出数道红色剑芒分刺向前！！\n" NOR;
    message_vision(msg, me);
    me->clean_up_enemy();
    me->add("neili", -150);
    me->set_temp("perform",time()+3);
    me->set_temp("perform_liaoyuan",1);
    for(i = 0; i < 3; i++)
        if (me->is_fighting(ob) && ob->is_fighting(me) && ob->query("eff_qi")>0){
              me->set_temp("action_msg",HIW"圣火熊熊\n"NOR);
                if (!weapon->query("equipped")) break;
              COMBAT_D->do_attack(me, ob,weapon, 0);
            }else break;
    me->delete_temp("perform_liaoyuan");
