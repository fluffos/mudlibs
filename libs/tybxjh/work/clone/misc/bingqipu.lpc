...
string query_save_file() { return DATA_DIR + "bingqipu"; }
...

{
       ...
       set_name(HIC "江湖兵器谱" NOR, ({ "bingqi pu","bingqi","pu" }) );

        

        set("unit", "本");

       set("long", HIY"这是一本流传久远的江湖兵器谱，里面记下了江湖上的成名兵器。



可以用(read pu)来翻看记载，

可以用(checks bingqi)来评定自己兵器的排名。
...

{
       ...
       if ( !arg || arg != "pu" )
               ...
               return notify_fail("格式read pu !？\n");



         write(HIC"                      天涯兵器谱(Bingqi pu)                   \n"
...

{
        ...
        if ( !arg || arg != "bingqi" )
               ...
               return notify_fail("格式checks bingqi !？\n");
