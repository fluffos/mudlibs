...
#
# Like fluffos/xkx100, fluffos/nt7, and fluffos/sanguozhi, this repo's
# mudlib root IS the repo root itself (config.txt: "mudlib directory :
# ./"), so staging is just "every git-tracked file except scripts/ and
# .github/" -- no lib/ subdir to select. Note this repo already ships its
# OWN native websocket web client at secure/www/ (used for a native,
# non-WASM boot); this script still copies the shared driver release's
# richer multi-session index.html over it for the WASM build, same as
# every other repo in this set -- the two are independent deployments.
#
...
STAGE=$(mktemp -d "${TMPDIR:-/tmp}/imudpack.XXXXXX")
...
#        in-image path here) --------------------------------------------
python3 - "$REPO_ROOT/config.txt" "$STAGE/mudlib/mudlib.cfg" <<'PYEOF'
import re, sys
