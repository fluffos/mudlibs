...
《逸俠之世界》（Daniel's World），ES II 引擎家族，台湾出品。和
`libs/yxsj` 是近亲关系（同样的标题、同样的联络信箱
daniel@tpts4.seed.net.tw、同样的 bug 特征、同样的档案结构），但
不是完全相同的档案——即使去掉空白/CRLF 差异后内容仍有差别（比如
`adm/daemons/combatd.lpc` 差了大约 80 行），这份档案的
`securityd.lpc` 存档路径检查本来就是对的（`yxsj` 那份需要修），说
明这是同一份代码库在不同时间点的另一个版本，不是重新打包的同一批
档案。
...
- 和 `yxsj` 是同一份代码库在不同时间点的两个版本：同样的校园开局
  ...
  值得都保留而不是去重。
- 同样是 BIG5 编码血统，踩中和 `yxsj` 一模一样的转档乱码事故（详
  见下方说明），修法也完全相同——这份档案是在处理完 `yxsj` 之后，
  用已经扩充过来源编码参数的转档脚本直接重转的。
