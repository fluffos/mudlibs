...
int do_sai (string arg)
{
	...
	if (what != "cs" && what != "qn" && what != "bl")
		return notify_fail("你要押什么样的龟？\n");

	ob = present (money+"_money", me);
	if (! ob)
		return notify_fail("你身上没有这种货币。\n");
	if( money !="gold") return notify_fail("小店只收黄金来当赌资。\n");
	if( amount >=  50 ) return notify_fail("太黑心了吧，还是小心些好！\n");
	if (amount < 1)
	...
	message_vision (sprintf("$N拿出%s%s%s放在紫檀龟台上，押%s。\n",
		chinese_number(amount),	ob->query("base_unit"),	ob->query("name"),
		guis[what]), me);
	if (amount == ob->query_amount())
		destruct (ob);
	else
