...
## 深度功能测试（第四轮，2026-08-19）——补完门派加入与经济系统测试，发现并修复新 bug 类别（AGENTS.md §7.116）
...
### 经济系统——发现并修复一个真实、影响面很广的 bug（新增 AGENTS.md §7.116）

- **`feature/dealer.lpc`（`F_DEALER`）混入类的商人 NPC，`init()` 里只注册
  了 `list`/`buy` 两个指令，`sell`/`value` 被漏掉，导致几乎所有商店只能买
  不能卖**：`feature/dealer.lpc` 本身完整实现了 `do_list`/`do_buy`/
  `do_sell`/`do_value` 四个函式，但混入类本身不会自动 `add_action`——每一
  个具体的商人 NPC 档案要在自己的 `init()` 里手动把全部四个指令逐一注册。
  对 `work/d/*/npc/*.lpc` 里全部 `inherit F_DEALER` 的档案做了一次全库扫
  描：**78 个商人 NPC 里有 72 个只注册了 `list`/`buy`，完全漏掉了
  `sell`/`value`**，只有 6 个（`dangpuzhang`、`tang`、`chaofeng`、
  `wanxi`、`wang`、`qiuhang`）正确注册了全部四个指令。另有 3 个
  （`jiaoxi`、`shaofanshifu`、`mianhuoji`）`inherit F_DEALER` 但完全没有
  `vendor_goods` 也没有任何 `add_action`——这是内容层面的"没做完的商人"，
  不是这个 bug，跳过不动。
  - **现场验证症状**：在兵器店（`d/changan/weapon-shop`）用 `buy sword`
    向老板慕容豪买下一把长剑，扣款正确（二两白银）；但 `sell sword` 只得
    到驱动通用的"什么？"回应——指令根本没有被注册，和玩家打错字看起来一
    模一样，`debug.log` 里没有任何痕迹。
  - **修复**：给这 69 个（72 减去 3 个非功能性商人）档案的 `init()`，在
    既有的 `add_action("do_list", "list");`/`add_action("do_buy", "buy");`
    这一对指令后面（以文件里两者中较晚出现的那一行为准，保持各档案原有
    的顺序习惯），补上：
    ```
    add_action("do_sell", "sell");
    add_action("do_value", "value");
    ```
    用二进制模式脚本化批量插入，逐文件保留原有行尾格式（CRLF/LF 按文件原
    样，用 `file` 命令确认过混合编码的档案没有被破坏）。
  - **现场验证**：全新驱动进程启动干净；对全部 69 个改动过的档案逐一发送
    巫师 `update <path>` 强制重新编译，69/69 全部显示"重新编译...成功！"，
    只有既有的、和本次改动无关的"Unused local variable"警告，没有新增的
    编译错误。完整买卖闭环验证：在兵器店 `buy sword` 扣款正确后，走到茶馆
    （`d/changan/tea-shop`）对茶馆小二 `sell sword`，正确收到"你卖掉了一
    柄长剑给茶馆小二。"，到手四十文铜钱（`长剑` 面值二两白银=200 文的
    70%=140 文，实际到手 49 两白银+40 文铜钱，和之前 48 两白银的余额对得
    上：48两+140文=48两4钱... 更准确地说是原有 48 两白银不变+新增的 140
    文铜钱换算成 1 两白银 40 文铜钱，符合 `do_sell()` 的 `value*70/100`
    公式）。另外确认了一个容易误判为"还是没修好"的正常设计：慕容豪拒绝
    买回长剑本身（"我卖给你好不好？"）——这是 `is_vendor_good()` 里防止
    买卖套利的既有逻辑（不能把商店自己卖的东西原路卖回去），不是 bug，测
    试卖出机制时要换一家不卖该商品的商店，或换一件非商店自卖商品来测。
  - **尚未做跨库扫描**：这个 `F_DEALER` 混入类很可能是本项目 ES2/XKX 血
    统里广泛共享的档案（和 §7.114 的 `feature/edit.lpc` 属于同一类"混入
    类本身没问题，但各档案各自的 `init()` 接线不完整、且不一致"的 bug 形
    状），`hell` 一个档案里 78 个商人有 72 个中招，如果这个混入类在其它档
    案里也很常见，这可能是一个体量很大的跨库扫描候选，已记入 AGENTS.md
    §7.116，留给未来的扫描轮次。

### 检查清单逐项复查结果（均已确认，无异常）

- **§7.111**（`master.lpc` 的 `standard_trace()` `file_name(0)` 崩溃保护）
  ：`work/adm/single/master.lpc` 第 230 行已有
  `error["object"] ? file_name(error["object"]) : "0"` 三元保护，确认已修
  复到位。
- **§7.112**（死亡阶段 `call_out` 重入保护）：`d/death/npc/bgargoyle.lpc`
  和 `wgargoyle.lpc` 都已经有 `query_temp("death_stage_active")`/
  `set_temp`/`delete_temp` 的重入保护，确认已修复到位；全库没有找到其它
  别名下的 `call_out("death_stage"` 实例。
- **§7.113**（netdead 重连不恢复 `heart_beat`）：`hell` 本来就在
  AGENTS.md 记载的"62 个候选库里已确认干净的第一批 15 个"名单中，本轮重
  新静态复查确认依旧成立——`adm/daemons/logind.lpc::reconnect()` 会呼叫
  `user->reconnect()`，`clone/user/user.lpc::reconnect()` 里无条件
  `set_heart_beat(1)`，链路完整、没有断点。
- **§7.114**（`private` 函式当 `input_to()` 回调经由混入类失效）：
  `feature/edit.lpc` 里的 `input_line()` 没有 `private` 修饰符，确认这份
  档案不在受影响之列。
- **§7.115**（`QUEST` 宏指向不存在的档案）：`include/quest.h`/
  `include/globals.h` 里没有形如 `#define QUEST "<path>"` 的裸档案路径宏
  （只有 `QUEST_OB`/`QUEST_DIR`/`QUEST_D` 等不同用途的宏），此 bug 形状在
  这份档案上不适用。
- **§7.90**（eval-cost 默认值）：`config.fluffos` 第 39 行已是
  `5000000`，确认维持在第三轮修复后的安全值。

### 清理说明

测试全程只使用既有的 fluffos 管理员测试账号，没有新建任何测试账号存档目
录。fluffos 账号上因测试产生的临时状态（丐帮师门关系、`str`/技能数值）已
在测试后清理并确认落盘；测试中通过 `pay_player()` 发放的少量测试货币（银
两/铜钱）因为该账号从未真正"投胎"（出生），存档本身不持久化物品栏，重启
驱动后自动不再出现，无需额外清理。
