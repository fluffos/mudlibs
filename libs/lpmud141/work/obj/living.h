...
hit_player(dam) {
	...
	corpse->move_object(resolve_ob(environment(this_object())));
...
transfer_all_to(dest)
{
	...
	if (!call_other(ob, "drop", 1))
	    ob->move_object(resolve_ob(dest));
	ob = next_ob;
    }
    local_weight = 0;
    if (money == 0)
...
drop_all_money(verbose) {
    ...
    mon->move_object(resolve_ob(environment()));
    if (verbose) {
