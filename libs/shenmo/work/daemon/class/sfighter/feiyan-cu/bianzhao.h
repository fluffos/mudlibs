...
if ( target->query_temp("fly") )  //浮空击 必中
	...
	for ( i=0;i<time;i++ )
		{
		if ( random(me->query("combat_exp")/3)>random(target->query("combat_exp")/3) )
			{
			damage = damage*3/4;
...
if ( !target || !me->is_fighting(target) )
	{if( !me->query("env/no_msg") && !target->query("env/no_msg") ) message_vision(RED"$N见已无敌手，收起了招式。\n"NOR,me);return 1;}
