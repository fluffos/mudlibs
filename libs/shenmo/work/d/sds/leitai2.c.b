...
void alternative_die(object me)
{
  ...
  if (!userp(me)&&me->query("id")=="bing he") {

     object ob=(me->query_temp("my_killer"));
      fullup(ob);

    if( present("sai zhang",ob) && ob->query("sds/tiane")!="done" )  
	{  
	message("chat",HIY+"【银河战役】沙织(Sha zhi):"+ob->name()+"打败了天鹅座：冰河!\n"NOR,users());  
	ob->set("sds/tiane","done");  
	ob->add("sds/number",1);  
	...
	me->move("/d/sds/saichang");
          message("chat",HIY+"【银河战役】沙织(Sha zhi):"+me->name()+ "败在了天鹅座：冰河的手下!\n"NOR,users());  
