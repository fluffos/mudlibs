...
void powerup()
{
        ...
        random(2)?
        message("channel:chat",HIC"【银河界】"NOR HIC+me->query("name")+HIC"("+capitalize(me->query("id"))+")：燃烧吧，小宇宙！\n"NOR,users()):
        message("channel:chat",HIC"【银河界】"HIC+me->query("name")+HIC"("+capitalize(me->query("id"))+")：小宇宙！爆发吧！\n"NOR,users());
