//add level 4  edit by huarong 2003.10
...
{       
	int zuochan_time = me->query_temp("zuochan_time");
        if ( zuochan_time>0 && time()-zuochan_time>0) finish(me);
...
me->start_busy(2);
        ...
        me->set_temp("finish",me->query("rulaitask/finish"));//huarong 2003.7
        me->set_temp("dfinish",me->query("rulaitask/dfinish"));//huarong 2003.7
        if (me->query("rulaitask/accomplish")) 
	{
		me->add_temp("rulaifin",1);
		reward(me);
		me->add_temp("finish",1);
	}
	else
		me->set_temp("rulaifin",0);
        ...
        me->set("rulaitask/finish",me->query_temp("finish"));//huarong 2003.7
        me->set("rulaitask/dfinish",me->query_temp("dfinish"));//huarong 2003.7
}

void reward(object me)
{
	int i,j,wuxue,level,kind_bonus,level_bonus,timep;
...
if(extra_bonus > 900) extra_bonus = 900;//huarong 2003.10
	...
	combat_exp_bonus=combat_exp_bonus;
	daoxing_bonus =daoxing_bonus;

/*        me->add("potential", potential_bonus);
        me->add("combat_exp", combat_exp_bonus);
        me->add("daoxing", daoxing_bonus);
*/
	combat_exp_bonus=EXP_D->addexp(me,combat_exp_bonus,"rulai");
	daoxing_bonus=EXP_D->adddx(me,daoxing_bonus,"rulai");
	potential_bonus=EXP_D->addpot(me,potential_bonus,"rulai");
//added by mudbug on 2003/3/15 

	if(me->query_temp("rulaifin")>=5)
	{
	        me->add("potential", 2000);
	        me->add("combat_exp", 8000);
	        me->add("daoxing", 8000);
        i=random(20);j=random(10);
        
        switch(random(2))
        {
        	case 0:str=i+"+"+j;
        		me->set("rulai/trig",i+j);
        		break;
        	case 1:str=i+"-"+j;
        		me->set("rulai/trig",i-j);
        		break;
        ...
        tell_object(me,"宝石天使让你请回答以下问题,用answer 答案回答！\n");
        tell_object(me,"/adm/daemons/text2pic_d"->Display(str));
        me->start_busy (1);
	}
...
//        if ( level > 1 ) CHANNEL_D->do_channel(me, "rumor",str);
        if ( level > 1 ) message("channel:rumor", HIM"【长安夜话】某人："+HIM""+str+"\n"NOR,users());
...
int do_answer(string arg)
{
   ...
   i=this_player()->query("rulai/trig");
   if(i==-100 || this_player()->query_temp("rulaifin")<5)
   	return notify_fail("你目前没有任何问题！\n");	
   this_player()->set("rulai/trig",-100);
  this_player()->set_temp("rulaifin",0);
    if(result!=i)
   {
    	return notify_fail("你回答错误，得不到任何奖励！\n");
   }   
   else
   {
        baoshi=new(BAOSHIOBJ);
        baoshi->move(who);
        message("channel:rumor",HIY"【"+HIR" 如来佛 "NOR+HIY"】："+who->query("name")+"不辞辛苦降妖除魔，赐的"+baoshi->query("name")+"一颗！\n"NOR, users() ); 
	if(random(150)==0)
