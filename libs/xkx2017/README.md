...
侠客行（"The Story of Hero"），MudOS v21 世系——和本项目里的 `xkx100`、`xkx2000zxb` 是同一核心代码库家族（`combatd.lpc`/`quest.h` 的结构一眼就能看出来），但地图明显更精简：华山、武当、少林、丐帮、峨嵋、明教等门派齐全，却少了 `xkx100` 那种漠北、哈萨克、台湾式的边疆场景，更像一个精简复刻版本；和同家族的 `xkx2000zxb` 不同，这份档案的管理员权限走标准的 `wizlist` 文本文件机制，没有站点 IP 锁定检查，架设起来更省心。
