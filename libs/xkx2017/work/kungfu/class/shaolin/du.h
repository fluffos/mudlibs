...
void attempt_apprentice(object ob)
{
	...
	if (ob->query("family/generation") == 37)
	{
		...
		if ((int)ob->query("str")<50)
		  ob->add("str",1);
		if ((int)ob->query("int")<50)
		ob->add("int",1);
		if ((int)ob->query("con")<50)
			ob->add("con",1);
		if ((int)ob->query("dex")<50)
			ob->add("dex",1);
//		ob->add("max_neili",100);
	
		command("say 从今以后你的法名叫做" + new_name + "，恭喜你荣升为少林派玄字辈大师!");
		command("recruit " + ob->query("id"));
		return;
	}
	command("say 你难道不懂少林拜师的规矩么？");
	return;
}
