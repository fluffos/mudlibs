...
int do_wancheng(string arg)
{
	...
	if(!me->query_temp("kill_by")) return notify_fail("你晚了一步，目标已经被人杀了。\n");
	...
	if ((int)me->query("combat_exp")<3000000)
	{
		...
		me->add("score",score);
		tell_object(me,HIY"你被奖励了：\n" + chinese_number(exp) + "点实战经验\n"+ chinese_number(pot) + "点潜能\n"+ "江湖阅历增加了"+chinese_number(score)+"点\n"NOR);
		destruct(this_object());
		return 1;
	}
	if ((int)me->query("combat_exp")>3000000 && me->query("combat_exp")<15000000)
	{
	...
	if ((int)me->query("combat_exp")>15000000 && me->query("combat_exp")<50000000)
	{
	...
	if ((int)me->query("combat_exp")>50000000 && me->query("combat_exp")<150000000)
	{
	...
	if ((int)me->query("combat_exp")>150000000 && me->query("combat_exp")<500000000)
	{
