...
int do_jie(string arg)
{
	...
	me = this_player();
	ob = this_player(); 
	exp=(int)me->query("xyzx_sys/level",1); 
	weizhis = wheres[random(sizeof(wheres))];

	if(arg != "wenshu" && arg != "wen") return 0;
	if(me->query_condition("gb_mission") || me->query_condition("gb_busy") || me->query_condition("xx_task") || me->query_condition("dali_songxin"))
	...
	if(exp>10000) position=5;
	else if(exp>5000) position=4;
	else if(exp>2000) position=3;
	else if(exp>300) position=2;
	else position=1;
