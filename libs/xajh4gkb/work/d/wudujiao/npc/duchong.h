...
private nosave mapping chongdu=([
	"du she" : __DIR__"obj/shedu.lpc",
	"zhi zhu" : __DIR__"obj/zhizhudu.lpc",
	"wu gong" : __DIR__"obj/wugongdu.lpc",
	"chan chu" : __DIR__"obj/chanchudu.lpc",
	"xie zi" : __DIR__"obj/xiezidu.lpc",
    ]);
...
int do_attack(string arg)
{
    ...
    if(environment(ob)->query("short") == "武庙" || environment(ob)->query("short") == "文庙")
	return notify_fail("大胆！在神像面前也敢胡来？！\n");	
	if( environment(ob)->query("no_fight") || environment(me)->query("no_fight"))
...
int do_stop()
{
	...
	if (!wizardp(me)) return notify_fail("放虫出来咬了人还想休战？？？\n");
    if(me->query("id")!=(who->query("host_id")))
	return notify_fail(who->query("name")+"连理都不理你！\n");
    who->set_leader(me);
...
int do_guard(string arg)            //160
{
	...
	if (!wizardp(me)) return notify_fail("毒虫的保护功能永久性关闭。\n");
    if(me->query("id")!=(who->query("host_id"))) return notify_fail(who->query("name")+"连理都不理你！\n");
    if(!arg) ob = this_player();
    if(arg) ob = present(arg,environment(me));
    if(!ob) return notify_fail("你要"+who->query("name")+"保护谁？\n");
    if(arg)
	{
		if(!userp(present(arg,environment(me)))) return notify_fail("你要"+who->query("name")+"保护谁？\n");
    }
