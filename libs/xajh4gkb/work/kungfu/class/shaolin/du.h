...
void attempt_apprentice(object ob)
{
    ...
    if (stringp(ob->query("marry"))){
        command("say 这位" + RANK_D->query_respect(ob) + "尘源未了，岂能入我空门？");
        return;
    }
    ...
    if (ob->query("family/generation") == 37)
    {
        if ((int)ob->query_skill("dodge", 1) < 1000 ||
            (int)ob->query_skill("parry", 1) < 1000 ||
            (int)ob->query_skill("force", 1) < 1000 ||
            (int)ob->query_skill("buddhism", 1) < 1000 ||
            (int)ob->query_skill("hunyuan-yiqi", 1) < 1000 ||
            (int)ob->query_skill("shaolin-shenfa", 1) < 1000) {
