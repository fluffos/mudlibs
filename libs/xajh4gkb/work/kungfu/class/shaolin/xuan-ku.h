...
//		return RANK_D->query_respect(fighter) +  "已然闯过罗汉大阵，可不要拿老衲开这等玩笑。";	
    if( fighter->query_temp("luohan_winner") )
        return RANK_D->query_respect(fighter) + "已然闯过罗汉大阵，可不要拿老衲开这等玩笑。";
    if ( mapp(fam = fighter->query("family")) && fam["family_name"] == "少林派" && !fighter->query_temp("jm_shaolin6") && fam["generation"] != (my_fam["generation"] + 2))
        return RANK_D->query_respect(fighter) + "辈份不合，不够资格闯罗汉大阵。";
	if ( (int)fighter->query("guilty") > 0 )
        return RANK_D->query_respect(fighter) + "你累犯数戒，身带重罪，我如何能准许你闯罗汉大阵！";
    if ( me->query("assigned_fighter") )
        return RANK_D->query_respect(fighter) + "，今日已有人挑战罗汉大阵，你过一段时间再来吧。";
	...
	for(j=1; j<10; j++) 
    {
		if(!( room = find_object("/d/shaolin/luohan" + j)) ) room = load_object("/d/shaolin/luohan" + j);
		me->move(room);
...
int preparing(object me, object fighter)
{
    ...
    if (monk)   monk->move("/d/shaolin/houdian");
...
int fighting(object me, object fighter, int count)
{
    ...
    if( !find_player(me->query("assigned_fighter")) || !objectp(present(me->query("assigned_fighter"), environment(me)) ) || !living(fighter) || fighter->query("qi") <= fighter->query("max_qi")*30/100 || fighter->query("qi") <=1)
    {
    ...
    else if( count >= 9 )
    {
        if( (int)fighter->query_temp("beat_count") >= 18 ){
            command("chat 恭喜"+fighter->name(1)+"闯过了十八罗汉大阵！ 届时必当为武林放一异彩！\n");
...
//	fighter->set("luohan_winner", 1);
    fighter->set_temp("luohan_winner", 1);
	lev_add = 20+200000/((int)fighter->query("xyzx_sys/level",1)+1000);
	if (!(int)fighter->query("array/luohan_exp"))
	{
		fighter->add("potential", 10000000);
		fighter->add("xyzx_sys/level", lev_add, 1);
		fighter->set("array/luohan_exp", 1);
		message("vision", HIR"恭喜你通过了罗汉阵考验，你的实战能力提高了！\n"NOR,fighter);
	}	
	if ((int)fighter->query_temp("jm_shaolin6"))
	{//少林派解密
		fighter->set_temp("jm_shaolin7",1);
		message("vision", HIM"你通过了罗汉阵考验，可以去达摩洞拜访达摩老祖。\n"NOR,fighter);
	}	
...
int do_back(object me)
{
	...
	if(!( room1 = find_object("/d/shaolin/wuchang")) ) room1 = load_object("/d/shaolin/wuchang");
	if(!( room2 = find_object("/d/shaolin/houdian")) ) room2 = load_object("/d/shaolin/houdian");
	...
	if(!( room1 = find_object("/d/shaolin/wuchang2")) ) room1 = load_object("/d/shaolin/wuchang2");
    for(i=1; i<10; i++) 
    {
		if(!( room2 = find_object("/d/shaolin/luohan" + i)) ) room2 = load_object("/d/shaolin/luohan" + i);
...
void attempt_apprentice(object ob)
{
    ...
    {
        if ( ob->query_temp("luohan_winner") )
        {   
