...
//		return RANK_D->query_respect(fighter) +  "已然闯过罗汉大阵，可不要拿老衲开这等玩笑。";	
	...
	for(j=1; j<10; j++) 
    {
		if(!( room = find_object("/d/shaolin/banruo" + j)) ) room = load_object("/d/shaolin/banruo" + j);
        me->move(room);
...
int preparing(object me, object fighter)
{
    ...
    message("vision", HIY "\n玄难大师大声宣布：西罗汉大阵合围！\n\n" NOR, room);
    command("chat " + fighter->query("title") + fighter->query("name") +  "于今日" + NATURE_D->game_time() + "挑战少林十八罗汉大阵！\n");
...
//	fighter->set("luohan_winner", 1);
    ...
    {
		say("玄难说道：好，好，好！" + fighter->query("name") + "，你愿意拜我等为师吗？\n");
...
int do_back(object me)
{
	...
	if(!( room1 = find_object("/d/shaolin/wuchang1")) ) room1 = load_object("/d/shaolin/wuchang1");
    for(i=1; i<10; i++) 
    {
		if(!( room2 = find_object("/d/shaolin/banruo" + i)) ) room2 = load_object("/d/shaolin/banruo" + i);
