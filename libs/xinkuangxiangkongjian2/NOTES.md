# xinkuangxiangkongjian2 — 狂想空间 (Fantasy Space) II
...
convention (no separate slug rename needed; `xinkuangxiangkongjian2` is
...
(#25)/`rzrmud`(#23)/`beimeixiakexing2001`(#45) — `master.c`'s header
...
7. **NEW gap, `..`-relative `#include` (driver disallows `..` in
   ...
   - **27 files** under `/open/cold/*.lpc` used `#include "../cold.h"`
     ...
     patterned missing macro header" precedent as `xiyangzaixian3`'s
...
`scripts/lpcc_check.sh libs/xinkuangxiangkongjian2/config.fluffos
libs/xinkuangxiangkongjian2/work`, run twice:
