...
//	if(!is_granted(me)) return notify_fail("你没有被授权!\n");
	if(!__DIR__"userchecker.lpc"->is_granted(me)) return notify_fail("你没有被授权!\n");
	...
	sample_filename="/u/mudy/sampleitem.lpc";
	if(file_size(sample_filename)<0)
	{
		sample_filename=__DIR__+"sampleitem.lpc";
		if(file_size(sample_filename)<0)
		{
	...
	if(sscanf(filename,"%s.lpc",this_file)!=1)
		filename=filename+".lpc";
	...
	insert_string(file_name(this_object())+".lpc","//OBJECT","\""+filename+"\" : 1,","//OVEROBJECT");
...
int get_itemdescribe(string arg)
{
	...
	if(!arg||arg=="")
	{
		tell_object(me,HIW"请输入物品的单位：\n"NOR);
		input_to((:get_itemunit:));
		return 1;
	}
...
int get_itemunit(string arg)
{
	...
	if(i!=1 )
	{
		tell_object(me,HIW"对不起，物品的单位必须是 1  个中文字。\n"NOR);
		input_to((:get_itemunit:));
		return 1;
	}
	while(i--) {
		if( arg[i]<=' ' ) {
			tell_object(me,HIW"物品的单位不能用控制字元。\n"NOR);
			input_to("get_itemunit",me);
			return 1;
		}
		if( !is_chinese(arg[i..<0]) ) {
			tell_object(me,HIW"对不起，请您用「中文」做单位。\n"NOR);
			input_to("get_itemunit",me);
			return 1;
		}
	}
	change_string(filename,"/*UNIT*/%s","/*SET_UNIT*/\""+arg+"\"");
...
int do_create_npc(string arg)
{
	...
	sample_filename="/u/mudy/samplenpc.lpc";
	if(file_size(sample_filename)<0)
	{
		sample_filename=__DIR__+"samplenpc.lpc";
		if(file_size(sample_filename)<0)
		{
			tell_object(me,"样本文件丢失!\n");
			return 1;
...
int get_npcid(string arg)
{
	...
	if(!arg||arg=="")
	{
		tell_object(me,HIW"请输入NPC的id(e.g. huang rong,huang,rong)：\n"NOR);
		input_to((:get_npcid:));
		return 1;
	}
	i=strwidth(arg);
...
int get_npcmsgchat(string arg)
{
	...
	insert_string(filename,"//MSG_CHAT","/*MSG_CHAT*/\""+arg+"\\n\",","//OVERMSG_CHAT");
...
int get_npcskill(string arg)
{
	...
	if(file_size("/kungfu/skill/"+skill+".lpc")<1)
	{
...
int do_del_describe(string arg)
{
	...
	change_string(file_name(this_object())+".lpc","/*DESCRIBE*/%s","//DESCRIBE");
	set("long","");
...
int do_add_describe(string arg)
{
	...
	insert_string(file_name(this_object())+".lpc","//DESCRIBE","/*DESCRIBE*/\""+arg+"\\n\"","//OVERDESCRIBE");
...
int do_del(string arg)
{
	...
	change_string(this_filename,"\""+arg+"\"%s","//EXIT");
	that_filename=file_name(to_room)+".lpc";
...
int do_link(string arg)
{
	...
	if(arg)
	{
		dirlength=strwidth(arg);
		while(dirlength--)
		{
			if(arg[dirlength]<=' ')
			{
				tell_object(me,HIW"请不要用控制字符（特别是空格）做路径名!\n"NOR);
				return 1;
			}
			/*if(arg[dirlength]<='9'&&arg[dirlength]>='0') continue;
			if(arg[dirlength]<'a'||arg[dirlength]>'z')
			{
				tell_object(me,HIW"请用英文和数字做路径名!\n"NOR);
				return 1;
			}*/
		}
		this_filename=file_name(this_object())+".lpc";
...
int do_cut(string arg)
{
	...
	for(i=0;i<sizeof(line);i++)
	{
		...
		if(sscanf(line[i],"%s//DEL",temp1)==1) continue;
		if(sscanf(line[i],"/*%s*/%s",temp1,temp2)==2)
		{
			des_line+=({temp2});
			continue;
		}
...
int do_write(string arg,string dir,object me)
{
	...
	sample_filename="/u/mudy/sample.lpc";
	if(file_size(sample_filename)<0)
	{
		sample_filename=__DIR__+"sample.lpc";
		if(file_size(sample_filename)<0)
		{
			tell_object(me,"样本文件丢失!\n");
			return 1;
	...
	if(!__DIR__"userchecker.lpc"->is_granted(this_player())) 
	{
...
int get_longname(string arg)
{
	...
	{
		rooms[i]->add("exits/"+convert_dir(dir),file_name(this_object())+".lpc");
