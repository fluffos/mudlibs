...
{
    message( "channel:" + "baowei",HIM + "【谣言】蒙古奸细混入帅府！\n"NOR,users());
return 1;
}
if(query("victory"))
{
...
{
object xytaskd;
xytaskd=load_object("/adm/daemons/xytaskd.lpc");
xytaskd->set("begin",0);
...
obj->die();
                ...
                message( "channel:" + "baowei",HIR + "【襄阳战报】郭靖夫妇被蒙古奸细杀死，帅府被占，襄阳沦陷！\n"NOR,users());
}
return 1;
}
add("index",1);
...
// 沦陷
message( "channel:" + "baowei",HIM + "【襄阳传闻】" + room->query("short") + "被奸细从内部将城门打开！\n"NOR,users());

                message( "channel:" + "baowei",HIR + "【襄阳战报】" + room->query("short") + "沦陷！\n"NOR,users());
(load_object("/adm/daemons/xytaskd.lpc"))->add("occupied",1);
(load_object("/adm/daemons/xytaskd.lpc"))->set(GATE,1);
}
}
...
{
if(sizeof(this_player()->query_enemy())>30)
{
...
message_vision(this_object()->query("name")+"死了,你仔细一看发现他竟然是蒙古奸细假扮的！\n",this_object());
    all_inventory(environment())->remove_killer(this_object());
message( "channel:" + "baowei",HIM + "【襄阳传闻】"WHT +me->query("name")+HIM"锐眼识穿蒙古奸细！\n"NOR,users());
destruct(this_object());
return 1;
}
/*
