...
*/
	...
	if( skill_basic < 1 )
		...
		//判断是否是自创武功
	if( !find_object(SKILL_D(skillname)) && file_size(SKILL_D(skillname)+".lpc") < 0)
		return notify_fail("自创武功必须到闭关室用selfpractice和selfthinking练习。\n");
	if( !SKILL_D(skillname)->valid_learn(me) ) return 0;
    if (ob->query("lian_level")==1)
    {
        bonus=L_BONUS1;
    }
    else if (ob->query("lian_level")==2)
    {
        bonus=L_BONUS2;
    }
    else if (ob->query("lian_level")==3)
    {
        bonus=L_BONUS3;
    }
    else if (ob->query("lian_level")==4)
    {
        bonus=L_BONUS4;
    }
    else
