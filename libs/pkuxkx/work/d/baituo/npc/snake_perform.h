...
int auto_perform_duwu()
{
		...
		if( random(poison) > target[i]->query_skill("force") || !living(target[i]) )
	{
			...
			F_POISON->poison_attack(me,target[i],"snake_poison", 
...
int auto_perform_chan()
{
	...
	me->set("default_actions", (: call_other, "/adm/daemons/race/snake.lpc", "query_action" :));
...
int auto_perform_double()
{
	...
	me->set("actions", (: call_other, __FILE__, "query_action" :));
