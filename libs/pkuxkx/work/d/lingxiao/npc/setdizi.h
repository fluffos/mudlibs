...
// Modified by jpei 调整了下功夫级别设置。05022008
...
//白自在：level = 350
{
        ...
        me->set("max_qi", level * 8+100);
        me->set("eff_qi", level * 8+100);
        me->set("qi", level * 8+100);
        me->set("max_jing", level * 8+100);
        me->set("eff_jing", level * 8+100);
        me->set("jing", level * 8+100);
        me->set("max_neili", level * 14+100);
        me->set("neili", level * 14+100);
        me->set("max_jingli", level * 14+100);
        me->set("jingli", level * 14+100);

        me->set("jiali",100);

        exp = pow(level/10, 3) * 100;
        exp += random((int)exp/2);
        me->set("combat_exp", exp+1000);
        me->set("shen_type", 1);
        me->set("shen", level * 100+1000);

        me->set("attitude", "peaceful");

        set("str", 20+random(14));
        set("dex", 20+random(14));
        set("con", 20+random(14));
        set("int", 20+random(14));
        set("per", 20+random(14));

        me->set_skill("force",    lv2);
        me->set_skill("dodge",    lv2);
        me->set_skill("parry",    lv1);
        me->set_skill("sword",    level+50);
        me->set_skill("strike",   lv3);
        me->set_skill("cuff",     lv2);
        me->set_skill("literate", lv3);
        me->set_skill("xueshan-neigong", level);
        me->set_skill("xueshan-jianfa",  level+50);
        me->set_skill("lingxiao-quan",   lv2);
        me->set_skill("piaoxue-zhang",   lv3);
        me->set_skill("yanyi-shenfa",    lv2);
...
void setequip()
{
        ...
        me->carry_object(__DIR__"obj/sword.lpc");
        me->carry_object(__DIR__"obj/sword.lpc")->wield();
        me->carry_object(__DIR__"obj/changpao.lpc")->wear();
}
