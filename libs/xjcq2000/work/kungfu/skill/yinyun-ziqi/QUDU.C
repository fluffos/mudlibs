...
int exert(object me, object target)
{
        ...
        b = ({""HIB"",         ""HIB"",      ""CYN"",   ""HIG"",       ""BLU"",      ""HIB"",     ""MAG"",          ""HIR"",        ""HIR"",     ""BLU"",     ""HIR"",   ""RED"",      ""HIR"",      ""HIG"",      ""HIB"",     ""HIB"",   ""YEL"" });
...
"$N坐在一旁，慢慢以真气通走三关，鼓荡丹田中的"MAG"「氤氲紫气」"NOR"，将体内的"+pc + pname[poison] + NOR"一丝丝的化掉。\n", me);
        ...
        else
        {
                ...
                if (objectp(myarmor) && objectp(urarmor))
                 {               
                msg = 
"    $N手指连伸，点了$n身上"+ chinese_number((poi_lvl/100)+3) +"处大穴。$p穴道被点，登时不再颤抖，脸上" +pc + cname[pc] + NOR"却愈来愈浓。\n"
...
"$N当下解开" + myarmor->name() + "，胸膛和$p的背心相贴。以"HIY"「纯阳无极功」"NOR"吸取$n身上的"+pc + pname[poison] + NOR"。\n\n";
                ...
                if( !objectp(myarmor) && objectp(urarmor) )
                {
                        urarmor->unequip();
                msg = 
"    $N手指连伸，点了$n身上"+ chinese_number((poi_lvl/100)+3) +"处大穴。$p穴道被点，登时不再颤抖，脸上" +pc + cname[pc] + NOR"却愈来愈\n"
...
int xidu(object me, object target, string poison, string pc, int yyzq_lvl, int poi_lvl, int neili_cost)
{
