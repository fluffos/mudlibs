...
#include <ansi.h>
inherit NPC;
inherit F_DEALER;inherit F_MASTER;
inherit F_UNIQUE;
...
int ask_me()
{
        ...
        {
                message_vision("薛神医拿出一根银针轻轻捻入$N受伤的部位附近的穴道，$N感觉舒服多了。\n", ob);
...
int ask_quest()
{
        ...
        if (me->query_skill("jingmai-xue",1)>=140) {
                command("say 学医者要有任道之心，你是否意帮我去救个人？");
                return 1;}
...
int do_teach(string arg)
{
        ...
        target = present(ob,environment(me));
        if (target->query("id") != "xue muhua")
...
void attempt_apprentice(object ob)
{
	...
	{
		command("say 你的悟性太低了，怎能做我薛幕华的徒弟。");
		return 0;
	}
	if((string)ob->query("gender")=="无性")
	{
		command("say 你阳气不足，我可不收不男不女的人妖。");
		return 0;
	}
	...
	command("say 你以后就是我神医薛幕华的徒弟了哈哈哈。");
        command("chat 如此，我便告知天下，你今后就是我神医薛幕华的徒弟。");
	command("recruit " + ob->query("id"));
	ob->set("title", HIR"神医弟子"NOR);
	return;
}