...
**与 `kxkj`（036 号，"狂想空间"）的血统关系**：`diff` 核对确认两者
的 `adm/obj/master.lpc`、`adm/daemons/securityd.lpc` 及整个
`open/death/` 死亡区目录逐字节完全相同——这不是"狂想空间"的改编/
续作，而是同一份 2002 年台湾巫师 wade 发布源码的另一份独立存档快
照，档案总数仅差 10 个文件（均为运行期存档差异，非源码）。两份档
案各自独立命名的"雪村"新手区内容不同（见下），说明是各自基于同一
份引擎源码分别搭建的实例，而非彼此改写。
