...
int valid_learn(object me)
{
          if ((int)me->query_skill("shenyuan-gong", 1) < 100)
                  return notify_fail("你的神元功火候还不够，不能学习斗转星移。\n");
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
        ...
        // 斗转星移反伤效果
        if (lvl > 280 &&
            me->query_skill("shenyuan-gong", 1) > 300 &&
            ! me->is_busy() &&
            ...
            random(ran) == 1 )
        {
                msg = HIM "\n$N" HIM "默运伸元功，施展起绝学「斗转星移」，一股真气在体内迅速惯行，"
                      "巧妙地牵引着$n" HIM "的招式。\n" NOR;
                ...
                if (dp / 2 + random(dp) < ap)
                {
                      ...
                      msg += COMBAT_D->do_damage(me, ob, REMOTE_ATTACK, damage, 200,
                                                 HIR "$n" HIR "大吃一惊，不料$N" HIR "竟将"
                                                 "$p" HIR"先前的招式尽数折回反攻向自己。猛"
