...
int follow_path(string dir, object last_followed)
{
//      if( (origin()==ORIGIN_LOCAL) || (origin()==ORIGIN_CALL_OUT) ) {
                this_object()->remove_all_enemy();
                return GO_CMD->main(this_object(), dir, last_followed);
//      }
}

int follow_me(object ob, string dir, object last_followed)
{
        if( !living(this_object()) || ob==this_object() ) return 0;

        if((ob==leader || (query("pursuer") && this_object()->is_killing(ob->query("id")))) && this_object()!=last_followed) {
                last_followed=ob;
                if( random(ob->query_skill("dodge")) > this_object()->query_skill("dodge") ) {
                        remove_call_out("follow_path");
                        call_out("follow_path", 1, dir, last_followed);
                        return 1;
                }
                return follow_path(dir, last_followed);
        }
}
