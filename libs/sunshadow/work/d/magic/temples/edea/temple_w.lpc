...
void create(){
    ...
    set_long("%^RESET%^%^CRST%^%^C034%^An overtaking %^C046%^sprawl%^C034%^ of %^C046%^lush%^C034%^ and %^C046%^green growth%^C034%^ overtakes the scenery here, %^C046%^vines%^C034%^ and %^C046%^roots%^C034%^ trailing along the %^C229%^sandy%^C034%^ ground with no explanation of how this growth survives under the %^C124%^oppressive heat%^C034%^, still, many %^C214%^flowers%^C034%^ bloom across the %^C046%^vine%^C034%^s and %^C130%^roots%^C034%^, bringing %^C196%^colour%^C034%^ to the %^C229%^sandy%^C034%^ environment. Overgrown, white, %^C226%^elven walls%^C034%^ that hold a hue of %^C164%^senzokuan%^C034%^ architecture, have been erected around the proximity to the %^C171%^west%^C034%^, warding off against %^C124%^clandestine%^C034%^ %^C160%^winds%^C034%^ and %^C039%^weather%^C034%^. A %^C228%^path%^C034%^ leads through the %^C046%^growth%^C034%^ from the %^C130%^west%^C034%^, where the walls connect to a gateway, %^C130%^east%^C034%^ towards the spire nestled at the centre and origin of the sprawl.%^CRST%^\n");
    ...
    set_exits(([
        "south" : "/d/magic/temples/edea/temple_sw",
        "north" : "/d/magic/temples/edea/temple_nw",
        "west" : "/d/shadow/virtual/desert/25,8.desert",
        ]));
}
