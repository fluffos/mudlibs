#include <ansi.h>
int ling_renwu(string arg)
{ 
    string *quest_place=({"厨房","磨房","菜园"});
    int now_place; // 1 or 2 or 3
    int now_time;
    int use_time;
    int add_class_score;
//   int which_temp;
...
//   object givething;
    ...
    if( !arg )
        return notify_fail("你要干什么啊？\n");
    if(arg != "任务" && arg != "renwu" )
    {
        if( arg == "雷公轰" || arg == "leigong hong" || arg == "hong")
        {
            tell_object(me,"你向司马卫恭身说道：司马掌门能不能将您的"+BLU"雷公轰"NOR+"给我一用？\n");
            return notify_fail("司马卫笑道：小老儿自己的兵刃，这位"+RANK_D->query_respect(me)+"难道还看得上？\n");
        }
        if( arg == "青布道袍" || arg == "qingbu daopao" || arg == "daopao")
        {
            tell_object(me,"你向司马卫恭身说道：观主能不能将您的道袍脱下给我？\n");
            return notify_fail("司马卫笑道：小老儿的衣服给这位"+RANK_D->query_respect(me)+"，我自己还穿什么呢？\n");
        }
        if( arg == "司马卫" || arg == "sima wei" || arg == "sima" )
        {
            tell_object(me,"你向司马卫恭身说道：观主能不能将您的终身托付给我呢？\n");
            return notify_fail("司马卫笑道：小老儿一无是处，这位"+RANK_D->query_respect(me)+"可是难以养活我的说！\n");
        } 
        return notify_fail("司马卫笑道：这位"+RANK_D->query_respect(me)+"说笑了，小老儿怎么会有"+arg+"呢？\n");
    }
    if(me->query("family/family_name") != "青城派") // 非我青城弟子
        return notify_fail("司马卫笑道：这位"+RANK_D->query_respect(me)+"可是在和小老儿说笑了！\n");
    
    if( me->query("class_score/青城派") <= 800 ) // 一般性的任务
    {
        if(me->query("xli_marks/do_renwu") ) // 正在作着renwu呢！
        {
            now_place = me->query("xli_marks/renwu_where");
            return notify_fail("司马卫说道：不是叫你到"+quest_place[now_place-1]+"去干活吗？\n");
        }
        ...
        if (me->query("xli_marks/finish_renwu") )  // 作完上一个了
        {
            ...
            if (add_class_score)
            {
                tell_object(me,"司马卫说道：好徒儿，为师没有白收你！\n");
                tell_object(me,"司马卫说道：你对我青城派真是忠心耿耿啊！\n");
                me->add("class_score/青城派",add_class_score ); 
            }
            else
                tell_object(me,"司马卫说道：这么一点小事都办得如此拖沓，你真是......\n");
        ...
        if (me->query("xli_marks/can_do_renwu") ) // 可以做
        {
            ...
            now_place = random(3)+1;
            me->set("xli_marks/do_renwu",1);
            me->set("xli_marks/renwu_where",now_place);
            me->delete("xli_marks/have_pin");
            tell_object(me,"司马卫说道：好徒儿，既然你想为我青城效力，为师就让你做一点事吧！\n");
            tell_object(me,"司马卫说道：现在"+quest_place[now_place-1]+"正缺人手，你就到那里去帮忙吧！\n");
    ...
    {   
        if(me->query("class_score/青城派")<900)
        {
         if( me->query("xli_marks/higher_renwu") ) // 正在做
         {
             ...
             {   
                ...
                tempint = me->query("class_score/青城派");
                me->set("class_score/青城派",tempint-10);
             }
       ...
       {       // 靠修道涨忠诚度的时候到了。:)
            tell_object(me,"司马卫捋了一下胡子，说道：你对我青城派的忠心耿耿，为师心里有数。\n");
            tell_object(me,"你也不用再作什么琐碎的事情了！\n");
            tell_object(me,"我道教中人，还是应该多多修道为上，你可不要荒废了正道啊！\n");
            tell_object(me,"司马卫慈祥地看着你，目光中充满赞赏和关爱！\n");
            return 1;
       }  
    }
