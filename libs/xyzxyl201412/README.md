...
- 是真正"夕阳再现"血统（`xyzx`/`jhfy3`/`xajh4gkb`）里内容最扩充
  的一个后期版本：地图同样延伸到日本（`d/japan/`）、高丽
  （`d/gaoli/`）等海外区域，还多了"自动地图"（`d/automap/`）支持。
- 角色创建比同宗档案多一步：天赋数值确认之后还要从 11 种魔法天赋
  里选一种，是这份档案自己独有的额外定制。
- 权限阶层比常规更细致，一路到 `(boss)`/`(ceo)`，本次为了和项目其
  它档案保持一致，统一只用到 `(admin)` 这一级。
