...
{
           string line,my_str,my_dex,my_con,my_int,my_kar,my_per,skill_type,master,couple,pker;
         mapping my,pmap;
         object ob, weapon;
         seteuid(getuid(me));
...
//     killer = this_player()->query("kill/killer_die") ? "「" +            pker=me->query("killer");
         ...
         line = "\n┏━━━━━"+HIR"【个人档案】"NOR"━━━━┓ "+HIC"       ＊ 亂 世 風 沙 欢 迎 您＊"NOR"\n";
         "┃",
         ...
         "┃ 性  别 :      ", ob->query("gender"),页)
         "游戏时间:    ",FINGER_D->age_string( (int)ob->query("mud_age")),);
                 line += sprintf("%s"HIW"%-16s"NOR"%s"HIM"%-28s"NOR"┃\n",
                 "┃战斗攻击力:    ",
         ...
         line += sprintf("%s%-32s┃\n",
                  "┃你一共死亡:    ",HIB+ob->query("dietimes")+NOR + " 次"
                 );                      
         ...
         "┃武功等级：     ", chinese_number(ob ->query("level")) + " 级 " ,
         " <精>   ",tribar_graph(my["jing"], my["eff_jing"], my["max_jing"] , GRN),
         );

       if ((ob->query("combat_exp")) >= ((ob->query("level")-1)*2000 + (ob->query("level"))*2000))
         line += sprintf("%s"HIB"%-15d"NOR"%s%-28s┃\n",
