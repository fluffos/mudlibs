#include <ansi.h>
#define QUEST_DUO "/quest/questduo"
inherit NPC;
inherit F_MASTER;
...
int do_fight(string arg)
{
        ...
        message_vision (HIW"石观音说道：“想和我打？先去打赢阿风再说”！\n"NOR,who);
...
int give_quest()
{
  mapping quest,myfam,skl,hp_status,skl_p;
  object ob,me,ob1;
  int i,combatexp, timep, factor, num,max_skl;
  string *sname,*sname_p;

  me = this_player();
/*  
  myfam = (mapping)me->query("family");
  if (myfam["family_name"] != "少林派" && myfam["family_name"] != "武当派" && myfam["family_name"] != "峨嵋派" && myfam["family_name"] != "华山派")
    {
      message_vision("单正对$N说：我这工作邪派人士不能做，你好自为之吧。\n",me);
      return 0;
    }
*/  
  ...
  if(combatexp < 10000)
    {
      message_vision("石观音盯了$N几眼说：你的身手还差点，再练几年再请来吧。\n",me); 
      return 0;
    }
  
   if(combatexp > 5000000)
{
      message_vision("石观音盯了$N几眼说：：阁下乃武林高手，国家栋梁，不敢劳动大架。\n",me); 
return 0;
}
  message_vision("石观音对$N说：你附耳过来。\n",me);
  ...
  tell_object(me, "你去把镖劫过来，他现在『" + quest["short"]
              + "』，你去吧！\n" NOR);
  quest["quest_type"] = "劫镖";
  quest["exp_bonus"] = random(80) + 140;
  quest["pot_bonus"] = random(60) + 100;
  quest["score"] = 0;
  ...
  num = random(40);
  if (num < 10) ob = new("/quest/biaotou1");
  else if (num < 20) ob = new("/quest/biaotou2");
  else if (num < 30) ob = new("/quest/biaotou3");
  else ob = new("/quest/biaotou4");
  ...
  ob->set("neili",     hp_status["neili"]);
   ob->set("combat_exp",hp_status["combat_exp"]/2);

  skl = ob->query_skills();
  if(!sizeof(skl))
    return notify_fail("目标设置出错，请报告阿风。\n");
...
//    return notify_fail("你没有按时间完成，不会有奖励了。\n"); 
  ...
  if(ob->query("player") != who->query("id"))
    {
      message_vision("石观音对$N大怒道：你敢冒领别人的功劳，实在小觑本圣母了。\n",who);
      return 1;
    }

  message_vision("石观音对$N说：你干的不错。\n",who);
