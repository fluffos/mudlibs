...
// 1999/2 重新组架由JackyBoy做了些信息显示上的修改。
...
void logon_main(string no_use,object ob)
{
	...
	if (ob!=usr[i]) 
		...
		(
			...
			log_num++;	if (log_num >= 5 )
	{  
		write(HIR+"    你只允许同时登陆五个IP，请退出其他帐号再试。\n"+NOR); 
		destruct(ob); 
		return;                 
	}
	write("您的英文名字：");
...
一个人物的天赋对于他或她所修习的武艺息息相关。侠驰骋江湖中的人物大多具有
以下四项天赋：
        膂力：影响攻击能力及负荷量的大小。
...
// End of modification
	...
	CHANNEL_D->do_channel( this_object(), "sys",
        ...
        HIW"【亂世風沙】：欢迎，欢迎，热烈大欢迎"+user->name(1)+"踏入江湖！\n"NOR,users());
        else 
                CHANNEL_D->do_channel( this_object(), "wiz",
                sprintf("%s由%s连线进入。", user->name()+"("+user->query("id")+")", query_ip_name(user)) );
	UPDATE_D->check_user(user);
        if( user->query("lastChangeSize") != file_size("/doc/help/gexing") ) {
               user->set("lastChangeSize", file_size("/doc/help/gexing"));
                write(BLINK HIW"\n本ＭＵＤ已有更新！！！请 help gexing 了解详细情况。\n\n"NOR);
}
