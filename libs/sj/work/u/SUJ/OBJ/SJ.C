...
void create()
{
    set_name("农民" + RMK_VERSION, ({ "roommaker", "maker" }) );
    set_weight(100);
    set("unit", "个" );
    set("long", @LONG
...
void init()
{
    if( wizardp(environment()) ) {
        ...
        add_action("do_delete", "rdel");
        add_action("do_connect", "rgo");
        add_action("do_saveroom", "rsave");
        add_action("do_addobj", "robj");
        add_action("do_long", "rlong");
        add_action("do_longa", "rlong1");
...
ROOM_CODE
    ...
    message_vision("只看到一个农民在到处挖了一正，一间小屋就做好了。\n", this_player());
    return 1;
}
...
int do_longa(string arg)
{
        string prop, str;
        mixed data;
        if(!arg)
        return notify_fail("指令格式：rlong <房间描述>\n");
        if( sscanf(arg, "%s %s", prop, str)!=2 ){
        environment(this_player())->set("long", arg+"\n");
        }else{
        if(prop=="+" ){
        environment(this_player())->set("long",environment(this_player())->query("long")+str+"\n");
        }else{  return notify_fail("指令格式：rlong + <房间描述>\n");}
        }
        write("描述更改完毕。\n");
        return 1;
}
