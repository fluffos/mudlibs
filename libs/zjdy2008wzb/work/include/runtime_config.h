...
/*
 * AGENTS.md: this mudlib originally bundled its own runtime_config.h with a
 * different BASE_CONFIG_INT numbering than this driver build (same bug class
 * as ds386's "Bad argument 4 to /" finding), which made get_config(__MUD_PORT__)
 * silently misindex into an unrelated driver config slot (a string) and crash
 * adm/daemons/network/messaged.lpc's socket_bind() -- breaking wizard login.
 * __SAVE_BINARIES_DIR__ no longer exists in this driver; alias it to the
 * closest still-valid equivalent so the handful of call sites that reference
 * it keep compiling correctly instead of misindexing. __PORT__ needs no
 * alias -- the driver already predefines it as a compiler literal.
 */
#define __SAVE_BINARIES_DIR__ __MUD_LIB_DIR__
