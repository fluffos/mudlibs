...
《模拟华附》是以广州市华南师范大学附属中学（华附）真实校园为背景的生活模拟 mudlib，没有任何武侠/奇幻元素——地图（`d/huafu/`）以教学楼、宿舍、电脑房、图书馆等真实校园场景为蓝本。和 `yxsj`/`yxzsj`（台湾"逸俠之世界"的校园+霹雳布袋戏设定）一样是校园题材，但这份档案是纯粹的大陆真实中学模拟。角色创建选择的是独立的"人物类型"（聪明好学型/活泼可爱型/冷静细心型/身高力壮型），而不是常见的门派/种族。游戏自带一整套"军训"新手引导流程，教官会依次带你了解经济系统（单车楼看车、麦当劳洗盘子赚钱）、拜师/读书/任务系统，最后在枢纽房间用数字菜单（1-6：聚清园/教学楼/办公楼/图书馆/麦当劳/后门花园）选择去处——比一般武侠 MUD 常见的"文字提示 + 方向指令"更接近手机游戏的引导体验。任务系统是限时寻物类型（`quest` 指令直接跟 NPC 领取，不是 `ask...about`），"草坪"场景有专门给新手练手用的"小鸟"NPC。
...
- 密码规则要求同时包含大小写字母和数字，本次测试账号密码也因此从
  ...
  见下方管理员账号说明）。
- 游戏自带一整套"军训"新手引导流程，教官会依次带你了解经济系统
  （单车楼看车、麦当劳洗盘子赚钱）、拜师/读书/任务系统，最后在枢
  纽房间用数字菜单（1-6）选择去处（聚清园/教学楼/办公楼/图书馆/
  麦当劳/后门花园）——比一般武侠 MUD 常见的"文字提示 + 方向指令"
  更接近手机游戏的引导体验。
- 任务系统是限时寻物类型（`quest` 指令直接跟 NPC 领取，不是
  `ask...about`），"草坪"场景有专门给新手练手用的"小鸟"NPC。

## 深度功能测试新发现的 bug（详见 NOTES.md）

`adm/daemons/logind.lpc` 里两条并行取名流程（接受系统随机中文名 /
自己手动输入）都各自残留了一行调试用的 `printf("%O\n", ob)`，紧挨
在设定中文名字之前，每一个新玩家注册时都会看到（AGENTS.md
§7.34）。已删除两处。
