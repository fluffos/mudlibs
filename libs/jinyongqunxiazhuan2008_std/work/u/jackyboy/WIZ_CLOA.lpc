...
set("long",@LONG
    ...
    这是一件刚出炉的面包师披风，为了巫师工作方便，巫师们常常需要穿这件披风
    如果巫师们想要有进一步的功能欢迎提供
...
void init()
{
  ...
  if (me)
    ...
    set_name ("面包师披风", ({ "wiz cloak", "cloak" }));
...
write(@Help
    ...
    你可以使用以下的几个命令:
       ...
       rem <装备> from <object>		<让某一 object 解除装备>
...

{
  ...
  if (!str||sscanf(str,"%s from %s",ob,player)!=2)
          return notify_fail("[cloak]: rem <装备> from <object>\n");;
  ...
  if(!obj)
    ...
    message_vision ("$N让"+player+"脱下$n("+ob+")\n", this_player(), obj);
