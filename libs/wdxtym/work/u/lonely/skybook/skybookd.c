...
string quest_feihu(object me)
{
        ...
        {
                        if(!query("quest/雪山飞狐/解药篇/over",me) )
                                str +=get_string(WHT"游戏纪录修为差："+chinese_number(query("combat_exp",me)-query("quest/雪山飞狐/combat_exp",me))+"。"NOR);
                        else str +=get_string(WHT"游戏纪录修为："+query("quest/雪山飞狐/combat_exp",me)+"。"NOR);
                        str +="\n";
        }
        ...
        {
                str +=get_string(WHT"寻宝纪录修为："+query("quest/雪山飞狐/宝藏/combat_exp",me)+"。"NOR);
                str +="\n";
        }
        ...
        if(query("quest/雪山飞狐/宝藏/shuku",me))
                str +=get_string(YEL"你从书房发现到一个密室，并在这里寻找到增长修为的神奇灵药。"NOR);
...
string feihu_skill(object me)
{
        ...
        if(me->query("quest/雪山飞狐/两页刀法篇/over"))
        {
                ...
                {
                        str +=get_string(WHT"武功纪录修为："+query("quest/雪山飞狐/武功/combat_exp",me)+"。"NOR);
                        str +="\n";
                }
...
string quest_lchj(object me)
{
        ...
        if(query("quest/连城诀/combat_exp",me))
        {
                if(!query("quest/连城诀/武馆探密篇/over",me) && !query("quest/连城诀/武馆探密篇/over",me))
                {
                        if(query("combat_exp",me)> query("quest/连城诀/combat_exp",me))
                                str +=get_string(WHT"游戏纪录修为差："+chinese_number(query("combat_exp",me)-query("quest/连城诀/combat_exp",me))+"。"NOR);
                        else if(query("combat_exp",me)== query("quest/连城诀/combat_exp",me))
                                str +=get_string(WHT"游戏纪录修为差没有任何变化。"NOR);
                        else str +=get_string(WHT"游戏纪录修为超出目前修为："+chinese_number(query("quest/连城诀/combat_exp",me)-query("combat_exp",me))+"。"NOR);
                }
                else str +=get_string(WHT"最后游戏纪录修为："+query("quest/连城诀/combat_exp",me)+"。"NOR);
                str +="\n";
        }
        ...
        if(query("quest/连城诀/武馆探密篇/baozang",me))
        {
                ...
                str +=get_string(YEL"虽然没有身外之财，你从中还是获得部分修为奖励。"NOR);
                str +=lchj_skill(me);
                return str;
        }
        str +=lchj_skill(me);
...
string lchj_skill(object me)
{
        ...
        if(query("quest/连城诀/雪谷激斗篇/start",me))
        {
                ...
                {
                        str +=get_string(WHT"武功纪录修为："+query("quest/连城诀/武功/combat_exp",me)+"。"NOR);
                        str +="\n";
                }
                if(query("quest/连城诀/武功/shenzhao",me)>=2)
                {
                        str +=get_string(HBYEL"你从丁典学到神照经，并更能灵活使用神照绝技。"NOR);
                        str +=get_string(HBYEL"你又从狄云处获得一些修为，对神照经有着更深层次的领悟。"NOR);
                }       
        ...
        if(query("quest/天龙八部/combat_exp",me))
        {
                if(!query("quest/天龙八部/复兴天下篇/over",me) && !query("quest/天龙八部/大辽救援篇/over",me))
                {
                        if(query("combat_exp",me)> query("quest/天龙八部/combat_exp",me))
                                str +=get_string(WHT"游戏纪录修为差："+chinese_number(query("combat_exp",me)-query("quest/天龙八部/combat_exp",me))+"。"NOR);
                        else if(query("combat_exp",me)== query("quest/天龙八部/combat_exp",me))
                                str +=get_string(WHT"游戏纪录修为差没有任何变化。"NOR);
                        else str +=get_string(WHT"游戏纪录修为超出目前修为："+chinese_number(query("quest/天龙八部/combat_exp",me)-query("combat_exp",me))+"。"NOR);
                }
                else str +=get_string(WHT"最后游戏纪录修为："+query("quest/天龙八部/combat_exp",me)+"。"NOR);
                str +="\n";
        }
...
string tlbb_skill(object me)
{
        ...
        if(query("quest/天龙八部/武功", me))
        {
                ...
                {
                        str +=get_string(WHT"最后学习纪录修为："+query("quest/天龙八部/武功/combat_exp",me)+"。"NOR);
                        str +="\n";
                }
        ...
        if(query("quest/射雕英雄传/combat_exp",me))
        {
                if(!query("quest/射雕英雄传/风雪惊变/over",me) )
                {
                        if(query("combat_exp",me)> query("quest/射雕英雄传/combat_exp",me))
                                str +=get_string(WHT"游戏纪录修为差："+chinese_number(query("combat_exp",me)-query("quest/射雕英雄传/combat_exp",me))+"。"NOR);
                        else if(query("combat_exp",me)== query("quest/射雕英雄传/combat_exp",me))
                                str +=get_string(WHT"游戏纪录修为差没有任何变化。"NOR);
                        else str +=get_string(WHT"游戏纪录修为超出目前修为："+chinese_number(query("quest/射雕英雄传/combat_exp",me)-query("combat_exp",me))+"。"NOR);
                                str +=get_string(HIC"你已经成功救下杨铁心、郭啸天两位大侠，并获得一些奖励。"NOR);
                }
                else str +=get_string(WHT"最后游戏纪录修为："+query("quest/射雕英雄传/combat_exp",me)+"。"NOR);
                str +="\n";
        }
        else
        {
...
string shediao_skill(object me)
{
        ...
        if(query("quest/射雕英雄传/yangjiaqiang",me))
        {
                ...
                {
                        str +=get_string(WHT"最后学习纪录修为："+query("quest/射雕英雄传/yangjiaqiang/combat_exp",me)+"。"NOR);
                        str +="\n";
                }
                if(query("quest/射雕英雄传/风雪惊变/over",me))
                {            
