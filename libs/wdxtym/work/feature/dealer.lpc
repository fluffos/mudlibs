...
int do_list(string arg)
{
	...
	else
	{
		...
		for (i = 0; i < sizeof(dk); i++)
		{
			...
			{
				if(amo[dk[i]])
					msg += sprintf(WHT"%s"NOR+"　%s:buy %s"ZJSEP, dk[i], vip[dk[i]]?(sprintf("%d",p)+"灵石"):MONEY_D->money_str(p),filter_color(dk[i]));
				else
					msg += sprintf(WHT"%s"NOR+"　%s:buy 1 %s"ZJSEP, dk[i],vip[dk[i]]?(sprintf("%d",p)+"灵石"):MONEY_D->money_str(p),filter_color(dk[i]));
			}
			else
			{
				if(amo[dk[i]])
					msg += sprintf(WHT"%s"NOR+"　%s"ZJBR"%s:buy %s"ZJSEP, dk[i], vip[dk[i]]?(sprintf("%d",p)+"灵石"):MONEY_D->money_str(p),long[dk[i]],filter_color(dk[i]));
				else
					msg += sprintf(WHT"%s"NOR+"　%s"ZJBR"%s:buy 1 %s"ZJSEP, dk[i],vip[dk[i]]?(sprintf("%d",p)+"灵石"):MONEY_D->money_str(p),long[dk[i]],filter_color(dk[i]));
			}
		}
...
int do_buy(string arg)
{
  ...
  local = localtime((time()+28800) * 60);
	...
	if (sizeof(filter_array(all_inventory(me), (: ! $1->query("equipped") :))) >= MAX_ITEM_CARRIED+me->query("zengrong"))
	{
...
if (sscanf(arg, "%d %s", amount, arg) != 2)
	...
	//	+	ZJOBACTS2+ZJMENUF(2,2,10,30)"买全部:buy " + zhong[ob->query("id")] + " " + ob->query("id") + ZJSEP+
		//	"给一半 ["+(ob->query_amount()/2)+"个]:give " + (ob->query_amount()/2) + " " + ob->query("id") + " to " + me->query("id") + ZJSEP+
		//	"买一个:buy 1 " + ob->query("id")+"\n"
...
*/
	...
	if (amount > 3 && ! ob->query_amount())
	{
		write(ob->name() + "一次最多只能买3" + ob->query("unit")+"。\n");
		return 1;
	}
	...
	{//灵石道具
		...
		{
			write(CYN + name() + "说道：“你有那么多灵石么?”"NOR"\n");
			return 1;
		}
		set_temp("busy", 1);
		if (ob->query_amount())
	...
	default:
		...
		if (ob->query_amount())
		{
			times = me->query("day_item/"+ob->query("id")+"/times");//购买次数
			
			if (amount > zhong[ob->query("id")]&&ob->query("end"))//限制输入的购买量
				{
				write(CYN + name() + "忙道：“"+ob->query("name")+CYN"为限量物品，每天最多购买"+zhong[ob->query("id")]+ob->query("base_unit")+"！”"NOR"\n");
				return 1;
				}
			
			if (me->query("day_item/"+ob->query("id")+"/last_time")||ob->query("end"))
			{
			last = localtime(me->query("day_item/"+ob->query("id")+"/last_time"));

			if(last[7]!=localtime(time())[7])//判断时间加8小时，这样零点就可以更新
				me->set("day_item/"+ob->query("id")+"/times",0);
			else if(last[7]==localtime(time())[7])
				{
				if (amount>(zhong[ob->query("id")]-times))
				{
				write(CYN + name() + "忙道：“你今天已经购买了"+times+ob->query("base_unit")+ob->query("name")+CYN",剩余购买量："+(zhong[ob->query("id")]-times)+"。”"NOR"\n");
				return 1;
				}	
				else	if(times>=zhong[ob->query("id")])
				{
        write("你今日已经购买"+ob->query("name")+"达"+zhong[ob->query("id")]+"次上限了。请明天再来。"NOR"\n",me);
				return 1;
				}
				
			}	
			me->set("day_item/"+ob->query("id")+"/last_time",time());//设定时间不用加
			me->add("day_item/"+ob->query("id")+"/times",ob->query_amount());	
    	} 	
