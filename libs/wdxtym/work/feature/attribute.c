...
int query_str()
{
	...
	if (lx >= 13) lx = 15;
	  ...
	  return str + improve + lx +query_temp("suit_eff/strength") + query_temp("apply/str") + query("titlea/str");
...
int query_int()
{
	...
	improve = (int)sk["literate"]+(int)query_temp("suit_skill/literate");
	      ...
	      return str + (improve / 10) + query_temp("apply/int")+query_temp("suit_eff/intelligence") + query("titlea/int");   
}
...
int query_con()
{
	...
	improve = (int)sk["force"]+(int)query_temp("suit_skill/force");
	      ...
	      return str + (improve / 10) + query_temp("apply/con")+query_temp("suit_eff/constitution") + query("titlea/con");
}
...
int query_dex()
{
	...
	improve = (int)sk["dodge"]+(int)query_temp("suit_skill/dodge");
	     ...
	     return str + (improve / 10) + query_temp("apply/dex")+ query_temp("suit_eff/spirituality") + query("titlea/dex");     
}
