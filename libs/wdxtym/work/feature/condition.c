...
void update_cd()
{
	...
	if(pfcds = this_object()->query("skill_cd"))
	{
		cds = keys(pfcds);
		for(i=0;i<sizeof(cds);i++)
		{
			this_object()->set("skill_cd/"+cds[i],pfcds[cds[i]]-1);
			if((pfcds[cds[i]]-1)==0)
			{
				this_object()->delete("skill_cd/"+cds[i]);
				cds[i] = replace_string(cds[i],"|","/");
				tell_object(this_object(),replace_string(cds[i]->query_name(),ZJBR,"")+"冷却完毕！\n");
			}
		}
	}

	if(this_object()->query("skill_pcd"))
	{
		if((m=this_object()->query("skill_pcd/num"))>0)
			this_object()->set("skill_pcd/num",m-1);
		else
			this_object()->delete("skill_pcd");
	}
}
