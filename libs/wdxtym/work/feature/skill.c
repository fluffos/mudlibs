...
{
	int num=0,mer=0,merr=0;
	num=query_temp("suit_skill/"+skill);   //套装技能
	 merr = query("taozhuang/jiacheng");//精炼加成
...
varargs void improve_skill(string skill, int amount, int weak_mode)
{
	...
	if (lvl > 600 + this_object()->query("zhuanshi") * 50)
	{
		tell_object(this_object(), HIR"研究无效："+NOR" 你的"+HIY+"「" +to_chinese(skill)+ "」"+NOR"到达瓶颈！\n");
		return;
	}
	else{
	skills[skill] = skills[skill]+1;
	lvl = skills[skill];
	this_object()->add("potential",-10000);
