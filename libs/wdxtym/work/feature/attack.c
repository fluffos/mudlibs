...
varargs int is_fighting(object ob)
{
	...
	if(env&&env->is_comb_ob())
	{
		if(ft=me->query_temp("fight_team"))
			emy = env->query_emy(ft);
		else
			emy = 0;
	}
	if (!emy) return 0;
	if (! ob) return sizeof(emy) > 0;
	return member_array(ob, emy) != -1;
}
...
void fight_ob(object ob)
{
	...
	env = environment();
		...
		(env->query("no_fight") && !ob->query_condition("killer")))
	    //env->query("no_fight"))
		return;
...
{       
	...
	{
		tell_object(ob, HIR "不能杀你要保护的人！"NOR"\n");
		return;
	}

	if (query_temp("owner") == ob)
	{
		tell_object(me, HIR "不能对你的主人下毒手。"NOR"\n");
		return;
	}
...
object select_opponent()
{
	...
	if (!emy||!(which = sizeof(emy))) return 0;
