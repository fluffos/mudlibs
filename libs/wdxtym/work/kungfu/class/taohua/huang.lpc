...
void create()
{
	...
	set_skill("force", 2700);	     // 基本内功
	set_skill("bibo-shengong", 2700);     // 碧波神功
	set_skill("luoying-xinfa", 2700);     // 落英心法
	set_skill("hand", 2500);	      // 基本手法
	set_skill("finger", 2700);	    // 基本指法
	set_skill("tanzhi-shentong", 2700);   // 弹指神通
	set_skill("unarmed", 2200);	   // 基本拳脚
	set_skill("strike", 2200);	    // 基本拳脚
	set_skill("luoying-shenzhang", 2500); // 落英神剑掌
	set_skill("xuanfeng-leg", 2500);      // 旋风扫叶腿
	set_skill("dodge", 2700);	     // 基本躲闪
	set_skill("anying-fuxiang", 2700);    // 暗影浮香
	set_skill("parry", 2700);	     // 基本招架
	set_skill("sword", 2700);	     // 基本剑法
	set_skill("throwing", 2070);	     // 基本暗器
	set_skill("luoying-shenjian", 2700);  // 落英神剑
	set_skill("yuxiao-jian", 2700);       // 玉箫剑法
	set_skill("lanhua-shou", 2700);       // 兰花拂穴手
	set_skill("qimen-wuxing", 2500);      // 奇门五行
	set_skill("count", 300);	     // 阴阳八卦
	set_skill("mathematics", 300);       // 算术
	set_skill("literate", 250);	  // 读书识字
	set_skill("chuixiao-jifa", 3000);     // 吹箫技法
	set_skill("bihai-chaosheng", 3000);   // 碧海潮生
	set_skill("taohua-yuan", 2050);       // 桃花怨
	set_skill("medical", 3000);	   // 基本医术
	set_skill("taohua-yaoli", 3000);      // 桃花药理
