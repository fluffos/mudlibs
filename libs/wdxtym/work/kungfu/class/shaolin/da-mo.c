...
void create()
{
	...
	set_skill("shenzhang-bada", 300);
	set_skill("wuxiang-zhi", 300);
	set_skill("duoluoye-zhi", 300);
	set_skill("zui-quan", 300);
...
void attempt_apprentice(object ob)
{
	...
	{
		command("say " + RANK_D->query_respect(ob) + "实战修为"
			"还颇有欠缺，不如先苦心修炼。");
		return;
	}
...
mixed ask_qingxin()
{
	...
	tell_object(me, HIG "你学会了达魔乱气剑。"NOR"\n");
...
mixed ask_luan()
{
	...
	message_vision(HIY "$n" HIY "看看$N" HIY
		       ...
		       me, this_object());
	command("say 记住，除非降妖除魔万不得已，勿要轻易使用。");
	tell_object(me, HIG "你学会了达魔清心剑。"NOR"\n");
...
mixed ask_shang()
{
	...
	message_vision(HIY "$n" HIY "看看$N" HIY
		       ...
		       me, this_object());
	command("say 记住，除非降妖除魔万不得已，勿要轻易使用。");
	tell_object(me, HIG "你学会了达魔伤神剑。"NOR"\n");
	if (me->can_improve_skill("force"))
		me->improve_skill("force", 160000);
...
mixed ask_zhe()
{
	...
	message_vision(HIY "$n" HIY "看看$N" HIY
		       ...
		       me, this_object());
	command("say 记住，除非降妖除魔万不得已，勿要轻易使用。");
	tell_object(me, HIG "你学会了达魔折元剑。"NOR"\n");
	if (me->can_improve_skill("force"))
		me->improve_skill("force", 160000);
