...
void set_from_me(object me)
{
	...
	if (my["combat_exp"] > 100000 && random(100) < 5)
	{
		// 修为 > 200K，5%几率出现多个敌人
...
void die(object killer)
{
	...
	if ((n = sizeof(obs)) > 0)
	{
		...
		// 所有人均分修为：为什么除以n + 2？这是因为
		// 打晕我的人分两份修为，所以是n + 1，再加上
		// 领任务的这个人算1，那么就是n + 2。
		exp = exp / (n + 2) + 1;
		pot = pot / (n + 2) + 1;
	...
	qob->add("yuanbao_2", 1);
tell_object(qob,HIM"恭喜你获得1灵石票。"NOR"\n");
	// 使用书信分配任务
	...
	if(give = query("die_give"))
	{
		...
		for(i=0;i<sizeof(items);i++)
		{
			if(random(give[items[i]])==0)
			{
				ob = new(items[i]);
				ob->move(aob, 1);
				tell_object(qob,HIC"掉落物品，自动存入背包。\n"NOR); 
			}
		}
	}
	::die();
