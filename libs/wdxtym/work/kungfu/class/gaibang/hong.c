...
void create()
{
	...
	set_skill("jiaohua-neigong", 2500);
	set_skill("huntian-qigong", 2700);
	set_skill("unarmed", 2300);
	set_skill("changquan", 2300);
	set_skill("dodge", 2400);
	set_skill("xiaoyaoyou", 2400);
	set_skill("feiyan-zoubi", 2400);
	set_skill("parry", 2500);
	set_skill("staff", 2700);
	set_skill("strike", 2700);
	set_skill("dragon-strike", 2700);
	set_skill("jiaohua-bangfa", 2500);
	set_skill("dagou-bang", 2700);
	set_skill("throwing", 2200);
	set_skill("mantianhuayu-zhen", 2200);
...
void attempt_apprentice(object ob)
{
	...
	if (ob->query_str() < 40 && ob->query_int() < 42 && ob->query_con() < 40)
	{
	    ...
	    tell_object(ob, "洪七公收徒要求并不高，后天臂力或者根骨任意一项达到40，亦或悟性达到42。\n");
		return;
	}
