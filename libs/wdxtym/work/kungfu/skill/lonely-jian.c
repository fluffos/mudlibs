...
int valid_learn(object me)
{
        ...
        if(me->query("family/family_name") != "华山派" && me->query("family/family_name") != "华山剑宗" )
                return notify_fail("独孤九剑只有华山弟子可以修习。\n");
			if( me->query("int")<40 )
        return notify_fail( "你只感剑法晦涩难懂。\n");
        if( me->query("dex")<40 )
         return notify_fail( "你只觉剑法灵动非凡。\n");
        if( me->query("con")<20 )
         return notify_fail("你只觉自身气血翻涌。\n");
        if( me->query("str")<25 )
        return notify_fail( "你只觉剑法太过沉重。\n");
        if( (int)me->query("max_neili") < 600 )
                return notify_fail("你的内力不够，没有办法练独孤九剑。\n");
        if( (int)me->query_skill("zixia-shengong",1)<100)
                return notify_fail("独孤九剑必须配合紫霞神功才能练习。\n");
       if (((int)me->query_skill("huashan-sword", 1)+150) < (int)me->query_skill("lonely-jian", 1))
               return notify_fail("你的剑宗华山剑法太低了。\n");
       if (((int)me->query_skill("kuangfeng-jian", 1)+150) < (int)me->query_skill("lonely-jian", 1))
               return notify_fail("你的气宗狂风快剑太低了。\n");
       if (((int)me->query_skill("poyu-quan", 1)+200) < (int)me->query_skill("lonely-jian", 1))
               return notify_fail("你的破玉拳太低了。\n");
       if (((int)me->query_skill("hunyuan-zhang", 1)+200) < (int)me->query_skill("lonely-jian", 1))
               return notify_fail("你的混元掌太低了。\n");
       if (((int)me->query_skill("feiyan-huixiang", 1)+200) < (int)me->query_skill("lonely-jian", 1))
               return notify_fail("你的飞燕回翔太低了。\n");
       if (((int)me->query_skill("huashan-xinfa", 1)+200) < (int)me->query_skill("lonely-jian", 1))
               return notify_fail("你的剑宗内功太低了。\n");
       if (((int)me->query_skill("zixia-shengong", 1)+200) < (int)me->query_skill("lonely-jian", 1))
...
lvl = me->query_skill("lonely-jian", 1);
        if (random(me->query_skill("lonely-jian",1)) < 100 ||		             me->query_skill("force",1) < 100 )
        return 0;

	weapon = me->query_temp("weapon");
        level = (int) me->query_skill("zixia-shengong",1);
        level2= (int) me->query_skill("zixia-shengong",1);
if (random(5)==0 && level>=300 && me->query_skill_mapped("force") == "zixia-shengong")
{
if (random(2)==0)
{
	victim->start_busy(2);
}
else 	victim->receive_wound("qi", (random(lvl))+200,me);
...
level2= (int) me->query_skill("zixia-shengong",1);             
	...
	{
	if( random(level2)>100) {
	victim->receive_wound("qi", (random(lvl)/2+level2),me);
...
return 0;
if (me->query_skill_mapped("sword") != "lonely-jian")
return 0;
if (me->query_skill_mapped("parry") != "lonely-jian")
return 0;
...
return 0; 
        ...
        switch(random(5))
        {
           case 1:
              ...
              {
                  me->add("neili", -25);
                  victim->receive_wound("qi", (random(lvl)),me);
          ...
          case 3:
             ...
             if (! me->is_busy() && random(lvl)> 180 && random(3)==0)
             {
                  weapon = me->query_temp("weapon");
...
if (!weapon2->query("systemmake") && !weapon2->query("ownmake"))
{
             ...
             if (random(ap) > dpp/2 && random(lvl)> 180 && weapon)
             {
             ...
             if (msg && me && victim) message_combatd(msg, me, victim);
             break;
...
if (victim){                   
        ...
        if (random(me->query_skill("lonely-jian",1)) > 200 &&
...
victim->receive_wound("qi", (random(lvl)+2/2),me);
  if( !victim->is_busy() )
{
        victim->start_busy(2);
}
        ...
        if (random(me->query_skill("lonely-jian",1)) > 140 &&	
  ...
  if( !victim->is_busy() )
{
        ...
        if (random(me->query_skill("lonely-jian",1)) > 130 &&	
  ...
  if( !victim->is_busy() )
{
        ...
        if (random(me->query_skill("lonely-jian",1)) > 100 &&
...
victim->receive_wound("qi", (random(lvl)+2/2),me);
	...
	return HIM"$n使出浑身解数才脱出剑圈！\n"NOR;
        ...
        if (random(me->query_skill("lonely-jian",1)) > 80 &&	
...
if (!jiali) jiali=10;
	if( damage_bonus > 50 && random(me->query_skill("lonely-jian",1))>140 
	&& me->query("fengset")
	) {
	victim->receive_wound("qi", (random(lvl)+level2),me);
	return HIM"$N剑气纵横! 一道剑气划过了$n的$l。结果鲜血狂奔!\n"NOR;
}
}
string perform_action_file(string action)
{
        return __DIR__"lonely-jian/" + action;
}
...
return damage;
        ...
        level = me->query_skill("lonely-jian", 1);
...
me->add("qi",(damage/3+level));
if ((int)me->query("eff_qi") < (int)me->query("max_qi"))
me->add("eff_qi",(damage/3+level));
            damage=100;
            j=100;
            return j;
...
if ((int)me->query("qi") < (int)me->query("eff_qi"))
        ...
        if (  random(level) > 100 /i 
         && me->query_skill_mapped("sword") =="lonely-jian"
         && me->query_skill_mapped("parry") =="lonely-jian"
         && me->query("fengset")
         && weapon
