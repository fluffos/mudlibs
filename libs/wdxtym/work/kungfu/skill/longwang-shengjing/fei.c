...
int perform(object me, object target)
{
        ...
        if ((int)me->query_skill("longwang-shengjing", 1) < 450)
                return notify_fail("你龙王圣经火候不够，难以施展" FEI "。\n");
 ...
 if(random(3)==0 || (random(me->query("combat_exp",1)) >= target->query("combat_exp")/3))
        {
                msg += HIR "$n" HIR "面对$N" HIR "这排山倒海般的攻"
                       "势，完全无法抵挡，招架散乱，连连退后。\n" NOR;
                count = me->query_skill("longwang-shengjing",1) / 20;
        } else
        {
