...
int perform(object me, object target)
{
        ...
        if ((int)me->query_skill("longwang-shengjing", 1) < 500)
                return notify_fail("你龙王圣经火候不够，难以施展" HUI "。\n");

        if (me->query_skill_mapped("strike") != "longwang-shengjing")
                return notify_fail("你没有激发龙王圣经，难以施展" HUI "。\n");

if ((int)me->query_skill("yinyangjiuzhuanshierchongtian",1) < 500)
 ...
 if(random(3)==0 || (random(me->query("combat_exp",1)) >= target->query("combat_exp")/3))
        {
                damage = me->query_skill("strike",1)+300;
                if (userp(target) && damage > target->query("qi",1)/2) damage=target->query("qi",1)/2;
                target->receive_damage("qi", damage*4,me);
                target->receive_wound("qi", damage*4,me);

                        msg =COMBAT_D->do_damage(me, target, UNARMED_ATTACK, damage, 130,
 ...
 if(random(3)==0 || (random(me->query("combat_exp",1)) >= target->query("combat_exp")/3))
        {
                damage = me->query_skill("force",1)+500;
                if (userp(target) && damage > target->query("qi",1)/2) damage=target->query("qi",1)/2;
        ...
        if ( objectp(weapon = target->query_temp("weapon")))
{
       msg = HIR "$N" HIR "暴喝一声，全身内劲迸发，气贯右臂奋力外扯，企图将$n"
        ...
        ap = me->query_skill("strike",1) + me->query("str") * 10;
        dp = target->query_skill("parry") + target->query("int") * 10;
 ...
 if(random(3)==0 || (random(me->query("combat_exp",1)) >= target->query("combat_exp")/3))
        {
            ...
            dmg += jiali + jianu*5;
            dmg += random(dmg/2);
dmg=dmg+me->query_skill("strike",1)+100;
        if (wizardp(me))
                printf("damage = %d\n",dmg);
                           if (userp(target) && dmg > target->query("qi",1)/2) dmg=target->query("qi",1)/2;
        ...
        //me->add_temp("apply/dodge",-100);
                         me->add_temp("apply/attack", 100);
	         me->add_temp("apply/damage", 1000);
        msg = WHT "$N" WHT "施出龙王圣经之「" HIW "排山倒海" NOR +
