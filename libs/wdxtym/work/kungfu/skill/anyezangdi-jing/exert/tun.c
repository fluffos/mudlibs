...
string query_name() { return "葬帝"ZJBR"呑天"; }
...
int exert(object me, object target)
{
	...
	if ((int)me->query_skill(" anyezangdi-jing", 1) < 100)
		return notify_fail("你的暗夜葬经功力不够，不能吸取对方的内力！n");

	if ((int)me->query("neili") < 20)
		return notify_fail("你的内力不够，不能使用暗夜葬经。\n");
	...
	if ((sp + random(sp) > dp + random(dp) ) || ! living(target))
	{
		...
		sucked = 1 + (me->query_skill("anyezangdi-jing", 1) - 90) / 10*100;
