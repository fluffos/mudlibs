...
int exert(object me, object target)
{
	...
	if (! me->query("breakup"))
		return notify_fail("你还没有领悟利用真气激励精神的诀窍，只有打通任督二脉之后才能使用疗神。\n");
	...
	if ((int)me->query("eff_jing") >= ((int)me->query("max_jing")+me->query_gain_jing()))
		return notify_fail("你现在精神饱满，有什么好激励的？\n");

	if ((int)me->query("neili") < 200)
		return notify_fail("你的真气不够。\n");

	if ((int)me->query("eff_jing") < ((int)me->query("max_jing")+me->query_gain_jing()) / 3)
		return notify_fail("你的精损伤太重，现在难以振奋自己。\n");

	message_vision(HIW "$N" HIW "盘膝坐下，闭目冥神，缓缓的呼吸吐纳。"NOR"\n", me);
...
int inspiring(object me)
{
	...
	if (me->query("eff_jing") < (me->query("max_jing")+me->query_gain_jing()))
	{
		// 需要继续疗伤
		...
		{
		case 0:
			tell_object(me, "你运用" + to_chinese(force) + "沿着诸大穴慢慢运转，振奋精神。\n");
			break;
		case 1:
			tell_object(me, "你将内力纳入丹田，再上行冲顶，透泥宫，顿觉精神爽快。\n");
			break;
		case 2:
			tell_object(me, "你运转内力，缓缓呼吸吐纳，额头上渗出点点汗珠，随即化作白雾蒸腾而起。\n");
			break;
		}
	...
	me->set_short_desc(0);
	message_vision(HIY "$N" HIY "缓缓睁开双目，精光四射，神清意爽。"NOR"\n",me);
	return 0;
}
