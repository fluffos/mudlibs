...
#define RI "「" HIW "剑破万法" NOR "」"
string query_name() { return "[群攻]"ZJBR"剑破万法"; }
int perform(object me)
{
        ...
        if (me->query_skill("wuyin-jianjing", 1) < 200)
                return notify_fail("你的无垠剑经还不够娴熟，难以施展" RI "。\n");

        if ((int)me->query("neili") < 2500)
                return notify_fail("你的真气不够，无法运用" RI "。\n");

        msg = HIY "只见$N" HIY "双目微闭，单手执剑。周围空间破碎，"
              "时空泯灭，正是奥\n义「" NOR + HIW "剑破万法" NOR + HIY "」。霎时"
