...
#define YINYANG "「" HIW "万世镇压" NOR "」"

string query_name() { return "万世"ZJBR"镇压"; }
...
int perform(object me, object target)
{
        ...
        if (me->query_skill_mapped("force") != "yinyangjiuzhuanshierchongtian")
                return notify_fail("你没有激发太始帝经为特殊内功，无法使用" YINYANG "。\n");
        ...
        if ((lvl = (int)me->query_skill("yinyangjiuzhuanshierchongtian", 1)) < 50)
	        return notify_fail("你的太始帝经还不够熟练，无法使用" YINYANG "！\n");

        message_sort(HIM "\n$N" HIM "运起太始帝经，左手" HIB "阴"
