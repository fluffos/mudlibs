...
#include <ansi.h>
string query_name() { return "[火魔法]"ZJBR"焦热地狱"; }
inherit F_SSERVER;
int perform(object me, object target)
{
        ...
        if (!target || !target->is_character() || !me->is_fighting(target) )
                return notify_fail(RED"「火魔法·焦热地狱」只能对战斗中的对手使用。\n"NOR);

	if((int)me->query("neili")< 3000)
	return notify_fail("你的法力太差了！\n");

	if((int)me->query_skill("huozhi-shengdian",1)< 100)
...
msg += HIY"$N"HIB"变成爆炎，把一切都烧毁吧，火魔法·焦热地狱！！\n"NOR;
        if ( random(me->query("combat_exp"))>(int)target->query("combat_exp")/3)
        {
                damage = (int)me->query_skill("huozhi-shengdian", 1);
//对火属性目标无作用
