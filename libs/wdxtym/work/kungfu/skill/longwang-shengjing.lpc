...
&& victim!=me)
{
	if ( victim->query_temp("weapon") || victim->query_skill("longwang-shengjing", 1) < 100
	|| victim->query_skill_prepared("strike") != "longwang-shengjing" )
		return SKILL_D("parry")->query_parry_msg(weapon);
        else {
        string *slbw = ({
                "$n更不回身，右手一招「神龙摆尾」向后挥去，这是龙王圣经中的救命绝招，$p在情急之下使将出来，更是威力倍增！！\n",
...
([      "action": "$N突然身形飞起，双掌居高临下一招"+(order[random(13)])+"「龙王咆哮」"NOR"拍向$n的$l",     
    "dodge": 350,
    "parry": 350,
    "force": 350,
        "damage": 500,
...
([      "action": "$N右掌一招"+(order[random(13)])+"「龙王之怒」"NOR"，迅捷无比地劈向$n的$l",
    "dodge": 350,
    "parry": 350,
    "force": 350,
        "damage": 500,
...
([      "action": "$N双掌施出一招"+(order[random(13)])+"「龙王獠牙」"NOR"，隐隐带着风声拍向$n的$l",
    "dodge": 350,
    "parry": 350,
    "force": 350,
        "damage": 500,
...
([      "action": "$N左掌聚成拳状，右掌一招"+(order[random(13)])+"「龙王神威」"NOR"缓缓推向$n的$l",
   "dodge": 350,
    "parry": 350,
    "force": 350,
        "damage": 500,
...
int valid_learn(object me)
{
   ...
   return notify_fail("练龙王圣经必须空手。\n");

    if ((int)me->query("max_qi") < 20000)
   return notify_fail("你的气血太弱，无法练龙王圣经。\n");

    if ((int)me->query("max_neili") < 20000)
   return notify_fail("你的内力太弱，无法练龙王圣经。\n");
    return 1;
}
...
mapping query_action(object me, object weapon)
{
	...
	{
//             if ( me->query_skill("longwang-shengjing") > 280 && random(me->query_skill("force")) > 200 && me->query("neili") > 1000  )
             if ( random(me->query_skill("longwang-shengjing")) > 280 && random(me->query_skill("force")) > 200 && me->query("neili") > 1000  )
		{
			return ([
			...
			"parry": 25,
			"force": 650,
			"damage": 800+random(600),
			"damage_type": random(2)?"内伤":"瘀伤" ]);
		}
		else if ( random(me->query_skill("longwang-shengjing")) > 230 && random(me->query_skill("force")) > 200 && me->query("neili") > 1000  )
		{
			me->add("neili", -10);
			return ([
		...
		else if ( random(me->query_skill("longwang-shengjing")) > 200 && random(me->query_skill("force")) > 150 && me->query("neili") > 500)
		{
			me->add("neili", -10);
			return ([
			...
			"damage": 1000+random(300),
			"damage_type": random(2)?"内伤":"瘀伤" ]);
		}
    else    if (random(me->query_skill("longwang-shengjing",1)) > 250 &&		 me->query_skill("huntian-qigong",1) > 200 &&
        me->query("neili") > 200)
{
	me->add("neili", -10);
        return ([
...
else	if (random(me->query_skill("longwang-shengjing",1)) > 200 &&		me->query_skill("huntian-qigong",1) > 100 &&
        me->query("neili") > 200)
{
	me->add("neili", -10);
        return ([
...
me!=ob)
{
   if(living(me) &&
      ...
      me->query_skill("longwang-shengjing", 1) >= 180 
      &&  !me->is_busy()
      &&      me->query("neili") > 600 &&
      ...
      )
      {
       ...
       // me->set("jiali", me->query_skill("force")*2);
        ...
        message_vision(HIC "但见$N硬接了$n这一招，右掌紧接着向后一摆，一招「神龙摆尾」出其不意地按在了$n的身上！\n" NOR, me, ob );
        COMBAT_D->do_attack(me, ob, me->query_temp("weapon"), 1);
        COMBAT_D->do_attack(me, ob, me->query_temp("weapon"), 1);
        COMBAT_D->do_attack(me, ob, me->query_temp("weapon"), 1);
        COMBAT_D->do_attack(me, ob, me->query_temp("weapon"), 1);
        COMBAT_D->do_attack(me, ob, me->query_temp("weapon"), 1);
        ob->receive_wound("qi", me->query_skill("longwang-shengjing", 1) , me);
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        lvl = me->query_skill("longwang-shengjing", 1);
...
//return 0;
        if (random(3)==0)
        {
                victim->receive_wound("qi", (lvl+random(50)) , me);
...
{
    return __DIR__"longwang-shengjing/" + action;
}
int difficult_level()
{
        return 2500;
