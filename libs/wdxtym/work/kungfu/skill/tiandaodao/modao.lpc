...
#define XIAN "「" HIR "魔刀三式" NOR "」"
string query_name() { return "魔刀"ZJBR"三式"; }
inherit F_SSERVER;
...
int perform(object me, object target)
{
        ...
        if ((int)me->query_skill("tiandaodao", 1) < 800)
                return notify_fail("你天道刀火候不足，无法施展" XIAN "。\n");
        ...
        if (me->query_skill_mapped("blade") != "tiandaodao")
                return notify_fail("你没有激发天道刀，无法使用" XIAN "。\n");
        ...
        ap=me->query_skill("blade")+me->query_skill("force")+me->query_skill("martial-cognize",1)+me->query("str");
        ...
        // 第一刀
        msg = HIY "$N" HIY "踏步向前，施展出魔刀三式之「魔道横行」，" + weapon->name() + HIY
              "挥刀横扫，如魔行道，大地回归黑暗 ……\n" NOR;
        ...
        // 第二刀
        msg += HIY "\n$N" HIY "轻叹，猛然间，手臂一挥，一式魔刀三式之「天地无情」已然使出，" + weapon->name() + HIY "长鸣一声"
