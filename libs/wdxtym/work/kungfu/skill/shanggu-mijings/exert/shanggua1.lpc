...
string query_name() { return "[古秘术]"ZJBR"恶鬼吞噬"; }
...
int exert(object me, object target)
{
	...
	if (target->query("race") != "人类" ||
		...
		return notify_fail("搞错了！只有活着的生物才能有内力！\n");
	...
	if ((int)me->query_skill("shanggu-mijings", 1) < 120)
		return notify_fail("你的上古秘经功力不够，不能吸取对方的内力！n");

	if ((int)me->query("neili") < 50)
		return notify_fail("你的内力不够，不能使用上古秘经。\n");
	...
	if ((sp + random(sp) > dp + random(dp) ) || ! living(target))
	{
		...
		tell_object(me, HIG "你觉得" + target->name() +
			    HIG "的内力自手掌源源不绝地流了进来。"NOR"\n");

		sucked = 1 + (me->query_skill("shanggu-mijings", 1) - 150) / 10*100;
