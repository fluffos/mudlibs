...
int valid_learn(object me)
{
	...
	lvl = me->query_skill("hongyan-wu", 1);
...
string query_txt()
{
	return "要求：内力(3000+等级*2)"ZJBR
	"特效：轻功化解伤害，对比自身(轻功激发)和攻击者轻功激发，同激发等级1/1概率触发，受阴阳八卦克制";
}
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
	...
	if ((int)me->query_skill("hongyan-wu", 1) < 100 ||
	    ! living(me))
		return;
...
int practice_skill(object me)
{
	...
	cost = me->query_skill("hongyan-wu", 1) / 2 + 120;
	if ((int)me->query("qi") < 70)
		return notify_fail("你的体力太差了，难以练习红颜玲珑舞。\n");

	if ((int)me->query("neili") < cost)
		return notify_fail("你的内力不够了，无法练习红颜玲珑舞。\n");
...
{
	return __DIR__"hongyan-wu/" + action;
}
