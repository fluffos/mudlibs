...
int valid_learn(object me)
{
      ...
      if (me->query("con") < 36)
        ...
        if ((int)me->query_skill("force", 1) < (int)me->query_skill("liumai-shenjian", 1) + 10)
                return notify_fail("你现在必须先提高你基本内功的水平。\n");

        if ((int)me->query_skill("finger", 1) < (int)me->query_skill("liumai-shenjian", 1) + 10)
                return notify_fail("你现在必须先提高你基本指法的水平。\n");
...
int practice_skill(object me)
{
        ...
        cost = me->query_skill("liumai-shenjian", 1) / 2 + 100;
...
int difficult_level()
{
        ...
        if (me->query_skill("jiuyang-shengong", 1) > 200 && !me->query("reborn"))
       return 1000;
       else   return 600;
}
