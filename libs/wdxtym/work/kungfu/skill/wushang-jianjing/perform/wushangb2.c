...
string name() { return HIB "绝命剑气" NOR; }
string query_name() { return "绝命"ZJBR"剑气"; }
inherit F_SSERVER;
...
int perform(object me, object target)
{
        ...
        if (me->query_skill("wushang-jianjing", 1) < 600)
                return notify_fail("你的无上剑经修为不够，难以施展" +name()+ "。\n");
        ...
        if (1)
        {
                ...
                ran = 35;
                ndam = 10;
                if (me->query_skill("wushang-jianjing", 1) >= 500)
                {
                        ran = 100;
                        ndam = 5;
                }
                else if (me->query_skill("wushang-jianjing", 1) >= 400)
                {
                        ndam = 7;
                }
                else if (me->query_skill("wushang-jianjing", 1) >= 300)
                {
                        ran = 50;
                }
                else if (me->query_skill("wushang-jianjing", 1) >= 200)
                {
                        ran = 50;
                }
                // 追加一次武器伤害
                if (random(100) < ran || wizardp(me))
                {
                        ...
                        target->receive_damage("qi", weapon->apply_damage() * 13 / 2, me);
                        target->receive_wound("qi", weapon->apply_damage() * 13 / 2, me);
                }
        
                if( !target->query_temp("wushang-jianjing/jinglei") )
                {
                        // 第一次直接10%最大气血伤害
                        ...
                        target->set_temp("wushang-jianjing/jinglei", 1);
                }
        } else
        {
                me->add("neili", -2500);
