...
#define RI "「" HIW "无限剑阵" NOR "」"
string query_name() { return "[群攻]"ZJBR"无限剑阵"; }
int perform(object me)
{
        ...
        if (me->query_skill("wushang-jianjing", 1) < 250)
                return notify_fail("你的无上剑经还不够娴熟，难以施展" RI "。\n");

        if ((int)me->query("neili") < 5000)
                return notify_fail("你的真气不够，无法运用" RI "。\n");

        msg = HIY "只见$N" HIY "双目微闭，单手执剑。周围空间破碎，"
              "时空泯灭，正是奥\n义「" NOR + HIW "无限剑阵" NOR + HIY "」。霎时"
        ...
        for (flag = 0, i = 0; i < sizeof(obs); i++)
        {
                ...
                if (ap * 5 / 1 + random(ap) > dp)
                {
                        ...
                        obs[i]->receive_damage("jing", damage * 2 / 1, me);
                        obs[i]->receive_wound("jing", damage * 2 / 1, me);

                        p = (int)obs[i]->query("qi") * 2 / (int)obs[i]->query("max_qi");
