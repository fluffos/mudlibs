...
/* 
        if (userp(me) && ! me->query("can_perform/tanzhi-shentong/xiao"))
                return notify_fail("你暂时还不会施展" XIAO "。\n"); */
		...
		ap = me->query_skill("finger")+me->query("qimen-wuxing")/10;
		dp = target->query_skill("dodge");

        damage = me->query_skill("finger")+me->query("qimen-wuxing");
