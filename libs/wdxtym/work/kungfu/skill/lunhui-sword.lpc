...
/*
        if( !ob->query("lunhui") )
        {
                tell_object(me, "演练六道仙经必须在轮回司中，需要借助上古神器昊天塔之力。\n");
                return 0;
        }
       */
        if( me->query("int")<50 )
        ...
        {
                tell_object(me, "你觉得有所感悟，或许再演练一次就能融会贯通，练成六道仙经。\n");
                return 0;
        }

        if (me->query_skill("sword", 1) < 1000)
        {
                tell_object(me, "你基本剑法火候不足，难以演练成六道仙经。\n");
                return 0;
        }
        if (me->query_skill("force", 1) < 1000)
        {
                tell_object(me, "你基本内功火候不足，难以演练成六道仙经。\n");
                return 0;                
        }
        if (me->query_skill("martial-cognize", 1) < 1000)        
        {
                tell_object(me, "你武学修养火候不足，难以演练成六道仙经。\n");
                return 0;                
        }
        if (me->query_skill("dodge", 1) < 1000)        
        {
                tell_object(me, "你基本轻功火候不足，难以演练成六道仙经。\n");
                return 0;                
        }        
        if ((int)me->query_skill("buddhism", 1) < 1000)
        {
                tell_object(me, "你禅宗心法修为不足，难以演练成六道仙经。\n");
                return 0;        
        }
        me->save();
        ...
        CHANNEL_D->do_channel(this_object(), "rumor", "听说" + me->name() + "演练出"HIY"六道仙经"HIM"。\n");
        return 1;
}
...
int valid_learn(object me)
{
        ...
        if( me->query_skill("badao",1) )
                return notify_fail("开天仙经刀意太强，与你体内的六道仙经相互冲突。\n");                    
                
        if( me->query("max_neili")<10000 )
                return notify_fail("你的内力修为不够，难以修炼六道仙经。\n");
                                            
        if ((int)me->query_skill("sword", 1) < (int)me->query_skill("lunhui-sword", 1))
                return notify_fail("你的基本剑法水平有限，无法领会更高深的六道仙经。\n");
...
{
        return notify_fail("六道仙经博大精深，无法简单的通过练习进步。\n");
}
...
{ 
        ...
        { 
                tell_object(me, HIC "你通晓了六道仙经「" HIR "修罗炼狱" HIC "」的奥秘。\n" NOR);     
        ...
        { 
                tell_object(me, HIC "你通晓了六道仙经「" HIR "轮回剑" HIC "」的奥秘。\n" NOR);     
