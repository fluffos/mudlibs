...
string query_name() { return "风雨"ZJBR"交加"; }
...
int perform(object me, object target)
{
	...
	msg = HIC "$N" HIC "大喝一声，手中的" + weapon->name() +
	      HIC "如雨点一般向$n" HIC "打去，$n" HIC "如同小舟一般在刀雨中漂泊不定。"NOR"\n";

	if (lvl / 2 + random(lvl) > target->query_skill("parry") * 2 / 3)
	{
		msg += HIY "这阵刀势变化莫测，$n" HIY "顿时觉得眼花缭乱，无法抵挡。"NOR"\n";
		count = lvl / 5;
		me->add_temp("apply/attack", count);
	} else
	{
		msg += HIC "$n" HIC "不禁心中凛然，不敢有半点小觑，使出浑身解数抵挡。"NOR"\n";
		count = 0;
	}
