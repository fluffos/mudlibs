...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	if (me->query("family/family_name") == "华山剑宗")
	{
		...
		} else
		{
			...
			return HIM "$N" HIM "默运内功，突然抬头攻出一招，$n" HIM "顿时发现四处剑气纵横，避无可避！"NOR"\n";
		}
	}
	else if(random(2))
	{
		victim->receive_wound("qi", damage_bonus / 4);
		return HIR "只见$N" HIR "这招已然刺伤$n" HIR "，怎知$N" HIR "内劲吞吐，剑气直侵$n" HIR "体内！"NOR"\n";
	}
...
int practice_skill(object me)
{
	...
	cost = me->query_skill("lonely-sword", 1) / 3 + 50;
	if ((int)me->query("neili") < cost)
		return notify_fail("你的内力不够练独孤九剑。\n");

	//tell_object(me, HIG "你心中默念独孤九剑之「总诀式」，将手中" + weapon->name() + HIG "随意挥舞击刺。"NOR"\n");
