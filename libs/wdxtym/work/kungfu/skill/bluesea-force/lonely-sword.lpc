...
mapping query_action(object me, object weapon)
{
	...
	if( level > 350 )
		return ([
			...
			"dodge": 400+random(100),
			"parry": 400+random(100),
			"force": 400 + random(200),
	            "attack": 600,
			"damage": 200 + random(40),
			"damage_type": "刺伤",
		]);
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	if ((me->query("family/family_name") == "华山剑宗" &&
	    i > 50 && random(2) == 0) || (me->query("family/family_name") == "华山剑宗" && weapon->query("magic/skill/lonely-sword")))
	{
		if (i < 120)
		{
			victim->receive_wound("qi", damage_bonus / 4, me);
			return HIR "只见$N" HIR "这招已然刺伤$n" HIR "，怎知$N" HIR
			       "内劲吞吐，剑气直侵$n" HIR "体内！"NOR"\n";
		}
		if (i < 240)
		{
			victim->receive_wound("qi", damage_bonus / 2, me);
			return HIG "但见$N" HIG "微微一笑，全身劲气贯于剑中，剑"
			       "气源源不断扑向$n" HIG "！"NOR"\n";
		} else
		{
			victim->receive_wound("qi", damage_bonus *2, me);
			return HIM "$N" HIM "默运内功，突然抬头攻出一招，$n" HIM
			       "顿时发现四处剑气纵横，避无可避！"NOR"\n";
		}
	}
}
...
int practice_skill(object me)
{
	...
	if ((int)me->query("neili") < 400)
		return notify_fail("你的内力不够练独孤九剑。\n");

	//message_vision(HIG "$N"HIG"使出独孤九剑之「总诀式」，将手中" +weapon->name()+ HIG "随意挥舞击刺。"NOR"\n",me);
