   ...
   if (me->query("id")=="beidao")
	...
	for (flag = 0, i = 0; i < sizeof(obs); i++)
	{
		...
		if (ap / 2 + ap > dp)
		{
			...
			damage2 = me->query("max_qi")+me->query("gain/max_qi");
			damage2 = damage2 * (15 + random(6)) / 100;
			obs[i]->receive_damage("qi", damage2, me);
			obs[i]->receive_wound("qi", damage2 / 2, me);
