...
//}
     if( (int)me->query_skill("zixia-shengong",1)<100)
        ...
        if( (int)me->query_skill("lonely-jian", 1) < 150 || (int)me->query("max_neili") < 500 )
	...
	if (!objectp(weapon = me->query_temp("weapon"))
		...
		damage = random((int)me->query_skill("lonely-jian"))*2;
        if( random(me->query("combat_exp")) > (int)target->query("combat_exp")/3 ) {
            if( !weapon = target->query_temp("weapon") ) {
             switch (random(2)) {
             case 0:
                msg += COMBAT_D->do_damage(me, target, WEAPON_ATTACK, damage, 100,
				HIR"$p意外之余，闪避无从，惊骇中连中数剑受伤。\n" NOR); 
                target->add("eff_qi",-(random((int)me->query_skill("lonely-jian"))*2));
                break;
             case 1:
                msg += COMBAT_D->do_damage(me, target, WEAPON_ATTACK, damage, 100,
				HIY"$p惊骇莫明，欲避不能，被$P刺中丹田，功力受损。\n" NOR);
                target->add("neili",-(random((int)me->query_skill("lonely-jian"))*2));
                break;
             }
            }
            else {
...
if (!weapon->query("ownmake"))
{
             ...
             case 1:
                msg += COMBAT_D->do_damage(me, target, WEAPON_ATTACK, damage, 100,
				HIM"$p惊骇莫明，欲避不能，被$P刺中丹田，功力受损。\n" NOR);
                target->add("neili",-(random((int)me->query_skill("lonely-jian"))*2));
                break;
             }
            }
           target->start_busy( (int)me->query_skill("lonely-jian") / 50+1 );
        } 
        else {
