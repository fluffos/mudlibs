...
skill=(int)me->query_skill("lonely-jian", 1);
...
//           return notify_fail("你不是风清扬的弟子，不能使用绝招！\n");
    ...
    if ((int)me->query_skill("lonely-jian", 1) < 250 )
    ...
    if ((int)me->query("max_neili")<2000)
        ...
        if ( !wizardp(me) && me->query_skill_mapped("force") != "zixia-shengong")
                return notify_fail("不使用紫霞神功，如何使用本门绝学!\n");
...
if (extra >=200) extra =200;
	...
	me->add_temp("apply/damage", -2000);
       ...
       msg = HIW "$N" HIW "施出独孤九剑「破气式」，手中" + weapon->name() +
        ...
        {
                damage = me->query_skill("sword")+(int)me->query_skill("lonely-jian", 1);
target->receive_damage("qi", damage*2,me);
target->receive_wound("qi", damage*2,me);
//target->start_busy(2);
                        msg +=COMBAT_D->do_damage(me, target, SPECIAL_ATTACK, damage, 100,
						HIR "$n" HIR "被$N" HIR "一剑划过气门，登感一阵"
                "锥心的刺痛，全身真气源源而泻！\n" NOR);
...
if (skill > 350 && me && target) 
{
 ...
 if(random(2)==0 && (random(me->query("combat_exp",1)) >= target->query("combat_exp")/2 || !userp(target)))
  {
		damage=target->query("eff_qi")/6;
		damage2=target->query("eff_jing")/6;
