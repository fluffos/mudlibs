...
/* 	if (! me->query("couple/id"))
		return notify_fail("你伴侣都没有，还黯然什么？\n"); */
	...
	// if (! stringp(couple = me->query("couple/id")))
		// return notify_fail("你没有伴侣，体会不到这种万念俱灰的感觉。\n");

/* 	if (! objectp(cob = find_player(couple)))
		return notify_fail("老婆不在啊，苦闷向谁发？\n"); */

/* 	if (time() - me->query_temp("last_perform/sad-strike/tuo") < 20)
	...
	{ 
		damage = (ap + random(ap / 2))*(me->query_skill("taishang-wangqing", 1)+400)/400;
		me->add("neili", -300);
