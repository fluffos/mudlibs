...
string query_name() { return "祭血"ZJBR"神刀"; }
...
int perform(object me, object target)
{
	...
	if ((int)me->query_skill("xue-dao", 1) < 100)
		return notify_fail("你血刀不够娴熟，使不出「祭血神刀」。\n");
	...
	if (me->query_skill_mapped("blade") != "xue-dao")
		return notify_fail("你没有激发血刀，不能使用「祭血神刀」。\n");

       if (! living(target))
	      return notify_fail("对方都已经这样了，用不着这么费力吧？\n");

	msg = HIR "$N" HIR "右手持刀向左肩一勒，一阵血珠溅满刀面，紧接着右臂抡出，一片血光"
	      "裹住刀影向$n" HIR "当头劈落。"NOR"\n";
	...
	zhenshang = me->query_skill("xue-dao", 1)*daogang/50;
		msg += HIR "$P" HIR "爆喝一声，血海魔刀运转至极致，刀身竟催出丈许刀罡！"NOR"\n"HIR"真实伤害 "+zhenshang+"点。"NOR"\n";
