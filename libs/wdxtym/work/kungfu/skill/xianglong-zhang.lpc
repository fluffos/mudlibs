...
{	
	if( me->query("zhuanshi")<3 )
        return notify_fail( "你只感觉修为无法驾驭。\n");
	if( me->query("int")<23 )
        return notify_fail( "你只感掌法晦涩难懂。\n");
        if( me->query("dex")<28 )
         return notify_fail( "你只觉掌法灵动非凡。\n");
        if( me->query("con")<30 )
         return notify_fail("你只觉掌法气血翻涌。\n");
        if( me->query("str")<40 )
        return notify_fail( "你只觉掌法太过沉重。\n");
   ...
   if(living(me) &&
      ...
      )
      {
       ...
       // me->set("jiali", me->query_skill("force")*2);
        ...
        COMBAT_D->do_attack(me, ob, me->query_temp("weapon"), 1);
        COMBAT_D->do_attack(me, ob, me->query_temp("weapon"), 1);
        ob->receive_wound("qi", me->query_skill("xianglong-zhang", 1) , me);
