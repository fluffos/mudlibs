...
mapping *action = ({
([      "action":"$N右手托住杖端，一招「拦鹊式」，左掌居中一击，令其凭惯性倒向$n的肩头",
        "force" : 65,
        "attack": 12,
        "dodge" : -10,
        "parry" : 5,
...
([      "action":"横持$w，一招「苍龙摆尾」，杖端化出无数个圆圈，凝滞沉重，把$n缠在其中",
        "force" : 240,
        "attack": 67,
        "dodge" : 5,
        "parry" : 22,
...
int valid_learn(object me)
{
        ...
        if(me->query("max_neili")<200 )
                return notify_fail("你的内力修为不足，无法学习鹿头杖法。\n");

        if ((int)me->query_skill("force") < 60)
                return notify_fail("你的内功火候太浅，无法学习鹿头杖法。\n");

        if ((int)me->query_skill("staff", 1) < 20)
                return notify_fail("你的基本杖法太浅，无法学习鹿头杖法。\n");

        if ((int)me->query_skill("staff", 1) < (int)me->query_skill("lutou-zhang", 1))
                return notify_fail("你的基本杖法水平有限，无法领会更高深的鹿头杖法。\n");
...
int practice_skill(object me)
{
        ...
        if(me->query("qi")<80 )
                return notify_fail("你的体力不够练鹿头杖法。\n");

        if(me->query("neili")<80 )
                return notify_fail("你的内力不够练鹿头杖法。\n");
