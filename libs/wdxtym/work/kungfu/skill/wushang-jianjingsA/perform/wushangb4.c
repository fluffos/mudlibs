...
string query_name() { return "剑仙"ZJBR"之影"; }
...
int perform(object me, object target)
{
        ...
        message_sort(HIM "\n$N" HIM "口中默念：剑修之影，"
                     "空间似乎也扭曲了起来。$n" HIM "大惊失色，仿佛被剑仙包围！\n" NOR, me, target);


        ap = me->query_skill("force") + me->query_skill("wushang-jianjingsAA", 1);
        ...
        // 第一招，判断对方内力
        message_sort(HIW "$N" HIW "剑身一震，一股浑厚至极的内劲汹涌澎湃，"
                     "带着排山倒海之势向$n" HIW "劈砍而至。\n" NOR, me, target);
...
ap1 = me->query_skill("guzheng-jifa", 1) + me->query_skill("tanqin-jifa", 1) + me->query_skill("chuixiao-jifa", 1) + ap;

dp1 = target->query_skill("guzheng-jifa", 1) + target->query_skill("tanqin-jifa", 1) + target->query_skill("chuixiao-jifa", 1) + dp;

        message_sort(HIW "\n$N" HIW "一声清啸，凭借浑厚的内力，一剑刺出,"
		     HIW "时光仿佛倒流。\n" NOR, me, target);
        ...
        if (ap1 * 2 / 3 + random(ap1) > dp1)
        {
                ...
                target->receive_damage("jing", damage * 3 / 4, me);
                target->receive_wound("jing", damage / 3, me);
                p = (int)target->query("qi") * 125 / (int)target->query("max_qi");

                msg = HIY "$N" HIY "趁着对方短暂失神，迎步而上，$n" HIY "却仍在失神中，"
                      HIY "$N" HIY "一击得手。\n" NOR;
                msg += "( $n" + eff_status_msg(p) + " )\n";
        } else
        {
                msg = CYN "幸而$n" CYN "精通剑术，严阵以待，$N" CYN "无处下手。\n" NOR;
        }
        message_sort(msg, me, target);

        // 第三招，判断对方佛学及道学修养.
ap1 = me->query_skill("buddhism", 1) + me->query_skill("lamaism", 1) + me->query_skill("taoism", 1) + ap;

 dp1 = target->query_skill("buddhism", 1) + target->query_skill("lamaism", 1) + target->query_skill("taoism", 1) + dp;
        ...
        if (ap1 * 2 / 3 + random(ap1) > dp1)
        {
                ...
                msg = HIY "霎时间$n" HIY "只感心头滚热，喉干舌燥，全身似被万剑拂过。\n" NOR;
                msg += "( $n" + eff_status_msg(p) + " )\n";
        } else
        {
	...
	dp1 /= 20000;
        ...
        // 第五招，判断对方先天臂力及基本招架
  ap1 = me->query("str") * 30 + me->query_skill("parry", 1) + ap;
  ...
  dp1 = target->query("str") * 30 + target->query_skill("parry", 1) + dp;
        ...
        ap1 = me->query_str() * 2 + me->query_skill("unarmed", 1) + ap;
        ...
        ap1 = me->query("int") * 30 + me->query_skill("parry", 1) + ap;

        dp1 = target->query("int") * 30 + target->query_skill("parry", 1) + dp;
        ...
        // 第八招，判断对方后天悟性及读书写字
        ap1 = me->query_int() * 2 + me->query_skill("literate", 1) + ap;
		...
		// 第九招，判断对方先天根骨及基本招架
        ap1 = me->query("con") * 30 + me->query_skill("parry", 1) + ap;
        ...
        // 第十招，判断对方后天根骨及基本内功
        ap1 = me->query_con() * 2 + me->query_skill("force", 1) + ap;
        ...
        // 第十一招，判断对方先天身法及基本招架
        ap1 = me->query("dex") * 30 + me->query_skill("parry", 1) + ap;
        ...
        // 第十二招，判断对方后天身法及基本轻功
        ap1 = me->query_dex() * 2 + me->query_skill("dodge", 1) + ap;
