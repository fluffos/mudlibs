...
#include <ansi.h>
string query_name() { return BYEL+HIR"万法"ZJBR"唯我"NOR; }
inherit F_CLEAN_UP;
...
int exert(object me, object target)
{
        int skill;
        if( target != me ) return
            notify_fail("你只能对自己用["+HIB+"万法唯我"+NOR+"]\n"NOR,);
	if ((int)me->query_skill("liangyi-yinyangjue", 1) < 200)
		return notify_fail("你的阴阳道经太差了。\n");
	...
	me->set_temp("jiuyin/powerup", 1);
        ...
        message_combatd(HIC"$N运起了万法唯我，瞬间将丹田之气运转一周天，功夫威力陡然大增！\n" NOR, me);
        me->add_temp("apply/armor", skill/2);
        		tell_object(me, HIG"你的防御提升了。\n"NOR);
        me->add_temp("apply/attack", skill/2);
        tell_object(me, HIG"你的命中提升了。\n"NOR);
        me->add_temp("apply/dodge", skill/2);
        tell_object(me, HIG"你的闪避提升了。\n"NOR);
...
void remove_effect(object me, int skill)
{
        ...
        me->add_temp("apply/dodge", -skill/2);
        me->add_temp("apply/damage", -skill/2);
        tell_object(me, "你的["+HIB+"万法唯我"+NOR+"]运行完毕，将内力收回丹田。\n");
}
