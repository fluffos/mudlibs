...
int valid_learn(object me)
{
	if ((int)me->query("str") < 24)
		return notify_fail("你这小身板练七伤拳，恐怕打不倒别人自己先倒下了吧！\n");

	if ((int)me->query("con") < 30)
		return notify_fail("你先天根骨不足，七伤拳恐怕伤己大过伤敌，别强学了。\n");

	if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
		return notify_fail("练七伤拳必须空手。\n");

	if ((int)me->query_skill("force") < 120)
		return notify_fail("你的基本内功不足120级，无法学七伤拳。\n");

	if ((int)me->query("max_neili") < 700)
		return notify_fail("你的最大内力不足700点，无法练七伤拳。\n");

	if ((int)me->query_skill("cuff", 1) < 100)
		return notify_fail("你的基本拳法不足100级，无法练七伤拳。\n");

	if ((int)me->query_skill("cuff", 1) < (int)me->query_skill("qishang-quan", 1))
		return notify_fail("你的基本拳法水平还不够，无法领会更高深的七伤拳法。\n");
