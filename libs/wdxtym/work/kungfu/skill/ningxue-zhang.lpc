...
int valid_learn(object me)
{
        ...
        if(me->query("max_neili")<200 )
                return notify_fail("你的内力修为不足，无法学习凝雪杖法。\n");

        if ((int)me->query_skill("force") < 60)
                return notify_fail("你的内功火候太浅，无法学习凝雪杖法。\n");

        if ((int)me->query_skill("staff", 1) < 20)
                return notify_fail("你的基本杖法太浅，无法学习凝雪杖法。\n");

        if ((int)me->query_skill("staff", 1) < (int)me->query_skill("ningxue-zhang", 1))
                return notify_fail("你的基本杖法水平有限，无法领会更高深的凝雪杖法。\n");
...
int practice_skill(object me)
{
        ...
        if(me->query("qi")<80 )
                return notify_fail("你的体力不够练凝雪杖法。\n");

        if(me->query("neili")<80 )
                return notify_fail("你的内力不够练凝雪杖法。\n");
