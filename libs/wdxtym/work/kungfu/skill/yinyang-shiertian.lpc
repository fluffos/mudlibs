...
int query_neili_improve(object me)
{
        ...
        return lvl /20 * lvl /20 * 7 * 20 / 20 / 40;
}
...
int get_finish(object me)
{
        ...
        {
                tell_object(me, "你觉得有所感悟，或许再演练一次就能融会贯通，练成十二重天。\n");
                return 0;
        }
        ...
        CHANNEL_D->do_channel(this_object(), "rumor", "听说" + me->name() + "演练出"HIY"十二重天"HIM"。\n");
        return 1;
}
...
string *usage_skills = ({ "unarmed", "strike",  "finger",
                          "sword", "blade",  "club",
                          "dodge",  "parry", "force" });
...
int valid_learn(object me)
{
       ...
       if (me->query_skill("martial-cognize", 1) < level &&
            me->query_skill("martial-cognize", 1) < 600)
                                return notify_fail("你的武学修养不够，无法继续修炼。\n");

        if( level>380 && me->query("yinyang-shiertian/yinyang") )
                                return notify_fail("你的十二重天已达到新的层次，须修炼出阴阳境界方能再次提升。\n");

        if( level>480 && me->query("yinyang-shiertian/qiankun") )
                                return notify_fail("你的十二重天已达到新的层次，须修炼出乾坤境界方能再次提升。\n");

        if( level>600 && me->query("yinyang-shiertian/shier") )
                                return notify_fail("你的十二重天已达到新的层次，须修炼出十二重天境界方能再次提升。\n");
...
{
        return notify_fail("十二重天博大精深，无法简单的通过练习进步。\n");
}
