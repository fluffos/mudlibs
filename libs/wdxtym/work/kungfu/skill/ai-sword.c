               ...
               if( random(mecor) >= random(victimcor))
                {
                   ...
                   victim->receive_damage("qi",sk+random(1000));
 //                  victim->receive_damage("jing",1*(victim->query("max_jing")/10));
                   victim->receive_damage("jing",(victim->query("max_jing")/10));
                   if (damage <= 0)
...
void five_kill(object me,object victim,object weapon,int damage)
{
	int dmg_qi,dmg_jing,bell = me->query("bellicosity");
...
//        victim->receive_damage("jing",1*(victim->query("max_jing")/5));
	...
	else
	{
        ...
        victim->receive_damage("qi",dmg_qi / 2 );
        victim->receive_damage("jing",1*(victim->query("max_jing")/10));
        victim->start_busy(1);
        me->add("neili",-(me->query("neili")/10));
  }
...
void power_slash(object me,object victim, object weapon,int damage)
{
 ...
 victim=enemy[j];
    ...
    {
     victim->receive_wound("qi", damage+random(200));
     switch(random(4)+1)
    {
     case 1 :
...
message_vision(sprintf(MAG"结果被$N确确实实的命中要害，$n身上立即多了一道无情的伤痕！\n"NOR),me,victim);
          victim->receive_wound("qi",900+random(2000));
       ...
       for(i=1;i<=3;i++)
       {
         switch(i)
         {
           ...
           if( me->query_skill("dream-blade",1) > random(60)+100)
           {
                  ...
                  victim->receive_wound("qi",800+random(1000));
                  me->add("neili",-50);
                  if(i != 3)
                  COMBAT_D->report_status(victim,1);
                  else if( damage <=0 )
...
int valid_learn(object me)
{
        ...
        if(skill < 180 || me->query_skill("sword", 1) < 500 )         //解ai-sword的mark
        {
         ...
         /*if( bell > 100 )
         {
                tell_object(me, HIY"你心存杀念, 怎么能够学习情剑呢！\n");
                return 0;
         }*/
