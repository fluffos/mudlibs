...
int query_neili_improve(object me)
{
        ...
        lvl = (int)me->query_skill("shanggu-mijings", 1);
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        lvl = me->query_skill("shanggu-mijings", 1);

        if (damage_bonus < 150
           || lvl < 140
           || me->query("neili") < 300
           || me->query_skill_mapped("force") != "shanggu-mijings")
                return 0;

        // 未学会驾御兵器
        if (! me->query("can_learned/shanggu-mijings/enable_weapon"))
        {
             if (me->query_skill_prepared("unarmed") != "shanggu-mijings")
                      return 0;
        }
        ...
        {             
             ...
             if (objectp(weapon) && weapon->query("skill_type") == "sword"
                 && me->query_skill_mapped("sword") != "shanggu-mijings")
                         return 0;
            
             if (objectp(weapon) && weapon->query("skill_type") == "blade"
                 && me->query_skill_mapped("blade") != "shanggu-mijings")
                         return 0; 

             if (objectp(weapon) && weapon->query("skill_type") == "axe"
                 && me->query_skill_mapped("axe") != "shanggu-mijings")
                         return 0;

             if (objectp(weapon) && weapon->query("skill_type") == "club"
                 && me->query_skill_mapped("club") != "shanggu-mijings")
                         return 0;	

             if (objectp(weapon) && weapon->query("skill_type") == "hammer"
                 && me->query_skill_mapped("hammer") != "shanggu-mijings")
                         return 0;	

             if (objectp(weapon) && weapon->query("skill_type") == "staff"
                 && me->query_skill_mapped("staff") != "shanggu-mijings")
                         return 0;	

             if (objectp(weapon) && weapon->query("skill_type") == "spear"
                 && me->query_skill_mapped("spear") != "shanggu-mijings")
                         return 0;	

             if (objectp(weapon) && weapon->query("skill_type") == "whip"
                 && me->query_skill_mapped("whip") != "shanggu-mijings")
                         return 0;						 
        }
        ...
        if (random(damage_bonus) / 2 > victim->query_con() && random(2))
        {
                ...
                if (me->query("can_learned/shanggu-mijings/enable_weapon") && objectp(weapon))
                {
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
        ...
        dp = me->query_skill("shanggu-mijings", 1);
...
{
	return notify_fail("上古秘经只能用学的，或是从运用(exert)中增加熟练度。\n");
}

string perform_action_file(string action)
{
        return __DIR__"shanggu-mijings/perform/" + action;
}

string exert_function_file(string action)
{
        return __DIR__"shanggu-mijings/exert/" + action;
}

void skill_improved()
{
	this_player()->add("exception/anye-count",
			   (int)this_player()->query_skill("shanggu-mijings", 1) + 1);
}
