...
int ask_quest()
{
	...
	if (userp(me)) {
		if (me->query("fuben/FB001")) {
		ob_list = children("/adm/daemons/fuben_FB001");
		...
		last = localtime(me->query("fuben/FB001/times"));
		times = me->query("fuben/FB001/mun");
		if(last[7]!=localtime(time())[7])
			me->set("fuben/FB001/mun",0);
...
int ask_job(string arg)
{
	...
	if (me->query("fuben/FB001/mun") <= cs)
	{
		me->set("fuben/FB001/times",time());
		me->add("fuben/FB001/mun", 1);
		me->add("fuben/FB001/"+data, 1);
		fb_room = new("/adm/daemons/fuben_FB001");//改
		fb_room->init(me);
