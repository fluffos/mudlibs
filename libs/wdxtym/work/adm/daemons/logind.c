...
#define IP_MAX_USER 999 //同一IP允许同时登录的玩家数量
void jiance(string arg,object ob,string str)
{
	...
	else if (arg!=crypt(ZJKEY,str[2..3]) && arg!="zjmDMaIpOvxdb" && arg!="123456789abcd" )
	{
		write("客户端非法\n");
    ...
    if (iplimit > IP_MAX_USER)
    {
        write( ZJTMPSAY+"仅允许同一IP同时登录"+IP_MAX_USER+"个账号。\n");
        destruct(ob);
        return;
    }
	
	if(arg=="123456789abcd") ob->set_temp("web_log",1);
	write("版本验证成功\n");
...
private void init_new_player(object user)
{
	...
	user->set("food", (user->query("str") + 15) * 25);
	user->set("water", (user->query("str") + 15) * 25);
...
varargs void enter_world(object ob, object user, int silent)
{
    ...
    {
        message("channel:chat",HIY"武动仙途："HIW"欢迎新人  "
             +user->query("name")+"("+user->query("id")+")来到武动仙途！请各位多多关照！\n"NOR,users());
    }
...
*/
	if(FAMILY_D->query_family_leader(user->query("family/family_name"))==user->query("id"))
		CHANNEL_D->do_channel(this_object(), "chat",sprintf("%s首席弟子%s(%s)连线进入游戏，武动仙途。",
...
int check_legal_id(string id)
{
	...
	if( (i<4||i>24) && (string)SECURITY_D->get_status(id) == "(player)" ) {
		write(ZJTMPSAY"对不起，你的ID长度不符合要求(4-24)。\n");
		return 0;
	}
