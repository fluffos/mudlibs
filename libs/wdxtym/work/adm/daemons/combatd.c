...
// 修为底线(random(my_exp) > EXP_LIMIT，则不加修为)
...
varargs int do_attack(object me, object victim, object weapon, int attack_type)
{
	...
	int crit,delta;
	int ap, dp, pp,fangbao,kezhi,xixue,fanzhen,chuanjia;
	...
	{  // Does the victim dodge this hit?
		...
		if (userp(me) || userp(victim))  
		{
			...
			{
				//如果随即被攻击者的修为不大于极限修为，并且随机到真就加奖励
				if (your["combat_exp"] < EXP_LIMIT && victim->query("combat_exp")<me->query("combat_exp"))
				{
					tell_object(victim,"你在"HIC"躲避"NOR+me->query("name")+"时对"HIG"【"+to_chinese(dodge_skill)+"】"NOR"顿有所悟: "YEL"武功熟练度 "HIY"↑"NOR","YEL"实战修为 "HIY"↑" NOR"\n");				
					your["combat_exp"]++;
	...
	else  //如果躲闪不成功，就判断是否增加攻击的奖励
	{
		...
		if (userp(me) || userp(victim))     
		{
			...
			if (random(ap + dp) < dp && random(my["int"]) > 12 )
			{
				if( me->query("combat_exp") < victim->query("combat_exp"))
				{
					...
					{
						tell_object(me,"你在"HIR"攻击"NOR+victim->query("name")+"时对"HIG"【"+to_chinese(attack_skill)+"】"NOR"顿有所悟:"YEL" 武功熟练度 "HIY"↑"NOR","YEL"修为点 "HIY"↑" NOR"\n");
						my["potential"]++;
			...
			//如果招架值小于攻击，而招架成功，并且计算 xx>30
		      if (userp(me) || userp(victim))  
		      {
				if (random(ap + pp) < pp && random(my["int"]) > 14)
				{
					...
					if (your["combat_exp"] < EXP_LIMIT && victim->query("combat_exp")<me->query("combat_exp"))
					{
						tell_object(victim,"你在"HIM"招架"NOR+me->query("name")+"时对"HIG"【"+to_chinese(parry_skill)+"】"NOR"顿有所悟:"YEL" 武功熟练度 "HIY"↑"NOR","YEL"修为点 "HIY"↑" NOR"\n");					
						your["combat_exp"]++;		   
						victim->improve_skill(parry_skill, 1);
					}
					else
					{
					     tell_object(victim,WHT+"【你的对手太弱了，无法取得任何修为！！！】"NOR"\n");
					}
				}
		      }
		} 
		else
		{
			...
			//
			//几率触发生成暴击
			xixue=me->query("xixue");	//吸血
			if (random(100)>50){
				if(xixue>0)
				{
					if(damage>100)
					{
						xixue= damage*xixue/ 100;
						me->add("eff_qi", xixue);
						me->add("qi", xixue);
						result += HIY"$N双手运决天道酬勤，一个响指挥之而去！一道闪光！天道汲取$n被抽取了"HIY"百分之"HIY+chinese_number(damage*xixue/ 100)+HIR"的气血！你感觉。浑身一阵！气血洪拥而至！瞬间精神大起！两眼放光？"NOR"\n";
					}else
						me->add("eff_qi", 1);
						me->add("qi", 1);
						result += HIY"$N双手运决天道酬勤，一个响指挥之而去！一道闪光！天道汲取$n被抽取了1点气血！你感觉。浑身一阵！气血洪拥而至！瞬间精神大起！两眼放光？"NOR"\n";
				}}
...
//暴击修正
			...
			if( me->query("yuanshen/level") && me->query("env/yuanshen") == 1 )
			{
				damage += me->query("yuanshen/level") * 70 + random(100);
				me->add("jing",-me->query("yuanshen/level") * 20 + random(100));
				victim->receive_wound("qi",me->query("yuanshen/level") * 70 + random(100), me);
				victim->receive_wound("jing",me->query("yuanshen/level") * 70 + random(100), me);
				if(me->query("env/yuanshenk")==1)
				{
				tell_object(environment(me), sprintf(ZJSIZE(30)HIY"【元神】"HIG"%s"HIW"在对"HIR"%s"HIW"发出攻击的同时附加了"+chinese_number(me->query("yuanshen/level") * 70 + random(100))+ "点元神伤害。\n"NOR,me->name(),victim->name()));
				}
			}
#if INSTALL_COMBAT_TEST
				if (wizardp(me) && me->query("env/combat_test"))
...
void killer_reward(object killer, object victim)
{
	...
	while (1)
	{
		...
		if (ks["combat_exp"] < vs["combat_exp"])
		{
			...
			if(random(20)==1)
			{
				tell_object(killer,"\n"HIM"<恭喜>:"NOR"  你"HIR"杀死了"NOR+victim->query("name")+HIG", 奖励:"GRN"【初级宝箱】"NOR"... "HIY"  <额外修为>: "NOR+HIG+win_exp+YEL+" ↑" NOR"\n");
		...
		if(killer->query("daily/id")==victim->query("id"))
            DAILY_D->set_rewards(killer);
	...
	if (playerp(victim)&&victim->query("combat_exp")<1500000)
	{
		// clear punish info.
		...
		tell_object(victim,HIM + "新手保护期[<150万修为]免去了你的死亡惩罚" + NOR"\n");
	...
	else if (playerp(victim)&&victim->query_temp("kuilei_used"))
	{
		// clear punish info.
		if (objectp(killer) && killer->is_punisher())
			victim->delete("combat/need_punish");
	...
	else if (playerp(victim))
	{
		...
		if (vs["potential"] > vs["learned_points"] && vs["combat_exp"] > 800000)
			vs["potential"] += (vs["learned_points"] - vs["potential"]) *30 / 100;			
