...
private void auto_check()
{
	...
	if (sizeof(total) == 5)
	{
		...
		for(i=0;i<sizeof(total);i++)
		{
			...
			tell_object(total[i],"你进入本次大赛前5强，获得"+jifen+"活动点券。\n");
		}
		return;
	} else
	if (sizeof(total) == 3)
	{
		...
		for(i=0;i<sizeof(total);i++)
		{
			...
			total[i]->add("pkd_pot",jifen);
			tell_object(total[i],"你进入本次大赛前3强，获得"+jifen+"活动点券。\n");
		}
		return;
	} else
	if (sizeof(total) == 2)
	{
		...
		for(i=0;i<sizeof(total);i++)
		{
			...
			total[i]->add("pkd_pot",jifen);
			tell_object(total[i],"你进入本次大赛前2强，获得"+jifen+"活动点券。\n");
		}
		return;
	} else
	if (sizeof(total) == 1)
...
/*
	...
	msg = HIG "通过这次大赛，你获得了 " + (string)exp + " 点修为";
	if (pot > 0)
	{
		pot = random(pot / 3) + 1;
		msg += "和 " + (string) pot + " 点潜能。"NOR"\n";
	} else
	{
		pot = 0;
		msg += "。"NOR"\n";
...
*/
	...
	msg = HIG"恭喜你获得本次大赛冠军，活动点券增加"HIY+jifen+NOR"。\n";
	msg += HIG"获得屠人大赛冠军额外奖励物品"+ob->short()+NOR"。\n";
	ob->move(me);
...
int check_out(object me)
{
	...
	if (ob = me->query_last_damage_from())
	{
		...
		tell_object(ob,HIM"你击败"+me->name(1)+"活动点券 +"+(selected+1)+NOR"。\n");
	} else
