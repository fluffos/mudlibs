...
int main(object me, string arg)
{
	...
	/*
	if (!wizardp(me) && me->query("id") != "yehu112") {
	...
	if (!arg) {
		...
		switch(type) {
			case "-list_a" :
				...
				msg += ZJSEP"查看好友:look " + id;
				msg += ZJSEP"千里传音:bei2 " + id;
		...
		if (sscanf(arg, "%s %s", type, id) == 2) {
			...
			if (type == "-yes") {
				if (ob->query("friend/temp/"+me->query("id"), 1)) {
					if (sizeof(me->query("friends")) < limit || wizardp(me))
						me->set("friends/"+id, ({ob->name(), time() }) );
					else
						tell_object(me, "你的好友已经达到上限"+limit+"个，无法添加"+ob->name()+"。\n");	
					
					if (sizeof(ob->query("friends")) < limit || wizardp(ob)) 
