...
int main(object me, string arg)
{
        ...
        str = ZJOBLONG;
        str +=sprintf(HIC    "序号              功效                      等级          升级点数\n"NOR);
        str +=sprintf(HIG "——————————————————\n" NOR);
        str +=sprintf(HIC "基本能力进阶：\n\n" NOR);

        for(i=0; i<sizeof(bas_ability); i++)
                str+=(sprintf(ZJURL("cmds:ability + %d")+ZJSIZE(25)"【加点】", (i+1)) + HIC"\n"+ get_ability_info(ob, bas_ability[i], 0) + NOR);
        str +=sprintf("\n");     

        str +=sprintf(HIG "——————————————————\n" NOR);
        str +=sprintf(HIM "门派能力进阶：\n\n" NOR);
        ...
        if( !stringp(myclass=query("family/family_name", ob)) )       
                ...
                for( i=0; i<30; i++ )
                {
                        ...
                        str+=(sprintf(ZJURL("cmds:ability + %3d")+ZJSIZE(25)"【加点】", (i+21)) + HIC"\n"+ get_ability_info(ob, fam_ability[myclass][i],1) + NOR);
                }

        str +=sprintf("\n");     
        str +=sprintf(HIG "——————————————————\n" NOR);

        str +=sprintf(HIW"你现在总共获得 "HIC + ab + NOR+HIW" 点能力。\n" NOR);
        str +=sprintf(HIW"你已经用去了 " RED+ learned_ability +NOR+HIW" 点能力，还剩下 "HIG+ (ab-learned_ability) + NOR+HIW" 点可供分配。\n" NOR);
