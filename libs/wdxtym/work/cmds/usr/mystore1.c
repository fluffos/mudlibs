...
/*
	...
	if(!objectp(store=present("baina dai",me)))
	{
		store = new("/clone/misc/baina");
		store->move(me);
	}
	...
	if (!arg)
	{
		...
		str += "所有物品:mystore1 全部 1";
		str += ZJSEP+ "武器类:mystore1 武器 1";
		str += ZJSEP+"防具类:mystore1 防具 1";
		str += ZJSEP+"其它类:mystore1 杂货 1";
		str += ZJSEP+"搜  索:mystore1 搜索";
	...
	if (arg == "搜索")
	{
		write(INPUTTXT("请输入搜索的关键字","mystore1 名称 $txt# 1")+"\n");
		return 1;
	} else
	...
	if (arg=="put")
	{
		...
		for (i = 0; i < sizeof(inv); i++)
		{
			...
			dk += ({ short_name + ":mystore1 put " + file_name(inv[i]) });
		}
