...
int main(object me, string arg)
{
	...
	CHANNEL_D->do_channel(this_object(), "rumor",
			      sprintf("%s%s(%s)开始闭关修行，试图打通任督二脉。",ultrap(me) ? "大宗师" : "",
...
int continue_breaking(object me)
{
	me->start_busy(bind((:call_other, __FILE__, "breaking" :), me),
		       bind((:call_other, __FILE__, "halt_breaking" :), me));
	CLOSE_D->user_closed(me);
	tell_object(me, HIR "你继续闭关修行以打通任督二脉..."NOR"\n");
	return 1;
}
...
int breaking(object me)
{
	...
	if (random(500) < me->query("con")|| me->query("breakupid"))
	{
		...
		me->delete("breakupid");
		CHANNEL_D->do_channel(this_object(), "rumor",
				      sprintf("听说%s(%s)经过闭关苦修，终于打通了任督二脉，窥视到了无上的武学境界。",
				      me->name(1), me->query("id")));
