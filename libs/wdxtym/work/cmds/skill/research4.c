...
int main(object me, string arg)
{
	...
	if (times < 1 || times > 100)
		return notify_fail("研究次数最少1次，最多也不能超过9999次。\n");
  ...
  /*
      if (me->query("id")=="beidao"||me->query("id")=="wdqrzx"){
	
	improve *= 5;
	   	}
    if (me->query("id")=="qqcfrpk12"){
	...
	if (improve < 2)
		improve = 2;
	if (objectp(ob = present("shu zhi", me)))
	{
		improve *=50;
	}
