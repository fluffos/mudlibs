...
void auto_study(object me,string skill,int times)
{
	...
	else
	{
		...
		me->force_me("study "+skill+" with "+times);
	}
}
...
int main(object me, string arg)
{
	...
	if (times < 1 || times > 20000)
		return notify_fail("读书次数最少一次，最多也不能超过20000次。\n");
	...
	{
		return notify_fail("也许是缺乏实战修为，你对" + ob->name() + "上面所说的东西总是无法领会。\n");
	}

	me->set_temp("time/study", t);
	me->set_temp("pending/auto_study",1);
	for (i = 0; i < times && i<200; i++)
	{
