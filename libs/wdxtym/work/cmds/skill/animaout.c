...
int main(object me, string arg)
{
	...
	if ((int)me->query("max_jingli") < 2000)
		return notify_fail("你觉得精力颇有不足，看来目前还难以修炼元婴出世。\n");
	...
	CHANNEL_D->do_channel(this_object(), "rumor",sprintf("%s%s(%s)开始闭关修行，苦心修炼元婴出世。",
					ultrap(me) ? "大宗师" : "",me->name(1), me->query("id")));
...
int continue_outing(object me)
{
	me->start_busy(bind((:call_other, __FILE__, "outing" :), me),
		       bind((:call_other, __FILE__, "halt_outing" :), me));
	CLOSE_D->user_closed(me);
	tell_object(me, HIR "你继续闭关修行以修炼元婴出世..."NOR"\n");
	return 1;
}
...
int outing(object me)
{
	...
	if (me->query("potential") <= me->query("learned_points"))
	{
		...
		CLOSE_D->user_opened(me);
		CHANNEL_D->do_channel(this_object(), "rumor",sprintf("听说%s(%s)闭关结束，似乎没有什么成果。",me->name(1), me->query("id")));
		if (! interactive(me))
		{
			me->force_me("chat* sigh");
			call_out("user_quit", 0, me);
	...
	if (random(500) < me->query("con"))
	{
		message_vision(HIY "只见$N" HIY "头上现出万朵金莲，光华四射，一时间麝香扑鼻、氤氲遍地！"NOR"\n", me);
		tell_object(me, HIM "你觉得精力源源而生，忽然心如止水，如身出天际，无源无尽、登时大彻大悟。"NOR"\n");
		me->set("animaout", 1);
		CHANNEL_D->do_channel(this_object(), "rumor",
				      sprintf("听说%s(%s)经过闭关苦修，终于修成正果，练得元婴出世。",
