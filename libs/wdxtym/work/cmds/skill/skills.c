...
int main(object me, string arg)
{
	...
	int cha_type;
	int lvl;int ceng;
int comexp;int answer;int newceng;
	int i, k,j,all,page;
	seteuid(getuid());
	skill1 = 0;
...
if (ob != me  && ob->query("nolook"))
		return notify_fail("以你目前的实力，还无法探查他的技能修为。\n");
//	if (! valid_check)
...
str += "技能上限为：";
		...
		answer = i+1 ;
                                newceng=answer/20+1;
		str += HIY+ newceng+ NOR +" 层"+HIY+ ((answer%20)/2)+ NOR +" 成\n";
	if (skill1)
	{
...
*/
	...
	str += "\n";
	str += ZJOBACTS2+ZJMENUF(3,3,7,32);
	if(!arg)
		str += "　分配激发"ZJBR"　enable:fskill"ZJSEP"　查看激发"ZJBR"　enable:jifa"ZJSEP"   查看准备"ZJBR"　prepare:bei"ZJSEP"　取消准备"ZJBR"　bei none:bei none"ZJSEP;
	text = ({});
	for (i = 0; i < sizeof(sname); i++)
	{
		int percent;
...
ceng = lvl/20;
		...
		//text +=({ sprintf("%s%s%s%s"NOR ZJBR WHT"　第%d层(%d成)(%d级)"NOR":%s",
		text +=({ sprintf("%s%s%s%s"NOR ZJBR WHT"　第%d层(%d成)"ZJBR WHT"　lv：%d级"NOR":%s",
			skcolor,
			(lrn[sname[i]] >= (skl[sname[i]]+1) * (skl[sname[i]]+1)) ? HIM : "",
			(member_array(sname[i], mapped)==-1? "　": HIR"㊣"NOR),
			skillname, lvl/20+1, (lvl%20)/2,lvl,pops) });
