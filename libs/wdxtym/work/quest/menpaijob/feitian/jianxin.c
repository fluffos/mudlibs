...
void create()
{
        ...
        set_skill("xuanhualiu-quanfa", 320);
...
string ask_give()
{
        ...
        if(me->query_temp("tmark/刀")==2) {
                ...
                if(query("book_count")>0) {
                        ob=new("/d/feitian/npc/obj/nirendao");
                        ob->move(me);
                        add("book_count",-1);
...
if ((int)ob->query_condition("guojob2_busy"))
     ...
     if(ob->query("combat_exp")<500000)
	  ...
	  ob->set_temp("fthwjob",where);
          ob->apply_condition("feitian2_busy",9);
//          ob->delete_temp("shirenok");
...
int ask_fail()
{
        ...
        if (me->query_temp("fthwjob")) {
          ...
          me->apply_condition("feitian2_busy",9);
