...
int ask_job()
{
	...
	{
		if(!team[i]) continue;
        	if (team[i]->query("combat_exp") <= 8000000)
                {
                	tell_object(me, HIG"呵呵～,你们队伍里面还有没到800万的，以后再来吧！"NOR);
                 	return 1;
                }
                if (team[i]->is_ghost())
                {
                	tell_object(me, HIY"呵呵～,你的队伍里还有个死人!!!!!!"NOR);
                 	return 1;
                }
    }
