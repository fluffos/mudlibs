...
int begin(object me)
{
	...
	message_vision(CYN "$N说道：敢问阁下是谁，凭什么对我发号施令，我们先来比试比试。"NOR"\n", me);
	message_vision(CYN "$N说道：兵刃、拳脚、内功、暗器、轻功、水功，随便哪一桩，由你自己挑，我都奉陪。"NOR"\n", me);
...
void biwu(object me)
{
	...
	message_vision(HIR "\n张翠山说着缓步走到左首山峰前一堵大石壁前，吸一口气，猛地里双脚一撑，提身而起。"NOR"\n",this_object());
	remove_call_out("biwu2");
	call_out("biwu2",8,me);
}
void biwu2(object me)
{
	message_vision(HIR "\n张翠山身形纵起丈余，跟着使出“梯云纵”绝技，右脚在山壁一撑，一借力，又纵起两丈。"NOR"\n",this_object());
	message_vision(HIR "\n手中判官笔看准石面，嗤嗤嗤几声，已写了一个“"NOR+HIY"武"NOR+HIR"”字。"NOR"\n",this_object());
	message_vision(HIR "\n张翠山一个字写完，身子便要落下，突然左手挥出，银钩在握，倏地一翻，钩住了石壁的缝隙。"NOR"\n",this_object());
	message_vision(HIR "\n利用缝隙支住身子的重量，右手跟着又写了个“"NOR+HIY"林"NOR+HIR"”字。"NOR"\n",this_object());
	remove_call_out("biwu3");
	call_out("biwu3",8,me);
}
void biwu3(object me)
{
	message_vision(HIR "\n张翠山两个字写罢，跟着又写“"NOR+HIY"至"NOR+HIR"”字，“"NOR+HIY"尊"NOR+HIR"”字。越写越快，但见石屑纷纷而下。"NOR"\n", this_object());
	message_vision(HIR "\n字迹或如灵蛇盘腾，或如猛兽屹立，须臾间二十四字一齐写毕。"NOR"\n", this_object());
	message_vision(HIR "这一番石壁刻书，当真如李白诗云：\n"NOR"\n",this_object());
	message_vision(HIW "	    飘风骤雨惊飒飒，落花飞雪何茫茫。"NOR"\n", this_object());
	message_vision(HIW "	    起来向壁不停手，一行数字大如斗。"NOR"\n", this_object());
	message_vision(HIW "	    恍恍如闻鬼神惊，时时只见龙蛇走。"NOR"\n", this_object());
	message_vision(HIW "	    左盘右蹙如惊雷，状同楚汉相攻战。"NOR"\n", this_object());
	message_vision(HIR "\n张翠山写到“锋”字的最后一笔，银钩在石壁上一撑，翻身落地，轻轻巧巧的落在殷素素身旁。"NOR"\n", this_object());
	message_vision(HIR "\n$N凝视着石壁上那三行大字，良久良久，没有作声。"NOR"\n", me);
	remove_call_out("shu");
	call_out("shu",4,me);
}
void shu(object me)
{
	message_vision(HIR "\n$N终于叹了一口气，说道：“我写不出，是我输了。”"NOR"\n", me);
	message_vision(CYN "$N说道：“请问阁下有何吩咐？”"NOR"\n", me);
...
{
	message_vision(HIB "\n突然之间，$N身子一侧，滚了几个转身，只听得疾风呼啸，"NOR"\n",this_object());
	message_vision(HIB "波浪轰击之声，似乎千百个巨浪同时袭到。"NOR"\n",this_object());
	remove_call_out("storm2");
	call_out("storm2",4,me);
}
void storm2(object me)
{
	message_vision(HIB "\n脚底下舱板斗然间向左侧去，船中的海水又向外倒泻，"NOR"\n",this_object());
	message_vision(HIB "但听得狂风呼啸，身周尽是海水。"NOR"\n",this_object());
...
{
	object obj = present("yin susu");
	message_vision(HIY "只见正北方一条大船向这方驶来，看来回到中原了。"NOR"\n",this_object());
	message_vision(CYN "张翠山抱拳说道：咱们就此别过，后会有期。"NOR"\n",me,this_object());
...
int do_get(string arg)
{
	...
	if(what == "tulong blade"||what == "blade"||what == "dao"||what == "tulong" || where =="ding")
       {
	      message_vision(CYN "$N说道：在下武当张翠山，这厢有理了！"NOR"\n",this_object());
