...
int begin()
{
	...
	if (!me->query_temp("bishi", 1))
		//return 0;
            return notify_fail("你得先和常金鹏比试，然后再来找我。"NOR"\n");
	...
	message_vision(HIW "$N叫手下装了两大碗毒盐，$N和$n各吃一碗。"NOR"\n",
			   this_object(),me);
...
void die()
{
	...
	{
		command("say 佩服，佩服......");
		message_vision(HIR "\n$N嘶哑着嗓子干嚎了两声，双眼翻白，跌倒在地，口中吐出一些白沫，就再也没动了。"NOR"\n",
				   this_object());
...
int do_get(string arg)
{
	...
	{
		message_vision(CYN "$N喝道：无耻小贼，给我住手！你凭什么得此宝刀？"NOR"\n",
				   obj,this_player());

		message_vision(CYN "$N冷冷地说道：要得此刀，先比(bi)赢了我再说！"NOR"\n",
				   obj,this_player());
