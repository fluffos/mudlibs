...
int begin(object me)
{
	...
	message_vision(HIR "\n$N喝道：小心了！我这第一拳乃“催筋断骨”！"NOR"\n",
			   this_object());
	COMBAT_D->do_attack(this_object(), this_player(),query_temp("weapon"));

	message_vision(HIR "\n$N喝道：好！看我第二拳“崩山裂石”！"NOR"\n",
			   this_object());
	COMBAT_D->do_attack(this_object(), this_player(),query_temp("weapon"));

	message_vision(HIR "\n$N喝道：第三拳来了！这式叫做“横扫千军，直摧万马”！"NOR"\n",
			   this_object());
...
int do_get(string arg)
{
	...
	{
		message_vision(CYN "$N哼了一声。"NOR"\n",
				   obj);
		message_vision(CYN "$N说道：要想得到宝刀，先过我三拳！"NOR"\n",
				   obj);
