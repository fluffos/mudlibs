...
*/
	...
	if (sword)
	{
		...
		message_vision(CYN "阿大微微点了点头，道：“遵命！”"NOR"\n",
				   obj, this_player());
		...
		message_vision(HIW "只听“嗖”的一声破空声响，号称天下第一神兵的倚天剑已被阿大握在手中。"NOR"\n",
...
void unconcious()
{
	...
	if (objectp(query("anti")))
	{
		...
		message_vision(HIR "$n" HIR
				   "得势不饶人，紧接着连攻出数剑，将阿大全身各处筋脉尽数削断。"NOR"\n",
				   this_object(),me);

		message_vision(CYN "\n赵敏一张俏脸顿时惨白，叹了口气，说道：“很好，这倚天剑你就拿去吧！”"NOR"\n",
				   this_object());

		message_vision(CYN "赵敏对着$n" CYN 
				   "嫣然一笑，随即轻轻伸了伸腰，轻声说道：“鹿先生，鹤先生，我累了。”"NOR"\n",
				   this_object(), me);
