...
void create()
{
	...
	set_name("魔头", ({ "xiaoguai", "xiaoguai" }));
	set("title", HIY "boss" NOR);
	set("gender", "无性");
	set("age", 20);
	set("long","这是个看起来三十来岁的男子，眉如双剑，眼如繁星，着一袭战袍，昂然挺立。\n");
    ...
    set("sequence",5);//设置任务人数数量
    set("lvl",200);//设置任务人物成长值
	 set_temp("apply/reduce_busy", 150);//化解忙乱
	set_temp("apply/attack", 400);//命中
...
void die()
{
	    ...
	    if (newsequence <= 1)
	  {
	me->move("/d/sishenfuben/fangjian2");
	 ::die();
	  }
	  else
	  if (newsequence >= 1)
	  ...
	  newob = new("/d/sishenfuben/npc/bosshuxing");
		}
  
   	jilv= random(15);
	  if (jilv == 1)
	  {
	  objwp =new("d/sishenfuben/obj/putao1");
	  objwp->move(environment(me));
	 message("channel:chat",HBRED"副本挑战"HIR"："+me->query("name")+"击杀魔王 掉落神奇葡萄\n"NOR,users());
	  }
	  else
	   if (jilv == 5)
	  {
	  objwp =new("d/sishenfuben/obj/dan_str");
	  objwp->move(environment(me));
	    message("channel:chat",HBRED"副本挑战"HIR"："+me->query("name")+"击杀魔王 掉落臂力丹\n"NOR,users());
	  }
	    else
	    if (jilv == 4)
	  {
	  objwp =new("d/sishenfuben/obj/dan_int");
	  objwp->move(environment(me));
	    message("channel:chat",HBRED"副本挑战"HIR"："+me->query("name")+"击杀魔王 掉落悟性丹\n"NOR,users());
	  }
	   else
	    if (jilv == 3)
	  {
	  objwp =new("d/sishenfuben/obj/dan_dex");
	  objwp->move(environment(me));
	    message("channel:chat",HBRED"副本挑战"HIR"："+me->query("name")+"击杀魔王 掉落身法丹\n"NOR,users());
	  }
	  else
	   if (jilv == 0)
	  {
	  objwp =new("d/sishenfuben/obj/dan_con");
	  objwp->move(environment(me));
	    message("channel:chat",HBRED"副本挑战"HIR"："+me->query("name")+"击杀魔王 掉落根骨丹\n"NOR,users());
	  }
...
object me = this_object()->query_last_damage_from();
 distribute_skill(newob,me->query_temp("zhuangtai")+70);
    ...
    newob->set("qi", ob->query("lvl")*3);
	newob->set("max_qi",ob->query("max_qi")*3);
	newob->set("eff_qi",ob->query("max_qi")*3 );
	newob->set("jing", ob->query("max_jing")*3);
	newob->set("max_jing", ob->query("max_jing")*3);
	newob->set("neili", ob->query("max_neili")*3);
	newob->set("max_neili", ob->query("max_neili")*3);
