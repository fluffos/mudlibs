...
string *names = ({
	"龙族卫兵","龙族哨兵","龙人武士","黑龙刺客","龙族高手","七彩巨龙","龙王","龙皇",
});
...
void die()
{
	...
	{
		//获得修为和实战，跟副本等级有关
		exp = ob->improve_experience(sllv*200);
		pot = ob->improve_potential(sllv*500);

		tell_object(ob, sprintf(HIG "你击败"+query("name")+"，获得了"+pot+"点潜能、"+exp+"点实战体会。"NOR"\n",
