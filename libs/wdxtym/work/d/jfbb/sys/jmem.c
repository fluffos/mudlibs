...
#define SYSTEAM "/d/jfbb/sys/j_sys"
...
varargs void heart(int open)
{
	...
	if(me->query("make_jboss") >= 1000 && !me->query("send_jboss"))
	{
		for(i=0;i<size;i++)
		{
			...
			player = find_player(str);
			tell_object(player,BLINK+HIR"【诸子百家】幻心出现，请各位小心。\n"NOR);
			players += ({ player });
		}
	...
	if(me->query("jfbb_time") <= time())
	{
		for(i=0;i<size;i++)
		{
			...
			if(!str || !stringp(str)) continue;
			player = find_player(str);
			tell_object(player,BLINK+HIG"【诸子百家】幻心镜像已经瓦解。\n"NOR);
		}
		if(me->delete_map())
		{
			destruct(me);
	...
	size = sizeof(maps);
		...
		for(i=0;i<size;i++)
		{
			...
			for(j=0;j<size2;j++)
			{
				...
				sscanf(file,"/d/jfbb/room/%s",fileName);
