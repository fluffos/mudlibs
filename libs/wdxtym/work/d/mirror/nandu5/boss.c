...
void create()
{
	set_name(HIW"boss-罗乡"NOR, ({ "luo xiang" }));
	set("gender", "男性");
...
void die()
{
	...
	size = sizeof(team);
						message("channel:chat",HIW HBBLU"【除暴】"+team[i]->query("name")+"小队成功地杀死了第五波boss，通过了五层考验\n"NOR,users());
	...
	for(i=0;i<size;i++)
	{
        ...
        message_vision("\n$N摇身一晃，华为一缕缕青烟消散了。\n", this_object());
        obj1 = new("/d/mirror/nandu6/1");
        obj1->move(environment(ob));
        obj1 = new("/d/mirror/nandu6/2");
        obj1->move(environment(ob));
        obj1 = new("/d/mirror/nandu6/3");
        obj1->move(environment(ob));
        obj1 = new("/d/mirror/nandu6/4");
        obj1->move(environment(ob));
        obj1 = new("/d/mirror/nandu6/5");
        obj1->move(environment(ob));
        obj1 = new("/d/mirror/nandu6/boss");
        obj1->move(environment(ob));
	destruct(ob);
}
