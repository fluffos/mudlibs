...
int do_baishi(string arg)
{
	...
	if(!arg)
		...
		return notify_fail("你已经有导师了，这样不好吧！\n");
...
int do_chushili()
{
	...
	if(me->query("combat_exp")<500000)
		return notify_fail("你历练不足还不够资格出师。\n");
	else if(me->query("zjvip/out_master"))
		return notify_fail("你都已经出师了，还来出什么师？\n");
	else if(!me->query("p_master/id"))
		return notify_fail("你都没有导师，出什么师？\n");
	...
	else
	{
		...
		CHANNEL_D->do_channel(me,"chat","多日以来承蒙导师"+ob->query("name")+"悉心照顾指导，今日在此受徒儿最后一拜！");
	}
	return 1;
}
...
int do_shoutu(string arg)
{
	...
	{
		return notify_fail("想收人做徒弟？人家没想拜你吧？\n");
	}
	else if((me->query("zjvip/daoshizg")*3+3)<=sizeof(dizi))
	{
		return notify_fail("你现在的弟子已经够多了，还是多照顾照顾未出师的弟子吧！\n");
	}
	...
	else
	{
		...
		if(environment(ob)==this_object())
		{
			...
			{
				tell_object(ob,"拜师成功！赶紧向师傅学习一些江湖行走修为，问师傅要些行走江湖日用品行走江湖吧！\n");
			}
			else
				tell_object(ob,"拜师成功！抽空赶紧多向导师请教请教吧！\n");
		...
		{
			tell_object(ob,"拜师失败，你已经离开圣贤堂了！\n");
			return notify_fail("收徒失败，对方已不在这里。\n");
		}
	}
	return 1;
}
