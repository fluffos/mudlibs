...
int working(object me)
{
	...
	if (finish)
	{
		if (me->query_temp("work_times")>=199||me->query("id")=="xiaoyaoyou")
		{
			...
			me->add("yuanbao",yb);
			tell_object(me,HIY"你得到了" + chinese_number(1) + "两黄金。\n"
				"你得到了" + chinese_number(300) + "点修为。\n"
				"你得到了" + chinese_number(yb) + "个灵石。\n"
				"你得到了" + chinese_number(500) + "点潜能。"NOR"\n");
            log_file("guwu",sprintf("%s：%s在谷物加工厂工作奖励%s灵石。\n",ctime(time()),me->query("id"),chinese_number(yb)));
...
if (me->query("zjvip/times") >1)
{
		message_vision("老板对$N说：这是你的工钱，因为你是vip会员，所以获得2成加成。\n", me);
}
else
message_vision("老板冷冷对$N说：这是你的工钱。\n", me);
		tell_object(me,HIW"你得到了" + chinese_number(num) + "两银子。\n"
		//	HIM"你得到了" + chinese_number(pot) + "点修为。\n"
