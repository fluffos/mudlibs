...
{
	if(me->query_temp("论剑-jjsign"))
	{
		tell_object(me,"你离开了华山之巅，报名自动取消。\n");
		me->delete_temp("论剑-jjsign");
	}
...
private void auto_check()
{
	...
	if (sizeof(total) < 1)
	{
		msg = "听说华山论剑参赛者死的死，逃的逃，现在一个人都没有啦！";
	} else
	if (sizeof(total) == 1)
	{
		// change the daemon's state
		msg = "听说华山论剑圆满结束，" + total[0]->name(1) + "获得【武林至尊】的称号！";
...
private void kickout_players()
{
	...
	foreach (ob in total)
	{
		...
		tell_object(ob, HIC "本次华山论剑结束，你被送出比赛场地！”"NOR"\n");
...
private void give_bouns(object me)
{
	...
	// bouns
	tell_object(me, HIY"恭喜你获得了【武林至尊】称号，你可以每天来这里领取一次奖励"NOR"。\n");
}
...
void heart_beat()
{
	...
	{
		set("start",1);
		inv = filter_array(all_inventory(this_object()), (: playerp($1)&&$1->query_temp("论剑-jjsign") :));
		if(sizeof(inv)<LUNJIAN_USER_MIN)
		{
			tell_room(this_object(),"报名人数不足"+LUNJIAN_USER_MIN+"人，本次活动未能成功进行。。。\n");
			delete("pre_start");
			delete("start");
			set_heart_beat(0);
			return;
		...
		for(i=0;i<sizeof(inv);i++)
		{
			...
			tell_object(inv[i], HIY "你定神一看，才发现自己到了" + environment(inv[i])->short() + HIY "。\n");
		}
	}
	...
	{
		message_competition("听说本次华山论剑众英雄势均力敌，未能决出武林至尊！");
		delete("pre_start");
		delete("start");
		kickout_players();
		set_heart_beat(0);
...
{
	message_competition("听说" + me->name(1) + "临阵脱逃，从华山论剑竞技场溜走了。");
	restore_status(me);
	if (arrayp(total))
		total -= ({ me });
...
int check_out(object me)
{
	...
	{
		msg = "听说" + me->name(1) + "在华山论剑中惨遭" + ob->name(1) + "的毒手，被无情淘汰。";
	} else
		msg = "听说" + me->name(1) + "在华山论剑中运气不佳，本领有限、已经惨遭淘汰。";
	message_competition(msg);
