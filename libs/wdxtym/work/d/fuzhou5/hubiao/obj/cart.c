...
int do_open(string arg)
{
	...
	if( me->query("combat_exp") < 500000 )
		return notify_fail("就凭你这点修为，还想学别人抢镖车？\n");
...
if (team[i]->query_condition("db_exp")) {
        team[i]->add("combat_exp", jl);
	tell_object(team[i],HIY"由于精英之书的作用，你额外获得了"+chinese_number(jl)+"点修为！"NOR"\n");}
			...
			log_file("job/hubiao_rewards", sprintf("%s，%s%s护镖完成奖励一颗冲脉丹[中]，修为%d，潜能%d，现在修为%d。\n",
				ctime(time()),
				team[i]->query("name"),
			...
			tell_object(team[i],HIY"你被奖励了"+chinese_number(jl)+"点修为，"+chinese_number(j)+"点潜能，"+chinese_number(5)+"两黄金！"NOR"\n");

			log_file("job/hubiao", sprintf("%8s%-10s护镖任务奖励修为%4d，潜能%3d，现在修为%d。\n",
				team[i]->query("name"),
				"("+capitalize(team[i]->query("id"))+")",
				jl,
				j,
			...
			write_file("/log/job/Hubiao", sprintf("%8s%-10s护镖奖励修为%4d,潜能%3d,现在修为%d,应得%d,步数%d,费时%d,劫匪%d,推时%d/%d,走错%d)。\n",
				team[i]->query("name"),
				"("+capitalize(team[i]->query("id"))+")",
				jl,
				j,
		...
		for(j=0; j<sizeof(team); j++) {
			...
			if (team[j]->query_temp("protecting")==cart)
				team[j]->delete_temp("protecting");
	 	        log_file("job/hubiao",sprintf("%8s%-10s护镖任务失败，现在修为%d。\n",
