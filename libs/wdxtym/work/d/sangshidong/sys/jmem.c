...
#define SYSTEAM "/d/sangshidong/sys/j_sys"
...
varargs void heart(int open)
{
	...
	if(me->query("make_jboss") >= 1000 && !me->query("send_jboss"))
	{
		for(i=0;i<size;i++)
		{
			...
			player = find_player(str);
			tell_object(player,BLINK+HIR"【剑诀】丧尸密洞出现，请各位小心。\n"NOR);
			players += ({ player });
		}
	...
	//已到副本生存期限
	if(me->query("sangshidong_time") <= time())
	{
		for(i=0;i<size;i++)
		{
			str = userId[sprintf("%d",i)];
			if(!str || !stringp(str)) continue;
			player = find_player(str);
			tell_object(player,BLINK+HIG"【剑诀】丧尸密洞镜像已经瓦解。\n"NOR);
		}
		if(me->delete_map())
		{
			destruct(me);
	...
	size = sizeof(maps);
		...
		for(i=0;i<size;i++)
		{
			...
			for(j=0;j<size2;j++)
			{
				...
				sscanf(file,"/d/sangshidong/room/%s",fileName);
