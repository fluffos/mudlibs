...
#include "/d/jfbd/sys/j.h"

#define SYSTEAM "/d/jfbd/sys/j_sys"

void create() { set_name("镜像房间内存",({ "jmee"})); }
...
varargs void heart(int open)
{
	...
	if(me->query("make_jboss") >= 1000 && !me->query("send_jboss"))
	{
		for(i=0;i<size;i++)
		{
			...
			player = find_player(str);
			tell_object(player,BLINK+HIR"【诸子百家】幻魔出现，请各位小心。\n"NOR);
			players += ({ player });
		}
	...
	if(me->query("jfbd_time") <= time())
	{
		for(i=0;i<size;i++)
		{
			...
			if(!str || !stringp(str)) continue;
			player = find_player(str);
			tell_object(player,BLINK+HIG"【诸子百家】幻魔魔镜像已经瓦解。\n"NOR);
		}
		if(me->delete_map())
		{
			destruct(me);
	...
	size = sizeof(maps);
		...
		for(i=0;i<size;i++)
		{
			...
			for(j=0;j<size2;j++)
			{
				...
				sscanf(file,"/d/jfbd/room/%s",fileName);
