...
void create()
{
        set_name(HIR"真神能丸"NOR, ({ "zshennen wan", "wan" }) );
        set_weight(500);
        if( clonep() )
...
//                return notify_fail("你正忙着哪。\n");
        ...
        if( !objectp(ob = present("zshennen wan", me)) )
                return notify_fail("你没有这种东西。\n");

        message_vision(HIY"\n$n"HIY"一口吃下了$N,"HIY" $n觉的潜能和修为加多了!  \n"NOR,this_object(),me);
me->add("qi",500);
me->add("eff_qi",500);
...
me->set("eff_qi",me->query("max_qi"));
	...
	me->add("combat_exp", 200000+random(88888));
	me->add("potential",80000+random(88888));        
	me->add("max_neili",800);

message_vision(HIY"$N觉的潜能和修为加多了!!\n"NOR, me);
