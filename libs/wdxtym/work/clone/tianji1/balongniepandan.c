...
void create()
{
        set_name(HIY "八龙涅槃丹" NOR, ({"balongniepan dan"}));
        if (clonep())
...
{    
   int effect;
	 mapping my;
        ...
        log_file("static/using", sprintf("%s(%s) eat 八龙涅槃丹 at %s.\n",
...
if( me->query("meridian/手三阳经")>0 )
            return notify_fail("你已经开始打通经脉，无法承受八龙涅槃丹的药力!\n");
...
if( me->query("meridian/冲脉")>0  )
 ...
 CHANNEL_D->do_channel(this_object(), "rumor", "恭喜" + me->name() + "打通了"HIR"全身大小周天经脉"HIM",功力已如神人。。\n");
                ...
                write("\n "+HIR+"恭喜你："+NOR+HIG+"服用八龙涅槃丹，奇经八脉步入天人之境！！！ "+NOR);
