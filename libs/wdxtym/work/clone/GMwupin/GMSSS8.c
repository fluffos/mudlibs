...
void create()
{
        set_name(HIY "八龙化生丹·极品" NOR, ({"bmsd danSSS"}));
        if (clonep())
...
{    
   int effect;
	 mapping my;
        ...
        log_file("static/using", sprintf("%s(%s) eat 八龙化生丹·极品 at %s.\n",
...
if( me->query("meridian/冲脉")>0  )
 ...
 CHANNEL_D->do_channel(this_object(), "rumor", "恭喜" + me->name() + "打通了"HIR"全身大小周天经脉"HIM",功力已如神人。。\n");
				...
				me->add("gain/break", 1250);
				me->add("gain/armor", 1250);
				me->add("gain/2dp", 125);
				me->add("gain/2ap", 125);
				me->add("gain/attack", 400);
				me->add("gain/defense", 400);
				me->add("gain/damage", 1000);
				me->add("gain/max_qi", 22500);
