...
{    
   int effect;
	 mapping my;
        ...
        log_file("static/using", sprintf("%s(%s) eat 八龙再生丹 at %s.\n",
...
if( me->query("meridian/手三阳经") <24 )
            return notify_fail("你的经脉不是满级，无法承受八龙再生丹的药力!\n");
...
if( me->query("meridian/冲脉") <24  )
 ...
 CHANNEL_D->do_channel(this_object(), "rumor", "恭喜" + me->name() + "重塑了"HIR"全身大小周天经脉"HIM",功力已如神人。。\n");
				...
				me->add("gain/break", -232);
				me->add("gain/armor", -232);
				me->add("gain/2dp", -16);
				me->add("gain/2ap", -23);
				me->add("gain/attack", -192);
				me->add("gain/defense", -142);
				me->add("gain/damage", -232);
				me->add("gain/max_qi", -5000);
                write("\n "+HIR+"恭喜你："+NOR+HIG+"服用八龙再生丹，奇经八脉已然重塑成功！！！ "+NOR);
