...
void create()
{
	set_name(HIY"洗点卡"NOR, ({"xidian ka", "ka"}) );
	set_weight(1);
	if( clonep() )
	...
	else
	{
		...
		set("long","这是一"+query("base_unit") +name()+"，可以重新分配你的先天属性点"ZJBR+
							HIR"只能使用一次，使用后一旦有不符合先天的技能将直接被删除，慎用！"NOR"\n" );
		set("yuanbao", 1000);
	}
	set("action_list", ([
		"使用" : "usesxidian",
...
varargs int do_uses(string arg)
{
	...
	if (me->query("usesxidian", 1)) {
		tell_object(me, "你已经使用过一次洗点卡了，不可以再使用。\n");
		return 1;
	}
	if(!arg) {
		...
		max_sx = me->query("str") + me->query("int") + me->query("con") + me->query("dex");
		
		write(INPUTTXT(HIR"注：值必须在13-45之间"ZJBR"总和不能超过你现有的先天"+max_sx+"点。"
		...
		if (tmpstr < 13 || tmpstr >45
			|| tmpint < 13 || tmpint >45
			|| tmpcon < 13 || tmpcon >45
			|| tmpdex < 13 || tmpdex >45) {
				write(HIR"你有一项输入不在(13-45)范围之内。"NOR"\n");
				return 1;
			}
		...
		CHANNEL_D->do_channel( this_object(), "chat", "据说"+me->name()+"用洗点卡重塑了身体！");
