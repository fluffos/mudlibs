...
void init_sockets(object me)
{
	...
	if (me->query("diablo/" + TYPE_SURCOAT + "/id") != query("id")) return;

	set("sockets", me->query("diablo/" + TYPE_SURCOAT + "/sockets"));
	set_armor_desc();
}
...
int spec_effect(object ob, object me, int damage, string socket, int socket_level)
{
	...
	if (me->query("diablo/" + TYPE_SURCOAT + "/id") != query("id")) return 0;
