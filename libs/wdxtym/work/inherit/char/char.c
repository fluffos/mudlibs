...
void heart_beat()
{
	...
	if(is_player&&environment(me)&&environment(me)->query("close_room"))
	{
		...
		if(interactive(me)) {
			if (texp<30) tell_object(me,"你绞尽脑汁，思索过去的战斗经历。"NOR"\n结果却毫无头绪：你获得了"+texp+"点修为和"+tpot+"点潜能。\n");
                  else if (texp>30 && texp<40) tell_object(me,"你瞑思苦想，仔细体会战斗技巧。"NOR"\n你感觉功力又深了一层：你获得了"+texp+"点修为和"+tpot+"点潜能。\n");
                  else if (texp>50) tell_object(me,"你用手比划着，静心研究战斗技巧。"NOR"\n你顿感豁然开朗，受益匪浅：你获得了"+texp+"点修为和"+tpot+"点潜能。\n");
		}
	}
	if (! my["not_living"])
