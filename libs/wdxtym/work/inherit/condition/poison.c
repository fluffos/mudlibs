...
{
	return HIG "$N" HIG "痛苦的呻吟了一声。"NOR"\n";
}

string update_msg_self()
{
	return HIG "你中的$?" HIG "发作了。"NOR"\n";
}

string die_msg_others()
{
	return RED "$N" RED "嘶哑着叫了两声，就不再喘气了。"NOR"\n";
}
...
int dispel(object me, object ob, mapping cnd)
{
	...
	if (need_lvl > my_lvl)
	{
		if (me == ob)
		{
			tell_object(me, MAG "你运用内功驱散" + pos + "，然而似乎没有半点效果。"NOR"\n");
			me->start_busy(1);
			return -1;
		} else
		{
			tell_object(me, MAG "你运用内功帮助" + ob->name() + "驱散" + pos + "，然而似乎没有半点效果。"NOR"\n");
			tell_object(ob, MAG + me->name() + "将内力缓缓的输入你的体力，你觉得一阵恶心，几欲呕吐。"NOR"\n");
			me->start_busy(1);
			if (! ob->is_busy())
				ob->start_busy(1);
			return -1;
	...
	// 去毒能力
	power = my_lvl + me->query_skill("dispel-poison", 1) + me->query_temp("apply/dispel-poison");
	if (ob->query("breakup"))
		power += power * 3 / 10;
	if (me == ob)
	{
		if (cnd["id"] == me->query("id"))
		{
			tell_object(me, WHT "你运用内功，将" + pos +"尽数驱散。"NOR"\n");
			me->add("neili", -100);
			cnd["remain"] = 0;
			me->start_busy(1);
		} else
		{
			...
			{
				dis = cnd["remain"];
				cost_neili = cnd["remain"] * 2 * (cnd["level"] + 1) / power;
				tell_object(me, WHT "你运用内功，将" + pos + "尽数驱散。"NOR"\n");
			} else
			{
				tell_object(me, WHT "你运用内功，驱散了一些" + pos + "。"NOR"\n");
			}
			cnd["remain"] -= dis;
			if (cnd["remain"] < 0)
				cnd["remain"] = 0;
...
int update_condition(object me, mapping cnd)
{
	...
	tell_object(me, HIM "你"+cnd["name"]+"毒发作，气血 - " + qd +  "，受伤"+qw+"。"NOR"\n");
	tell_object(me, HIM "你"+cnd["name"]+"毒发作，精神 - " + jd +  "，受伤"+jw+"。"NOR"\n");
...
//	if (cnd["id"] == "nature poison" || improve == -1 || cnd["level"] / 2 + random(cnd["level"]) < (int)me->query_skill("force") + improve)
	if (1)
	{
		...
		cnd["remain"] -= kkk;
		if(playerp(me))
			tell_object(me, HIM "以你目前的内功修为，身上剩余毒素将在" + cnd["remain"]/kkk +  "回合之后清除。"NOR"\n");
