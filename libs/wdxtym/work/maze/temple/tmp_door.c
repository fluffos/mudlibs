...
void create()
{
        ...
        set_name(HIY"失落神庙"NOR, ({ "shenmiao door","door"}) );
        set_weight(99999);
        set("no_sun",1);
	set("value", 5000);
	set("no_get", 1);
        if( clonep() )
                ...
                set("long", GRN"从这个入口可以进入失落神庙。(enter door)\n"NOR);
                set("long","挑战每次需要300阅历300威望"ZJURL("cmds:enter door")ZJSIZE(15)""HIR"进入"NOR"修为最低进入条件all sk > 500级!\n");                
		set("unit", "个");
                set("lore",1);
        }
        setup();
...
int do_enter(string arg)
{
	...
	if (me->query("weiwang")<300)
        ...
        MAZE_D->enter_maze(this_player(), "temple");
        return 1;
}
