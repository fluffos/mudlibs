...
int do_join()
{
       ...
       if (bad_bunch(me))
       {
               if( query("bunch/bunch_name", me) == bunch_bad["bunch_name"] )
               {
                       ...
                       return notify_fail("你已经是我"+query("bunch/bunch_name", me)+"的成员了，昏头了？\n");
               }
               return notify_fail("你现在是"+query("bunch/bunch_name", me)+"的成员，要加入我"+bunch_bad["bunch_name"]+"，得先脱离(disengage)原来的帮会。\n");
       }
...
int do_disengage()
{
       ...
       if( query("bunch/boss", me) != bunch_bad["boss"] )
               return notify_fail("想脱离我"+bunch_bad["bunch_name"]+"？找你的介绍人"+query("bunch/boss", me)+"去吧。\n");
