...
int recruit( object dizi )
{
    ...
    if ( dizi->query( "oldquest/status" ) )
        dizi->delete( "oldquest/status" );

    if ( dizi->query( "oldquest/task_time" ) )
        dizi->delete( "oldquest/task_time" );   
...
void attempt_apprentice(object ob)
{
    ...
    {
        message_vision( "$N微微一笑，道：“乖徒儿，从今天起你就是" + belong + "的弟子了。\n可别给少林丢脸呵。”\n",this,ob);
        recruit(ob);
	}
}
