...
int accept_object(object who, object ob)
{
      ...
      if( (string)ob->query("name") != quest["quest"])
 {
           fam = this_player()->query("family");
               if(who->query_temp("kmarks/wugu3"))
        {
                if( !who || environment(who) != environment() ) return 0;
                if (!objectp(ob) ) return 0;
                if ( !present(ob, who) ) return notify_fail("你没有这件东西。\n");

                if (  (string)ob->query("name") != "书信")
                        return notify_fail("何太冲不需要这件东西。\n");


                if((fam["family_name"] == "白驼山派")||(fam["family_name"] == "星宿派")
                        ||(fam["family_name"] == "桃花岛"))
                {
                        if(random(4)==1)
                        {
                                command("say 邪门歪道也想要我的玉真散，拿命吧！");
                        ...
                        {
                                command("smile");
                                command("say 这位"+RANK_D->query_respect(who)+"竟能医好贱内，请收下这颗玉真散。\n");
                                command("say 另外我还可以传授你一些武功。\n");

                                obn1=new("/d/kunlun/obj/yuzhen");
                                obn1->move(who);
                                who->add_temp("kmark/xue", 150);
                                message_vision("铁琴先生给了$N一颗"+HIY"玉真散\n"NOR,who);

                                return 1;
                        }
                }
                else if(fam["family_name"] == "昆仑派")
                {
                        command("say 能治好五姑的病，好，算我没看错你。");
                        command("say 奖赏你两颗玉真散。");

                        obn1=new("/d/kunlun/obj/yuzhen");
                        obn1->move(who);
                        obn2=new("/d/kunlun/obj/yuzhen");
                        obn2->move(who);
                        message_vision("铁琴先生给了$N两颗" + HIY"玉真散\n"NOR,who);
                        return 1;
                }
                else
                {
                        if(random(10)==1)
                        {
                                tell_object(who, "只见何太冲诡异的一笑，露出复杂的表情。\n" );
                                command("say 不是我小气，怪你运气不好，认命吧！");
                                remove_call_out("kill_ob");
                                call_out("kill_ob", 1, this_player());
...
int give_quest(string arg)
{
 ...
 "512000",
        ...
        me = this_player();
       obfam = query("family");
       if (!(fam = this_player()->query("family")) || fam["family_name"] != obfam["family_name"] )
      {
         write(RANK_D->query_respect(this_player()) +"与本派素无来往，不知此话从何谈起？\n");
         return 1;
       }
        ...
        quest = UQUEST_D(tag)->query_quest();
...
// A factor here to make time random
        ...
        tell_object(me,"杀了『"+quest["quest"]+"』，伸张正义、铲邪除恶，扬我"+fammsg+"威名！\n" NOR);
