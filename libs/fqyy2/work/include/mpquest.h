...
*/
	...
	if(j<sizeof(menpai))
        ...
        tag=levels[num];      
   //     tell_object(me,HIW" "+obj->query("name")+"沉思了一会儿，说道：本门暂时还没有任务开放。\n");
        quest = ((string)"/quest/menpaijob/"+pai+"/qlist"+num)->query_quest();
        timep = quest["time"]; if (timep>300) timep=300;
        ...
        {
        tell_object(me,"去把『"+quest["quest"]+"』打扫(sao)一下吧。\n" NOR);
        }
        else if (quest["quest_type"] == "送")
        {
        tell_object(me,"把这封信送给『"+quest["quest"]+"』吧。\n" NOR);
        xin=new("/quest/menpaijob/xin");
        xin->set("target",quest["quest"]);
        xin->set("score",quest["score"]);
        xin->move(me);
        }
...
int accept_object(object who, object ob)
{
        ...
        else
        {
                ...
                who->set("faith",0);
                who->add("faith",score);
		exp_bound=100+random(100);
                qn_bound=40+random(40);
                tell_object(who,"经验增加了"+chinese_number(exp_bound)+"点\n");
                tell_object(who,"潜能增加了"+chinese_number(qn_bound)+"点\n");
                who->add("combat_exp",exp_bound);
                who->add("potential",qn_bound);
                return 1;
        }
}
