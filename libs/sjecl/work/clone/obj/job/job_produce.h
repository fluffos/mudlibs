...
void job_tell_player(object player) {
  ...
  if (file_name(environment(player)) == master_place_full) {
    if (objectp(master = present(master_id, room_master))) {
      ...
      if (!master_tell_player(player))
        ...
        else if (wizardp(player) && (string)player->query("env/job_system") == "open")
  ...
  msg += menpai_name + "弟子说道:" + "“" + explode(
    family_master[menpai_name],
    "/"
  )[1] + "口训我已传到，告辞了!”\n" + menpai_name + "弟子走了出去\n" NOR;
...
void finish_job(object player) {
  string job_kind;
  job_kind = get_mapping(
    player->query("id"),
    "job_player",
    job_data->query_job_data()
  )["job_finish"];
  switch (job_kind) {
    case "oppose_pker":
