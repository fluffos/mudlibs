...
void tell_player_family_job_contribute(object player, object master) {
  ...
  if (!mapp(family_map)
    ...
    message_vision(
      CYN "$n对$N摇摇头说道：" + family_name + "弟子不争气，还没为" + family_name + "做出任何贡献呢。\n" NOR,
      player,
      master
    );
    return;
  ...
  message_vision(
    CYN "$n对$N说道：目前我" + family_name + "的贡献度为" + chinese_number(family_map["job_contribute"]) + "点。\n" NOR,
    player,
    master
  );
  ...
  message_vision(
    CYN "$n对$N说道：你为" + family_name + "所做的贡献为" + chinese_number(point) + "点。" + msg + "\n" NOR,
    player,
    master
  );
