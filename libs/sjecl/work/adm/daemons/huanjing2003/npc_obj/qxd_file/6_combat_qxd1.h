...
void fighting() {
  ...
  if ((srl = present("shenren ling", target))) {
    message_vision(
      CYN "$N" NOR + CYN "身上的" + srl->query("name") + NOR + CYN "突然发出一阵光芒！\n" NOR,
      target
    );
  ...
  if (figh_type == "tools") {
    ...
    if (use_tools->query("hj_game/obj") == "sword") {
      // 先确定攻击力、忙时等
      damages = (me_power * 3 / 4 + random(me_power / 4)) + (use_tools->query("power") * 3 / 4) + random(use_tools->query("power") / 4);
    ...
    // 否则是魔法水晶类
      ...
      tell_room(
        environment(me),
        color_name + "只见" + use_tools->query("att_msg") + "向" + target->query("name") + NOR + color_name + "攻去，造成了 " HIG + damages + NOR + color_name + " 点的伤害！\n" NOR,
        target
      );
      tell_object(
        target,
        color_name + "只见" + use_tools->query("att_msg") + "向你攻来，造成了 " HIR + damages + NOR + color_name + " 点的伤害！\n" NOR
      );
    }
  }
  ...
  if (figh_type == "skills") {
    ...
    damages = me_power / 4 + random(me_power / 4) + (me->query_temp("hj_game_damages/" + use_skills) * 3 / 4) + random(me->query_temp("hj_game_damages/" + use_skills) / 4);
