...
void checking() {
  ...
  if (!hp || hp < 1) {
    ...
    CHANNEL_D->do_channel(
      me,
      "rumor",
      sprintf("听说%s[%s]探险失败，离开了幻境。", me->query("name"), me->query("id"))
    );
    return;
  }
  ...
  if (wait_over == "yes" && time > DENG_TIME / 5) {
    ...
    CHANNEL_D->do_channel(
      me,
      "rumor",
      sprintf("听说%s[%s]探险成功，离开了幻境。", me->query("name"), me->query("id"))
    );
    return;
  }
...
void enter_hj() {
  ...
  if (me->query("huanjing2003/last_power_times")) {
    if (!me->query("huanjing2003/last_power")) {
      tell_object(
        me,
        sprintf(
          "你还可以使用 %d 次累积力量，但是现在你的累积力量比你能得到的基本力量还低，并未生效。\n",
  ...
  if (me->query("huanjing2003/last_skills_times")) {
    if (!me->query("huanjing2003/last_skills_lv")) {
      tell_object(
        me,
        sprintf(
          "你还可以使用 %d 次累积技能，但是现在你没有任何累积技能，并未生效。\n",
  ...
  if (me->query("huanjing2003/gift")) {
    tell_object(
      me,
      HIW "你曾是幻境游戏的" + me->query("huanjing2003/gift") + "者！\n你将得到增多 300 点游戏内最大气息、力量增强 10 点的奖励！\n" NOR
    );
...
*/
  ...
  CHANNEL_D->do_channel(
    me,
    "rumor",
    sprintf("听说%s[%s]进入了幻境，开始探险。", me->query("name"), me->query("id"))
  );
  remove_call_out("checking");
  call_out("checking", 1);
