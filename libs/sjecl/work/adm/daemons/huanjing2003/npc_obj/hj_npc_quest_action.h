    ...
    if (query("go_where") != "ok") {
      if (!query("following_name"))
        ...
        if (temp_obj && temp_obj->query_temp("huanjing") == "start" &&
          ...
          tell_object(
            temp_obj,
            "过了许久，你才想起来你曾答应过带一个迷路的小精灵找什么地方的！\n你觉得心里掠过一丝内疚。\n" HIR "游戏提示：你的得分减少了 " + award_score + " 点。\n" NOR
          );
        }
      }
    } else {
...
int do_found(string arg) {
  ...
  message_vision(
    CYN "$N" NOR + CYN "兴奋地拿出一" + f_obj->query("unit") + f_obj->name(1) + NOR + CYN "，对着$n" NOR + CYN "说道：“你是不是在找这个？”\n",
    me,
    ob
  );
  ...
  message_vision(
    sprintf(
      CYN "$N" NOR + CYN "把%s" NOR + CYN "接了过去，审视良久，叹道：“这%s" NOR + CYN "我能得一见，已无憾矣！”\n",
      f_name,
      f_name
    ),
    ob
  ...
  message_vision(
    CYN "$n" NOR + CYN "对$N" NOR + CYN "说道：“所谓宝物配英雄，这" + f_name + NOR + CYN "正是该由你所有！”\n$n" NOR + CYN "说完，把" + f_name + NOR + CYN "又交回了$N" NOR + CYN "手中。\n" NOR,
    me,
    ob
  );
