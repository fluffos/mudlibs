...
int ask_baowu() {
  ...
  message_vision(
    CYN "$N" NOR + CYN "拿出一个" + my_gem->query("name") + NOR + CYN "递给了" + ob->query("name") + NOR + CYN "。\n",
    me
  );
  destruct(my_gem);
  command("idle2");
  message_vision(
    CYN "$n" NOR + CYN "微微一笑，把一个" + baowu->query("name") + NOR + CYN "递给了$N" NOR + CYN "。\n" NOR,
    me,
    ob
  );
  ...
  message_vision(
    CYN "$n" NOR + CYN "又拿出了一个" + baowu->query("name") + NOR + CYN "递给了$N" NOR + CYN "。\n" NOR,
    me,
    ob
  );
