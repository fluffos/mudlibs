...
void get_npc_name() {
  ...
  switch (ob->query("family/family_name")) {
    case "少林派":
      ...
      set_name(
        sl_fre_name[i] + name["name"],
        ({ sl_fre_id[i] + " " + name["id"][0], sl_fre_id[i], name["id"][0] })
      );
      break;
      //娥眉派
    case "峨嵋派":
      ...
      set_name(
        em_fre_name[i] + name["name"],
        ({ em_fre_id[i] + " " + name["id"][0], em_fre_id[i], name["id"][0] })
      );
      break;
    case "姑苏慕容":
      ...
      if (sizeof(name["id"]) == 1)
        ...
        set_name(
          "慕容" + name["name"],
          ({ "murong " + name["id"][1] + name["id"][2], "murong", name["id"][1] + name["id"][2] })
        );        //特别注释：０是生成的全名，１和２才是分开的字，下同
        break;
    case "桃花岛":
      ...
      if (sizeof(name["id"]) == 1)
        ...
        set_name(
          name["name"] + "风",
          ({ name["id"][1] + name["id"][2] + " feng", name["id"][1] + name["id"][2], "feng" })
        );
      break;
    case "天龙寺":
      name = get_random_name(1, 1);
      ...
      set_name(
        tls_fre_name[i] + name["name"],
        ({ tls_fre_id[i] + " " + name["id"][0], tls_fre_id[i], name["id"][0] })
      );
      break;
    default:
      set("gender", (sex ? "男性" : "女性"));
