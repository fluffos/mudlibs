...
string *have_items = ({ "mian hua", "ya ma", "da ma", "zhu ma", "mu mianhua", });
string *have_items_c = ({ HIW "棉花", YEL "亚麻", YEL "大麻", HIY "苎麻", HIY "木棉花", });
    ...
    if (time() > (mpv[i]["atime"] + TIMEOUT_TICK)) {
      if (mpv[i]["action"] == 3) {
        ...
        if (me && userp(me))
          tell_object(
            me,
            HIR "你错过了" + (mpv[i]["action"] == 1 ? "浇水" : "施肥") + "的时机，植物长势受到影响。\n" NOR
          );
        mpv[i]["lost"]++;
        decide_action(mpk[i]);
      }
...
void shouge_finish(object me) {
  ...
  message_vision(
    YEL "$N总共收获了" + CHINESE_D->chinese_number(bu) + obj->query("unit") + obj->name() + "。\n" NOR,
    me
  );
  if (j) message_vision(
  ...
  log_file("job/wnongsang", sprintf("%s(%s)：bu:%d,lvl:%d;cj:%d,exp:%d,pot:%d;ncj:%d,nexp:%d\n",
    me->name(), me->query("id"), bu, mp["product"], i, j, k, me->query_skill(
      "nongsang",
      1
