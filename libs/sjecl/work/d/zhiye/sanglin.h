...
string *have_items = ({ "can si", "yucan si", "bingcan si", "tiancan si", "longjian si", });
string *have_items_c = ({ HIW "蚕丝", GRN "玉蚕丝", HIW "冰蚕丝", WHT "天蚕丝", HIY "龙茧蚕丝", });
    ...
    if (time() > (mpv[i]["atime"] + TIMEOUT_TICK)) {
      if (mpv[i]["action"] == 3) {
        ...
        if (me && userp(me))
          tell_object(
            me,
            HIR "你错过了" + (mpv[i]["action"] == 1 ? "喂食" : "清扫") + "的时间，蚕的成长受到影响。\n" NOR
          );
        mpv[i]["lost"]++;
        decide_action(mpk[i]);
      }
...
void chousi_finish(object me) {
  ...
  message_vision(
    YEL "$N总共抽得到" + CHINESE_D->chinese_number(bu) + obj->query("unit") + obj->name() + "。\n" NOR,
    me
  );
  if (j) message_vision(
    YEL "但是$N拿不动那么多，只好把几" + obj->query("unit") + obj->name() + "放到地上。\n" NOR,
    me
  );
  ...
  log_file("job/wnongsang", sprintf("%s(%s)：bu:%d,lost:%d,lvl:%d;cj:%d,exp:%d,pot:%d;ncj:%d,nexp:%d\n",
    me->query("name"), me->query("id"), bu, mp["lost"], mp["product"], i, j, k, me->query_skill(
      "nongsang",
      1
