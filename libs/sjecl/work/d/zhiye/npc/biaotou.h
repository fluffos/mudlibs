...
string ask_guard() {
  ...
  if (me->query_user_type() == "worker") {
    ...
    return "本镖局提供贴身保镖服务，每次服务的时间为一刻钟，目前能为您提供三钟档次的保镖：\n" + sprintf(
      ...
      "佣金：",
      MONEY_D->price_str((i > 300) ? (i * 1500) : (i * 1000))
    ) + sprintf(
      "%-32s%-14s%-6s%-30s\n",
      "高级保镖(guard2) －－－ 武功等级：",
      CHINESE_D->chinese_number(j),
      "佣金：",
      MONEY_D->price_str((j > 300) ? (j * 1500) : (j * 1000))
    ) + sprintf(
      "%-32s%-14s%-6s%-30s\n",
      "超级保镖(guard3) －－－ 武功等级：",
      CHINESE_D->chinese_number(k),
      "佣金：",
      MONEY_D->price_str((k > 300) ? (k * 1500) : (k * 1000))
    );
  } else
...
string do_fire(int type) {
  ...
  message_vision(
    "$N付给$n" + MONEY_D->price_str((lvl > 300) ? (lvl * 1500) : (lvl * 1000)) + "来雇佣保镖。\n",
    me,
    this_object()
  );
