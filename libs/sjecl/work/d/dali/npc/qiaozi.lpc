...
void create()
{
        ...
        set("chat_chance", 3);
	set("chat_msg", ({
		"樵子唱道：“城池俱坏，英雄安在？云龙几度相交代？想兴衰，弱为怀。”\n",
		"樵子唱道：“唐家才起隋家败，事态有如云变改。疾，也是天地差！迟，也是天地差！”\n",
		"樵子唱道：“天津桥上，凭栏遥望，春陵王气都凋丧，树苍苍，水茫茫，云台不见中兴将，千古转头归灭亡。功，也不久长！名，也不久长！”\n",
	}));
...
string ask_axe()
{
        ...
        if ((string)me->query("gender") == "女性" )
                 return RANK_D->query_respect(me) + "是女儿身，请不要和我开这种玩笑!";
        if ((int)me->query_skill("qiantian-yiyang",1) < 100  )
                return RANK_D->query_respect(me) + "乾天一阳功如此之差，我和你交换！";
        if (me->query_str() < 25)
                return RANK_D->query_respect(me) + "要学我的看家本领，必须臂力奇高，你似乎资质不够。";
        if ((int)me->query("job_time/大理") < 300)
                return RANK_D->query_respect(me) + "为镇南王府做事做的还不够啊，努力做吧！";
        if (!me->query("tls"))
                return RANK_D->query_respect(me) + "还没有出家，可以直接拜我为师，不用交换！";
        if (me->query_skill("pangen-fu",1) > 1)
                return RANK_D->query_respect(me) + "已经学会了盘根撮结斧法，不要来这里捣乱！";
        if (me->query("pgf/jiaohuan"))
                return RANK_D->query_respect(me) + "已经交换过了，不要在这捣乱！";

	me->add("combat_exp", -15000);
	me->set_skill("pangen-fu",91);
	me->set("pgf/jiaohuan",1);
	tell_object(me, HIY "\n你用(15K) 经验值换了(91)级的盘根撮结斧(pangen-fu)。\n" NOR);
	return "哈哈，你已经交换过了，可不要后悔哦！";
}
...
/*
        if ((int)ob->query("job_time/大理") < 50){
