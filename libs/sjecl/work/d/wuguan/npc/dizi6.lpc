...
int give_tools()
{
	...
	switch( ob->query_temp("job_name") ){
		...
		default:  return notify_fail(CYN+me->name()+"说道：「你跟大师兄领的什么活？我这里没这种工具。」\n"NOR);
	}
...
int give_gaotou()
{
	...
	tools->move(ob);
	tell_object(ob, CYN+me->name()+"说道：「我就给你这把工具，小心保管，别弄丢了。」\n"NOR);
	message_vision("$N交给$n一"+tools->query("unit") + tools->query("name")+"。\n", me, ob);
	return 1;
}
...
void greeting(object ob)
{
	...
	if( ob->query_temp("mark/完了")){
		...
		tell_object(ob, CYN+this_object()->name()+"说道：「工作做完了，可以还工具 "HIY HBCYN"give wu "+ ob->query_temp("tools_name")+CYN" 给我了！」\n"NOR);
		return;
	}
