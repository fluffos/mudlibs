...
细看大钟，整整有两丈来高，直径也有丈许。钟身似由青铜浇铸，上面满是
密密麻麻的印文古篆，估计重逾数千斤。巨杵粗可合抱，丈把来长，一头裹
以厚皮，正对着钟腰。尽管山风吹人欲倒，谁都禁不住想撞一下钟(bell)试
试。往外看，有个很大的汉白玉窗台(out)，上面足可站一个人。
LONG
	);
...
int do_out(string arg)
{
	...
	me->set_temp("last_damage_from", "从钟楼上跳下来摔");
	if ( i < 50)
		me->die();
	else if( i < 125)
		me->unconcious();
		...
		if( i > 175)
        if (me->query_skill_mapped("dodge") && me->query_skill("dodge", 1) < me->query_skill(me->query_skill_mapped("dodge"), 1))
