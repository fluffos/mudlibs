...
int do_han(string arg) {
  ...
  if (arg == "七公" || arg == "师父" || arg == "帮主") {
    ...
    if (arg == "七公" && count == 2) {
      ...
      me->add_temp("h7g_quest/han", 1);
      message_vision(
        CYN "$N清了清嗓子，小声又喊了一声：“" + arg + "！你在嘛，徒儿" + me->name() + "" CYN "来了。”\n" NOR,
        me
      );
      return 1;
    }
    if (arg == "师父" && count == 3) {
      message_vision(
        CYN "$N等了一会，发现没动静，试探着又喊了一声：“" + arg + "！不肖徒儿" + me->name() + "" CYN "让您久等了。”\n" NOR,
        me
      );
      me->start_busy(1);
      me->add_temp("h7g_quest/han", 1);
...
void happen(object me) {
  ...
  if (i < 200 || j < 100) {
    ...
    message_vision(
      HIW "$N大声得意地说道：“师父，我的打狗棒现在已经" + CHINESE_D->chinese_number(i) + "级，棒诀已经" + CHINESE_D->chinese_number(j) + "级了。”\n" NOR,
      me
    );
...
int do_da(string arg) {
  ...
  if (arg == "轻灵奇巧") {
    ...
    message_vision(
      CYN "洪七公满意地点了点头道：”" + me->name() + "" CYN "，你不愧为师的得意门生。我想起一个新招，你附耳过来。”\n" NOR,
      me
    );
