...
int do_duijiang() {
  ...
  for (k = 0; k < SIZE_OF_SOURCE / 2; k++) {
    for (j = 0; j < SIZE_OF_SOURCE / 2; j++) {
      if (strTemp[k] == strTemp2[j]) {
        ...
        me->add(
          "caipiao_shot",
          me->query("caipiao_shot") + sizeof(me->query("caipiao_zhongjiang_list"))
        );
  ...
  if (me->query("caipiao_zhongjiang_list")) {
    ...
    log_file(
      "caipiao",
      sprintf(
        HIG "%s(%s)第%d张彩票：%s 中奖，系统中奖号：%s 中奖排列:%s" NOR,
        me->name(),
        me->query("id"),
        i,
        me->query("caipiao/第" + chinese_number(i) + "张/my_caipiao"),
        strPriKaijiang,
        strReportLog
      )
    );
    if (me->query("caipiao/第" + chinese_number(i) + "张/huobi") == "黄金") {
      tell_object(
        me,
        HIY "恭喜!此次中奖,使你获得了" + HIW + chinese_number((int)me->query("caipiao_shot") * GOLD_COST / 10000) + HIY + "锭黄金!\n" NOR
      );
