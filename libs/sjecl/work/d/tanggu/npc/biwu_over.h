...
int cancel(string arg) {
  ...
  log_file(
    "quest/zhaoqin",
    sprintf(HIR "%-8s%-10s 主动取消比赛biwu_over" NOR, me->query("name"), me->query("id")),
    me
  );
  biwu_stop(1);
  return 1;
}
void biwu_stop(int i)  //0 biwu_over 1 cancel
{
  ...
  if (query("station") < 2 && i == 0) {
    log_file(
      "quest/zhaoqin",
      sprintf(HIR "%-8s%-10s 程序运行错误biwu_stop" NOR, ob->query("name"), ob->query("id")),
      ob
    );
    return;
  }
  if (query("station") >= 6 && i == 0) {
    log_file(
      "quest/zhaoqin",
      sprintf(HIR "%-8s%-10s 程序运行修正biwu_stop" NOR, ob->query("name"), ob->query("id")),
      ob
    );
    return;
  }
  if (!ob->query("bwzh/host_player")) {
    log_file(
      "quest/zhaoqin",
      sprintf(HIR "%-8s%-10s 丢失美女数据biwu_over" NOR, ob->query("name"), ob->query("id")),
      ob
    );
    return;
  }
  if (!ob->query("bwzh/winner_player")) {
    log_file(
      "quest/zhaoqin",
      sprintf(HIR "%-8s%-10s 丢失擂主数据biwu_stop" NOR, ob->query("name"), ob->query("id")),
      ob
    );
    return;
  }
  ...
  if (!room) {
    log_file(
      "quest/zhaoqin",
      sprintf(HIR "%-8s%-10s 缺少更新文件biwu_stop" NOR, ob->query("name"), ob->query("id")),
      ob
    );
    return;
  }
  ...
  for (p = 0; p < sizeof(ob_list); p++) {
    log_file(
      "quest/zhaoqin",
      sprintf(
        HIC "%-8s%-10s 比武招亲中止biwu_stop" NOR,
        ob_list[p]->query("name"),
        ob_list[p]->query("id")
      ),
      ob_list[p]
    );
  ...
  if (!me || !me->query_temp("quest/bwzh/hoster")) {
    ...
    if (!me)
      log_file(
        "quest/zhaoqin",
        sprintf(
          HIC "%-8s%-10s 美女不在连线biwu_stop" NOR,
      ...
      log_file(
        "quest/zhaoqin",
        sprintf(
          HIC "%-8s%-10s 美女离开现场biwu_stop" NOR,
          ob->query("bwzh/host_player/name"),
          ob->query("bwzh/host_player/id")
        ),
        ob
  ...
  if (!winner_player || !winner_player->query_temp("quest/bwzh/biwu_host")) {
    ...
    if (i)
      CHANNEL_D->do_channel(
        this_object(),
        "chat",
        HIY + "" + ob->query("bwzh/host_player/name") + "(" + capitalize(ob->query("bwzh/host_player/id")) + ")的比武招亲活动被强行中止！" NOR
      );
    else
      CHANNEL_D->do_channel(
        this_object(),
        "chat",
        HIY + "因为时间关系，" + ob->query("bwzh/host_player/name") + "(" + capitalize(ob->query("bwzh/host_player/id")) + ")比武招亲活动中止！" NOR
      );
    if (winner_player) {
      CHANNEL_D->do_channel(
        this_object(),
        "chat",
        HIY + "最终比武胜利者：" + winner_player->query("name") + RANK_D->query_respect(winner_player) + "！" NOR
      );
      CHANNEL_D->do_channel(
        this_object(),
        "chat",
        HIY + "但是，" + winner_player->query("name") + RANK_D->query_respect(winner_player) + "目前不在现场，招亲取消！" NOR
      );
    }
    if (winner_player) log_file(
      "quest/zhaoqin",
      sprintf(
        HIC "%-8s%-10s 擂主不在现场biwu_stop" NOR,
        ob->query("bwzh/host_player/name"),
        ob->query("bwzh/host_player/id")
      ),
      ob
      ...
      "quest/zhaoqin",
      sprintf(
        HIC "%-8s%-10s 缺少擂主信息biwu_stop" NOR,
        ob->query("bwzh/host_player/name"),
        ob->query("bwzh/host_player/id")
      ),
      ob
  ...
  winner_player->set_temp(
    "apply/short",
    ({ winner_player->name() + "(" + winner_player->query("id") + ")" YEL " <成亲中> " NOR })
  );
