void zhaoqin(object me, object who) {
  ...
  if (!me || !me->query_temp("quest/bwzh/over")) {
    if (!me) log_file(
      "quest/zhaoqin",
      sprintf("%-8s%-10s 美女信息丢失zhaoqin", ob->query("name"), ob->query("id")),
      ob
    );
    else log_file(
      "quest/zhaoqin",
      sprintf("%-8s%-10s 美女标记丢失zhaoqin", ob->query("name"), ob->query("id")),
      ob
    );
    init_over();
    return;
  ...
  if (!who || !who->query_temp("quest/bwzh/over")) {
    if (!who) log_file(
      "quest/zhaoqin",
      sprintf("%-8s%-10s 擂主信息丢失zhaoqin", ob->query("name"), ob->query("id")),
      ob
    );
    else log_file(
      "quest/zhaoqin",
      sprintf("%-8s%-10s 擂主标记丢失zhaoqin", ob->query("name"), ob->query("id")),
      ob
    );
    init_over();
    return;
  ...
  if (environment(me) != environment(ob)) {
    ...
    log_file(
      "quest/zhaoqin",
      sprintf("%-8s%-10s 损失十万经验zhaoqin", me->query("name"), me->query("id")),
      me
    );
    init_over();
    return;
  ...
  if (environment(who) != environment(ob)) {
    ...
    log_file(
      "quest/zhaoqin",
      sprintf("%-8s%-10s 损失十万经验zhaoqin", who->query("name"), who->query("id")),
      who
    );
    init_over();
    return;
...
void answer_check(object me, object who, int i) {
  ...
  if (!me || !me->query_temp("quest/bwzh/over") || environment(me) != environment(ob)) {
    ...
    if (me) {
      ...
      log_file(
        "quest/zhaoqin",
        sprintf("%-8s%-10s 损失十万经验answer_check", me->query("name"), me->query("id")),
        me
      );
    }
    init_over();
  ...
  if (!who || !who->query_temp("quest/bwzh/over") || environment(who) != environment(ob)) {
    ...
    if (who) {
      ...
      log_file(
        "quest/zhaoqin",
        sprintf("%-8s%-10s 损失十万经验answer_check", who->query("name"), who->query("id")),
        who
      );
    }
    init_over();
