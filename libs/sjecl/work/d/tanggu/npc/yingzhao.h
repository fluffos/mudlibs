int yingzhao(string arg) {
  ...
  if (!winner_player || !host_player || environment(winner_player) != environment(ob) || environment(host_player) != environment(ob)) {
    ...
    if (!winner_player || !host_player)
      log_file(
        "quest/zhaoqin",
        sprintf(HIR "%-8s%-10s 缺少数据信息yingzhao" NOR, me->query("name"), me->query("id")),
        me
      );
    else log_file(
      "quest/zhaoqin",
      sprintf(HIR "%-8s%-10s 擂主美女不在yingzhao" NOR, me->query("name"), me->query("id")),
      me
    );
    return 1;
  }
  ...
  log_file(
    "quest/zhaoqin",
    sprintf("%-8s%-10s 开始比武应招yingzhao", me->query("name"), me->query("id")),
    me
  );
  ...
  if (!room) {
    log_file(
      "quest/zhaoqin",
      sprintf("%-8s%-10s 缺少擂台文件yingzhao", me->query("name"), me->query("id")),
      me
    );
    init_over();
  ...
  host_player->set_temp(
    "apply/short",
    ({ host_player->name() + "(" + host_player->query("id") + ")" YEL " <比武招亲中> " NOR })
  );
  winner_player->set_temp(
    "apply/short",
    ({ winner_player->name() + "(" + winner_player->query("id") + ")" YEL " <比武招亲中（擂主）> " NOR })
  );
  me->set_temp(
    "apply/short",
    ({ me->name() + "(" + me->query("id") + ")" YEL " <比武招亲中（挑战者）> " NOR })
  );
