int answer(string arg) {
  ...
  if (query("station") != 6) {
    log_file(
      "quest/zhaoqin",
      sprintf(HIR "%-8s%-10s 程序出现问题answer" NOR, me->query("name"), me->query("id")),
      me
    );
  ...
  if (!who || environment(who) != environment(ob)) {
    ...
    if (who) {
      ...
      log_file(
        "quest/zhaoqin",
        sprintf("%-8s%-10s 离开损失十万answer", who->query("name"), who->query("id")),
        who
      );
    } else {
      if (me->query("gender") == "女性")
  ...
  if (arg == "no" || arg == "n" || arg == "N" || arg == "No") {
    ...
    log_file(
      "quest/zhaoqin",
      sprintf("%-8s%-10s 拒绝损失十万answer", me->query("name"), me->query("id")),
      me
    );
    init_over();
    return 1;
  ...
  CHANNEL_D->do_channel(
    this_object(),
    "chat",
    HIY "" NOR + me->short("1") + HIY "与" NOR + who->short(1) + HIY "比武招亲，玉成良缘！" NOR
  );
  message_vision(HIM "\n$N和$n交换了定情信物。\n" NOR, me, who);
