...
int do_fish()
{
    ...
    if ( (int)me->query("combat_exp") > MAX_EXP)
      ...
      if ( ((int)me->query("potential") - (int)me->query("learned_points")) > 500000)
          return notify_fail("dream说你潜能太多，不许钓了。 \n");
    me->start_busy(FISH_BUSY(3));
...
void reward_me(object me)
{
 ...
 tell_object(me,"你得到了五十两白银,已经帮你存到您的户口里去了。\n");
	...
	me->set("combat_exp",exp+reward);
	tell_object(me,"你得到了"+reward+"点经验，");
	reward = POT_BASE + 30 + (random(kar))*POT_RATIO;
	me->set("potential",pot+reward);
	tell_object(me,reward+"点潜能。\n");
  me->add("money",5000);
 tell_object(me,"你得到了五十两白银,已经帮你存到您的户口里去了。\n");
