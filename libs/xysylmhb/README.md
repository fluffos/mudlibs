...
《夕阳再现III 之 炎龙封印》，AKAI Studio 出品。和 xyj2006 系列一样
用 Tomud/"笑傲江湖"客户端握手协议：**连线后第一行是裸的 `version `
提示**，期待字面回传 `2060`。注册没有"new"关键字，任何还没存档
的英文 ID 直接问"使用 X 这个名字将会创造一个新的人物，您确定吗
(y/n)？"；天赋是菜单选栏位（`1-4` 指定单项数值，`0` 系统随机整
组，然后 `y/n` 确认接受）。地图和"天涯"家族（`tybxjh`/`wlhd`/
`xhcii`/`zxty`/`ffxymud`/`jhfy2`）逐字节相同（`d/city/sj.lpc` 的
"世界之巅"跳崖场景一致），是这批档案里第七个确认的成员——虽然打
的是"夕阳再现"品牌，实际地图和"天涯"同源，和真正的"夕阳再现"血统
（`xajh4gkb`/`jhfy3`）反而没有关系，纯属品牌命名巧合。另外，
`xiyangzaixian3`（夕阳再现III之炎龙封印）和这份档案几乎完全一致
（`.lpc` 文件总数、目录结构、`d/city/sj.lpc` 都相同），很可能是同
一个原始压缩包被本项目拆分成了两份独立档案。
