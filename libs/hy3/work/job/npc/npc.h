...
"官府的捕头吧？！大家都是武林中人，今天能否高抬贵手？我必有重谢！你好好斟酌一下，是否愿意？\n",ob,me);
	...
	switch(random(4)) {
		...
		case 3:
			...
			call_out("kill_ob", 1, ob);
			break;
write(WHT"「"+ob->query_temp("guanfu_target")+"」好象往「"+dizhis["chinese"]+"」的方向逃去了。\n"NOR);
...
void die()
{
        ...
        message_vision(WHT"$N深吸口气，冷笑道，好身手！说完强忍伤痛纵身向「"+dizhis["chinese"]+"」处逃走。\n"NOR,ob,me);
        me->set_temp("mark/gkill2",1);
        me->add("combat_exp",200);
//        message_vision(WHT"「"+ob->query_temp("guanfu_target")+"」好象往「"+dizhis["chinese"]+"」的方向逃去了。\n"NOR);
        ...
        && (string)ob->query("name") == me->query_temp("guanfu_target")){
        message_vision(WHT"$N冷笑一声，说道，好俊的功夫！说完纵身向「"+dizhis["chinese"]+"」处逃走。\n"NOR,ob,me);
