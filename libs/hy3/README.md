...
mudlib。虽然 slug 与本项目"海洋"题材的 `hy`/`hy2000`/`hy2002` 共享
"hy"前缀，但经逐一 diff `master.lpc`/`securd.lpc`/`securityd.lpc` 核
心档案确认，两者代码血统完全独立，只是碰巧同属"ES2 衍生、金庸题材"
这一大类武侠泥巴（和 AGENTS.md §11 里其它十几个共享山头/门派命名但
互不相关的成员一样）。地图除华山、少林、峨嵋、崑仑、全真教、丐帮、
天龙寺、灵鹫宫、桃花岛等常见门派场景外，还延伸到蒙古、大理地区；
另有独立的"杀手楼"（`d/shashou/`）刺客势力和结婚系统（`d/marry/`）
玩法。管理员从白名单外的 IP 登录时，还会被要求当场解答一道随机数学
题才能通过，是这份档案自带的防盗号设计。
