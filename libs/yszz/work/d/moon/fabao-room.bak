...
void get_type(string arg, object ob)
{
        ...
        //制造特殊法宝
                ...
                ob->set_temp("fabao_type","special fabao");
                write("\n");
                write("可选择特殊法宝种类：\n");
                write("1. 逃跑类  2. 收兵器类  3. 伤人类\n");
                write("请选择：(q 键取消)");
        }
...
void get_subtype(string arg, object ob)
{
    ...
    sscanf(arg, "%d", order);
        ...
        if( (fabao_type == "weapon" && order <= 0 && order > 10) ||
            ...
            (fabao_type == "special fabao" && order <= 0 && order >3 ) ) {
                if( arg == "w" )    {
                        fabao_type = "weapon";
                        write("\n");
                        write("可选择武器种类：\n");
    ...
    if( arg == "" )
       ...
       fabao_unit = arg; 
        ...
        if( fabao_type == "weapon" )
                ...
                build_armor(ob);        
        else if( fabao_type == "special fabao" )
...
void build_special_fabao(object ob)
{
        ...
        newob1->set("special_fabao",1);        
        newob2->set("owner_id", getuid(ob));
        newob2->set("special_fabao",1);
...
{
   string name, newname;
   object fabao_ob,newob, me=this_player();
   string fabao_id;
   string *id_list, *t_list;
...
int  do_change_id(string arg)
{
   ...
   object fabao_ob,newob, me=this_player();
   string *id_list, *t_list;
   string fabao_id1,*id_list1,*t_list1;
   ...
   if(fabao_ob->query("special_fabao"))
   {
        ...
        if( newob )   {
            ...
            fabao_id1 = (string)newob->query("id");
            fabao_id1 = replace_string(fabao_id1, " ", "_");
            newob->set("id", fabao_id1);
            seteuid(fabao_id1);
            export_uid(newob);
            seteuid(getuid());

            id_list1 = ({ fabao_id1 });
            t_list1 = explode(fabao_id1, "_");
            if( sizeof(t_list1) > 1)   {
                        id_list1 += t_list1;
            }
            newob->set_name(newob->query("name"), id_list1);
            newob->save();
        }
        fabao_ob->set_name( fabao_ob->query("name"), id_list );
        newob->set_name( fabao_ob->query("name"), id_list );
        newob->save();
        destruct(newob);
       }
   else
...
int  do_change_desc(string arg)
{
   string name, newname;
   object fabao_ob,newob, me=this_player();
   string fabao_id,*t_list,*id_list;
