...
int do_copy(object me,int flag)
{
        ...
        ob->reincarnate(); 
          ...
          ob->set("jh_dj/dj",me->query(("jh_dj/dj")+("jh_dj/zhuan")));
          ob->set("neili",300);
    remove_call_out("do_back");
        call_out("do_back", 8,  ob);  
        return 1;
}
int do_back(object me)
{                       
...
void die()
{
        ...
        if( objectp(corpse = CHAR_D->make_corpse(this_object(), killer)) )
          // corpse->set_name(this_object()->name()+"的尸体",({ killer->query("id")+" jianxi corpse" }) );
