...
"她是浣花现任掌门萧西楼的小女儿。一直打理着剑庐的内务。\n");
	...
	set_skill("force", 540);
	set_skill("jiuqu-xinfa", 540); 
	set_skill("strike", 540);
	set_skill("feixu-zhang", 540);
	set_skill("dodge", 560); 
	set_skill("huanhua-shenfa", 560); 
	set_skill("sword", 540); 
	set_skill("parry", 540); 
	set_skill("literate", 540); 
	set_skill("confu", 540);
	set_skill("huanhua-jianfa", 540); 
...
void attempt_apprentice(object ob)
{
	...
	if (ob->query_skill("sword",1)< 80 || ob->query_skill("huanhua-jianfa",1) < 80)
	{
