...
void create()
{
	...
	set_skill("force", 440);
	set_skill("jiuqu-xinfa", 440); 
	set_skill("strike", 440);
	set_skill("feixu-zhang", 440);
	set_skill("dodge", 440); 
	set_skill("huanhua-shenfa", 440); 
	set_skill("sword", 440); 
	set_skill("parry", 440); 
	set_skill("literate", 440); 
	set_skill("confu", 440); 
	set_skill("huanhua-jianfa", 440); 
...
void attempt_apprentice(object ob)
{
	...
	if ((!(my_fam = ob->query("family")) || my_fam["family_name"] != "浣花剑派") && (int)ob->query("combat_exp")>200000000)
	{
...
int ask_job()
{
	...
	if ( !myfam || myfam["family_name"] != "浣花剑派" )
	{
        ...
        else if (environment(me)->query("short")!="虎啸堂")
        {
            command("kok");
            command("say 这不是我呆的地方啊！不要想歪主意啊");
	...
	else if ( myfam["generation"] > 5 && myfam["master_name"]!="李子木" )
	{
	  ...
	  ||me->query_skill("sword")<40
	  ||me->query_skill("dodge")<40
	  ||me->query_skill("parry")<40)
	{
		command("shake");
