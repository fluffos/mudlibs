...
void create()
{
   ...
   carry_object("/d/shaolin/obj/fumo-dao");
}
void init()
{
    object me, ob;
...
string ask_me()
{
  ...
  if(!objectp(dao=present("fumo dao",this_object())))
  return "你来晚了，伏魔刀已经不在我身上了。\n";  
dao->move(this_player());
    add("count", -1);
    message_vision("\n渡厄长笑一声，点点头，从身后的树洞里取出伏魔刀交给$N。\n\n", this_player());
   message("channel:rumor",HIW"\n〖江湖传闻〗: 听说"+this_player()->query("name")+"从金刚伏魔圈弄到了一把伏魔刀。\n\n"NOR,users(),({this_player()}));
