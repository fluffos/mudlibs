...
void create()
{
    ...
    set_skill("whip", 2000);
    set_skill("damo-jian", 2000);
    set_skill("jingang-quan", 2000);
    set_skill("buddhism", 2000);
     set_skill("literate", 2000);
    set_skill("riyue-bian", 2000);
...
void attempt_apprentice(object ob)
{
  ...
  if ((int)ob->query_skill("hunyuan-yiqi", 1) < 300) {
   ...
   if ((int)ob->query_skill("dodge", 1) < 300) {
        command("say 在轻功方面，" + RANK_D->query_respect(ob) +
            "是否还做得不够？");
        return;
    }
    if ((int)ob->query_skill("shaolin-shenfa", 1) < 300) {
        command("say 在轻功方面，" + RANK_D->query_respect(ob) +
            "是否还做得不够？");
        return;
    }
   ...
   if ((int)ob->query_skill("parry", 1) < 300) {
