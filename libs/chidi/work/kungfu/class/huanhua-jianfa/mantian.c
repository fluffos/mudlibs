...
int perform(object me)
{
        ...
        object ob;
        int skill, skill2, ap, dp, neili_wound, qi_wound, endure;
        ...
        if( me->query_skill_mapped("sword") != "huanhua-jianfa") 
                return notify_fail("你没有激发浣花剑法，无法使用「满天花雨」！\n");
 
        if (!weapon->query("endure"))
        {
                if (!weapon->query("material")) endure = 75;
                else if (weapon->query("material") == "paper") endure = 100;
                else if (weapon->query("material") == "cloth") endure = 30;
                else if (weapon->query("material") == "silk") endure = 75;
                else if (weapon->query("material") == "wood") endure = 40;
                else if (weapon->query("material") == "bamboo") endure = 75;
                else if (weapon->query("material") == "leather") endure = 90;
                else if (weapon->query("material") == "jade") endure = 65;
                else if (weapon->query("material") == "iron") endure = 75;
                else if (weapon->query("material") == "steel") endure = 130;
                else if (weapon->query("material") == "copper") endure = 160;
                else if (weapon->query("material") == "golden") endure = 120;
                else if (weapon->query("material") == "silver") endure = 90;
                else if (weapon->query("material") == "plant") endure = 25;
                else endure = 75;
        }
        else endure = weapon->query("endure");
        ...
        if (skill<270 || weapon->query("endure")<130)
        {
                ...
                if( random(ap) > dp )
                {
                        ...
                        message_vision(msg, me, target);
                        qi_wound = (100 + skill + random(skill*2)) * endure/50;
                ...
                call_out("perform_qiao", 10, me, target);
                return 1;
        }
...
上露出极为痛苦的表情。\n\n"NOR;
                        message_vision(msg, me, target);
                        qi_wound = 200 + 2*skill + random(skill*2);
...
int perform_qiao(object me, object target)
{
        object weapon;
//      target = me->select_opponent();
        ...
        if(me->is_fighting() && living(target) && living(me) && (!objectp(weapon = me->query_temp("weapon"))))
        {
                ...
                COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 0);
                COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 0);
/* 不知道为什么要这样，没什么道理啊？不quit 的话就会一直降！怎么玩？
                me->add_temp("dex", - count * 3);
                me->add_temp("str", - count * 2);
*/
        }
