...
victim->receive_wound("jing", (damage_bonus ));
   me->add("neili",random((int)me->query("jiali"))*5);
write(HIR "你融合九阳神功的精髓，发挥出九转十二天的刚劲，震毁了对手的丹田。！\n" + NOR);
     }

if(random(10001+me->query("kar"))>6666 && me->query_skill("jiuzhuan-jiuyang",1)<(me->query_skill("jiuyang-shengong",1)/2)&&me->query_skill("jiuzhuan-jiuyang",1)<(me->query_skill("jiuyang-shengong",1)))
{
   me->improve_skill("jiuzhuan-jiuyang",random((int)me->query_int())/10);
message_vision(HIR"$N在战斗中对九阳残篇有所领悟！\n"NOR,me,victim);
}
   
    if( random(3*(me->query_skill("jiuzhuan-jiuyang",1))) > (damage_bonus )) {
if (me->query("jiuzhuanlv")){
        ...
        return HIR "$N发挥出九转十二天九阳残篇的第"+chinese_number(me->query("jiuzhuanlv"))+"重内劲，摧毁了$n的真元！！！！\n" NOR;
                               }
else   {
                        ...
                        victim->add("neili", -(int)me->query("jiali"));
                          return HIR "$N的九转12天九阳残篇摧毁了$n的真元！！！！\n" NOR;
        }
}
}
...
{
   if ((int)me->query_skill("jiuzhuan-jiuyang",1) >= 0)
     return notify_fail("九转12天九阳残篇只能靠战斗来提升。\n");
    return 1;
}
...
{
           switch (me->query_skill("jiuzhuan-jiuyang",1))
   {
                case 1001:
     {
...
{
         return __DIR__"jiuzhuan-jiuyang/" + func;
}
