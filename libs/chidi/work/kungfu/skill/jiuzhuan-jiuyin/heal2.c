...
{
       int skill;
skill = me->query_skill("jiuzhuan-jiuyin",1);
    if( me->is_fighting() )
...
if (me->query("special_skill/gongfusoul") == 1 )
 ...
 if ((int)me->query_skill("jiuzhuan-jiuyin", 1) > (int)me->query_skill("mingyu-shengong",1)/2)
                return notify_fail("你的冥雨神功不能支撑你的九转残篇。\n");
  ...
  me->improve_skill("jiuzhuan-jiuyin",random((int)me->query_int())*4);
