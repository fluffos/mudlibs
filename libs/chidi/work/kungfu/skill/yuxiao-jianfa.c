...
int valid_learn(object me)
{
        ...
        return notify_fail("玉萧剑法乃是黄药师的成名武功，你只能找他学习。\n");
        if ((int)me->query_skill("yuxiao-jianfa", 1) > 500) 
       return notify_fail("黄药师说道: 玉箫剑法，只付有缘。你与此剑法缘尽于此，以后就看你自己的造化了。\n");

    if (((int)me->query("max_neili") < 20000) ||
    ((int)me->query_skill("bibo-shengong", 1) < 700) 
)
...
mapping query_action(object me, object weapon)
{
    ...
    level   = (int) me->query_skill("yuxiao-jianfa",1);
        if (random(level) > 2000 && me->query("jh_dj/dj")>=25)
         {
	return ([
	"action":HIG"$N身子滴溜溜的打了半个圈子，$w"NOR+HIG"一挺，丹田中提一口真气，$w"+HIG"上所附真力递增，青芒犹似长蛇般伸缩不定，蓦地剑芒突盛，刺向$n"HIG"的$l！"NOR,
	"force" : 500,
	"dodge" : 140,
	"parry" : 140,
	"damage": 500,
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
       string msg;
       int j, k;
       j = me->query_skill("yuxiao-jianfa", 1);
       ...
       if( random(10) >= 5 
        ...
        && j > 120
        && k > 120
        && me->query_skill_mapped("parry") == "yuxiao-jianfa"
