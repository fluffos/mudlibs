#include <ansi.h>
#include <skill.h>
#include <weapon.h>
#include <combat.h>
inherit F_SSERVER;
 
int perform(object me, object target)
{
    object weapon, ob;
    string msg;
    int i;
 
        me->clean_up_enemy();
    ob = me->select_opponent();
if (me->is_fighting())
{
if  (environment(me)!=environment(ob) ) 
     return notify_fail("对方已经不在了。\n");
}

if ( (string)me->query("family/family_name") != "少林派") 
     return notify_fail("只有少林派弟子才能使用该PFM。\n");


    if( !me->is_fighting() )
        return notify_fail("狂龙只能在战斗中使用。\n");
     if (me->query_temp("perform")>time()) return notify_fail("你上一招未使完！\n");
    if (!objectp(weapon = me->query_temp("weapon"))
    || (string)weapon->query("skill_type") != "whip")
    if (!objectp(weapon = me->query_temp("secondary_weapon"))
    || (string)weapon->query("skill_type") != "whip")
        return notify_fail("你使用的武器不对。\n");
    else me->set_temp("secondly_weapon",1);
    if( (int)me->query("neili") < 200 )
        return notify_fail("你的真气不够！\n");
    if( (int)me->query_skill("whip") < 1000 ||
     me->query_skill_mapped("whip") != "riyue-bian")
        return notify_fail("你的基本鞭法还不到家，无法使用狂龙！\n");
msg =  HIW+"$N手中"BLINK HIB""+weapon->name()+NOR HIW"虚晃，舞出漫天鞭影，犹如一条"BLINK HIR"狂龙"NOR HIW"般卷向对方!!!!\n\n" NOR;   
    message_vision(msg, me);
    me->clean_up_enemy();
    ob = me->select_opponent();
    me->add("neili", -150);
    me->set_temp("perform",time()+3);
    for(i = 0; i < 12; i++)
        if (me->is_fighting(ob) && ob->is_fighting(me) && ob->query("eff_qi")>0){
              me->set_temp("action_msg","紧接着");
                if (!weapon->query("equipped")) break;
              COMBAT_D->do_attack(me, ob,weapon, 0);
            }else break;
    me->delete_temp("secondly_weapon");
    return 1;
}


