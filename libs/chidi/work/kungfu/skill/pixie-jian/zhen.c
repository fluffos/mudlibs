...
//    mapping my_fam  = me->query("family");
    ...
    if( me->is_fighting() )
        return notify_fail("「针决式」不能在战斗中演练。\n");
...
if (me->query("special_skill/gongfusoul") == 1 )
 {
    ...
    if( !skill || (skill < 60))
        return notify_fail("你的辟邪剑法等级不够, 不能演练「针决式」！\n");
    if( me->query("neili") < 50 )
        return notify_fail("你的内力不够，没有力气演练「针决式」！\n");
    if (me->query_skill("sword",1)<= skill)
        return notify_fail("你的基本剑法不够，无法通过演练「针决式」提高辟邪剑法的技巧！\n");
    if( me->query("jing") < -jing_cost )
        return notify_fail("你现在太累了，无法集中精神演练「针决式」！\n");
 write(HIC "你使出辟邪剑法之「针决式」，将手中" + weapon->name() + HIC "随意挥舞击刺。\n" + NOR);
    me->add("neili", -50);
    me->add("jing", jing_cost);
  me->improve_skill("pixie-jian",random((int)me->query_int())*4);
