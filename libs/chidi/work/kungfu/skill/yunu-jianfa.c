...
int practice_skill(object me)
{
        ...
        if ((int)me->query_skill("yunu-xinjing", 1) < 85)
        ...
        if ((int)me->query_skill("meinu-quanfa", 1) < 80)
...
return ([      "action":HIW"$N急退急趋，纵上连刺三剑，$w"+HIW"飘忽来去，东刺西击，这一路玉女剑法更见使得英气爽朗，顾盼生姿！"NOR,
	...
	"dodge" : 20,
        "parry" : 20,
        "damage": 550 + random(level*4),
	"post_action": (: ke_qz :),
        "damage_type":  "刺伤"
]);

        if (random(me->query_skill("yunu-jianfa",1)) > 160 && me->query_skill("yunu-xinjing",1) > 100 &&
        me->query("neili") > 200)
{
	me->add("neili", -50);
	if (me->query("gender")== "男性") {
return ([      "action":HIG"$N剑走轻灵，招断意连，绵绵不绝，当真是闲雅潇洒，翰逸神飞，大有晋人乌衣子弟裙屐风流之态！"NOR,
        "force" : 550 + me->query_skill("yunu-jianfa"),
	"dodge" : 100,
        "parry" : 100,
        "damage": 500,
        "lvl" : 175,
...
int ke_qz(object me, object victim, object weapon, int damage)
{
	...
	&& victim->query_skill_mapped("dodge") == "qixing-huanwei" 
	&& (random(skill)/2 + skill/2) > random(victim->query_skill("jinyan-gong",1))
	&& random(3)==0
	&& me->query_skill("yunu-xinjing",1) > 50) {

	message_vision(HIW"岂知$N深悉七星换位的诸般巧妙变著，左脚勾出，正好算在$n闪躲之处，配合得丝丝入扣，$n被$P一勾，当即仰天摔倒！\n"NOR, me,victim );
        if (!victim->is_busy())
	victim->start_busy(3);
...
|| (victim->query_skill_mapped("unarmed")== "kongming-quan")
|| (victim->query_skill_mapped("sword")== "yunu-jianfa"))
&& (random(skill)/2 + skill/2) > random(victim->query_skill("parry",1)) )
&& random(2)==0
&& me->query_skill("yunu-xinjing",1) > 50) {
...
if (victim->query("qi") > level*2)
{
	...
	return HIR"$n突然双目直瞪，砰的一声，翻身摔倒，竟已受了极重的暗伤，同归剑神功已然被破！\n"NOR;
			}
...
if (victim->query("qi") > level*2)
{
	...
	return HIR"$n突然双目直瞪，砰的一声，翻身摔倒，竟已受了极重的暗伤，先天大法已然被破！\n"NOR;
			}
}
