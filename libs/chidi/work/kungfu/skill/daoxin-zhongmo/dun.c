...
*/
    ...
    if( target )
        return notify_fail("道心种魔大法只能自己使用！\n");
    write("你要钻到哪一个人身边？");
...
write("你无法感受到这个人的精力 ....\n");return;}
    ...
    if( random(ob->query("jingli")) > (int)me->query("jingli") / 2 || time() - me->query_temp("do_flee")<2) {
        message_vision( CYN "$N飞快地在地上扒了个洞，钻了进去 ....\n" NOR, me);

        write("咚咚几声，你挖到几块大石头，哇，没门！\n");
    me->start_busy(2);
            message( "vision",CYN+me->name()+"一脸气急败坏，灰头灰脸地从洞里又钻了回来。\n" NOR, environment(me),({me}));
        return;
    }
     me->set_temp("do_flee",time());
    if( !environment(ob) ||
     environment(ob)->query("no_magic") || environment(ob)->query("no_fight") || random(me->query_skill("daoxin-zhongmo",1)) < 30 ) {
        write("你手指都磨破了，才挖出个老鼠洞，MY GOD，怎么钻呀！\n");
    me->start_busy(2);
            message( "vision",CYN+me->name()+"使劲把脸往土里蹭了几下，真无聊。\n" NOR, environment(me),({me}));
        return;
    }
  ...
  if ( domain_file(base_name(environment(ob)))=="yihua" ){
    me->start_busy(2);
     write("恭喜你，你挖到了茅坑哩！\n");
            message( "vision",CYN+me->name()+"从口了吐出几口大粪大叫：好臭呀！好臭呀！\n" NOR, environment(me),({me}));
        return;
    }
      if ( domain_file(base_name(environment(ob)))=="xiakedao" || domain_file(base_name(environment(ob)))=="xixia"){
    me->start_busy(2);
     write("恭喜你，你挖到了茅坑哩！\n");
            message( "vision",CYN+me->name()+"从口了吐出几口大粪大叫：好臭呀！好臭呀！\n" NOR, environment(me),({me}));
        return;
  ...
  if ( domain_file(base_name(environment(ob)))=="jingcheng" ){
    me->start_busy(2);
  ...
  if ( domain_file(base_name(environment(ob)))=="gaochang" ) {
    me->start_busy(2);
     write("那里太遥远了。\n");
            message( 
 "vision",CYN+me->name()+"一脸气急败坏，灰头灰脸地从洞里又钻了回来。\n" NOR, environment(me),({me}));
        return;
    }
 ...
 if ( domain_file(base_name(environment(ob)))=="heimuya" )  
{
 ...
 "vision",CYN+me->name()+"一脸气急败坏，灰头灰脸地从洞里又钻了回来。\n" NOR, environment(me),({me}));
    ...
    message( "vision", CYN "\n一个黑影突然从地下冒了出来，喷了你满脸的灰！\n\n" NOR,
        environment(ob), ({ me }) );
        if (me->query_skill("taoism",1)>me->query_skill("daoxin-zhongmo",1))
        me->improve_skill("daoxin-zhongmo", random(me->query_skill("taoism")),0);
        else me->improve_skill("daoxin-zhongmo", random(me->query_skill("taoism")),1);
}
