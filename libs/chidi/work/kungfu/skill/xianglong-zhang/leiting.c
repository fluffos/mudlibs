...
int perform(object me, object target)
{
   ...
   return notify_fail("你的内力修为还不够高。\n");
    ...
    msg = HIC"$N默运混天气功，施展出「"+HIY"雷"NOR+HIC"霆"NOR+HIG"一"NOR+HIR"击"NOR+HIC"」，全身急速转动起来，越来越快，就犹如\n一股旋风，骤然间， $N已卷向正看得发呆的$n。\n"NOR;

   if( random(me->query("combat_exp")) > (int)target->query("combat_exp")/3) 
   { 
     me->start_busy(2);
     target->start_busy(random(3));
     damage=(int)me->query_skill("xianglong-zhang",1)
             +(int)me->query_skill("huntian-qigong",1);
     damage=damage*2+random(damage*4);

     target->receive_damage("qi", damage/2);
     target->receive_wound("qi", damage/3);
     msg += HIR"$n只见一阵旋风影中陡然现出$N的双拳，根本来不及躲避，被重重击中，五脏六腑翻腾\n不休，口中鲜血如箭般喷出！！！\n"NOR;
    } else
    {
       me->start_busy(4);
