...
int exert(object me, object target)
{
    int skill;
    if( target != me ) 
        return notify_fail("葵花宝典只能对自己使用。\n");
  ...
  if( (int)me->query("neili") < 10000  ) 
    ...
    skill = me->query_skill("kuihua-baodian",1);
        if (skill<900) return notify_fail("你的葵花宝典还不够娴熟。\n");
    message_vision( HIW "$N微一凝神，运起葵花宝典，整个人如同被烟雾笼罩一般，漂忽不定。\n"
    "鬼魅至极。\n" NOR, me);
    me->add("neili",-3000);
...
void remove_effect(object me, int amount)
{
    ...
    message_vision( BOLD "$N的葵花宝典运行完毕，全身忽的一顿，现出整个身形。\n" NOR, me);
}
void reset() { }
