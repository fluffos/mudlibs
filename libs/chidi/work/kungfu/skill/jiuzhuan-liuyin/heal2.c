...
{
       int skill;
skill = me->query_skill("jiuzhuan-liuyin",1);
    if( me->is_fighting() )
...
if (me->query("special_skill/gongfusoul") == 1 )
 ...
 if ((int)me->query_skill("jiuzhuan-liuyin", 1) > (int)me->query_skill("kuihua-xinfa",1)/2)
                return notify_fail("你的葵花心法不能支撑你的九转残篇。\n");
  ...
  me->improve_skill("jiuzhuan-liuyin",random((int)me->query_int())*4);
