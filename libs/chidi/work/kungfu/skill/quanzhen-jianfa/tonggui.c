...
int perform(object me, object target)
{
	...
	int extra;
        string msg;
	extra = me->query_skill("quanzhen-jianfa",1);
        ...
        if( (int)me->query_skill("quanzhen-jianfa", 1) < 1300 )
                return notify_fail("你的全真剑法不够娴熟，不会使用「同归剑法」。\n");

        if( (int)me->query_skill("xiantian-qigong",1) < 1000)
...
//        me->add_temp("apply/dodge", -(int)me->query_skill("dodge",1)/2);
        me->add_temp("apply/damage",(int)me->query_skill("quanzhen-jianfa",1)/2);
        me->set_temp("qzj_tong", 1);
	me->start_call_out( (: call_other, __FILE__, "remove_effect", me, (int)me->query_skill("dodge",1)/2, (int)me->query_skill("quanzhen-jianfa",1)/2:), 30);
	me->add("neili", -500);
	me->start_busy(3);
