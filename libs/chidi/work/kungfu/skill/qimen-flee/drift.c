...
int conjure(object me, object target)
{
    ...
    if  (!environment(me)->query("outdoors")||environment(me)->query("no_fly"))
...
void select_target(object me, string name)
{
  ...
  {write("你无法感受到这个人的精力 ....\n");return;}
   ...
   if( random(ob->query("jingli")) > (int)me->query("jingli") / 5 || time() - me->query_temp("do_flee")<2) {
    ...
    if( !environment(ob) ||
     environment(ob)->query("no_magic") || environment(ob)->query("no_fight") || random(me->query_skill("qimen-flee",1)) < 30 ||environment(ob)->query("outdoors")=="12gong") {
  ...
  if ( domain_file(base_name(environment(ob)))=="shaolin"
    ...
    || domain_file(base_name(environment(ob)))=="erengu" 
...
if ( domain_file(base_name(environment(ob)))=="erengu" ){
     write("恶人谷地下太硬了！你钻得动吗？\n");
      //      message( "vision",HIY+me->name()+"脑袋都钻破了，也没能钻进地下！\n" NOR, environment(me),({me}));
        return;
    }
  if ( domain_file(base_name(environment(ob)))=="cxwxm" ){
     write("恭喜你，你挖到了茅坑哩！\n");
       //     message( "vision",HIY+me->name()+"从口了吐出几口大粪大叫：好臭呀！好臭呀！\n" NOR, environment(me),({me}));
        return;
    }
 ...
 //"vision",HIY+me->name()+"一脸气急败坏，灰头灰脸地从洞里又钻了回来。\n" NOR,  //environment(me),({me}));
     ...
     if ( domain_file(base_name(environment(ob)))=="bianliang" ) {
       write("那里太遥远了。\n");
     //         message( 
  // "vision",HIY+me->name()+"一脸气急败坏，灰头灰脸地从洞里又钻了回来。\n" NOR,  //environment(me),({me}));
          return;
...
//   if ( domain_file(base_name(environment(ob)))=="city3" ){
  if ( domain_file(base_name(environment(ob)))=="city3"||domain_file(base_name(environment(ob)))=="residence" ){
     write("那里太遥远了。\n");
       //     message( 
 //"vision",HIY+me->name()+"一脸气急败坏，灰头灰脸地从洞里又钻了回来。\n" NOR,  //environment(me),({me}));
        return;
   ...
   if ( domain_file(base_name(environment(ob)))=="mingjiao" ||domain_file(base_name(environment(ob)))=="llzhuang"){
     write("那里太遥远了。\n");
       //     message( 
 //"vision",HIY+me->name()+"一脸气急败坏，灰头灰脸地从洞里又钻了回来。\n" NOR,  //environment(me),({me}));
        return;
       ...
       if ( domain_file(base_name(environment(ob)))=="banghui" ||domain_file(base_name(environment(ob)))=="llzhuang"){
     write("那里太遥远了。\n");
     //       message( 
 //"vision",HIY+me->name()+"一脸气急败坏，灰头灰脸地从洞里又钻了回来。\n" NOR,  // environment(me),({me}));
        return;
