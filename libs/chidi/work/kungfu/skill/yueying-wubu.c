...
int valid_learn(object me)
{
    if (me->query("gender")!="女性" || me->query("family/family_name")!="灵鹫宫")
        return notify_fail("月影舞步非灵鹫宫弟子不能习学！\n");
...
{
        me->set_temp("fight/dodge",(me->query_skill("yueying-wubu",1)/2)*3);
...
mixed ob_hit_dodge(object me, object victim, int damage)
{
        ...
        if ( (victim->query_skill_mapped("force") != "bahuang-gong" 
			|| !living(victim)
			|| victim->query("family/family_name") != "灵鹫宫")
			|| me->query("gender") != "女性"
			|| me->query_skill("yueying-wubu",1) < 100000 )
			return damage;

		n	= 1 + me->query_skill("yueying-wubu",1) / 600000;
		...
		{
			result = HIR "$n自知不堪此重击，临危之际将月影舞步的潜力发挥到极至，终能避过致命一击。\n" NOR;
			damage = damage/4;
        }
		else
		{
			result = HIR "$n轻舞飞扬，身姿曼妙，在险险之间避过要害。\n" NOR;
		}
