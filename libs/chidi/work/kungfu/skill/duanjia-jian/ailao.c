...
//		return notify_fail(target->name() + "目前正自顾不暇，放胆攻击吧ⅵ\n");
	...
	if (me->query("combat_exp")*10/target->query("combat_exp")<120)
		a=2;
	else {
		if (me->query("combat_exp")*10/target->query("combat_exp")<150)
			a=3;
		else {
			if (me->query("combat_exp")*10/target->query("combat_exp")<180)
				a=4;
			else a=5;
		}
	}
