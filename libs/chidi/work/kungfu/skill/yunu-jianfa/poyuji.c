...
int perform(object me, object target)
{
	...
	jiajin= (int)me->query("jiali");
       ...
       if( !target ) target = offensive_target(me);
        ...
        if(     me->query_skill("yunu-jianfa",1) < 1 ||
                me->query_skill("quanzhen-jianfa",1) < 1 
                )
                return notify_fail("必须通晓并备有玉女剑法与全真剑法！\n");

        if(me->query_skill("quanzhen-jianfa",1) <1200)
                return notify_fail("你的全真剑法不够熟练，不能双剑和璧左右互搏！\n");

        if(me->query_skill("yunu-jianfa",1) <1800)
	...
	if( (int)me->query("neili") < me->query("max_neili")/7+200 ) 
       ...
       if( !wizardp(me) &&(int)me->query_skill("meinu-quanfa", 1) < 500 )
		return notify_fail("你的本门外功不够!不能贯通使用！\n");

	if(!wizardp(me) && (int)me->query_skill("yunu-jianfa", 1) < 500 )
		return notify_fail("你的本门外功不够高!不能贯通使用！\n");

	if(!wizardp(me) && (int)me->query_skill("yunu-xinjing", 1) < 500 )
		return notify_fail("你的本门内功不够高!不能贯通使用！\n");

        if (!wizardp(me) && !me->query("guard/ok"))
		return notify_fail("你还没有通过华山论剑! 无法对本门武功贯通使用！\n");

        if ( !wizardp(me) && me->query_skill_mapped("force") != "yunu-xinjing")
	...
	me->add("neili", -me->query("max_neili")/7);
        ...
        skill =  ( me->query_skill("yunu-jianfa",1) + me->query_skill("yunu-jianfa",1)
                + me->query_skill("meinu-quanfa",1) +me->query_skill("yunu-xinjing",1) 
                + me->query_skill("yunu-xinjing")) / 10; 
        message_vision(
  ...
  if( ( me->query("jh_dj/dj") ) > ( target->query("jh_dj/dj") ) || !living(target) )
  {
			damage = random(  me->query_skill("meinu-quanfa",1) + me->query_skill("yunu-xinjing",1) + me->query("jiali") )*( 1 + random(me->query("jiali")/5) ) ;
  ...
  if( ( me->query("jh_dj/dj") ) > ( target->query("jh_dj/dj") ) || !living(target) )
  {
	damage = random(  me->query_skill("force",1) + me->query_skill("unarmed",1) + me->query("jiali") )*( 1 + random(me->query("jiali")/5) ) ;
	if(damage > 2000) damage = 2000;
	if(damage < 300) damage = 300;
