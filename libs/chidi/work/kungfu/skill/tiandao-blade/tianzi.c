...
//    mapping my_fam  = me->query("family");
    ...
    if( me->is_fighting() )
        return notify_fail("「天字诀」不能在战斗中演练。\n");
...
if (me->query("special_skill/gongfusoul") == 1 )
 {
    ...
    if( !skill || (skill < 60))
        return notify_fail("你的天刀刀法等级不够, 不能演练「天字诀」！\n");
    if( me->query("neili") < 50 )
        return notify_fail("你的内力不够，没有力气演练「天字诀」！\n");
    if (me->query_skill("blade",1)<= skill)
        return notify_fail("你的基本刀法不够，无法通过演练「天字诀」提高天刀刀法的技巧！\n");
    if( me->query("jing") < -jing_cost )
        return notify_fail("你现在太累了，无法集中精神演练「天字诀」！\n");
    write( HIW "你使出天刀刀法之「天字诀」，将手中" + weapon->name() + HIC "向四面八方乱砍。\n" + NOR);
   //message_vision(msg, me);
    me->add("neili", -50);
    me->add("jing", jing_cost);
   me->improve_skill("tiandao-blade",random((int)me->query_int())*4);
