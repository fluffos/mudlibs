    ...
    if (!weapon->query("equipped")){ me->start_busy(1); return; }
        for(i = 0; i < attack_time; i++)
            if (me->is_fighting(target) && target->is_fighting(me) && target->query("eff_qi")>0){
              me->set_temp("action_msg","迅猛无比的");
                if (!weapon->query("equipped")){ me->start_busy(1); return; }
        if (me->query_skill_mapped("blade")!="xiuluo-dao") {me->start_busy(1); return; }
...
int perform(object me, object target)
{
    ...
    if( !me->is_fighting() )
       return notify_fail("「修罗灭斩」只能在战斗中使用。\n");
    ...
    if((int)me->query_skill("dodge") < 135)
        return notify_fail("你的轻功修为不够, 不能使用修罗灭斩！\n");
