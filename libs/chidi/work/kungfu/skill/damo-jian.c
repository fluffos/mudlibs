...
mapping *action2 = ({([  "action":"$N一式「"+HIG+"浮世沧桑远"+NOR+"」，纵身飘开数尺，运发剑气，手中$w遥摇指向$n的$l",
        "force" : 550,
        "dodge" : 230,
        "parry" : 240,
        "damage" : 265,
...
mapping *action3 = ({([  "action":HIR"$N手中$w斜指苍天，剑芒吞吐，一式「"+HIW+"鱼龙听梵音"+NOR HIR+"」，对准$n的$l斜斜击出"NOR,
        "force" : 1080,
        "dodge" : 1035,
        "parry" : 1115,
        "damage" : 575,
 ...
 ([  "action":HIG"$N左指凌空虚点，右手$w逼出丈许雪亮剑芒，一式「"+HIW+"千里一苇去"+HIG+"」刺向$n的$l"NOR,
        "force" : 1040,
        "dodge" : 1140,
        "parry" : 1120,
        "damage" : 575,
 ...
 ([  "action":HIC"$N合掌跌坐，一式「"+HIB+"禅心顿自明"+NOR HIC+"」，$w自怀中跃出，如疾电般射向$n的胸口"NOR,
        "force" : 1100,
        "dodge" : 1145,
        "damage" : 675,
        "parry" : 1125,
...
mapping query_action(object me, object weapon)
{
        ...
        level   = (int) me->query_skill("damo-jian",1);
          me = this_player();
        if (me->query_skill("damo-jian",1)<401)  
    return action[random(sizeof(action))];
   
        if (me->query_skill("damo-jian",1)>400
        && me->query_skill("damo-jian",1)<1001)
    return action2[random(sizeof(action2))];
 
        if (me->query_skill("damo-jian",1)>1000
        && me->query("jh_dj/dj")>=5)
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
     if( damage_bonus < 110 ) return 0;
      ...
      if( random(10) < 2   ) return 0;
        ...
        if( me->query_skill("damo-jian",1)>5000
       && me->query("jh_dj/dj")>=25
   ) {
                victim->receive_damage("qi", (damage_bonus -100 ) * 2 , me);
return HIR "$N已领悟达摩剑法最高境界，瞬间剑气纵横！$n感觉尤如万剑穿心，顿时令$n痛不欲生！！\n" NOR;
  }
   // 第五等级的伤气

        if( me->query_skill("damo-jian",1)>4000
       && me->query("jh_dj/dj")>=20
   ) {
                victim->receive_damage("qi", (damage_bonus - 100)   , me);
return HIR "$N达摩剑法已经到了第五层境界，瞬间剑气纵横！$n感觉尤如千百剑穿心，顿时令$n痛不欲生！！\n" NOR;
  }
   // 第四等级的伤气

        if( me->query_skill("damo-jian",1)>3000
       && me->query("jh_dj/dj")>=15
   ) {
                victim->receive_damage("qi", (damage_bonus - 100) / 2  , me);
return HIR "$N达摩剑法已经到了第四层境界，瞬间剑气纵横！$n感觉尤如一剑穿心，顿时令$n痛不欲生！！\n" NOR;
  }
...
return HIR "$N趁着剑势未老，又以一股内家暗劲顺着剑尖传向$n的体内，顿时令$n伤上加伤！！\n" NOR;
  }
  
// 第二等级的伤气
        if ( me->query_skill("damo-jian",1)>1000
         && me->query_skill("damo-jian",1)<2001
  ) {
             victim->receive_damage("qi", (damage_bonus - 100) / 4 , me);
return HIW "$N的达摩剑法已初有小成,带出一小阵"HIR"剑气"HIW"扑向$n！！！！！\n" NOR;
}
