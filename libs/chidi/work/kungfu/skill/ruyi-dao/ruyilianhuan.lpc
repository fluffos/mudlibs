...
int perform(object me, object target)
{
	...
	if (!objectp(weapon = me->query_temp("weapon"))
      ...
      skill = me->query_skill("ruyi-dao");
        message_vision(HIY "$N一声长笑，只觉世事如意，豪气满胸，哪管什么是非荣辱，反手一刀砍出！\n\n"NOR,me,target);
        me->add("neili", -500);

       	COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
	        message_vision(HIB"$N刀风过处，意犹未尽，反手又是一刀！\n"NOR,me,target);
       	COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
	        message_vision(HIR"$N身随刀转，顺手又是一刀！\n"NOR,me,target);
       	COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
	        message_vision(HIW"$N身形再转，如意一刀又回转回来！\n"NOR,me,target);
             	COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
       me->start_busy(3);
        return 1;
}
