...
*/
    if (me) me->set_temp("fight/dodge",me->query_skill("duanshi-shenfa",1));

    if( (me->query_skill_mapped("force") == "kumu-shengong" 
            && me->query("family/family_name") == "大理段家") )
            me->add_temp("fight/dodge",me->query_skill("duanshi-shenfa",1));
...
mixed ob_hit_dodge(object me, object victim, int damage)
{
        int ap, dp, diff1, diff2;
        string result;
        object weapon;

        if ( (victim->query_skill_mapped("force") != "kumu-shengong" 
			|| victim->query_skill_mapped("parry") != "sun-finger"
			|| !living(victim)
        ...
        ap = me->query("combat_exp")/100 + me->query_skill("parry")*80;
        dp = victim->query("combat_exp")/100 + victim->query_skill("finger")*120;

        if ( (me->query_skill_mapped("force")=="hamagong") )
			ap = ap/2;
		if ( (int)me->query_skill("kuihua-xinfa", 1) > 100 || (int)me->query_skill("pixie-jian", 1) > 100 )
			ap = ap/3;
        ...
        if (random(dp)>ap/2 )
		{
			...
			{
				if (victim->query_skill("sun-finger",1)>180
					&& victim->query("neili") > 100)
				{
					if(random(4)==1)
					{
						result = HIY "$n看准了"+weapon->query("name")+"的来路，一阳指内劲激射而出，"+
							weapon->query("name")+"立刻被震为一堆碎片！\n" NOR;
