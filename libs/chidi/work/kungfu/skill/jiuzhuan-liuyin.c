...
if (me->query("special_skill/yinyangbody") == 1 && (me->query("neili")<me->query("max_neili")*5/4))
     {
    me->add("neili",random((int)me->query("jiali"))*5);
victim->receive_wound("qi", (damage_bonus ));
victim->receive_wound("jing", (damage_bonus ));
   me->add("neili",random((int)me->query("jiali"))*5);
 write(HIR "你融合葵花心法的精髓，发挥出九转十二天的暗劲，震毁了对手的丹田。！\n" + NOR);
     }

if(random(10001+me->query("kar"))>6666 && me->query_skill("jiuzhuan-liuyin",1)<(me->query_skill("kuihua-xinfa",1)/2)&&me->query_skill("jiuzhuan-liuyin",1)<(me->query_skill("kuihua-xinfa",1)))
{
   me->improve_skill("jiuzhuan-liuyin",random((int)me->query_int())/10);
message_vision(HIW"$N在战斗中对六阴残篇有所领悟！\n"NOR,me,victim);
}
   
    if( random(3*(me->query_skill("jiuzhuan-liuyin",1))) > (damage_bonus )) {
if (me->query("jiuzhuanlv")){
        ...
        return HIW "$N发挥出九转十二天六阴残篇的第"+chinese_number(me->query("jiuzhuanlv"))+"重暗劲，摧毁了$n的真元！！！！\n" NOR;
                               }
else   {
                        ...
                        victim->add("neili", -(int)me->query("jiali"));
                          return HIR "$N的九转12天六阴残篇摧毁了$n的真元！！！！\n" NOR;
        }
}
}
...
{
   if ((int)me->query_skill("jiuzhuan-liuyin",1) >= 0)
     return notify_fail("九转12天六阴残篇只能靠战斗来提升。\n");
    return 1;
}
...
{
           switch (me->query_skill("jiuzhuan-liuyin",1))
   {
                case 1001:
     {
...
{
         return __DIR__"jiuzhuan-liuyin/" + func;
}
