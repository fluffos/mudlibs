...
int heal_myself(object me)
{
  ...
  if ((int)me->query_skill(force_skill, 1) > 10000)
        return notify_fail("你的"+to_chinese(force_skill)+"只能练到这个程度了。\n");
    if ((int)me->query_skill(force_skill, 1) < 50)
        return notify_fail("你的"+to_chinese(force_skill)+"修为还不够。\n");
    if( (int)me->query("neili") < me->query("max_qi")/50 )
...
if (me->query("special_skill/gongfusoul") == 1 )
 ...
 me->improve_skill(force_skill, random(me->query_skill(force_skill, 1)*2),0);
        }  
