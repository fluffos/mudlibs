...
msg = msg + order[random(13)] + taijin[random(14)] + NOR;
        ...
        level > 500 && me->query("neili") > 10000){   
           me->add("neili", -30);    
              return ([
  ...
  if( !me->is_busy() )
{
      ...
      if(me->query("jh_dj/dj") > ob->query("jh_dj/dj")
         && me->query_skill_mapped("unarmed") == "taiji-quan"
         && me->query_skill_mapped("parry") == "taiji-quan"
         && me->query_skill("taiji-shengong", 1) > 200
         && neili > 100 
         && skill > 200 
         && living(me)
...
if (damage>50)
{
         if(neili >= neili1 + damage){
        	...
        	if( weapon ) msg = "$N急中生智，用借力打力之法使身体微侧，让"+weapon->name()+"劲力稍减！\n";
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
    ...
    if(random(20) >= 13 && !victim->is_busy() && j > 120 &&
       ...
       me->query_skill_mapped("unarmed") == "taiji-quan" &&
       me->query("neili") > 6000 && me->query("max_neili") > 12000 &&
           ...
           if (!victim->is_busy())
{
        ...
        switch(random(4)){
         ...
         case 2 : msg += HIR"登时便套得$n跌跌撞撞，身不由主的立足不稳，犹如醉酒昏迷。\n"NOR; break;
         case 3 : msg += HIR"$n想使出千斤坠之力定住身形，却已满脸胀得通红，身不由己，狼狈万分。\n"NOR; break;
         }
