...
mixed hit_ob(object me, object victim, int damage_bonus)
{
    ...
    if( me->query("neili") < 50000   ) return 0;
     ...
     //第六等级伤害
        if( me->query_skill("cxwx-quan",1)>5000
       && me->query("jh_dj/dj")>=25
 ) {
                victim->receive_wound("qi", (damage_bonus - 100) * 2 , me);
return HIR "$N将真元之力"HIC"痴心妄想"HIR"化成千百股罡气挥洒，顿时将$n"HIR"心脉震碎！\n" NOR;
}

    //第五等级伤害
        if( me->query_skill("cxwx-quan",1)>4000
       && me->query("jh_dj/dj")>=20
 ) {
                victim->receive_wound("qi", (damage_bonus - 100)  , me);
return HIR "$N将真元之力"HIC"痴心妄想"HIR"化成一股罡气运于拳上，顿时打的$n"HIR"鲜血狂喷！\n" NOR;
}
 //第四等级伤害
        if( me->query_skill("cxwx-quan",1)>2000
       && me->query("jh_dj/dj")>=15
 ) {
                victim->receive_wound("qi", (damage_bonus - 100)  / 2, me);
return HIR "$N将慢真气"HIC"痴心妄想"HIR"化成一股罡气运于拳上，顿时打的$n"HIR"鲜血直流！\n" NOR;
}
//第三等级伤害
        if( me->query_skill("cxwx-quan",1)>1000
...
return HIR "$N将内力"HIC"痴心妄想"HIR"化成一股劲道运于拳上，顿时打的$n"HIR"气血翻腾！\n" NOR;
  }      
...
return HIW "$N将慢脑子痴心妄想化成一股力道运于拳上，顿时打的$n"HIW"气血上涌！\n" NOR;
}
