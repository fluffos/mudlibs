...
if ( (string)me->query("family/family_name") != "峨嵋派") 
      return notify_fail("只有娥眉弟子才能使用该PFM。\n");
      ...
      if ((int)me->query_skill("huifeng-jian", 1) < 600 )
        return notify_fail(WHT"你的回风拂柳剑法不够娴熟，还使不出「灭剑」。\n"NOR);
      if ((int)me->query_skill("linji-zhuang", 1) < 600 )
...
damage = (int)me->query_skill("huifeng-jian", 1)*5+(int)me->query_skill("force",1)*2;
...
damage2 = (int)me->query_skill("huifeng-jian", 1)*5+(int)me->query_skill("force",1)*2;
