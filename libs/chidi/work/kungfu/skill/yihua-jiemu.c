...
{
   int dsk1,dsk2;
   dsk1 = me->query_skill("dodge");
   dsk2 = victim->query_skill("dodge");

        if(random(dsk1) < random(dsk2/2) ) 
        {
             damage += (dsk1-dsk2);  //功力不够，妄用神功反而会自食其果
             if(damage<=0) {
                     damage = 0;
                message_vision( BOLD "\n$N一招击在$n身上，却被$n暗运移花接木，将内力尽数移入地下。" NOR,me,victim);
                 }
               else message_vision( BOLD "\n$n左牵右引，$N如身处漩流，连续转了好几个圈,功力无法充分发挥出来!" NOR,me,victim);
            }
      return damage ;
    }
