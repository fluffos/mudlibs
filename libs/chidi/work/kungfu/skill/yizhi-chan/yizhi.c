...
int perform(object me, object target)
{
	...
	||	!me->is_fighting(target) )
		return notify_fail("绝技「一指点乾坤」只能对战斗中的对手使用。\n");
	if (objectp(weapon = me->query_temp("weapon")))
			return notify_fail("你不能使用武器！\n");

	if( (int)me->query_skill("yizhi-chan", 1) < 1000 )
		return notify_fail("你的一指禅太生疏，不会使用「一指定乾坤」。\n");

	if ( me->query("neili") < 500)
		return notify_fail("你的内力不够，现在无法使用「一指定乾坤」。\n");
	me->add("neili", -400);
...
//               message_vision(msg, me, target)
	...
	if( random( me->query_skill("force") ) > random((int) target->query_skill("force") ) )
	{
		...
		{
			int damage = 0;
			if (target->query_skill("dodge") > 300 && random(2) == 0)
			{
				switch (random(2))
				{
				case 0:
...
//               message_vision(msg, me, target);
			...
			if (target->query_skill("force") > 300 && random(2) == 0)
			{
				switch(random(2))
				{
				case 0:
