...
int perform(object me, object target)
{
    ...
    if((int)me->query_skill("hunyuan-yiqi",1) < 1000 )
 ...
 if( random(COMBAT_D->skill_power(me, "unarmed", SKILL_USAGE_ATTACK)) > COMBAT_D->skill_power(me, "target", SKILL_USAGE_DEFENSE) / 4 ) {
        msg += "结果$p被$P攻了个措手不及！\n" NOR;
        target->start_busy( (int)me->query_skill("yizhi-chan",1) / 30 + 2);
        me->add("neili", -150);
    } else {
