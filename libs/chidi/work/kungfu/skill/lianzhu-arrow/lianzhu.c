...
int perform(object me,object target)
{
 ...
 if( environment(me)->query("no_fight") )
  ...
  if (num >=32) num=32;//加了限制，超过42会出bug
...
int do_she(string arg,object me,object bow,int num)
{
   ...
   {
    message_vision("$N手中的"+bow->query("name")+"“嘣”地一声断了！\n",me);
    destruct(bow);
    return 1;
   }
   ...
   {
    if( n > bow->query("dam")/100 ) n=bow->query("dam")/100;
    for(i=0;i<n;i++)
    {
    ...
    if( room)
     ...
     if( room->query("no_fight") )  //add by luoyun 2006-1-30
      { 
       tell_object(me,"那里不准战斗！\n"); 
       return 1;
      }      
