...
int perform(object me, object target)
{
	...
	if( (int)me->query("neili") < me->query("max_neili")/7+2000 ) 
		return notify_fail("你的内力不够。\n");

       if( !wizardp(me) &&(int)me->query_skill("taiji-quan", 1) < 1500 )
		return notify_fail("你的本门外功不够!不能贯通使用！\n");

       if( !wizardp(me) &&(int)me->query_skill("taiji-jian", 1) < 1500 )
		return notify_fail("你的本门外功不够!不能贯通使用！\n");

	if(!wizardp(me) && (int)me->query_skill("taiji-shengong", 1) < 1500 )
