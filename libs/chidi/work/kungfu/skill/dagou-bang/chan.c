...
int perform(object me, object target)
{
        ...
        string msg;
    int busy;
    busy=(int)me->query("jh_dj/dj")/2+1; 
    if (busy > 20 ) busy=20;
 ...
 if ( (string)me->query("family/family_name") != "丐帮") 
      return notify_fail("「缠字决」只有丐帮弟子才能使用。\n");
...
if( random(me->query_skill("dagou-bang",1)+me->query_skill("club",1)) > (int)target->query_skill("unarmed",1) ) {
