...
mapping query_action(object me, object weapon)
{
    ...
    int level = me->query_skill("xianglong-zhang",1);
        ...
        ran_action["damage"]= level/2;
        ran_action["attack"]= level;
        }
    return ran_action;
}
...
void do_interlink(object me, object victim)
{
    ...
    skill=me->query_skill("xianglong-zhang", 1);
        if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
                return;
    if (skill>300 && me->query("max_neili")>5000 && random(10)<3){
        enemy=me->query_enemy();
        if (!sizeof(enemy)) return;
        for (i=0;i<sizeof(enemy);i++){
           for (j=0;j<(skill/(100000*sizeof(enemy))+15);j++)
                if ((me->is_fighting(enemy[i]) || enemy[i]->is_fighting(me)) && enemy[i]->query("qi")>0 ){
                    if (environment(me) == environment(enemy[i]))
                     {
          message_vision(HIY+"\n$N深深吸了口气，大吼一声，把降龙十八掌的精华融于双掌之中，\n"
          +"刹那间只看到狂风大作，飞沙走石，天地为之变色....\n"+NOR,me);                        COMBAT_D->do_attack(me, enemy[i], me->query_temp("weapon"), 0);
                     }
                }else break;
        }
          me->del_temp("sum_power",1);
    }
}
