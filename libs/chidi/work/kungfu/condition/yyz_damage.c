...
int update_condition(object me, int duration)
{
	...
	if (me->query("family/family_name")=="大理段家"
		&& me->query_skill("kumu-shengong")) return 1;
	if (me->query_temp("yyzenemy"))
	{
		message_vision(HIR"$N感到"+xuedao[random(sizeof(xuedao))]+"一阵酸麻，血气不畅，顿时动弹不得！\n"NOR, me);
		me->start_busy(random(6));
		if (duration>=200)
		{
			if (ob = LOGIN_D->find_body(me->query_temp("yyzenemy")))
				me->receive_wound("qi", duration, ob);
			else me->receive_wound("qi", duration, "遭一阳指劲不治而死");
		}
	}
	else me->receive_damage("qi", duration, "遭一阳指劲不治而死");

	if (me->query_condition("yyz_damage")>(50+me->query_skill("force",1)/10))
