...
//    tell_object(find_player("rock"),sprintf("combat_d: rand: %d  baoji:%d  name:%s\n",rand,baoji,me->name()));
	...
	{      // Does the victim dodge this hit?
		...
		{ if(vic_exp<900000000)
			...
			{
				{
					victim->improve_skill("dodge", random(100));
				}
		...
		if( (ap < dp) && !userp(me) ) 
		{
			...
			me->improve_skill(attack_skill, random(100));
		}
		damage = RESULT_DODGE;
	}
	else
	{
		...
		if( random(ap + cost)< pp ) 
		{
			...
			{   if(vic_exp<900000000)
				...
				{
					{
						victim->improve_skill("parry",random(100));
					}
				}
				else if(vic_exp>300000000)
				{
...
//加入北冥和混元一气神功护体效果
				...
				&& random((int)victim->query_skill("force")*2/3)>(int)me->query_skill("force"))
			{
				...
				victim->add("neili",-200);
                       ...
                       if(victim->query_skill("jiuzhuan-12tian", 1) > 600
			&& victim->query_skill_mapped("force") == "jiuzhuan-12tian"
				&& victim->query_temp("powerup")
				&& victim->query("neili")>5000
				&& random((int)victim->query_skill("force")*2/3)>(int)me->query_skill("force"))
			{
				message_vision(HIC"$N试图攻击$n，但是被$n的九转12天神功"
				+"挡住了,没有造成任何伤害。\n"NOR, me, victim);
				damage=0;
				victim->add("neili",-200);
			}

			if(victim->query_skill("jiuzhuan-12tian", 1) > 400
			&& victim->query_skill_mapped("force") == "jiuzhuan-12tian"
				&& victim->query_temp("powerup")
				...
				&& random(victim->query_skill("force")) > me->query_skill("force")/2)
			{
				...
				if( damage < 100 )
				message_vision(HIR"$N受到$n九转12天护体神功反震，发出一声闷哼。\n"NOR,me,victim);
				else if( damage < 200 )
			      message_vision(HIR"$N被$n九转12天护体神功反震，「嘿」地一声退了两步。\n"NOR,me,victim);
				else if( damage < 400 )
				message_vision(HIR"$N被$N九转12天护体神功一震，胸囗有如受到一记重锤，"
						+"连退了五六步。\n"NOR,me,victim);
			else message_vision(HIR"$N被$n九转12天护体神功一震，眼前一黑，身子如断线风筝"
						+"向後飞了出去。\n"NOR,me,victim);
				return 1;
			}
//ended
