...
#define SHEN_RACE "/adm/daemons/race/shen"
...
varargs object make_corpse(object victim, object killer)
{
	...
	if( victim->is_ghost() ) {
		...
		i = sizeof(inv);
                while(i--)
                {
                	if(inv[i]->query_xy() && inv[i]->query("id") != "sky sword" && inv[i]->query("id") != "dragon blade") continue;
                	inv[i]->move(room);
                }
		return 0;
	}
	...
	corpse->set("long", victim->long()
        ...
        if( !wizardp(victim) ) {
               ...
               {
                    if(obj->query_xy() && obj->query("id") != "sky sword" && obj->query("id") != "dragon blade") continue;
