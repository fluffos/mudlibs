...
static int *tlist = ({ 0,0450,0459,0500});
static int *hlist = ({ 42, 1, 1, 1 });
...
private void change_state(int new_state)
{
        ...
        case GET_READY_2:
                ...
                {
                message("channel:sys",sprintf("现在是 %d 点 %d 分，系统将在 %d 点 %d 分""自动备份所有玩家的数据，期间游戏会有停滞。\n",lt[LT_HOUR], lt[LT_MIN],(tlist[BACKUPING] / 100) % 100,tlist[BACKUPING] % 100),users());
                         break;
                } else
                        new_state = BACKUPING;
        ...
        case BACKUPING:
                state = new_state;
                message("channel:sys",sprintf(HIY "现在是 %d 点 %d 分，系统开始""自动备份所有玩家数据，请稍候...\n" NOR,lt[LT_HOUR], lt[LT_MIN]),users());

                backup_data();

                message("channel:sys",sprintf(HIC "系统已经处理完备份工作。\n" NOR,
                                       lt[LT_HOUR], lt[LT_MIN]),users());
...
//        }
       ...
       if (count)
                sys_info(sprintf("总共有%d个文件被保存到(%s)中。\n", count, bkdir));
