...
void greeting(object ob)
{
        ...
        // 每次加入社会声望值
        new_point += random(12) + 1;

        set("name", "青史");  set("id", "^_^");
        // set("name", "梦幻时空");  set("id", "World of bidd");

        // 得到卡片：均望卡，平望卡，
        if( random(50) == 0 ) {
                card = new("/quest/game/obj/go_card");
                card->move(ob);
                message_vision( HIY "$N在" + this_object()->query("short") 
                        + "得到" + card->query("name") + "！\n" NOR, ob);
        }
        else if( random(50) == 0 ) {
                card = new("/quest/game/obj/back_card");
                card->move(ob);
                message_vision( HIY "$N在" + this_object()->query("short") 
                        + "得到" + card->query("name") + "！\n" NOR, ob);
        }
                ...
                card = new("/quest/game/obj/start_card");
                card->move(ob);
                message_vision( HIY "$N在" + this_object()->query("short") 
                        + "得到" + card->query("name") + "！\n" NOR, ob);
        }
        else if( random(100) == 0 ) {
                card = new("/quest/game/obj/luck_card");
                card->move(ob);
                message_vision( HIY "$N在" + this_object()->query("short") 
                        + "得到" + card->query("name") + "！\n" NOR, ob);
        }
        else if( random(100) == 0 ) {
                card = new("/quest/game/obj/whammy_card");
                card->move(ob);
                message_vision( HIY "$N在" + this_object()->query("short") 
                        + "得到" + card->query("name") + "！\n" NOR, ob);
        }
        else if( random(200) == 0 ) {
                card = new("/quest/game/obj/all_average_card");
                card->move(ob);
                message_vision( HIY "$N在" + this_object()->query("short") 
                        + "得到" + card->query("name") + "！\n" NOR, ob);
        }
        else if( random(200) == 0 ) {
                card = new("/quest/game/obj/average_card");
                card->move(ob);
                message_vision( HIY "$N在" + this_object()->query("short") 
                        + "得到" + card->query("name") + "！\n" NOR, ob);
        }
        else if( random(200) == 0 ) {
                card = new("/quest/game/obj/get_card");
                card->move(ob);
                message_vision( HIY "$N在" + this_object()->query("short") 
                        + "得到" + card->query("name") + "！\n" NOR, ob);
        }
        ...
        if( random(40) == 0 ) {
                ...
                if( i > 0 ) {
                        ...
                        CHANNEL_D->do_channel(this_object(), "bidd", "由于" + something_good(ob)
                                + "，" + ob->name() + "获得“" + str + "”的职位！" );
                        ob->set("title",str);
                }
        }
        ...
        if( random(80) == 0 ) {
                ...
                CHANNEL_D->do_channel(this_object(), "bidd", "由于" + something_bad(ob)
                        + "，" + ob->name() + "被衰神附身！" );
                use_whammy_card( ob );
        }
...
int do_go(string arg)
{
        ...
        me->set_temp("bidd", 1);
        call_out( "compelete_bidd", 3, me );

        return 1;
}

private void compelete_bidd(object me)
{
        object env;
        string str, tempstr;
...
int do_qt(string arg)
{
        ...
        // 强行清掉 bidd 的标志。  99-03-11 Key bye ten@tianxia
            me->delete_temp("bidd");
        ...
        message("vision", CYN "可是...." + this_player()->name() + 
                "已经不见了。\n" NOR, this_object(), ({this_player()}));
         me->move("/quest/game/start");
