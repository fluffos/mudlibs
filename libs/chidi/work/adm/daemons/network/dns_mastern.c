...
// It requests every 60 seconds for 5 minutes.  If it doesn 't get on
// by then it switches to the backup server.  It then keeps alternatin
// until it gets a reply.  If it does switch to the backup server i
// doesn't bother checking for the main server, as this would chew cp
...
// this is used by the internet communication front ends.  It return
    	// start call outs - note we do the sequence clean u
