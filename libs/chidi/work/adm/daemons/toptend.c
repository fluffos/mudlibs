...
#define TOPTEN_RMB     "/topten/rmb.txt"

#define RICH_B   CYN"江湖十大"YEL"财主"NOR
...
#define RMB_B      CYN"江湖十大"GRN"富豪"NOR
...
int get_all_data(object player,string type)
{
        ...
        {
                case "rich":
                        total=(int)player->query("more_money")*10000+(int)player->query("money");
                        break;
                case "pker":
                        total=(int)player->query("PKS");
                        break;
                case "exp":
                        total=(int)player->query("jh_dj/dj");
                        break;
                case "shen":
...
int topten_add(object player,string type)
{
        ...
        if(file_size(f_name)==-1)
        {
                datum=get_all_data(player,type); 
                if( datum == -1 ) return 1;
                str=player->query("name")+"("+player->query("id")+")"+datum;
                return topten_save(f_name,b_name,str);
        }
        //主要程序！
