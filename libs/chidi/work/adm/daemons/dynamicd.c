...
void create()
{
       seteuid(ROOT_UID);
       set("channel_id", "系统精灵"); 
       CHANNEL_D->do_channel( this_object(), "sys", "动态地图已经启动。\n");
...
int regenerate_map()
{
        ...
        call_out("regenerate_map",1200); // regenerate map every 20 mins.
...
CHANNEL_D->do_channel(this_object(),"sys","开始重新生成绿柳山庄地图。\n");
CHANNEL_D->do_channel(this_object(),"rumor","听说开始重新生成绿柳山庄地图。\n");
        room_object = ({});
        for(i=0;i<size;i++)
        {
