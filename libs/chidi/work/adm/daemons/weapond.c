...
void bash_weapon(object me, object victim, object weapon, int damage)
{
    ...
    &&  ob = victim->query_temp("weapon") ) 
   {
    ...
    if( wap > 2*wdp && random(10)>7) 
    {
     if (ob->query("rigidity")+random(200) < weapon->query("rigidity"))
     {
      message_vision(HIW "只听见「喀嚓」一声轻响，$N手中的"NOR + ob->name() + HIW
                   + "已经断为两截！\n" NOR, victim );
      ob->unequip();
      ob->set("name", HIW"断掉的"NOR + ob->query("name"));
      ob->set("value", 0);
      ob->set("weapon_prop", 0);
      ob->move(environment(victim));
     } else
     {
      message_vision(HIW "$N只觉得手中"NOR + ob->name() + HIW + "把持不定，脱手飞出！\n" NOR,victim);
      ob->unequip();
     }

     if (!ob->query("no_get") && !ob->query("no_drop"))
     ob->move(environment(victim));
     victim->reset_action();
    } else 
    if( wap > 2*wdp && weapon->weight() > ob->weight()*2 && random(10)>7) 
    {
     message_vision(HIW "$N只觉得手中"NOR + ob->name() + HIW + "把持不定，脱手飞出！\n" NOR,
                victim);
     ob->unequip();
    ...
    if( wap > wdp*3/2 ) 
    {
     message_vision(BOLD+"$N只觉得手中"NOR + ob->name() + BOLD +"一震，险些脱手！\n" + NOR,
                victim);
    }  else 
    {
            if (wap>150)
            message_vision(HIY+"$N的"NOR + weapon->name() + HIY+"和$n的" + ob->name()
                + HIY+"相击，冒出点点的火星。\n" + NOR, me, victim);
           }
        }
