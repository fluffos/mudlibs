...
// modified by wangwwh@yxcs 2002.1.20
#include <login.h>
...
string get_status(mixed ob)
{
   ...
   if ( euid=="luoyun" || euid=="feng") return "(boss)";
...
int set_status(mixed ob, string status)
{
    ...
    if (get_status(this_player(1)) != "(boss)" || previous_object(0)!=find_object("/cmds/bos/promote"))
    {
    me=this_player(1);
    str="On "+ctime(time())+"\n"+
    ...
    if ( author_file(base_name(user)) == ROOT_UID 
         || author_file(base_name(user)) == "MudLib")
          return 1;
           ...
           if ( get_status( author_file(base_name(user)) ) != "(boss)" )
           {
              str="On "+ctime(time())+"\n"+
...
int valid_write(string file, mixed user, string func)
{
    ...
    if( func == "write_file" || func == "write" || func == "cp" || func == "rm")
    {
     ...
     if( sscanf(file,"/include/global%*s") || sscanf(file,"/adm/obj/master%*s")
         || sscanf(file,"/adm/simul_efun/file%*s") 
         || sscanf(file,"/adm/simul_efun/object%*s") 
         || sscanf(file,"/adm/daemons/security%*s")
         || sscanf(file,"/include/command%*s") ) 
     return 0;
    ...
    if ( sscanf( file , "/kungfu/%*s" ) == 1 
         ...
         || sscanf( file , "/adm/%*s" ) == 1
         || sscanf( file , "/doc/%*s" ) == 1
         || sscanf( file , "/log/%*s" ) == 1 )   return 1; 

    if (get_status(user) == "(boss)" )     return 1;
...
int valid_read(string file, mixed user, string func)
{
        ...
        if( sscanf(file,"/adm/simul_efun/file%*s") ||
            sscanf(file,"/adm/simul_efun/object%*s")||
            sscanf(file,"/adm/daemons/security%*s")||
            sscanf(file,"/adm/obj/master%*s") )
        {
             ...
             log_file("file/try_read_file",str);
