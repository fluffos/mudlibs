...
mapping channels = ([
    ...
    "music": ([ "msg_speak": HIY "【欢歌笑语】%s唱著：%s\n" NOR,
                "msg_emote": HIY "【欢歌笑语】%s" NOR,
                "msg_color": NOR+HIY,            
             ]),
                
    "admin": ([ "msg_speak": HIR "%s: %s\n" NOR,
                "msg_emote": NOR+HIR+"〖奇人奇语〗%s" NOR,
                "msg_color": NOR+HIW,
                "adm_only": 1,
             ]), 
    "wiz":   ([ "msg_speak": NOR+HIY+"【管理人员】"+HIG+"%s：%s\n" NOR,
                "msg_emote": NOR+HIY+"【管理人员】"+HIG+"%s" NOR,
                "msg_color": NOR+HIY,           
                "wiz_only": 1,
                "channel": "wiz",
             ]),
	...
	"jiaoyi":  ([ "msg_speak": HIC+"【"+HIM"交易"HIC"】%s：%s\n" NOR,
                  "msg_emote": HIC+"【"+HIM"交易"HIC"】"+CYN+"%s" NOR,
                  "msg_color": NOR+HIC,
                  "anonymous": "代理人",
				]),  
      ...
      "debug": ([ "msg_speak": HIW "【调试信息】%s：%s\n" NOR,
                  "msg_emote": HIY "【调试信息】%s" NOR,
                  "wiz_only": 1,
               ]), 
      "chat":  ([ "msg_speak": HIC+"【"+HIM"论道江湖"HIC"】%s：%s\n" NOR,
                  "msg_emote": HIC+"【"+HIM"论道江湖"HIC"】"+CYN+"%s" NOR,
                  "msg_color": NOR+HIC,
               ]), 
      "new":   ([ "msg_speak": HIY+"【"+HIG+"初入江湖"+HIY+"】"HIY"%s：%s\n" NOR,
                  "msg_emote": HIY+"【"+HIG+"初入江湖"+HIY+"】"+HIY+"%s" NOR,
                  "msg_color": NOR+HIW,
                  "newbie":1,
               ]), 
      "xw":    ([ "msg_speak": HIB+"【新闻】"HIM"%s：%s\n" NOR,
                  "msg_emote": HIB+"【新闻】"+HIM+"%s" NOR,
                  "wiz_only": 1,
                  "msg_color": NOR+HIY,
               ]), 
      "sale":  ([ "msg_speak": "" HIC "〖"HIG"和"HIY"气"HIR"生"HIM"财"HIC"〗%s %s\n" NOR,
                  "msg_emote": HIC+"〖和气生财〗"+CYN+"%s" NOR,
                  "msg_color": NOR+HIG,
               ]),
      "rumor": ([ "msg_speak": HIM "【谣言四起】%s：%s\n" NOR,
                  "msg_emote": HIM "【乱言乱语】%s" NOR,
                  "msg_color": NOR+HIM,
                  "anonymous": "某人",
               ]), 
       "es":   ([ "msg_speak": NOR+HIC+"〖"HIW+HBMAG"网际传情"NOR+HIC"〗"+NOR+BOLD+"%s：%s\n" NOR,
                  "msg_emote": NOR+BOLD+"【"HIW"网际传情"HIG"】"+NOR+BOLD+"%s" NOR,
                  "msg_color": MAG,
                  "intermud": GCHANNEL,
                  "intermud_emote": 1,
                  "channel": "es",
       ...
       "yx":   ([ "msg_speak": WHT "〖江湖〗"+NOR+WHT+"%s：%s\n" NOR,
                  "msg_emote": WHT "〖江湖〗"+NOR+WHT+"%s" NOR,
                  "msg_color": NOR+WHT,
                  "intermud": GCHANNEL,
                  "intermud_emote": 1,
                  "channel": "yx",
       ...
       "snp":  ([ "msg_speak": HIB "〖窃听信息〗%s：%s\n" NOR,
                  "msg_emote": HIB "〖窃听信息〗%s" NOR,
                  "arch_only": 1,
               ]),
      "game": ([ "msg_speak": HBRED "【股票频道】%s：%s\n" NOR,
                 "msg_emote": HBRED "【股票频道】%s" NOR,
                 "msg_color": NOR+HBRED,            
               ]),

/*
       "bh":   ([ "msg_speak": YEL "【%s】%s: %s\n" NOR,
...
CHANNEL_D->do_channel(this_object(),"sys",me->query("name")+"("+me->query("id")+")企图Flood毁坏公共频道！\n");
    ...
    if ( userp(me) && !wizardp(me)){
        ...
        if (time() - (int)me->query("chblk_yx") < 3600 && (verb == "yx"||verb=="yx*"))
        return notify_fail("你的频道被关闭了！\n");   
        if (time() - (int)me->query("chblk_party") < 3600 && (verb == "party"||verb=="party*"))
        return notify_fail("你的门派频道被关闭了！\n");
		//if (time() - (int)me->query("chblk_jiaoyi") < 3600 && (verb == "jiaoyi"||verb == "jiaoyi*"))
		//return notify_fail("你的交易频道被关闭了,想交易请用paimai 物品 add n gold！\n");

     }

     if( userp(me) ) {
        if( channels[verb]["wiz_only"] && !wizardp(me)&&me->query("id")!="feng") 
        return 0;
        if( channels[verb]["arch_only"] && wiz_level(me)<wiz_level("(arch)")&&me->query("id")!="feng")
        return 0;
        if( channels[verb]["adm_only"] && wiz_level(me)<wiz_level("(admin)")&&me->query("id")!="feng")
        return 0;
        ...
        if( userp(me) && !wizardp(me)) {
            ...
            if (time()-me->query_temp("chat_time")<2){
                ...
                do_channel( this_object(), "rumor",
                            sprintf("某人拿出一张狗皮膏药，在小火炉上细细地煨热以后，“啪”地捂住了%s的嘴巴。\n", me->name(1) ));
                return 1;
            }else{
    ...
    //if ( me->query("age")<18 && verb!="new" && verb!="es" && !wizardp(me))
    ...
    if( channels[verb]["anonymous"] ) {
        ...
        if (userp(me))
           if (random(10)>8) who=me->name(1) + "(" + capitalize(me->query("id")) + ")";
                ...
                do_channel( this_object(), "sys", sprintf("%s"+HIM+"躲在茅厕后面造谣。", me->short(1)));
...
/*
        }else if(verb=="bh")
        {
            ...
            ob=filter_array( users(), "banghui_listener", this_object(),
                                                       me->query("banghui"));
*/
        } else
...
/*
                ...
                message( "channel:" + verb,sprintf(channels[verb]["msg_emote"],me->query("banghui"),replace_string(localmsg,NOR,ecol) ), ob );
*/
            else
...
/*
            ...
            message( "channel:" + verb,sprintf( channels[verb]["msg_speak"],me->query("banghui"),who,arg ), ob );
          else
            message( "channel:" + verb, sprintf( channels[verb]["msg_speak"], who, arg ), ob );
*/
    ...
    if (userp(me) && !wizardp(me))
    {
       ...
       if (me->query_temp("channel_repeat") > 2) 
       {
                        ...
                        switch (verb) {
                                ...
                                case "yx": verb = "江湖"; break;
                                case "music":verb ="唱歌"; break;
								case "jiaoyi":verb="交易"; break;
                                default: verb = "交谈";
                        }
                        do_channel(this_object(), "rumor",
...
int filter_listener(object ppl, mapping ch)
{
    ...
    if(userp(ppl)&&ppl->query("id")!="root"){
    if( ch["arch_only"])
        if( userp(ppl) )
...
int banghui_listener(object ppl, string str)
{
        ...
        if(wizardp(ppl) || ppl->query("banghui")==str ) return 1;
        else return 0;
}
*/
