...
#include <ansi.h>
      ...
      { 
              ...
              { 
                     ...
                     { 
                             ...
                             { 
                                     ...
                                     { 
                                             case "↓" : 
                                                    ...
                                                    else if(random < 33) CHANNEL_D->do_channel( xw, "xw", sprintf("%s因庄家抛售，大幅度下跌。\n",keys[i])); 
                                                     else CHANNEL_D->do_channel( xw, "xw", sprintf("%s受到三路奶粉影响，股价下跌。\n",keys[i])); 
                                                     td_name[keys[i]] = ({info[0],1,1,info[3],info[4]}); 
                                             break; 
                                            case "↑" : 
                                                     if(random < 33) CHANNEL_D->do_channel( xw, "xw", sprintf("一个叫美国USA的地方，近日来爆发经济危机，严重影响%s的股票价格。\n",keys[i])); 
                                                     else CHANNEL_D->do_channel( xw, "xw", sprintf("%s邀请芙蓉姐姐作代言人，导致群众不满。\n",keys[i])); 
                                                     td_name[keys[i]] = ({info[0],1,1,info[3],info[4]}); 
                                             break; 
                                             case "→" : 
                                                     if(random < 33) CHANNEL_D->do_channel( xw, "xw", sprintf("%s邀请芙蓉姐姐作代言人，导致群众不满，纷纷抛售该股。\n",keys[i])); 
                                                     else if(random < 33) CHANNEL_D->do_channel( xw, "xw", sprintf("%s因经营不善，依据相关规定，勒令限期整改，大家赶快清仓。\n",keys[i])); 
                                                   else if(random < 33) CHANNEL_D->do_channel( xw, "xw", sprintf("%s因庄家抛售，大幅度下跌。很多散户已经清仓。\n",keys[i])); 
                                                     else CHANNEL_D->do_channel( xw, "xw", sprintf("%s受到三路奶粉影响，股价下跌。\n",keys[i])); 
                                                     td_name[keys[i]] = ({0,0,0,"退市",info[4]}); 
                                                     buy_name[keys[i]] = 0; 
                                             break; 
                            ...
                            rand = random(info[1]); 
                            if(random < 33 ) tmp = info[2]-2; 
                             else if(random < 66 ) tmp = info[2]+2; 
     ...
     { 
             if(!arg || !index || index < 1 || index > 5000) 
             { 
