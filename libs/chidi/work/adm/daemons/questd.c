...
int quest_reward(object me, object who,object quest_item)
{
        ...
        if(pointerp(member=me->query_team())){//ÎªÁË¼ÓÇ¿Íæ¼ÒÖ®¼äµÄºÏ×÷¾«Éñ,¶ÓÎé»ñµÃµÄ¾­Ñé´ó¼Ò¶¼ÓÐ·Ý
               if(who->query("name")!= quest_item->query("owner_name")||!who->query("name")) return 0;
                {
                	for(i=0;i<sizeof(member);i++)
                	{
                             if(me->query("combat_exp")<=20000000)
                             {
                            exp =10000+random(13000)+random(80);
                             pot = exp/2;
                                     member[i]->add("combat_exp",3*exp);
                                     member[i]->add("potential",3*pot);
                                     member[i]->add("score",3*score);
                                    tell_object(member[i],"¹§Ï²Äã£¡ÄãµÄ¶ÓÎéÍê³ÉÁËÒ»ÏîÌØÊâµÄÊ¹Ãü£¡\n");
                                  // tell_object("securityd","¶ÓÎéÍê³ÉÁËÒ»ÏîÌØÊâµÄÊ¹Ãü£¡\n");
                                     tell_object(member[i],HIW"Äã±»½±ÀøÁË£º\n" 
                                     +chinese_number(3*exp) + "µãÊµÕ½¾­Ñé\n"
                                     +chinese_number(3*pot) + "µãÇ±ÄÜ\n"NOR);   
                                     score=member[i]->query_temp("dynamic_find");        
                                     if(score+1==10){
                                     member[i]->delete_temp("dynamic_find");
                                     member[i]->add("combat_exp",10000);
                                     tell_object(member[i],"ÄãÕæÐÐ£¬ÄãµÃµ½ÁË"+chinese_number(10000) + "µãÎäÑ§¾­ÑéµÄ¸½¼Ó½±Àø£¡\n");        
                                     }
                                     else member[i]->set_temp("dynamic_find",score+1);                                                                
        ...
        tell_object(me, sprintf("Äã»ñµÃÁË"HIR"%s"NOR"µÄ¾­ÑéÖµ ,"HIY"%s"NOR"µãÇ±ÄÜ£¡\n"NOR, chinese_number(exp*2),chinese_number(pot*2)) );
        score=me->query_temp("dynamic_find");        
                        if(score+1==10){
            me->delete_temp("dynamic_find");
            me->add("combat_exp",10000);
            tell_object(me,"ÄãÕæÐÐ£¬ÄãµÃµ½ÁË"+chinese_number(10000) + "µãÎäÑ§¾­ÑéµÄ¸½¼Ó½±Àø£¡\n");        
        }
...
if( ! arg)
return notify_fail("zhao <ÎïÆ·id>\n");
    if(present(arg,me))
