...
// last modify by add some xyj's efun 
...
//  if (undefinedp(my["spi"])) my["spi"] = random(21) + 10;   //灵性 add by xiha@wmkj
  ...
  //    if( my["max_mana"] > 0 ) my["max_jing"] += my["max_mana"]/4;   //法力跟精的关系 add by xiha@wmkj
    }
    if( userp(ob) || undefinedp(my["max_qi"]) ) {
                ...
                if((qi_age=(int)ob->query_skill("taoism", 1)) > 39)
        {
            ...
            // End of appendence            
            if (qi_age > 0) my["max_qi"] += qi_age *((int)ob->query_skill("taiji-shengong", 1)/115) ; 
        }
