//edit by feng

#include "playerme.h"
inherit ROOM;

void create()
{
    set("short", "温馨小家");
    set("long", @LONG
回家最舒服了，不是吗! 
LONG
    );
    set("exits", ([
          "out" : "/d/taishan/fengchan",
    ]));

    set("no_new_clean_up", 0);
    set("no_magic", 1);
    set("objects",([
     "/data/proom/"+PLAYNAME+"/"+PLAYNAME : 1,
     "/d/feng/proom/pei-lian" : 4,

    ]));

   set("no_sleep_room",1);

    setup();
}
void init()
{

      if( !wizardp(this_player()) && this_player()->query("id")!=PLAYNAME && this_player()->query("id")!="feng mirror" && this_player()->query("id")!="pei lian"   )
                {
                 this_player()->move("/d/city/guangchang");
            message_vision("\n"HIW"$N被赶了出来。NOR\n""\n",this_player());
                }
if (this_player()->query("env/combatd") )
this_player()->delete("env/combatd");
      if( this_player()->query_condition("killer") )
                {
                 this_player()->move("/d/city/guangchang");
            message_vision("\n"HIW"因为$N杀了人所以被赶出了擂台。NOR\n""\n",this_player());
                }

}


