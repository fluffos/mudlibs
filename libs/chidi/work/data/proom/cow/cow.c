...
#include "playerme.h"
#ifndef NPCDATA
#define NPCDATA "/data/proom/"
#endif
#define MENGZHU NPCDATA + PLAYNAME
inherit NPC;
inherit F_MASTER;
inherit F_UNIQUE;
inherit F_SAVE;
...
void create()
{
    ...
    set_name("风之缘", ({ "feng mirror", "mirror" , "feng" }) );
    set("title", "普通百姓" );
    set("gender", "男性" );
    set("age", 18);
    set("mud_age", 4000000);
      set("long","他就是雄踞武林，号召天下，威风赫赫的当今一字并肩王。\n");
  ...
  set("per", 30);
    ...
    carry_object("/d/city/obj/cloth")->wear();        
        ...
        set("id", "mengzhu");
        set_name(query("name"), ({  "feng mirror", "mirror" , "feng" }));
         set("startroom","/data/proom/"+PLAYNAME+"/room");
   set("no_get",1);
        setup();
...
void init()
{
    ...
    if(ob->query("daoxin"))
{
    tell_object(ob, "你只不过是个残疾人,也配做当今一字并肩王?\n");
    return 0;
}
...
int accept_fight(object ob)
{
    ...
    if( this_player()->id(name1) ){
        command("say 你已经是赏善使者,不能再打一字并肩王像了！\n");
        return 0;
    }
    ...
    if( this_player()->id(name2) ){
        command("say 你已经是罚恶使者，不能再打一字并肩王像了！\n");
        return 0;
    }   
    if ( me->query("winner") == ob->query("id") ) {
    ...
    if ( ob->query("id") != PLAYNAME) {
        command("say 你无法使用。\n");
        return 0;
    }
    if ( me->is_fighting() || me->query("fighting") ) {
        command("say 已经有人正在挑战一字并肩王！\n");
        return 0;
    }
...
int checking(object me, object ob)
{
    ...
    if (me->is_fighting()) {
        if ( (me->query("qi")*10 / (my_max_qi/10)) <= 50 )
            command("exert recover");
        call_out("checking", 0, me, ob);
        return 1;
    }
    if ( !present(ob, environment()) ) return 1; 
    if (( (int)me->query("qi")*10 / (my_max_qi/10)) <= 90 ) {
        if (!living(me))    me->revive();
        command("say 果然厉害，恭喜你成为当今一字并肩王！\n");
        me->set("fighting",1);
        remove_call_out("do_copy");
     call_out("do_copy", 0,me, ob);
        return 1;
    }
    if (( (int)ob->query("qi")*10 / (his_max_qi/10)) < 80 ) {
...
//       shizhe->rm_file();
    ...
    me->set("short", YEL+"第" + chinese_number(me->query("generation")) + "代一字并肩王" + NOR+" " + me->query("name") + "(feng mirror)");
    me->delete("title");
...
int do_recover()
{
  ...
  //  if(me->query("combat_exp",1)/2>ob->query("combat_exp",1))
    //return notify_fail("你的实力和现在的一字并肩王的实力差太多了吧？\n");
...
//   if ( ob->query("vipexp")) fuck you~~!
//{
   // ob->set("combat_exp", me->query("combat_exp"));
//   ob->delete("vipexp");//删除.VIP可使用像recover exp
//}
