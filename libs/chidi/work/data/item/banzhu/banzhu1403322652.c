/**±¾ÎÄ¼şÓÉºÏ³ÉÏµÍ³×Ô¶¯Éú³É**/

#include <ansi.h>
#include <armor.h>

inherit NECK;

void create()
{
	object me = this_player();

	set_name("[1;36mÁúÉñÓù·çÑ¥"NOR, ({"banzhu boots", "hecheng", "boots"}) );
	set("hcloth/name", "[1;36mÁúÉñÓù·çÑ¥"NOR);
	set("hcloth/id", "banzhu boots");
	set("long", "ÕâÊÇÓÉ°çÖí(banzhu)ºÏ³ÉµÄ[1;36mÁúÉñÓù·çÑ¥" + NOR + "¡£\n");
	set("armor_type", "boots");
	set_weight(5000);
	set("unit", "Ë«");
	set("no_get", 1);
	set("no_give", 1);
	set("no_sell", 1);
	set("no_steal", 1);
	set("no_beg", 1);
	set("no_drop", 1);
	set("armor_prop/armor", 265);
	set("armor_prop/constitution", 607);
	set("armor_prop/dexerity", 706);
	set("armor_prop/intelligence", 683);
	set("armor_prop/strength", 706);
	set("wear_msg", me->name() + NOR "×°±¸" + name () + NOR "£¡\n");
	set("unwear_msg", me->name() + NOR "ÍÑÏÂ" + name () + NOR "£¡\n");

   setup();
}

