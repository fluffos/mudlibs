...
spe_skill_name="";
spe_skillnum=0;
if( !me->query("zhuanshi/jineng2"))
       return notify_fail("你目前还不能自创武功。\n");
        if(!arg) return notify_fail("指令格式：messkills 类型 功夫的英文名 功夫的中文名 第几招\n    例如: meskills sword jueshi 绝世剑法 1\n");
if( sscanf(arg, "%s %s %s %d", skill_base_name,skill_en_name,skill_ch_name,skill_num)!=4)
    return notify_fail("指令格式：meskills 类型 功夫的英文名 功夫的中文名 第几招\n    例如: meskills sword jueshi 绝世剑法 1\n");
...
if((int)me->query("skillmaxim")!=0
        && sizeof(keys(me->query("skillmaxim/"))) >6 )
    return notify_fail("你最多只能创建六种高级功夫。\n");
...
if(skill_num < 1)
  ...
  if((int)me->query("combat_exp") < (skill_num-1)*2000000+20000000)
...
if(skill_num_result > 1) {
      write("\r请给招式起个名字(例如: 横空出世)(不要此时断线，否则永远无法再正确创建):");
                   }
...
//here init the skill 招式
  spe_skill["skill_name"]=yn;
     ...
     write(HIR"\n注意问题: $N 代表你 $n 代表敌人 $l 代表攻击部位 $w 代表你的武器 \n"NOR);
         write(BLINK WHT"例如:(拳脚类)\n"NOR);
  write("\t$N两臂紧伸，腿脚尖点地，一式「同归黄泉」，死死的包住$n往半空中自旋而上\n\n");
     write(BLINK WHT"又例如:(带武器类)\n"NOR); 
   write("\t$N纵身一跃，手握$w,顿时一招「斜阳竖月」对准$n的某部位斜斜刺出一剑\n\n");
...
//here replace 你 and 老猫 to ....
 spe_skill["action"]= replace_string(spe_skill["action"], "你","$N");
  spe_skill["action"]= replace_string(spe_skill["action"], "老猫","$n");               
...
{
        write(@HELP
指令格式：meskills 类型 功夫的英文名 功夫的中文名 第几招?
    例如: meskills sword jueshi 绝世剑法 1
指令说明：这是指令用来创建自己武功的指令，随着经验值的
          断增长，所可以实现的招式越来越多，招式的威力?
          越来越大。玩家增加招式的熟练程度的方式是使用?
          用指令：mylian 
注意问题：由于是自己摸索招式，所以难度要大些。玩家自创?
          武功后就难以对武功再做修改。所以玩家要事先规?
          好招数，以及招数的描述后再建立?
          目前可以利用的基本功夫有?
         
          unarmed,sword, blade, staff, hammer, whip
          finger, hand , cuff , claw , strike
          武功的英文名称后面会自动加好类?
          (例如: meskills sword jueshi 绝世剑法 1)当建立
          以后，这个武功的英文会是(jueshi-sword),玩家要?
          先想好，一旦开始创建则无法随意改动英文名称.招?
          描述中不要加入控制字符,否则可能会出错。玩家最?
          只能创建三种高级功夫?
          招式描述中的:$N 代表自己, $n 代表敌人 , $l 代?
          攻击部位, $w 代表自己的武器,这些都将组合起来,?
          以用来对付各种不同敌人,招式的描述不要带有对自?
          或对手状态的描写,不要不负责的乱写。巫师将检查?
          家所创建的功夫，如不符合规定，将被删除?
