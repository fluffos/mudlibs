...
int main(object me, string arg)
{
        ...
        if( !me->query("zhuanshi/jineng2"))
              return notify_fail("你目前无法练习自创武功。\n");
        ...
        if (times < 1 || times > 10)
                return notify_fail("练习次数必须大于一次，但不能超过十次。\n");
...
*/
        ...
        for (i = 0; i < times; i++)
        {
                if (SKILL_D(skillname)->practice_skill(me))
                        me->improve_skill(skillname, ((int)me->query("kar")/100), skill_basic > skill? 0: 1);
                else break;
        }
        if (i > 0)
...
这个指令让你练习自创技能，这个技能必须是经过 enable 的自创技能。

如果你是2转的玩家就可以练习自创技能，你对这方面的基本技能够高，可以经
由练习直接升级，而且升级的上限只跟你基本技能的等级有关，换句话说，勤
加练习是使你的所学「青出于蓝胜于蓝」的唯一途径，当然，在这之前你必须
从实际运用中获得足够的经验以提升你的基
本技能。
TEXT );
        return 1;
