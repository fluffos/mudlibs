...
if( me!=this_player(1) ) return 0;
    ...
    if(SECURITY_D->get_status(name)=="(boss)")
    	return notify_fail("你没有权利删除他的档案!\n");
	seteuid(getuid());
	...
	rm(DATA_DIR + "user/" + name[0..0] + "/" + name + ".o");
        rm(DATA_DIR + "bank/" + name[0..0] + "/" + name + ".o");
        rm(DATA_DIR + "bag/" + name[0..0] + "/" + name + ".o");
        rm(DATA_DIR + "pet/" + name[0..0] + "/" + name + ".o");	
//        rm(DATA_DIR + "password/" + name[0..0] + "/" + name + ".pas");
//	PASSWORD_D->main(name,"delete");
...
protected int do_purge_players(int day)
{
	...
	seteuid(getuid());
if(day<30)
    return notify_fail("您还是手下留情。别连他们都杀了。\n");
	message("system", "\n*** 整理玩家储存档中，请稍候.... ***\n", users());
	write("处理中：");
	count = 0;
	ppl_cnt = 0;
