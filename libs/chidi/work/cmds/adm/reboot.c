...
{
        if( !intp(i) ) i = 3; 
        message("system",BLINK + HIG"\n【警告】江湖将在"RED+chinese_number(i)+HIG"分钟后重新启动，请在线玩家退出，稍后尝试连线。\n\n" NOR, users() );
...
void countdown(int min)
{
        min--;
        if( min ) {
                  message("system",BLINK+HIG"\n【警告】江湖(jh)将在"HIC+chinese_number(min)+HIG" 分钟后重新启动，请在线玩家退出，稍后尝试连线。\n\n"NOR, users() );
                call_out("countdown", 60, min);
        } else {
message("system","\n\n"+ BLINK + HIW + "【警告】游戏马上重启，玩家有何损失我们将不负责。\n\n"NOR,users() );
       
         if( find_object(DNS_MASTER) ) NATURE_D->auto_shutdown();
         NATURE_D->auto_shutdown(5);
  }
}
