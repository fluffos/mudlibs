...
int do_get(object me, object obj,object env,int amount)
{
        ...
        if( obj->is_character() ){
                if( living(obj) && wizhood(me)!="(boss)")  return notify_fail(obj->query("name")+"不用你背。\n");
        }
        ...
        foreach (object ob in all_inventory(env)){
                ...
                if( ob->is_character() && living(ob) && wizhood(me)!="(boss)"){
 ...
 if( obj->query("material")=="baoshi" )
{
       dirname = obj->query("name");
       me->add("终极宝石",1);
       message("channel:chat",HIG"【"+HIR"奇珍异宝"+HIG"】"+HIM"听说"+name+"得到了"NOR+dirname+HIM"了，大家快去抢啊!\n"+NOR,users());
}
   if (!i) return 1;
