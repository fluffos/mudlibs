...
int main(object me, string arg)
{
        ...
        if(me->query("doing"))
         return notify_fail("还是先将你手上的事情做完了再给吧!\n"); 
        if((int)who->query_encumbrance()
            > (int)who->query_max_encumbrance()||sizeof(all_inventory(who))>70 ) 
            return notify_fail( "这对他来说已经负重太重了。\n");
...
int do_give(object me, object obj, object who,int amount)
{
        ...
        if( obj->query("dynamic_quest") && userp(who) )
                return notify_fail("这种物件岂可随意给人！\n");

        if(obj->query("dynamic_quest"))
            if(JOB_D->quest_reward(me,who,obj))
                return 1;
	...
	//if( obj->query("no_give") && (!wizardp(me)) )
        if( obj->query("no_give") && (!wizardp(me)) && userp(who))
		{
