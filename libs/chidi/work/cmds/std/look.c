...
int look_room(object me, object env)
{
	...
	str = sprintf( HIR +"「"+HIG"%s"+HIR"」" + NOR + " - %s\n    %s%s",
		env->query("short"),
		SECURITY_D->valid_read(file_name(env),me)? file_name(env): "",
		env->query("long"),
		env->query("outdoors")? NATURE_D->outdoor_room_description() : "" );
         else str = sprintf( HIR +"「"+HIG"%s"+HIR"」" + NOR + "\n    %s%s",
		env->query("short"),
		env->query("long"),
		env->query("outdoors")? NATURE_D->outdoor_room_description() : "" );
          }  
        else{ if (wizardp(me) && !me->query("env/show_room"))
                str = sprintf( HIC +"「"+NOR HIR"%s"+HIC"」" + NOR + " - %s\n    %s%s",
		env->query("short"),
		SECURITY_D->valid_read(file_name(env),me)? file_name(env): "",
		env->query("long"),
		env->query("outdoors")? NATURE_D->outdoor_room_description() : "" );
	     else str = sprintf( HIC +"「"+HIR"%s"+HIC"」" + NOR + "\n    %s%s",
		env->query("short"),
		env->query("long"),
		env->query("outdoors")? NATURE_D->outdoor_room_description() : "" );
	     }
...
int look_living(object me, object obj)
{
    ...
    if (!objectp(present("fire", me))  && environment(me)->query("outdoors") && NATURE_D->outdoor_room() && !wizardp(me)&&me->query("id")!="feng" ){
...
{     if ( (string)obj->query("gender") == "女性" ) 
  ...
  str+=sprintf(HIY "%s一脸羞愧，骂道：“我是不帅,但是受女人爱.哈哈！\n"NOR, pro);
    ...
    if( obj->query("max_qi") )
	{
		if((int)obj->query("max_qi") <= 100000)
		{
