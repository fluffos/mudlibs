...
int main(object me, string arg)
{
        ...
        if( base_name(environment(ob)) != ob->query("area_file") )
        ...
        if(something=="-skl")   
        {
                ...
                if(level * 2 >= me->query_skill(skl,1) )
                        return notify_fail(ob->query("name")+"的这门武功你已经教授到顶了。\n");
                ...
                money = me->query("money")+me->query("more_money")*100000000-money;
                me->set("money",money%100000000);
                me->set("more_money",money/100000000);
                me->save();
                ob->save();
        }
        else if(something=="-exp")      {
               if((int)ob->query("combat_exp")*2>=me->query("combat_exp"))
                        return notify_fail(ob->query("name")+"的实战经验已经到顶了。\n");
               if(me->query("money")+me->query("more_money")*100000000<10000)
                        return notify_fail("你帐上的钱不够，"+ob->query("name")+
                        "的战斗经验提升1000点需要一两黄金！\n");
               money = me->query("money")+me->query("more_money")*100000000-10000;
               me->set("money",money%100000000);
               me->set("more_money",money/100000000);
               me->save();
               ...
               me->save();
               money=random(5);
               if((int)ob->query("zhongcheng")+money>=(int)ob->query("max_zhongcheng"))
                   ob->set("zhongcheng",(int)ob->query("max_zhongcheng"));
               else    ob->add("zhongcheng",money);
                ob->save();
                write("你花了一两黄金，将"+ob->query("name")+
                "的忠诚度提升了"+chinese_number(money)+"点！\n");
        }
        return 1;
}
