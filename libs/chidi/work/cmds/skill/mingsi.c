...
int main(object me, string arg)
{
        ...
        if( !arg
        ||      !sscanf(arg, "%d", jing_cost) )
                return notify_fail("你要花多少精神冥思？\n");

        if( jing_cost <= 0 && me->query_temp("pending/meditating") ){
//                remove_call_out("exec_int");
        ...
        if( jing_cost < 20 ) return notify_fail("你最少要花 20 点「精神」才能冥思。\n");
