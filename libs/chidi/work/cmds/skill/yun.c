...
int main(object me, string arg)
{
        ...
        if( environment(me)->query("no_fight") 
         && arg!="recover" && arg!="regenerate"
         && arg!="heal" && arg!="jingheal")
        ...
        {
         if(userp(env[i]) && env[i]->query("age")<20
            && env[i]->query("id") != me->query("id"))
         return notify_fail("注意，边上还有孩子呢！\n"); 

         if(objectp(env[i]) && env[i]->query("is_quest")
            && env[i]->query("killer")!=me->query("id"))
         return notify_fail(
           "嘘！边上好象有江湖人物，还是不要轻易惹祸的好！\n");           
        }
...
*/
   ...
   && (int)me->query_skill(force,1) > (int)me->query_skill("force",1))
                      //原来的逻辑判断有错误。
                me->improve_skill(force,random(me->query_con())/4+1,0);
            else
                me->improve_skill(force,random(me->query_con())/4+1,1);
                        return 1;
