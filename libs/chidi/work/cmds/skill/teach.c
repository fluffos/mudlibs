...
int main(object me, string arg)
{
        ...
        object ob;
        int student_skill, my_skill, gin_cost, grade, student_combat_exp,pot;

        if (me->is_busy())
                return notify_fail("你现在正忙着呢。\n");

        if(!arg || (sscanf(arg, "%s %s with %d", student, skill,pot)!=3 ))
      return notify_fail("指令格式：teach <某人> <技能> [with <点数>]\n"); 
        if( sscanf(arg, "%s %s", student, skill)!=2 )
        return notify_fail("指令格式：teach <某人> <技能> [with <点数>]\n"); 
        ...
        if( student != me->query("couple/couple_id") 
            && ob->query("id") != me->query("couple/child")    ) {
            if ((string)me->query("gender") == "女性") 
                return notify_fail("他不是你的丈夫或孩子。\n");
             else return notify_fail("她不是你的妻子或孩子。\n"); }
        ...
        if( ( userp(ob) && !SKILL_D(skill)->valid_learn(ob) )
            || (userp(ob) && SKILL_D(skill)->valid_enable("force")) ) 
           return notify_fail("依" + ob->name() + "目前的能力，没有办法学习这种技能。\n");

        if( my_skill >= 1000) 
             return  notify_fail("这项技能你只能教到这个程度了。\n");
        ...
        if( (int)ob->query("potential")+pot > (int)ob->query("potential") )
        ...
        if((int)ob->query("jing") > gin_cost )
        {
          if( (string)SKILL_D(skill)->type()=="martial"
           ...
           message_vision("也许是缺乏实战经验，$N对$n的解释总是无法领会。\n",ob,me);
        } else
        {
          if(skill_name = SKILL_D(skill)->query_skill_name(student_skill))
            ...
            tell_object(me,ob->query("name")+"听了你的指导，似乎有些心得。\n");
            me->add("potential", -pot);
            ob->add("potential", -pot);

            if ( grade < 1 )
                                grade = 1;
            ob->improve_skill(skill,pot*grade*(10+random(me->query("int")-9)));
            }
        }
        else
        {
