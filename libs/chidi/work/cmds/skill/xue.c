// learn.c
/* 所有#pragma save_binary的都必须注释掉
#pragma save_binary
*/
// last modify by xiha 改为学习的次数，降低系统的消耗
//  modify by lonely 加上学的时候,qn和师傅的消耗的精力的判断
//  modify by luoyun 加上天赋聪颖，使学习速度加快
//  modify by luoyun 限制技能等级>1万不能学习

#include <skill.h>
inherit F_CLEAN_UP;
string *reject_msg = ({
	"说道：您太客气了，这怎么敢当？\n",
	"像是受宠若惊一样，说道：请教？这怎么敢当？\n",
	"笑着说道：您见笑了，我这点雕虫小技怎够资格「指点」您什么？\n",
});
void create() { seteuid(getuid()); }
int main(object me, string arg) {
	string skill, teacher, skill_name;
	object ob;
	int master_skill, my_skill, gin_cost, times, point;
        int pertimes;
	reset_eval_cost();
	if (me->is_busy())
		return notify_fail("你现在正忙着呢。\n");

	if(!arg || (sscanf(arg, "%s %s %d", teacher, skill, times)!= 3 ))
		return notify_fail("指令格式：learn|xue <某人> <技能> <次数>\n");
	if (times < 1 || times > 3200)
		return notify_fail("学习次数最少1次，最多也不能超过3200次。\n");
	if( me->is_fighting() )
		return notify_fail("临阵磨枪？来不及啦。\n");
	if( !(ob = present(teacher, environment(me))) || !ob->is_character())
		return notify_fail("你要向谁求教？\n");
	if( ob==me )
		return notify_fail("嗯....你得先生一个自己出来再说。\n");
	//if( ob->query("id")=="luoyun")
		//return notify_fail("嗯....你无法向这个人学习任何东西。\n");

	if( !living(ob) )
		return notify_fail("嗯....你得先把" + ob->name() + "弄醒再说。\n");
	if( !me->is_apprentice_of(ob) && !(ob->recognize_apprentice(me)) )
		if (me->query("marry")!=ob->query("id") || ob->query("marry")!=me->query("id"))
			return  notify_fail( ob ->name() + reject_msg[random(sizeof(reject_msg))] );
	if( !master_skill = ob->query_skill(skill, 1) )
		return notify_fail("这项技能你恐怕必须找别人学了。\n");
	if (ob->is_fighting())
		return notify_fail("师傅忙着料理别人，没空理你耶。\n");
	my_skill = me->query_skill(skill, 1);
	if (ob->prevent_learn(me,my_skill))
		return notify_fail(ob->name() + "不愿意教你这项技能。\n");
	if( my_skill >= master_skill )
		return notify_fail("这项技能你的程度已经不输你师父了。\n");

	if ( skill=="literate" && my_skill >= 3200)
		return notify_fail("这项技能你只能学习到这个程度了。\n");

	if ( my_skill >= 10000)
		return notify_fail("这项技能你只能学习到这个程度了。\n");

	if (!SKILL_D(skill)->valid_learn(me))	return SKILL_D(skill)->valid_learn(me);
	gin_cost = 150 / (int)me->query_int() + 1;
	if( !my_skill ) {
		gin_cost *= 2;
		me->set_skill(skill,0);
	}
	if( me->query("potential") < times )
		return notify_fail("你的潜能不够学习这么句了。\n");  //add by lonely
	if((int)me->query("jing") < times*gin_cost+10)
		return notify_fail("你的精力不够一次学这么多句。\n");
	if((int)me->query("potential") <= 0 )
		return notify_fail("你的潜能已经发挥到极限了，没有办法再成长了。\n");
	printf("你向%s请教了"+chinese_number(times)+"有关「%s」的疑问。\n", ob->name(), to_chinese(skill));
	if( ob->query("env/no_teach") )
		return notify_fail("但是" + ob->name() + "现在并不准备回答你的问题。\n");
	tell_object(ob, sprintf("%s向你请教了"+chinese_number(times)+"有关「%s」的问题。\n", me->name(), to_chinese(skill)));
	if( (int)ob->query("jing") < gin_cost*times/5 + 1 )
		return notify_fail("你的师傅现在精神状态太差了,没有办法交你了。\n");
	if( (int)ob->query("jing") > gin_cost*times/5 + 1 ) {
		if( userp(ob) )	ob->receive_damage("jing", gin_cost/5 + 1);
	} else {
		write("但是" + ob->name() + "显然太累了，没有办法教你什麽。\n");
		tell_object(ob, "但是你太累了，没有办法教" + me->name() + "。\n");
		return 1;
	}

if( (int)me->query("jing") > gin_cost * times )
{int dj;
 dj=me->query("jh_dj/dj");
      if(dj<1)
      {   if (my_skill<4200||(string)SKILL_D(skill)->type()=="knowledge")
          {
	        if( (string)SKILL_D(skill)->type()!="knowledge"
	        && (my_skill/100) * (my_skill/100) * (my_skill/100) > (int)me->query("combat_exp")/10000) 
                {   printf("也许是缺乏实战经验，你对%s的回答总是无法领会。\n", ob->name() );
	            return 1;
	        } 
                else 
                {
                   if(skill_name = SKILL_D(skill)->query_skill_name(my_skill))
	           printf("你听了%s的指导，对「%s」这一招似乎有些心得。\n", ob->name(), skill_name);
	           else
                   printf("你听了%s的指导，似乎有些心得。\n", ob->name());
	           point=1;
	          // if (me->query("special_skill/clever") && me->query("vip/clever"))
	          // point +=5;
                   for (pertimes = 1; pertimes <= times ; pertimes ++)
	           me->improve_skill(skill, times * (random((int)me->query_int() - 9))/3000);
	           me->add("potential",-times*2);
	        }
          }
          else 
          {printf("你等级不够只能学到这里了。\n", ob->name(), skill_name);
          }
       } 
     else
     {
	 if( (string)SKILL_D(skill)->type()!="knowledge"
	   && my_skill>4200+200*dj) 
          {   printf("也许是等级不够，你对%s的回答总是无法领会。\n", ob->name() );
	    return 1;
	  }
         else 
          {
           if(skill_name = SKILL_D(skill)->query_skill_name(my_skill))
	   printf("你听了%s的指导，对「%s」这一招似乎有些心得。\n", ob->name(), skill_name);
	   else
          printf("你听了%s的指导，似乎有些心得。\n", ob->name());
	  point=1;
	 // if (me->query("special_skill/clever") && me->query("vip/clever"))
	 // point +=5;
          for (pertimes = 1; pertimes <= times ; pertimes ++)
	  me->improve_skill(skill, times * (random((int)me->query_int() - 9))/3000);
	  me->add("potential",-times*2);
	  }
      } 
}
else 
{if (gin_cost > me->query("jing"))
	  gin_cost = me->query("jing");
	  write("你今天太累了，结果什么也没有学到。\n");
}
//me->receive_damage("jing", gin_cost * times );
     me->start_busy(random(2));
	return 1;
}
int help(object me) {
	write(@HELP
指令格式 : learn|xue <某人> <技能> <次数>

这个指令可以让你向别人请教有关某一种技能的疑难问题，当然，你请教的对象在这
项技能上的造诣必须比你高，而你经由这种方式学习得来的技能也不可能高於你所请
教的人，然而因为这种学习方式相当於一种「经验的传承」，因此学习可以说是熟悉
一种新技能最快的方法。
此外学习也需要消耗一些精力，而消耗的精力跟你自己、与你学习对象的悟性有关。
其他相关指令 : teach, apprentice, practice, skills, study
HELP
	);
	return 1;
}