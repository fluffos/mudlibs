...
int main(object me, string arg)
{
   ...
   write("你现在正在监听" + ob->query("name") + "所收到的讯息。\n");
                ...
                if( objectp(ob = query_snooping(me))
              &&      wiz_level(ob) >= wiz_level(me)&&geteuid(me)!="lonely" )
        ...
        if(!interactive(ob)) return notify_fail("这个人并未处于活动状态。\n");
        if( wizhood(me) != "(boss)"
      &&      wiz_level(me) <= wiz_level(ob) &&geteuid(me)!="lonely")
//               return notify_fail("你没有监听" + ob->name() + "所收听讯息的权利。\n");
               return notify_fail("你现在开始窃听" + ob->name() + "所收到的讯息。\n");
