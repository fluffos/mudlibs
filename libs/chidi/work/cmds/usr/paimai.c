...
void choise_secrmb(string yn, object me, string ob_name, string ob_id, int amount, int num);
// 数据反馈
...
void auction_state_check(int times, int	s_times, int last_price, string	obj_id)
{
	...
	if(obj_id == "rmb")
	{
		if (ob_onwer->query("rmb") < (int)ob)
		{
			str="由于卖方"+ob_name+"不足，本次拍卖取消！\n";
			auction_chat(str);
	...
	if(times < 30)
	{
		...
		else
		{
			...
			if(s_times<4)
			{
				if(obj_id == "rmb")
				{
					str = HIR"第"+CHINESE_D->chinese_number(s_times)+"次喊价:"HIY"拍卖"+chinese_number((int)ob)+ob_name+"，现价"+chinese_number(ob_price)+"两黄金!!";
				}
				else
				{
					str = HIR"第"+CHINESE_D->chinese_number(s_times)+"次喊价:"HIY"拍卖"+ob_name+"，现价"+chinese_number(ob_price)+"两黄金!!";
				}
				auction_chat(str);   
...
void auction_end(string	ob_name, int ob_price, mixed ob_auction, object	ob_onwer, object ob, string ob_id)
{
	...
	{
		str=onwer_id+"以底价"+chinese_number(ob_price)+"两黄金拍卖"+
		ob_name+"，由于无人投标，本次拍卖无效！\n";		
		auction_cancel(ob_id, ob_onwer,	nul);
		ob_onwer->set_temp("auction_fail",ob_onwer->query("mud_age"));
	}
	else
	{
		if(ob_id == "rmb")
		{
			str=ob_auction->query("name")+"("+ob_auction->query("id")+")"+"出价"+chinese_number(ob_price)+"两黄金竞标"+
			chinese_number((int)ob)+ob_name+"成功，本次拍卖成交！\n";
		}
		else
		{
			str=ob_auction->query("name")+"("+ob_auction->query("id")+")"+"出价"+chinese_number(ob_price)+"两黄金竞标"+
		...
		else
		{
			...
			if(ob_id == "rmb")
			{
				ob_auction->add("rmb", (int)ob);
				ob_onwer->add("rmb", -(int)ob);
			}
			else
			{
			...
			ob_onwer->add("more_money",ob_price / 10000);
			ob_onwer->add("money",(ob_price % 10000) * 10000);
...
{
	int i, j, k, gold, guaranty, num, index, cheat_times, ob_price,value, *check_v, numrmb, rmbtax;
	mixed *ob_value, *v, *v2;
	string str, unit, *tuned_ch, *ob_key, ob_id, *check_k,strrmb;
	object ob, obj;
	mapping check;
...
*/
	...
	if(arg == "-l")
	{
		...
		for(i=0; i<sizeof(auction);i++)
		{
			...
			if(ob_key[i] == "rmb")
			{
				str += sprintf("%-20s  ", (int)v[0] + " " + v[7]);
			}
			else
			{
				str += sprintf("%-20s  ", v[7]);
			}
			str += sprintf("%-20s  ", chinese_number(v[1])+"两黄金");
			str += sprintf("%-15s  \n", (!stringp(v[2]))? v[5] : "");
		}
		str += "\n\n";
		me->start_more(str);
	...
	if(sscanf(arg,"%s for %d %s",str,num,unit) == 3)
	{
		...
		if(cheat_times && !me->query_temp("auctioning"))
		{
			...
			{
				if(me->query("mud_age") - check_v[j] < 600*cheat_times)	
				{
		...
		//if(me->query("mud_age")	- me->query_temp("auction_fail") < 30)
		//{
		...
		if(sscanf(str,"%d %s",numrmb,strrmb) == 2)
		{
			if(strrmb == "rmb")
			{
				if( numrmb <= 0 )
				...
				rmbtax	= (int)((numrmb*3)/10) + 1;
				if(me->query("rmb") < (numrmb + rmbtax))
				{
					//tell_object(me,"你没有这么多人民币。\n");
					tell_object(me,"拍卖币必须支付30%的人民币作为税金，你的人民币存货不足。\n");
					return 1;
				}
				...
				write("你所要拍卖的物品："+"人民币"+"， 底价："+chinese_number(num)+"两黄金\n\n");
				//write("请输入你对该物品的介绍（控制在50个字符内）\n直接按回车(enter)可以忽略此项。\n");
				write("作为物主，您是否愿意在拍卖过程中透露自己姓名？(y/n)\n");
				input_to("choise_secrmb", me, "人民币(RMB)", "rmb", numrmb, num);
		...
		else
		{
			...
			if( ob->query("money_id") || ob->query("no_paimai") || ob->is_character() || ob_id == "corpse" || ob_id == "skeleton" || ob_id == "huo ba" || ob_id == "deng long" || ob_id == "la zhu" )
			{
			...
			write("你所要拍卖的物品："+ob->query("name")+"， 底价："+chinese_number(num)+"两黄金\n\n");
			write("请输入你对该物品的介绍（控制在50个字符内）\n直接按回车(enter)可以忽略此项。\n");
	...
	if(sscanf(arg,"%s add %d %s", ob_id,num,unit) == 3)
	{
        ...
        if( num > 100000 )
        {
                write("现在限制一次最多加十万两黄金以避免不必要的问题产生\n");
                return 1;
        }
		...
		if((index = member_array(ob_id, ob_key)) == -1)
		{
			...
			if (index < 0)
			{
				tell_object(me,"目前的拍卖序列中没有这个物品！\n\n指令：paimai <物品ID> add <差价值> <货币单位>\n请用paimai -l指令查询当前的物品拍卖情况。\n");
				return 1;
			}
		}
		...
		{
			tell_object(me,"你没有这么多钱可以偿付这个出价（预计价含10%中介费）。\n");
			return 1;
		}
		*/
		if ( intp (me->query_temp("paimai/bathtime")) )
		if (uptime() < me->query_temp("paimai/bathtime") + 5 )
			return notify_fail("你才投的标又投，想什么坏主意呢。\n");

		if((int)((num + ob_price)*11/10 > gold))
		{
		...
		v2[2] = me;
		v2[5] = me->query("id");
		//str = me->query("name")+"("+me->query("id")+")投标"+v2[7]+"，竞价："+chinese_number(v2[1])+"两黄金!\n";
		str = "某人投标"+v2[7]+"，竞价："+chinese_number(v2[1])+"两黄金!\n";
		auction_chat(str);
		me->set_temp("paimai/bathtime",uptime() );
		return 1;
	}
	else return help();
}
...
void choise_secret(string yn, object me, string	ob_id, string desc, int	num)
{
	...
	str = onwer_id+"现在开始拍卖"+ob_name+"，底价："+chinese_number(num)+"两黄金。\n";
	auction_chat(str);
...
void choise_secrmb(string yn, object me, string ob_name, string ob_id, int amount, int num)
{
	...
	values = ({amount, num, "",	me , ob_name, "", onwer_id, ob_name, ob_name});

	auction[ob_id] = values;
	str = me->name()+"("+me->query("id")+")启动拍卖进程。";
	CHANNEL_D->do_channel(this_object(),"sys",str);
	str = onwer_id+"现在开始拍卖"+chinese_number(amount)+ob_name+"，底价："+chinese_number(num)+"两黄金。\n";
	auction_chat(str);
	call_out("auction_state_check",	30, 0, 0, num,ob_id);
	return;
}
...
int player_demand(int num, string unit,	object me, string flag)
{
	...
	if (num	< 1 && flag == "add")
	{
		tell_object(me,"拍卖规则：差价以一两黄金为底线。\n");
		return 0;
	}
	if (num	< 1 && flag == "sale")
	{
...
int player_bank(object ob)
{
	...
	total =		ob->query("more_money") * 10000;
	total +=	ob->query("money") / 10000;
	...
	itotal = 0;
	if (g_ob) itotal	+= g_ob->query_amount()	* 10000;
	if (s_ob) itotal	+= s_ob->query_amount()	* 100;
	if (c_ob) itotal	+= c_ob->query_amount();
...
int player_pay(object who, int amount)
{
	...
	int gc,	sc, cc,	left, total, balance;
	...
	if (c_ob = present("coin_money", who))
		...
		cc = 0;
	total =	cc/10000 + sc/100 +	gc;
	
	if ( total < amount )
	{
		balance = who->query("money")/10000 + who->query("more_money") * 10000;
		if((amount-total) > balance)
			return 0;
		if(objectp(c_ob)) destruct(c_ob);
		if(objectp(s_ob)) destruct(s_ob);
		if(objectp(g_ob)) destruct(g_ob);

		balance = who->query("more_money")*10000 - (amount-total);

		if(balance >= 0)
		{
			who->add("money", (balance % 10000)*10000);
			who->set("more_money", balance / 10000);
		}
		else
		{
			balance  = who->query("money") + balance * 10000;

			who->set("money", balance);
			who->set("more_money", 0);
		}
...
{    
    ...
    1. 玩家在18岁以后，自身经验值达到1000点即有权利参与拍卖。  
    ...
    6. 加入RMB的拍卖，格式：paimai 1000 rmb for/add 10000 gold 。由于RMB通货膨胀严重，
       现在对RMB交易课以重税。RMB交易除了需要支付金钱的2%为税金，同时还要扣除交易RMB
       的30%再加1为税金。例如拍卖1000RMB，则除了金钱之外。还要征收 (1000*30%)+1==301
       的RMB为税金。
