...
int main(object me, string arg)
{
    ...
    wiz_only = ({"invisibility", "immortal"});
		...
		if(arg == "no_auto_up")
		{
			if(!me->query("no_auto_up"))
			{
				me->set("no_auto_up",1);
				return notify_fail(HIG"设置为不自动升级。\n"NOR);
			}
			else
			{
				me->delete("no_auto_up");
				return notify_fail(HIG"设置为自动升级。\n"NOR);
			}
		}
        ...
        if( data < "1" ) 
                return notify_fail("如果想删除变量请用unset指令!\n");
        if( strsrch(term, '/')!=-1 ){
        ...
        if( term && term!="" ) {
           ...
           sscanf(data, "%d", data);
       if (term=="msg_mout" || term=="msg_min"|| term=="hecheng1"|| term=="hecheng2"){
