...
{
         string line,my_str,my_dex,my_con,my_int,my_kar,my_per,skill_type,master,me_str,me_int,me_con,me_dex,couple,killer;
         mapping my,pmap;
         object ob, weapon;
...
//         seteuid(getuid(me));
 ...
 } else if (!me->query("couple/child") || !ob->query("is_baby") || me->query("couple/child") != ob->query("id") )
         ...
         line += sprintf("%-32s┃%s\n",
         "┃",
         CYN"       膂力：["NOR+my_str+CYN"/"NOR+me_str+CYN"]     悟性：["NOR+my_int+CYN"/"NOR+me_int+CYN"] "NOR
         );
         line += sprintf("%s%-17s┃%s\n",
         "┃ 头  衔 :    ",RANK_D->query_rank(ob),
         CYN"       根骨：["NOR+my_con+CYN"/"NOR+me_con+CYN"]     身法：["NOR+my_dex+CYN"/"NOR+me_dex+CYN"]"NOR
         );
        ...
        if (ob->query("shen") >0)
         line += sprintf("%s"RED"%-13s"NOR"%s"HIM"%-13s"NOR"%s"HIY"%-12s"NOR"┃\n",
         "┃ 正 气  ：    ",ob->query("shen")+"","实战经验 ：",ob->query("combat_exp")+"",
         "潜  能 ：",(int)ob->query("potential") - (int)ob->query("learned_points")+
         " ("+(1 + (int)ob->query("learned_points")) * 100 / (1 + (int)ob->query("potential"))+"%)",
         );
        else
         line += sprintf("%s"RED"%-13s"NOR"%s"HIM"%-13s"NOR"%s"HIY"%-12s"NOR"┃\n",
         "┃ 邪 气  ：    ",ob->query("shen")+"","实战经验 ：",ob->query("combat_exp")+"",
         "潜  能 ：",(int)ob->query("potential") - (int)ob->query("learned_points")+
         " ("+(1 + (int)ob->query("learned_points")) * 100 / (1 + (int)ob->query("potential"))+"%)",
         );
         line
 += sprintf("%s"RED"%-13s"NOR"%s"HIM"%-13s"NOR"%s"HIY"%-12s"NOR"┃\n",
        ...
        line
       += sprintf("%s"HIW"%-13s"HIR"%s"BLINK+HIR"%-50s"NOR"┃\n",
                 "┃浴血币 ：     ",display_attr(my["rmb"],ob->query("rmb") ),
              "转生次数 ：",display_attr(my["zhuanshi/times"],ob->query("zhuanshi/times"))
         
        
         );
		 
         line +="┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\n"NOR;
       line += sprintf(WHT+" %s在"+HIG+CHINESE_MUD_NAME+NOR+"里的游戏时间是:"+BLINK+HIY"%s "+NOR",性格是:"+BLINK+WHT"%s\n\n"+NOR,ob==me?"你":ob->name(1),FINGER_D->age_string( (int)ob->query("mud_age")),(string)ob->query("character"));
