...
{     
        ...
        output = 
BBLU+HIC"游戏名称                             国际网路位址     端口  运行时间   在线人数\n"NOR+
"───────────────────────────────────────\n";
        ...
        for(loop = 0, size = sizeof(muds); loop<size; loop++) 
        {
          ...
          mudname += undefinedp(mud_list[muds[loop]]["NAME"])?
              "":"("+mud_list[muds[loop]]["NAME"]+")";
          mudtime=mud_list[muds[loop]]["UPTIME"];
          ...
          if( !arg && mud_list[muds[loop]]["VERSION"] != MUDLIB_VERSION )
                ...
                if( htonn(lower_case(muds[loop])) == htonn(lower_case(INTERMUD_MUD_NAME)) )
                    output +=  BGRN+HIW;
                        ...
                        output +=  sprintf("%-36s %-17s%-5s%-12s  %3s",
                               mudname,
                               mud_list[muds[loop]]["HOSTADDRESS"]==0?"LOCALHOST":mud_list[muds[loop]]["HOSTADDRESS"],
                               mud_list[muds[loop]]["PORT"],
                               undefinedp(mud_list[muds[loop]]["UPTIME"])?
                                   "": mudtime[0..26],
 ...
 output+="───────────────────────────────────────\n";
 output+=sprintf("共有"WHT"%s"NOR"个站点与江湖网络互连中。",chinese_number(count));
