...
int main(object me, string arg)
{
        ...
        object gold;
        seteuid(getuid());
		return notify_fail("修城暂不开放，开放时间待定！ \n");
        if(!arg || sscanf(arg,"%s %s",dir,stitle) != 2)
        return notify_fail("指令格式：　xiucheng east 我的城池 \n");
        if(!me->query("rank")|| me->query("rank")> 7) 
        return notify_fail("你没有这种权力。\n");
   if( (string)me->query("family/family_name") !="朝廷" )
             return notify_fail("你不是朝廷命官，不可以修城！\n");
        ...
        if((int)me->query("xiucheng_room") >2)
        return notify_fail("每个人最多可以建2座城池！\n");
        env = environment(me);
       if(!env->query("owner") && (string)env->query("owner") != "public" 
        && (string)env->query("owner") != (string) me->query("id"))
        return notify_fail("你不可在这建城池！\n");
 ...
 write(HIR"注意：城池的描述请想好再写，以后不能更改，如有疑问，可以参照黑客的城池：\n"NOR);
...
// deduct the cost and go build it!
              ...
              BR_D->buildroom(me,s_title,text,dir,4);
        }
        else
        {
...
int check_legal_long(string name)
{
        int i;
        i = strlen(name);
        if( (strlen(name) < 40) || (strlen(name) > 600 ) ) {
                write("对不起，城池中文描述必须是二十到三百个中文字。\n");
                return 0;
        }
        while(i--) {
                if(name[i]==' ' || name[i] == '\n') continue;
