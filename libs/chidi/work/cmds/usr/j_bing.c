...
{
   string QINBING_FILE = "/u/luoyun/qingbing.c";
   object qinbing;
   int max_bing = 0;
   int current_bing = 0;
   ...
   {//let wiz find all qinbing copies.
     ...
     if(me->query("rank") > 3||!me->query("rank")||(string)me->query("family/family_name") !="朝廷") 
   ...
   else if(me->query("combat_exp") >= 20000000)
   {
     max_bing = 1;
   }
   else max_bing = 0;
