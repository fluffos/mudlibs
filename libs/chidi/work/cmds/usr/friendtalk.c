...
int main(object me,string arg)
{
        ...
        if(!stringp(outarg = arg))
                outarg = "．．．．．";  
        message("vision",BOLD "【好友频道】" +me->query("name") + "(" + capitalize(me->query("id")) + ")" + 
        "：" + outarg + "\n" NOR, target);      
         tell_object(me,BOLD "【好友频道】" +me->query("name") + "(" + capitalize(me->query("id")) + ")" +
"：" + outarg + "\n" NOR);
        return 1;
}
