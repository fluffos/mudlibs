...
*/
        ...
        if (ob->query("id")=="pyter"||ob->query("id")=="zoujun") return notify_fail("咦... 有这个人吗?\n");
                if (    !living(ob)
                ||      !ob->is_character()
                ||      ob==me )
...
sprintf("%s(%s) kickout %s(%s) on %s\n",
me->name(1), geteuid(me), ob->query("name"),geteuid(ob),ctime(time()) ));
//	seteuid(getuid(ob));
//	command("quit");
//me->save();
	seteuid(ROOT_UID);
