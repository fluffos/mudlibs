#include <ansi.h>;
inherit "/quest/wudang/npcskill_n";
#include <command.h>;
#include "name.h";


int other_qi=0, other_jing=0;
...
void die ()
{
    int pot,exp,shen,max_qi,ratio;
    object killerob,me,jinyong,who;
object questob;
    me=this_object();
    who=this_player();
    max_qi=query("max_qi");
    ...
    questob=(object)me->query_temp("player1"); 
if (objectp(questob))
     questob->set_temp("kill",0);
        
        if(questob->query_temp("song_job")==1)
          { 
               message_vision("$N惨叫一声，死了。\n",this_object());
                     ...
                     questob->add("shen",shen);
                     questob->apply_condition("questbusytime",4+random(4));
                     questob->add("potential",pot);
                     tell_object(questob,"你得到了\n"+HIW+
chinese_number(exp)+NOR"点经验和"+HIW+chinese_number(pot)+NOR"点潜能以及"+HIW+chinese_number(shen)+"点正气!\n");    
