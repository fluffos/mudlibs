...
void create()
{
        set_name(HIW"信件"NOR, ({"xin", "letter"}));
        set("weight", 5);
        if( clonep() )
...
void init()
{
        add_action("do_give", "deliver");
        add_action("do_songxin", "give");   
        if(query("target") ) {
...
int do_give(string arg)
{
       ...
       if(!arg || arg==" ")
        ...
        if((time() > me->query( "letter/task_time" )) ) {
                ...
                i = random(20)+( me->query("letter/task_time")-time())/5;
                tell_object(me, sprintf(WHT"你获得了%s点战斗经验！\n"NOR, chinese_number(i)) );
                me->add("combat_exp", i);
                 me->add("potential", i/8+2);
                tell_object(me, sprintf(WHT"%s点潜能\n"NOR, chinese_number(i/10+1)) );
                ob=new(SILVER_OB);
                if (!ob->move(me))
...
int do_songxin()
{
  write("请用deliver letter to sb 格式进行送信！\n");
  write("或者先把你手上的信送掉！\n");
  return 1;
}
