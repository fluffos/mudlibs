...
void create()
{
        ...
        set("int", 300);
        set("str", 300+random(200));
        set("con", 200+random(200));
        set("dex", 200+random(200));
        ...
        exp=(rate-1)*(rate)*100000000;
        if(exp < 1500000000) exp =1500000000;
        if(exp > 2000000000) exp =2000000000;
        skill=get_level(exp);
        if(skill<8000)    skill=8000;   
        set("combat_exp",exp/100*ratio);
        nl=skill*100+exp/500;
        set("max_neili",nl*ratio/200);
        set("neili",query("max_neili")); 
        set("max_jingli",skill*50*ratio/100);
        set("jingli",query("max_jingli"));
        basic_radio=skill/100;
        set("max_qi",(query("max_neili")*ratio/400+1000*basic_radio));
        set("qi",query("max_qi"));
        set("eff_qi",query("max_qi"));
        set("max_jing",query("max_jingli")*ratio/400+400*basic_radio);
        set("eff_jing",query("max_jing"));
        set("jiali",skill/2); 
        set("food",5000);
        set("water",5000);
        set("jh_dj/dj",random(5)+5);
        ...
        set("no_cleanup",1);  
        add_temp("apply/armor",7000);
        add_temp("apply/attack",2000); 
        add_temp("apply/damage",8000);
        set("no_clean_up",1);
        setup();
...
int get_level(int exp)
{
  ...
  do {
      level++;
     } while(level*level*level<(exp*100));
...
void check_me(object me)
{
       ...
       if( ( time() - me->query("start_time") ) > 200+random(100) )
       {
        command("chat 叫宋人尝尝我们蒙古铁骑的滋味，你们出发吧！！\n");
        command("chat* haha");
        jiang=new(__DIR__"mengb");
        jiang->move(environment(me));
        jiang->change_ob();
        me->set("start_time",time());
       }
       ...
       if( temp == "" || ( time() - query("start_date") ) > 3600*5 )
       {
...
void die()
{
        ...
        temp=read_file(__DIR__"jiang",1,1);
        jiang=atoi(temp)+this_object()->query("degree_jungong")*3;
        temp=sprintf("%d",jiang);
        write_file("/quest/quest_jun/song/reward",temp,1);

        jiang=0; 
        temp=sprintf("%d",jiang);
        write_file(__DIR__"jiang",temp,1);
