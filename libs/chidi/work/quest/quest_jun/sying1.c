...
void init()
{
     ...
     if( me->query("id") == temp )
     {
      message_vision(HIW"帐中的将士看见$N走了进来，连忙"
      +"将身体站得笔直，尊敬地向$N行礼致敬！\n"NOR,me);
...
int do_assign(string arg)
{
     ...
     if( sizeof(jiang) > 2*me->query("degree_jungong") && flag )
     ...
     if( ob->query("combat_exp") < 1000000 )
      return notify_fail("对方的经验才能太低了，难堪大用！\n");     
     ...
     switch(what)
     {
      ...
      {
      ling=new(__DIR__"song/obj/lingjian");
     ...
     { 
      n=sizeof(jiang)+1;
      me->set("jun_quest/jiang/jiang"+n,ob);
     }
...
int do_dianjiang()
{
     ...
     message_vision(HIR"$N召来传令兵，下令道：火速传令各营统领"
     +"到本帅帐前候令！！\n",me);
