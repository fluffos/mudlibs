...
string ask_chubing()
{
       object me=this_player(),ob=this_object(),bingfu,kaijia;
       object oldmaster;
       string temp;
       int liangcao,i,reward;

       if( me->query("combat_exp") >= 2100000000 )//经验上限21亿
       return "大人已经是誉满天下，您就不用再操心了？！\n";
       
       if( me->query("combat_exp") < 1500000000 )//经验下限8亿
       return "大人已虽说是名声在外，不过在我看来好像经验不够？！\n";
       
       if (me->query("zhuanshi/times") < 5)
              return ("你还没有到达五转，不能领取任务！\n");
       if (me->query("jh_dj/dj") < 50)
              return ("你还没有到达50级，不能领取任务！\n");
      ...
      if( me->query_temp("ask_chubing") )                      //防止刷可汗造成callouts过多
         return "大人稍安勿躁，"
                +"你刚班师回朝，请耐心恭候圣上旨意吧！\n";

       temp=read_file("/quest/quest_jun/song/master",1,1);
       oldmaster = find_player(temp);                           //查找oldmaster是否在线
       if(!oldmaster){                                          //如果不在线就取消master资格                       
          /* message("channel:rumor",MAG"【大宋京城】"HIY"奉天承命，皇帝诏曰：\n"
           +HIY"抗蒙大元帅不知所踪，延误战机，即刻削去军职。"+HIY"钦此！\n\n"NOR,users());*/
           write_file("/quest/quest_jun/song/master","",1);    
       }
       else                                                     //如果在线就判断oldmaster的jun_quest
      {
          if (oldmaster->query_temp("jun_quest"))               //有此属性不取消master资格            
         {  
            return "现在已经有人率军出征了，大人可以前去助阵。\n";
          }
       else {                                                   //判断到无此属性就取消master资格
         /* message("channel:rumor",MAG"【大宋京城】"HIY"奉天承命，皇帝诏曰：\n"
          +HIY"抗蒙大元帅贪生怕死，临阵退缩，即刻削去军职，另择良将出关抗蒙。"+HIY"钦此！\n\n"NOR,users());*/
          write_file("/quest/quest_jun/song/master","",1);    
                 }
       }
       temp=read_file("/quest/quest_jun/song/master",1,1);        
       ...
       me->set_temp("junquest_quit",1);//by luoyun
       ...
       me->apply_condition("junquest_song",1000);       
...
void quest_begin(string who,object me)
{
      ...
      +"蒙古可汗大举南侵，大宋派出\n "+who+"\n"
      +MAG" 兴师北上抗敌.....\n"NOR,users());
      call_out("add_tuibing",1800,me);//限制30分钟完成任务，由于这个任务的难度比较大，玩家一旦完成不了其他玩家将无法领取任务。
      num=0;
      
      temp=sprintf("%d",num);
      write_file("/quest/quest_jun/meng/jiang",temp,1);
      
      smen=load_object("/quest/quest_jun/syuanmen1");
      if( smen) smen->set("defence",100);
      mmen=load_object("/quest/quest_jun/myuanmen1");
      if( mmen) mmen->set("defence",50);
...
{                         
        ...
        exp=(reward/2+random(reward/2))*me->query("degree_jungong")*26000;
        //exp=reward/2*me->query("degree_jungong")*10000;
        pot=exp*3/4;
        jungong=exp/30000;
        ...
        exp=(reward/2+random(reward/2))*jiang["jiang"+i]->query("degree_jungong")*2200;
      //exp=reward/2*jiang["jiang"+i]->query("degree_jungong")*10000;
        pot=exp*3/4;
        jungong=exp/32000;
        ...
        jiang["jiang"+i]->add("money",10000000);    
        message("channel:rumor",MAG+jiang["jiang"+i]->short(1)+"\n"+HIY"抗蒙有功，赏黄金一千两，绸缎两百匹，"
        ...
        me->set("qi",me->query("eff_qi"));
        me->delete_temp("junquest_quit");//by luoyun
        write_file("/quest/quest_jun/song/master","",1);
        me->set_temp("ask_chubing",1);
        call_out("add_chubing",10,me);
        return;
}
                 
void add_tuibing(object me,int reward)
{                         
        int pot,exp,jungong,i;
        mapping jiang;
       ...
       else                                                     //如果在线就对oldmaster做出惩罚
      {
        ...
        exp=me->query("degree_jungong")/40000;
        pot=exp/8;
        if (pot>me->query("potential"))
        ...
        me->add("combat_exp",-exp);
        me->add("potential",-pot);
        me->add("money",-5000000);        
        message("channel:rumor",MAG"【大宋京城】"HIY"奉天承命，皇帝诏曰：\n"
        +me->short(1)+"\n"+HIY"抗蒙失败，扣俸禄黄金五百两，"
        +" 削军功 "HIR+chinese_number(jungong)
        +HIY" 策，钦此！\n\n"NOR,users());       

        tell_object(me,HIM"你被扣除了"+HIR+chinese_number(exp)
        +HIW"点经验和"+HIR+chinese_number(pot)+HIW"点潜能处罚！！\n"NOR);
        ...
        {
        if( !objectp(jiang["jiang"+i]) || !userp(jiang["jiang"+i])) continue;
        exp=jiang["jiang"+i]->query("degree_jungong")/38000;
        pot=exp/10;
        if (pot>jiang["jiang"+i]->query("potential"))
        pot=jiang["jiang"+i]->query("potential");        
        jungong=exp/32000;
        jiang["jiang"+i]->add("guo_shoucheng/reward_exp",-exp);
        jiang["jiang"+i]->add("guo_shoucheng/reward_potential",-pot);
        jiang["jiang"+i]->add("guo_shoucheng/reward",-jungong);
        jiang["jiang"+i]->add("combat_exp",-exp);
        jiang["jiang"+i]->add("potential",-pot);
        jiang["jiang"+i]->add("money",-5000000);    
        message("channel:rumor",MAG+jiang["jiang"+i]->short(1)+"\n"+HIY"抗蒙失败，扣俸禄黄金五百两，"
        +" 削军功 "HIR+chinese_number(jungong)+HIY" 策，钦此！\n\n"NOR,users()); 
        jiang["jiang"+i]->delete("party");
        tell_object(jiang["jiang"+i],HIM"你被扣除了"+HIR+chinese_number(exp)
        +HIW"点经验和"+HIR+chinese_number(pot)+HIW"点潜能处罚！！\n"NOR);  
        }
        bingfu=present("bing fu",me);
        if( objectp(bingfu) ) destruct(bingfu);
        ...
        remove_call_out ("add_tuibing");
        me->delete_temp("jun_quest/party");        
        me->delete_temp("jun_quest");
        me->delete("party");
        me->delete("jun_quest");
        me->delete("jun_quest/jiang"); 
...
void add_chubing(object me)
{
    if (!me)    return;
    if ( me->query_temp("ask_chubing"))
    me->delete_temp("ask_chubing");
    tell_object(me,HIY"圣上旨意：你现在可以领命出兵！\n"NOR);
    return;
}
