...
void init()
{
        ...
        if( me->query("id") == ob->query("owner") )
        {
          ...
          add_action("do_kill", "liant");
          add_action("do_kill", "practice");
          add_action("do_kill", "xue");
          add_action("do_kill", "learn");
          add_action("do_kill", "lingwu");
          ...
          add_action("do_kill", "respire");
          add_action("do_kill", "touxi");
          add_action("do_kill", "ansuan");
          add_action("do_kill", "steal"); 
          add_action("do_kill", "tuna");
          add_action("do_kill", "meditate");
          add_action("do_kill", "dazuo");
          add_action("do_kill", "exercise");
        } 
}
...
void move_me(object me,object ob)
{
       ...
       else if( me->query_temp("jun_quest/move") == "backward" )
       {
          move_way=backward_way;
         if( site=="/quest/quest_jun/sying1")
         {
           ...
           liangcao=atoi(temp)+1000+random(100);
...
void train_begin(object me)
{
       ...
       if( !me->query_temp("jun_quest/train") )
       {
        ...
        tell_object(me,"你并没有发布任何训练队伍的命令！\n");
        return;
       }
       ...
       if( liangcao <= 10 )
       {
        ...
        if( objectp(master) && environment(master)
            && master->query_condition("jun_quest") )
        {
            tell_object(master,
       ...
       if(me->query_temp("jun_quest/group")>me->query("degree_jungong")*4)
       {
        remove_call_out("train_begin");
        me->delete_temp("jun_quest/train_begin");
       ...
       if( random(me->query("degree_jungong"))*5 > 1 )
       {
...
void check_me(object me)
{
       ...
       mapping exits;
       int group,num,reward,i;

       if( !objectp(me) ) return;

       temp=read_file("/quest/quest_jun/master",1,1);
       if( temp == "" )
       {
        ...
        {
        tell_object(me,
        HIR"你们的军队被击败了，真是靖康耻犹未雪，臣子恨何时灭啊！\n"NOR);
        }

        return;
       }

       group = ( me->query("eff_qi") - me->query("max_qi") ) / 1000; 
       me->set("qi",me->query("eff_qi")); 
       num = group - me->query_temp("jun_quest/group")+1;
    
       if( num > 0 && me->query_temp("jun_quest/train") )
       {
        tell_object(me,HIR"你的队伍精锐度提高了！\n"NOR);
        me->add_temp("jun_quest/group",num*2);
       }
       ...
       {
        tell_object(me,HIW"你的队伍已经损失殆尽了！\n"NOR);
...
int do_inquiry()
{
     ...
     if( !(jiang=me->query("jun_quest/jiang")) )
     return notify_fail("你现在没有任命过任何将领！\n");

     str=HIR"您帐下效力的将领共计"+chinese_number(sizeof(jiang))+"员\n"NOR;
     str=str+HIY"目前尚可调度的将领名册如下：\n"NOR;
     str=str+me->short(1)+HIW"\n        统军： "
             +chinese_number(me->query_temp("jun_quest/group"))+"营"NOR; 
     str=str+HIC"                战斗指数： "+sprintf("%d\n"NOR,
             me->query("combat_exp"));
     n=0;
     for( i=0;i<sizeof(jiang);i++)
     {
         obb=me->query("jun_quest/jiang/jiang"+(i+1));
         if( objectp(obb) )
         {
          str=str+obb->short(1)+HIW"\n        统军： "
     ...
     if( n == 0 )  str=str+MAG"你目前已经没有可用之将了！\n"NOR;
     str=str+HIB"已阵亡将领"+chinese_number(sizeof(jiang)-n)+"名\n"NOR;

     temp=read_file("/quest/quest_jun/song/liangcao",1,1);
     liangcao=atoi(temp);
     str=str+HIM"目前军中还有粮草"+chinese_number(liangcao)+"担\n"NOR;
