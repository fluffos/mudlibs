...
void create()
{
        set_name(MAG"紫晶石"NOR, ({"zijing shi", "zijing", "shi"}));
	set_weight(1);
	if( clonep() )
...
int do_combine()
{
	...
	if( (int)me->query("mana") < 2000)
	 ...
	 if ( !(b=present("huangjing shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
        if ( !(c=present("hongjing shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
        if ( !(d=present("chengjing shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
        if ( !(e=present("lanjing shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
        if ( !(f=present("lvjing shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
        if ( !(g=present("baijing shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
