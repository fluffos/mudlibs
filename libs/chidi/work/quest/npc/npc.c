...
*/
 ...
 if (inv[i]->query("no_zm")&&inv[i]->query("gold")==0&&inv[i]->query("12gong")==0)
                  { if( random(3)==0) carry_object(base_name(inv[i]))->wield();                  }else{carry_object(base_name(inv[i]))->wield();}     
              }
                else if( inv[i]->query("armor_prop") 
                &&  inv[i]->query("equipped") 
                &&  !inv[i]->query_unique() ) {
//                        carry_object(base_name(inv[i]))->wear();
                if (inv[i]->query("no_zm")&&inv[i]->query("gold")==0)
        ...
        me->set("nk_gain", 100000+random(50000));
        eff_dx = ob->query("combat_exp");
        if(eff_dx>95000000000) 
        nk_factor = 9;
        else if(eff_dx>800000000)
        nk_factor = 8;
        else if(eff_dx>600000000)
        nk_factor = 7;
        else if(eff_dx>300000000)
        nk_factor = 6;
        else if(eff_dx>100000000)
        nk_factor = 5;
        else if(eff_dx>500000000)
        nk_factor = 4;
        else if(eff_dx>200000000)
        nk_factor = 3;
        else if(eff_dx>100000000)
        nk_factor = 2;
        else nk_factor = 1;
...
void die()
{
        ...
        ob->add("combat_exp",reward*8);
        ob->add("potential", reward*4);
 //       ob->add("office_number", 1);
//        tell_object(users(),HIW"【奖励】:"HIY+ob->name()+"成功保护了门派安全，受到了门派奖励\n"NOR);
        tell_object(ob,"\n你得到了"+COMBAT_D->chinese_number(reward)+"经验和"
                + reward/2 + "点潜能！\n" );
        tell_object(ob,"你的门派忠诚度增加了。\n");
        ob->add("weiwang",10);
        MONITOR_D->report_system_object_msg(ob,"得到了"+COMBAT_D->chinese_number(reward)+"经验和"
