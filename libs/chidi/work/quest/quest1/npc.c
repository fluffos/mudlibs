...
int accept_object(object who, object ob)
{
        if (ob->query("money_id"))
        {
             ...
             {
                   message_vision("信差故意叹口气：那我可就不客气了。！\n",who);
                   return 1;
             }
             else
             {
message_vision("信差笑道：这次就这样吧！下次再来呀！\n",who);
                   who->set("quest",0);
                   who->delete("songxin");
                   if (ob = present("xin", this_player()))
                   {
                   message_vision("信差将信收了回去！\n",who);
                   destruct(ob);     
                   }
                   return 1;
             }
...
int give_quest()
{
  ...
  if(combatexp < 10000)
                {
message_vision("信差轻蔑的看了$N一眼说：你就这点本事？你可不要路也不识得！\n",me);
                  return 0;
          }
   if(combatexp > 1000000)
        {
                message_vision("信差对$N说：您已是成名人物了，这种小事怎敢劳您大驾？！\n",me);
                return 0;
        }
...
//   此处设置送信的间隔时间。如果不需等待可以将之关上！使送信可以随时送！
             if (((int)time() - (int)me->query("last_task_time")) < 5)
                 {
                   message_vision("信差对$N说：现在没信可以送，你还是等会再来吧。\n",me);
