...
//here begin to set_skill of this_object()
        ...
        set("max_qi",(me->query("max_qi")*ratio/200+5*basic_radio));
        set("qi",query("max_qi"));
        set("eff_qi",query("max_qi"));
        set("max_jing",(me->query("max_jing")*ratio/200+10*basic_radio));
        ...
        set("neili",query("max_neili")*3/20);
        set("max_jingli",me->query("max_jingli")*ratio/100);
        set("jingli",query("max_jingli"));
        set("jh_dj/dj",me->query(("jh_dj/dj")+("jh_dj/zhuan")));
        basic_radio = me->query("max_neili") / 500 ;
        set("jiali",me->query("max_neili")/(20+50*basic_radio));
        set("killer","xingyun");
        set("food",500);
        set("water",500);
        set("combat_exp",me->query("combat_exp")/200*ratio);
        set("no_clean_up",1);
        set("is_quest",1);
        set("no_guard",1);
        setup();
...
void die()
{
 ...
 /*
        if( userp(killer) && random(30) < 1 )
        { 
           dir = get_dir("/quest/quest_obj/equip/");
           file = "/quest/quest_obj/equip/"+dir[random(sizeof(dir))];
           if( file_size(file) != -1 )
           {
               equip = new(file);
               if( objectp(equip) )
...
*/
        ...
        if((int)killer->query_temp("shan_quest/count") <
         ...
         reward_exp=killer->query("combat_exp")/5000+count*100;  
         reward_rmb = killer->query_temp("bh_quest/count") + 10 + 5*random(killer->query_temp("bh_quest/count")/2+1) + random(55);
         ...
         reward_pot=reward_exp/4+random(50)+50-killer->query_int()/50;
