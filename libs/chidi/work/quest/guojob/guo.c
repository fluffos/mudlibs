...
//                 "奸细"  : (: ask_jianxi :),
        ...
        set_skill("arrow", 2000000);
        set_skill("lianzhu-arrow", 2000000);
...
string ask_shoucheng()
{
       ...
       if ( me->query("combat_exp") > 1000000000)//经验上限10亿
             return "“"+RANK_D->query_respect(me)+"你都那么厉害了，还是别领任务的为好！”\n";

       if ((int)me->query("combat_exp")<200000000)//经验下限2亿
...
string ask_fuming()
{
        ...
        {
                job_exp=((int)me->query_temp("guo_shoucheng/help_killed"))*20;
                job_pot=job_exp/2;
                job_exp=job_exp;
                me->add("combat_exp", job_exp);
                me->add("potential",job_pot);
                tell_object(me,HIW"你被奖励了"+chinese_number(job_exp)+"点经验值！及"+chinese_number(job_pot)+"点潜能！\n"NOR);
        ...
        job_exp=((int)me->query_temp("guo_shoucheng/killed_mgb"))*20;
        job_exp=job_exp+((int)me->query_temp("guo_jianlou/killed_mgb"))*20; 
        job_pot=job_exp/2;
        job_exp=job_exp;
        reward=((int)me->query_temp("guo_shoucheng/killed_mgb")
               + (int)me->query_temp("guo_jianlou/killed_mgb"))/2+1;

        tell_object(me, HIW"你被奖励了"+chinese_number(job_exp)+"点经验值及"+chinese_number(job_pot)+"点潜能！\n"NOR);
                //me->apply_condition("guojob2_busy",2);
        me->add("combat_exp", job_exp);
        me->add("potential",job_pot);
        me->add("guo_shoucheng/reward_exp", job_exp);
...
string ask_chetui()
{
         ...
         me->add("combat_exp",-1000+random(2000));
        me->apply_condition("jobshadi_failed",4);
         me->delete_temp("guo_shoucheng");
         me->delete("guo_shoucheng/failed");
         return "没关系,下次继续努力 。";
...
string ask_arrow()
{
      ...
      if ( me->query("guo_shoucheng/reward") < 200)
      {
       message_vision(HIC"$N哈哈大笑道：“这套连珠箭法我就"
       +"传于你了，\n望你奋勇杀敌，多立战功，我也无憾了！\n"NOR,ob);
       return "时间紧迫，现在就开始学吧(qingjiao lianzhu-arrow)！\n";
      } else
      {
       message_vision(HIC"$N微微点头道：“这套连珠箭法是我的"
       +"毕生心血之作，\n不便轻易相传！！\n"NOR,ob);
...
string ask_lianzhu()
{
      ...
      if ( me->query("guo_shoucheng/reward") > 500)
      {
...
int do_study(string arg)
{
 ...
 if (arg!="arrow" && arg!="lianzhu-arrow")
     ...
     if( arg="arrow"&&arg!="lianzhu-arrow")
   {
     ...
     message_vision(HIC"$N指点了$n一些有关"HIW"「箭法」"HIC"的问题，"
                    +"$n似乎有些心得。\n"NOR,ob,me);
     me->improve_skill("arrow",me->query_int()*me->query_int()/10,0);

     return 1;
    }
     else if(  arg!="arrow"&&arg="lianzhu-arrow")
     ...
     if( me->query_skill("lianzhu-arrow",1)*me->query_skill("lianzhu-arrow",1)
        > me->query("guo_shoucheng/reward")*30 )
     ...
     message_vision(HIC"$N指点了$n一些有关"HIW"「连珠箭法」"HIC"的问题，"
                    +"$n似乎有些心得。\n"NOR,ob,me);
     me->improve_skill("lianzhu-arrow",me->query_skill("arrow",1)+me->query_int()*10,0);
