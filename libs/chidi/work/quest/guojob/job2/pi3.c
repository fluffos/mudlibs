...
void greeting(object ob)
{
        ...
        say( "裨将皱了皱眉：" + ob->query("name") 
                                + "，你还是安心守城吧。\n");
        else if( ob->query_temp("guo_shoucheng/mark_shadi") == 3
                 && !ob->query_temp("guo_shoucheng/job_over"))

        {
                say( "裨将拱手说道：" + ob->query("name") 
                      + "，你来得正是时候，蒙古靼子已经快攻上城了。\n");
...
/*
        if( this_player()->query_temp("guo_shoucheng/start_job") ) 
        {
...
*/
        if( environment(this_player())->query("short")=="朱雀内门")
        {
