//长弓 writed by xingyun 2001.10.22
...
void create()
{
        set_name(HIG"水晶连弩"NOR,({ "crystal bow","bow","gong" }) );
        set_weight(16000);
        if(clonep())
                set_default_object(__FILE__);
        else {
                set("long", "一把寒铁制的组合弓弩，威力巨大无比。\n");
                set("unit", "把");
                set("value", 80000);
                set("duration",4000);
                set("dam",1400);         
...
int do_she(string arg)
{
   ...
   if( me->query("qi") < 1000 || me->query("jing") < 250 )
   return notify_fail("你现在精气不够充盈，硬拉功怕有危险啊！\n"); 
if( !arg)
        return notify_fail("指令格式：she <方向> <距离>\n");

   if( sscanf(arg,"%s %d",what,n)==2 )
   {
...
if( ob->query("no_die")||ob->query("no_fight"))
     {
      tell_object(me,"“砰”地一声，你被"+bow->query("name")
                                  +"震了一下，原来你的目标是不死身呀！\n");
      return 1;
         ...
         || random(me->query("combat_exp")) > ob->query("combat_exp")*2 )
     {
      ...
      ob->receive_wound("qi",
          damage - (int)ob->query_temp("apply/armor"), me);
      ...
      if( !userp(ob) )
      me->improve_skill("arrow",random((damage/2+me->query_int()/10)*ratio/100),0);
      ...
      if( random(3) )
      {
       ...
       if( damage > 2000 )
        ...
        obj->receive_wound("qi",
               damage - (int)obj->query_temp("apply/armor"), me);
     ...
     bow->add("duration",-1);
     me->receive_damage("jing",100,me);
     me->receive_damage("qi",200,me);
