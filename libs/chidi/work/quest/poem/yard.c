...
void create ()
{
      set("short", "仙人诗香阁");
    set("long", @LONG
  在这个四面都挂着对联，个个都是名家手笔，在房中央放着两张大圆桌，一
  个上面放着各色茶具。一个上面摆着笔墨纸砚,一位仙姑站在边上。正笑着对着大家
   在书桌上放着写满字的纸。你不禁想看下他在写什么（what)
LONG);
...
{   object me=this_player();
    ...
    if(arg==author) { 
      ...
      if(me->query_temp("poem/wrong")>20)
        {
// poem_penalty(me);
       message_vision(HIR"仙姑对$N说道：“你再道朱喜那里去学两年再来吧”\n"NOR,me);
         message_vision(HIR"仙姑说完就将$N推了出去。\n"NOR,me);
...
void poem_reward(object me,string arg)
{
    me->set_temp("poem/wrong",0);
       message_vision(HIR"仙姑点头道："+arg+"好诗！好诗！\n"NOR,me);
    poem_reward1(me);
}
...
{  
   ...
   if(random(2)==0) {
     ...
     switch(random(2)) {
       case 0: 
                  tell_room(this_object(),CYN"仙姑道："+quest+"..."+CYN+"的后半句是什么？\n"NOR);
         break;
       case 1:
                tell_room(this_object(),CYN"仙姑说："+quest+"．．．"+CYN+"后面是什么来着？\n"NOR);
         break;
     }
   } else {
     ...
     switch(random(2)) {
       case 0:
         tell_room(this_object(),CYN"仙姑道："+quest+CYN+"的前半句是什么？\n"NOR);
         break;
       case 1:
                 tell_room(this_object(),CYN"仙姑说："+quest+CYN+"前面是什么来着？\n\n"NOR);
         break;
     }
   }
