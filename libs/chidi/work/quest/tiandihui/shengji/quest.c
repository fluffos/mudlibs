...
int find_zhiwei(int level)
{
        ...
        while(i--)
        {
                ...
                {
                        write(CYN"陈进南说道：我天地会兄弟各个尽职尽责，现在没有合适的位置给你。\n"NOR);
                        return 0;
                }
        }
        return ttang;
...
CYN"陈进南对$N"CYN"说道：你的经验太少，无法胜任"+zhiwei[level]+CYN"之职。\n"NOR,ob);
        ...
        {
                message_vision(
CYN"陈进南对$N"CYN"说道：论到贡献，我会中还有很多兄弟在你之上。我看你还无法胜任"+zhiwei[level]+CYN"一职。\n"NOR,ob);
                return 0;
        }
        return 1;
}
...
{
        message_vision(
CYN"陈进南对着$N"CYN"点了点头道：不错，我天地会兄弟中数你尽职尽责，"+tang[ttang]+zhiwei[level]+CYN"之职就由你来担任吧。\n"NOR,ob);
        ob->set("tiandihui/tang",ttang);
        ob->add("tiandihui/level",1);
        ob->add("combat_exp",10000);
        ob->set("title",HIW"天地会"+tang[ttang]+zhiwei[level]);
        if(level>3)
                ob->set("title",HIW"天地会"+zhiwei[level]);
        message_vision(
"陈进南从怀中取出一些银两说道：这些银两是你辛苦所得。\n",ob);
        MONEY_D->pay_player(ob,200000);
        return (HIW"天地会"+tang[ttang]+zhiwei[level]);
}
