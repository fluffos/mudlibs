...
void die ()
{
    int pot,exp,gg;
    int reward_rmb;
    object killerob,me,killgold;
    mixed questob;
    me=this_object();  
    
    killerob=me->query_temp("last_damage_from");

killgold=new("/clone/money/gold");
...
*/
   ...
   //reward_rmb = killer->query_temp("bh_quest/count") + 10 + 3*random(killer->query_temp("bh_quest/count")/2+1) + random(25);
    killgold->set_amount(pot/600);
    gg=killgold->query_amount();
/*************************************************/
//if (exp>MAX_EXP) exp=MAX_EXP; 
//if (pot>MAX_POT) pot=MAX_POT;
    questob->add("potential",pot);
    questob->add("combat_exp",exp);
   //questob->add("rmb",reward_rmb);

    killgold->move(questob);
    tell_object(questob,"你得到了"+HIW+
    chinese_number(exp)+NOR"点经验和"+
    HIW+chinese_number(pot)+NOR"点潜能"+
    HIW+chinese_number(gg)+NOR"两黄金\n"NOR); 
