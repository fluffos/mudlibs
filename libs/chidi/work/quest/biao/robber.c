...
*/
        ...
        //set("max_qi",(me->query("max_qi")*ratio/100+400*basic_radio));
        set("max_qi",(me->query("max_qi")/100));//杀手的气血太高了，玩家很难完成任务。by luoyun
        set("qi",query("max_qi"));
        set("eff_qi",query("max_qi"));
        //set("max_jing",(me->query("max_jing")*ratio/100+200*basic_radio));//杀手的气血太高了，玩家很难完成任务。by luoyun
        set("max_jing",(me->query("max_jing")/100));
...
void die()
{
        ...
        if(!killer1) killer1=find_living(me->query("killer")); 

        message_vision(HIC"$N惨嚎一声，口喷鲜血，到在地上死了。\n"NOR,me);
        
        if( userp(killer) && random(40) < 1 )
        { 
           dir = get_dir("/quest/biao/obj/");
           file = "/quest/biao/obj/"+dir[random(sizeof(dir))];
           if( file_size(file) != -1 )
           {
               equip = new(file);
               if( objectp(equip) )
        ...
        if(killer1)
        {
         ...
         if(reward_exp > 6000) reward_exp=6000;   
