...
{       
        seteuid(getuid());
        set_name(HIY"福威大镖车"NOR, ({ "da biaoche", "biaoche", "che"}) );
        set_weight(500000);
        set("no_get", 1);
        set("unit", "辆");
        set("value", 0);
        set("long", 
          HIY"一辆福威镖局的大镖车，上面结结实实盖着蓬布，一群镖头赶(gan)着车，牢牢地护在一边。"NOR); 
        set("owner","xingyun");
        set("parter","xingyu");
        set("owner_name","无云");
        set("no_clean_up",1);
...
{        
       ...
       {
        reward_exp1=(int)me->query("kar")*2000+(int)me->query("per")*2000;
        reward_exp1=reward_exp1+(int)me->query("quest_yunbiao/bonus")*20;
        reward_exp1=reward_exp1+(int)me->query_temp("quest_yunbiao/bandit_killed")*200;        
        if(reward_exp1>300000) reward_exp1=300000;
        reward_exp1=reward_exp1*20+random(reward_exp1*20);
        reward_pot1=reward_exp1/4;
        reward_score1=reward_exp1/40;
        reward_exp2=(int)parter->query("kar")*2000+(int)parter->query("per")*2000;
        reward_exp2=reward_exp2+(int)parter->query("quest_yunbiao/bonus")*20;
        reward_exp2=reward_exp2+(int)parter->query_temp("quest_yunbiao/bandit_killed")*200;        
        if(reward_exp2>300000) reward_exp2=300000;
        reward_exp2=reward_exp2*20+random(reward_exp2*20);
        reward_pot2=reward_exp1/4;
        reward_score2=reward_exp1/40;

message("channel:rumor",HIY"【镖局传闻】"+me->query("name")+
"率领的镖队历经千辛万苦，终于顺利抵达！\n"
+me->query("name")+"的镖队共获得了"+CHINESE_D->chinese_number(reward_exp1+reward_exp2)
+"点实战经验、"+CHINESE_D->chinese_number(reward_pot1+reward_pot2)+"点潜能\n"
+"及"+CHINESE_D->chinese_number(reward_score1+reward_score2)+"点综合评价奖励！\n"NOR,
users());
