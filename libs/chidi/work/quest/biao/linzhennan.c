...
string ask_biaodui()
{
    ...
    if( parter->query("combat_exp")<1500000000)
      return "运镖可是件危险的差使，不要让你的同伴去送死了！\n";

    if( me->query("combat_exp")>=2100000000)//经验下限15亿
      return "你的功夫已经达到颠峰，不必在去运镖了！\n";

    if( parter->query("combat_exp")>=2100000000)
      return "你的同伴功夫已经达到颠峰，不必在去运镖了！\n";
