...
void create()
{
    	set_name("分望卡", ({ "average card", "card", "average" }) );
	if( clonep())  set_default_object(__FILE__);
	set("unit", "叠");
	set("base_unit", "张");
	set("long", "梦幻时空的卡片，使用（use）可以平均分配自己与别人的社会声望。\n");
	set("value", 0 );
        set("base_weight", 3);
        set_amount(1);
...
int do_use(string arg)
{
 ...
 if( !arg)
	...
	if( !ob ) return notify_fail("现在找不到这个人！\n");
        ...
        if( me == ob )
                return notify_fail("你分你自己的东西做什么！\n");
...
*/
        ...
        me->set_temp("wqa's room/point", n);
        ob->set_temp("wqa's room/point", n);
        
        // set("name", "卡片");  set("id", "card");
        CHANNEL_D->do_channel(this_object(), "game", me->name() + "对着"
                + ob->name() + "奸笑：“你的就是我的，我的也是你的！”\n"
                "        " + me->name() + "与" + ob->name() + "平均分配" 
                + chinese_number(n*2) + "点社会声望！\n" );
