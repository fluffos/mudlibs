...
int tydamage = this_player()->query("tydamage/sword")?this_player()->query("tydamage/sword"):this_player()->query_skill("dugu-jiujian");
string *parry_msg = ({
...
([  "action" : "$N含笑抱剑，气势庄严，$w轻挥，尽融「达摩剑」为一式，闲舒地刺向$n$l",
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N神声凝重，$w上劈下切左右横扫，挟雷霆万钧之势逼往$n$l，「伏摩剑」的剑意\n表露无遗",
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "却见$N突然虚步提腰，使出酷似武当「"+BLU+"蜻蜓点水"+NOR+"」的一招攻向$n$l",
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N满场游走，东刺一剑，西刺一剑，令$n莫明其妙，分不出$N剑法的虚实，不料$n$l已然受伤"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N抱剑旋身，转到$n身后，杂乱无章地向$n$l刺出一剑，不知使的是什么剑法"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N突然一剑点向$n的$l，虽一剑却暗藏无数后着，$n手足无措，不知如何是好"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N剑挟刀势，大开大阖地乱砍一通，但招招皆击在$n的$l破绽，迫得$n不得不守"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N举剑狂挥，迅速无比地点向$n的$l，却令人看不出其所用是什么招式"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N随手一剑指向$n，落点正是$n的$l的破绽所在，端的是神妙无伦，不可思议"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N脸上突现笑容，似乎已看破$n的武功招式，胸有成竹地一剑刺向$n的$l"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([
    "action" : "$N将$w随手一摆，但见$n的$l自己向$w撞将上来，神剑之威，实人所难测"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N突然运剑如狂，一手关外的「"+NOR YEL+"乱披风剑法"+NOR+"」，猛然向$n的$l乱刺乱削"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N剑招突变，使出衡山的「"+HIC+"一剑落九雁"+NOR+"」，削向$n的左肩，怎知剑到中途，突然转\n向$n的$l，大出$n意料之外"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([
    "action" : "$N剑随身转，续而刺出十九剑，竟然是华山「"HIB"玉女十九剑"NOR"」，但奇的是这十九\n剑便如一招，直刺$n的$l而去！直是匪夷所思"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
([  "action" : "$N剑势忽缓而不疏，剑意有余而不尽，化恒山剑法为一剑，向$n的$l慢慢推去"NOR,
        "force" : 560,
        "dodge" : 280,
        "parry" : 240,
        "damage" : tydamage,
...
return 0;
 ...
 //  第六等级的伤气
      if( me->query_skill("dugu-jiujian",1)>5000
  && me->query("jh_dj/dj")>=25  ) {
              victim->receive_damage("qi", (damage_bonus ) * 2 , me);
return WHT  "$N的九剑已到最高等级，一剑攻出，剑气横飞,时若游龙穿空，时若惊鸿渡云扑向$n！\n" NOR;
  }      

 //  第五等级的伤气
      if( me->query_skill("dugu-jiujian",1)>4000
  && me->query("jh_dj/dj")>=15 ) {
              victim->receive_damage("qi", damage_bonus , me);
return YEL "$N的九剑已到第五等级，一剑随意挥出,如生了眼睛一般扑向$n！\n" NOR;
  }      
      if( damage_bonus < 100 ) return 0;

 //  第四等级的伤气
      if( me->query_skill("dugu-jiujian",1)>2000
  && me->query("jh_dj/dj")>=10 ) {
           victim->receive_damage("qi", (damage_bonus - 100) / 2 , me);
return MAG "$N的独孤九剑已到第四等级，随意一剑带出九大阵剑气扑向$n！\n" NOR;
  }      
...
if (me->query("gender") == "女性" ) 
return 0;   
    //if(( me->query_skill_mapped("force") != "zixia-shengong") && userp(me) )
     //return;
    if (skill>400&&me->query("lonely/duguqiubai")){
        enemy=me->query_enemy();
        if (!sizeof(enemy)) return;
        if (sizeof(enemy)==1 && enemy[i]->query("eff_qi")<0 && enemy[i]->query("neili")<10000) return;
        all_enemy=enemy[0]->name();
        for (i=1;i<sizeof(enemy);i++)
            all_enemy+=("、"+enemy[i]->name());
        message_vision(HIC+"$N瞬间领悟了独孤九剑的微妙之处，手中兵器光芒大盛，只见无数剑光同时射向"+all_enemy+"。\n\n" NOR,me);
        me->add("neili", -2000);
        me->set_temp("dugu-jiujian",1);
        for (i=0;i<sizeof(enemy);i++){
            for (j=0;j<(skill/(80000*sizeof(enemy))+40);j++)
