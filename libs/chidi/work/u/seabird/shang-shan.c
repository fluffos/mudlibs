...
void init()
{
    ...
    if (base_name(environment())!=this_object()->query("startroom")) return;
     ...
     if(ob->query("daoxin"))
{
  tell_object(ob, "你只不过是个残疾人,也配做当今赏善使者?\n");
return 0;
}
...
int accept_fight(object ob)
{
 ...
 if( (int)this_player()->query("combat_exp") > 500000000 || (int)ob->query("age")>=18 ||(int)ob->query("mud_age")>345600 ){
        command("say "+ob->query("name")+" 你已经不是新手了，这位"+RANK_D->query_respect(ob)+"还是去挑战其他的盟主吧！");
        return 0;
}
*/
 if(!this_player()->query_temp("agree")) { 
   ...
   this_player()->start_busy(2); 
        message("system",HIW"【赏善护法】: "+this_player()->query("id")+"正在挑战赏善使者!特此告之!!!\n"NOR,users());
...
int do_copy(object me, object ob)
{
   ...
   if(!( xinxiu = find_living("jianghu xinxiu")) )
    ...
    ob->set("party/rank",HIC+"赏善使者"+NOR);
  
    me->set("short", HIC+"第" + chinese_number(me->query("generation")) + "代赏善使者"+ NOR+" " + me->query("name") + "(Shangshan shizhe)");
    me->delete("title");
...
/* copy entire dbase values */
    ...
    newob = new("/d/taishan/npc/shang-shan");
...
int do_gengxin()
{
    ...
    command("chat 我退出江湖后，赏善使者之位将由东方日大侠暂时接任。\n");
    rm(query_save_file()+__SAVE_EXTENSION__);
    room=new("/d/taishan/npc/shang-shan");
    room->move("/d/taishan/xiayi");
        seteuid(getuid());
    destruct(me);
    destruct("winner");
         destruct("id");
