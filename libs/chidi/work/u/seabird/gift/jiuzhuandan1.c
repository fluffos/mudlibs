...
void create()
{
        set_name(NOR + HIR "九转九阳丹" NOR, ({ "jiuzhuan dan1", "dan" }));
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", WHT "这是颗天界里最为珍贵的宝物，据说可以让你学会九转残篇(10%成功率)。\n" NOR);
                set("unit", "碗");
                set("no_give", 1);
                set("no_drop", 1);
                set("no_put", 1);
                ...
                set("gift_type", "jiuzhuan-jiuyang");
                set("gift_name", "九转九阳残篇");
                set("gift_point", 10);
                set("gift_msg", HIM "突然你的脑中一阵混乱。\n" NOR);
        }
        setup();
}
...
int do_eat(string arg)
{
	 ...
	 return notify_fail("你要吃什么？\n");
       ...
       if (this_player()->query("special_skill/lucky"))
        ...
        message_vision(WHT "$N" WHT "一仰脖，喝下了一" +
                       query("unit") + name() + WHT
                       "。\n" NOR, this_player());
        ...
        } else
        {
                ...
                tell_object(this_player(), HIC "你学会了--" + query("gift_name") +
                                HIC "\n" NOR);
                ...
                // 增加相应的天赋属性
                this_player()->set_skill("jiuzhuan-jiuyang",300);
        }
