...
int accept_fight(object ob)
{
 ...
 if(!this_player()->query_temp("agree")) {
   ...
   this_player()->start_busy(2); 
        message("system",HIW"【盟主护法】: "+this_player()->query("id")+"正在挑战武林盟主!特此告之!!!\n"NOR,users());
...
/* if( (int)this_player()->query("bug")== 1){
        command("say "+ob->query("name")+" 你的id是bug出来的吧！");
        return 0;
}
*/
...
int do_recover()
{
   ...
   // return notify_fail("你的实力和现在的盟主实力差太多了吧？\n");
     //if(me->query("combat_exp",1)/2<ob->query("combat_exp",1))
  //  return notify_fail("你的实力和现在的盟主实力差太多了吧？\n");
