// combatd.c
// Create by lonely@yxcs 01/01/2002
#pragma save_binary
#include <ansi.h>
#include <skill.h>
#include <weapon.h>
...
string damage_msg(int damage, string type,object ob)
{
	string str;
	int qi=ob->query("qi");
	//if (ob->query("id")=="luoyun") return "造成 " + damage + " 点" + type + "。\n";       
...
varargs int skill_power(object ob, string skill, int usage)
{
    ...
    level = ob->query_skill(skill)*2/50;
    switch(usage) {
    ...
    if( !level || level<1){
        power=(int)ob->query("combat_exp")/200;
        if (power>10000000){
            power /= 100;
            power*=status;
        }else{
    ...
    if (usage == SKILL_USAGE_ATTACK){
        if (skill=="unarmed")
        power*=(ob->query_str()*2/150 + ob->query_temp("str")-ob->query("jiali")/50);
        else
        power*=(ob->query("str") + ob->query_temp("str")+ob->query_skill(skill,1)/200);
        power=(power/5)*2;
    }else
        power*=(ob->query_dex()*2/150 + ob->query_temp("dex"));
    ...
    if (level>30000 && power<0) return 2100000000;
    return power;
}
-------------------------------------------------------------------------------------*/
varargs int skill_power(object ob, string skill, int usage)
{
    int status, level, power,qi,jing,max_qi,max_jing;
    ...
    if (ob->query("max_qi")<1 || ob->query("max_jing")<1) return 1;
        ...
        qi=ob->query("eff_qi");
        if (qi > 21400000) qi=21400000;
        jing=ob->query("eff_jing");
        if (jing >21400000) jing=21400000;
        max_qi=ob->query("max_qi");
        if (max_qi > 21400000) max_qi=21400000;
        max_jing=ob->query("max_jing");
        if (max_jing >21400000) max_jing=21400000;

        status=qi*10/max_qi;
        status=status*jing*10/max_jing;
    }
    if (status<1)   return 1;
    level = ob->query_skill(skill)/50;
    switch(usage) {
        case SKILL_USAGE_ATTACK:
            level += ob->query_temp("apply/attack");
            if (ob->is_fighting())
    ...
    if( !level || level<1){
        power=(int)ob->query("combat_exp")/2000;
        if (power>10000000){
            power /= 1000;
            power*=status;
        }else{
    ...
    if (usage == SKILL_USAGE_ATTACK){
        if (skill=="unarmed")
        power+=(ob->query_str()/150 + ob->query_temp("str")-ob->query("jiali")/500);
        else
        power+=(ob->query("str") + ob->query_temp("str")+ob->query_skill(skill,1)/200);
        power=(power/5)*2;
    }else
        power+=(ob->query_dex()/150 + ob->query_temp("dex"));

      power += (ob->query("combat_exp")/2000);
    if (power>10000000){
        power /= 1000;
        power*=status;
    }else{
    ...
    if (level>1000000 && power<0) return 2100000000;
    return power;
}
...
// npc不可战斗区 by lonely
   ...
   return 0;
        ...
        if (!victim || !me || victim && me && environment(me)!=environment(victim) ) return 0;
       ...
       if (my["character"] == "狡黠多变") 
{
                             ...
                             victim->improve_skill("parry", random(victim->query_int())/20);
                               ...
                               else { your["combat_exp"] += victim->query_int()/40;
...
//                                damage -= damage / 3;
                                damage = damage*2/3;
//【系统出错】执行时段错误：*Too long evaluation. Execution aborted.;程式：adm/daemons/combatd.c 第 625 行
				defense_factor /= 2;
			}
			//
...
//start 用于太极拳和乾坤大挪移等反震效果 Added by lonely
    if(stringp(parry_skill)
       && SKILL_D(parry_skill)->is_reinpose(victim, me))
...
//       && victim->query("family/family_name")=="少林派"   
    ...
    if(victim->query_skill("taixuan-gong", 1) > 100
...
//     tell_object(victim,HIR"你受到创伤,你的怒气增加了！\n"NOR);
	if( victim->query("anger")>100 ) victim->set("anger",100);
       ...
       if (my["character"] == "狡黠多变") 
				...
				me->improve_skill(attack_skill, random(me->query_int()));
				if( my["potential"]  < 10*me->query_int() )
					my["potential"] += 1;
...
void killer_reward(object killer, object victim)
{
    ...
    string *skills;
    int exp, pot, score, bonus,factor,i;
...
//我这样保护新手,可能会被玩家用age<18岁的id 去pk超过１８岁的id,sigh,目前还没有想到好的办法 add by lonely
		...
		if( victim->query("thief") )
			victim->set("thief", (int)victim->query("thief") / 2);
		//鬼王做惩罚pk的杀手，增大对pk者的惩罚//add by luoyun
             if (!userp(killer) && killer->query("id")=="demogorgon"){
			all_skills=victim->query_skills();
                     skills=keys(all_skills);
                         for(i=0;i<sizeof(skills);i++)     
                     if (all_skills[skills[i]] > 100)			
			    victim->set_skill(skills[i],all_skills[skills[i]]-100);
                     if( (int)victim->query("potential") > 0)
                         victim->add("potential", - (int)victim->query("potential")/2);
                    if( (int)victim->query("combat_exp") > 0)
                         victim->add("combat_exp", -(int)victim->query("combat_exp")/2);}
              //end//add by luoyun
                else{  if( (int)victim->query("potential") > 0)
                              victim->add("potential", - (int)victim->query("potential")/8 );
