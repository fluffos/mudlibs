...
void check_user(object ob)
{
        ...
        int value,lv,value2,lv2,i;
        
     
      skl=ob->query_skills();
      sname=keys(skl);
      if (!ob->query("zhuanshi/jineng2"))
     {
      for(i=0;i<sizeof(skl);i++)     
      if (skl[sname[i]] > 100000) 
     ob->set_skill(sname[i],100000);
       }
//   temp
...
//
    ...
    if (ob->is_ghost() && !wizardp(ob)){
        ...
        ob->set("qi", 1);
        ob->set("eff_qi", 1);
        ob->move(DEATH_ROOM);//add by luoyun
    }
