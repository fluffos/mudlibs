...
int main(object me, string arg)
{
        ...
        if( lp->query("money") < 
                      rest_money+(sizeof(lp->query("area"))+1)*100000000 )
        return notify_fail("帮中现在没有这么多钱可供调用。\n");

        lp->add("money",-rest_money);
        lp->set("last_money",rest_money);
        lp->save();
        destruct(lp);
        ...
        { 
               ...
               for(k=0;k<count;k++)
               {
                  reward_money = rest_money*(bh[k]->query("banghui/rank_lv")+1);
                  ...
                  if( bh[k]->query("banghui/quest") >= 4 )
                  {
                   ...
                   reward_money = bh[k]->query("money")+reward_money;
                   if( reward_money > 100000000 )
                   {
                       bh[k]->add("more_money",reward_money/100000000);
                       bh[k]->set("money",reward_money%100000000);
                   } else
                       bh[k]->set("money",reward_money);
                }
        }
