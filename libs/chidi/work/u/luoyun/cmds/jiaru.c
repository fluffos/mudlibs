...
//         return 
//notify_fail("对不起，你已经加入了"+me->query("banghui_leader")+"的帮会。\n");
      ...
      if( ob->query_temp("yaoqing") )
         {
          if( ob->query_temp("yaoqing") == me->query("id") )
            {
             if( ! stringp(ob->query("banghui_leader")) )
	    ...
	    me->set("title", HIG"【" + ob->query("banghui") + "】"NOR + sprintf("%s第%s代弟子", family["family_name"],
