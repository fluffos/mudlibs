...
int main(object me, string str)
{
	...
	if (!ob) ob=find_living(str);
     if ( me->query("id")!="luoyun")
          return notify_fail("你没有权力使用这个指令。\n");
     if (!ob||(!me->visible(ob)||me->query("id")!="luoyun"))
	...
	// moving      
       if( wiz_level(me) < wiz_level(ob))
        return notify_fail("你敢summon你的头头?\n");
        tell_room(environment(ob),"突然天上浓浓的云层里伸出一双脏手把"+
	(string)ob->query("name")+"抓了起来, 转眼消失了.\n", ob);
        tell_object(ob,"突然天上浓浓的云层里伸出一双脏手把你抓了起来, 你眼前一阵黑....\n");
	...
	"在你眼前\n",({me,ob}));
	if(!wizardp(ob))
	message("channel:sys",HIY"〖管理监控〗: "+me->query("name")+"("+me->query("id")+")"+"将"+ob->query("name")+"("+ob->query("id")+")从"+pos+"抓到了"+environment(me)->query("short")+"。\n"NOR,users());
	 //ok ..
	return 1;
}
int help(object me)
