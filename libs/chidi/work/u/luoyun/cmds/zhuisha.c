...
int main(object me, string arg)
{
        ...
        if(! (banghui=me->query("banghui/name"))  || me->query("banghui/rank_lv") < 8 )
              return notify_fail("只有帮会的帮主或副帮主才能使用这条命令。\n");
        if(! arg)       {
                msg=HIR"黑名单\n"NOR;
                msg+="—————————————————————————————————————\n";
                name=users();
        ...
        if(sscanf(arg,"-a %s",str)==1)  {
               ...
               HIR"【谣言四起】"+ "某人：「"+banghui+"」开始追杀本帮"+
                target->query("name")+"("+target->query("id")+")。\n"NOR,users());
                else
               message("channel:rumor",HIR"【谣言四起】"+ "某人：「"+banghui+"」开始追杀"+target->query("name")+"("+target->query("id")+")。\n"NOR,users());
                return 1;
        }
        if(sscanf(arg,"-d %s",str)==1)  {
                ...
                target->save();
                message("channel:rumor",
                YEL"【谣言四起】"+ "某人：「"+banghui+"」停止追杀"+target->query("name")+"("+target->query("id")+")。\n"NOR,users());
                return 1;
        }
