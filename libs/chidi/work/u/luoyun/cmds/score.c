...
int main(object me, string arg)
{
         ...
         mapping my,pmap;
         object ob, weapon;
         int i, cc_len, attack_points, dodge_points, parry_points,balance,money;
//         seteuid(getuid(me));
         seteuid(getuid());
         if(!arg)
                 ob = me;
         ...
         balance = (int)me->query("money");
         money   = (int)me->query("more_money")*10000;
        ...
        if( wizardp(me) || (int)ob->query("age") >= 18 ) 
         ...
         if( ob->query("title") )
         { 
            if( ob->query("degree")||ob->query("title"))
            /*{  
                 line += sprintf("%s%-58s",
            ...
            {
                 line += sprintf("%s%-19s%s%-26s",
                 "┃ 称  谓 ：    ",ob->query("title"),
                  "银行存款 ：  ",chinese_number(money)+"两黄金");
        ...
        if (ob->query("shen") >0)
         line += sprintf("%s"RED"%-11s"NOR"%s"HIM"%-11s"NOR"%s"HIY"%-16s"NOR"┃\n",
         ...
         line
 += sprintf("%s"RED"%-11s"NOR"%s"HIM"%-11s"NOR"%s"HIY"%-16s"NOR"┃\n",
                "┃综合评价 ：   ",ob->query("score")+"",
