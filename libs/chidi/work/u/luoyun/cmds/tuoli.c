#include <ansi.h>
inherit F_CLEAN_UP;

int main(object me, string arg)
{
        string banghui,msg;
        object lingpai;
        if(me->is_fighting()||me->is_busy())
          return notify_fail("你正忙着呢。\n");
        if(! (banghui=me->query("banghui/name")))
           return notify_fail("你没有参加任何帮会，无需脱离。\n");
        if(! arg)
           return notify_fail("你要脱离什么帮会？\n");
        if(arg != banghui)
           return notify_fail("你不是这个帮会的成员。\n");
        lingpai=new("/banghui/obj/lingpai");
        lingpai->create(banghui);
        if(lingpai->query("no_use"))    {
                destruct(lingpai);
                return notify_fail("你的帮会文件有问题，请与巫师联系。\n");
        }
        if(me->query("id")==lingpai->query("bangzhu_id") && 
           sizeof(lingpai->query("player")) >= 2 )
        {
                return notify_fail("你是帮会「"+banghui
                +"」的帮主，脱离前必须先让位(rangwei)！\n");
        } else if(me->query("id")==lingpai->query("bangzhu_id"))
        {
                msg="你是帮会「"+banghui+"」的唯一成员，脱离后将导致帮会解体。\n";
                msg+="你真的要脱离吗(y/n)？";   
                write(msg);
                destruct(lingpai);
                input_to("yes_or_no",me,banghui);
                return 1;      
        } else    msg="你是帮会「"+banghui+"」的帮众。\n";
        msg +="脱离会被你现在所在帮派追杀的,你真的要脱离吗(y/n)？";
        write(msg);
        destruct(lingpai);
        input_to("yes_or_no",me,banghui);
        return 1;
}

int yes_or_no(string str,object me,string banghui)
{
        mapping data;
        object lingpai;
        str=lower_case(str);
        if(str=="y"||str=="yes")        {
                lingpai=new("/banghui/obj/lingpai");
                lingpai->create(banghui);
                if( lingpai->query("no_use"))   {
                        destruct(lingpai);
                        return notify_fail("你的帮会文件有问题，请与巫师联系。\n");
                }
                if(userp(me))   data=lingpai->query("player");
                else    data=lingpai->query("npc");
                if(! mapp(data))        data=([]);
                map_delete(data,me->query("id"));
                if(userp(me))   lingpai->set("player",data);
                else    lingpai->set("npc",data);

        message("channel:rumor",YEL"【谣言四起】某人："+me->query("name")+"脱离"+
                "帮会「"+banghui+"」！\n"NOR,users());
        me->delete("banghui");
        if(userp(me)) 
        me->set("vendetta/"+banghui,1);
        if(!undefinedp(me->query("score")))
                me->delete("score");
        me->save();
        lingpai->save();
        if(! sizeof(lingpai->query("player")))
        {
                if(me->query("vendetta/"+banghui))
                { 
                      me->delete("vendetta/"+banghui);
                      me->save();
                }
                rm("/data/guild/"+banghui+".o");
                message("channel:rumor",YEL"【谣言四起】某人：帮会「"+banghui+
                        "」土崩瓦解了！\n"NOR,users());
        }
        destruct(lingpai);
        return 1;
        }
        write("好吧，请想清楚后再做决定．\n");
        return 1;
}
int help(object me)
{
        write(@HELP
指令格式：tuoli <帮会名称>
这个指令可以使你脱离帮会。
前提是你必须是该帮会成员。
HELP
        );
        return 1;
}

