...
int main(object me, string arg)
{
        ...
        if((string)ob->query("banghui/name")==me->query("banghui/name"))
                return notify_fail("这个NPC已经是本帮兄弟，不必通过切蹉武功来征招。\n");
        if((object)ob->query_temp("zhengzhao/who")!=me)
       ...
       message_vision(HIY"$N道：“如此"+RANK_D->query_self(ob)+
        ...
        if(ob->query_temp("zhengzhao"))
        ob->delete_temp("zhengzhao");
        me->fight_ob(ob);
        ob->fight_ob(me);
        call_out("check_qi",2,me,ob);
        return 1;
}
...
void check_qi(object me,object ob)
{
        ...
        if(! me->query("banghui/name")) return;
