...
int create_banghui(string str)
{
	...
	me=this_player();
       ...
       if(me->query("more_money") < 20)
       {
          tell_object(me,RANK_D->query_respect(me)+"，开帮立会需要二十万两黄金，"
                      +"你帐目上的钱还不够。\n");
          return 1;
       }
       if(me->query("score") < 500000)
       {
		...
		(string)(500000-me->query("score"))+"点，暂时无法开帮立会。\n");
		return 1;
       }
	if(me->query("combat_exp")<2100000000)	{
		tell_object(me,RANK_D->query_respect(me)+"的实战经验还差"+
		(string)(2100000000-me->query("combat_exp"))+"点，暂时无法开帮立会。\n");
		return 1;
	}
	if(! count=me->query("banghui/have_create"))	count=0;
	if(count>=2)	{
	...
	count++;
	me->set("banghui/have_create",count);
	me->save();
