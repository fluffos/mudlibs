...
int do_init()
{
       ...
       //if (me->query("more_money")<20) return notify_fail("½ø»¯ºÏ³ÉÌ¨ÊÇ°×ÓÃÂğ£¿¶şÊ®ÍòÁ½»Æ½ğÒ»´Î£¡\n");  
...
int do_finish()
{
   ...
   int i,lvl,sh_lvl,sj_lvl,suss,suss1,*color1; //staÎªÁ¶Íê×´Ì¬£¬0£ºÊ§°Ü£¬ÎïÆ·ÏûÊ§£»1£ºÊ§°Ü£¬ÎïÆ·²»ÏûÊ§£»
  ...
  if (inv[0]->query("material")=="baoshi")
{
   ...
   suss1=baoshi->query("point");//µÈ¼¶Ô½¸ß£¬³É¹¦ÂÊÔ½¸ß
   if (sj_lvl >= 4 && sj_lvl <= 7)
     suss1 -=suss1/4;//×°±¸µÈ¼¶>=4²¢ÇÒ<=7Ôò³É¹¦ÂÊÏÂ½µ25%
   if (sj_lvl >= 7 && sj_lvl <= 9 )
     suss1 -=suss1/2;//×°±¸µÈ¼¶>=7²¢ÇÒ< 9Ôò³É¹¦ÂÊÏÂ½µ50%
   suss=random(100); //³É¹¦ÂÊ 
suss1=100;  
 tell_object(me,HIY"³É¹¦ÂÊ£º"+(100-suss)+"£¥\n"NOR);	      
 if (suss1>=suss && sh_lvl==0) return get_baoshi(2,mess);//µÈ¼¶>=³É¹¦ÂÊ£¨KÇÒé0¼‰Ñb‚ä£©Ôò³É¹¦ºÏ³É×°±¸
 else if (suss1 >=suss && sh_lvl == 1) return get_baoshi(3,mess); //µÈ¼‰>=³É¹¦ÂÊ£¨KÇÒé>0¼‰Ñb‚ä£©„t³É¹¦éx³ÉÑb‚ä
...
int get_baoshi(int sta,string mess1)
{
   ...
   switch(sta)
   {
   	case 0:
         ...
         if (zhuangbei->query("sh_lvl")==1){
              rm(base_name(zhuangbei)+ ".c");
	     DBASE_D->clear_object(zhuangbei);}
                           destruct(zhuangbei);   
   			   destruct(baoshi);             
...
if (zhuangbei->query("armor"))
       obj->set("item/armor",  zhuangbei->query("armor")*lvl);
if (zhuangbei->query("attack1"))
       obj->set("item/attack1",  zhuangbei->query("attack1")*lvl);
if (zhuangbei->query("defense1"))
	obj->set("item/defense1", zhuangbei->query("defense1")*lvl);
if (zhuangbei->query("intelligence"))
	obj->set("item/intelligence",  zhuangbei->query("intelligence")*lvl);
if (zhuangbei->query("dexerity"))
	obj->set("item/dexerity",  zhuangbei->query("dexerity")*lvl);
if (zhuangbei->query("constitution"))
	obj->set("item/constitution",  zhuangbei->query("constitution")*lvl);
if (zhuangbei->query("strength"))
	obj->set("item/strength",  zhuangbei->query("strength")*lvl);
if (zhuangbei->query("karma"))
	obj->set("item/karma",  zhuangbei->query("karma")*lvl);
if (zhuangbei->query("personality"))
	obj->set("item/personality",  zhuangbei->query("personality")*lvl);
if (zhuangbei->query("dodge"))
	obj->set("item/dodge",  zhuangbei->query("dodge")*lvl);
if (zhuangbei->query("force"))
	obj->set("item/force",  zhuangbei->query("force")*lvl);
	...
	obj->move(me);
        ...
        case 3://•N¼‰¸ß¼‰Ñb‚ä³É¹¦
          ...
          if (inv[0]->query("material")=="baoshi")
           ...
           obj=zhuangbei;
           sjlvl=zhuangbei->query("sj_lvl",1);
           sjlvl += 1;  
              if (sjlvl >=7) sjlvl=7;      
/*------------------------------------×·¼ÓÌØÊâŒÙĞÔ£¨»ù”µ¡ÁßM»¯´Î”µ£©---------------------------------------*/
              if (zhuangbei->query("armor")) //×·¼ÓÑb‚ä·À¶RÁ¦
                    zhuangbei->set("armor_prop/armor",  zhuangbei->query("armor")*sjlvl);
              if (zhuangbei->query("attack1")) //×·¼Ó¹¥“ô
                    zhuangbei->set("armor_prop/attack1",  zhuangbei->query("attack1")*sjlvl);
              if (zhuangbei->query("defense1")) //×·¼Ó¶ã±Ü
	             zhuangbei->set("armor_prop/defense1", zhuangbei->query("defense1")*sjlvl);
              if (zhuangbei->query("intelligence")) //×·¼ÓÎòĞÔ
	             zhuangbei->set("armor_prop/intelligence",  zhuangbei->query("intelligence")*sjlvl);
              if (zhuangbei->query("dexerity")) //×·¼ÓÉí·¨
	             zhuangbei->set("armor_prop/dexerity",  zhuangbei->query("dexerity")*sjlvl);
             if (zhuangbei->query("constitution")) //×·¼Ó¸ù¹Ç
	             zhuangbei->set("armor_prop/constitution",  zhuangbei->query("constitution")*sjlvl);
             if (zhuangbei->query("strength")) //×·¼Ó±ÛÁ¦
	             zhuangbei->set("armor_prop/strength",  zhuangbei->query("strength")*sjlvl);
             if (zhuangbei->query("karma")) //×·¼Ó¸£Ôµ
	             zhuangbei->set("armor_prop/karma",  zhuangbei->query("karma")*sjlvl);
             if (zhuangbei->query("personality")) //×·¼ÓÈİÃ²
	             zhuangbei->set("armor_prop/personality",  zhuangbei->query("personality")*sjlvl);
             if (zhuangbei->query("dodge")) //×·¼ÓÇá¹¦
                    zhuangbei->set("armor_prop/dodge",  zhuangbei->query("dodge")*sjlvl);
             if (zhuangbei->query("force")) //×·¼ÓÄÚ¹¦
	             zhuangbei->set("armor_prop/force",  zhuangbei->query("force")*sjlvl);
/*---------------------------------------------END-------------------------------------------------------*/

             zhuangbei->set("sj_lvl",sjlvl);             
             zhuangbei->move(me);
             zhuangbei->save();                        
                   for (k=0;k<sizeof(inv);k++) 
             if (inv[0]->query("material")=="baoshi")
            {
              ...
              baoshi=inv[1];
                ...
                destruct(baoshi);
   		mess+=HIG+"·¢ÏÖÀïÃæÒ»"+zhuangbei->query("unit")+zhuangbei->name()+HIG+"ÉÁÉÁ·¢¹â¡£\n";
   		mess+=me_name+"ÆÈ²»¼±´ıµØÒ»°Ñ×¥ÁË³öÀ´¡£\n"+NOR;
              message("channel:chat",HIM"¡¾¿ÕÑ¨À´·ç¡¿£ºÌıËµ"+me_name+"ºÏ³É½ø»¯×°±¸³É¹¦£¬Á¶³ÉÁË"+zhuangbei->name()+"¡£\n"NOR,users());
		break;
   	   }
   mess+=mess1;
