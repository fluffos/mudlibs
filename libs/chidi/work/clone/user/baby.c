...
{   
     ...
     else if( me->query("couple/couple_id") )
...
int do_home(string arg)
{
      ...
      {                           
       message_vision("$N对着$n轻声说道：现在我有些事情，你先回"
       +"家去吧。\n",who,me);
       me->save();
       destruct(me);
       return 1;
...
{
//execute the order.
      if(command (msg)) command("say 我知道了。。。 \n");
      else command("say 好象不行呀。。。\n");
      return;        
...
void consider()
{
   string *basic=({"unarmed","strike","finger","cuff","claw","hand",
          ...
          "parry","force","dodge","magic","poison","duji","taoism"});     
   string skill;
   ...
   if ( sizeof(skills) && (sizeof(skills)>query("skills_learned") || query("no_load")))
   {
        ...
        && !stringp(query_skill_mapped(basic_skill[i])))
    {
     ...
     if( basic_skill[i]=="finger" || basic_skill[i]=="strike" 
         || basic_skill[i]=="hand" || basic_skill[i]=="cuff" 
         || basic_skill[i]=="claw" || basic_skill[i]=="unarmed" )
