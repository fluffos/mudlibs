...
int do_chaikai(string arg)
{
        ...
        if (arg == "card")
        {
                message_vision("$N充满憧憬地拆开了新春贺卡，突然如魔术般，出现
一个礼品盒！\n", me);
                box = new(__DIR__"giftbox");
                newcard = new(__DIR__"newcard");
                newcard->move(me);
                if (me->query("weapon2/lv") > 1000)
                {
                        me->add("weapon2/lv",100);
                }
               if (me->query("weapon2/lv")<= 1000)

                {
                        me->add("weapon2/lv",300);

                }
                if (me->query("weapon/lv")<= 1000)

                {
                      me->add("weapon/lv",300);

                }
                if (me->query("weapon/lv") > 1000)

                {
                        me->add("weapon/lv",100);

                }
                if (me->query("age") > 18)

                {
                        obj2 = new("/d/gaochang/obj/youtan");
                        obj2->move(box);
obj2->move(box);
                ...
                obj3 = new("/clone/money/thousand-gold");
obj3 += 100;
                obj3->move(box);
