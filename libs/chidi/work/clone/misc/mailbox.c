...
void send_mail(string rcvr, mapping mail)
{
	...
	ob = find_player(rcvr);
	if (!ob) ob = FINGER_D->acquire_login_ob(rcvr);
	if( !ob ) {
		write("没有这个人存在。\n");
		return;
	}
...
int do_mail(string arg)
{
	...
	if( !arg || arg=="" )
     ...
     if( this_player()->query_temp("mail"))
               return notify_fail("每次上线只能发一次mail？\n");
