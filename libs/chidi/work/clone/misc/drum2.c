...
int do_knock(string arg)
{
    ...
    for(i=0, j=0;i<sizeof(env);i++)
	{
        ...
        //if (env[i]->query("mud_age")<86400*2)  continue;
...
void de_group(object *fighters)
{
    ...
    if(sizeof(fighters)<9)
	{
        if (query("loop"))
		{
            ...
            delete("drum_knocked");
            delete("lunshu");
			environment()->set("exits",([ "southdown":"/d/huashan/houzhiwangyu",]));
			delete("is_drum_knocked");
            return;
        }
		else
		{
			tell_room(room, HIW+"\n\n比武大会第"+chinese_number((int)query("lunshu")+1)+"轮参赛人数总共"+chinese_number(sizeof(fighters))+"名"+NOR);
			...
			for (i=0;i<sizeof(fighters);i++)
			{
				for (j=i;j<(sizeof(fighters)-1);j++,k++)
				{
					...
					msg += ((string)fighters[i]->query("name")+"("+(string)fighters[i]->query("id")+")"+HIY+"--V.S.--"+NOR+(string)fighters[j+1]->query("name")+"("+(string)fighters[j+1]->query("id")+")"+"\n\n");
				}
	...
	else
	{
        ...
        if (sizeof(list)%2)
		{
            msg += HIY+((string)list[i]->query("name")+"("+(string)list[i]->query("id")+")"+"------(此轮免战)\n\n"+NOR);
            set("group0",([ list[0]: 0 ]) );
            i=1;
        ...
        for (i;i<(sizeof(list))/2;i++)
		{
            ...
            msg += ((string)list[i]->query("name")+"("+(string)list[i]->query("id")+")"+HIC+"--V.S.--"+NOR+(string)list[sizeof(list)/2+i]->query("name")+"("+(string)list[sizeof(list)/2+i]->query("id")+")"+"\n\n");
        }
    }
...
string clear_title(string arg)
{
    ...
    if( (strsrch(arg, "【天尊】") >= 0)
      ||(strsrch(arg, "【地皇】") >= 0)
      ||(strsrch(arg, "【人杰】") >= 0)
||(strsrch(arg, "本月天下第四") >= 0)
||(strsrch(arg, "本月天下第五") >= 0)
||(strsrch(arg, "本月天下第六") >= 0)
||(strsrch(arg, "本月天下第七") >= 0)
||(strsrch(arg, "本月天下第八") >= 0)
||(strsrch(arg, "本月天下第九") >= 0)
||(strsrch(arg, "本月天下第十") >= 0)
)
        {
        arg = replace_string(arg, HIY+"【天尊】"+NOR,"");
        arg = replace_string(arg, HIY+"【地皇】"+NOR,"");
        arg = replace_string(arg, HIY+"【人杰】"+NOR,"");
arg = replace_string(arg, HIY+"本月天下第四"+NOR,"");
arg = replace_string(arg, HIY+"本月天下第五"+NOR,"");
arg = replace_string(arg, HIY+"本月天下第六"+NOR,"");
arg = replace_string(arg, HIY+"本月天下第七"+NOR,"");
arg = replace_string(arg, HIY+"本月天下第八"+NOR,"");
arg = replace_string(arg, HIY+"本月天下第九"+NOR,"");
arg = replace_string(arg, HIY+"本月天下第十"+NOR,"");
...
int give_money(object *players,object *fighters)
{
    if (query("drum_knocked"))
	{
        ...
        message("vision",query("drum_knocked")+"给参赛的江湖人士每人一张金票：“小小意思，笑纳，笑纳。”\n",environment());
    }
    de_group(fighters);
    return 1;
}
