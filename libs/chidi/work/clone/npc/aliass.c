...
// end
    ...
    if (time()-last_cmd_time<1){
            if (kill_time> 4){  //原来的是 if (kill_time> 4)
...
/*                  seteuid(getuid());
                    ...
                    cmd_time=0;
                    kill_time=0;
*/     //取消鬼王杀人,防止利用在no_fight房间用busy武功定着鬼王,无限使用指令。
    ...
    if( str==last_input ) {
        ...
        if( repeat_cnt > MAX_REPEAT && !wizardp(this_object())) {
            tell_object( this_object(),
                RED "\n\n侮天鬼王突然在一阵烟雾中出现。\n\n"
                "\n\n侮天鬼王手执"HIG"--霰 血 天 魔 杵--"RED"往你头上砸去！\n\n"
                 HIY"\n\n一道闪电划过.........!!!\n\n" NOR);
                //"\n\n侮天鬼王喝道：你是机器人!!! 连我也看不惯了，滚吧!!!\n\n" NOR); //取消该描述。
                // this_object()->kick(); // 取消kickout。
          this_object()->unconcious(); // 修改成玩家昏迷。
          return "";
        }
    } else {
        repeat_cnt = 0;
