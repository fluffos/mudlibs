...
inherit BHNPC;
string query_save_file() { return DATA_DIR + "npc/朱半仙"; }
...
set("area_name","扬州广场");
set("area_file","/d/city/guangchang.c");
        set("gender", "男性" );
        set("age", 56);
	...
	set_skill("force", 15000);
        set_skill("unarmed", 10000+random(2000));
        set_skill("sword", 10000+random(2000));
	set_skill("parry", 10000);
        set_skill("dodge", 10000+random(2000));
	   set_skill("langzi-sword", 10000+random(2000));
	   	set_skill("tiyunzong", 15000);
	   set_skill("damo-force", 15000);
			...
			me->add_temp("quest_count",1);
				...
				i=random(sizeof(ob));
					...
					if (ob[i]->query("no_die")) continue;
                                       ...
                                       if (sscanf(base_name(ob[i]),"/quest/%*s" )) continue;
                                      if (sscanf(base_name(ob[i]),"/d/xiangyang/%*s" )) continue;
                                      if (sscanf(base_name(ob[i]),"/d/tudimiao/%*s" )) continue;
                                      if (sscanf(base_name(ob[i]),"/d/xiaoyao/%*s" )) continue;
...
//                  i=(i/3+1)*2;
    ...
    if (i>1000)  i=1000;
		...
		me->add("potential",i);
		i=random(i)*2/3+10;
		tell_object(me, sprintf(YEL"%s点综合评价！\n"NOR, chinese_number(i)) );
		me->add("score",i);
		me->delete("quest");
	        return 1;
	}else{
