...
void create()
{
        set_name(HIY HIM "屠龙刀" NOR,({ "dragon blade", "dragon", "blade", "dao" }) );
        set_weight(10000);
	   set("flag",2);
        if( clonep() )
                ...
                set("unit", "柄");
								...
								set("level",1);
								set("exp",5000);
								set("weapon_prop/baoji",1000);
                set("material", "steel");
                set("_level",20);
                set("jing",100);
                set("jingli",50);
                set("weapon_prop/strength",300);
                set("weapon_prop/dexerity",300);
                set("weapon_prop/intelligence",300);
                set("weapon_prop/constitution",300);
								set("dp",10);                //防御
								set("pp",10);                //追加防御
								set("ap",1000);                //追加攻击
								set("cp",1000);
                set("wield_msg", HIY "$N“唰”的一声抽出$n" + HIY + "但见此刀寒芒吞吐，电闪星飞，天地变色。 ....$N不由得从心底升起一股凌云豪气。\n" NOR);
                set("unwield_msg", HIW "屠龙刀"HIY"幻作一道白光，「唰」地飞入刀鞘。\n" NOR);        }
        set("max_lasting",5750);
        set("gender","男、女");    //限制性别
        init_blade(400);
        setup();
}
...
int move(mixed dest)
{
        if(!objectp(dest) || !userp(dest))       return ::move(dest);
				if(objectp(dest) && userp(dest) && present("sky sword",dest))
				{
					tell_object(dest,"你刚伸手去捡屠龙刀，它却被倚天剑震飞，甚是可惜。\n");
					::move("/d/city/guangchang");
					return 0;
				}
				if( ::move(dest)==0)    return 0;
        message("channel:rumor",YEL"【谣言】"HIM"某人：武林至尊，宝刀屠龙，号令天下，莫敢不从。屠龙刀被"+dest->query("name")+"抢到手了！\n"NOR,users());
        return 1;
}
...
/*
		...
		*/
		str += "└──────────────┘\n"HIY"这是一柄六尺来长的宝刀，刀鞘上金\n丝镶着两个字“屠龙”。整个刀身隐\n隐发出一层青气，刀未出鞘，已可见\n其不凡。\n"
		"必须装备武器，方可使用下列功能。\n"
		//"[武器技能]屠龙斩: perform tulong\n"
	...
	if(userp(owner))
	{
				if(present("sky sword",owner))
				{
					tell_object(owner,"屠龙刀被倚天剑震飞了，甚是可惜。\n");
...
int do_perform(string arg)
{
	...
	if(arg == "tulong") //屠龙崭
	{
		...
		if(obj->query("max_lasting") < 1000)
		{
			tell_object(me,"耐久度不足，屠龙刀的锋利已大不如前。\n");
			return 1;
		}
		...
		tell_room(env,sprintf(BLINK+HIY"%s使出屠龙斩，手中屠龙刀，刀光暴涨。扫出一道煞白杀气，"BLINK+HIR"武林至尊 宝刀屠龙 号令天下 莫敢不从"BLINK+HIY"。\n"NOR,me->name()) );

		for(i=0;i<size;i++)
		{
			if( !objectp(player[i]) || !userp(player[i]) || player[i]->is_npc() || player[i] == me) continue;

			if(blade = present("sky sword",player[i]))
			{
				tell_room(env,sprintf(BLINK+HIY"%s的倚天剑爆出一阵白光，将%s的杀气弹了回去，%s躲闪不急，硬是被自己的杀气击成废人。\n"NOR,player[i]->name(),me->name(),me->name()) );
				index = 1+random(10);
				me->set("eff_qi",index);
				me->set("qi",0);
				me->start_busy(120);
			...
			else if(player[i]->query("age") < 14)
			{
				tell_room(env,sprintf(HIR"屠龙刀杀气从%s头顶掠过，断了几根毫发，幸好没有大碍，还是赶紧离开为妙！\n"NOR,player[i]->name()) );
				continue;
			}
			else
			{
				...
				{
					case 0 : 
						tell_room(env,sprintf(HIR"屠龙杀气击中%s胸膛，身体顿时击成碎肉四处飞溅。\n"NOR,player[i]->name()) );
						break;
					case 1 :
						tell_room(env,sprintf(HIR"屠龙杀气击中%s头颅，血液随勃颈喷射而出，头颅早已落地。\n"NOR,player[i]->name()) );
						break;
					case 2 :
						tell_room(env,sprintf(HIR"屠龙杀气击中%s臂膀，将其从左至右削成两段，煞是恐怖！\n"NOR,player[i]->name()) );
						break;
					case 3 :
						tell_room(env,sprintf(HIR"屠龙杀气击中%s下身，整个人往当中断开，血液倾盆。\n"NOR,player[i]->name()) );
						break;
					case 4 :
						tell_room(env,sprintf(HIR"屠龙杀气尽数击中%s，只剩一具骷髅站立，隐约可见少许内脏，煞是恐怖！\n"NOR,player[i]->name()) );
						break;
				}
				player[i]->set_temp("die_for","被屠龙刀杀气震");
				player[i]->die();
			}
		}
		me->start_busy(60);
		me->set("neili",0);
		obj->add("max_lasting",-1000);
		tell_object(me,HIG"你将屠龙斩尽数打完，只觉一阵虚脱，暂时无法运功。\n"NOR);
		return 1;
	}
	return 0;
}
