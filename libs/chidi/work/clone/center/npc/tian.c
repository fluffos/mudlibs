...
void create()
{
        ...
        set("gender", "女性" );
        set("long", HIM "她是【江湖】服务中心的管理人员，听说她和望天涯关系非同寻常。\n" NOR);
        set("title", HIY "【江湖】会员服务中心管理员" NOR);
...
int give_card()
{
        ...
        if (me->query("is_member"))
        {
              ...
              {
                     write(HIG "小天说道：“你已经领取了【江湖】会员卡！请不要在这捣乱！”\n" NOR);
                     return 1;
              }
              // 判断是否在挂失
              if (! me->query("card_lost"))
              {
                     write(HIR "小天说道：“如果你卡片丢失，请先到【江湖】会员服务中心大厅挂失！”\n" NOR);
                     return 1;
              }

              write(HIW "小天说道：“以后小心保管，这张【江湖】会员卡就交还给你了！\n" NOR);
              hycard = new("/clone/center/hycard");
              hycard->move(me);
              message_vision(HIC "$N" HIC "从小天处领取一张" HIY "【江湖】会员卡" NOR "。\n" NOR,
                             me);
              me->delete("card_lost");
              return 1;            
        }
        ...
        else
        {
             ...
             write(NOR + CYN "小天说道：“好吧，从现在开始你就成为【江湖】会员了！\n" NOR);
             write(NOR + CYN "小天说道：“这张会员卡就交给你了，以后请遵守会员条例！\n" NOR);
             message_vision(HIG + me->name() + HIG "从小天处领取一张" HIY "【江湖】会员卡" NOR " 。\n" NOR,
                            me);
             hycard = new("/clone/center/hycard");
             hycard->move(me);
             me->set("is_member", 1);
             CHANNEL_D->do_channel(this_object(), "rumor", me->name() + "正式成为【江湖】会员！");
             return 1;       
        }
}
