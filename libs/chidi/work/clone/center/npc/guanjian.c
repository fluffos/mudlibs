...
void create()
{
        set_name(HIW "关键" NOR, ({"guan jian", "guan", "jian"}));
        set("gender", "男性" );
        set("long", HIC "他看起来有点丑，但是怎么也看不出来哪儿温柔。\n" NOR);
        set("title", HIY "【江湖】会员服务中心营业员" NOR);
...
int guashi()
{
        ...
        if (me->query("is_member"))
        {
              ...
              {
                     write(HIG "关健怒道：“你的【江湖】会员卡并没有丢失！不要在这捣乱！”\n" NOR);
                     return 1;
              }

              if (me->query("card_lost"))
              {      
                     write(HIG "关健怒道：“你已经挂失过了，不要在这捣乱！”\n" NOR);
                     return 1;                     
              }

              if (me->query("guashi") >= 300)
//挂失次数不能大于300
              {
                     write(HIG "关健告诉你：“你已经挂失超过三百次了，如要领回会员卡请与在线巫师联系！”\n" NOR);
                     return 1;         
              }
              command("nod");
              write(HIC "关健说道：“好了，你已经成功挂失了，你可以在" HIG "小天" HIC 
                   "那再领取一张【江湖】会员卡。”\n" NOR);
              write(HIG "你已经成功挂失！\n" NOR);
              ...
              CHANNEL_D->do_channel(this_object(), "rumor", me->name() + "的【江湖】会员卡已挂失，请拣"
                                   "到者速交与在线巫师！");
              return 1;            
        }
