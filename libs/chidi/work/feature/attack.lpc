...
void reset_action()
{
        ...
        if( (query_temp("secondary_weapon") && ob = query_temp("secondary_weapon"))
...
void init()
{
       ...
       int adverse=0;
        ...
        if(stringp(query("banghui/name"))&& query("banghui/name")==(string)ob->query("banghui/name")&& !userp(this_object()) && userp(ob))     {
        ...
        if( userp(ob) && is_killing(ob->query("id")) ) {
                ...
                return;
        } else if( stringp(vendetta_mark = query("banghui/name"))
...
void canjian(object ob,object me)
{
        ...
        if( ob->is_killing(me) )
        message_vision(HIC"$N大喝一声：$n，今天"+RANK_D->query_self_rude(ob)
                       +"要讨个公道！\n"NOR,ob,me);
        else if( me->query("vendetta/" +ob->query("banghui/name")) &&
                 ob->query("banghui/rank_lv") < 9 )
        message_vision(CYN"$N朗声道："+RANK_D->query_self(ob)+
        "奉命行事，还请"+RANK_D->query_respect(me)+"多多担待。\n"NOR,ob,me);
        else if( ob->query("banghui/rank_lv") < me->query("banghui/rank_lv") )
        {
           if(stringp(me->query("banghui/rank")))
           message_vision(HIC"$N躬身对$n道：属下参见"+me->query("banghui/rank")
                                                             +"。\n"NOR,ob,me);
        }
        return;
}
