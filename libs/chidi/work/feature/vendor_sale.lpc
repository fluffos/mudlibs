...
int do_vendor_buy(string arg)
{
        int value, val_factor,sale_price;
        string ob_file;
        object ob;
//      mapping fam;
...
//              return notify_fail("你是个穷叫化，买什麽东西！\n");
        ...
        sale_price = price_ratio;
//        if (sale_price > 5)
//                sale_price = 5;
        sale_price = ob->query("value")*sale_price/10;
                ob->set_temp("sale_price", sale_price);
        if (!take_money(sale_price, ob->query("value")))
        {
           destruct(ob);
           return 0;
        }
        
        switch (MONEY_D->player_pay(this_player(), sale_price *  val_factor / 10)) {
        case 0:
...
int do_bid(string arg)
{
       ...
       CHANNEL_D->do_channel( this_object(), "sale",
...
void reload(string filename)
{
      object me;
        ...
        if(owner_id)
        set_new_owner(owner_id, owner_name, owner_long);
       if(close_time<time()+120) close_time+=120; 
