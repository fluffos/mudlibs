...
int is_team_member(object me,object ob);//判断ob是不是me队伍的成员,
int follow_team(object ob,string dir);
int is_team_member(object me,object ob)
{
        if(!objectp(me)
           ||!objectp(ob)
           ||!me->query_team()
           ||me->query_team()==({})
           ||me->query_team()==0) return -1;
        return member_array(ob,me->query_team());
}       
...
//              this_object()->remove_all_enemy();  //delete by zzz year 2000
