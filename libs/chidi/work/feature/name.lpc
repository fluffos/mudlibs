...
//add by lonely for banghui
    if( stringp(query("banghui/name")) ) { 
        str=sprintf(HIG"%s%s"+NOR"%s%s",query("banghui/name"),
        (string)query("banghui/rank")? query("banghui/rank"):"帮众"," ",str);
        }
//the end
  ...
  if((string)this_object()->query("doing")=="breakup")   str += HIY " <闭关中>" NOR;
  if((string)this_object()->query("doing")=="closed")   str += HIY " <修炼中>" NOR;
