 ...
 if (prop == "money"&&userp(this_object()))
{
	...
	if (data>500000000)
	{
//                 add("money",data/100000000);
//                 data=data-((data/100000000)*100000000);
//                 tell_object(this_object(),HIR"你在钱庄的钱已达到五万两黄金！\n"NOR);
//                 tell_object(this_object(),HIR"系统已经帮你把钱庄里的黄金转换成了"HIY"RMB"HIB"存在了钱庄!\n"NOR);
	}
}
//自动升级
else if(prop == "combat_exp"&&userp(this_object()))
{
	if(data > 2100000000 || data < 0)
	{
		tell_object(this_object(),HIR"经验已达上限，无法获得更多经验！\n"NOR);
		"/d/jh/shengji"->do_shengji(this_object());
		data = 2100000000-query("combat_exp");
	}
}

else if(prop == "potential"&&userp(this_object()))
{
	if(data > 2000000000)
	{
		tell_object(this_object(),HIR"潜能已达上限，无法获得更多潜能！\n"NOR);
		data = 2000000000;
	}
