...
void restore_autoload()
{
        ...
        sscanf(file,"/%s/%*s",dir);
        if(wiz_level(this_object()) == 0 && (dir == "u" || dir == "open" ) ) {
                write("\n系统认为你身上带有非法物品，自动清除，有疑问请通知巫师。\n");
                log_file("BADLOAD",sprintf("Fail to load %s's %s\n",this_object()->query("id"),file));
                continue;
        }
