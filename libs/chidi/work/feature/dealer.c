...
int do_pawn(string arg)
{
   ...
   // if (ob->query("unique") || ob->query("no_drop") || ob->query("no_put") || ob->query("no_sell"))  return notify_fail("这么重要的东西你也敢拿来典当？\n");
  //  if(ob->query("no_pawn")) return notify_fail("这样东西不能典当！\n");
...
int do_sell(string arg)
{
    ...
    if ( obj->query("unique") || obj->query("no_sell") || obj->query("no_drop")) return notify_fail("这东西是无价之宝，不能卖断！\n");
    value = obj->query("value");
    if( !value)
        if (!value=obj->query("base_value")) return notify_fail("这样东西不值钱。\n");
        if (value >= 500000)
        return notify_fail("这可是无价宝贝，小店承受不起，还是另找别家吧。\n");
