...
int accept_fight(object who)
{
        ...
        if( (int)query("jing") * 100 / (int)query_max_jing() >= 90
        &&        (int)query("qi") * 100 / (int)query_max_hp() >= 90 ) {
                switch(att) {
                        case "friendly":
...
int chat()
{
        ...
        if( !environment() ) return 0;
         if (query("neili")>100 &&this_object()->query_max_neili() > 200 && query_skill("force")>0
            && query("race") == "人类" && !is_busy() && !query_temp("disable_inputs")) {
                if (query("eff_jing")
                ...
                if (query("eff_qi") && query("qi") >= 0
                &&this_object()->query_max_hp() && !is_fighting() && !query("mute")
                && query_skill_mapped("force")
                && query_skill("force") > 50
                && query("eff_qi") <this_object()->query_max_hp()
                && query("eff_qi") >=this_object()->query_max_hp()/3)
                        command("exert heal"); 
        }
...
int random_move()
{
        ...
        if( is_busy() || !environment()||!living(this_object()) ) return 0; 
