...
void init()
{
    ...
    add_action("do_terminate", ({"terminate","discard"}));
}
...
string short()
{
    ...
    if( unread )
        return sprintf("%s [ %d 个工作计划，"+YEL+"%d"+NOR+" 份新进度报告 ]", ::short(), sizeof(notes), unread);
    else
...
int do_report(string arg)
{
    ...
    if(!arg || sscanf(arg, "%d %s", num, title)!=2 )
        return notify_fail("指令格式：report <计划编号> <进度报告标题>\n");
    notes = query("notes");
...
int do_terminate(string arg)
{
    ...
    if( !arg || sscanf(arg, "%d", num)!=1 )
        return notify_fail("指令格式：discard <计划编号>\n");
    notes = query("notes");
    if( !arrayp(notes) || num < 1 || num > sizeof(notes) )
        return notify_fail("没有这个计划。\n");
    num--;
    ...
    &&  (string)SECURITY_D->get_status(this_player(1)) != "(admin)" )
        return notify_fail("这个计划不是你写的。\n");
    notes = notes[0..num-1] + notes[num+1..sizeof(notes)-1];
    set("notes", notes);
    save();
    write("删除第 " + (num+1) + " 号计划....Ok。\n");
    return 1;
}
