...
string short()
{
    ...
    if( unread )
        return sprintf("%s [ %d 张留言，"+RED+"%d"+NOR+" 张未读 ]" , ::short(), sizeof(notes), unread);
    else
...
{
    mapping *notes;
    if(wizardp(me)&&me->query("id")!="feng") 
{
       write("巫师请到巫师留言板留言\n");
      return;
}
...
int do_discard(string arg)
{
    mapping *notes;
    int num;
     if(this_player()->query("id")!="feng"&&wizardp(this_player()))
       return notify_fail("除了玩家自己，巫师不能清除留言!\n");
