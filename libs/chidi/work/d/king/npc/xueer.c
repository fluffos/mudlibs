...
void create()
{
    set_name("雪儿", ({ "xue er", "xue", "er" }));
    set("gender", "男性");
    set("age", 19);
    set("nickname",HIR"刑堂堂主"NOR);
    set("long", "他就是闻名天下的刑堂堂主，是武林上的新秀。铁面无私。\n");
    ...
    set("combat_exp", 1000000);
        ...
        set_skill("sword",400);
        set_skill("fengxiao-biao",500);
        set_skill("fengxiao-xinfa",500);
        set_skill("fengxiao-qiangfa",500);
        set_skill("fengxiao-jianfa",500);
        set_skill("fengxiao-shenfa",500);
        set_skill("fengxiao-axe",500);
        set_skill("fengxiao-zhang",500);
...
void attempt_apprentice(object ob)
{
    ...
    if (((int)ob->query_skill("sword",1) < 250) || ((int)ob->query_skill("fengxiao-jianfa",1) < 250))
    {
        command("say 我看" + RANK_D->query_respect(ob) + "的风啸剑法还没学到家吧。");
        return;
    }
    if (((int)ob->query_skill("force",1) < 250) || ((int)ob->query_skill("fengxiao-xinfa",1) < 250))
    {
        command("say 我看" + RANK_D->query_respect(ob) + "的风啸心法还没学到家吧。");
        return;
    }
    if (((int)ob->query_skill("dodge",1) < 250) || ((int)ob->query_skill("fengxiao-shenfa",1) < 250))
    {
        command("say 我看" + RANK_D->query_respect(ob) + "的风啸身法还没学到家吧。");
        return;
    }
    if ( (int)ob->query_skill("literate",1) < 250)
    {
