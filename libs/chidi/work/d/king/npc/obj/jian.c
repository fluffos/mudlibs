...
//                set("no_put",1);
                set("material", "steel");
                set("long", "这是一柄四尺来长的古剑，剑鞘上铁丝镶着两个字“倚天”。
可惜是件假货。\n");
                set("wield_msg", HIY "$N“唰”的一声抽出$n" + HIY + "挽了个剑花，但见寒芒吞吐，
远看挺象真的。\n" NOR);
                set("unwield_msg", HIW "假倚天剑"HIY"幻作一道白光，「唰」地飞入剑鞘。\n" NOR);
                set("unequip_msg", HIW "假倚天剑"HIY"幻作一道白光，「唰」地飞入剑鞘。\n" NOR);
        }
        init_sword(1);
        setup();
}
...
int do_hukan()
{
        ...
        else if ( jian->query("name") == HIW HIY "屠龙刀" NOR )
        {
                jian->unequip();
                ...
                book = new("/d/binghuo/obj/wumu-yishu");
                own = me->query("id");
                book->set("master",own);
                book->move(me);
