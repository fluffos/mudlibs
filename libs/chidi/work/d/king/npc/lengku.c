...
void create()
{
    set_name("冷酷到底", ({ "leng ku", "leng", "ku" }));
    set("gender", "男性");
    set("age", 19);
    set("nickname",HIR"地煞堂主"NOR);
    set("long", "他是风啸楼中主管收帐和洗黑钱的。\n");
    ...
    set("combat_exp", 800000);
        ...
        set_skill("sword",250);
        set_skill("fengxiao-biao",250);
        set_skill("fengxiao-xinfa",250);
        set_skill("fengxiao-qiangfa",250);
        set_skill("fengxiao-jianfa",250);
        set_skill("fengxiao-shenfa",250);
        set_skill("fengxiao-axe",250);
        set_skill("fengxiao-zhang",250);
...
void attempt_apprentice(object ob)
{
    ...
    if (((int)ob->query_skill("sword",1) < 100) || ((int)ob->query_skill("fengxiao-jianfa",1) < 100))
    {
        command("say 我看" + RANK_D->query_respect(ob) + "的风啸剑法还没学到家吧。");
        return;
    }
    if (((int)ob->query_skill("force",1) < 100) || ((int)ob->query_skill("fengxiao-xinfa",1) < 100))
    {
        command("say 我看" + RANK_D->query_respect(ob) + "的风啸心法还没学到家吧。");
        return;
    }
    if (((int)ob->query_skill("dodge",1) < 100) || ((int)ob->query_skill("fengxiao-shenfa",1) < 100))
    {
