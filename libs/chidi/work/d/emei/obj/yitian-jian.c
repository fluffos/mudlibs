...
int move(mixed dest)
{
        ...
        if( ::move(dest)==0)    return 0;
         message("channel:rumor",HIB"【神兵利器】"+ "某人：倚天剑被"+
...
int do_hukan()
{
         ...
         if ( dao->query("id") != "tulong dao" )
	{
        ...
        if ( me->query("neili") <500000 )
		return notify_fail(HIW"你的内力太低了，只怕砍不断吧！\n"NOR);

       else if ( dao->query("id") == "tulong dao")
	{
		jian->unequip();
                jian->set("name", "断掉的兵刃");
		...
		message_vision(HIC"$N大喝一声，拿起屠龙刀向倚天剑砍去。\n"+
                ...
                book = new("/d/taohua/jiuyin");
		own = me->query("id");
		book->set("master",own);
            if (random(13)>10)                book->move(me);
            destruct(dao);
            destruct(jian);
