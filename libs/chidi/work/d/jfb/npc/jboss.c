...
void die()
{
	...
	{
		jmem = environment(ob)->query("jmem");
		if(clonep(jmem)) jmem->delete_map();
		destruct(ob);
		return;
	}
	...
	{
		if(!team[i]) continue;
		level = team[i]->query("jh/dj/dj");
		exp = 10000*(1+random(level))/size;
		
		if(exp)
		{
			pot = exp/3;
			tell_object(team[i],sprintf("你获得了%d点经验，%d点潜能。\n",exp,pot));
			tell_object(team[i],HIY"【号令天下】恭喜你挑战心魔成功！请拾取物品，镜像将在1分钟后关闭。\n"NOR);
	...
	if(rand < 10) file = PRODUCT->get_random_level_product(1+random(15),ob->query("jh/dj/dj")/10,ob->query("jh/dj/dj")/10);
	else if(rand < 5) file = PRODUCT->get_random_level_product(1+random(15),ob->query("jh/dj/dj")/10,(ob->query("jh/dj/dj")/10)+5);
