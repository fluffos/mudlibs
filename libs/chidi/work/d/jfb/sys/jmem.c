...
#define SYSTEAM "/d/jfb/sys/j_sys"
...
varargs void heart(int open)
{
	...
	if(me->query("make_jboss") >= 1000 && !me->query("send_jboss"))
	{
		for(i=0;i<size;i++)
		{
			...
			player = find_player(str);
			tell_object(player,BLINK+HIR"【号令天下】心魔出现，请各位小心。\n"NOR);
			players += ({ player });
		}
	...
	if(me->query("jfb_time") <= time())
	{
		for(i=0;i<size;i++)
		{
			...
			if(!str || !stringp(str)) continue;
			player = find_player(str);
			tell_object(player,BLINK+HIG"【号令天下】心魔镜像已经瓦解，救回了被剥夺魂魄的士兵。\n"NOR);
		}
		if(me->delete_map())
		{
			destruct(me);
	...
	size = sizeof(maps);
		...
		for(i=0;i<size;i++)
		{
			...
			for(j=0;j<size2;j++)
			{
				...
				sscanf(file,"/d/jfb/room/%s",fileName);
