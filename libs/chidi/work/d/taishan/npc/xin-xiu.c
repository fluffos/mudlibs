...
void create()
{
    seteuid(getuid());
    if (!restore()) {
      set_name("无聊", ({ "jianghu xinxiu", "xinxiu" }) );
     set("title", "江湖新秀" );
    set("gender", "男性" );
    set("age", 35);
      set("long","一位笑容可掬的中年人，他就是乐善好施，爱贫如子的武林江湖新秀。\n");
   ...
   set("no_get",1);
         set("startroom","/d/taishan/newplayer1");
...
int accept_fight(object ob)
{
    ...
    if( this_player()->query("id")==name2 ){
        command("say 你是罚恶使者，我是江湖新秀，我们打什么架？！\n");
        return 0;
    }
    if ( me->query("winner") == ob->query("id") ) {
    ...
    if (me->is_fighting() || me->query("fighting"))
        return notify_fail("已经有人正在挑战江湖新秀！\n");
 ...
 call_out("checking", 1, me, ob);
    ...
    if (( (int)me->query("qi")*100 / my_max_qi) <= 80 ) {
        if (!living(me))    me->revive();
         command("say 果然厉害，恭喜你成为当今江湖新秀！\n");
       command("chat 哈哈哈，到底是长江後浪推前浪，一代新人换旧人！\n");
        command("chat 恭喜" + ob->query("name") + "被推举为当今江湖新秀！\n");
        me->set("fighting",1);
        remove_call_out("do_copy");
 call_out("do_copy", 1, me, ob);
        return 1;
    }
    if (( (int)ob->query("qi")*100 / his_max_qi) < 80 ) {
...
int do_gengxin()
{
    ...
    command("chat 我退出江湖后，江湖新秀之位将由无聊少侠暂时接任。\n");
    rm(query_save_file()+__SAVE_EXTENSION__);
