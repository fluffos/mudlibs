...
void create()
{
    seteuid(getuid());
    if (!restore()) {
     set_name("江湖新秀", ({ "jianghu xinxiu", "xinxiu" }) );
        set("title", "江湖新秀" );
    set("gender", "男性" );
    set("age", 40);
        set("long","他就是当今武林，名动一时，称霸一方的后起之秀。\n");
    ...
    carry_object("/d/city/obj/cloth")->wear();        
        ...
        set("id", "xinxiu");
        set_name(query("name"), ({  "jianghu xinxiu", "xinxiu" }));
        set("no_get",1);
        setup();
...
void init()
{
    ...
    if (base_name(environment())!=this_object()->query("startroom")) return;
     ...
     if(ob->query("daoxin"))
{
  tell_object(ob, "你只不过是个残疾人,也配做当今江湖新秀?\n");
return 0;
}
...
int do_kill(string arg)
{
    ...
    command("say 你想谋害本新秀，当真是自找死路！ 座下红衣武士何在！");
...
int accept_fight(object ob)
{
    ...
    if( this_player()->query("id")==name1 ){
        command("say 盟主大人，在下江湖新秀怎么是您的对手？！\n");
        return 0;
    }
    ...
    if( this_player()->query("id")==name2 ){
        command("say 罚恶使者大人，在下江湖新秀怎么是您的对手？！\n");
        return 0;
    }
    ...
    if( this_player()->query("id")==name2 ){
        command("say 赏善使者大人，在下江湖新秀怎么是您的对手？！\n");
        return 0;
        
    /*
    if(!( xinxiu = find_living("jianghu xinxiu")) )
    if(!( xinxiu = find_object("xin-xiu")) )
    xinxiu = load_object(__DIR__"xin-xiu");
    xinxiu->restore();
    name3 = xinxiu->query("winner");
    if( this_player()->query("id")==name3 ){
        command("say 你是江湖新秀，我是赏善使者，我们打什么架？！\n");
        return 0;
        */
    }
    
 if( (int)this_player()->query("combat_exp") > 100000000 || (int)ob->query("age")>=18 ||(int)ob->query("mud_age")>345600 ){
        command("say "+ob->query("name")+" 你已经不是新手了，这位"+RANK_D->query_respect(ob)+"还是去挑战其他的使者吧！");
        return 0;
}
 if(!this_player()->query_temp("agree")) { 
   this_player()->set_temp("agree",1); 
   this_player()->start_busy(2); 
        message("system",HIW"【新秀书童】: "+this_player()->query("id")+"正在挑战武林新秀!特此告之!!!\n"NOR,users());
...
int checking(object me, object ob)
{
    ...
    CHANNEL_D->do_channe(this_object(),"chat","哈哈哈，到底是长江後浪推前浪，一代新人换旧人！\n");
    CHANNEL_D->do_channel(this_object(),"chat","恭喜"+ob->query("name")+"被推举为当今赏善使者！\n");
        me->set("fighting",1);
...
int do_copy(object me, object ob)
{
   ...
   if(!( xinxiu = find_living("jianghu xinxiu")) )
    ...
    ob->delete("party");
    ob->set("party/party_name",HIC+"第" + chinese_number(me->query("generation")) + "代"+ NOR );
    ob->set("party/rank",HIC+"江湖新秀"+NOR);
  
    me->set("short", HIC+"第" + chinese_number(me->query("generation")) + "代江湖新秀"+ NOR+" " + me->query("name") + "(jianghu xinxiu)");
    me->delete("title");
...
int do_recopy(object me, object ob)
{
    ...
    ob = this_player();
    if( (int)this_player()->query("combat_exp") > 100000000 || (int)ob->query("age")>=18 ||(int)ob->query("mud_age")>345600 ){
        command("say "+ob->query("name")+" 你已经不是新手了，不能使用recopy这个指令！");
        return 0;
         }
...
/* copy entire dbase values */
    ...
    me->set("birthday",hp_status["birthday"]);
      ...
      me->set("jh_dj/dj",ob->query("jh_dj/dj"));
        me->set("jh_dj/int",ob->query("jh_dj/int"));
        me->set("jh_dj/dex",ob->query("jh_dj/dex"));
        me->set("jh_dj/str",ob->query("jh_dj/str"));
        me->set("jh_dj/con",ob->query("jh_dj/con"));
    ...
    newob->move("/d/taishan/newplayer1");
    destruct(me);
    return 1;
}
...
int do_recover()
{
    ...
    return notify_fail("你的实力和现在的江湖新秀的实力差太多了吧？\n");
...
return notify_fail("您偷偷的跑去升级了吧,还是先recopy重新存记录吧,要不然不能恢复的?\n");
    ...
    if (me->query("jh_dj/int")!=ob->query("jh_dj/int"))
    return notify_fail("您偷偷的跑去升级属性了吧,还是先recopy重新存记录吧,要不然不能恢复的?\n");
    if (me->query("jh_dj/dex")!=ob->query("jh_dj/dex"))
    return notify_fail("您偷偷的跑去升级属性了吧,还是先recopy重新存记录吧,要不然不能恢复的?\n");
    if (me->query("jh_dj/str")!=ob->query("jh_dj/str"))
    return notify_fail("您偷偷的跑去升级属性了吧,还是先recopy重新存记录吧,要不然不能恢复的?\n");
    if (me->query("jh_dj/con")!=ob->query("jh_dj/con"))
    return notify_fail("您偷偷的跑去升级属性了吧,还是先recopy重新存记录吧,要不然不能恢复的?\n");
    
if( (int)this_player()->query("combat_exp") > 100000000 || (int)ob->query("age")>=18 ||(int)ob->query("mud_age")>345600 ){
        command("say "+ob->query("name")+" 你已经不是新手了，不能使用recover这个指令！");
        return 0;
         }
...
int do_gengxin()
{
    ...
    command("chat 我退出江湖后，新秀使者之位将由江湖新秀大侠暂时接任。\n");
    rm(query_save_file()+__SAVE_EXTENSION__);
    room=new("/d/taishan/npc/xin-xiu");
    room->move("/d/taishan/newplayer1");
    destruct(this_object());
    return 1;
}
