...
#define BUNCH_D         "/d/bangpai/include/bunchd"
...
int main(object me, string arg)
{
        ...
        {
                write("你的人民币好像不够哟！筹集好资金再来吧！\n");
                return 1;
        } 
        /*else if (me->query("rmb") < 50000000)
        {
                write("你这点名望还不够振臂一呼，聚帮结众，还是以后再说吧。\n");
                return 1;
        } else
        if (me->query("rmb") < 80000000)
        {
                write("你现在在江湖上也算是有些名头了，可惜还是不够。\n");
                return 1;
        } else
        if (me->query("rmb") < 100000000)
        {
                write("你再努力努力提升自己的综合评价，能"
                      "够创建帮派的日子也不远了。\n");
                return 1;
        } */
        ...
        {
                write("你江湖经验太少，还是不要丢人现眼了。\n");
                return 1;
        }

        if (me->query("HellZhenPass")!=1)
        {
                write("江湖高手如云，你还是转世后再来吧！\n");
                return 1;
        }

        if (me->query_skill("dodge") < 1000000 || me->query_skill("force") < 1000000
             || me->query_skill("unarmed") < 1000000 || me->query_skill("parry") < 1000000
             || me->query("max_neili") < 100000000 || me->query("max_jingli") < 16000000)
        {
                write("只怕你的武功担不起这么重的担子。\n");
                return 1;
        }
        
        /*if (me->query("age") < 25)
        {
                write("小小年纪就有如此本事，实在难得，江湖需要的就是你这种人才。\n");
                write("只是人心险恶，你还是长大一点再来吧！\n");
                return 1;
        }*/
        ...
        foreach (tob in t)
        {
                ...
                if (mapp(tob->query("league")) || mapp(tob->query("bunch")))
                        return notify_fail("现在你队伍中有人已经加入别的组织了。\n");

				if (tob->query_skill("dodge") < 200000 || tob->query_skill("force") < 200000
					 || tob->query_skill("unarmed") < 200000 || tob->query_skill("parry") < 200000
					 || tob->query("max_neili") < 1000000 || tob->query("max_jingli") < 160000)
				{
						return notify_fail("现在你队伍中有人武功太差，恐怕很难服众吧。\n");
				}
		        
				/*if (tob->query("age") < 16)
				{
						return notify_fail("你居然诱拐幼童加入帮派，这等人品，怎能聚帮结众？\n");
				}*/
        }
        ...
        me->set_temp("pending/team_bunch/id", id);
...
int do_right(object me, object ob)
{
	...
	if (! living(ob))
        ...
        if (sizeof(t) == sizeof(r) && ! sizeof(t - r) && ! sizeof(r - t))
        {
                ...
                id = ob->query_temp("pending/team_bunch/id");
                BUNCH_D->create_bunch(bunch_name, id, type, base, t);
                ob->add("more_money", -100);
                ob->add("rmb", -500000);
                switch (random(3))
                {
                case 0:
                ...
                msg = sort_string(msg, 100, strlen("【帮派】某人："))[0..<2];
