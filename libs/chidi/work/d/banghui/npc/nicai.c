...
//int delate_banghui(string str);

void create()
{
	set_name("尼采",({"ni cai","ni","nicai"}));
	set("long","世纪狂人之一，负责武林帮会(banghui)的建立。\n");
	set("title",RED"巨人"NOR);
	set("gender","男性");
	set("age",30);
        set("per",90);
...
void init()
{
	...
	add_action("delate_banghui","delate");
}
/*
int create_banghui(string str)
{
    ...
    if((string)me->query("banghui/name"))
	{
		tell_object(me,"你已经是"+me->query("banghui/name")+"的人了，不能另创帮会。\n");
		return 1;
	}

	if(!me->query("zhuanshi/times"))
	{
		tell_object(me,RANK_D->query_respect(me)+"还没有转世，暂时无法开帮立会。\n");
		return 1;
	}
	...
	if( me->query("age")< 20)
	{
		tell_object(me,RANK_D->query_respect(me)+"你还太年轻了，暂时无法开帮立会。\n");
		return 1;
	}

	if(me->query("more_money") < 100)
	{
		tell_object(me,RANK_D->query_respect(me)+"，开帮立会需要一百万两黄金，"
				  +"你帐目上的钱还不够。\n");
		return 1;
	}
	if(me->query("rmb") <200000)
	{
		tell_object(me,RANK_D->query_respect(me)+"的人民币存款不够二十万，不能开帮立会。\n");
		return 1;
	}
	if(me->query("combat_exp")<200000000)
	{
		tell_object(me,RANK_D->query_respect(me)+"的实战经验还差"+
                (string)(200000000-me->query("combat_exp"))+"点，暂时无法开帮立会。\n");
		return 1;
	}
    ...
    me->add("rmb",-200000);
	me->set("banghui/name",str);
	me->set("banghui/rank","帮主");
    me->set("banghui/rank_lv",9);
	me->save();
	command("rumor 恭喜"+me->query("name")+"的帮会「"+str+"」创建成功！\n");
	return 1;
}
*/
void greeting(object ob)
{
	...
	if((string)(banghui=ob->query("banghui/name")))
	{
		lingpai=new("clone/misc/lingpai");
		...
		else if(lingpai->query("bangzhu_id")!="???")
		{
			if( lingpai->query("bangzhu_id")==ob->query("id") &&
				...
				tell_object(ob,"尼采一抱拳道：这位"+ob->query("banghui/name")+
					ob->query("banghui/rank")+"，贵帮生意如何？\n");
			else
				tell_object(ob,"尼采道："+ob->query("name")+
					"，贵帮帮主身体可好？\n");
		}
		else
			tell_object(ob,"尼采偷偷在你耳边道：贵帮主失踪多日，"+
                        RANK_D->query_respect(ob)+"只需花一千两黄金就可以买下帮主之位。\n");
		destruct(lingpai);
	}
	else
		tell_object(ob,"尼采对你一抱拳道：这位"+RANK_D->query_respect(ob)+"是来创建帮会的吗？\n");
}
int accept_object(object who,object obj)
{
	...
	if(obj->value()<1000000000)	{
		tell_object(who,"尼采道：至少十万两黄金，不然我如何向上面交待？\n");
		destruct(lingpai);
		return 0;
	}
	...
	lingpai->save();
	destruct(lingpai);
    who->set("banghui/rank","帮主");
    who->set("banghui/rank_lv",9);

	command("chat "+who->query("name")+
		"花了十万两黄金，买下了"+banghui+"帮主之位。\n");
	return 1;
}
