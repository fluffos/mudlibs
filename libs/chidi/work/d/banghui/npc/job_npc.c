...
void create()
{
        ...
        string name1,name2;
        int i,skill,index,index1,index2,nl;
...
//here begin to set_skill of this_object()
		...
		skill += (skill*(me->query_temp("bh_quest/count")+1))/100 + random((skill*(me->query_temp("bh_quest/count")+1)*2)/100);

        if(skill<0)			skill=2000000;
        if(skill<100000)		skill=100000;
		if(skill>3000000)	skill=3000000;
        ...
        set("jiali",me->query("max_neili")/500 + (me->query_temp("bh_quest/count")+1)*5000);
        set("killer","xingyun");
        set("food",500);
        set("water",500);
        set("no_suck",1);
...
void init()
{
        ...
        call_out("check_room",2,this_object());

}

void start_escape(object ob,object me)
{
        call_out("escape_me",720,ob,me);
}
...
void escape_me(object ob,object me)
{
        ...
        if (objectp(me) ) 
        {
            ...
            me->set_temp("bh_quest/killed",ob->query("id"));
...
void die()
{
        ...
        if( killer && random(2000) < 10 )
        { 
           dir = get_dir("/d/banghui/npc/stone/");
           file = "/d/banghui/npc/stone/"+dir[random(sizeof(dir))];
           if( file_size(file) != -1 )
           {
               equip = new(file);
               if( objectp(equip) )
        ...
        {
			reward_exp=10000+random(5000)+level+
				(int)killer->query_temp("bh_quest/count")*2000;
			reward_rmb = ( killer->query_temp("bh_quest/count") + random(killer->query_temp("bh_quest/count")+1) ) * 4 + 5;
			...
			reward_pot=reward_exp/3+random(50)+50;
			if (killer->query("combat_exp") > 10000000 && 
			(query("title")=="神秘杀手" || query("title")=="神秘高手") )   
			{
				reward_exp +=50000+random(10000); 
				reward_pot +=25000+random(5000);
			}
			if (reward_pot<0) reward_pot=0;

			tell_object(killer,HIW"你为帮会除去了障碍，在实战中获得了：\n"+
...
"其中一半自动转入帮会的账户，你为帮会的贡献会有回报的。\n"NOR); 
			killer->add("bh_quest/reward_exp",reward_exp);
			killer->add("bh_quest/reward_potential",reward_pot);
			killer->add("combat_exp",reward_exp);
			killer->add("potential",reward_pot);
			killer->add("money",(reward_gold/2)*10000);
			killer->add("rmb",reward_rmb/2);
			killer->add("score",random(5));
			killer->add_temp("bh_quest/count",1);
			killer->set_temp("bh_quest/killed",me->query("id"));
			killer->set_temp("bh_quest/killed_name",
								me->query("name"));
			killer->set_temp("bh_quest/killed_place_name",
								me->query("place_name"));
			killer->delete_temp("player_quit");
			killer->set_temp("bh_quest/wait_start",time());
			killer->set_temp("bh_quest/have_task",0);

			lingpai = new("clone/misc/lingpai");
			lingpai->create( killer->query("banghui/name") );

			lingpai->add("rmb",reward_rmb/2);
			lingpai->add("money",reward_gold/2);
			lingpai->save();
			destruct(lingpai);

			reward_gold = killer->query("bhgxrest") + (((reward_rmb/2) % 3000) / 3) + ((reward_gold/2) % 15000) / 15;
			killer->add("bhgx", reward_gold/1000);
			killer->set("bhgxrest", reward_gold%1000);
        }
		else if( objectp( killer = find_living(me->query("killer")) ) ) 
        {
			reward_exp=10000+random(5000)+level+
				(int)killer->query_temp("bh_quest/count")*2000;
			reward_rmb = ( killer->query_temp("bh_quest/count") + random(killer->query_temp("bh_quest/count")+1) ) * 4 + 5;
			...
			tell_object(killer,HIW"你为帮会除去了障碍，在实战中获得了：\n"+killer->query_temp("bh_quest/count")+
CHINESE_D->chinese_number(reward_exp) +"点的『实战经验』"+
CHINESE_D->chinese_number(reward_pot)+"点的『潜能』！\n"+
CHINESE_D->chinese_number(reward_gold) +"两的『黄金』"+
CHINESE_D->chinese_number(reward_rmb) +"圆的『人民币』\n"+
"其中一半自动转入帮会的账户，你为帮会的贡献会有回报的。\n"NOR); 
		...
		{
			killer->set_temp("bh_quest/count",0);
            killer->set_temp("bh_quest/have_task",0);
