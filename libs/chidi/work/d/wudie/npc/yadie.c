...
set_skill("taoism", 1200);
        ...
        carry_object(__DIR__"obj/dieyi")->wear();
...
void attempt_apprentice(object who)
{
        ...
        if ((int)who->query("per") < 30 ){         //原来的判断是!=30
                command("say 我舞碟琼山只收容色超群的女子！\n");
                return 0;
        }

/*
        if (who->query("mud_age") > 300 || who->query("age") > 14){
                command("say 我只收刚登陆的女子！\n");
                return 0;
        }
*/
        ...
        who->set("family/family_name","舞蝶仙");
        who->set("family/generation",48);
        who->set("family/enter_time",time());
        who->set("family/master_id","qiong yadie"); 
        who->set("family/master_name","琼雅蝶");
        who->set("family/title","弟子");
        who->set("title",HIG"舞"HIM"蝶"HIR"仙"NOR);
}
void init()
{
        object ob;
