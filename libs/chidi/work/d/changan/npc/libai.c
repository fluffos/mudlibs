       ...
       { 
               ...
               set("gender", "男性"); 
               set("age", 65); 
               set_skill("literate", 15000); 
               set("max_jing",22000000); 
      ...
      { 
              ...
              learn= learn?learn*100:500; 
             if( !who->query_temp("mark/李") ) { 
                      if( ob->value() >= learn ) { 
                              say(this_object()->name()+"点了点头，说道：很好，从今天起你随时可以来问我有关读书识\n" 
                                      "                      字(literate)的任何问题。\n"); 
                              who->add_temp("mark/李", ob->value()/10); 
                              return 1; 
                      } else { 
...
if (times < 1 || times > 1600)
        return notify_fail("学习次数最少一次，最多也不能超过1600次。\n");
              ...
              if( !me->query_temp("mark/李") || me->query_temp("mark/李")<learn){ 
              ...
              if( !my_skill ) { 
                      gin_cost *= 2; 
                      me->set_skill(skill,0);               }               printf("你向%s请教有关「%s」的疑问。\n", ob->name(), to_chinese(skill)); 
           ...
           { 
             ...
             if(me->query_skill("literate ",1) > 9000)
                grade = 22;
        else if(me->query_skill("literate ",1) > 7500)
                grade = 26;
        else if(me->query_skill("literate ",1) > 6000)
                grade = 28;
        else if(me->query_skill("literate ",1) > 5000)
                grade = 30;
        else if(me->query_skill("literate ",1) > 3000)
                grade = 35;
        else
                grade = 35;
        if ( grade > 35 )
             ...
             { 
                     ...
                     me->improve_skill(skill, grade * (10 + random((int)me->query_int() - 9))* times/35); 
            } 
             else 
             { 
