...
LONG
      ...
      set("objects", ([
        ...
        set("item_desc", ([
           "shadai"  :  "一个大沙袋，挂在柱子上，好象有很多用途，几个人在那里拍打(pai)。\n",
	   "paizi" :
		HIY"
※※※※※※※※※※※※※※※※※※
※※※※　　　　　　　　　　※※※※
...
{       mapping fam;
        ...
        message_vision(HIR"$N用力过猛, 一个不小心,戳在墙上!\n"NOR,me);
        me->unconcious();
	return 1;
		}
...
//              me->add("combat_exp",random((int)(me->query("con") / 10)));
//              me->improve("combat_exp", (int)(me->query("con") / 3));
//                    }
        return 1;
}
...
{       mapping fam;
        ...
        if ((int)me->query("jingli") < costj || (int)me->query("qi") < costq){
        message_vision(HIR"$N用力过猛, 一个不小心踢了个空,摔在地上!\n"NOR,me);
        me->unconcious();
		return 1;
		}
        ...
        message_vision("$N运气于腿，向沙袋踢去。\n", me);

        if ( (int)me->query_skill("leg", 1) < 20 && random(10)>6 )
                {  write(HIM"你把沙袋踢的摆来摆去，觉的自己的基本腿法有了提高!\n"NOR);
...
{       mapping fam;
        ...
        if ((int)me->query("jingli") < costj || (int)me->query("qi") < costq){
        message_vision(HIR"$N用力过猛, 一个不留神,抓在自己身上!\n"NOR,me);
        me->unconcious();
		return 1;
		}
        ...
        if ( (int)me->query_skill("claw", 1) < 30 && random(10)>6 )
                {  write(HIM"你不断的抓着沙袋，觉的自己的基本抓法有了提高!\n"NOR);
...
{       mapping fam;
        ...
        if ((int)me->query("jingli") < costj || (int)me->query("qi") < costq){
        message_vision(HIR"$N用力不当，控制不住，一下拍空，摔了一跤!\n"NOR,me);
        me->unconcious();
		return 1;
		}
