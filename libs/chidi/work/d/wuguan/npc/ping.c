...
void create()
{
    set_name("平孝心", ({ "ping xiaoxin", "ping", "xiaoxin" }));
    set("title", "药房管事");
    set("gender", "男性");
    set("long", "他就是医术高超的「杀人神医」平一指的侄儿。可是他性格开朗，爱好行医行善。\n");
    set("age", 35);
    set("int", 30);
...
set("max_jingli",999999999);
set("jingli",999999999);
set("no_get",1);
  set("no_suck",1);
    set("shen_type", 0);
...
void greeting(object ob)
{
	...
	switch(hurt)
	{
		case 1 : message_vision("平孝心满脸关切地对$N说道：这位" + RANK_D->query_respect(ob)
				+ "看来受了点轻伤，我这由上好的丹药，不知是否要买些？\n",ob);
			 break;
		case 2 : message_vision("平孝心对$N说道：这位" + RANK_D->query_respect(ob)+"看来受伤不轻，买些丹药回去吧！\n",ob);
		         break;
		case 3 : message_vision("平孝心伤心地对$N说道：这位" + RANK_D->query_respect(ob)+"伤重垂危，命不久矣，嘿嘿！\n",ob);
		         break;
		default : command("look "+ob->query("id"));
		         break;
        }
      if(!ob->query_temp("fengyaobegin"))
           say( "平孝心高兴地说道：药房正缺人手，这位" + RANK_D->query_respect(ob)
...
int ask_over()
{
 ...
 kar = me->query("kar");
  if (!me->query_temp("fengyaoover")) {
    message_vision("平孝心惊讶的对$N说道：“哦！ 你再开什么玩笑，我这不是养老院。”\n",me);
    return 1;
    }
 ...
 {
    message_vision("平孝心惋惜，对$N说：“你是怎么搞的，乱分药，是想毁了我的药房吗？\n           看在你还努力的份上还是赏你点钱吧。”\n",me);
      exp = 120 + random(20);
mon = new("/clone/money/silver");
    mon->set_amount(exp);
    tell_object(me,HIW"平孝心给了你" + chinese_number(exp) + "两白银。 \n"NOR);
    mon->move(me);
    me->delete_temp("fengyaow");
    return 1;
 }
 	   
        message_vision(" 平孝心惋惜的，对$N说：你是怎么搞的呀，乱分药，是想砸我的招牌吗？\n",me);
        return 1;

    }
   message_vision("平孝心笑着对$N说：“小兄弟还不错嘛，可以可以，这是你应该得的。”\n",me);
      exp = 180 + random(80);
      pot = 100 + random(50);
