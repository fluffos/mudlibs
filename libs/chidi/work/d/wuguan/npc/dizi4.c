//dizi4.c 孙均 by river

#include <ansi.h>
inherit BHNPC;
int ask_wuguan();
int ask_liguan();

void create()
{
        set_name("孙均", ({ "sun jun","sun","jun", "man" }));
        set("title", "襄阳武馆四弟子");
        set("gender", "男性");
        set("age", 21);       
        set("long", "他是万震山的四弟子，相貌和蔼，一天到晚笑呵呵的。\n");
        set("combat_exp", 5000);
    set("shen_type", 1);
        set("attitude", "friendly");   
     
        set_skill("force", 500);
        set_skill("strike", 500);
        set_skill("sword", 500);
	        
 
        set("inquiry", 
                ([
                        "万震山" : "他老人家是我的恩师啊。",
			"离馆" : (:ask_liguan:),
			"武馆" : (:ask_wuguan:),
			"新人武馆" :(:ask_wuguan:),
		]));

        setup();
        carry_object("/d/city/obj/cloth")->wear();
}

void init()
{
        object ob;

        ::init();
        if (interactive(ob = this_player()) && !is_fighting()) {
                remove_call_out("greeting");
                call_out("greeting", 1, ob);
        }
}

void greeting(object ob)
{      
       if( !ob || environment(ob) != environment() ) return;
	   if ( ob ->query("combat_exp") < 5000000){
       command("bow "+ob->query("id"));
       command("say 你不妨四处看看"HIY HBCYN"look"CYN"。地上有什么东西你都可以捡起来"HIY HBCYN"get"CYN"收着。
你可以先查查"HIY HBCYN"i or inventory"CYN"，看你现在身上有些什么。你要不时地查查"HIY HBCYN"hp
"CYN"你的身体状态，要是饿了或渴了就去找些吃的喝的吧。你要是有什么问题可
以问我。"HIY HBCYN"ask sun about 新人武馆，ask sun about 离馆"CYN"。
至于各种指令的具体用法可以参看 "HIY HBCYN"help wuguan"CYN"。"NOR);
	   }
}                               

int ask_wuguan()
{
      object ob;       
	  ob = this_player();
      if (ob ->query("combat_exp") < 5000000){
		  command("say  襄阳武馆是由我师傅“五云手”万震山他老人家一手建立的，当初
创立的目的就在于给一些刚入江湖的新手一些练功和实践的机会。具体的你可以
看看 "HIY HBCYN"help wuguan"CYN" 和 "HIY HBCYN"help newbie"CYN"。"NOR);
		  return 1;
	  }else{
	  command("kick "+ ob->query("id"));
	  command("say 在武馆这么长时间了，你在干吗啊？自己熟悉去吧！\n");
	  return 1;
	  }
}

int ask_liguan()
{
	  object ob;
	  ob = this_player();
      if ( ob->query("离馆")){
	 command("congra "+ob->query("id"));
	 command("chat 有位叫" +ob->query("name")+"的" + RANK_D->query_respect(ob)+"离开武馆开始闯荡江湖了，江湖险恶，万事小心。");		 
	 ob->set_temp("marks/离馆", 1);
	 return 1;
	 }
         else{
	 command("hmm "+ob->query("id"));
	 command("say 这位" + RANK_D->query_respect(ob)+"没有馆主的同意，我可不敢轻易让你离开武馆。");
	 return 1;
	 }
}
