...
void create()
{
        set_name(BLINK""HIM"紫宝石"NOR, ({"bao shi", "baoshi","shi"}));
	set_weight(500);
	if( clonep() )
		...
		set("unit","颗");
                ...
                set("no_put",1);
                set("no_sell",1);
		set("long",HIB"一块发着闪闪紫光的宝石，传说是女娲补天石的碎块,也不知道什么时候落入凡间的。\n"NOR);
	}
	setup();
}
...
int do_enchase(string arg)
{
   ...
   if(!in_obj->query("weapon_prop/damage"))   //防具
  {
       ...
       message_vision(CYN""+in_obj->query("name")+CYN"似乎变得有点神秘莫测了！\n"NOR,me);
       me->set("bellicosity",0);  
       me->add("potential", -1000);
       in_obj->set_weight(in_obj->query_weight()+1000);
       in_obj->set("inset_num",inset_num);
       in_obj->set("no_sell",1);
       in_obj->add("inset_z",inset_num);  //装备Z
       in_obj->set("name",HIM+in_obj->query("name")+NOR);
