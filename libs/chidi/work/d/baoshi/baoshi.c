...
int random_place(string dirss,object bs)
{
        ...
        for(k=0;k<50;k++) { // try 50 times
           ...
           if( file[j][1] > 0 ) {
             strerr = catch(newob=load_object(dirss+"/npc/"+file[j][0]));
             if(strerr || !newob) continue;
