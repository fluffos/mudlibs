...
int do_duanlian(string arg)
{
    ...
    if( (int)me->query("max_neili") < 1000 )
        return notify_fail("你的内力不够，无法锻炼鞭气！\n");
    if( (int)me->query("qi") < 150 )
        return notify_fail("你的气不够，无法锻炼鞭气！\n");
    if( (int)me->query("eff_qi") < 50 )
        return notify_fail("你现在的体力太弱，无法锻炼鞭气！\n");
    if( (int)me->query("eff_jing") < 50 )
        return notify_fail("你现在的精力无法集中，不能锻炼鞭气！\n");
    if( ((int)me->query("potential") - (int)me->query("learned_points"))< 2 )
        return notify_fail("你的潜能不够，无法锻炼鞭气！\n");
write(HIR "你手指鞭脊，一股强大的内力一丝丝的传到鞭上。！\n" + NOR);
    me->add("max_neili",-10);
    ...
    if( w_zhi >= ((w_level + 1) * (w_level + 1)))
    {
        ...
        write(HIY "鞭身忽的一亮，一道金光隐入你的鞭体，不见了。！\n" + NOR);      
  write(HIG "你的鞭的等级提高了。！\n" + NOR);  
        weapon=this_object();
...
{   
    ...
    {
    message_vision(HIR "你大喝一声，一掌擎鞭，一掌猛力击下。只见"
