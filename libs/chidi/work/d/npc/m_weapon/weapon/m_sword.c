    ...
    if (!(me->query("weapon/name")))
{
   ...
   set("wield_msg", ""+HIY+"$N反手拔剑，剑光一闪，"+w_name+""+HIY+"稳稳地握在$N的手中。\n");
   set("unwield_msg", ""+HIY+"$N剑光一闪，一反手，"+w_name+""+HIY+"「唰」的一声已然还鞘。\n");
    init_sword(w_lv * 10 + w_lv2,2);
    }
    setup();
}   
...
write(HIR "你手指剑脊，一股强大的内力一丝丝的传到剑上。！\n" + NOR);
    me->add("max_neili",-10);
...
write(HIY "剑身忽的一亮，一道金光隐入你的剑体，不见了。！\n" + NOR);      
  write(HIG "你的剑的等级提高了。！\n" + NOR);   
