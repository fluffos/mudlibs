...
void create()
{
    ...
    set("inquiry", ([
        "铸镖"  : "师傅命我帮助别人铸镖，但到现在也不见一个人影。谁要铸镖呢？\n",
        "原料"  : "铸镖可用寒丝羽竹，海底金母，千年神木。但效果各不一样。\n",
    ]) );
    setup();
...
{   
    if(!(who->query_temp("biao") ) ){
        message_vision(HIC"师傅没有同意为你铸镖，您请回吧！\n"NOR, who);
        return 0;
    }
    who->delete_temp("biao");
    who->set_temp("m_biao",1);
    ...
    message("vision","侍镖低头检视了一下手中的"+(string)ob->query("name")+
            "点了点头说：\n", who);
    message("vision","。。。好我们现在就开始铸镖\n", who);
    write("请这位"+RANK_D->query_respect(who)+"给镖起名字吧！(zhu 镖的中文名字(使用颜色请参考help nick))\n");
    return 2;
}
int do_zhu(string arg)
{
    ...
    object me;
    me = this_player();
    if(!(me->query_temp("m_biao")) ) 
        return notify_fail("什么?\n");
    if((me->query("weapon/make")) ){
        say("侍镖一脸茫然：您已经有一把自造的武器了，还来干什么？难到不可手么？\n");
        return 1;
    }
    if( !arg )
        return notify_fail("侍镖认真的说：想好镖的名称后再对我说。\n");
    sscanf(arg ,"%s" ,w_name);
    id="my biao";
    if(!w_name)
        return notify_fail("什么?(使用zhu 镖的中文名字\n)");
    me->delete_temp("m_biao");
    ...
    message_vision(me->name()+"沉吟了一会，对侍镖巧声说了几句话。侍镖点了点头。说：好吧！\n",me );
    message_vision("\n侍镖回过身，转向身后的一个巨大的火炉，鼓动真气燃起了熊熊的大火。说：开始！\n",me );
    ...
    message_vision(HIR "只听咯的一声轻响，一柄巨镖从炉中倏然跃起。化作一道青电猛的向$N的前胸刺来！！\n" NOR,me );
    say("侍镖见状大叫：神镖初成，人血以祭！！闪开！\n");
    message_vision("$N只觉得眼前一花，一条白影迅捷无比的挡在了$N的身前。\n",me );   
    message_vision("巨镖透胸穿出，侍镖惨号一声，鲜血溅得你满脸都是！\n",me );
    message_vision(RED "侍镖脚下一个不稳，倒在了地上。侍镖已经奄奄一息了。\n" NOR,me);
    this_object()->set("max_qi",10);
    message_vision("巨镖又飞了起来，飞到半空，当的一声落回到地上。\n炉中的火灭了。一室的劲气化于无形，一切又归于沉寂。\n",me );
    ...
    if ( me->query("weapon_prop/damage") )
    me->set("hecheng/damage",me->query("weapon/damage"));
    //me->delete_temp("weapon");
   */ 
    me->set("weapon/type","镖");
    me->set("weapon/time",make_time);
    sword=new("/d/npc/m_weapon/weapon/m_biao",1);  //生成新镖
    sword->move(this_player());
    message_vision("侍镖摸起地上，带着斑斑血迹还有些烫手的巨镖，说：\n镖。。已。。成。。，侍镖的任务。。。也就完成了。。。。\n",me );
    say("侍镖艰难的说：镖。。您。。收好，我该走了。。。\n");
    message_vision("侍镖说完。倏的便不见了。\n",me);
    this_object()->die();
    return 1;
}
