...
create_family("冶炼厂",2,"弟子");
    ...
    set("inquiry", ([
        "铸刀"  : "老板命我帮助别人铸刀，但到现在也不见一个人影。谁要铸刀呢？\n",
...
{   
    if(!(who->query_temp("blade") ) ){
        message_vision(HIC"老板没有同意为你铸刀，您请回吧！\n"NOR, who);
        return 0;
    }
    ...
    switch ((string)ob->query("name") ) 
    {
        ...
        case "寒丝羽竹": 
            ...
            break;
        default :
            message("vision","这是什么！老板说这不能用来打造兵器！",who);
            return 0;
    }
    message("vision","铸刀工人低头检视了一下手中的"+(string)ob->query("name")+
            "点了点头说：\n", who);
    message("vision","。。。好我们现在就开始铸刀\n", who);
    write("请这位"+RANK_D->query_respect(who)+"给刀起名字吧！(zhu 刀的名字(使用颜色请参考help nick))\n");
    return 1;
}
int do_zhu(string arg)
{
    ...
    if((me->query("weapon2/make")) ){
        say("铸刀工人一脸茫然：您已经有一把自造的武器了，还来干什么？难到不可手么？\n");
        return 0;
    }
    if( !arg )
        return notify_fail("铸刀工人认真的说：想好刀的名称后再对我说。\n");
    sscanf(arg ,"%s" ,w_name);
    id="my blade2";
    if(!w_name||!id)
        return notify_fail("什么?(使用zhu 刀的中文名字\n)");
    ...
    message_vision(me->name()+"沉吟了一会，对铸刀工人巧声说了几句话。铸刀工人点了点头。说：好吧！\n",me );
    message_vision("\n铸刀工人回过身，转向身后的一个巨大的火炉，鼓动真气燃起了熊熊的大火。说：开始！\n",me );
    ...
    say("铸刀工人见状大叫：神刀初成，人血以祭！！闪开！\n");
    message_vision("$N只觉得眼前一花，一条白影迅捷无比的挡在了$N的身前。\n",me );   
    message_vision("巨刀透胸穿出，铸刀工人惨号一声，鲜血溅得你满脸都是！\n",me );
    message_vision(RED "铸刀工人脚下一个不稳，倒在了地上。铸刀工人已经奄奄一息了。\n" NOR,me);
    ...
    me->set("weapon2/type","刀");
    blade=new("/d/npc/m_weapon2/weapon2/m_blade",1);  //生成新刀
    blade->move(this_player());
    message_vision("铸刀工人摸起地上，带着斑斑血迹还有些烫手的巨刀，说：\n刀。。已。。成。。，铸刀工人的任务。。。也就完成了。。。。\n",me );
    say("铸刀工人艰难的说：刀。。您。。收好，我该走了。。。\n");
    message_vision("铸刀工人说完。倏的便不见了。\n",me);
    this_object()->die();
    return 1;
}
