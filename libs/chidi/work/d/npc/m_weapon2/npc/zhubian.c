...
create_family("冶炼厂",2,"弟子");
    ...
    set("inquiry", ([
        "造鞭"  : "老板命我帮助别人造鞭，但到现在也不见一个人影。谁要造鞭呢？\n",
        "原料"  : "造鞭只能用寒丝羽竹。\n",
    ]) );
    setup();
...
{   
    if(!(who->query_temp("whip") ) ){
        message_vision(HIC"老板没有同意为你造鞭，您请回吧！\n"NOR, who);
        return 0;
    }
    ...
    switch ((string)ob->query("name") ) 
    {
        case "寒丝羽竹": 
            break;
        default :
            message("vision","这是什么！老板说这不能用来打造长鞭！",who);
            return 0;
    }
    message("vision","铸鞭工人低头检视了一下手中的"+(string)ob->query("name")+
            "点了点头说：\n", who);
    message("vision","。。。好我们现在就开始造鞭！\n", who);
    write("请这位"+RANK_D->query_respect(who)+"给鞭起名字吧！(zhu 鞭的中文名字(使用颜色请参考help nick))\n");
    return 1;
}
int do_zhu(string arg)
{
    ...
    if((me->query("weapon2/make")) ){
        say("铸鞭工人一脸茫然：您已经有一把自造的武器了，还来干什么？难到不可手么？\n");
        return 2;
    }
    if( !arg )
        return notify_fail("铸鞭工人认真的说：想好剑的名称后在对我说。\n");
    sscanf(arg ,"%s" ,w_name );
    id="my whip2";
    if(!w_name||!id)
        return notify_fail("什么?(使用zhu 鞭的中文名字\n)");
    me->delete_temp("m_whip");
    me->delete_temp("get_orc");
    me->delete_temp("or");
    o_name="寒丝羽竹";
    make_time=NATURE_D->game_time();
    message_vision(me->name()+"沉吟了一会，对铸鞭工人巧声说了几句话。铸鞭工人点了点头。说：好吧！\n",me );
    message_vision("\n铸鞭工人回过身，拿出一把寒气逼人的匕首，交到了$N的手中，自己拿出寒丝羽竹说：开始吧！\n",me );
    ...
    message_vision(HIM "一身精血胶合着汩汩的内气，源源不断的向铸鞭工人手中的寒丝羽竹涌去\n" NOR,me );
    ...
    say("铸鞭工人见状大叫：神鞭初成，人血以祭！！闪开！\n");
    message_vision("$N只觉得眼前一花，一条青影迅捷无比的挡在了$N的身前。\n",me );   
    message_vision(HIR "长鞭透胸穿出，铸鞭工人惨号一声，鲜血溅得$N满脸都是！\n" NOR,me );
    message_vision(RED "铸鞭工人脚下一个不稳，倒在了地上。铸鞭工人已经奄奄一息了。\n" NOR,me);
    ...
    me->set("weapon2/name",w_name+NOR);  //武器的名称
    me->set("weapon2/make",1);       //武器制造后防止再造的标志      
    me->set("weapon2/id",id);        //武器的代号
    me->set("weapon2/lv",1);     //武器等级
    me->set("weapon2/or",o_name);    //记录制造原料
    me->set("weapon2/value",0);          //记录武器升级已有的点数    
    me->set("weapon2/type","鞭");
    me->set("weapon2/time",make_time);
    whip=new("/d/npc/m_weapon2/weapon2/m_whip",1);    //生成新剑
    whip->move(this_player());
    message_vision("铸鞭工人摸起地上，带着斑斑血迹的长鞭，说：\n鞭。。已。。成。。，铸鞭工人的任务。。。也就完成了。。。。\n",me );
    say("铸鞭工人艰难的说：鞭。。您。。收好，我该走了。。。\n");
    message_vision("铸鞭工人说完。倏的便不见了。\n",me);
    this_object()->die();
    return 1;
}
