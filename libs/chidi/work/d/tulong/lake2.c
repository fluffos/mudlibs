...
void boat_come(object me)
{
   ...
   if (!objectp(boat=find_object("/d/tulong/boat")))
   ...
   message("vision","过了半晌，一叶小舟缓缓地从雾中荡出，靠到了湖边。\n",this_object()); 
    boat->set("exits/up","/d/tulong/lake2");
  set("exits/down","/d/tulong/boat");
...
int valid_leave(object me, string dir)
{
   ...
   if (userp(me) && dir == "down") {
      boat=find_object("/d/tulong/boat");
      if (!objectp(boat))
         boat=load_object("/d/tulong/boat");
         inv=all_inventory(boat);
            for(i=0; i<sizeof(inv); i++) {
               ob = inv[i];
               if(living(ob)){
