...
message_vision("$N的头撞在潭壁上，一下子昏了过去！\n", me); 
                call_out("be_saved",8);
                me->unconcious();
                room = find_object("/d/tulong/spirit10");
                 if(!objectp(room)) room=load_object("/d/tulong/spirit10");
                me->move(room);
                break;
                case 2:
message_vision("$N走近深潭探头向里望去。哎呀！一不小心掉了进去！\n", me);
              room = find_object("/d/tulong/underwater");
                if(!objectp(room)) room=load_object("/d/tulong/underwater");
                me->move(room);
                break;
        }
