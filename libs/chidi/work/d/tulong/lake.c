...
湖边一块巨石，上写＂三清湖＂，湖中有一叶扁舟。
LONG
        );
        set("exits", ([ 
     "north" : "/d/tulong/village",
     "east" : "/d/tulong/lake1",
]));
...
int do_yell(string arg)
{
    ...
    if( arg=="船家")
    { 
     if(!this_player()->query_temp("m_success/初级"))          return notify_fail("你是怎么来的？巫师抓的？那可不好。\n"); 

     if(!this_player()->query_temp("m_success/幻影"))          return notify_fail("你没能亲力解开封印，不能渡你过去呢。\n"); 

     if(!this_player()->query_temp("m_success/孽龙"))          return notify_fail("你参加屠龙了吗？或者你没亲手杀了它？这样是不行的。\n"); 
        check_trigger();
        return 1;
    }
    else
