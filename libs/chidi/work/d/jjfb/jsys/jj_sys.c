...
int jjfb(object player)
{
	...
	tell_object(player,HIY"【江湖】正在为你制造中级副本挑战心魔，请稍候......\n"NOR);
...
//  tell_object(find_player("jerry"),sprintf("DEBUG: %2",start_jroom));
		...
		tell_object(player,"Error: 中级副本JOB制造失败，请联系巫师。\n");
//  tell_object(find_player("jerry"),sprintf("DEBUG: %3",start_jroom));
	...
	else
	{
		...
		jjmem->set("jjfb_time",time()+ 5400); //任务结束时间
		jjmem->set("open_time",time());        //任务开始时间
		jjmem->len_1(); //连通地图(小心)
		jjmem->heart(1);  //启动心跳
		call_out("make_jjkiller",60,player);   //一分钟后开始刷怪
	}
}
