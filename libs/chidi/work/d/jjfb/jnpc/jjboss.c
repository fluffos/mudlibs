...
void die()
{
...

	{
		...
		if(exp)
		{
			...
			tell_object(me,HIY"【江湖】恭喜你挑战心魔成功！请拾取物品，镜像将在1分钟后关闭。\n"NOR);
	...
	if(rand < 10) file = PRODUCT->get_random_level_product(1+random(15),ob->query("jh_dj/dj")/10,ob->query("jh_dj/dj")/10);
	else if(rand < 5) file = PRODUCT->get_random_level_product(1+random(15),ob->query("jh_dj/dj")/10,(ob->query("jh_dj/dj")/10)+5);
