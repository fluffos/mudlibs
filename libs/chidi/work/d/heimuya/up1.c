...
{
	set("short", "悬崖");
	set("long", "你终于走出了那片阴深的森林，不由长长地舒了口气。可是，一座\n"
      "陡峭的悬崖又立在了你的面前。悬崖上仿佛有座白色的建筑，可云烟缭绕看不清晰。\n"
      "从悬崖上垂下一条绳索(rope). 看上去象是悬崖上下互通消息的工具。绳索的上面似乎\n"
      "隐约有一个掉篮(basket),悬崖的墙上似乎有一个匣子(xiazi)。\n"
      "    你似乎走入一个雾宅,真不知如何是好!\n" 
        );
    set("exits", ([
    ...
    set("item_desc", ([
        "basket": "那是一个掉篮,看样子有些古怪,似乎象个机关开动脑筋吗,实在不行就先来点脑黄金,嘻嘻!\n",
        "rope"  : "绳子就是绳子,可能是向上的工具,嘻嘻!\n",
        "xiazi" : "这是一个匣子,看样子是一个开关,匣子的侧面有一个插(insert)孔,可能有原因!\n",
    ]));
     setup();
