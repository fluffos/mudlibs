...
void create()
{
        ...
        set_skill("unarmed", 2000500);
        set_skill("jinghun-zhang", 2000500);
        map_skill("unarmed", "jinghun-zhang");
        set_skill("dodge", 2000500);
        set_skill("ghost-steps", 2000500);
...
void die()
{
        ...
        if( objectp(owner_ob = find_player(owner)) ) {
                if( (object)query_temp("last_damage_from") != owner_ob ) {
                        ...
                        tell_object(owner_ob, "你得到了"+chinese_number(bonus/4)+"点经验！\n");
                }
        }
        destruct(this_object());
