...
int accept_object(object who, object ob)
{
       ...
       tell_object(who, "你已经没有判师记录了呀！\n");
       return 1;
       }
    if (ob->query("money_id") && ob->value() >= 100000)
    {
        tell_object(who, "赵六笑眯眯地说到：好吧，每清除一次判师记录要减去30%的经验和30%潜能、并且各门功夫降两级。\n");
...
int do_betray()
{
    ...
    me->add("betrayer", -1);
    exper = (me->query("combat_exp") / 100* 30);
    if(exper <= 0)
        exper = 0;
   poter =  me->query("learned_points")- me->query("potential");
   poter = (poter/100 * 30);
    if(poter >= 0)
        poter = 0;
    if ( !(skill_status = me->query_skills()) )
    {
        me->add("potential", poter);
        me->add("combat_exp", -exper);
