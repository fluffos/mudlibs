...
int ask_job()
{
        ...
        int i, skill = 0;
          quest = (__DIR__"where")->query_quest();
             if (me->query("combat_exp") < 5000000) {
                command("say " + RANK_D->query_respect(me) + "救人没那么容易，你还是把武功练高再来吧，免得把自己也搭里！");
                return 1;
}
        ...
        if (me->query("shaolinjob") == 1) {
                ...
                command("say " +RANK_D->query_respect(me) + "方生大师在『" + quest["short"]             + "』遇险，你快去解救大师吧.\n" NOR);
                me->set("shaolinjob", 1);
...
int ask_fangqi()
{
        ...
        if (me->query("shaolinjob") == 1) {
                command("say 你先下去吧！");
                me->add("potential",-50);
                  me->apply_condition("menpai_busy",2);
                call_out("delete", 1);
                return 1;
                }
}
