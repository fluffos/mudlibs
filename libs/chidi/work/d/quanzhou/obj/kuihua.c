...
void create()
{
        set_name("葵花宝典", ({ "kuihua baodian1", "baodian1" }));
        set_weight(600);
        if( clonep() )
...
int do_du(string arg)
{
        ...
        int pxlevel; 
    int neili_lost;
    if (!(arg=="kuihua baodian1" || arg == "baodian1" ))
    return 0;
    ...
    if ( me->query_skill("pixie-jian", 1) > 74) neili_lost = 10;
        ...
        if( me->query_skill("pixie-jian", 1) > 100&&me->query_skill("kuihua-xinfa", 1) <100){
                write("你研读了一会儿，但是发现上面所说的对你而言都太浅了，可能是你的葵花心法不够。\n");
