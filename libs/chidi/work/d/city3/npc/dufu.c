...
int do_learn(string arg)
{
        ...
        if (me->query_skill("literate",1)<499)
                return notify_fail("你还是先到朱熹那学习吧。\n");
        if( !living(ob) )
        ...
        if((int)me->query("jing") > gin_cost+10 )
        {
                ...
                me->improve_skill(skill, grade * (10 + random((int)me->query_int() - 9))/150);
        }
        else
        {
...
int do_sleep(object ob)
{
        ...
        if (!room=find_object("/d/city3/dufu_sleep"))
                room=load_object("/d/city3/dufu_sleep");
        ob->move(room);
        command("sleep");
