...
{      object obj;
    ...
    if (present("huolong bian",me)) 
        { 
          if ( ob->id("huolong bian") )
         {
                message_vision("$N眼中暴出一丝精芒,对$n说道：你怎么知道……，罢了，罢了！$n也算是一个英雄人物，那么你得到的东西带来了么？\n",this_object(),me);
                me->set_temp("huolonggai1", 1);        
         }      
        }
    if ( ob->id("dragon soul") && me->query_temp("huolonggai1")) 
        {
                message_vision(HIC"$N仰望苍天，双目含泪：神器终于现世了！！\n"
                                NOR+YEL"只见$N右手托着龙元，左手擎鞭奋力向右手甩去，右手顿时"HIR"鲜血淋漓！\n"
                                NOR+YEL"龙元突然爆出一阵"BLINK+HIW"耀眼的光芒"NOR+YEL"，于缠绕其上的火龙鞭"HIC"合二为一！！\n"
                                NOR+YEL"你正惊愕于眼前的奇景时，$N已经颓然倒地。\n"
                                NOR+YEL"$N说道：很好！神器现世，当有"HIR"血祭"NOR+YEL"，我今生能见此宝，死而无憾！\n"
                                NOR+YEL"说罢，$N将火龙鞭塞进$n手中晕了过去……\n"NOR,this_object(),me);
               
                obj=new("/clone/wx/huolong-bian");
                obj->move(me);
                me->delete_temp("huolonggai1"); 
                this_object()->unconcious();
                this_object()->set("qi",-1);
                
                return 1;
        }
      if ( ob->id("dragon soul"))
        {        message_vision("$N大惊道：天下竟有如此不识宝贝之人！！$N一脸奸笑吧龙元塞进内怀口袋……\n",this_object(),me);
                return 1;
        }
    if ( ob->id("han yu") ) 
        {
