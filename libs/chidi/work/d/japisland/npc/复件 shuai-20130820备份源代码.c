...
set("cor",10000);
  set("lunhui_zhuanshi",random(20));
       set("max_kee", 2500000+3000*laa);
       set("max_sen", 2500000+3000*laa);
...
if(random(i)==0)
{
	    if (userp(inv[i]) && random(2)==1)
	    {
		switch (random(4)) { 
		case 0:
		    msg = YEL"$N突然觉得身法越来越凝滞！。\n"NOR;
		    message_vision(msg,inv[i]);
                    if (inv[i]->query_busy()<3)
                    {
                        div=inv[i]->query("no_busy");
                        inv[i]->delete("no_busy");
                        inv[i]->start_busy(2);
                        if(div) inv[i]->set("no_busy",div);
                    }
                    else inv[i]->add("kee",-inv[i]->query("max_kee")/20);
		    break;
		case 1:
		    msg = RED"浑然不觉之间，$N捂着鲜红的衣襟！。\n"NOR;
		    message_vision(msg,inv[i]);
                    inv[i]->add("eff_kee",-inv[i]->query("eff_kee")/20);
                    inv[i]->add("kee",-inv[i]->query("kee")/20);
		    break;
		case 2:
		    msg = BLU"$N心头一震，猛然喷出一口鲜血！。\n"NOR;
		    message_vision(msg,inv[i]);
                    inv[i]->add("eff_sen",-inv[i]->query("eff_sen")/20);
                    inv[i]->add("sen",-inv[i]->query("sen")/20);
		    break;
		case 3:
...
{
    int gin,kee,sen,i,j;
    int mykills,expg,potg,scoreg,moneyg,mexpg,mpotg,mscoreg,mmoneyg;
    object room;
    object *inv;
    string reward_msg;
    string info_msg;
...
int m;
    ...
    //
      if (environment()) environment()->set_temp("wokoureset",-10);
    CHANNEL_D->do_sys_channel("info",sprintf("%s，盘踞小岛多年的%s被我们无数的灭寇志士击杀了。", NATURE_D->game_time(),name()));
reset_eval_cost();
    inv = children("/obj/user");
    if (sizeof(inv))
    {
	j=0;
	for (i=0;i<sizeof(inv);i++)
	{
	    room = environment(inv[i]);
	    if (!room) continue;
	    if ((string)room->query("indoors")=="japan") j++;
	}
	if (j==0) j=1;
	expg = EXPBONUS/j;
	potg = EXPBONUS/j/3;
	...
	for (i=0;i<sizeof(inv);i++)
	{
	    room = environment(inv[i]);
	    if (!room) continue;
	    if ((string)room->query("indoors")=="japan")
	    {
		message_vision(HIM"倭寇首领被打败了，$N撤出了倭寇小岛！\n"NOR,inv[i]);
		inv[i]->move("/d/city/kezhan");
		mykills=50;
		//
						if (inv[i]->query_temp("jap_badguy/bing1")>200) inv[i]->set_temp("jap_badguy/bing1",200);
		if (inv[i]->query_temp("jap_badguy/bing2")>180) inv[i]->set_temp("jap_badguy/bing2",180);
		if (inv[i]->query_temp("jap_badguy/bing3")>160) inv[i]->set_temp("jap_badguy/bing3",160);
		if (inv[i]->query_temp("jap_badguy/bing4")>130) inv[i]->set_temp("jap_badguy/bing4",130);
		if (inv[i]->query_temp("jap_badguy/jiang1")>40) inv[i]->set_temp("jap_badguy/jiang1",40);
		if (inv[i]->query_temp("jap_badguy/jiang2")>25) inv[i]->set_temp("jap_badguy/jiang2",25);
		mykills+=inv[i]->query_temp("jap_badguy/bing1");
		mykills+=inv[i]->query_temp("jap_badguy/bing2");
		mykills+=inv[i]->query_temp("jap_badguy/bing3")*2;
		mykills+=inv[i]->query_temp("jap_badguy/bing4")*5;
		mykills+=inv[i]->query_temp("jap_badguy/jiang1")*20;
		mykills+=inv[i]->query_temp("jap_badguy/jiang2")*40;
		...
		tell_object(inv[i],reward_msg);
		inv[i]->add("combat_exp",mexpg);
		inv[i]->add("potential",mpotg);
		inv[i]->add("daoxing",mscoreg);
if(inv[i]->query("balance")<1800000000)
		inv[i]->add("balance",mmoneyg*10000);
		inv[i]->delete_temp("jap_badguy");

		//
						info_msg =inv[i]->query("name")+"参与剿灭倭寇，奖励" +
		chinese_number(mexpg) + "点实战经验，"+
		chinese_number(mpotg) + "点潜能，" +
		chinese_number(mscoreg) + "点道行，" + 
		chinese_number(mmoneyg) + "两黄金。";
	...
	info_msg =HIC+killer->query("name")+HIY"英勇击毙倭寇首领，额外奖励" +
	HIR+chinese_number(mexpg) +HIY "点实战经验，"+
	HIR+chinese_number(mpotg) + HIY"点潜能，" +
	HIR+chinese_number(mscoreg) + HIY"点道行，" + 
	HIR+chinese_number(mmoneyg) + HIY"两黄金。"NOR;
//	CHANNEL_D->do_sys_channel("info",info_msg);


 message("channel:mp",HIY"【大唐战报】："+HIM+killer->name()+HIY"在灭寇大战中击杀"HIW"倭寇终极首领--"+BLINK+HBRED"月牍勋帅"NOR+HIY+"!!\n"+info_msg+"\n"NOR,users());
