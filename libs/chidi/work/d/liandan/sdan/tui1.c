...
void create()
{
        set_name(HIB"活血止痛汤"NOR, ({ "qufu shengjisan","shengjisan","san"}));
        set_weight(100);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("long", "一颗活血止痛汤,听说是治疗腿伤的圣药!\n");
                set("unit", "颗"); 
                set("no_get",1);
                set("no_s",1);             
                set("no_put",1);
...
{ 
  ...
  if (me->query("daoxin/dex")>=3) {
    ...
    tell_object(me,HIC"你感到你的腿伤似乎好点了!\n"NOR); 
    } else {
    me->add("dex",1);
    me->add("daoxin/dex",-1);
    tell_object(me,HIG"你感到你的腿伤似乎好点了!\n"NOR); 
}
}
    me->add("combat_exp",random(10000));
    me->add("potential",random(5000));

    message_vision(HIY"$N拿出活血止痛汤吃了下去,感觉身上一阵暖洋洋的,有一股说不出的舒服。\n"NOR,me);
