...
{
	string *skill_name = ({
   "martial-cognize",          //赵客缦胡缨
               "sword",           //吴钩霜雪明
               "dodge",           //银鞍照白马
               "throwing",        //飒沓如流星
               "pixie-jian",      //十步杀一人 sword
       "kuihua-mogong",//千里不留行 dodge
      "bluesea-force",     //事了拂衣去 dodge
  "never-defeated",   //深藏身与名 dodge
...
//         message_vision("$N正在钻研石壁上的图案和注释。\n",me);
	for (i = 1; i<=times; i++)
	{
                me->receive_damage("jing",jing_cost/10);
		...
		if (item == 4)
if ((string)me->query("gender") != "无性"||(int)me->query_skill("pixie-jian",1)<450||(int)me->query_skill("pixie-jian",1)>10000 )
                {
			{
 write("可是壁上是辟邪剑法图谱，欲练神功，必先自宫!\n");
		...
		if((int)(me->query_skill(skill_name[item],1) * 
			me->query_skill(skill_name[item],1) * 
                     1 ) >= me->query("combat_exp")&&(string)skill_name[item]!="dodge")
		{
		...
		if (learn_pot > 0)
		{
			if(item == 22)
			{
                       me->improve_skill(skill_name[item], random((int)me->query("kar"))+(10 + random((int)me->query_int() - 9))/450);
			me->add("qi",-random(me->query("kar")));
			success=1;
		}
		...
		if(item == 4)
			{
  me->improve_skill(skill_name[item], random((int)me->query("kar"))+(10 + random((int)me->query_int() - 9))/500);
//                         me->add("qi",-random(me->query("kar")));
			success=1;
		}
		else if(me->query("combat_exp") < 1000000000)
		{
       
               me->improve_skill(skill_name[item], random((int)me->query("kar"))+(10 + random((int)me->query_int() - 9))/10);
//  me->add("qi",-random(me->query("kar")));
			success=1;
		}
		else if(me->query("combat_exp") > 1000000000)
       ...
       me->improve_skill(skill_name[item], random((int)me->query("kar"))+(10 + random((int)me->query_int() - 9))/5);
//     me->add("qi",-random(me->query("kar")));
			success=1;
		}
