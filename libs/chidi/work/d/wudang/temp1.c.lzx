// Room: /d/wudang/chashi.c// By Marz 04/01/96
#include <ansi.h>
inherit ROOM;
...
要送来上好的茶叶。这茶叶由少女在三更至日出间采摘，并放在怀中
用体温焙制，得天地精华，汇处子体香，故名＂女儿香＂，据说喝了
能提精补神，十分珍贵。屋里四周得体地摆着些桌子(table)和椅子
(chair)，坐满了武当的男女弟子。南边有扇窗子(window)，景色宜
人。
LONG
);
    ...
    set("item_desc", ([
        ...
        "chair" : "一只青竹打制的靠椅，躺(sit)上去摇摇晃晃，好舒服耶！\n",
...
{
    if (!objectp(present("xiao cui", this_object()))){
        me->delete_temp("tea_cup");
        return ::valid_leave(me, dir);
    }
    if (  (dir == "north") && ( present("xiang cha", this_player())   && present("mi tao", this_player()) )        && objectp(present("xiao cui", environment(me))) )   switch ( random(2) )    {
    case 0:      return notify_fail     ("小翠把嘴一撇：吃饱了喝足了还不够，临走怀里还揣上一些，小女子也替您脸红呢！\n");
     break;
    case 1:  message_vision("小翠对$N道了个万福：张真人吩咐，饮食不得带出茶房。", me);
     return notify_fail("\n");
     break;
    }       me->delete_temp("marks/sit");
    me->delete_temp("tea_cup");
