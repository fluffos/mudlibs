...
([      "title1":               "镇南王府弃仆",
        ...
        "prepare1":             "finger",
        "exert1":               "powerup",
      //  "perform1":             "finger.qiankun",
     //   "perform2":             "finger.sandie",
        "weapon":                __DIR__"obj/changjian",
]),
([      "title1":               "天龙寺恶僧",
        "title2":               "天龙寺长老",
...
//here begin to set_skill of this_object()
		...
		skill += (skill*(me->query_temp("bh_quest/count")+1))/2 + random((skill*(me->query_temp("bh_quest/count")+1)*2)/2);

        if(skill<0)			skill=500;
        if(skill<1000)		skill=1000;
        if(skill<2000)	     skill=2000;
        if(skill<3000)	     skill=3000;
        if(skill<4000)	     skill=4000;
        if(skill>4000)	     skill=4000;
...
void init()
{
        ...
        if(this_object()->query("jiali")==0)
			set("jiali", (int)this_object()->query_skill("con")/20);
...
void die()
{
        ...
        {
			reward_exp=50000+random(100000)+level+
				(int)killer->query_temp("bh_quest/count")*2000;
			reward_rmb = killer->query_temp("bh_quest/count")*3 + random(killer->query_temp("bh_quest/count")*2) ;
			reward_gold = reward_rmb + random(reward_rmb * 50);
			if(killer->query("combat_exp") < 100000 )
				reward_exp -=reward_exp/50;
			if(killer->query("combat_exp") > 1000000)
				reward_exp +=reward_exp/30;

			reward_pot=reward_exp+random(5000)+5000;
			if (killer->query("combat_exp") > 10000000 && 
			(query("title")=="神秘杀手" || query("title")=="神秘高手") )   
			{
				reward_exp +=40000+random(50000); 
				reward_pot +=5000+random(5000);
			}
			if (reward_pot<0) reward_pot=0;

			tell_object(killer,HIW"你为恶人谷除去了障碍，在实战中获得了：\n"+
...
CHINESE_D->chinese_number(reward_rmb) +"圆的『人民币』\n"NOR); 
			...
			killer->add("money",reward_gold*100);
		...
		else if( objectp( killer = find_living(me->query("killer")) ) ) 
        {
			...
			(query("title")=="神秘杀手" || query("title")=="神秘高手") )   
			{
				reward_exp +=30000+random(50000); 
				reward_pot +=500+random(500);
			}
			if (reward_pot<0) reward_pot=0;
