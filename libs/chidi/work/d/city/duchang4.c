...
int do_sai (string arg)
{
  ...
  if(ob->query("base_value") > 10000000)
  {
    return notify_fail("不能押这种钱。\n");
  }

  //if(amount > (10000000/ob->query("base_value")))
 // {
   // return notify_fail("由于经营不善，现在最高只能押1000两黄金。\n");
  //}
...
void gamble_prepare ()
{
  ...
  tell_room (room,"青鬏龟童说：请各位大人赏押，一赢二。\n");
  room_status = 1;
