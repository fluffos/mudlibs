...
if (times < 1 || times > 1600)
       ...
       /* if ((sscanf(arg, "%s from %s", skill,teacher)!=3 ))
                if ((sscanf(arg, "%s %s", teacher, skill)!=3 ))
        ...
        if (ob->query("jing<30"))
{
       ...
       // my_combat_exp = (int)me->query("combat_exp");
        ...
        if((int)me->query("jing") > gin_cost+10 )
        {
                ...
                me->improve_skill(skill, grade * (10 + random((int)me->query_int() - 9)) * times/20);
        }
        else
        {
