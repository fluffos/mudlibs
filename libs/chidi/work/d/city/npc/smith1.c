...
if (me->is_busy()) 
return "你现在正忙。"; 
if (me->query("combat_exp") > 1800000000) //检查EXP是否大于8000万
return "让您老干这个未免屈尊了吧？"; 

if (me->query("jh_dj/zhuan") > 9) //检查是否大于九转
return "让您老干这个未免屈尊了吧？"; 
...
//coin = new("/clone/money/gold"); 
//coin->set_amount(2 + random(10)); 
//coin->move(this_player()); 
//message_vision("$N对$n道：这是给你的工钱。\n", 
//this_object(), me); 
if (me->query("combat_exp") < 1800000000) 
{
me->add("potential", random((int)me->query_int())); 
me->add("combat_exp", random((int)me->query_int())); 
} 
