...
int do_chai(string arg)
{
  ...
  if (!me->query(THIS_PAR + "_chai"))  {
    ...
    if (jp >= 10) {
       ob->set_amount(20);
       ob->move(me);
        me->set("max_neili", 500);
         me->set("max_jingli",500);
         me->set("potential",300000);
    ...
    if (jp < 3) {
       ...
       if (me->query_skill("literate",1) < 100 ) me->set_skill("literate",me->query_skill("literate",1) + 200 ); 
   ...
   message_vision(HIC "$N小心翼翼的拆开礼物包，只见一阵烟雾过后，$N顿时隐隐约约感到自己身体有了一些变化。\n" NOR);
       tell_object(me,HIG "黑客在你的耳边悄声说道：拿去混吧！小网虫！\n");
//删除上次派礼物生成的参数，增加本次参数，确保每位玩家只能用一次。
      ...
      tell_object(me,HIC "你小心翼翼的拆开礼物包，只见一阵烟雾过后，你顿时隐隐约约感到自己身体有了一些变化。\n" NOR);
        ...
        remove_action("do_chai","chai");//如果不将chai移除,会引起当机
