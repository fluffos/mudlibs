...
void create()
{
        ...
        set_weight(5000);
        set("unit", "把");
        set("long", "一把晶莹透明的弯刀，以大雪山凌霄城中千年不化的寒冰制成。\n");
        set("value", 8);
        set("material", "ice");
        set("wield_msg",
...
void melt()
{
        ...
        if( env->is_character()&&env->query("id")!="corpse" )
        {//a player or a NPC.
...
"$N运力一震，只见冰花四溅！一把冰刀顿时化为无数碎片向$n激射而去！\n" NOR;
        ...
        {//here only check this if previously failed.
                ...
                {
                        if( target->query("eili")/40 > (10+me->query("neili_factor")) )
        ...
        if(success == 1 )
        {
                ...
                message_vision(msg, me, target);
                target->receive_wound("qi", random(3*me->query("force_factor"))+50, me);
