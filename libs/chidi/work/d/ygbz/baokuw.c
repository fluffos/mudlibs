...
墙有一个放RMB币的箱子(xiangzi)!
LONG
        ); 
        set("item_desc", ([       
"xiang" : "箱子里刻着一些字符，好象是一样武功，你不由想仔细看(think)一下!\n",          
   "xiangzi" : "箱子里有很多钱,你要拿出来吗??\n",      
...
int do_pick(string arg)
{
 ...
 if(me->query_temp("RMB_money"))
return notify_fail("你已经拿过钱了,不好意思再拿了!\n");
      if( arg=="xiangzi" )
                             {  
if(query_temp("nasword")){ 
   message_vision("$N想去拿RMB币,发现已经被人拿了\n", this_player());
     return 1;
 }
                                message_vision(
HIY"$N在箱子里拿出RMB币!\n"NOR, this_player());
...
me->set_temp("RMB_money",1);
 message("channel:chat", HBRED HIB"【系统】"+me->query("name")+"在杨公宝库里发现了大量的RMB币了!\n"NOR,users()); 
return 1;
                }
        return 1;
}
...
int do_push(string arg)
{
  ...
  message_vision(HIG"$N轻轻推开墙壁，发现有道暗门!\n"NOR,me);
   set("exits/west", "/d/changan/ximen"); 
