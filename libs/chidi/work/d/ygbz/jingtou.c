...
if(query("mingzhu")==6)
{
 message("channel:chat", HBRED HIB"【系统】"+me->query_temp("name")+"因为太贪心,拿光了夜明珠，被机关踢出宝库了!\n"NOR,users());
  message_vision(HIC"突然$N的脚底下出现一个洞，$N掉了下去。\n"NOR, this_player());
  this_player()->unconcious();
      me->move("/d/changan/ca");
}
}
void check_tian()
{
...
delete("tian");
delete("di");
me->delete_temp("jhdi");message_vision(HIB"$N用力按下，发出「的」一声脆响。\n"NOR, this_player()); 
return 1;
} 
if (me->query_temp("jhok")){ 
...
HIR"$N拿下了一颗夜明珠。\n"NOR, this_player());
                        ...
                        zhu->move(me);  
add("mingzhu",1);
 message("channel:chat", HBRED HIB"【天涯】"+me->query_temp("name")+"在杨公宝库里偷拿了一颗夜明珠!\n"NOR,users());
  check_mingzhu();
return 1;
                }
                message_vision(
...
me->delete_temp("tian");me->move("/d/changan/ca");
this_player()->unconcious();
   call_out("on_quxiao", 1);
return 1;
 } 
