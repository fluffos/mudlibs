...
if(query("mingzhu")==6)
{
  message("channel:chat", HBRED HIW"【天涯】"+me->query("name")+"因为太贪心,拿光了夜明珠，被机关踢出宝库了!\n"NOR,users());
...
HIR"$N拿下了一颗夜明珠。\n"NOR, this_player());
                        ...
                        zhu->move(me);  
add("mingzhu",1);
message("channel:chat", HBRED HIB"【天涯】"+me->query("name")+"在杨公宝库里偷拿了一颗夜明珠!\n"NOR,users());
  check_mingzhu();
return 1;
                }
                message_vision(
