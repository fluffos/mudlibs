...
{
 if(me->query_temp("ssl_jh")+5 > time())
  return notify_fail("您正在吸取圣舍利的内力呢。\n");
}
me->set_temp("ssl_xi",1);
me->set_temp("ssl_jh",time());
...
if (!me->query("jhygbz_ssl"))  { 
  if (random(6)>=3) 
  me->set("jhygbz_ssl","daoxin");
 else 
 me->set("jhygbz_ssl","zhanshen");
}
  switch(me->query("jhygbz_ssl"))
  {
  case "daoxin":   
