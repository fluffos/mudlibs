...
int do_order()
{
      ...
      if( ilist[i]->is_character() && ilist[i]->query_temp("chousha") )
      ilist[i]->delete_temp("chousha");

      if(me->query_temp("chousha"))
      {
       message("channel:rumor",HIB"【江湖仇杀】风闻"+me->query("name")
       +"在仇杀中临阵脱逃，仓皇离开了紫金城！\n"NOR,users());
       me->delete_temp("chousha");
       me->add("score",-1000); 
      }
