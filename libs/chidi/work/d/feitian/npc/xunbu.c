...
void create()
{
	...
	set("chat_chance",40);
        set("chat_msg", ({
			"带刀警察从身上掏出要犯画像，正仔细的和你比较。\n",
			"带刀警察小声嘀咕着：这年头只要花银子，啥事不好办？\n",
                (: random_move :),
        }) );
	set("inquiry", ([
	"通缉" : "去找包大人问问看吧。\n",
...
int do_steal()
{
	object me;
	me=this_player();
	message_vision(CYN"带刀警察“哼”的冷笑一声：居然有人敢在我面前干从偷鸡摸狗的事！\n"NOR,me);
...
int do_kill(string str)
{
	...
	if(npc==this_object())	{
message_vision(CYN"带刀警察对着$N长叹一声：我早已厌倦了江湖上的恩恩怨怨，别逼我了。\n"NOR,ob);
	return 1;
			}
	return 0;
}
...
void greeting(object ob)
{
	...
	if(ob->query("oldsix_flag/tongji")==1)
			{
	message_vision(RED"\n带刀警察冲着$N怒喝一声："+ob->query("name")+"！哪里跑！！跟我回去见官！！\n"NOR,ob);
