...
void create()
{
        set_name(NOR + HIW "刚猛拳脚丹" NOR, ({ "quanjiao dan", "quanjiao", "dan" }));
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", HIW "这是一颗通体乌黑的药丸，据说吃了可以使你拳脚更加刚猛。\n" NOR);
                set("unit", "颗");
                set("no_give", 1);
                set("no_drop", 1);
                set("no_put", 1);
                ...
                set("gift_type", "unarmed");
                set("gift_name", "基本拳脚");
                set("gift_point", 15);
                set("gift_msg", HIG "突然间你只觉全身充满了力量，好像有使不完的力气。\n" NOR);
        }
        setup();
}
int do_eat(string arg)
{
		...
		if(me->query_skill("unarmed",1) > 20000)
		{
			return notify_fail(HIR "你的手脚已经够利索了，吃再多也是无用。\n" NOR);
		}
