...
void create()
{
        set_name(NOR + HIW "炼骨丹" NOR, ({ "liangu dan", "liangu", "dan" }));
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", HIW "这是一颗通体发黄的药丸，据说吃了可以使你体壮如牛。\n" NOR);
                set("unit", "颗");
                set("no_give", 1);
                set("no_drop", 1);
                set("no_put", 1);
...
int do_eat(string arg)
{
		...
		if(me->query_skill("force",1) > 20000)
		{
			return notify_fail(HIR "你的体质已经够强壮了，留些给体质弱的吃吧。\n" NOR);
		}
