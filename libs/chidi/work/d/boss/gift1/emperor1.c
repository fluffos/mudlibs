...
void create()
{
        set_name(NOR + HIR "真命天子丹" NOR, ({ "emperor dan", "dan" }));
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", WHT "这是颗天界里最为珍贵的宝物，据说可以增加你的先天特技-真命天子(10%成功率)。\n" NOR);
                set("unit", "颗");
                set("no_give", 1);
                set("no_drop", 1);
                set("no_put", 1);
                set("no_get", 1);
                set("value", 10000);
                set("weight", 50);set("merit", 1000);//价值

                set("only_do_effect", 1);
                set("gift_type", "emperor");
                set("gift_name", "真命天子");
                set("gift_point", 10);
                set("gift_msg", HIM "突然你头部出现一道七彩霞光，随后自己化身成一条青龙直冲九重天。\n" NOR);
        }
        setup();
}
...
int do_eat(string arg)
{
	 ...
	 return notify_fail("你要吃什么？\n");
        ...
        // 已经是真命天子不能吃
       if (this_player()->query("special_skill/emperor"))
       return notify_fail("你已经拥有这项技能了。\n");


        // 已经是天煞孤星不能吃
       if (this_player()->query("special_skill/lonely"))
       return notify_fail("你已经拥有这项技能了。\n");
       ...
       if (this_player()->query("special_skill/lucky"))
        ...
        } else
        {
                ...
                tell_object(this_player(), HIC "你获得先天技能--" + query("gift_name") +
                                HIC "\n" NOR);
                ...
                // 增加相应的天赋属性
                this_player()->set("special_skill/"+ query("gift_type"), 1);
        }
