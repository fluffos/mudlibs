...
void create()
{
        set_name(HIY "九转吸阳丹" NOR, ({ "xiyang dan", "xiyang", "dan" }));
        if (clonep())
                ...
                set("gift_type", "maxsuckcond");
                set("gift_name", "九转吸阳大法");
                set("gift_point", 20);
                set("gift_msg", HIY "冥冥中你似乎已经领悟了“九转吸阳大法”。\n" NOR);
        }
        setup();
}
...
/*
        if (this_player()->query("gift/" + query("gift_type") + "/all") >= 10)
        {
...
*/
        ...
        } else
        {
                ...
                tell_object(this_player(), HIC "恭喜你暂时领悟了" + query("gift_name") +
                                HIC "。\n" NOR);
