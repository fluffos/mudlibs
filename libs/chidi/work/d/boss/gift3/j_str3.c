...
void create()
{
        set_name(HIY "冒牌蚩尤诛元仙丹" NOR, ({ "zhuyuan xiandan", "zhuyuan",
                                            "xiandan", "dan" }));
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", HIY "传说此丹乃是天宫中的神品，凡人吃了可以减少膂力，呵呵！假货来的。\n" NOR);
                set("unit", "颗");
                set("no_give", 1);
                set("no_drop", 1);
                set("no_put", 1);
                set("no_get", 1);
                set("value", 100000);
                set("weight", 50);set("merit", 20);//价值

                set("only_do_effect", 1);
                set("gift_type", "str");
                set("gift_name", "先天膂力");
                set("gift_point", 85);
                set("gift_msg", HIY "突然间你只觉双臂几乎萎缩，力量减少了不少。\n" NOR);
        }
        setup();
}
