...
void create()
{
        set_name(NOR + HIW "冒牌神恩通慧仙丹" NOR, ({ "tonghui xiandan", "tonghui",
                                                  "xiandan", "dan" }));
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", HIW "传说此丹乃是天宫中的神品，凡人吃了可以平增悟性，呵呵！冒牌货。\n" NOR);
                set("unit", "颗");
                set("no_give", 1);
                set("no_drop", 1);
                set("no_put", 1);
                set("no_get", 1);
                set("value", 100000);
                set("weight", 50);set("merit", 100);//价值

                set("only_do_effect", 1);
                set("gift_type", "int");
                set("gift_name", "先天悟性");
                set("gift_point", 85);
                set("gift_msg", HIG "突然间你只觉灵台处一片混黜，奇痛无比。\n" NOR);
        }
        setup();
}
int do_eat(string arg)
{
	...
	return notify_fail("你要吃什么？\n");
        ...
        } else
        {
                ...
                tell_object(this_player(), HIC "你的" + query("gift_name") +
                                HIC "永久减少一点。\n" NOR);
                ...
                // 增加相应的天赋属性
                this_player()->add(query("gift_type"), -1);
        }
