...
void create()
{
       ...
       set_skill("changge-jianfa",10000);
...
void attempt_apprentice(object ob)
 {  if (ob->query_skill("yuxuan-guizhen",1)>50)   
    if (ob->query_skill("shuiyue",1)>500)
    if (ob->query_skill("langzi-sword",1)>500)
    if (ob->query_skill("qimen-wuxing",1)>500)
    if (ob->query_skill("jiuyin-baiguzhao",1)>500)
    if (ob->query_skill("shenkong-xing",1)>500)  {
        command("nod");
...
................唉！你听进去没有呀\n");
 ...
 else command("say 唉,我哪有能力收人做徒弟呢.....\n");
 return;
 }
