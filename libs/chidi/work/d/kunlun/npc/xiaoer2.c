...
他老人家曾以一手满天花雨连败几十位武林高手。\n",
        ...
        set("vendor_goods", ({
           ...
           set_skill("force", 8500);
           set_skill("dodge", 8500);
           set_skill("parry", 7500);
           set_skill("yan-shuangfei",7000);
           set_skill("ding-unarmed",7000);
           set_skill("unarmed", 7500);
...
int do_name(string target)
{
   ...
   if (base_name(environment())!=query("startroom")){
    ...
    /*if (dest->query("age")<=18)
        return notify_fail(me->name()+"用鄙夷的目光看着你：老子不杀无名之辈，哼...... \n"); */
