...
void create()
{
        ...
        set_skill("force", 3000);             // 基本内功
        set_skill("kumu-shengong", 3000);
        set_skill("duanshi-shenfa", 3000);
        set_skill("finger", 3000);
        set_skill("sun-finger", 3000);
        set_skill("unarmed", 3000);           // 基本拳脚
        set_skill("dodge", 3000);             // 基本躲闪
        set_skill("parry", 3000);             // 基本招架
...
void attempt_apprentice(object ob)
{
        ...
        if ((int)ob->query_skill("force",1) < 1000 ) {
           command("say 你的基本内功太低了,还是努努力先提高一下吧!");
        return;
        }
