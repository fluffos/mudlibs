...
void  pay_player(object  me,  int  total)
{
        object  pay_tgold, pay_cash,  pay_gold,  pay_silver;

        if(total/10000  >  0)
        {
                pay_tgold  =  new("/clone/money/thousand-gold");
                pay_tgold->set_amount  (total/10000);
                total  -=  10000*pay_tgold->query_amount();
                pay_tgold->move(me);
        }
...
{//player  pay  money  out,  in  unit  of  silver.
  ...
  //then  destroy  all  his  cash,  gold,  silver...then  pay  him  back  the  difference.
	...
	//now  destroy  this  players  cash,  gold  and  silver.
        tgold  =  present("thousand-gold_money",  me);
        cash  =  present("thousand-cash_money",  me);
	gold  =  present("gold_money",  me);
	silver  =  present("silver_money",  me);
        if(  tgold  )  destruct(tgold);
...
{//this  function  check  the  money  in  silver  a  player  has,
	...
	silver  =  present("silver_money",  me);
	total_silver  =  0;
        if(  tgold  )  total_silver  +=  10000*(tgold->query_amount());
