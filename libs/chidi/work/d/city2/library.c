...
string look_shelf();
#define STORY_TOC   "/doc/poem/doc"
#define STORY_DI "/doc/poem/"
inherit ROOM;
void create()
{
	set("short", "性文学图书馆");
...
{
        if (arg == "" || file_size(STORY_DI + arg) <= 0)
                return notify_fail("我们没有这本书。\n");
	switch (MONEY_D->player_pay(this_player(), 92222222)) {
	        case 0:
	...
	log_file("LIBRARY", sprintf("%s read %s.\n", this_player()->query("name"), arg));
        this_player()->start_more(read_file(STORY_DI + arg));
	return 1;
}
