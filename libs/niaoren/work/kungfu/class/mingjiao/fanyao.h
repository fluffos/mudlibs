...
//                "職位不合，不夠資格闖光明聖火陣。";
	...
	fighter->set_temp("xuannan-asked", 1);
        ...
        say("\n范遙說道：好吧，我來召集般若堂長老們于西練武場集合，我稍候在中央練武場上等你。\n");

	me->set("assigned_fighter", fighter->query("id")); 		
	
	if( !fighter->query_temp("xuanku-asked") )
        say("\n范遙又道：請你速去告知羅漢堂楊左使，請他即刻招集般若堂諸羅漢于西練武場集合。\n");
        ...
        for(j=1; j<10; j++) 
	{
        ...
        me->move("/d/mingjiao/dadian");
                        message_vision("\n范遙走了過來。\n\n", fighter);
                        return "真是對不起，般若堂中有人不在，無法舉行光明聖火陣。\n";
		}

                message("vision", "\n范遙走了過來，跟" + monk->query("name") + "俯耳輕聲說了几句。\n", room, monk);
...
int waiting(object me)
{
	...
	if( !objectp( fighter = present( me->query("assigned_fighter"), environment(me) ) ) )
	{
        ...
        else if( !present("yang xiao", environment(me)) || !fighter->query_temp("xuanku-asked") )
	{
		if( random(10) == 0 ) 
...
int preparing(object me, object fighter)
{
	...
	room = load_object("/d/shaolin/wuchang");
        ...
        say( "范遙說道：不相干的人，如不欲現場觀摩光明聖火陣，請即刻離開此處。\n");
...
int do_recruit(object me, object fighter)
{
	...
	{
                say("\n范遙說道：好，好，好！" + fighter->query("name") + "，你愿意拜我等為師嗎？\n");
