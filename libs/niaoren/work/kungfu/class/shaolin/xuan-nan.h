...
string ask_me()
{
	...
	say("\n玄難說道：好吧，我來召集般若堂長老們于西練武場集合，我稍候在中央練武場上等你。\n");

	me->set("assigned_fighter", fighter->query("id")); 		
	
	if( !fighter->query_temp("xuanku-asked") )
	say("\n玄難又道：請你速去告知羅漢堂玄苦大師，請他即刻招集般若堂諸羅漢于西練武場集合。\n");
        ...
        for(j=1; j<10; j++) 
	{
		...
		{
	        	me->move("/d/shaolin/luohan");
        	        message_vision("\n玄難大師走了過來。\n\n", fighter);
			return "真是對不起，般若堂中有人不在，無法舉行羅漢大陣。\n";	
		}

                message("vision", "\n玄難大師走了過來，跟" + monk->query("name") + "俯耳輕聲說了几句。\n", room, monk);
...
int do_recruit(object me, object fighter)
{
	...
	{
	        say("\n玄難說道：好，好，好！" + fighter->query("name") + "，你愿意拜我等為師嗎？\n");
...
int do_back(object me)
{
	...
	message("vision", "\n玄難大師也揮了揮手， 般若堂長老們隨即也離開練武場。\n", environment(me) );
        ...
        room1->set("exits/south", "/d/shaolin/mile" );
        room1->set("exits/north", "/d/shaolin/zoulang");
...
{   int i = random(sizeof(titles));
	...
	if ( ob_fam["generation"] == (my_fam["generation"] + 1) && name[0..1] == "澄")
	{
		...
		command("recruit " + ob->query("id"));
              ob->set("title",YEL"少林般若堂座下 "NOR + titles[i] );
	}
