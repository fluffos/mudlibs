...
int do_name(string target)
{
	...
	if(me->query("waiting_target"))
		return notify_fail("李四嘿嘿一笑：我現在正忙著哪，您呆會兒在來吧！\n");
	
	if( !this_player()->query_temp("fee_paid") ) 
		return notify_fail("李四說道：不管叫我做什麼，您可得先付錢哪！\n");

	if( !target || target==" ") 
		return notify_fail("李四惡狠狠地說道：快告訴我那家伙的名字，我可沒功\夫聽你閑聊！\n");
	...
	if( !dest )
		return notify_fail("李四一聲冷笑：他現在不在游戲中，過會再來吧。\n");
		
	if (dest->is_ghost())
		return notify_fail("李四很不高興：他已經是個死鬼了，你還來找我干什么\！\n");
...
int do_back(object me)
{
	...
	message("vision", "李四走了進來，拍了拍身上的塵土，把劍上的血跡抹拭干淨，\n"
		"他接著一笑說道：老天爺保佑，活兒干得干淨利落，您老以後有什麼事還可以找我。\n", 
		environment(), me );
