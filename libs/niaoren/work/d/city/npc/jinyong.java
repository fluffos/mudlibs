...
void create()
{
        ...
        set("age", 65);
        set("long",
            "這是一位顯得略胖的教書匠風度的慈祥老者，早年筆耕不惰，如今早已功\成名就。\n");
        set("combat_exp",1);
        set("class","scholar");
        set("shen_type", 1);
...
int ask_me()
{
        ...
        command("chat 好，現在天下第一擂台開擂，全體集合!\n");
        ...
        tell_object(this_player(),"金庸告訴你: 可以用setname somebody 指定某人為當前勝者。\n"+
...
int do_bihua(string arg)
{
        ...
        if(wizardp(ob1)) return notify_fail("巫師瞎摻合個什么\勁啊? \n");
...
int do_setcan(string arg)
{
        ...
        if(!arg||!objectp(ob=present(arg,environment(this_player()))))
                return notify_fail("金庸說: 對不起，此人不在這里。\n");
        set("candidate",ob);
        command("nod");
...
int do_setname(string arg)
{
        ...
        if( !dest )
                return notify_fail("金庸說：對不起，此玩家不在這里。\n");
        if (dest->is_ghost())
