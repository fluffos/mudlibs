...
void create()
{
        ...
        set("no_get",1);
        set("no_steal",1);
        set("no_drop","你的婚約不能丟棄,只能去紅娘庄解除婚約。\n");
        if( clonep() )
...
int do_put(string arg)
{
        ...
        {
          tell_object(me,"這是你最珍貴的東西，怎么\可以放到那兒去？\n");
          return 1;
        }
}
...
int do_cemote(string arg)
{
        ...
        if( !couple_ob )
          return notify_fail("你的伴侶現在聽不見你，或者已經離開游戲了。\n");
        ...
        if (environment(me) == environment(couple_ob ) )
        {
           if (arg == "kiss")
           {
              write( MAG "你擁住你的愛侶，深深的一吻，許\久...許\久...\n" NOR);
              tell_room(environment(me), CYN+(string)me->name()+
...
//        me = this_player();
        ...
        if (couple_ob)
        {
             if ((string)couple_ob->query("gender") != "女性")
             {
              ...
              write(sprintf( MAG "你的%s也在這里，快去找啦...\n"NOR,tmpstr1));
