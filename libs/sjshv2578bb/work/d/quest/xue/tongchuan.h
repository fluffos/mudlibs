...
*/
  ...
  switch (random(3)) {
    case 0: msg = "朝廷急召「" + me->query("name") + "」进殿，你速去「" + me->query("place") + "」通传。";
    break;
    case 1: msg = "朝廷有要事和「" + me->query("name") + "」商量，你马上到「" + me->query("place") + "」去传令一下。";
    break;
    case 2: msg = "皇上思念旧友「" + me->query("name") + "」，你帮忙去从「" + me->query("place") + "」将他找过来吧。";
    break;
  }
  ...
  me->set("owner", ob);
  me->move(ob);
  if (ob->query("id") == "kuku") ob->move(ob->query_temp("tongchuan/where")); ^ M return msg + "\n";
}
