...
void check_yingzhan(object me, object target, string faname) {
  if (environment(target) != this_object()
    ...
    tell_object(
      target,
      GRN "你耳边传来" + SHILI_D->query_master(faname) + "的声音：" + RANK_D->query_rude(target) + "，居然对本掌门不理不睬！\n" NOR
    );
...
void alternative_die(object me) {
  ...
  me->set("kee", 1);
  me->set("sen", 1);
  if (me->query("eff_kee") < 10) me->set("eff_kee", 10);
  if (me->query("eff_sen") < 10) me->set("eff_sen", 10);
  me->remove_all_killer();
  all_inventory(environment(me))->remove_killer(me);
  if (me->query("questguai")) { destruct(me); return; }
