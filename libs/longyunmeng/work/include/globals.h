...
// nosave/protected/private compatibility shim neutralized: this FluffOS
// build predefines __SENSIBLE_MODIFIERS__, so the #else branch (aliasing
// `private` to `protected`) was the one actually active, silently
// collapsing private's distinct semantics. All three are real, correctly-
// behaving, independent keywords on this driver and need no translation
// at all (see AGENTS.md §15z).
...
#undef XYZX_SYS_JULI
/************数据安全Start***********/
#define DATAKEY_FILE "/data/key"
#define DATAKEY_NAME ".key"
/************数据安全 End ***********/
