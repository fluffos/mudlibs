	...
	string *skill_name = ({
		...
		for (i = 1; i<=times; i++)
		{
			...
			if ( item == 4 || item == 22 )
			{
				...
				// 限制辟邪等级上限
				if ( item == 4 && me->query_skill("pixie-jian", 1) >= 690 )
				{
			...
			if( (int)(me->query_skill(skill_name[item],1) * me->query_skill(skill_name[item],1) * 81 ) >= me->query("combat_exp") )
			{
			...
			if (learn_pot > 0)
			{
				...
				{
					if ( me->query("kar") >= 32 && me->query_kar() >= 63 )
					{
						me->improve_skill(skill_name[item], random((me->query_kar()/2 + me->query_int()/2)/100)+1, 1);
						me->add("qi", -learn_pot);
						success = 1;
