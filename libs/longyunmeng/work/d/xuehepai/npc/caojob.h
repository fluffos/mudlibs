int ask_job()
{
        ...
        quest = "/d/Xuehepai/quest/xhquest"->query_quest();
...
void do_datan(object me)
{
        ...
        if (me->query_temp("followed"))
                    return;
        if (file_name(where)==me->query_temp("where"))
        {
                if (!(present("di zi",where)))
                {
                        tell_object(me,HIC"你等了一会，没有人影，可能弟子已经被杀了。\n"NOR);     
