...
ES2/Annihilator 系架构衍生的武侠 mudlib，"泥潭"系列的一支，共
13000+ 个 `.lpc` 文件，属性存储代码为独立编写、并非与 nt6/nt1 分
支同源。同样设有"出生仪式"关卡（`d/register/` 区域，`regroom.lpc`
等文件名甚至和 nt6/nt6nitan6win 完全相同），但具体文案与实现各自
独立——直接比对内容可确认不是同一份代码，而是"泥潭"系里多个团队
各自独立开发出的相近设计范式，地图规模也和 nt6 系相近。
