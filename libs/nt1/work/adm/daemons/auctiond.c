                                                ...
                                                } else
                                                {
                                                        ...
                                                        default:                                                                                                                        
                                                                ...
                                                                tell_object(me, "你把" + obj->short() + "交人带给"
                                                                                "了" + ob->name() + "。\n");
                                                                tell_object(me, "你的拍卖所得已经存入你的银行帐户。\n");
...
public void add_auction(object me, object ob, int money)
{
        ...
        tell_object(me, "你开始拍卖" + ob->short() + NOR "，目前" + 
                        ...
                        "你需要付出" + msg : "不需要付出") + "佣金。\n");
        message_auction(sprintf("%s(%s)开价%s拍卖%s。", me->name(),
...
public void decide_auction(object me, string player)
{
        ...
        if (player == id )
        {
                tell_object(me, "你把自己的物品拍卖给自己？\n");       
                return;
        }
        
        if (! objectp(who = find_player(player)))
        {
                tell_object(me, "此人已经离开游戏。\n");
                return;
        }
        ...
        {
                tell_object(me, "此人并没有参与竞标或已经退出竞标。\n");
                return;
        }
        ...
        tell_object(me, "你决定把" + name + "拍卖给" + who->query("name") + "(" + player + ")。\n");  
              
        message_auction(sprintf("%s(%s)决定将%s的%s拍卖给%s(%s)。", me->name(), id,
                                gender_pronoun(me->query("gender")), name, who->query("name"), player, ));
                                
        switch (MONEY_D->player_pay(who, money))
        {
        case 0:
        case 2:
                if (who->query("balance") < money)
                {                                                        
                        tell_object(me, "但是" + who->name() + "付不起钱"
                                        "，只好作罢。\n");
                        tell_object(who, "但是你付不起钱，只好作罢。\n");
                        map_delete(auction_info, id);
                        return;
                } else
                {
                        ...
                        default:                                                                                                                        
                                ...
                                tell_object(who, "你收到了" + me->name() + "送来的" +
                                                 ob->short() + "。\n");
                                tell_object(me, "你把" + ob->short() + "交人带给"
                                                "了" + who->name() + "。\n");
                                tell_object(me, "你的拍卖所得已经存入你的银行帐户。\n");
                                me->add("balance", money);
                                ob->move(who);
