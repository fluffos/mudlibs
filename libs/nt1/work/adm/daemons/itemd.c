...
// 防具升级
void armor_upgrade(object me, object item)
{
	int defense_lvl, points;
        mapping o;
        string my_id;
        ...
        if (my_id == item->item_owner())
                item->add("magic/blood", 1); 

        message_vision(HIY + item->short() + HIY + "忽的一亮，一道金光隐入$N的" + item->short() + HIY + "，不见了！\n" NOR, me);
        message_vision(HIG "$N的" + item->short() + HIG + "的等级提高了！\n" NOR, me);
}
