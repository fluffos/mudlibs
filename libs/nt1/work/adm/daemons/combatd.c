...
#define DEFAULT_MAX_PK_PERDAY           4
...
varargs int skill_power(object ob, string skill, int usage, int delta)
{
        ...
        if (level < 1)
	{
        ...
        if (level > 500)
                power = (level / 10) * (level / 100) * level;
        else
                power = level * (level / 10) * (level / 100);

	power += valid_power(dbase["combat_exp"]);
        // power += dbase["level"] * dbase["level"] * dbase["level"];

        if (usage == SKILL_USAGE_ATTACK)
        {
                power = power / 30 * (dbase["str"] + (temp ? temp["str"] / 10 : 0));
                if (fight && ob->is_fighting())
                         power += power / 1000 * fight["attack"];
        } else
        {
                power = power / 30 * (dbase["dex"] + (temp ? temp["dex"] / 10 : 0));
                if (fight && ob->is_fighting())
                         power += power / 1000 * fight[skill];
        }
        return power;
}
...
void killer_reward(object killer, object victim)
{
        ...
        if (playerp(victim))
        {
                ...
                // adjust combat exp.
                vs["combat_exp"] -= (vs["combat_exp"] / 100) * 2;
