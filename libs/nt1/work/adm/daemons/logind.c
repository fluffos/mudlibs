...
void temp_ban_ip(string ip, int time, string reason);
...
void logon(object ob)
{
        ...
        if (mapp(temp_time = temp_ban[query_ip_number(ob)]))
        {
	...
	login_usr = filter_array(children(LOGIN_OB), (: query_ip_number($1) == $2 :), query_ip_number(ob));
        ...
        if (sizeof(login_usr) > 20)
        {
                log_file("static/ip_attack",
                        sprintf("%s %s attack system by fake login.\n",
                                log_time(),
                                query_ip_number(ob)));

                temp_ban_ip(query_ip_number(ob), 1800, "您的IP产生了大量废弃连接！");
        ...
        if (iplimit > 1)
                printf("您所在的地址已有 " HIY "%d" NOR " 位玩家在线上。\n",
                       iplimit-1);
        */
        write(sprintf("目前共有 " CYN "%d" NOR " 位巫师、"
                      CYN "%d" NOR " 位玩家在线上，以及 "
                      CYN "%d" NOR " 位使"
                      "用者尝试连线中。\n\n", wiz_cnt, ppl_cnt, login_cnt));
               
        if (! ob->query_temp("big5"))
                write("Select 国标码 GB or き絏 BIG5 (" HIR "gb" NOR "/" HIY "big5" NOR ")：\n");
        else
...
private void get_id(string arg, object ob)
{
        ...
        if (! check_legal_id(arg))
        {
                ...
                {
                        log_file("static/ip_attack",
                                sprintf("%s %s attack system use fake id.\n",
                                        log_time(),
                                        query_ip_number(ob)));
                        temp_ban_ip(query_ip_number(ob), 300, "您的IP持续登录无效ID！");
                        destruct(ob);
                        return;
                }
        ...
        } else if (file_size(ob->query_save_file() + __SAVE_EXTENSION__) >= 0)
        {
                ...
                write("您的人物储存挡出了一些问题，请利用 guest 登陆通知巫师处理。\n");
                destruct(ob);
                return;
        }
...
private void get_wizpwd(string pass, object ob)
{
        string old_pass;
...
private void confirm_password(string pass, object ob)
{
        ...
        write(HIW "性格对一个人拜哪个门派并没有影响，但是它会影响学某些武功，在\n"
                  "除了某些特殊情况对性格有要求外，性格在战斗中还有各自特点，如\n"
                  "光明磊落：具有最大的愤怒值和加怒能力，还具有更好的抗打击能力\n"
                  "狡黠多变：使用绝招时忙状态短一些，还能更短时间内连续使用绝招\n" 
                  "心狠手辣：可以运用愤怒技能，另外在命中对手时具有更强的杀伤力\n"
                  "阴险奸诈：攻击时命中率更高，能让你更轻松的命中和伤害攻击对象\n\n" NOR);
        write("您要扮演角色的性格类型？\n"
              "[" HIC "1" NOR ". 光明磊落  "
                  HIC "2" NOR ". 狡黠多变  "
                  HIC "3" NOR ". 心狠手辣  "
...
private void get_character(string character, object ob)
{
        int n;
        if (character == "")
                n = 1;
        else
                sscanf(character, "%d", n);

        if (n < 1 || n > 4)
        {
                write ("对不起，您只能选择1-4，按下回车直接选择光明磊落。\n\n");
                write("您要扮演角色的性格类型？(4)\n");
                input_to("get_character", ob);
                return;
        }
        ...
        {
        case 1:
                ob->set_temp("character", "光明磊落");
                break;
        case 2:
                ob->set_temp("character", "狡黠多变");
                break;
        case 3:
                ob->set_temp("character", "心狠手辣");
                break;
        case 4:
                ob->set_temp("character", "阴险奸诈");
                break;
        }

        write("您选择了" HIC + ob->query_temp("character") + NOR "的性格角色。\n\n");
...
private void get_gender(string gender, object ob)
{
        ...
        user->set("per", 20);
        user->set("kar", 20);
        user->set("character", ob->query_temp("character"));
...
void temp_ban_ip(string ip, int time, string reason)
{
        ...
        lonely["reason"] = reason;
        lonely["time"] = time();
        lonely["punish"] = time;
        map_delete(temp_ban, ip);
        temp_ban[ip] = lonely;
...
void remove_temp_ban(string ip)
{
        if (ip == "all")
                temp_ban = ([]);
        else
