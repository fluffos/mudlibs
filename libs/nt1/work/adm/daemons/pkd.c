...
private void restore_status(object me)
{
	...
	me->remove_all_killer();
        j = (int)me->query_condition("putizi-drug");
        k = (int)me->query_condition("pill-drug");
	me->clear_condition();
	...
	me->delete_temp("block_msg/all");
        if (j > 0)
        me->apply_condition("putizi-drug", j);
        if (k > 0)
        me->apply_condition("pill-drug", k);
        if (! mapp(cnd)) return;
        ks = keys(cnd);
