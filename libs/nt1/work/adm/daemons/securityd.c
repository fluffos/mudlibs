...
string get_status(mixed ob)
{
        ...
        if (euid == "lonely" || euid == "ken") return "(admin)";
...
int valid_write(string file, mixed user, string func)
{
        ...
        if (func == "save_object")
        {
                ...
                if (sscanf(file, "/data/business/%*s")) return 1;
        }

        if (base_name(user) == "/kungfu/condition/junquest_song" &&
            func == "write_file") return 1;

        if (sscanf(file, "/data/board/%*s")) return 1;
...
int valid_read(string file, mixed user, string func)
{
        ...
        if (sscanf(file, "/data/%*s")) return 1;

        if (sscanf(base_name(user), "/feature/%*s")) return 1;

        if (func == "file_size") return 1;

        if (base_name(user) == "/kungfu/condition/junquest_song" &&
            func == "read_file") return 1;
