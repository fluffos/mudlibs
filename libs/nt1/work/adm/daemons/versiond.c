...
// 版本同步代理进程所在的服务器的IP地址
#define AGENT_IP        ({ "127.0.0.1", "61.128.162.81", "202.102.16.5", "218.70.159.117", "218.70.159.149" }) 
...
// 不需要构造的目录
static string *exclude_dir = ({ "/adm/etc", "/adm/simul_efun", "/adm/tmp",
...
int generate_version()
{
        ...
        if (sizeof(socket_info) > 0)
                return notify_fail("目前正在有站点和本站同步版本，你不能构造版本。\n");
...
private void cmd_getver(int fd, string arg)
{
        ...
        if (arg != "dbase")
        {
                ...
                {
                        sys_info(sprintf("%s正在读取版本号，准备同步版本。", mudn));
