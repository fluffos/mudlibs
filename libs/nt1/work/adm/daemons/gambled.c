...
private void get_type(string arg, object me)
{
        ...
        case "D": 
                ...
                items = keys(item_data);
                // 必须是已经停止的项目才行
                items = filter_array(items, (: query_status($1) == STOPPED :));
...
private void select_name_tochip(string arg, object me)
{
        ...
        if (sizeof(user_data[me->query("id")][name]) >= 2)
        {
...
// 选择投注的队伍
public void select_team_tochip(string arg, object me, string name)
{
            ...
            ! n = sizeof(teams))
        {
                write("\n输入格式错误，请重新输入。\n");
                write("请输入你要投注的队伍[多个用“" WHT "," NOR "”隔开]：");
                input_to("select_team_tochip", me);
                return;
        }
        
        all_teams = item_data[name]["subject"];
