...
// Last modified by jjgod@FYTX For 「侠客新传」邮件注册
...
#define MUD_WEB                 "http://www.chinesemud.org/" 
#define PUBLIC_MUD_MAIL         "nitan@chinesemud.org" 
...
protected string mailname = "lonely", mailpasswd = "921121";

protected mixed content = ([]);

/* 这里设定你的 SMTP 服务器的域名和 IP 地址 */
protected string domain_name = "localhost", address = "127.0.0.1";
...
void send_mail(object user,string mail)
{
	...
	msg += sprintf(user->name()+"，你好，感谢您光临"+MUD_NAME+"网络游戏，这是一个虚拟世界。\n");
	msg += "\n在这里你可以尽情的聊天----超出聊天室里数倍的速度和更多的动词。\n";
	msg +=   "在这里你可以成为一代天骄----可以一展雄才伟略，开拓你自己的事业。\n";
	msg +=   "在这里你可以拥有自己的家园----与虚拟世界中你的爱妻和爱子幸福生活。\n";
	msg +=   "在这里你可以了解各地人文历史----在游戏中可以游览全国各地的名山大川。\n";
	msg +=   "在这里你可以游览于神奇武侠世界----可以与武侠小说中的人物一起并肩战斗。\n";
	msg +=   "朋友，让我们一起体验这个美丽的世界吧！\n"; 
	msg += "\n泥潭网络游戏主页 http://www.chinesemud.org/\n";
	msg +=   "泥潭网络游戏论坛 http://bbs.chinesemud.org/\n";
...
protected void success_register(int fd)
{
	...
	(mailmsg->user)->stop_busy();
	map_delete(content,fd);
	tell_object(usr,sprintf("给您的一封邮件已根据您登记的地址发往：" HIW "%s" NOR "
...
祝您在%s玩的愉快！\n", mailmsg->rcpt, PUBLIC_MUD_MAIL, MUD_NAME));
	...
	// usr->set("registered", 1);
	// usr->delete("new_begin");
	link->save();
	usr->save();
	tell_room(environment(usr),"你只觉得眼前一花，"+usr->query("name")+"不见了。\n",({usr}));
        message("channel:sys",sprintf(HIR"【邮件注册】"HIW"%s(%s)注册邮件发出成功。\n"NOR,
...
protected int mail_have_reg(object user,string mail)
{
	...
	if(member_array(mail,mail_reg)!=-1)
	{
		tell_object(user,"这个邮件地址已经注册过了，本游戏不允许同一地址多重注册。\n对不起！\n");
                message("channel:sys",sprintf(HIR"【邮件注册】：%s(%s)注册请求被拒绝。\n"NOR,
