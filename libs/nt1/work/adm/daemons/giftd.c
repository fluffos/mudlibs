...
varargs public void bonus(object who, mapping b, int flag)
{
        ...
        contribution = b["contribution"] * percent / 100;

        /*
        // 玩家经验太高，削弱奖励。
        if (who->query("combat_exp") >= 80000000)
        {
                exp /= 4;
                pot /= 4;
                mar /= 4;
        } else
        if (who->query("combat_exp") >= 50000000)
        {
                exp /= 3;
                pot /= 3;
                mar /= 3;
        } else
        if (who->query("combat_exp") >= 30000000)
        {
                exp /= 2;
                pot /= 2;
                mar /= 2;
...
varargs public void gift_bonus(object who, mapping b)
{
        ...
        if (! stringp(temp) || ! who->query(temp))
        {
                ...
                if (contribution > 0) msg += chinese_number(contribution) + "点门派贡献、";
