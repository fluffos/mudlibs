...
{ 
        ...
        exp = b["exp"] * percent / 50; 
        pot = b["pot"] * percent / 25; 
        mar = b["mar"] * percent / 50;
        shen = b["shen"] * percent / 50; 
        score = b["score"] * percent / 50; 
        weiwang = b["weiwang"] * percent / 50; 
        contribution = b["contribution"] * percent / 50; 
        ...
        if (contribution > 0) msg += chinese_number(weiwang) + "点门派贡献、"; 
        ...
        who->add("contribution", contribution); 
} 
...
int accept_object(object me, object who, object ob)
{
	...
	mixed special = 0;      // 是否有特殊奖励
        ...
        { 
                ...
                exp = 200 + random(100) + lvl * 60; 
                pot = 60 + random(30) + lvl * 20; 
                mar = 20 + random(10); 
                weiwang = 20 + random(10) + lvl / 2; 
                score = 50 + random(30) + lvl / 2; 
                contribution = 50; 
                ...
                {
                        // 连续完成了超过300次的任务
                        exp += 300 + random(quest_count / 2 + 1);
                        pot += 100 + random(quest_count / 4 + 1);
                        mar += 50 + random(20);
                        weiwang += 20 + random(20);
                        score += 30 + random(15);
                        contribution += 20 + random(20); 
                } else 
                if (quest_count >= 200) 
                { 
                        // 连续完成了超过200次的任务 
                        exp += 200 + random(quest_count / 2 + 1); 
                        pot += 60 + random(quest_count / 4 + 1); 
                        mar += 20 + random(10); 
                        weiwang += 10 + random(10); 
                        score += 20 + random(10); 
                        contribution += 10 + random(10); 
                } else 
                if (quest_count >= 100)
                {
                        // 连续完成了超过100次的任务
                        exp += 100 + random(quest_count / 2 + 1);
                        pot += 35 + random(quest_count / 4 + 1);
                        mar += 10 + random(5); 
                        weiwang += 5 + random(5);
                        score += 10 + random(5);
                        contribution += 5 + random(5);
                } else
                if (quest_count >= 10)
                {
                        // 连续完成了超过10次的任务
                        exp += 50 + random(quest_count / 2 + 1);
                        pot += 25 + random(quest_count / 4 + 1);
                        mar += 5 + random(3);
                        weiwang += 3 + random(3);
                        score += 5 + random(5);
                        contribution += 3 + random(3);
                }
                ...
                if (quest_count == 100)
                {
                        msg += CYN "$N" CYN "哈哈大笑两声道：“真有你的，连着上百"
                               "次任务都完成的漂漂亮亮，很好，很好。”\n" NOR;
			special = 1;
                } else
                if (quest_count == 200)
                {
                        msg += CYN "$N" CYN "大喜道：“不错不错，真实不错，两百"
                               "次任务干得很好。”\n NOR";
			special = "/clone/gift/puti-zi";
                } else
                if (quest_count == 300)
                {
                        msg += CYN "$N" CYN "叹道：“真是长江后浪推前浪，想不到你接连三百"
                               "次任务都无一疏漏，不易，不易！”\n" NOR;
			special = "/clone/gift/tianxiang";
                } else
                if (quest_count == 400)
                {
                        msg += CYN "$N" CYN "深深叹了口气，道：“想不到你连着四百次"
                               "任务无一失手，看来我们" + me->query("family/family_name") +
                               "真是后继有人啊！”\n";
			special = "/clone/gift/jiuzhuan";
                } else                
                if (quest_count >= 500)
                {
                        msg += CYN "$N" CYN "深深叹了口气，道：“想不到你连着五百次"
                               "任务无一失手，看来我们" + me->query("family/family_name") +
                               "将有宗师出世拉！”\n" NOR;
        ...
        bonus(who, ([ "exp" : exp, "pot" : pot, "mar" : mar,
                      "weiwang" : weiwang, "score" : score, "contribution" : contribution ]), 1);
...
{ 
        ...
        case "kill": 
                ...
                // adjust gongxian 
                dbase["contribution"] -= c; 
                if (dbase["contribution"] < 0) 
                        dbase["contribution"] = 0; 
                msg += HIR "，师门对你的信任降低了"; 
