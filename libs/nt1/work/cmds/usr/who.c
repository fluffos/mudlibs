...
mixed main(object me, string arg, int remote)
{
	...
	string who_id;
        ...
        if (opt_party)
        {
                ...
                ob = filter_array(ob, (: $1->query("bunch/bunch_name") ==
                        $2->query("bunch/bunch_name") :), me);
        }
	...
	if (objectp(vob))
        ...
        if (opt_zhuanshi)
                ob = filter_array(ob, (: $1->query("zhuanshi") :));
        ...
        {
                if (me->query("bunch/bunch_name"))
                        str += HIR + " (" + me->query("bunch/bunch_name") + ") " + NOR WHT;
			...
			if (! stringp(who_id))
				who_id = "#" + geteuid(list[i]);
                        name = sprintf("%s%s%-10s(%-12s",
                                        PREFIX(list[i]),
                                        wizardp(list[i]) ? HIY : list[i]->query("gender") == "女性" ? HIM : NOR WHT,
                                        list[i]->name(1),
                                        capitalize(who_id)+")");
