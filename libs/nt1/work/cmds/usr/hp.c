...
int main(object me, string arg)
{
           ...
           || sscanf(arg, "%s -m", arg)))
        {
                ...
                if (wizardp(me) || me->query("couple/child"))
                {
                        ...
                        if (! wizardp(me) && me->query("couple/child") != ob->query("id"))
                                return notify_fail("你要察看谁的状态？\n");
                ...
                exp = ob->query("combat_exp") / 1000;

                lv = ob->query("level");
                if (lv < 1) lv = 1;
                need = (lv + 1) * (lv + 1) * (lv + 1) * 1000 - ob->query("combat_exp");
