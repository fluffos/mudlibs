...
int main(object me, string arg)
{
	...
	dodge_points = COMBAT_D->skill_power(ob, "dodge", SKILL_USAGE_DEFENSE);
        ...
        line += sprintf( YEL "┃" HIW "【帮派】" NOR "%-18.18s", stringp(ob->query("bunch/bunch_name"))?ob->query("bunch/bunch_name"):"自由人士");
        ...
        if (stringp(ob->query("bunch/title")))
                line += sprintf( YEL "┃" HIW "【职务】" NOR "%" + sprintf("%d",18 + color_len(ob->query("bunch/title"))) + "-s", ob->query("bunch/title"));
        else
        ...
        */
        ...
        line += sprintf( YEL "┃" NOR HIY " 武学宗师： %s" NOR HIY "    任督二脉： %s" NOR HIY "   元婴出世： %s" NOR HIY "    转世重生：%s " NOR YEL "┃\n" NOR,
                ultrap(ob) ? "○" : HIC "×", ob->query("breakup") ? "○" : HIC "×", ob->query("animaout") ? "○" : HIC "×", ob->query("zhuanshi") ? "○" : HIC "×" );
