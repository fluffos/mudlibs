...
int main(object me, string arg)
{
        ...
        else
        {
                ...
                if (sscanf(arg, "%d", i))
                {
                        ...
                        if (arrayp(analectas = ANALECTA_D->query_analecta_list()[sprintf("%d", year)]) &&
...
选，即输入 analecta 2000 3。 使用 new 参数可以阅读你尚未阅
读的新文选。
巫师可以使用 new 参数来添加文选，del 参数来删除文选。 删除
文选需要指定年度和序号，添加文选则必须指定所在房间的留言板
ID，如：
    客栈留言板(board) [ 4 张留言 ]
使用 analecta add 1 from board，就可以把客栈留言板的第一号
留言添加到本年度文选中。
