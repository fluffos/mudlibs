...
void do_over(object me, object target)
{
        ...
        if (target->query("id") == me->query("couple/id") 
        &&  me->query("id") == target->query("couple/id") 
        &&  ! target->query_condition("huaiyun")
        &&  ! me->query("couple/child") 
        &&  ! target->query("couple/child") 
        &&  random(100) < 21)
        {
                message("shout", HIR "【家有喜事】：" HIM "恭喜 " HIY + me->query("name") + "(" + me->query("id") + ")"
                        HIM " 与 " HIY + target->query("name") + "(" + target->query("id") + ")" HIM " 做了准父母。\n" NOR,
                        users());
                message_vision(HIY "\n$N突然觉得一阵做呕，旋即羞红着脸，咬咬牙想起了那个该死的。\n" NOR, target);
                message_vision(HIM "\n$N怀孕了。赶快通知孩子的爸爸" HIY + me->query("name") + "(" + me->query("id") + ")" HIM "啊？．．．\n" NOR, target);
                target->apply_condition("huaiyun", 1200);           
                target->set("longbak", target->query("long"));
                target->set("long", HIY "\n"+ target->query("name") + "挺着个大肚子，一看就知道是怀孕了，混身散发出一股慈祥的母爱。\n" NOR);
        }
}
