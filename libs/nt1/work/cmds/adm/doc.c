...
void QueryNpcInfo(string path)
{
        ...
        foreach(string tmp in file)
        {
                ...
                write_file(filename, WHT "中文名字：" NOR + obj->query("name") + "\n", 0);
...
string QueryEquipProp(object equip,mixed props)
{
        ...
        if (stringp(type = equip->query("skill_type")))
                msg += sprintf("兵器类型：%-10s     攻 击 力：%d",
                               type, (int)equip->query("weapon_porp/damage"));
                               
        else 
        if (stringp(type = equip->query("skill_type")))
                msg += sprintf("防具类型：%-10s     防 御 力：%d",
                               type, (int)equip->query("armor_porp/armor"));
