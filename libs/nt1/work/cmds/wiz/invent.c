#include <ansi.h>
#include <command.h>
#define  INVENT_OB  "/clone/meskill/invent"
...
int inventing(object me)
{
        ...
        if( random(10) == 1 ) {
                ...
                tell_object(me, HIW"你自创出了一套" + valid_types[skill] + "。\n"NOR
...
void input_skill_name(string arg, object me)
{
        ...
        while (i--)
        {
                if (arg[i] < 'a' || arg[i] > 'z' )
                {
                        write("对不起，英文名只能用英文字母。\n请重新输入：");
                        input_to((: input_skill_name :), me);
                        return;
                }
        }

        arg = arg + "-" + me->query_temp("invent/skill");

        if( file_size(SKILL_D(arg) + ".c") > 0 
         || find_object(SKILL_D(arg)) 
         || load_object(SKILL_D(arg)) 
         || arg != to_chinese(arg) ) {
                        write("对不起，已经有("+ arg +")这种武功存在。\n请重新输入：");
                        input_to((: input_skill_name :), me);
                        return;
        }

        if( !is_chinese(c_name) ) {
                        write("对不起，请用【中文】做中文名。\n请重新输入：");
                        input_to((: input_skill_name :), me);
                        return;
        }

        skill = me->query_temp("invent/skill");
        c_name = c_name + type_name[skill];
        if( CHINESE_D->find_skill(c_name) ) {
                        write("对不起，已经有[" + c_name + "]这种武功存在。\n请重新输入：");
                        input_to((: input_skill_name :), me);
                        return;
        }
         ...
         CHANNEL_D->do_channel(this_object(), "rumor",
                              sprintf("%s(%s)将生平所学的" + chinese_number(sizeof(me->query_temp("invent/mut_skills")))
                                         + "套" + valid_types[skill] + "合而为一，自创出一路绝世的" HIW
                                         + "《" + c_name + "》" HIM "，从此进入"
                                         + valid_types[skill] + "的全新境界 。\n",
                              me->name(1), me->query("id")));

        invent = new(INVENT_OB);
        invent->set_skill_name(arg);
