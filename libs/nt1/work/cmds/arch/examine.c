...
private string is_illegal(object ob)
{
        ...
        dpot - ob->query("potential") - ob->query("last_examine/potential");
        dmar - ob->query("experience") - ob->query("last_examine/experience");
        dt = time() - (int)ob->query("last_examine/time");
        if (dt > 60)
        {
                ob->set("last_examine/combat_exp", exp);
                ob->set("last_examine/potential", ob->query("potential"));
                ob->set("last_examine/experience", ob->query("experience"));
