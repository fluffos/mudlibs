...
int main(object me, string arg)
{
        ...
        if ((me->query("potential") - me->query("learned_points")) < times * 5)
        ...
        for (i = 0; i < times ; i++)
        {
                ...
                rand = me->query_int() + my_skill / 4;
                rand *= 2;
                me->improve_skill(skill, 10 + random(rand));
        }
...
int can_learn(object me, string skill)
{
        ...
        foreach (skill_name in keys(skills))
        {
                ...
                if (! SKILL_D(skill_name)->valid_force(skill))
                        return notify_fail("你不散了" +
                                           to_chinese(skill_name) +
                                           "，怎么学习" + to_chinese(skill) +
                                           "？\n");
        }
