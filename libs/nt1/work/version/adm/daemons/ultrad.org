...
public mixed accept_ask(object me, object who, string topic) 
{
        ...
        if (topic == "线索" || topic == "clue") 
        {
                ...
                if (who->query("ultraquest/finish") >= 5) 
                ...
                who->delete("ultraquest/finish");
                who->delete("ultraquest/npc1");
                who->delete("ultraquest/name1");
                who->delete("ultraquest/npc2");
                who->delete("ultraquest/name2");
...
{ 
        string *lines, file; 
        string quest, next_npc, msg, npc, npc1, npc2, npc3, npc4, npc5; 
        string type, next_name, quest_msg, name; 
        object item, *npcs; 
        int j, i;

        if (me->query("ultraquest/npc5"))  
        {
                npc5 = me->query("ultraquest/npc5");
        ...
        for (j = 0; j < sizeof(npcs); j++) 
        {
	        ...
	        if (! environment(npcs[i]) || environment(npcs[i])->query("no_fight")) 
	                continue;
	                
                if (userp(npcs[i]) || playerp(npcs[i]) || 
                    ! living(npcs[i])) 
                        continue;
                        
                if (npcs[i]->is_quester() || npcs[i]->is_knower() || 
                    npcs[i]->query_temp("quest_ob"))
                        continue;
                
                if (npcs[i]->query("combat_exp") < 200000)
                        continue;
                                                               
                if (sscanf(base_name(npcs[i]), "/d/%*s") != 1) 
                        continue;
                
                if (sscanf(base_name(npcs[i]), "/d/room/%*s") == 1 ||
                    sscanf(base_name(npcs[i]), "/d/llszhuang/%*s") == 1) 
                        continue;
                                                
                next_npc = npcs[i]->query("id");
                
                if (next_npc == npc1 || next_npc == npc2 ||
                    next_npc == npc3 || next_npc == npc4 ||
                    next_npc == npc5)
                        continue;
                
                next_npc = npcs[i]->query("id");
                next_name = npcs[i]->query("name");
...
{ 
        ...
        // 查看当前位置，如为目的地，则任务完成 
        if (place == environment(me)->query("short") ||
            me->query_temp("last_place") == place)
        { 
                // 如果被保护人昏迷，则先转醒 
                if (! living(me)) 
                        me->revive(); 
...
{ 
        ...
        {                        
                ...
                exp = 10000 + random(5000) + lvl * 10; 
                pot = 10000 + random(3000) + lvl * 10; 
                mar = 1000 + random(2000); 
...
public void generate_information(object me) 
{
        ...
        {              
                ...
                case "kill":
                        ...
                        NPC_D->place_npc(ob, me->query("combat_exp") < 500000  ? 
                                             ({ "大理一带", "终南山", "关外", "西域" }) :
                        ...
                        tell_object(me, CYN + me->query("ultraquest/name5") + CYN "小声地对你说道：那幕后主人是" 
        ...
        */
        ...
        for (j = 0; j < sizeof(npcs); j++) 
        {
	        ...
	        if (! environment(npcs[i])) 
                ...
                if (npcs[i]->query("combat_exp") < 10000)
                        continue;
                ...
                next_npc = npcs[i]->query("id");
                next_name = npcs[i]->query("name");
                npc_place = environment(npcs[i])->query("short");
        ...
        {
        case 1:
                msg = "听人说，这个幕后黑手遗失过一个重要物品，通过这个物品可以知"
                      "道他是谁，好象" + npc_place + "的" + next_name + "就知道这"
                      "个物品的下落，" + ma + "可去找他问问，但也不知是真是假";
                break;
        ...
        case 4:
                msg = "很遗憾，你要的东西我前几天搞丢了，不清楚" + ma + "是否认识" 
                      + npc_place + "的" + next_name + "，他见过此东西，而且好象知"
                      "道那个物品的主人是谁";
                break;

        default:
                msg = "我想想…" + ma + "去找" + npc_place +
                      "的" + next_name + "问问看吧，去年我听"
                      "他说起过这件事";
                break;
        }
