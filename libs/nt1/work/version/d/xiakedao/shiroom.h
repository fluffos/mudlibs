...
int do_study(string arg)
{
        ...
        if ((int)me->query_skill("literate", 1) < 1 &&
            file_name(environment(me)) != "/d/xiakedao/shiroom24")
        ...
        for (i = 1; i <= times; i++)
        {
                ...
                if (item == 4)
                {
                        ...
                        {
                                write("可是你的辟邪剑法不够，无法领悟壁上是辟邪剑法图谱。\n");
                                return 1;
                        }
                }
                else
                ...
                if (learn_pot > 0)
                {
                        if(item == 23)
                        {
                                if (kar = random((int)me->query("kar")) > 0)
                                {
                                        me->improve_skill(skill_name[item], kar);                          
                                        success = 1;
                                }
                        } else
                        {
        ...
        if (success == 1)
                write("你对着石壁冥思苦想了" + chinese_number(times) + "回，似乎" +
                      CHINESE_D->chinese(skill_name[item]) + "有些心得。\n");
