...
int is_bunch_brother(mixed ob)
{
        ...
        if (objectp(ob))
                name = ob->query("bunch/name");
                // 取得其帮派的名称
        else
                return 0;

       	banghui = (string)query("bunch/name");
       	if (! stringp(banghui))
       		return 0;
       		
        // 是否是联盟帮派
        data = query("bunch/brother");
        if (! mapp(data)) data = ([]);
        
        bunch = keys(data);
        if (member_array(name, bunch) != -1) 
        	return 1;
