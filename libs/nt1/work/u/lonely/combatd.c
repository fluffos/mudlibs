...
// Modified by Lonely for qiankundanuoyi & do_interlink
...
varargs int do_attack1(object me, object victim, object weapon, int attack_type,
mapping action, string limb, string attack_msg);
...
string damage_msg(int damage, string type)
{
        ...
        case "割伤":
                ...
                if (damage < 100) return "结果「嗤」地一声划出一道伤口！\n"; else
                if (damage < 200) return "结果「嗤」地一声划出一道血淋淋的伤口！\n"; else
                if (damage < 400) return "结果「嗤」地一声划出一道又长又深的伤口，溅得$N满脸鲜血！\n"; else
...
{
	int maxqi = (int)ob->query("max_qi");
	if (! intp(maxqi)) maxqi = 100;
	
	if (effective)
        {
		message_vision( "( $N" + eff_status_msg(
			(int)ob->query("eff_qi") * 100 / maxqi) 
		    	+ " )\n", ob);
        }
	else
		message_vision( "( $N" + status_msg(
			(int)ob->query("qi") * 100 / maxqi) 
			+ " )\n", ob);
}
...
varargs int skill_power(object ob, string skill, int usage, int delta)
{
        ...
        if (level < 1)
	{
        ...
        if (level > 500)
                power = (level / 5) * (level / 2) * (level / 10);
        else
                power = (level / 2) * (level / 5) * (level / 10);
...
string attack_msg)
{
        ...
        if (! action)
	{
        ...
        if (parry_skill =="douzhuan-xingyi" || parry_skill=="qiankundanuoyi")   
        {
                foo = SKILL_D(parry_skill)->parry_action(victim, me, weapon, attack_type, action, limb);    
        }

        if (foo == 0)      
        {
                if (stringp(attack_msg)) 
                        do_attack1(me, victim, weapon, attack_type, action, limb, attack_msg); 
                else
                        do_attack1(me, victim, weapon, attack_type, action, limb); 
        }
}
...
mapping action, string limb, string attack_msg) 
{
	...
	if (! my["not_living"])
	{
        ...
        */
        ...
        if (intp(action["attack"]))
                me->set_temp("fight/attack", action["attack"] / 100);
        if (intp(action["dodge"]))
        {
                if (action["dodge"] > 0)
                        me->add_temp("fight/dodge", action["dodge"] / 150);
                else
                        victim->add_temp("fight/dodge", action["dodge"] / 150);
        }
        if (intp(action["parry"]))
        {
                if (action["parry"] > 0)
                        me->add_temp("fight/parry", action["parry"] / 200);
                else
                        victim->add_temp("fight/parry", action["parry"] / 200);
        }
        // ---------------------------------------------------------
        ...
        damage = 0;
	wounded = 0;
        damage_info = ""; // 这样会导致出现信息中出现0，我在后面做了处理了(by Lonely)

        // if (random(100) < dp * 100 / (ap + dp))
        // 因为在 nt 下的 mudos 的 random 值偏小，故恢复原始写法
        // if (random(ap + dp) < dp) 
        if (random(100) < dp * 100 / (ap + dp))
        ...
        } else
	{
		...
		} else
		{
			if (weapon) delta = -10;
                ...
                if (me->query("zhuanshi/jineng") && victim->is_fighting(me) && !victim->query("zhuanshi/jineng"))
                        pp -= skill_power(victim, "parry", SKILL_USAGE_DEFENSE, delta)/2; 
                        ...
                        if (my["jiali"] && (my["neili"] > my["jiali"]))
			{
                                if (force_skill = me->query_skill_mapped("force"))
				{
                        ...
                        while (damage > 0)
                        {
                                ...
                                if (victim->query_temp("jin_power",1) && (int)victim->query("age") < 18 ||
                                ...
                                {
                                      damage_info += HIW"$N试图攻击$n，但与$n身体仆一接触，"
                                      +"立刻被神魔金身把攻势全被档了回来。\n" NOR;
                                      damage = 0;
                                      break; 
                                }

                                if (objectp(shield = victim->query_temp("armor/shield")) && objectp(victim) &&
                                    shield->query("id_id") == "hei-ke shield" && random(2) > 0)
                                {
                                        message_vision(result, me, victim,"fight");
                                        damage_info += MAG"忽然，$n手中的"NOR+shield->query("name")
                                        +MAG"光芒大盛，尽数挡住了$N的攻击！\n"
        ...
        damage_info = replace_string(damage_info, "0","");
        // Add by Lonely 
        ...
        if (damage > 0)
	{
                    ...
                    your["qi"] * 3 <= your["max_qi"])
		{
                        ...
                        // 兵器谱争斗
                        if (playerp(me) && playerp(victim))
                        {
                                if (objectp(weapon))
                                        me->set("top/weapon", weapon->query("name"));
                                else
                        	{
                                        switch (attack_skill)
                        		{
                                        case "unarmed":
                        	        case "cuff":
                                                me->set("top/weapon", "拳　　头");
                                                break;
                                        case "claw":
                                                me->set("top/weapon", "利　　爪");
                                                break;
                                        case "strike":
                                                me->set("top/weapon", "铁　　掌");
                                                break;
                                        case "hand":
                                        case "finger":
                                                me->set("top/weapon", "手　　指");
                        	                break;
                                        default:
                                                me->set("top/weapon", "神秘武器");
                                                break;
                        		}
                        	}
...
void start_vendetta(object me, object obj)
{
        ...
        if (me->is_fighting(obj) ||
            environment(me) != environment(obj) ||
	/*
            ! (stringp(vendetta_mark = me->query("vendetta_mark")) &&
            obj->query("vendetta/" + vendetta_mark) &&
            stringp(vendetta_mark = query("bunch/name"))) ||
	*/
            environment(me)->query("no_fight"))
		return;
...
void killer_reward(object killer, object victim)
{
        ...
        while (1)
        {
                ...
                if (ks["combat_exp"] < vs["combat_exp"])
                {
                        if (ks["combat_exp"] >= 100000 &&
                            ks["combat_exp"] >= vs["combat_exp"] / 3)
                        {
                                // adjust experience
                                if (! intp(ks["experience"]))
                                        ks["experience"] = 0;
                                killer->improve_experience(10 + random(21));
                        }
        ...
        if (! userp(victim) && objectp(killer) && playerp(killer))
        {
                // 杀死师傅出谣言
                if (killer_master_id == (string)victim->query("id")) 
                {
                        CHANNEL_D->do_channel(this_object(), "rumor",
                                sprintf("%s将自己的师傅%s杀死了,真是惨无人道啊。", 
                                killer->name(0),victim->name(1)));
                        if (killer->query("class") == "bonze" && killer->query("zhuanshi/jineng"))
                        {
                ...
                killer->query_temp("chousha/fam") == victim->query("family/family_name")) 
                {
                        ...
                        if (chousha_exp > 4000) chousha_exp = 4000;
                        ...
                        if (killer->query_temp("chousha/reward_exp") > 50000)
                        {
                                message("channel:rumor",HIB"【江湖仇杀】"NOR+HIR+killer->query("name")+NOR
...
void compare(object killer,object victim)
{
        ...
        str = read_file("/data/hate/mp_num",1,1);
        sscanf(str,"%d",mp_num);
        str = "";
        for(i=0;i<mp_num-1;i++)
        {
                allstr = "";
                pai3 = "";
                allstr = read_file("/data/hate/"+pai2,i+1,1);
        ...
        write_file("/data/hate/"+pai2, str, 1);
