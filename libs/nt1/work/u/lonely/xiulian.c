...
int main(object me, string arg)
{
        ...
        if (me->query("yinyang-shiertian/succeed"))
        ...
        case "qiankun":	
	        ...
	        if (me->query("max_jingli") < 30000)
	                return notify_fail("你觉得精力颇有不足，看来目前还难以修炼乾坤境界。\n");

                if (me->query("max_qi") < 30000)
		        return notify_fail("你觉得气血颇有不足，看来目前还难以修炼乾坤境界。\n");

                if (me->query("potential") - me->query("learned_points") < 300000)
                        return notify_fail("你的潜能不够，没法修炼。\n");		        
...
int mending(object me)
{
	...
	switch (me->query("doing"))	
        ...
        case "shier":
	        ...
	        if (me->query("max_qi") < 10000 || me->query("max_neili") < 10000
		|| me->query("max_jingli") < 2000)
	        {
		        tell_object(me, "你的状态不佳，无法继续修炼阴阳九转十二重天。\n");
		        message_vision("$N睁开双目，缓缓吐了一口气，站了起来。\n", me);
		        msg = "退出";
                }
	        me->set("max_qi", me->query("max_qi") - 2);
	        me->set("max_neili", me->query("max_neili") - 2);
                me->set("max_jingli", me->query("max_jingli") - 1);
		
	        break;

        case "qiankun":
        
	        if (me->query("max_jingli") < 2000)
	        {
		        tell_object(me, "你的状态不佳，无法继续修炼阴阳九转十二重天。\n");
	                message_vision("$N睁开双目，缓缓吐了一口气，站了起来。\n", me);
	                msg = "退出";
        ...
        case "yinyang":
	        ...
	        if (me->query("max_neili") < 2000)
	        {
                ...
                me->set("max_neili", me->query("max_neili") - 1);
        ...
        if (random(40000) < me->query_skill("yinyang-shiertian", 1))
        {
                message_vision(HIW "只见$N" HIW "心有所悟，霍然贯通，武学境界又有突破！\n" NOR, me);
                tell_object(me, HIR "恭喜你领悟了阴阳九转十二重天的新境界！\n" NOR);
 
	        if (me->query("doing") == "shier")
	        {
		        switch ((int)me->query("yinyang-shiertian/shier"))
		        {
	                case 12:
		                me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/succeed", 1);
			        me->set("can_perform/yinyang-shiertian/tian", 1);
			        me->set("potential", (int)me->query("learned_points"));
			        CLOSE_D->user_opened(me);
        	                CHANNEL_D->do_channel(this_object(), "rumor",
				                sprintf("听说%s(%s)领悟到了阴阳九转十二重天的「十二重天」。",
        							me->name(1), me->query("id")));
		                if (! interactive(me))
		                {
		        ...
		        case 11:
			        me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 12);
			        break;

		        case 10:
			        me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 11);
			        break;

		        case 9:
			        me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 10);
			        break;

		        case 8:
			        me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 9);
			        break;

		        case 7:
			        me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 8);
			        break;

		        case 6:
			        me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 7);
			        break;

		        case 5:
			        me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 6);
			        break;

		        case 4:
		                me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 5);
			        break;

		        case 3:
			        me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 4);
			        break;

		        case 2:
		                me->delete("yinyang-shiertian/shier");
		                me->set("yinyang-shiertian/shier", 3);
			        break;

		        case 1:
			        me->delete("yinyang-shiertian/shier");
			        me->set("yinyang-shiertian/shier", 2);
			        break;
	                case 0:
			        me->set("yinyang-shiertian/shier", 1);
			        break;
		        }		
	        }

	        if (me->query("doing") == "qiankuan")
	        {
		        switch ((int)me->query("yinyang-shiertian/jiu"))
		        {
		        case 9:
			        me->delete("yinyang-shiertian/jiu");
			        me->set("can_perform/yinyang-shiertian/jiu", 1);
			        me->set("potential", (int)me->query("learned_points"));
			        CLOSE_D->user_opened(me);
        		        CHANNEL_D->do_channel(this_object(), "rumor",
					        sprintf("听说%s(%s)领悟到了阴阳九转十二重天的「九转乾坤」。",
							me->name(1), me->query("id")));
			        if (! interactive(me))
		                {
			                me->force_me("chat* haha");
		        ...
		        case 8:
			        me->delete("yinyang-shiertian/jiu");
			        me->set("yinyang-shiertian/jiu", 9);
			        break;
        
        	        case 7:
			        me->delete("yinyang-shiertian/jiu");
			        me->set("yinyang-shiertian/jiu", 8);
			        break;

		        case 6:
			        me->delete("yinyang-shiertian/jiu");
			        me->set("yinyang-shiertian/jiu", 7);
			        break;

		        case 5:
			        me->delete("yinyang-shiertian/jiu");
			        me->set("yinyang-shiertian/jiu", 6);
		                break;

		        case 4:
		                me->delete("yinyang-shiertian/jiu");
			        me->set("yinyang-shiertian/jiu", 5);
			        break;
        
                        case 3:
		                me->delete("yinyang-shiertian/jiu");
			        me->set("yinyang-shiertian/jiu", 4);
			        break;

		        case 2:
		                me->delete("yinyang-shiertian/jiu");
			        me->set("yinyang-shiertian/jiu", 3);
			        break;

		        case 1:
			        me->delete("yinyang-shiertian/jiu");
		                me->set("yinyang-shiertian/jiu", 2);
			        break;
		        case 0:
			        me->set("yinyang-shiertian/jiu", 1);
			        break;
		        }		
	        }
	
                if (me->query("doing") == "yinyang")
	        {
	                switch ((string)me->query("yinyang-shiertian/zhen"))
		        {
	                case "yiyang":
			        me->delete("yinyang-shiertian/zhen");
		                me->set("can_perform/yinyang-shiertian/zhen", 1);
		                me->set("potential", (int)me->query("learned_points"));
		                CLOSE_D->user_opened(me);
        		        CHANNEL_D->do_channel(this_object(), "rumor",
				        sprintf("听说%s(%s)领悟到了阴阳九转十二重天「镇阴阳」。",
					        me->name(1), me->query("id")));
			        if (! interactive(me))
		                {
			                me->force_me("chat* haha");
		        ...
		        case "yin":
			        me->delete("yinyang-shiertian/zhen");
		                me->set("yinyang-shiertian/zhen", "yinyang");
			        break;

		        case "yang":
			        me->delete("yinyang-shiertian/zhen");
			        me->set("yinyang-shiertian/zhen", "yin");
			        break;
			
		        default:
			        me->set("yinyang-shiertian/zhen", "yang");
		        }
	        } 
	        if (me->query("yinyang-shiertian/succeed"))
                {
                        CHANNEL_D->do_channel(this_object(), "rumor",
		                              sprintf("听说%s(%s)经过闭关苦修，领悟到了武学阴阳九转十二重天的最高境界。",
					      me->name(1), me->query("id")));					
                } else
                {
                        CHANNEL_D->do_channel(this_object(), "rumor",
					      sprintf("听说%s(%s)经过闭关苦修，领悟到了武学阴阳九转十二重天新的境界。",
					      me->name(1), me->query("id")));
                }
