// ultraquestd.c 处理宗师任务的守护进程
// 属于连环寻找线索任务
// Created by Lonely for nitan(1)
...
// 定义提供给外部调用的接口函数 
public int assign_message(object ob, string id, string dir);
...
public int assign_message(object ob, string id, string dir)
{
        ...
        /*
        ob->set_temp("messages/who", id);
        ob->set_temp("messages/msg", messages[random(sizeof(messages))]);
        */
        return 1;
}
...
{ 
        ...
        // 保持与正常的 give 信息一致 
        message_vision(me->query("ultraquest/npc") + "拿出一" + 
...
{ 
        ...
        {                
                ...
                // 执行提供线索的程序，必须预先设定提供者 
                who->set("ultraquest/npc", me->name()); 
        ...
        if (topic == "线索" || topic == "clue") 
        {
                ...
                if (who->query("ultraquest/finish") == "all") 
                        return "东西不是已经找到了么？还需要什么线索。"; 
        
                // 最多只能重新提供三次线索 
                if (who->query("ultraquest/new_begin") >= 3) 
                        return "事不过三，剩下的事情你自己想办法解决。"; 
        
                switch (who->query("ultraquest/new_begin")) 
                { 
                case 0: 
                ...
                // 取消以前的任务属性 
                who->delete("ultraquest/quest"); 
        
                // 记录重新询问的次数 
                who->add("ultraquest/new_begin", 1); 
...
{ 
        ...
        next_npc = who->query("ultraquest/next_npc"); 
        npc_place = who->query("ultraquest/npc_place"); 
        
        if (stringp(type = who->query("ultraquest/quest/type")) 
            && stringp(obj = who->query("ultraquest/quest/obj")) 
            && topic == obj) 
        { 
                // 提供任务人物对所提供出的任务信息的解答 
                if (me->name() == next_npc) 
        ...
        { 
                // 询问目标为上一个提供信息的人 
                if (who->query("ultraquest/npc") == me->name()) 
        ...
        { 
                ...
                // 如果以前的任务无法执行，则中断 
                if (who->query("ultraquest/quest/fail")) 
...
{ 
        ...
        */ 
        ...
        who->set("ultraquest/quest/obj", me->name()); 
...
{ 
        ...
        next_npc = me->query("ultraquest/next_npc"); 
        ...
        me->set("ultraquest/quest/obj", obj); 
...
{ 
        ...
        if (! objectp(me) 
            && objectp(killer)) 
        { 
                tell_object(who, HIG "你眼见" + who->query("ultraquest/quest/obj") + 
                                 HIG "被杀，心中连连叫糟。\n" NOR); 
        
                // 记录下失败记录 
                who->set("ultraquest/quest/fail", 1); 
                return; 
        } 
...
{ 
        ...
        // 如果对象为接受信件或包裹的NPC 
        if (me->name() == who->query("ultraquest/quest/obj")) 
        { 
                switch (type) 
                { 
                case "send" : 
        ...
        // 如果对象为发布任务的NPC 
        if (me->name() == who->query("ultraquest/next_npc")) 
        { 
                switch (type) 
                { 
                case "send" : 
...
{ 
        ...
        */ 
        ...
        // 记录下下一个线索人物及出现地 
        me->set("ultraquest/next_npc", next_npc); 
        me->set("ultraquest/npc_place", npc_place); 
        ...
        if (npc == QUESTER) 
        { 
                if (me->query("ultraquest/new_begin")) 
                        msg = "…嗯…你去" + npc_place + 
                              "问问" + next_npc + "看，他那儿" 
                              "似乎又有些线索"; 
                else 
