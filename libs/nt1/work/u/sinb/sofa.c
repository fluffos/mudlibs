...
void create()
{
        string *pname = ({"布艺", "牛皮", "羊皮","鹅绒" });
        string *mname = ({"", "豪华", "靠背", "安乐", "扶手", "折叠" });
        string *lname = ({"长沙发","单人沙发" });
        string name;
        ...
        set("class", zname);
        set_weight(30000);
        set("long", "这是一张沙发，你可以坐(sit)下来休息一会儿。\n" );
        set("no_sell", "这样东西不能随便买卖");
        set("no_get","凳椅是安全中心的财产,任何人都不能拿走！");
        if( clonep() )
...
int do_sit(string arg)
{
        ...
        ob = this_object();
        who = this_player();
        if( arg != "sofa" && arg != "沙发" && arg != ob->name() )
