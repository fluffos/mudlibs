...
{ 
        ...
        */
        ...
        if (! me->query("can_learned/taixuan-gong/enable_weapon"))
                ...
                || usage == "sword" || usage =="blade" || usage == "parry";
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
        ...
        */
        ...
        if ((int)me->query_skill("taixuan-gong", 1) < 150
           || me->query_skill_mapped("dodge") != "taixuan-gong"
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        */
        ...
        {
                me->add("neili", -100);
                result = ([ "damage" : (damage_bonus - 76) / 2 ]);
...
int valid_learn(object me)
{
        ...
        if ((int)me->query("int") < 27)
                return notify_fail("你若有所悟，然而总是有点不明白。\n");

        if ((int)me->query("dex") < 27)
                return notify_fail("你研究了半天，只觉招式始终无法随意施展。\n");

        if ((int)me->query("con") < 27)
                return notify_fail("你研究了半天，只觉满头大汗，气喘吁吁。\n");

        if ((int)me->query("str") < 27)
                return notify_fail("你研究了半天，只觉自己的手臂已无法提起。\n");
