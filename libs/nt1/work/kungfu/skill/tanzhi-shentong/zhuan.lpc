...
int perform(object me, object target)
{
        ...
        ap = ap_power(me, "finger") + me->query_skill("qimen-wuxing", 1);
        dp = dp_power(target, "dodge") + target->query_skill("qimen-wuxing", 1);
        ...
        if (ap * 11 / 20 + random(ap) > dp)
        {
                ...
                } else
                {
                        ...
                        me->add("neili", -500);
                        msg += COMBAT_D->do_damage(me, target, UNARMED_ATTACK, damage, 125,
        ...
        if (damage < 0)
        {
                target->receive_damage("qi", 21000, me);
                target->receive_damage("jing", 21000, me);
                target->die(me);
        }
