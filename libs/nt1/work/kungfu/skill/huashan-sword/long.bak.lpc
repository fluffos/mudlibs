...
int perform(object me, object target)
{
        ...
        int ap, dp, my_li, op_li, my_jing, my_nei, op_jing, op_nei;
        ...
        if (me->query_skill("force") < 100)
                return notify_fail("你的内功火候未到，无法运劲施展天外飞龙！\n");
        ...
        ap = me->query_skill("huashan-jianfa", 1) * 3 / 2 + me->query("level") * 20 + 
        ...
        if (ap / 2 + random(ap) > dp)
        {
                if (random(my_li) > random(op_li)) 
                {
                        ...
                        {
                                message_combatd(HIR "$N闪避不及，只好咬牙举起"+op_weapon->name()+"奋起全力强接攻势。\n" NOR, target);
                                message_combatd(HIR "硬砰之下，"+op_weapon->name()+"竟断为几截，但仍阻不了飞剑来势！\n" NOR, target);
                                op_weapon->unequip();
                        ...
                        message_combatd(HIR "$N避无可避，发出一声凄历的惨嚎，"+weapon->name()+"穿身而过！\n" NOR, target);
                        weapon->move(target);
                ...
                else if (me->query_str() > target->query_str()) 
                {
                        if (objectp(op_weapon)) 
                        {
                                message_combatd(HIR "硬砰之下，"+op_weapon->name()+"竟断为几截，终於抵消了飞剑来势！\n" NOR, target);
                                message_combatd("飞剑攻势受阻，力尽跌落。$N虽挡过一击，却也因精力内力钜耗，摇摇欲坠！\n", target);
                        ...
                        {
                                message_combatd(HIR "$N奋力闪躲，但却力有不逮，发出一声凄历的惨嚎，中剑重伤！\n" NOR, target);
                                target->receive_wound("qi", target->query("max_qi") / 2, me);
                        }
                }
                else if (random(2) == 1 && objectp(op_weapon)) 
                {
                        message_combatd(HIR "硬砰之下，$N的"+weapon->name()+"及$n的"+op_weapon->name()+"双双跌下！\n" NOR, me, target);
                        weapon->move(environment(me));
                        op_weapon->move(environment(me));
                }
                else if (objectp(op_weapon)) 
                {
                        message_combatd(HIR "硬砰之下，$N的"+weapon->name()+"及$n的"+op_weapon->name()+"双双损毁！\n" NOR, me, target);
                        op_weapon->unequip();
                        op_weapon->move(environment(me));
                        op_weapon->set("name", "断掉的" + op_weapon->name());
                        op_weapon->set("value", 0);
                ...
                {
                        message_combatd(HIR "$N奋力闪躲，但却力有不逮，发出一声凄历的惨嚎，中剑受伤！\n" NOR, target);
        ...
        {
                message_combatd(HIY "$N身形连闪，只守不攻，好不容易将此招避过，已吓得亡魂直冒！\n" NOR, target);
                message_combatd(HIY "一击未中，"+weapon->name()+"嗖的一声又飞回$N手中。\n" NOR, me);
        }
