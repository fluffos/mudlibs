...
int perform(object me, object target)
{
        ...
        if (userp(me) && ! me->query("can_perform/mantianhuayu/zheng")) 
                return notify_fail("你还没有受过高人指点，无法施展掷金针。\n");
       ...
       if (! living(target))
        ...
        me->start_busy(2);
        my_exp = me->query_skill("throwing") + me->query("level") * 20 +  
             me->query_skill("martial-cognize", 1); 

        ob_exp = target->query_skill("dodge") + target->query("level") * 20 +
