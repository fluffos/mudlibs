...
int perform(object me, object target)
{
        ...
        if (userp(me) && ! me->query("can_perform/fanliangyi-dao/hebi"))
                return notify_fail("你还没有受过高人指点，无法施展双刀和璧。\n"); 
        ...
        if( target->query_skill_mapped("blade") != "fanliangyi-dao" )
        ...
        message_combatd(HIR "$N" HIR "与$n" HIR "突然间双刀一交，攻势及守势飓然大盛，配合得天衣无缝！\n" NOR, 
                        me, target);
        ...
        target->apply_condition("fanliangyi_check", 1);
...
void check_fight(object me, object target, object victim, int amount)
{
        ...
        if( !target || !objectp(target) )
        {
                ...
                tell_object(me, GRN"双刀和璧已然瓦解，压力顿减，你不由然地松了一口气。\n"NOR);
                return;
        }
                ...
                victim->query("qi") <= 50 && me->is_fighting(victim) && target->is_fighting(victim) ) 
        {
                remove_effect(me, amount);
                remove_effect(target, amount);
                message_combatd(HIG"$N与$n个自收招，相视一笑。\n\n"NOR, me, target);
                return;
        }
