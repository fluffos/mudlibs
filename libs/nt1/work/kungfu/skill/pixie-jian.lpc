...
int query_neili_improve(object me)
{
        ...
        return lvl /10 * lvl /10 * 19 * 15 / 10 / 20;
}
...
int valid_learn(object me)
{
        ...
        if (me->query("gender") == "女性" && !me->query("special_skill/ghost"))
                return notify_fail("自古以来没听说过女人可以修习葵花宝典。\n");

        if (me->query("gender") == "男性" && !me->query("special_skill/ghost"))
        {
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        &&  victim->query_skill_mapped("sword") == "dugu-jiujian"
        &&  victim->query_skill_mapped("parry") == "dugu-jiujian"
        &&  victim->query_skill("dugu-jiujian", 1) * 2 / 3 > 
            random(me->query_skill("pixie-jian", 1) / 2)) 
                return 0; 
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
        ...
        if (ap / 2 + random(ap) < dp)
        {
                if (objectp(weapon = ob->query_temp("weapon")) && 
                    ...
                    ob->query_skill_mapped("sword") == "dugu-jiujian" &&
                    ob->query_skill_mapped("parry") == "dugu-jiujian" &&
                    ob->query_skill("dugu-jiujian", 1) * 2 / 3 > random(me->query_skill("pixie-jian", 1) / 2))
                {
