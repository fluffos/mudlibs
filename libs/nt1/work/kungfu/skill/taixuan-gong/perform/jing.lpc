...
int perform(object me, object target)
{
        ...
        if (flag)ap = ap_power(me, "sword") + me->query_str() * 2;
        else ap = ap_power(me, "blade") + target->query_str() * 2;
        ...
        {
                if (flag) damage = da_power(me, "sword");
                else damage = da_power(me, "blade");
        ...
        if (flag)ap = ap_power(me, "sword") + me->query("int") * 20;
        else ap = ap_power(me, "blade") + target->query("int") * 20;
        // 第二招，判断对方悟性
        ...
        if (flag)ap = ap_power(me, "sword") + me->query_con() * 2;
        else ap = ap_power(me, "blade") + target->query_con() * 2;
        // 第三招，判断对方根骨
        dp = dp_power(target, "force") + target->query_con() * 2;
        ...
        if (flag)ap = ap_power(me, "sword") + me->query_dex() * 2;
        else ap = ap_power(me, "blade") + target->query_dex() * 2;        
        dp = dp_power(target, "dodge") + target->query_dex() * 2;
