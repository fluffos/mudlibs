...
string ask_job()
{
        ...
        if (ob->query("combat_exp") > 20000 && 
            (ob->query_skill("force") < good_skill ) && random(15) > 0) 
        {
        ...
        message_vision("$N交给$n一个怪蛇。\n", me, ob);
        
        staff = new("/clone/weapon/gshezhang");
        staff->move(ob);
        message_vision("$N交给$n一个怪蛇杖。\n", me, ob);
...
int accept_object(object who, object ob)
{
        ...
        if (playerp(ob))
        {
                command("say 最近天神管理的严啊，女玩家我可不敢非礼了。");
                return 0;
        }
