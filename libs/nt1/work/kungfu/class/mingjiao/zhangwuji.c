...
void attempt_apprentice(object ob)
{
        ...
        if (ob->query_skill("shenghuo-shengong", 1) < 220)
        {       
                command("say 你圣火神功不够娴熟，还是下去多练习一下吧！");
                return ;
        }

        if (ob->query_skill("force", 1) < 220)
        {       
...
mixed ask_ling()
{
        ...
        if (objectp(ob = present("shenghuo ling", this_object())))
                ...
                return "你来晚了，现在我身上没有圣火令了。\n";

        message_vision("张无忌从怀里取出一枚圣火令交给" +
                       this_player()->query("name") + "。\n",
                       this_player());
                       
        return "好吧，圣火令我可交给你了，如果不小心丢失了，可要受教规处置的！\n";
}
