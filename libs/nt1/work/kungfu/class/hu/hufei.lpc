...
LONG 
        ...
        set_skill("mantian-xing", 460); 
        set_skill("strike", 440); 
        set_skill("tianchang-zhang", 440); 
        set_skill("cuff", 440); 
...
{ 
        ...
        message_vision(CYN "$N" CYN "大笑数声，道：好，我这柄宝刀你就拿去用吧。\n" NOR, 
                       this_object(), me); 
        command("give moon blade to " + me->query("id")); 
        return 1; 
} 
...
{ 
        ...
        message_vision(CYN "$N" CYN "点了点头，沉声道：你小子有出息，闯王军" 
                       "刀你就拿去用吧。\n" NOR, this_object(), me); 
        command("give chuangwang baodao to " + me->query("id")); 
        return 1; 
} 
...
string ask_book()
{
        ...
        me = this_player();
        if (! me->query_temp("marks/hujiadaopu", 1)) 
                return "你说什么? \n";
