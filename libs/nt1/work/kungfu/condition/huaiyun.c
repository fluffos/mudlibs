...
int birth1(object me)
{
        ...
        target = find_player(me->query("couple/id"));
...
int birth3(object me)
{
	...
	me->delete("longbak");
        ...
        {
                baby->set("gender", "男性");
                baby->set_name("小" + target->query("name"), 
                               ({"xiao_" + target->query("id"), "baby"}));
                               
                baby->set("long", "这是" + target->query("name") + "和" + me->query("name") + "的孩子。长的好象" + target->query("name") + "啊！\n");
                message("shout", HIR "【家有喜事】" HIM "恭喜 " HIR + me->query("name") +"(" + me->query("id") +")" 
                        HIM " 给 " HIR + target->query("name") + "(" + target->query("id") +")" HIM " 添了一个大胖小子。\n" NOR,
                        users());
        } else
        {
                baby->set("gender", "女性");
                baby->set_name("小" + me->query("name"),
                               ({"xiao_" + me->query("id"), "baby"}));
                               
                baby->set("long", "这是" + target->query("name") + "和" + me->query("name") + "的孩子。长的好象" + me->query("name") + "啊！\n");
                message("shout", HIR "【家有喜事】" HIM "恭喜 " HIR + me->query("name") +"(" + me->query("id") +")" 
                        HIM " 给 " HIR + target->query("name") + "(" + target->query("id") +")" HIM " 添了一个千金小宝贝。\n" NOR,
                        users());                
        }
        ...
        me->set("couple/child", baby->query("id"));
        me->set("couple/child_name", baby->query("name"));
        target->set("couple/child", baby->query("id"));
