...
private void game_log( string msg )
{
	if( !LOG_ON || !msg )
		return;
...
// 组成牌九核心系统的函数
private int			*get_value( string p1, string p2 );
private string			get_cname( int *v );
private int			which_one_is_bigger( int *v1, int *v2 );
private varargs string		get_pic( string *card, int type, int need_name );
private string			*get_random_all_cards();
private varargs string		get_show_all_msg( string *v, int need_name );
private string			*lineup( string *p );
private string			*system_AI( string *p );
...
// 有 *p 时，对 *p 内列表玩家发布信息。
private varargs void message_p9( string msg, string *p )
{
...
private void set_del_no_msg_env( object me, string t )
{
	if( !me || !userp(me) )
		return;
	if( t == "set" )
...
private int	help_cmds();				// help commands
private int	do_showcards();				// 查看所有图案演示
private int	do_check_now();				// 查询现在的游戏状态
private int	do_chk( string arg );			// 玩家查询他人积分
private int	do_lineup_card( string arg );		// 玩家将牌排列（仅大牌九时有效）
private int	do_is_ready();				// 玩家准备好了（仅大牌九时有效）
private int	do_join_game( int bet );		// 玩家加入游戏

private void	do_start_game();			// 准备启动赌局
private void	start_game_set( string inp );		// 启动赌局设置中
private void	start_game_ok( object me );		// 设置成功，启动赌局
private void	game_running_start();			// 游戏运行中 - 开始
private void	game_running_ready_open( string t );	// 游戏运行中 - 准备开牌 (大牌九局时用)
private void	game_running_open( int round );		// 游戏运行中 - 开牌 (大牌九局时调用两次)
private void	game_running_over();			// 游戏运行中 - 结束 (恢复空闲)

private string	*system_host_get_special_card();	// 允许系统的庄家获得一副特别的牌
...
private void start_game_set( string inp )
{
	object me = this_player();
...
private void start_game_ok( object me )
{
...
private void game_running_start()
{
	string hid;
	object hob;
	// 第二部分使用
...
private string *system_host_get_special_card()
{
...
private void game_running_ready_open( string t )
{
...
// 开牌了啊
private void game_running_open( int round )
{
...
// 结束，统计，恢复空闲状态
private void game_running_over()
{
	string *p, t, Hmsg;
	object ob;
...
private int do_lineup_card( string arg )
{
	object me = this_player();
	int *c, *c1, i;
...
private int do_join_game( int bet )
{
	object me = this_player();
	string id;
	int i;
