...
#define MAX_TRADE       10000000 // 每笔最大的贸易额为 1 千万两白银
#undef IN_TEST
...
nomask varargs protected void do_count(string name,int silver)
{
	...
	for(i=0;i<n;i++)
	{
		...
		{
                        origin += oneitem->o_value * oneitem->o_amount; /* silver数 */
	...
	if(stringp(name) && silver && (abs(silver) >= origin/100))
	{
		...
		{
			if(silver > 0)
				CHANNEL_D->do_channel( this_object(), "bill",
