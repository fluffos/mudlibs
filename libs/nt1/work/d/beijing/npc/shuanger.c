  ...
  string new_name,new_id,host_id;
   me=this_player();
           if(query("id")!="shuang er")
        {
         ...
         if ((int)this_player()->query("weiwang")>=70 && this_player()->query_temp("zhuang_liwu"))
           {
...
int do_attack(string arg)
{
  ...
  if (environment() && environment()->query("no_fight")) return notify_fail("双儿小心翼翼的看了你一眼说：这里是安全区啊！\n");
  if(!living(ob)) 
