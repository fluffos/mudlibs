...
int greeting(object me)
{
        ...
        if(me->query_temp("kick")) // 被踢的人
        {
		write("喜来福冲你哼了一声：“不想尝尝老夫的佛山无影腿就老实点。”\n");     
	} else 
        if (! query_temp("party_finish")) //喜宴未结束，吃喜酒的人
        {
...
int accept_object(object me, object ob)
{
	...
	if ((int)me->query_temp("songli") > 7 && i < 100000)
	{
		write("喜来福不悦道：“我警告你啊！你这送礼就到此为止吧，再下去我不客气了！”\n");
		me->add_temp("songli", 1);
	} else 
	if((int)me->query_temp("songli") > 4 && i < 100000)
	{
...
int ask_party()
{
        ...
        if (! objectp(wife = present(me->query("couple/id"), environment(me))))
        {
...
int do_start()
{
        ...
        call_out("finish_party", 600, me, ob);
        return 1;
}
...
int give_money(object me,object ob)
{
	...
	if (! me || environment(me) != environment(ob))
	{
		...
		write_file("/u/lonely/marry",sprintf("%s(%s)%s",me->name(),me->query("id"),ob->query("money")));
