...
int do_climb(string arg)
{
   ...
   case 2:
     ...
     room = find_object("/d/tulong/spirit9");
     if(!objectp(room)) room=load_object("/d/tulong/spirit9");
      me->move(room);
...
void do_flush(object me)
{
   ...
   tell_object(me,"ÄãÏİÈëÒ»Æ¬ÍêÈ«µÄºÚ°µÖ®ÖĞ£¬ĞÄÖĞ³äÂúÁË¿Ö¾å¡­¡­\n\n");
    room = find_object("/d/tulong/lake");
    if(!objectp(room)) room= load_object("/d/tulong/lake");
   me->move(room);
