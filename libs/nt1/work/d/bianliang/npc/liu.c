...
void quest_begin(string who,object me)
{
      ...
      if( smen) smen->set("defence",120);
      mmen=load_object("/quest/quest_jun/myuanmen1");
      if( mmen) mmen->set("defence",120);
...
{                         
        ...
        exp=(reward/2+random(reward/2))*me->query("degree_jungong")*1000;
        pot=exp/4;
        jungong=exp/1000;
        ...
        exp=(reward/2+random(reward/2))*jiang["jiang"+i]->query("degree_jungong")*1000;
        pot=exp/4;
        jungong=exp/1000;
