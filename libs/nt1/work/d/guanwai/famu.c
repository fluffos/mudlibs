...
void create()
{
	set("short", "伐木场");
	set("long", @LONG
这里是鹿鸣峰西坡下的一片森林，长着许多百年的参天大树，笔直挺拔，
都是上好的木材，许多人在这里砍伐(famu)木料，然后运到中原供达官贵人
们建筑豪宅。这里地处深山老林，时常有凶猛的野兽出没。
LONG );
	set("exits", ([
...
int working(object me)
{
        ...
        if (finish)
        {
                ...
                if (coagent->query("combat_exp") >= 1000000 &&
                    coagent->query("combat_exp") < 8000000)
                {
                        // bonus to coagent
                        b = 2000 + random(100);
                        coagent->add("combat_exp", b);
                        coagent->add("potential", (b + 2) / 3);
                        coagent->improve_skill("force", (b + 3) / 4);
                        if (coagent->query("potential") > coagent->query_potential_limit())
                                coagent->set("potential", coagent->query_potential_limit());
