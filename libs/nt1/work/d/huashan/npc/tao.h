...
int tear_and_kill(object me, object ob, string msg)
{
	...
	if (random(me->query("combat_exp")) > (int)ob->query("combat_exp") / 2) 
	{
		...
		if (ob->query_temp("caught") == 4)
		{
		        ...
		        ob->delete_temp("caught");
		        ob->unconcious();
		        ob->set_temp("die_reason", "被桃谷六仙撕成四块了");
...
void lost()
{
	...
	if (ob->name() == "桃根仙") me->set_temp("win/tao1", 1);
	if (ob->name() == "桃干仙") me->set_temp("win/tao2", 1);
	if (ob->name() == "桃枝仙") me->set_temp("win/tao3", 1);
        if (ob->name() == "桃叶仙") me->set_temp("win/tao4", 1);
        if (ob->name() == "桃花仙") me->set_temp("win/tao5", 1);
        if (ob->name() == "桃实仙") me->set_temp("win/tao6", 1);
