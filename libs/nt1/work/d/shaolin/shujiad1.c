...
void create()
{
        ...
        set("short", "书架顶");
        set("long",
"书架顶上积满了厚厚的一层灰尘,书架顶和房梁还有三尺多高的距离\n"
"由于不经常打扫,这里步满了蜘蛛丝，书架顶的浮尘上还能依稀见到一些\n"
"爬虫留下的痕迹有时候有些小鸟也会在这里筑巢。\n"
); 
        set("exits", ([ /* sizeof() == 1 */
...
int do_pull()
{
        ...
        if (query("bamboo_count") > 0)
        {
                ...
                ob->move(me, 1);
                add("bamboo_count", -1);
                CHANNEL_D->do_channel(this_object(), "rumor", sprintf("%s搞到了少林镇寺之宝易筋经秘籍啦。", me->query("name")));
        } else
        if (query("yijinjing_count") > 0)
        {
                ...
                CHANNEL_D->do_channel(this_object(), "rumor", sprintf("%s搞到了少林镇寺之宝易筋经残谱啦。", me->query("name")));
        }
        return 1;
}
