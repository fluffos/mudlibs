...
void init()
{
        ...
        if (! wizardp(this_player())) {
                set("no_get",   HIB"\n带毒的东西，还是不要碰的好。"NOR"\n\n");
                set("no_drop",  "反正也不占地方，"HIC"锦盒"NOR"还是先留着吧。\n");
                set("no_sell", 1);
                set("no_steal", ""HIC"锦盒"NOR"你也敢偷？小心蜘蛛咬你一口！\n");
        }
}
...
int do_zhua(string arg)
{
        ...
        if (spider->query("qianzhu-wandushou/level") < 1) 
        ...
        if (spider->query("qianzhu-wandushou/level") > 
            player->query_skill("qianzhu-wandushou", 1)) 
        {
...
{
        if (ob->query("qianzhu-wandushou/spider") < 1) 
                return 1;
...
int do_lian(string arg)
{
        ...
        if (spider->query("qianzhu-wandushou/spider") < 1) 
                return notify_fail(spider->name(1)+"无法用来练千蛛万毒手。\n");
...
int lian_stages(int stage, object player, object spider) 
{
        ...
        lvl = player->query_skill("qianzhu-wandushou", 1);

        if (stage < 0 || ! objectp(player) || ! objectp(spider)) 
                return 1;
        ...
        if (stage == 1) 
        {
                ...
                i = spider->query("qianzhu-wandushou/level");
                if (i > lvl) 
                {
        ...
        {     // finish
                ...
                k = 100 + spider->query("qianzhu-wandushou/poison");
                while (k > 1000) 
                {
                        player->improve_skill("qianzhu-wandu", 1000);
                        k -= 1000;
                }
                player->improve_skill("qianzhu-wandushou", k);
                        
                spider->add("qianzhu-wandushou/spider", -1);
                if (spider->query("qianzhu-wandushou/spider") <= 0) 
                {
                        message_vision(HIW "$n的颜色变得惨白, 挣扎了几下就此死了。\n" NOR, player,
                                spider);
                        spider->die();
                }else 
                if (spider->query("qianzhu-wandu/spider") < 3) 
                {
                        message_vision(HIW "只见$n的颜色竟然慢慢从黑灰色转而发白。\n" NOR, player,
                                spider);
                }else 
                if (spider->query("qianzhu-wandu/spider") < 6) 
                {
    ...
    stage ++;
    call_out("lian_stages", 5, stage, player, spider);
    player->start_busy(5);
    return 1;
}
