...
int do_fish()
{
	...
	object me = this_player();
        ...
        tell_object(me, "你掏出一点鱼饵，仔细的放在钓钩上，轻轻一甩杆，只见水面上荡起了\n"
...
int finishing(object me)
{
        ...
        case 2:
        case 3:
                msg = random(2) ? "$N的浮子忽左忽右摇摆个不停。"
                                         : "$N的浮子猛然向下一窜，然后又跳出水面。";
                stage++;
                break;
        case 4:
                msg = random(2) ? "一阵剧烈的摇摆过后，浮子安静"
                                           "下来，$N连忙拉起钓竿，一看钩上空空如也。"
...
int do_draw(string arg)
{
        ...
        if (stage < 2)
        {
                tell_object(me, "你连忙一拉钓竿，高高的举了起来，却见上面鱼饵依然，还"
                                "是老样子，只好\n垂头丧气的重新把杆甩了出去。\n");
                me->set_temp("fishing", 0);
                return 1;
        }
        ...
        tell_object(me, random(2) ? "你手急眼快，猛地一收钓竿，只见上面挂着一" +
                                   ob->query("unit") + "扭来扭去的" +
                                   ob->name() + "。\n"
                                 : "你顺势一拉杆，登时把一" + ob->query("unit") +
                                   ob->name() + "钓了上来。\n");
