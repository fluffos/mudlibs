...
void create()
{
        ...
        if (clonep())
	        ...
	        set("unit", "件");
	        set("long", @LONG
这是泥潭的开站礼物，可以把四点先天天赋分（fenpei）到你想要的先天属性上
格式为 fenpei 悟性 1 这种格式。
LONG );
	        set("value", 0);
                set("no_give", 1);
                set("no_get", 1);
...
{    
        ...
        if (! arg)  return notify_fail("指令格式：fenpei <先天属性> <点数>\n"); 

        if (sscanf(arg, "%s %d", htsx, jnd) != 2) 
                return notify_fail("指令格式：fenpei <先天属性> <点数>\n"); 
