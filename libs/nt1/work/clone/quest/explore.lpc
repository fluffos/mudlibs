...
string ask_qob(object knower, object me)
{
        ...
        if (me->query("score") >= 2000)
        {
                if (! mapp(dirs = env->query("exits")) ||
                    ! sizeof(dirs))
                        exits = "没有出口";
                else
                {
                        str = keys(dirs = env->query("exits"));
                        room = get_object(dirs[str[random(sizeof(str))]]);
                        short = room->query("short");
                        str = map_array(str, (: GO_CMD->query_chinese_dir($1) :));
                        str -= ({ 0 });                        
                        exits = "大概具有" + implode(str, "、") + "出口且周围有个" + short + "的";
                }
