...
int do_effect(object me)
{
        ...
        message_vision("$N一仰脖，吞下了一颗" + this_object()->name() +
                       "。\n", me);
        if (me->query("gift/int/succeed") < 5 && random(2))
        {
                effect++;
        ...
        if (me->query("gift/dex/succeed") < 5 && random(2))
        {
                effect++;
        ...
        if (me->query("gift/con/succeed") < 5 && random(2) &&
            me->query("gender") != "无性")
        {
                effect++;
        ...
        if (me->query("gift/str/succeed") < 5 && random(2))
        {
                effect++;
