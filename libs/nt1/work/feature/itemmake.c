...
string weapon_long()
{
        ...
        result += HIW "攻·防修正：" + query("bless") * 10 + "\n" NOR;
...
string armor_long()
{
	...
	lvl = sizeof(levels);
        while (--lvl)
                if (defense_lvl >= levels[lvl])
                        break;
        lvl++;	

	if (defense_lvl == ULTRA_LEVEL)
        {
        	result = "";
                if (stringp(tessera_name = query("magic/tessera")))
                        result += "它上面镶嵌着" + tessera_name + "，闪烁着奇异的光芒。\n";
...
int apply_armor()
{
        ...
        defense_lvl = armor_level();
        p = query("point");
        d = defense_lvl * p / MAX_LEVEL + query("bless") * 10; // 乾坤圣水圣化一次增加十点防御
        // if (d > p * 3 / 2) d = p * 3 / 2;
        return d + p;
}
...
// 防具升级
void armor_upgrade(object me)
{
	ITEM_D->armor_upgrade(me, this_object());
}
