...
nosave string _LiesNotiz(int id);
...
//int. Funktionen
nosave status sort_prio(int key1, int key2);
nosave nomask status check_allowed();
protected int restore_me();
varargs protected void save_me(int delay);
nosave string query_autoloadobj();
nosave string set_autoloadobj(mixed arg);
varargs nosave int checkStatus(int nocache);
nosave string update_deps(string str,int id);
nosave int note_filter(int id,int filterstate);
nosave int aelter_als(int id,int zeit);
nosave int *getUnresolvedDeps(int id);
nosave int *getDeps(int id,int rec);
