...
void create()
{
       set_name(HIW"一号摄像机"NOR, ({"she xiangji", "xiangji"}));
       if (clonep())
...
int do_start(string arg)
{
       ...
       if(objectp(npc = me->query_temp("jingtou1")))
          return notify_fail("你已经在使用摄像机中。\n");
          
       message_vision(HIY"\n$N大声说道：各单位注意，镜头对准"+target->name()+HIY"，开拍！\n\n" NOR, me);

     npc = new("/cmds/leitai/bwdh/npc/jingtou1");
       npc->do_setup(me, target);
       me->set_temp("jingtou1", npc);
       me->start_busy(1);
       add("used", 1);
       return 1;
}
...
int do_stop()
{
        ...
        if(!me->query_temp("jingtou1"))
          return notify_fail("你还没打开摄像机呢。\n");

        npc = me->query_temp("jingtou1");
        if(objectp(npc))
           npc->do_remove();
        message_vision(HIC"\n$N关上了摄像机，说道：还算不错，收工。\n\n" NOR, me);
        me->delete_temp("jingtou1");
        me->start_busy(1);
        return 1;
}
