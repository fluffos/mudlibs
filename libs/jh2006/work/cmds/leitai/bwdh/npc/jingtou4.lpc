...
void relay_message(string msgclass, string msg)
{
        ...
        for (i=1;i<sizeof(ary);i++)
                if (room){
                  ...
                  write_file("/log/html/BIWU4", sprintf("%s\n", remove_ansi(ary[i])));
                }
}
...
void tohtml()
{
        string *message;
        int i;
        message = explode (read_file ("/log/html/BIWU4"), "\n");
        write_file("/log/html/mud4.html", "<html>\n<head>\n<meta http-equiv='refresh' content='8;url=mud4.html'>\n<META HTTP-EQUIV='Pragma' CONTENT='no-cache'>\n<meta http-equiv='Content-Type' content='text/html; charset=gb2312'>\n<title>书剑</title>\n<link rel='stylesheet' href='http://202.96.84.26/my_style.css' type='text/css'>\n</head>\n<body>\n", 1);
        write_file("/log/html/mud4.html", "<center><font color=BLUE>"+CHINESE_MUD_NAME+"</font>　比武大会实况　"+ctime(time())[11..18]+"(自动刷新)\n<HR size=1 width='95%'>\n",0);
        write_file("/log/html/mud4.html", "<table border=0 cellpadding=0 cellspacing=0 width='95%'>\n",0);
        for (i=0; i<sizeof(message); i++)
                write_file("/log/html/mud4.html", "<tr><td width='100%' align=left>"+message[i]+"</td></tr>\n",0);
        write_file("/log/html/mud4.html", "</table>\n<HR size=1 width='95%'><a href=index.html>返回首页</a></center></body></html>",0);
        write_file("/log/html/BIWU4", "", 1);
        remove_call_out("tohtml");
        call_out("tohtml",8);
}
