...
LONG);
  	...
  	set("exits", ([ /* sizeof() == 4 */
  	"北-毒虫谷" : __DIR__"duchonggu"+north,
  	"南-毒虫谷" : __DIR__"duchonggu"+south,
  	"东-毒虫谷" : __DIR__"duchonggu"+east,
  	"西-毒虫谷" : __DIR__"duchonggu"+west,
]));
  	set("outdoors","xingxiuhai");
  	if(POS%5==0) set("exits/southeast", __DIR__"duchonggu-out");
  	if(POS%3)set("objects",([
                FOOD_D("hulu") : 2,
                FOOD_D("xzhang") : 1,

        ]));
   	set("coor/x",-280+(POS/10)*10);
   	set("coor/y",200+(POS%10)*10);
   	set("coor/z",0);
   	setup();
...
void init() 
{
  	...
  	if(this_player()->query("combat_exp")<2000000 &&
  	mapp(fam) && fam["family_name"] =="星宿派"){
