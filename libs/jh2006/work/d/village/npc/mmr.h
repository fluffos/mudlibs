...
//add log to HS_JOB_LOG
 write_file("/log/job/HS_JOB2",sprintf("%s %s(%s) 随机任务采花大盗等级是 %d，本人等级是 %d。\n",
        ctime(time()),ob->name(1),ob->query("id"),number,(int)ob->query("max_pot")-100));
...
//	me->delete_temp("hs/meetjob");
//add log to HS_JOB_LOG
 write_file("/log/job/HS_JOB2_GIFT",sprintf("%s %s(%s) 随机任务奖励经验值%d。\n",
        ctime(time()),me->name(1),me->query("id"),exp));
	}
	}
        ::die();
}
