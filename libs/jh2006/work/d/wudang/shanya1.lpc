        ...
        if ((int)me->query_skill("force", 1) < 20) 
           {
            ...
            me->receive_damage("jingli", random(20));
            me->receive_damage("neili", 60-(int)me->query_skill("force", 1)/20);
            if ((int)me->query_skill("force",1)<180)
                me->improve_skill("force", me->query("con")+random((int)me->query_skill("force",1)/10));
            me->move(__DIR__"shanya2");
	tell_room(environment(me), me->name() + "从上面爬了下来。\n", ({ me }));
          }
        else
          {message_vision("$N颤颤噤噤地爬了上去。\n", me);
           me->receive_damage("neili", random(20));
           ...
           me->move(__DIR__"husunchou");
	tell_room(environment(me), me->name() + "从下面爬了上来。\n", ({ me }));
          }
        if ((int)me->query("jingli", 1)<0||(int)me->query("neili", 1)<0)
         {
