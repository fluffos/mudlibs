...
- 地图和 `jyqxc`/`jyqxc2` 系家族相似（华山、武当、少林、丐帮等），
  归档里甚至带着一份和 `jyqxc`/`fy2mg`/`xkx2017` 框架字节相同的
  `adm/obj/master.lpc`，但实际启动用的是另一份 `/adm/single/
  master`，这份"看起来同源"的档案其实从未被真正加载过。
...
- 原始压缩包本身有个小陷阱：文件扩展名是 `.rar`，但实际内容是带
  `../` 成员路径的 POSIX tar 归档。
