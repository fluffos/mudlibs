...
一个货真价实的金庸多部小说混搭地图，而非单一原著设定的武侠 MUD：除了与 `jyqxc`/`jyqxc2` 家族共有的华山、武当、少林、丐帮等经典门派外，还收录了天龙寺（《天龙八部》）、黑木崖（《笑傲江湖》日月神教总坛）、桃花岛（《射雕英雄传》）与神龙教香堂（《鹿鼎记》），把多部金庸原著的地标拼接进同一个江湖世界。归档里还带着一份未被使用的 `adm/obj/master.lpc`，恰好和 `jyqxc`/`fy2mg`/`xkx2017` 那套框架字节相同，但实际启动用的是另一份 `/adm/single/master`，这份"看起来同源"的档案其实从未被真正加载过，只是血统追溯上的一个巧合红鲱鱼；原始压缩包本身也有个小陷阱——扩展名是 `.rar`，内容却是带 `../` 成员路径的 POSIX tar 归档。

## 内容亮点

- 本次修复中发现一个极其隐蔽的 bug：`SIMUL_EFUN_OB` 宏指向了一个
  未被使用的死代码文件，导致每一个新玩家 `quit` 退出游戏都会失
  败——表面上看连线像是报错断开了，实际上角色仍然留在游戏世界里，
  连线根本没有真正关闭（详见下方 bug 修复说明）。
