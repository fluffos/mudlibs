...
void reload()
{
SAVE_NAME=base_name(this_object());
if(!restore())
...
{
        string id;
       id = this_object()->query("save_name");
        if( !stringp(id) ) return 0;
        return sprintf("/data/city/%s",id);
}
void init()
{
       add_action("do_diaobing","diao");
       add_action("do_jimou","jimou");
       add_action("do_tax", "tax");
      add_action("do_attack", "jingong");
       add_action("do_fangqi", "fangqi");
        add_action("do_defend", "defend");
        add_action("do_status", "status");
add_action("do_bingli", "bingli");
    add_action("do_kaifa", "kaifa2");
        add_action("do_fangqi", "tuibing");
       add_action("do_zhuza","zhuza");
       add_action("do_combat","zengyuan");
       add_action("do_true","true");
       add_action("do_xunlian","xunlian");
     add_action("do_set_name","set_name");
        set("no_clean_up", 1);
}
int do_diaobing(string arg)
{
object ob=this_object();
object me=this_player();
int tlv,tlv1;
int pts, rs; 
if(me->query("id")!=this_object()->query("owner")) return notify_fail("你不是城主，不能够掉兵。\n");
if( !arg || !sscanf(arg, "%d", pts)) return notify_fail("指令格式：diao <军队> (整数)\n");
if(pts <=0) return notify_fail("哼哼，人还有不存在的么？\n");
if(pts >(int)ob->query("soilder")) 
return notify_fail("这里没有那么多士兵来调用。\n");
tell_object(me,"你将城里的兵调用了一些由自己带领。\n");
tlv=(int)me->query("train_level");
tlv1=(int)this_object()->query("train_level");
tlv=(tlv*(int)me->query("soilder")+tlv1*pts)/((int)me->query("soilder")+pts);
me->add("soilder",pts);
me->set("train_level",tlv);
ob->add("soilder",-pts);
message("vision",HIY+"【飞鸽传书】:"+NOR+HIC+me->query("name")+"从"+
this_object()->query("short")+"掉离了"+chinese_number(pts)+"名士兵.\n"NOR,users());
rs=(int)me->query("soilder");
tell_object(me,"你现在带领士兵人数为"+chinese_number(rs)+"。\n");
save();
return 1;
}
...
message("vision",HIY+"【飞鸽传书】:"+NOR+HIC+me->query("name")+
...
COMBAT_D->do1_attack(me,haha,who);
return 1;
}
...
tlv=(int)me->query("train_level");
tlv1=(int)this_object()->query("train_level");
tlv1=(tlv*pts+tlv1*(int)this_object()->query("soilder"))/((int)this_object()->query("soilder")+pts);
this_object()->add("soilder",pts);
this_object()->set("train_level",tlv1);
me->add("soilder",-pts);
message("vision",HIY+"【飞鸽传书】:"+NOR+HIC+me->query("name")+"派驻了"+chinese_number(pts)+
...
message("vision",HIY+"【飞鸽传书】:"+NOR+HIC+me->query("name")+"派了"+chinese_number(pts)+
...
if (me->query("more_money")!=0)
{
me->add("money",me->query("more_money")*100000000);
me->set("more_money",0);
}
if(me->query("money",1) < 10000000) return notify_fail("你的资金不足，还是先多赚点钱吧。\n");
if(arg=="farm")
{
...
me->add("money",-10000*this_object()->query(arg));
save();
return 1;
}
...
else one="整个城市都是城主的狂热追随者，他们为他塑像膜拜，真不知城主使用了什么手段，笼络\n了如此众多的人心。\n";
...
message("vision",HIY+"【飞鸽传书】:"+NOR+HIC+me->query("name")+"取消了对"+this_object()->query("short")+
"的进攻，自动退兵了.\n"NOR,users());
save();
return 1;
}
...
message("vision",HIY+"【飞鸽传书】:"+NOR+HIC+me->query("name")+"自动放弃了对"+this_object()->query("short")+
"的统治，撤离了该城.\n"NOR,users());
save();
return 1;
}
...
obejct ob=this_object();
int do_jimou(string arg)
{
object me=this_player();
object who;
int one,two,three;
if(this_object()->query("attacker")!=me->query("id")||!me->query("attacking")||!this_object()->query("receive_attack"))
return notify_fail("你并没有进攻这个城啊？\n");
who=find_player(this_object()->query("owner"));
if(!who) return notify_fail("城市的主人不在线，你不能攻击她的城市。\n");
ap=me->query("soilder")*me->query("train_level");
if(ap<=0) return notify_fail("你不带兵就想打仗。你以为你是谁呀？\n");
message_vision("只见$N口中念念有词：天啊，赐予我力量吧$n\n"+NOR,users());
if(ob->query("owner")!=who->query("id"))
return notify_fail("你不是城主在这里瞎搅和什么,小心挨扁!");
if(!ob->query("soilder"))return notify_fail("这个城没有兵啊!");
if((int)who->query("qi")<100||(int)who->query("shen")<50)
return notify_fail("你快晕了,休息一下吧!");
*/
int do_set_name(string arg)
{
        ...
        if(me->query("id")!= this_object()->query("owner") && !wizardp(me) )
                return notify_fail("你不是城主。\n");
        if ( !arg || strlen(arg)<1 ) return notify_fail("描述不能为空!\n");
        if ( arg=="-d" ) {
        if ( !query("old_long") ) return notify_fail("你并没有自定义描述!\n");
        this_object()->set("long",query("old_long"));
        this_object()->delete("old_long");
        write("ok!\n");
        this_object()->save();
        return 1;
        }
        if ( query("old_long") )
                return notify_fail("你已经定义了描述,请先用set_name -d 删除!\n");
        ...
        arg = replace_string(arg, "NN", "
");
        arg = replace_string(arg, "$NOR$", NOR);
        this_object()->set("old_long",query("long"));
        this_object()->set("long",arg);  //让描述立即生效
        this_object()->save();
        write("ok!\n");
        return 1;
}
...
{
object me=this_player();
printf(HIB""+me->name()+"("+me->query("id")+")"+"\n"+
HIC"可用兵力："+me->query("soilder")+"
士兵训练度:："+me->query("train_level")+"\n"+
HIC"士兵忠诚度:："+me->query("train_level")+"\n");
return 1;
}
