        ...
        for(i=0; i<sizeof(dir); i++) {
                ...
                for(j=0; j<sizeof(ppls); j++) {
                        if( sscanf(ppls[j], "%s.o", name)==1 ){
                                ...
                                if (ob->query("combat_exp")>=50000000 || (int)ob->query("potential") - (int)ob->query("learned_points")>100000 || ob->query_max_neili()>150000 )
                                        str+=sprintf("%10s(%s) 经验:%10d,潜能:%10d,内力:%10d,上线时间:%d\n",ob->name(),name,ob->query("combat_exp"),(int)ob->query("potential") - (int)ob->query("learned_points"),ob->query_max_neili(),ob->query("mud_age")/86400);
