...
int perform(object me, object target)
{
	...
	if( !target ) target = offensive_target(me);
      if (!target || !target->is_character() || !me->is_fighting(target) )
        return notify_fail("「凌空」只能对战斗中的对手使用。\n");
...
及攻敌，拼力躲避$N指力顿时动作大乱。\n";
    if( random( me->query_skill("tiangang-zhi",1)+me->query_skill("unarmed",1) ) > (me->query_skill("unarmed",1)*2)/3 ) 
	{
		msg +=  HIG "结果$p被$P指力点中几处穴道! \n" NOR;
             target->start_busy( (int)me->query_skill("tiangang-zhi") / 40 + 2);
		me->add("neili", -100);
	}
	else
	{
