...
`xyxy2`（20150716未知lib）、003-1 `xiaoyuxiyou`（小雨
...
- 和 `xiaoyuxiyou`/`xyxy2` 同属"小雨西游"引擎家族，
  地图包含"傲来国"（`d/aolai/`）——《西游记》里孙悟空出生地的真实
  地名，比"三界神话"系家族更贴近原著地理设定。
- `daemon/class/spells/40/` 下的法术/迷宫内容按等级分层编号（如
  `tupo40-N`），是一种按数值门槛分级的突破式玩法结构。
- 本次修复过程中意外发现并顺手修正了一个文档层面的疏漏：姊妹档案
  `xiaoyuxiyou`/`xyxy2` 实际上早就被修好并完整验证过
  （各自的 `NOTES.md` 里有记录），但 `meta.json`/README 一直没有
  跟着更新，长期被错误标记成 `limited`——这次一并修正。
...
1. **`uptime()` 开机保护闸门**（AGENTS.md §1.3(e) 的既定处理方
   ...
   `xyxy2`）在 AGENTS.md 里被明确点名为"已知受影响"，
