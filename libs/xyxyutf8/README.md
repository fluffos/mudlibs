...
《小雨西游》/《小雨西游Ⅱ》引擎的另一份独立压缩包，档案名带"utf8"标注已经预先转成 UTF-8；和本项目里的 `xyxy2`、`xiaoyuxiyou` 同属一个引擎家族，三者共享同一张"傲来国"新手地图——安全陪练、NPC 教学的两段式学艺流程、起步客栈的商店与留言板都一致——但内容哈希比对显示彼此并非字节级重复（8753 对 11149 个 `.lpc` 档案），是游戏开发不同阶段留下的独立快照，不只是重新编码而已，所以单独完整处理。在共享的新手内容之外，这份档案自己还叠加了一套按等级分层的法术/迷宫进阶玩法（`daemon/class/spells/40/` 下按"突破"编号的层级内容）。

## 内容亮点

- 和 `xiaoyuxiyou`/`xyxy2` 同属"小雨西游"引擎家族，三者共享同一张
  "傲来国"（`d/aolai/`）新手地图——《西游记》里孙悟空出生地的真实
  地名，并非这份档案独有。
