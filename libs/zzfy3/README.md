...
**更进一步的血统发现（§10.7 深度测试新发现）**：本库其实和另一个
独立编号的档案 `zzfy`（020 号，同名"郑州风云3"）是同一套引擎/地图
的近乎重复副本——`work/` 目录都是 10345 个档案，只有 183 个内容不
同（大多是个别 NPC 微调、日志噪音，或两边各自独立修过的同一批
bug）。`zzfy` 早前已经做过一轮完整的 §10.7 深度游玩测试，本轮把它
现场验证过的发现逐条移植过来并重新验证，详见 `NOTES.md`。
...
- `daemon/class/` 和 `cmds/daemon/class/` 是两份内容完全相同的
  ...
  过波及 49 处的更大规模版本）。
- 完整的死亡/复活系统靠鬼门关的判官 NPC（`d/death/npc/panguan.lpc`）
  推动一段五阶段对话，鬼魂可以在阴间自由走动（不像部分兄弟档案会
  锁住鬼魂的移动）——判官在场时立刻触发对话，判官恰好不在同一房间
  时鬼魂只是安静等待，下次撞见判官会重新触发，§10.7 深度测试里完整
  现场验证过全流程（含真实 `quit`+重连验证死后登入地点没被永久
  篡改）。
