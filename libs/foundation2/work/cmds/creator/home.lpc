...
mixed cmd(string str) {
    ...
    if(!str) who = (string)this_player()->GetKeyName();
    ...
    if( ob = (object)this_player()->query_property("workroom") ) {
        ...
        if( (int)this_player()->eventMove(ob) ) {
            ...
            message("telin", (string)this_player()->GetMessage("telin"), ob, 
              ({ this_player() }));
            return 1;
        }
    }
      ...
      if( !(ob = load_object(str+"workroom")) &&
        ...
        if( (int)this_player()->eventMove(ob) ) {
            ...
            if(hiddenp(this_player()) || this_player()->GetInvis())
              return 1;
            message("mmout", (string)this_player()->GetMessage("home"), prev);
            message("telin", (string)this_player()->GetMessage("telin"),
    ...
    if( (int)this_player()->eventMove(ob) ) {
        ...
        message("mmout", (string)this_player()->GetMessage("home"), prev);
        message("telin",(string)this_player()->GetMessage("telin"),ob,({this_player()}));
        return 1;
    }
    return "You stay where you are.";
}
