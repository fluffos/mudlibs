string ask_tongchuan(object me)
{
    object ob;
    string msg;
...
//    message_vision( me->name()+"拿出一个金令交给$N。\n",ob);  这里描述不对：笑霸王拿出一个金令交给你。 
//  自己交给自己？？
    message_vision( "一枚金令交到了$N的手里。\n",ob);
    me = NPC_D->task_npc();
    ...
    switch(random(3))
    {
        case 0: msg = "本门急召「" + me->query("name") +
                      "」进殿商议，你速去「" + me->query("place") + "」通传。";
                break;
        case 1: msg = "本门有要事和「" + me->query("name") +
                      "」商量，你马上到「" + me->query("place") + "」去传令一下。";
                break;
        case 2: msg = "吾思念旧友「" + me->query("name") +
