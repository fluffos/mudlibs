...
if ((i/20-j) > 0)
  {
    message_vision ("$N对$n摇摇头：西游记现有"+chinese_number(i)+"位玩家，"
                    "此处仅有数位仙贤在邀，可否再邀"+chinese_number(i/8-j)+
                    "位仙贤前来观礼？\n",me,who);
    if (! DEBUG)                 
      return 1;
  }
...
//
           if(me->query("combat_exp")<4000000) {
                message_vision("$N摇了摇头，你还是再磨练磨练吧。\n",this_object());
               return 1; }
...
很容易会令自身堕入魔道的，你这就去禅房好好参悟刚才的战斗去吧。\n"NOR,me);
             ...
             if(me->query("combat_exp")>50000000)
                 {
/*
            if(!me->query("rulaitask_log/num"))
             {me->set("rulaitask_log/num",1);}
         if(me->query("quest/number")-1 < me->query("rulaitask_log/num"))
               {
             tell_object(me,CYN"你需要解一难,再来修行。\n"NOR);
         return 1;}
         me->set("rulaitask_log/num",me->query("quest/number"));
*/
        if (me->query_condition("rulai_job") > 1)
             tell_object(me,CYN" 暂时没有邪魔为非作歹，请稍后再来。\n"NOR);
               }
        //确定任务等级
        level = random(10);
        if (level<5) level=1;
        ...
        guai = new("/d/quest/stey/"+list[random(sizeof(list))]);
...
怪本性不坏，只是未能冲破迷津，你这就去点化它吧。\n"NOR,me);
        ...
        msg = "接到一个难度"+chinese_number(level)+"的"+kind+"妖任务。";
        MONITOR_D->report_quest_object_msg(me, msg);
        return 1;
}        
