...
void create()
{
        set("short", "小木屋"); 
        set("long", @LONG
这是间破旧而简陋的小木屋，但收拾的干干净净。木屋里只有
一床一桌一椅，几个粗碗，一盏瓦灯和一个红泥的火炉，每样东西
都很破旧。床上一个疲倦不堪的中年女子象孩子一般静静睡着。桌
子上摆了几碗饭菜，筷子旁边压着张便签。
LONG); 
        //{{ --- by MapMaker
        set("type","house");
...
{       
    ...
    if (arg=="桌子"||arg=="table")
    {
        tell_object(me,"一张破旧但很整洁的桌子，上面摆着两碗饭，一盆青菜，一碟霉豆腐，一碗腊肉，\n两双筷子整齐的摆在饭碗旁边。饭菜都一点没有动过，如果不是已经冷掉了的\n话，简直就象刚刚端上桌来的样子。其中一副碗筷的旁边，放着一个小酒壶，酒\n壶的旁边有一张纸笺。\n");
        me->set_temp("libie/离别钩_碗筷",1);
        return 1;
    }
    if (this_player()->query_temp("libie/离别钩_碗筷")&&(arg=="纸笺"||arg=="sheet"))
    {
...
铮郎已去三日，不归。妾先小憩，桌上饭菜亦候三日。但望与铮郎共食。 
                                                     素文\n"NOR,this_player());
        me->set_temp("libie/离别钩_纸笺",1);
        return 1;
    }
    if (lusuwen=present("lu suwen",this_object()))
    if (!living(lusuwen))    
    if (lusuwen->query("disable_type")==HIG"<睡梦中>"NOR)
    if (me->query_temp("libie/离别钩_纸笺")&&(arg=="吕素文"||arg=="lu"||arg=="lu suwen"))
        {
...
int valid_enter(object me)
{
        object lu,yang;
        int finale; 
        if(!me->query("libie/离别钩_尾声")&&lu=present("lu suwen",this_object())) 
        if(finale=me->query("libie/离别钩_杀死狄青麟"))
        {  
                if(finale==1)
                {
                        set("结局1人物",me);
                        me->set_temp("离别钩_目睹最后结局",1);
                        if(!living(lu))
...
{
   if(me->query_temp("离别钩_目睹最后结局"))
                return notify_fail(HIY"你想离开，但鬼使神差的却迈不动脚步。\n"NOR);
        return 1;
} 
void reset(){
        object ob;
