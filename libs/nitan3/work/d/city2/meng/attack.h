...
int get_exp_rate()
{
        ...
        foreach (object user in users())
        {
                ...
                if (count_gt(user->query("combat_exp"), max_exp))
                        max_exp = atoi(user->query("combat_exp"));
        }
...
void check_me(object me)
{
        ...
        me->set_temp("warequest/group", (group+1));
        env = environment(me);
        ...
        if (sizeof(ob) > 0 && ! me->query_temp("warequest/attack"))
        {
                for (i = 0; i < sizeof(ob); i++)
                {
                        ...
                        if (obb->query_temp("warequest/party") != "meng")
                        {
                                ...
                                me->set_temp("warequest/attack", 1);
                                me->set_temp("warequest/attacking", 1);
        ...
        &&  ! me->query_temp("warequest/attack")
        &&  me->query("qi") > me->query_temp("warequest/group") * 200
        &&  me->query("jing") > me->query_temp("warequest/group") * 100)
        {
                dirs = keys(exits);
        ...
        if (! me->query_temp("warequest/attack"))
                me->delete_temp("warequest/attacking");
...
varargs void attack_begin(object me, object target, object room, object env, string dir)
{
        ...
        if (objectp(weapon2))
                weapon2->wield();
        for (i = 0; i < me->query_temp("warequest/group"); i++)
        {
                if (objectp(target) && objectp(weapon2) && weapon2->query("equipped") && weapon2->query("is_bow") && stringp(dir))
                {
                        weapon2->do_shoot(target->query("id"), me, weapon2, room, dir, 1);
                        target->add_temp("warequest/reward", 1);
                }
                else
                if (room == env && objectp(target))
                {
                        ...
                        target->add_temp("warequest/reward", 2);
                } else break;
        }
        return;
}
...
void attack_over(object me)
{
        ...
        if (objectp(me))
        {
                message_vision(HIB"$N手中军刀一挥，蒙古骑兵攻势一缓，重新"
                                + "开始列阵，准备发起新一轮的冲击！！\n"NOR,
                                me);
                me->delete_temp("warequest/attack");
        }
        return;
}
