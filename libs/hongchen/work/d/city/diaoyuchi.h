...
void init()
{
    ...
    add_action("do_banned2",({"attack","sha","array","cast","conjure", "hit", "perform", "scribe"}) );
    add_action("do_banned2",({"array", }));
    add_action("do_ban_jiuyin",({"practice","lian"}));
}
...
{
    tell_object(this_player(),"钓鱼的时候乱闹岂不把鱼吓跑了？！\n");
    return 1;
}
int do_ban_jiuyin(string arg)
{
    object me=this_player();
    int skill;
    if( !arg 
      || (me->query_skill_mapped(arg) != "jiuyin-baiguzhao")
    ) return 0;
	...
	write("你的内力太弱，无法练九阴白骨爪。\n");
	return 1;
    }
    if( (skill = me->query_skill("jiuyin-baiguzhao",1)) > 1){
	message_vision(
	  HIG "$N用九阴白骨爪凌空乱抓了几下，似乎脸色有点不对劲。\n"NOR,me);
	me->set_skill("jiuyin-baiguzhao",skill - 1);
    }
    else{
	message_vision("$N随手在空中胡乱抓了几下。\n",me);
    }
    return 1;
}
...
int do_fish()
{
    ...
    if ( (int)me->query("combat_exp") > MAX_EXP)
      ...
      if ( ((int)me->query("potential") - (int)me->query("learned_points")) > 1000000)
      //last modified by wings
          return notify_fail("happy说你潜能太多，不许钓了。 \n");
...
void reward_me(object me)
{
	...
	tell_object(me,reward+"点潜能。\n");
     me->add("money",5000);
      tell_object(me,"你得到了五十两白银,已经帮你存到您的户口里去了。\n");
	return ;
    }

    if (random(kar+cps)>((kar+cps)/5)*3) {
	message_vision("$N钓到一条白鲳鱼\n", me);
	ob = new("/d/city/obj/fish_white");
	ob->move(me);
