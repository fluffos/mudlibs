# zhongjidiyu_zhijian — 地狱无门 ("No Escape From Hell") — archive #80
...
Doing Lu for hell (2K)"*) — same broad family as `xinkuangxiangkongjian2`,
`kuangxiangkongjian`, `yuxuechongsheng`, `wuhanzhan`, `huoying`, `shenzhou`,
...
(`终极地狱之爱若幽兰1.166正式版.rar` → `libs/zhongjidiyu_airuoyoulan`).
...
(`zhongjidiyu_zhijian`) is confirmed ES II/XKX lineage (`master.lpc`
...
- `mudlib directory`/`binary directory`: absolute paths to
  `libs/zhongjidiyu_zhijian/work` (both, matching the archive's original
...
- `log directory : /log` resolves relative to the driver's own launch
  CWD (§6), NOT a mudlib-virtual path — created `libs/zhongjidiyu_zhijian/
