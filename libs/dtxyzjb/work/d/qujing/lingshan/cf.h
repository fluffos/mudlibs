...
me->start_busy(2);
        ...
        if (me->query("rulaitask/accomplish")) {reward(me);me->add_temp("finish",1);};//huarong 2003.7
...
void reward(object me)
{
	...
	kind  = me->query("rulaitask/kind");     
        ...
        if ( level == 1 ) level_bonus=200;
        else if ( level == 2 ) level_bonus=300;
        else level_bonus=500;
        
        if ( kind == "杀" ) kind_bonus=200;
        else if ( kind == "擒" ) kind_bonus=300;
        else if ( kind == "劝" ) kind_bonus=500;
        else kind_bonus=600;
        
        extra_bonus = (me->query("combat_exp")/1000)/5;
        
//if(extra_bonus > 2000) extra_bonus = 2000;//太高了,比原来翻了一倍!!!
//if(extra_bonus > 1000) extra_bonus = 1000;
if(extra_bonus > 700) extra_bonus = 700;//huarong 2003.10
        potential_bonus = random(extra_bonus) + level_bonus + kind_bonus;
        combat_exp_bonus = extra_bonus*3 + (random(level_bonus) + random(kind_bonus))*3;
        daoxing_bonus = extra_bonus*3 + (random(level_bonus) + kind_bonus/2+random(kind_bonus/2))*3;
