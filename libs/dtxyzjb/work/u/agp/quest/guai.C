//modified by kuku@sjsh 2003.10.4
...
void create()
{        
        set_name(HIC"护宝妖怪"NOR, ({"yao guai"}) );
...
int do_kill(string arg)
{
        ...
        if(me->query("owner")&&me->query("owner")==who){
...
{
   object killer;  
   int exp_reward,dx_reward,qn_reward;
   
   exp_reward=900+random(600);
   dx_reward=1000+random(500);
   qn_reward=300+random(200);  
   killer = query_temp("last_damage_from");
        command("say 护宝经年，今日却败在小毛孩手上，我没脸活了。");
        tell_room(environment(),"说完，"+query("name")+"招来彩云往天上飞去，消失了。\n");
   if(this_object()->query("owner")!=killer){
        if(random(10)<2){
                killer->add("combat_exp",exp_reward);
                killer->add("daoxing",dx_reward);
                killer->add("potential",qn_reward);
                message("channel:chat",YEL"【小道消息】店小二："+killer->query("name")+"杀死护宝妖怪，奖励"+exp_reward+"点武学、"+COMBAT_D->chinese_daoxing(dx_reward)+"道行和"+qn_reward+"点潜能！\n"NOR,users()); 
                tell_object(killer,HIW"由于杀妖有功，你被奖励了"+exp_reward+"点武学、"+COMBAT_D->chinese_daoxing(dx_reward)+"道行和"+qn_reward+"点潜能。\n" NOR);
        }
        else if(random(10)<6){
                killer->add("combat_exp",exp_reward);                   
                killer->add("potential",qn_reward);
                message("channel:chat",YEL"【小道消息】店小二："+killer->query("name")+"杀死护宝妖怪，奖励"+exp_reward+"点武学和"+qn_reward+"点潜能！\n"NOR,users()); 
                tell_object(killer,HIW"由于杀妖有功，你被奖励了"+exp_reward+"点武学和"+qn_reward+"点潜能。\n" NOR);
        }
        else{
                killer->add("daoxing",dx_reward);
                killer->add("potential",qn_reward);
                message("channel:chat",YEL"【小道消息】店小二："+killer->query("name")+"杀死护宝妖怪，奖励"+COMBAT_D->chinese_daoxing(dx_reward)+"道行和"+qn_reward+"点潜能！\n"NOR,users()); 
                tell_object(killer,HIW"由于杀妖有功，你被奖励了"+COMBAT_D->chinese_daoxing(dx_reward)+"道行和"+qn_reward+"点潜能。\n" NOR);
        }   
        }
