...
void setname()
{
        ...
        string name;
        name=ob->query("name")+" 强盗";
        set_name(name, ({ob->query("id") + " qiang dao"})); 
...
void die()
{
        object killer,tu,gold;  
                
        killer = query_temp("last_damage_from"); 
                
        if( this_object()->query("owner")&&this_object()->query("owner")==killer){
                if(killer->query("level")<=50){
                gold=new("/obj/money/gold");
                gold->set_amount(random(11));
                gold->move(environment());      
                tell_room(environment(),"由于跑的匆忙，强盗身上的钱丢了。\n");
                if(random(100)<16){
                        tu=new(__DIR__"cbt");
                        tu->move(environment());
                        command("say 完了完了，我的好东西要被抢去了。");
                        tell_room(environment(),"说完，"+query("name")+"飞也似的逃了。\n");
                        tell_room(environment(),"从强盗身上好象掉出一样东西。\n");
                }
        }  
                else {
                        gold=new("/obj/money/gold");
                        gold->set_amount(random(6));       
                        gold->move(environment());      
                        tell_room(environment(),"由于跑的匆忙，强盗身上的钱丢了。\n");
                        if(random(100)<16){
                                tu=new(__DIR__"cbt");
