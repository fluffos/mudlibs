...
{    
    if ( victim->query("taskguai") && victim->query("owner") && killer != victim->query("owner") && killer->query_temp("invoker") != victim->query("owner") )
    {
        return notify_fail(victim->query("name") + "瞪了你一眼，你觉得手软脚软了。\n");
    }
    ...
    if ( userp(victim) && userp(killer) )
    {
        ...
        if ( (killer->query("combat_exp") + killer->query("daoxing")) < 5000 ) 
        {
            return notify_fail("你自己功夫还没练好，就想要杀人？\n");
        }
...
int set_to_killer(object killer,object target)
{
    ...
    killer->delete_temp("is_killer");//清除上次记录。
