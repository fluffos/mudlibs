...
void invite_one(object me,string pos);
	...
	int do_cast(string arg) {
	    write("挑战中请勿使用法术,多谢合作。\n");
	    return 1;
	}

	int do_ji(string arg) {
	    write("挑战中请勿使用法宝,多谢合作。\n");
	    return 1;
	}
	...
	int do_tiaozhan()
	{
		...
		if(me->query("weiwang") < 360)
			 return notify_fail("你何德何能敢出头挑战?\n");
		if(me->query("faith") < 360)
			 return notify_fail("你何德何能敢出头挑战?\n");
	...
	void invite_one(object me,string pos)
	{
		...
		while(i--){
		    ...
		    if (list[i]->is_fighting())
				continue;
		    if(list[i]->query("faith")<100)
		    		continue;        
	   	    if(list[i]->query("weiwang")<150)
		    		continue;    
	...
	void check_yingzhan(object me,object target,string faname)
	{
		if(environment(target)!=this_object()
		    &&target->query_temp("ask_yingzhan")){
		       ...
		       target->add("weiwang",-10-random(5));
		}
	}
