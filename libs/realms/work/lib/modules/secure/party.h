...
private object service =
    load_object("/lib/modules/secure/dataServices/partyDataService.lpc");
...
private nomask string memberType(object member)
{
    ...
    if (member(inheritList, "/lib/realizations/companion.lpc") > -1)
    {
        ret = "companion";
    }
    else if (member(inheritList, "/lib/realizations/henchman.lpc") > -1)
    {
        ret = "henchman";
    }
    else if (member(inheritList, "/lib/realizations/npc.lpc") > -1)
    {
        ret = "npc";
    }
    else if (member(inheritList, "/lib/realizations/player.lpc") > -1)
    {
        ret = "player";
    }
    return ret;
...
{
    string ret;
    object *potentialLeaders = keys(information["following"]);
...
private nomask void saveParty()
{
    if(Creator)
    {
        mapping saveData = ([
            ...
            "members": memberInfo(keys(Members)) +
