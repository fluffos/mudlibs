...
/////////////////////////////////////////////////////////////////////////////
protected nomask void loadTraits(mapping data, object persistence)
{
    if (isValidPersistenceObject(persistence))
    {
        traits = persistence->extractSavedMapping("traits", data);
        string *traitList = keys(traits);
        if (sizeof(traitList))
        {
...
/////////////////////////////////////////////////////////////////////////////
protected nomask mapping sendTraits()
{
    mapping ret = ([
        "traits":([]),
        "temporaryTraits": implode(keys(mkmapping(temporaryTraits)), "##")
    ]);

    string *list = filter(keys(traits),
        (: (member(temporaryTraits, $1) < 0) :));

    foreach(string trait in list)
    {
        string *keys = keys(traits[trait]);
        if (sizeof(keys))
        {
            ret["traits"][trait] = ([]);
