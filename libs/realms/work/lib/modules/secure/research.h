...
/////////////////////////////////////////////////////////////////////////////
protected nomask void loadResearch(mapping data, object persistence)
{
    if (isValidPersistenceObject(persistence))
    {
        ...
        string *researchItems = keys(research);
...
private nomask mapping sendResearchMapping(mapping input)
{
    ...
    string *researchList = keys(input);
    foreach(string researchItem in researchList)
    {
        string *keys = keys(input[researchItem]);
        if (sizeof(keys))
        {
            ret[researchItem] = ([]);
...
/////////////////////////////////////////////////////////////////////////////
protected nomask mapping sendResearch()
{
    return ([
...
private nomask void removeCompositeResearch(string player, string name,
    string constraint)
{
    load_object("/lib/modules/secure/dataAccess.lpc")->removeCompositeResearch(
...
{
    load_object("/lib/modules/secure/dataAccess.lpc")->removeSavedResearch(
...
private nomask void removeConstructedResearch(string player, string name,
    string constraint)
{
    load_object("/lib/modules/secure/dataAccess.lpc")->removeConstructedResearch(
        player, name, constraint);
}
