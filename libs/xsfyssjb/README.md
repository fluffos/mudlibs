...
- 以古龙小说为背景，地图偏重大漠/西域风情：楼兰、新疆、藏北、流
  沙等场景齐全，和常见的中原门派地图（华山、武当、少林、丐帮等）
  并存，突出古龙笔下"大漠孤烟"式的江湖。
- 注册时除了性别，还要选择民族（0-3），和同为古龙背景的 `wqfy`
  一样体现了跨民族的世界观设定，但两者代码库并不相同。
- 有 `dynamic_npc_quest` 目录，暗示这份档案带有动态生成 NPC 任务
  的机制，而非纯手写的固定任务列表。
