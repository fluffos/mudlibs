...
int perform(object me, object target)
{
 ...
 ob = me->select_opponent();
    ...
    if( !me->is_fighting() )
        return notify_fail("「海潮汹涌」只能在战斗时使用。\n");
    ...
    if( (int)me->query_skill("sword") < 100 ||
        ...
        return notify_fail("你的「玄铁剑法」还不够精纯，无法使用「海潮汹涌」！\n");
 msg = HIB "\n$N大吼一声，手持玄铁剑挟巨浪潮涌之势,从轻而响，从响而轻，忽寂然\n
无声，忽轰轰之声，反复三次，化做阵阵剑潮向前席卷而去。\n" NOR;
    message_vision(msg, me);
    me->clean_up_enemy();
    me->add("neili", -350);
    me->set_temp("perform",time()+3);
    me->set_temp("perform_haichao",1);
    for(i = 0; i < 3; i++)
        if (me->is_fighting(ob) && ob->is_fighting(me) && ob->query("eff_qi")>0){
              me->set_temp("action_msg",HIR"潮声不绝\n"NOR);
