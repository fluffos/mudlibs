...
int do_wancheng(string arg)
{
	...
	if(target->query("victim_idd") != me->query("id"))
...
//调高一些奖励 	
        exp=me->query_int()*20+me->query("kar")*200+me->query_skill("literate")*2+random(me->query_skill("literate")*2);  

	if (exp > 92000) exp = 92000;
        pot=exp/10;
