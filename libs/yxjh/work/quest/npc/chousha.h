...
int i=random(sizeof(master1));
int j=random(sizeof(master2));
private nosave string fam1=menpai1[master1[i]];
private nosave string fam2=menpai2[master2[j]];
...
int chousha_begin()
{
  int k;
   ...
   message("channel:chat", HIB"【仇杀】"+master1[i]+
   "："+fam1+"门下弟子听令，命你们在一个时辰内将\n"
   +fam2+"彻底击溃，以匡武林正义！\n"NOR,users());

   message("channel:chat", HIB"【仇杀】"+master2[j]+
...
void chousha_close()
{
   ...
   message("channel:rumor", HIB"【仇杀】历时半月的"+fam1+"与"+fam2
   +"之间的仇杀终于结束了，双方皆死伤无数！\n"NOR,users());
