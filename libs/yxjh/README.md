...
《浴血江湖3梦回江湖》，KMMUD.COM 出品，新角色从祀奉五代名将铁枪王彦章的乡野古庙"铁枪庙"起步（庙旁还有一座乌鸦筑巢的高塔），战斗系统把攻击力、躲闪、防御、招架分开结算，招式描述随部位和武器变化；注册流程为英文 ID、一次性输入的中文全名（不分姓和名）、管理密码与登陆密码双密码制、天赋菜单（0 随机整组，y 接受）、电子邮件与性别。地图和"天涯"家族的 `wlhd`（武林浩荡/金庸梦II）几乎逐字节一致（连"世界之巅"跳崖场景都用的是 `wlhd` 独立改写过的版本），同时和同属天涯家族的 `yzxiiizylfy`（夕阳再现III之炎龙封印）共享同一座铁枪庙起点、约 85% 的档案路径，但共享部分里只有约三分之一逐字节相同，是分化独立的姊妹档案而非重复打包。
...
- 新角色从"铁枪庙"（祀奉五代名将铁枪王彦章的乡野古庙，庙旁有座乌
  鸦筑巢的高塔）起步，战斗系统数值感很足（攻击力/躲闪/防御/招架
  分开结算，招式描述随部位、武器变化）。

## 深度功能测试新发现的 bug（详见 NOTES.md）

- `logind.lpc` 里两条并行取名流程（接受随机中文名 / 自己手动输
  入）都各自残留了一行调试用的 `printf("%O\n", ob)`，把登录物件的
  内部路径打在"请输入你的管理密码："提示之前，每一个新玩家注册时
  都会看到（AGENTS.md §7.34）。已删除。
- 新玩家食物/饮水满值初始化的判断条件写成了 `ob->query("age") ==
  14`（`ob` 是登录连线物件，从来没有人给它设过 age），应该是
  `user->query("age")`（玩家身体，新角色创建瞬间的 age 正好是
  14）——和 `cctx`/`niaoren` 是完全不同的血统，独立犯了同一个错误
  （AGENTS.md §8.9 第三个确认实例）。已修复；修复前所有新角色食
  物/饮水槽永远是空的，修复后正确显示满值。
