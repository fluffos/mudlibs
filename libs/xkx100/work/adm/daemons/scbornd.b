...
string *bid=({"jolin","waiter","solar","raul","dugu"});
...
varargs int save_perform(object user,string skill,string pfm)
{
	...
	ob->set("scborn/data/scborn/perform/"+skill,1);
	return 1;
}
...
// 六转 应该是 reborn(user);
	switch(times)
	{
		...
		case 1:
			...
			save_skill(user,etc1,1);           //保存一个知识技能
