...
AKAI Studio 出品、Tomud 品牌西游记世界观的一个分支，和同宗档案 `xyj2006n` 图档目录结构逐字节相同，连线握手方式也一样：连线后第一行必须是裸的 `2060`（Tomud/"笑傲江湖"客户端的自动应答），否则会被判定为非法客户端并断线；新角色从南城客栈起步，可以拜入灵台方寸山三星洞门下，由二代弟子广云子收徒，入门时会有一位赠礼 NPC 送上十三门基础武功作为新手大礼。

## 内容亮点

- 和 `xyj2006n`/`dtxywzxzb` 是同一套底层代码库的又一个发行版本
  （地图目录结构逐字节相同），标题里的"最终幻想"应是这个特定分支
  自己加上的品牌噱头，和游戏内容本身无关。
- 本次修复是本项目里第一次完全依靠自动化工具（`lib_bulk_fix.py`
  新规则 + `scan_known_bugs.py`）在开机测试之前就把已知的 5 类 bug
  全部提前修好，而不是像 `xyj2006n` 那样一次次靠开机报错慢慢定位。
