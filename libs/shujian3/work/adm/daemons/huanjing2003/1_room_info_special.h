...
void lost_hp( object me, string id )
{
    ...
    write(HIB"你忽地感到一阵强烈的诡异气息不知从哪儿散发出来，只觉得心烦气闷，竟似灵魂正在与自己远离一般！"NOR"\n");
...
void checking_job1( object me, string id )
{
    ...
    // 好的！获得奖励。
    message_vision( HIY"$N"HIY"正在凝神细思，忽地自己所带权杖都发出了耀眼光芒，不约而同地指向幽灵族的权杖！"NOR"\n",me);
    msg = HIY + "";
...
void checking_job2( object me, string id )
{
    ...
    if( yeah && ling )
        message_vision( HIB"$N"HIB"身上的$n"HIB"忽然发出一阵鬼魅之声，周围骤地厉声四起，似是响起了一片呼应！"NOR"\n", me, ling);
    ...
    kill_npc->move( rooms );
    message_vision( BLU"忽地一阵阴风掠起，$N"NOR+BLU"骤然现身，诡异之极。"NOR"\n", kill_npc);
    // JOB 奖励由该 npc 实现。
