...
void create()
{
    ...
    set("host_id","no");
    set("msg", HIW"只见一个人影渐渐清晰，终于凭空出现了一个陌生的人。"NOR"\n");
...
void wait_singsong()
{
    if( singing == 0 )
    {
        ...
        message_vision(CYN"$N"NOR+CYN"微微一笑，轻声唱起了歌来。"NOR"\n",this_object());
...
void sing_song()
{
    message_vision(CYN"      "+songs[i]+NOR"\n",this_object());
    i++;
...
void song_over()
{
    message_vision(CYN"\n$N"NOR+CYN"唱罢，又是微微一笑，似是想起许多往事。"NOR"\n",this_object());
    singing=0;
}
...
void delete_me()
{
    ...
    if(my_host && my_host->query_temp("huanjing") =="start" &&
       my_host->query_temp("hj_find_ok") != "yes" )
        tell_object(my_host,HIR"你心中似乎掠过一丝不安，却又不知道为什么。"NOR"\n");

    message_vision(HIW"$N"HIW"似乎满心失落地张望了一下，摇了摇头，忽地消失不见了。"NOR"\n",this_object());
    remove_call_out("sing_song");
...
int ask_baowu()
{
    ...
    {
        message_vision(CYN"$N"NOR+CYN"微微笑了一笑，却并不说话。"NOR"\n",ob);
        return 1;
    }
...
// 下调，带上一个即可
    ...
    if(!obj1 && !obj2)
    {
        message_vision(CYN"$N"NOR+CYN"似乎刚说点什么，忽地想起什么似的微微笑了一笑，却又不说话了。"NOR"\n",ob);
        return 1;
    }
...
// 成功寻到该族宝物
    ...
    destruct(my_gem);
    command("idle2");
    message_vision( CYN"$n"NOR+CYN"微微一笑，把一个"+baowu->query("name")+NOR+CYN"递给了$N"NOR+CYN"。"NOR"\n", me, ob);
...
// 新添物品“幽灵之令”，可在 hj_room1.c 快速得到幽灵武士JOB。
    ...
    message_vision( CYN"$n"NOR+CYN"又拿出了一个"+baowu->query("name")+NOR+CYN"递给了$N"NOR+CYN"。"NOR"\n",me,ob);
    command( "say “近日林中竟有幽灵族人现身，请带上这个令牌，或有些许帮助。”");
    message_vision(CYN"$N"NOR+CYN"说完，身影突然朦胧，竟然就此消失不见！"NOR"\n",ob);
    message_vision(CYN"$N"NOR+CYN"细细思索着，一时间竟然呆住了。"NOR"\n",me);
