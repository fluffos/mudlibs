...
int do_gheal(string arg)
{
    ...
    if(target == me )
    {
        ...
        message_vision(CYN"$N"NOR+CYN"定下心来，静静地调和自己的气息。"NOR"\n",me);
    }
    else
    {
        ...
        message_vision(CYN"$N"NOR+CYN"把手掌贴在$n"NOR+CYN"的背上，运起「恢复之技」来。"NOR"\n", me, target );
    }
...
void healing( object me)
{
    ...
    if( !target
      ...
      || present( "hj temp leave obj", target )
         ...
         return hj_halt( CYN"$N"NOR+CYN"内劲一撤，停止了运用恢复之技。"NOR"\n" );
    ...
    {
        if( me == target )
            return hj_halt( CYN"$N"NOR+CYN"调息已顺，长长地舒了一口气。"NOR"\n");
        return hj_halt( CYN"$N"NOR+CYN"见得$n"NOR+CYN"已经调息顺畅，撤回了掌来。"NOR"\n");
    }
    ...
    if(target == me)
    {
        ...
        message_vision(CYN"$N"NOR+CYN"定心调和气息，只觉得伤势恢复了些。"NOR"\n",me);
          ...
          && me->query_temp("hj_game_mepower") > 20 )
        {
            ...
            tell_object(me,HIY"你只觉得元气恢复了许多，但力气却似乎小了。"NOR"\n");
        }
        */
        // 但也可能获得等级提升
        if( !random( me->query_temp("hj_game_damages/heal") * 2 ) )
        {
            ...
            else tell_object(me,HIY"你对「恢复之技」的领会又进了一层！"NOR"\n");
        }
        remove_call_out("healing");
        call_out("healing", 1, me );
        return;
    ...
    if( me->query_temp("hj_hp") < 100 )
        return hj_halt( CYN"$N"NOR+CYN"正在帮助$n"NOR+CYN"疗伤，却发现自己气息不畅，只得罢手。"NOR"\n");
    ...
    message_vision(CYN"$N"NOR+CYN"全力施行为$n"NOR+CYN"疗伤，只见得$n"NOR+CYN"的脸色好转了许多。"NOR"\n", me, target);
    // 帮助他人(无论是否NPC)，将获得一小点的加分，但需要消耗自身的气息。
    // 已更改，需要消耗自己气息（小量），但不会有得分。
    me->add_temp("hj_hp",-random( heal_lv /3 ) );
    message_vision(CYN"$N"NOR+CYN"聚精会神，转眼间脸上已现出疲惫之色！"NOR"\n",me);
    remove_call_out("healing");
    call_out("healing", 1, me );
    return;
}
...
"NOR"\n");
    ...
    if ( arg == "fail" )
    {
        ...
        write("你输入了强制退出指令，视作失败的情况退出 幻境 1 游戏。"HIR"本指令将扣除你一点贡献度！"NOR"\n");
    ...
    message_vision(CYN"$N"NOR+CYN"高声叫道：“现实映现！”"NOR"\n",me);
    write(HIR"使用本指令，将扣除你的 500 点游戏得分！"NOR"\n");
    me->add_temp("hj_score" , - 499);
    me->start_busy(3);

    qxd->set("use_huigui","yes");

    message_vision(WHT"只见一阵白雾不知从何处飘来，愈来愈浓……"NOR"\n",me);
    return 1;
}
...
int do_halt()
{
    ...
    if( temp == "healing" )
    {
        ...
        if( target == me )
        {
            hj_halt(CYN"$N"NOR+CYN"急吸一口大气，停止了调整气息。"NOR"\n");
            return 1;
        }
        hj_halt(CYN"$N"NOR+CYN"掌力一收，停下了对$n"NOR+CYN"的疗伤。"NOR"\n");
        return 1;
    }
    hj_halt(CYN"$N"NOR+CYN"虚晃一招，跃出战圈不打了。"NOR"\n");
    return 1;
}
