...
int do_wizhg()
{
    ...
    set("use_huigui","yes");
    message_vision( CYN"$N"NOR+CYN"大声叫道：“我是巫师，让我回家！！！”"NOR"\n"+WHT"只见一阵白雾不知从何处飘来，愈来愈浓……"NOR"\n",me);
    return 1;
}
...
//    write("checking.\n");
    ...
    if( !hp || hp < 1 )
    {
        ...
        message_vision(HIR"$N"HIR"忽然两眼发直，不能动弹了！"NOR"\n",me);
        me->start_busy(3);
        ...
        if(naihe && !naihe->query("env/no_hj_wizmsg") )
            tell_object( naihe, sprintf( HIR"幻境游戏通知奈何：%s"HIR"(%s"HIR")失败状态离开了游戏。 ( <set no_hj_wizmsg> 不再发送本类信息)"NOR"\n",
        me->query("name"), me->query("id") ));
    // add for yuchang.
    CHANNEL_D->do_channel( me, "rumor", sprintf( "听说%s[%s]探险失败，离开了幻境。", me->query("name"), me->query("id") ));
        return;
    }
    ...
    if( wait_over == "yes" && time > DENG_TIME / 5 )
    {
        ...
        naihe = find_player("naihe");
        if(naihe && !naihe->query("env/no_hj_wizmsg") )
            tell_object( naihe, sprintf( HIR"幻境游戏通知奈何：%s"HIR"(%s"HIR")成功状态离开了游戏。 ( <set no_hj_wizmsg> 不再发送本类信息)"NOR"\n",
        me->query("name"), me->query("id") ));
    // add for yuchang.
    CHANNEL_D->do_channel( me, "rumor", sprintf( "听说%s[%s]探险成功，离开了幻境。", me->query("name"), me->query("id") ));
        return;
    }
    ...
    if( wait_over !="yes" && time > DENG_TIME )  // 时间到，结束游戏（ 不算 fail ）
    {
        ...
        else
        {
            ...
            else set("long",BLU"这是一个四四方方的小盒子，盒盖透明，里面已经一丝亮光也见不到了。"NOR"\n"  + get_deng_pic(deng) );
        }
    }
        ...
        if(me->query_temp("hj_hp") < (me->query_temp("hj_hp_max") *2 / 10) )
        {
            if( random(10) == 1) 
                tell_room(environment(me),
                    RED+me->query("name")+""RED"一副头重脚轻的样子，看来就要支持不住了。"NOR"\n",({me}));
        }
        else if(me->query_temp("hj_hp") < (me->query_temp("hj_hp_max") *4/10) )
        {
            if( random(10) == 1) 
                tell_room(environment(me),
                    HIR+me->query("name")+""HIR"受伤甚为严重，堪堪就要倒下似的。"NOR"\n",({me}));
        }
        else if(me->query_temp("hj_hp")<(me->query_temp("hj_hp_max") *6 /10 ) )
        {
            if( random(10) == 1) 
                tell_room(environment(me),
                    HIY+me->query("name")+""HIY"似乎受了伤，看起来状况不大好。"NOR"\n",({me}));
        }
    }
    // 这里为了回归之镜的。
...
void game_over()
{
    ...
    message_vision(HIR"$N"HIR"突然一副灵魂出窍的样子，紧接着消失不见了。"NOR"\n",me);
    me->move( HJ_OUT_ROOM );
    message_vision(HIR"$N"HIR"身影逐渐显现，似是回到现实里来了。"NOR"\n",me);
...
void sing_song(int i, object me)
{
    ...
    if (i<sizeof(songs))
    {
        if(i==0) message_vision(CYN"$N忽地似乎感到一阵歌声又在耳边响起，不禁凝神细听起来。"NOR"\n",me);
        else write(MAG"   "+songs[ i ] + NOR"\n");
        i ++;
        remove_call_out("sing_song");
    ...
    else
    {
        ...
        message_vision(CYN"你听得声音渐弱，终于静止下来了。"NOR"\n",this_object());
        write("\n在灯灭之后，就是你归来之时……若你气息(ghp)已无，就将失败而归……\n");
        write(HIW"在幻境里，你只能使用特别的指令来进行攻击。详细请用 <help huanjing> 指令查询。"NOR"\n");
...
void enter_hj()
{
    ...
    message_vision(HIR"$N"HIR"踏步走出，忽然间感到一阵强烈的震荡！……"NOR"\n",me);
...
*/
    ...
    //    tell_object(me,HIB"你忽地觉得一片寂静，刚才的声音丝毫都听不见了。"NOR"\n");
    //    if(!wizardp(me)) me->delete("channels");

    naihe = find_player("naihe");
    if(naihe && !naihe->query("env/no_hj_wizmsg") )
        tell_object( naihe, sprintf( HIR"幻境游戏通知奈何：%s"HIR"(%s"HIR")进入幻境游戏。 ( <set no_hj_wizmsg> 不再发送本类信息)"NOR"\n",
            me->query("name"), me->query("id") ));

    // add for yuchang.
    CHANNEL_D->do_channel( me, "rumor", sprintf( "听说%s[%s]进入了幻境，开始探险。", me->query("name"), me->query("id") ));
    remove_call_out("checking");
    call_out("checking",1);
