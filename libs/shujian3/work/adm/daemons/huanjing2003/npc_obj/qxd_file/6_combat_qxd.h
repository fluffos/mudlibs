...
void do_attack( object me, object target, int damages )
{
    ...
    if( !random(50) )
    {
        message_vision( HIR"忽然一阵"HIY"金光"HIR"闪过，$N"HIR"这一击的威力加强了数倍！"NOR"\n", me );
        damages *= 2+random(2);
    }
    // 打NPC时
    ...
    message_vision(NOR"（ $N"+describe+NOR" ）"NOR"\n",target);
    // 完毕。
}
...
void fighting()
{
    ...
    {
        message_vision( CYN"$N"NOR+CYN"身上的"+srl->query("name")+NOR+CYN"突然发出一阵光芒！"NOR"\n", target );
        return hj_halt( HIB"$N"HIB"忽感一阵莫名的巨大压力，只得罢下手来。"NOR"\n" );
    }
    // 目标离开游戏
    if( present( "hj temp leave obj", target ) )
    {
        return hj_halt( CYN"$n"NOR+CYN"身形忽隐忽现，竟似并无实体一般！$N"NOR+CYN"只得罢下手来。"NOR"\n" );
    }
    // 好的，下面准备开始攻击目标。
    // 忙时中，等待下一秒。
    if( me->is_busy() )
...
//            if( !use_tools ) use_tools = present( fight_use, environment( me ) );
    ...
    if( figh_type == "hand" )
    {
        ...
        tell_room(environment(me),CYN"\n"+me->query("name")+NOR+CYN"奋力向"+target->query("name")+NOR+CYN+temp+"！"NOR"\n",({me,target}));
        tell_object(target,CYN"\n"+me->query("name")+NOR+CYN"奋力向你"+temp+"，造成了 "HIR""+damages+""NOR+CYN" 点的伤害！"NOR"\n");
        tell_object(me,CYN"\n你奋力向"+target->query("name")+NOR+CYN+temp+"，造成了 "HIG""+damages+""NOR+CYN" 点的伤害！\n");
        if( !random(me->query_temp("hj_game_mepower") * 2) )
        {
            write(HIY"你只觉得打拳脚功夫多了，力量似乎又大了些。"NOR"\n");
    ...
    if( figh_type == "tools" )
    {
        ...
        if( use_tools->query("hj_game/obj") == "sword" )
        {
            ...
            tell_room(environment(me),CYN"\n"+me->query("name")+NOR+CYN"握起"+use_tools->query("name")+NOR+CYN"向"+target->query("name")+NOR+CYN+temp+"去！"NOR"\n",({me,target}));
            tell_object(target,CYN"\n"+me->query("name")+NOR+CYN"握起"+use_tools->query("name")+NOR+CYN"向你"+temp+"来，造成了 "HIR+damages+NOR+CYN" 点的伤害！"NOR"\n");
        ...
        else
        {
            ...
            message_vision(color_name+"\n$N"NOR+color_name+"举起$n"NOR+color_name+"大声呼唤道：“魔法速现！”"NOR"\n",me, use_tools);
            tell_room(environment(me),color_name+"只见"+use_tools->query("att_msg")+"向"+target->query("name")+NOR+color_name+"攻去，造成了 "HIG+damages+NOR+color_name+" 点的伤害！"NOR"\n",target);
            tell_object(target,color_name+"只见"+use_tools->query("att_msg")+"向你攻来，造成了 "HIR+damages+NOR+color_name+" 点的伤害！"NOR"\n");
        }
    }
    ...
    if( figh_type == "skills" )
    {
        ...
        tell_room(environment(me), color_name + "\n"+me->query("name")+NOR+color_name+"运起「"+skills_name+"」向"+target->query("name")+NOR+color_name+"攻去！"NOR"\n",({me,target}));
        tell_object(target,color_name+"\n"+me->query("name")+NOR+color_name+"运起「"+skills_name+"」向你攻来，造成了 "HIR+damages+NOR+color_name+" 点的伤害！"NOR"\n");
        tell_object(me,color_name+"\n你运起「"+skills_name+"」向"+target->query("name")+NOR+color_name+"攻去，造成了 "HIG+damages+NOR+color_name+" 点的伤害！"NOR"\n");
        ...
        if( !random( me->query_temp("hj_game_damages/"+use_skills) * 2 ) )
        {
            ...
            else tell_object(me,HIY"你对「"+skills_name+"」的领会又进了一层！"NOR"\n");
        }
    }
    // 绝技攻击
    if( figh_type == "pfm" )
    ...
    // 特别的技能或道具，令对方有忙时
    if( figh_type == "skills" && skills_name =="夺魂之技" )
    {
        me->add_temp( "hj_hp", -(3+random(3)) );
        // 若是使用夺魂之技，将额外需要更多的气息。
            ...
            && target->query_busy() < 6 )
        {
            ...
            tell_room( environment(me), sprintf( HIR"%s"HIR"突然两眼发直，整个人呆住了似的。"NOR"\n", target->name(1)), ({me,target}));
            tell_object( target, HIR"你突然两眼发直，整个人呆住了似的。 (造成忙时 "+target_busy+" 秒)"NOR"\n" );
            tell_object( me, HIR+target->name(1)+HIR+"突然两眼发直，整个人呆住了似的。 (造成忙时 "+target_busy+" 秒)"NOR"\n" );
        }
    }
    ...
    {
        message_vision( GRN"忽然不知从哪里引出一道"BLINK+HIG"闪电"NOR+GRN"劈向$N"NOR+GRN"，打个正中！"NOR"\n", target );
