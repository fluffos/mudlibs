...
int do_found( string arg )
{
    ...
    award_score = 30 + random(21);
    f_obj->move( me );
    message_vision(CYN"$N"NOR+CYN"只觉得$n"NOR+CYN"入手沉甸，已不似原先的感觉。"NOR"\n", me, f_obj);
    write(HIR"游戏提示：你的得分增加了 "+award_score+" 点！"NOR"\n");
...
int do_foundall()
{
    ...
    message_vision(CYN"$N"NOR+CYN"冲着$n"NOR+CYN"兴奋地说道：“我找齐那一套"+msg+"了！”"NOR"\n", me, ob );
    ...
    message_vision( CYN"$n"NOR+CYN"接过了这一套"+msg+"，接着伸掌在$N"NOR+CYN"脸前一晃。"NOR"\n", me, ob);
...
int do_huida(string arg)
{
    ...
    // 若玩家在本轮游戏里，已把5个预设的问题都答对过了，将不再发放问题。
        ...
        write(HIR"游戏提示：你只有60秒的时间！快快回答！"NOR"\n");
        remove_call_out("delete_me");
    ...
    if(me->query_temp("hj_score"))
    {
        ...
        write(HIR"游戏提示：你的游戏得分增加了 "+score_is+" 点！"NOR"\n");
    }
    set("asking","ok");
    call_out("delete_me",1);
    return 1;
...
int do_answer(string arg)
{
    ...
    if(arg == "here?")
    {
        ...
        // 最高可获得 80 分的奖励！很不错啦。
        write("你帮助了别人，觉得愉快极了。\n"HIR"游戏提示：你的得分增加了 "+award_score+" 点！"NOR"\n");
        me->add_temp("hj_score", award_score );
        remove_call_out("delete_me");        
        call_out("delete_me",1);
        return 1;
