...
// 其他文件内使用这个方式随机  set.
    ...
    set("msg", MAG"半空中忽地掉下一件物事，像是一件宝物！"NOR"\n");
    if( iii !=3 )
...
int do_zuhe(string arg)
{
    ...
    message_vision(my_color+"$N"NOR""+my_color+"取出"+names[0]+my_color+"、"+names[1]+my_color+"和"+names[2]+my_color+"，把它们组合成了"+my_msg+"的宝物——"+names[4]+my_color+"！"NOR"\n",me);
