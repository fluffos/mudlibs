void init()
{
	if (environment() == this_player())
		add_action("do_du", ({"read", "du", "study"}));
}
...
int do_du(string arg)
{
	...
	if( me->query_int() < ob->query("difficulty"))
		return notify_fail("你研读了一会儿，发现自己悟性太低，根本不能领悟到上面写些什么。\n");
	if(i < ob->query("min_skill"))
		return notify_fail("你研读了一会儿，但是发现上面所说的对你而言都太深奥了，没有学到任何东西。\n");
	if(i > ob->query("max_skill"))
		return notify_fail("你研读了一会儿，但是发现上面所说的对你而言都太简单了，没有学到任何东西。\n");
