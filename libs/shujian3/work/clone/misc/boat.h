...
{   
    ...
    if(stepw >10 && steps >10 && steps < 20){
        ...
        msg += "你似乎迷失了方向。\n";
          if(stepn && stepn>20){
              if(stepn>=130) msg += WHT"船已经很难再前进了，四周都是白茫茫的一遍，围满了浮冰。"NOR"\n";
              else if(stepn>=100) msg += HIW"船里的淡水也已经结冻，海面漂浮着一块块的浮冰。"NOR"\n";
              else if(stepn>=70) msg += HIB"阵阵寒风呼啸而过，海水里也漂来一块块小浮冰。"NOR"\n";
              else if(stepn>=50) msg += HIM"海风已经变得刺骨，天空也阴沉沉地。"NOR"\n";
...
int do_stop()
{
        ...
        message_vision("$N将"+this_object()->name()+"的帆缓缓将下，船便停在了大海上。"NOR"\n", me);
...
int do_start()
{
        ...
        if(room = find_object(query("boat_of"))){
               ...
               message_vision("$N大喊一声：开船喽！将"+this_object()->name()+"缓缓驶向大海。"NOR"\n", me);
               tell_room(room, HIY+this_object()->name()+"缓缓驶向了大海。"NOR"\n");
               delete("boat_of");
...
//宁波出海口在 w15,s15,
	if (steps>=14 && steps<=16 && stepw>=15){
                ...
                message_vision(HIY+ob->name()+"渐渐地在宁波溪口靠了岸，你可以上岸(out)了。"NOR"\n", me);
...
//灵蛇岛在 e100,s5,
	if (steps>=4 && steps<=6 && stepe>=99 && stepe<=101){ 
                ...
                message_vision("只听咚的一声，"+ob->name()+"触到了岸边。\n", me);
                message_vision(HIY+ob->name()+"渐渐地在一个不大的小岛旁靠了岸，你可以上岸(out)了。"NOR"\n", me);         
                remove_call_out("close_passage");
                call_out("close_passage", 20, ob);              
	        return 1;
	        }	
//冰火岛在 e200,n950,
	if (stepn>=948 && stepn<=951 && stepe>=198 && stepe<=202){
                message_vision(HIR"\n只听嘣地一声，"+ob->name()+"碰上了一座冰山，所有人都从船舱中被甩了出去！"NOR"\n");
...
void over(object ob)
{
     ...
     call_out("dest", 3, this_object());
       if(!obj) return;
        ...
        write(HIR"突然一阵狂风吹过，刮断了桅杆，海船也被抛上了天！"NOR"\n");
        for(i=0; i<sizeof(obj); i++) {
