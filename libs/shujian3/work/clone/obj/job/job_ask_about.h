...
void tell_player_family_job_contribute(object player,object master)
{
		...
		family_name!=master->query("family/family_name"))
	{
		...
		+RANK_D->query_self(master)+"怎么知道那种事情呢。"NOR"\n", player, master);
		return;
	...
	{
		message_vision(CYN "$n对$N说道：你去问"+master_call+"吧。"NOR"\n", player, master);
		return;
	}
	...
	{
		message_vision(CYN "$n对$N摇摇头说道："+family_name+"弟子不争气，还没为"+family_name+"做出任何贡献呢。"NOR"\n", player, master);
		return;
	}
	message_vision(CYN "$n对$N说道：目前我"+family_name+"的贡献度为"+chinese_number(family_map["job_contribute"])+"点。"NOR"\n", player, master);
	...
	message_vision(CYN "$n对$N说道：你为"+family_name+"所做的贡献为"+chinese_number(point)+"点。"+msg+NOR"\n", player, master);
	...
	if(!undefinedp(family_map["family_assess_num"]))
	{
		...
		+chinese_number(point)+"时，就召开下次的"+family_name+"大会。"NOR"\n", player, master);
	}
