int answer(string arg)
{
	...
	if(arg=="no" || arg=="n" || arg=="N" || arg=="No")
	{
		...
		if(me->query("gender")=="女性")
		{
			message_vision(HIR"\n$N脸色惨白，钢牙咬成玉碎，对$n一个万福说到：「承蒙公子错爱，然小女子福德浅薄，配不上公子人品……」"NOR"\n",me,who);
			command("chat* sigh "+getuid(me));
			command("chat "+ob->query("bwzh/host_player/name")+"既不愿嫁给他，为什么比武招亲？终身大事，如此轻薄！");
		}
		else
		{
			message_vision(HIR"\n$N撇了撇嘴，双手一摊，嘿嘿一笑：「与$n在拳脚上玩玩，倒也有趣。招亲嘛，哈哈，可多谢了！」"NOR"\n",me,who);
	...
	if(me->query("gender")=="女性")
	{
        message_vision(HIM"$N羞得双脸红烫，却万福答礼，轻若蚊声道：「女儿家……，不是摇头……，便是点头啦……」"NOR"\n", me);
		command("say 恭喜，恭喜！");
	...
	{
		message_vision(HIG"$N哈哈大笑：「这比武都比过了，如此贤妻，复又何求！」"NOR"\n", me);
		command("say 恭喜，恭喜！");
	...
	message_vision(HIY"\n突然一股清烟消散，一个满脸红光，须眉皆白的老人走了过来。"NOR"\n", me);
	command("ah "+obj->query("id"));
	message_vision(HIG"$N面带笑容，对月下老人一抱拳：「你老来了，今天可正好是个好日子！」"NOR"\n", me);
	message_vision(HIC"$N点了点头：「我就为这事来的。哈哈哈哈！双方如意，一切齐备，我这就为你们登记！」"NOR"\n", obj);
