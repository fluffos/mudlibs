...
{ 
	...
	if(!me ||!me->query_temp("quest/bwzh/hoster"))//缺少美女
	{
		...
		{
			tell_object(winner_player,HIR""+ob->query("bwzh/host_player/name")+"竟然离开现场！招亲中止！"NOR"\n");
		...
		{
			tell_object(challenge_player,HIR""+ob->query("bwzh/host_player/name")+"竟然离开现场！招亲中止！"NOR"\n");
		...
		if(me)
		{
			tell_object(me,HIR"你因为在「比武招亲」中无故离开现场，而被扣除了十万点经验！"NOR"\n");
	...
	if(!winner_player || !winner_player->query_temp("quest/bwzh/biwu_host"))
	{
		...
		for(i=0; i<sizeof(ob_list); i++) 
		{
			tell_object(ob_list[i],HIR"擂主正在变化中。。。。。。！比赛暂时中止！"NOR"\n");
			log_file("quest/zhaoqin",sprintf(HIC"%-8s%-10s 比武招亲中止checking"NOR,ob_list[i]->query("name"),ob_list[i]->query("id")),ob_list[i]);
			ob_list[i]->quit(ob_list[i]);
		}
		...
		if(challenge_player=ob->query("bwzh/challenge_player/object"))
		{
			...
			tell_object(challenge_player,HIR"通知："+ob->query("bwzh/winner_player/name")+"不再担任擂主，"+me->query("name")+"重新担任擂主！比赛继续！"NOR"\n");
		...
		tell_object(me,HIR"通知："+ob->query("bwzh/winner_player/name")+"不再担任擂主，你重新担任擂主！比赛继续！"NOR"\n");
		...
		if(winner_player)
		{
			...
			tell_object(winner_player,HIR"你因为在「比武招亲」中无故离开，而被扣除了十万点经验！"NOR"\n");
