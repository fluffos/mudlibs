...
//0 biwu_over 1 cancel
{
	...
	if(me) 
	{
		me->delete_temp("apply/short");  
 		if(i) tell_object(me,HIR"招亲活动比武被强行中止！"NOR"\n");
		else tell_object(me,HIR"时间到，招亲活动比武部分中止！"NOR"\n");		
	}
	winner_player=ob->query("bwzh/winner_player/object");
	if(winner_player)
	{
 		if(i) tell_object(winner_player,HIR"招亲活动比武被强行中止！"NOR"\n");
		else tell_object(winner_player,HIR"时间到，招亲活动比武部分中止！"NOR"\n");
	}
	challenge_player=ob->query("bwzh/challenge_player/object");
	if(challenge_player)
	{
		if(i) tell_object(challenge_player,HIR"招亲活动比武被强行中止！"NOR"\n");
		else tell_object(challenge_player,HIR"时间到，招亲活动比武部分中止！"NOR"\n");
