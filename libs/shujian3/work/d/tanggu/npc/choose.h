...
void choose_other(object me,int x)
{
	...
	if(x==1)
		tell_object(me,HIW"\n你已经设定【最低经验："+chinese_number(query("bwzh/exp"))+"兆】"NOR"\n");
	else if(x==2)
		tell_object(me,HIW"\n你已经设定【最低相貌："+chinese_number(query("bwzh/per"))+"】"NOR"\n");
	else 
		tell_object(me,HIW"\n你已经设定【年龄："+chinese_number(i)+"岁----"+chinese_number(p)+"岁】"NOR"\n");
		...
		&& query("bwzh/age")) 
	{
		...
		tell_object(me,HIW"\n你现在心目中的夫君的要求设置如下：\n"NOR"\n");
		tell_object(me,HIR"  1、【经验】最低经验"+chinese_number(query("bwzh/exp"))+"兆。"NOR"\n");
		tell_object(me,HIR"  2、【相貌】最低相貌"+chinese_number(query("bwzh/per"))+"。"NOR"\n");
		tell_object(me,HIR"  3、【年龄】在"+chinese_number(i)+"岁和"+chinese_number(p)+"岁之间。"NOR"\n"); 
		tell_object(me,HIW"\n如果你同意，可以选择："NOR"\n");
		tell_object(me,WHT"（Y）同意 （R）重选 （Q）取消"NOR"\n");
