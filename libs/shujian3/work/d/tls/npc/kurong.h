...
int ask_6m()
{
	...
	time = time() - ob->query("quest/6m/time");
	...
	if (ob->query("6m/fail") >=3 && ob->query("registered") < 3){
		command("hmm");
		command("say 我不是说了，我没兴趣教你啊！");
		return 1;
	}

	if ( ob->query("quest/6m/time") && time < 86400 ) {
		command("shake");
		command("say 你上次刚刚来问过，怎么这么快又来了，没看我正忙着吗。");
		return 1;
	}

	if ( ob->query("6m/fail") >= i ){
		command("shake");
	...
	if ( ob->query("combat_exp") < 1000000) {
		...
		command("say 你的武功底子实在太糟糕了，无法研习更高深的武功。");
		return 1;
	}

	if (random(ob->query("kar")) < 28 || random(ob->query_int(1)) < 50){
		command("hoho "+ ob->query("id"));
		command("say 你的福泽不够深厚，还是以后再说吧！");
		ob->set("quest/6m/time", time());
		log_file("quest/6m",sprintf("%8s%-18s从枯荣长老，失败，福：%d。\n",
	...
	message_vision("枯荣长老对$N说道：“我这里有一本我派六脉神剑的秘籍，你拿去自行好好钻研。只要你能\n"+
			"勤学苦练，将来未始不能无敌于天下。”\n", ob);
	ob1 = new("/clone/spec/six-finger");
	ob1->move(ob);
	ob1->set_temp("6m/book",ob->query("id"));
      ob->set("6m/pass",1);
	log_file("quest/6m",sprintf("%8s%-18s从枯荣长老处学得六脉神剑，福：%d。\n",
		me->name(1), "("+capitalize(me->query("id"))+")", me->query("kar") ), me);
	return 1;
}
...
string ask_jianpu()
{
        ...
        if (!me->query("6m/pass"))
                return "六脉剑谱？什么六脉剑谱？我不知道。";
        if (present("liumai jianpu",me))
                return "耶？你身上不是有一本吗？";
        ob = unew("/clone/spec/six-finger");
        if (!ob)
                return "你来晚了，剑谱已经被人拿走了！";
        ob->move(me);
        ob->set_temp("6m/book",me->query("id"));
        message_vision("枯荣长老递给$N一本剑谱。\n", me, );
        return "这本剑谱你拿去好好研读，将我们这门功夫好好发扬光大。";
}
...
void do_congra(object me, object fighter)
{
	...
	room->set("exits/out","/d/tls/songlin-1");
	message_vision(HIY"\n枯荣长老一掌向地拍去，身后轰然移开一扇石门，$N看也不看，走了过去。"NOR"\n",fighter);
