...
/*set killer's level*/
	...
	if(ob->query("combat_exp") >= 2000000){
	 ...
	 && ob->query("combat_exp") >= 300000){
		...
		killerskill1 = skill1[random(sizeof(skill1))];
		number = (int)(number*(4+random(2))/10) ;
	}
...
/* copy entire dbase values */
	...
	me->set("jiali", 20+random(50));

	if( ob->query("combat_exp") < 100000) {
        me->set("jiali", 0);
	me->set("max_qi",k_qi*3/6);
	me->set("eff_qi",k_qi*3/6);
	me->set("qi",k_qi*3/6);
          }
...
void die()
{
	...
	if(ob 
	&& objectp(ob) 
	&& ob->query_temp("dali_songxin"))
        {
        exp=50+random(ob->query_int(1));
        ob->add("combat_exp",exp);
//        ob->add("potential",exp/2);
        tell_object(ob,HIG"你杀死了"+me->query("name")+"，实战经验增加"+chinese_number(exp)+"点。\n"NOR);
        }
	::die();
}