...
int greeting(object me)
{
        ...
        tell_object(me,HIY"你站在小径上，四周打量，仿佛看见"+ me->query_temp("wdpass/steps") +"面有些亮光。"NOR"\n");
        return 1;
}
...
int valid_leave(object me, string dir)
{
	if( dir != me->query_temp("wdpass/step")){
		...
		if ( me->query_temp("wdpass/fail") >= 4 +random(3)
		  && dir == "north"){
		  	me->delete_temp("wdpass");
                        tell_room(environment(me), me->name()+"往北面的"YEL"小径"NOR"快步离开。"NOR"\n", ({ me }));
			me->move(__DIR__"xiaolu2");
                        tell_room(environment(me), me->name()+"从"YEL"小径"NOR"快步走了过来。"NOR"\n", ({ me }));
			return notify_fail("");
		}
	}
	else {
		...
		if (me->query_temp("wdpass/done") >= 9){
                        me->delete_temp("wdpass");
                        tell_room(environment(me), me->name()+"往南面的"WHT"院门"NOR"快步离开。"NOR"\n", ({ me }));
