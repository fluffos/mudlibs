...
int do_change(string arg)
{
	...
	if (sscanf(arg, "check %d", num) == 1){
		...
		msg = HIG"提示：如果要修改 "+num+" 点属性，你大约要扣除 "+tmpexp+" 点EXP，还剩 "+newexp+" 点EXP。"NOR"\n";

		if ( newexp < 95000 )
			msg += HIR"抱歉：修改"+num+"点属性后经验将不够 "+MIN_EXP+" ，建议你一次少修改几点属性。"NOR"\n";

		if (me->query_skill("jinshe-zhangfa",1) || me->query_skill("jinshe-jianfa",1))
			msg += HIR"注意：如果修改臂力高于20，金蛇剑法和金蛇掌法将被删除。"NOR"\n";

		if (me->query("double_attack") && me->query("zhou"))
			msg += HIR"注意：如果修改悟性高于25，左右互搏将被删除。"NOR"\n";

		if (me->query("dali") && me->query_skill("lingbo-weibu", 1))
			msg += HIR"注意：如果修改悟性低于22，凌波微步将被删除。"NOR"\n";

		if ( me->query("quest/qianzhu/pass") )
			msg += HIR"注意：如果修改根骨低于15或悟性低于20，千蛛万毒手将被删除。"NOR"\n";

		if ( me->query("quest/pixie/pass") )
			msg += HIR"注意：如果修改身法低于15或悟性低于20，葵花神功将被删除。"NOR"\n";

		if ( me->query("quest/ningxue/pass") )
			msg += HIR"注意：如果修改身法低于15或根骨低于15，凝血神爪将被删除。"NOR"\n";
...
private void confirm_passwd(string pass, object me, string from, string to, int num)
{
	...
	me->setup();
	me->save();
	write(HIY"你将"+num+"点"+gift_desc[from]+"调整到"+gift_desc[to]+"上。"NOR"\n", me);
...
int help()
{
	write(HIR"修改先天属性，每点属性修改将扣除 5% 经验值。"NOR"\n");
	...
	write("    身法  dex           悟性  int\n");
	write("\n");
	write(HIR"请慎重使用，操作失误后果自负，改错不补。"NOR"\n");
	return 1;
}
