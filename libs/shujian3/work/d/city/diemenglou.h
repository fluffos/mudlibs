...
int do_chakan(string arg)
{
	...
	if(me->is_fighting()) 
        return notify_fail(RED"你正忙着呢。"NOR"\n");
    if(me->is_busy()) 
        return notify_fail(RED"你正忙着呢。"NOR"\n");
	...
	if(!arg)
	{
		if(me->query("quest/special/score"))
			str = HBYEL"你蝶梦游戏分数总共累计："+chinese_number(me->query("quest/special/score"))+"分；其中当前游戏分数："+chinese_number(me->query_temp("quest/special/score"))+"分。"NOR"\n";
		else 
			str = HBYEL"你蝶梦游戏目前没有积分。"NOR"\n";
		if(me->query("quest/special/second_floor"))
		{
			...
			if(me->query("quest/special/gift"))
				str += YEL"并获得了"+chinese_number(me->query("quest/special/gift"))+"次宝物奖励。"NOR"\n";
			else 	
				str += YEL"只是还未有获得宝物奖励。"NOR"\n";
		}
		tell_object(me,str);
	}
	else if(arg=="top")
	...
	else
	{
			...
			if (!obj || !me->visible(obj)) return notify_fail(HIY"你要察看谁的状态？"NOR"\n");
			if( obj==me && !wizardp(me)) return notify_fail(HIY"查看自己单独使用chakan即可。"NOR"\n");
			if (!wizardp(me)) return notify_fail(HIY""+obj->query("name")+"的状态岂是你能查看的？"NOR"\n");
...
int do_liaoshang(string arg)
{
    ...
    if(me->query("eff_qi")>=me->query("max_qi")/3 && me->query("eff_jing")>=me->query("max_jing")/3) 
        return notify_fail(RED"你受伤不重，请使用yun heal疗伤。"NOR"\n");
    
    //疗伤
    if(me->query("eff_qi")<me->query("max_qi")/3)
    {
      me->set("eff_qi",me->query("max_qi")/3);
      tell_object(me,HIR"突然你感到一股神力充斥在房间之中，你的伤势竟然康复大半。"NOR"\n");
    }
    //疗精
    if(me->query("eff_jing")<me->query("max_jing")/3)
    {
      me->set("eff_jing",me->query("max_jing")/2);
      tell_object(me,HIR"突然你感到一股神力充斥在房间之中，你的精血恢复大半。"NOR"\n");
    }
    tell_room(environment(me),HIR"突然你感觉一股神力充斥在房间之中，原来"+me->name()+"正在招呼异力疗伤！"NOR"\n", ({ me }));
    return 1;
}
void again_check(object me) //为了防止dazhuan 类似的pfm 出现兵器留在上面的bug
{
	...
	for (i = 0; i < sizeof(obj); i++) {
		...
		{
			obj[i]->move(me);
			tell_room(this_object(),HIG"\n突然一个老人溜了进来看了看地上的"NOR+obj[i]->query("name")+HIG"，顺手捡了起来。"NOR"\n", ({}));
			x=1;
		}
		else
			destruct(obj[i]);
