...
int do_duijiang()
{
    ...
    if(!present("cai piao",me)){
        ...
        return notify_fail("你的摸彩凭证呢？"NOR"\n");
    }

    if(!query("no_buy")) return notify_fail("还没开奖呢，凑啥热闹？该干啥干啥去。。"NOR"\n");

    if(!(i=sizeof(me->query("caipiao")))) return notify_fail("你还没买彩票，来兑什么奖？一边凉快去！"NOR"\n");

    if(me->query("caipiao_id")!=This_Source){
        me->delete("caipiao");
        me->delete("caipiao_id");
        tell_object(me,"小样！拿过期的彩票来对奖？当Wiz都傻的啊？"NOR"\n");
    ...
    if(me->query("caipiao_zhongjiang_list")){
        ...
        if(me->query("caipiao/第"+chinese_number(i)+"张/huobi")=="黄金"){
            tell_object(me,HIY"恭喜!此次中奖,使你获得了"+HIW+chinese_number((int)me->query("caipiao_shot")*GOLD_COST/10000)+HIY+"锭黄金!"NOR"\n");
            me->add("balance",(int)me->query("caipiao_shot")*GOLD_COST);
        }
    }
    else tell_object(me, HIR"很遗憾,这张彩票,您没有中任何奖,感谢您的参与,祝您下次中奖!"NOR"\n");
