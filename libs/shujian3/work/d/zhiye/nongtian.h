...
void init()
{
	...
	if(query("show_msg")!=1 && fib>0)
        ...
        add_action("do_zhongzhi","zhongzhi");//需要输入
...
void exam()
{
        ...
        for(i=0;i<count;i++) {
                ...
                if( time() > (mpv[i]["atime"]+TIMEOUT_TICK) ) {
                        if(mpv[i]["action"]==3) {
                                if(me && userp(me)) 
                                        tell_object(me,RED"你错过了收割的时机，心血都浪费掉了！"NOR"\n");
                                overtime += ({ mpk[i] });
                                continue;
                        }
                        else {
                                if(me && userp(me))
                                        tell_object(me,HIR"你错过了" + (mpv[i]["action"]==1?"浇水":"施肥") + "的时机，植物长势受到影响。"NOR"\n");
                                mpv[i]["lost"]++;
                                decide_action(mpk[i]);
                        }
                }
                        ...
                        if(me && userp(me)) {           
                                switch (mpv[i]["action"]) {
                                        case 1: tell_object(me,GRN"你该给你所种植的作物浇水了。"NOR"\n"); break;
                                        case 2: tell_object(me,GRN"你该给你所种植的作物施肥了。"NOR"\n"); break;
                                        case 3: tell_object(me,HIG"你所种植的植物已经成熟，你可以开始收割了。"NOR"\n"); break;
                                }                                       
                        }
                }
        }
...
int do_zhongzhi(string arg)
{
        ...
        if( !arg ) {
                ...
                if (sizeof(inv) >0) {
                       ...
                       for(i=0;i<sizeof(inv); i++)
                        {
                            if(inv[i]->query("zhongzi")){
                                 str += HIY"种植 "NOR+inv[i]->query("name")+":zhongzhi "+file_name(inv[i])+ZJSEP;
                            }
                        }
                        if (str!="")
                            tell_object(me, ZJOBLONG + "\n"+ZJOBACTS2+ZJMENUF(3,3,9,30) + str+"\n");
                        else
                            tell_object(me, ZJOBLONG + "你身上没有种子。\n");
                }
                return 1;
        }
        ...
        destruct(zhongzi);      
        message_vision(HIY"$N开始用铁锹松土......"NOR"\n""\n", me);
        delay = 10 + random(20);
...
void zhongzhi_finish(object me,int step,int delay)
{
        ...
        if(!me) return;
        switch (step) {
                case 1: message_vision(HIY"然后$N用铁锹挖了一个坑......"NOR"\n", me); break;
                case 2: message_vision(HIY"$N把种子放到坑里......"NOR"\n", me); break;
                case 3: message_vision(HIY"$N小心的把坑埋好......"NOR"\n", me); break;
                case 4: message_vision(HIY"$N开始在种子上浇水......"NOR"\n", me); break;
                case 5: message_vision(HIY"$N开始在种子上施肥......"NOR"\n", me); break;
                case 6: message_vision(YEL"$N总算种好了！"NOR"\n", me); 
                        me->start_busy(-1);
                        me->interrupt_me();
                        for(i=0;i<3;i++) {
...
int do_jiaoshui(string arg)
{
        ...
        message_vision(HIW"$N从田地旁的水沟里挑了些水，开始浇地......"NOR"\n""\n", me);
        delay = 10 + random(20);
...
int improve;
        ...
        me->start_busy(-1);
        me->interrupt_me();
        message_vision(HIW"$N累地满头大汗，总算浇完了地。"NOR"\n", me);

        improve = (int)me->query_int();
        improve = improve*10 + random(improve*10);
        WORKER_D->check_impove(me,"nongsang",improve,2);                
        //me->add("combat_exp", improve/10);
       // tell_object(me,HIC"你的农桑熟练度增加了"+improve+"点，实战经验增加了"+(improve/10)+"点。"NOR"\n"); 
...
int do_shifei(string arg)
{
        ...
        message_vision(HIY"$N从田地旁的肥料坑里铲了些肥料，开始施肥......"NOR"\n""\n", me);
        delay = 10 + random(20);
        if(wizardp(me) && (int)me->query("env/test")) delay = 2;
        me->start_busy(delay+1);
...
int improve;
        ...
        me->start_busy(-1);
        me->interrupt_me();
        message_vision(HIY"$N累地满头大汗，总算完成了施肥的工作。"NOR"\n", me);
...
int do_shouge(string arg)
{
        ...
        message_vision(YEL"$N兴高采烈的开始收割......"NOR"\n""\n", me);
        delay = 10 + random(20);
        if(wizardp(me) && (int)me->query("env/test")) delay = 2;
        me->start_busy(delay+1);
...
//        improve = me->query_int(1)*i/100;
        ...
        message_vision(YEL"$N总共收获了"+CHINESE_D->chinese_number(bu)+obj->query("unit")+obj->name()+"。"NOR"\n", me);
        if(j) message_vision(YEL"但是$N拿不动那么多，只好把几"+obj->query("unit")+obj->name()+"放到地上。"NOR"\n", me);
        
        improve *= bu;
        improve = improve*2 + random(improve*2);
        //improve = improve*ZY_NONGSANG_MUL/100;
        i = improve * 12;
        WORKER_D->check_impove(me,"nongsang",i,2);
        j = improve+random(improve/5);
                
//        j = me->add_exp_combat( j ,0,"农桑",1);
        me->add("combat_exp", j);
       
        k = j/8;  me->add("potential", k);

        tell_object(me,HIC"你的农桑熟练度增加了"+i+"点，潜能增加了"+k+"点，实战经验增加了"+j+"点。"NOR"\n");      
