...
void exam()
{
	...
	for(i=0;i<count;i++) {
		...
		if( time() > (mpv[i]["atime"]+TIMEOUT_TICK) ) {
			if(mpv[i]["action"]==3) {
				if(me && userp(me))
					tell_object(me,RED"你错过了抽丝的时机，蚕蛹已经孵化为蝴蝶，心血都浪费掉了！"NOR"\n");
				overtime += ({ mpk[i] });
				continue;
			}
			else {
				if(me && userp(me))
					tell_object(me,HIR"你错过了" + (mpv[i]["action"]==1?"喂食":"清扫") + "的时间，蚕的成长受到影响。"NOR"\n");
				mpv[i]["lost"]++;
				decide_action(mpk[i]);
			}
		}
			...
			if(me && userp(me)) {
				switch (mpv[i]["action"]) {
					case 1: tell_object(me,HIY"你该给你所养的蚕喂食了。"NOR"\n"); break;
					case 2: tell_object(me,YEL"你该给你所养的蚕清扫了。"NOR"\n"); break;
					case 3: tell_object(me,HIW"你所养的蚕已经结成蚕茧，你可以开始抽丝了。"NOR"\n"); break;
				}
			}
		}
	}
...
int do_yangcan(string arg)
{
	...
	if( !arg ) {
			...
			if (sizeof(inv) >0) {
				...
				{
					if(inv[i]->query("canzi")){
						str += HIY"喂养 "NOR+inv[i]->query("name")+":yangcan "+file_name(inv[i])+ZJSEP;
					}
				}
				if (str!="")
					tell_object(me, ZJOBLONG + "\n"+ZJOBACTS2+ZJMENUF(3,3,9,30) + str+"\n");
				else
					tell_object(me, ZJOBLONG + "你身上没有蚕卵。\n");
			}
		return 1;
	}
...
void yangcan_finish(object me)
{
	...
	me->start_busy(-1);
	me->interrupt_me();
	message_vision(YEL"$N忙了半天，蚕总算孵化出来了。"NOR"\n", me);
}
...
int improve;
	...
	me->start_busy(-1);
	me->interrupt_me();
	message_vision(HIW"$N累地满头大汗，总算都喂完了。"NOR"\n", me);
...
void qingsao_finish(object me)
{
	...
	me->start_busy(-1);
	me->interrupt_me();
	message_vision(HIY"$N累地满头大汗，总算清扫完了。"NOR"\n", me);
...
void chousi_finish(object me)
{
	...
	message_vision(YEL"$N总共抽得到"+CHINESE_D->chinese_number(bu)+obj->query("unit")+obj->name()+"。"NOR"\n", me);
