...
void create()
{
	...
	set("gender", "男性" );
	set("age", 48);
	set("long", "李铁嘴是个买卜算卦的江湖术士，兼代客写书信、调幅。\n");
...
int do_forge()
{
	...
	write("李铁嘴轻声跟你说道：不是我吹牛，保证蓦的跟原来字体一模一样，每封信\n"+
		"	            五十两白银，不二价。\n");
...
int accept_object(object who, object ob)
{
	...
	if ((string)ob->query("name")=="信纸") {
		if( who->query_temp("marks/李") ) {
			...
			write("李铁嘴皮笑肉不笑的对你说道：伪造这封信可是关系重大啊，得多收五十两。\n");
...
//		say("李铁嘴说：这是写好了的信，给我干什么?\n");
	...
	if( ob->value() >= 5000 ) {
		if( who->query_temp("marks/李4") ) {
			write("李铁嘴对你说道：这封信是造好了，但" + RANK_D->query_respect(who)+ 
				"要是出了事，千万别把我说出来呀。\n");
		...
		if( who->query_temp("marks/李") ) {
			write("李铁嘴对你说道：你想伪造什么啊? 总得拿个样子来吧。\n");
			who->set_temp("marks/李2", 1);
			who->set_temp("marks/李", 0);
			return 1;
		}
		say("李铁嘴一面急忙把钱收起来，一面笑嘻嘻说道：" + RANK_D->query_respect(who) +
			"，这怎么敢当。\n");
		return 1;
	}
        if( ob->value() >= 500 ) {
		say("李铁嘴笑嘻嘻说道：调幅写好了，请您收起来吧，多谢关照。\n");
		return 1;
	}
	if( ob->value() >= 50 ) {
