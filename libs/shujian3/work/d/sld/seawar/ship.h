...
int do_steer(string arg)
{
	...
	set("steer",me);
	message_vision(CYN"$N站到船尾，开始掌舵。"NOR"\n", me);
	return 1;
}
...
int do_gunner(string arg)
{
	...
	set("gunner",me);
	message_vision(CYN"$N站到船头，准备开炮。"NOR"\n", me);
	return 1;
}
...
int do_stand(string arg)
{
	...
	message_vision(CYN"$N从原来的工作处脱身而出。"NOR"\n", me);
	me->start_busy(2);
	return 1;
}
...
protected void turn_finish(object me,int way)
{
	...
	if(way>360) 
		message_vision(sprintf(CYN"$N把船停下。"NOR"\n",way),me);
	else
		message_vision(sprintf(CYN"$N转舵%d度完成。"NOR"\n",way),me);		
}
...
int do_shot(string arg)
{
	...
	message_vision(HIR"$N开始校准方向，装填火药，准备发炮..."NOR"\n",me);
	me->start_busy(2);
...
protected void fire_finish(object me,int angle,int elevation)
{
	...
	message_vision(RED"只听轰的一声，一枚炮弹飞了出去！"NOR"\n",me);
}
