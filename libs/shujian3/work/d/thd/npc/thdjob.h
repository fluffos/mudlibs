...
//  设置 busy
	...
	tell_object(ob,GRN"黄蓉在你耳边悄悄说道: 你要好好守墓。如果有什么人进到墓里，你不要对他客气!"NOR"\n");
// 设置 任务失败时间
...
int accept_object(object who, object ob)
{
		...
		if (who->query("combat_exp") <= 2000000 ){
			...
			if (who->query("potential") > who->query("max_pot")*50)
				who->set("potential", who->query("max_pot")*50);
			...
			tell_room(environment(),HIY "黄蓉在"+ who->query("name")+ "的耳边低声说了几句，"+ who->query("name") +"听的不住点头，面呈喜色。"NOR"\n", ({ who }));
			switch( random(3)) {
				case 0:
	                                tell_object(who,HIG"你觉的脑中豁然开朗，得到了"+CHINESE_D->chinese_number(exp)+"点经验，"
	                                        +CHINESE_D->chinese_number(pot)+"点潜能。"NOR"\n");
	                                break;
	                        case 1:
	                                tell_object(who,HIY"你获得了"+CHINESE_D->chinese_number(exp)+"点经验，"
	                                        +CHINESE_D->chinese_number(pot)+"点潜能的奖励。"NOR"\n");
	                                break;
	                        case 2:
		...
		who->apply_condition("thd_job",1+random(3));
