...
int do_teach(string arg)
{
	...
	if ( random(level) > 20	 && !teacher->query_temp("meetmotou")){
		...
		tell_room(environment(), HIC"一阵紧急的锣声响起，有外贼偷进少林寺图谋不轨！"NOR"\n");
		message_vision(HIY + "$N神情振奋，一声大叫向"+where->query("short")+"奔去！"NOR"\n", me);
...
void waiting(object me,int wait_time)
{
	...
	if (wait_time >= 100 || !query("teacher")) {
		message_vision(HIY + "$N伤痕累累，已经支持不住了！"NOR"\n", me);
		me->die();
		return;
	}
	...
	message_vision(HIC "$N已精疲力尽，见$n到来喜出望外，转身离开战场。"NOR"\n", me, teacher);
	place = me->query("place");
	me->move(place);
	me->set("startroom", place);
	message_vision(BLU"一个面色灰暗，满脸凶光的大汉一把拦住$N，说道：小和尚跑了，老的别想逃！"NOR"\n", teacher);
	teacher->start_busy(random(3));
}
