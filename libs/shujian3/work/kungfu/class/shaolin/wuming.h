...
int ask_fofa()
{
	...
	command("buddhi");
	message_vision(HIW"无名老僧端坐念到：即心即佛，即佛即心，心明识佛，识佛明心，离心非佛，离佛非心……"NOR"\n", me);
...
int ask_reset_skill()
{
	...
	if (me->query("passdu") && me->query_skill("riyue-bian", 1) > 200) {
		...
		message_vision(HIR"$N因为强行放弃了原本修习的少林神功，体内积存的内力耗损了大半。"NOR"\n", me);
...
string ask_wtc()
{
	...
	command("buddhi");
	command("say 如若你想练习少林绝技韦陀杵，必须散去原来修练的武功且内力减少。");
