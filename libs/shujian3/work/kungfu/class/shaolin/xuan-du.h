...
//	ob->add("ffxm_job",1);                                     // ? 是不是漏了的？修改如下。
	...
	if (present("jiuhuan xizhang",ob)){
		...
		message_vision( ob->name()+"一声断喝，云淡风清，四处踪迹不见。"NOR"\n",ob);
		say("玄渡大师合什礼拜道:大师以无边佛法渡化凶徒，使其破去心魔，真乃功德无量。\n");
	}
	else{
		message_vision( HIY +ob->name()+"在风雷声中大声念诵经文，然而风雷越演越烈。"NOR"\n", ob);		   
		message_vision( GRN "风雷渐渐远去,法台一片平静。"NOR"\n", ob);
