...
string ask_qkdny()
{
	...
	if (me->query("family/master_name") != "张无忌")
		return ("这是我明教的圣地，你并非我的传人，不能带你过去！\n");

        if (!me->query_temp("mj_jiaozhu"))
                return ("你先去找回了本教圣物再说！");

        if (me->query_skill("qiankun-danuoyi",1) < 30 )
                return ("你的乾坤大挪移还不够娴熟，不能前往秘室！\n");

        message_vision("$N对$n说道：好吧，我这就带你过去。。\n",this_object(), me);
            me->move("/d/mingjiao/didao/bidao14");
            return ("张无忌把你带到了秘室，又转身离开了。");
}
	...
	write("你听了张无忌的指点，对"+to_chinese(arg)+"的体会又深了一层。\n");
        ...
        if(me->query("relife/quest/lwds")){
         if(random(100) > me->query("relife/quest/lwds")*10)
            write(HIC"你经过转世重修后，领悟到了武技万变不离其宗的道理，瞬间领悟到了乾坤大挪移的精华所在。"NOR"\n");
...
int accept_object(object who, object ob)
{
	...
	if((string)ob->query("id") == "shenghuo ling"){
		...
		&&(fam["master_name"] != "张无忌" || who->query_temp("mj_jiaozhu"))){
			...
			tell_object(who,HIW"你被奖励了一些正神和经验值。"NOR"\n");
			return 1;
		}
		dest = users();
		i = sizeof(dest);
	...
	if(ob->query("name") == "成昆的首级" && ob->query("id") == "shouji"){
		...
		tell_object(who,HIW"你被奖励了一些正神、潜能和经验值。"NOR"\n");
		return 1;
	}
	else return 0;
}
	...
	switch (count) {
		case 0:
			if (present("wen cangsong",room) && present("yan tan",room)) {
				message_vision(HIG"厚土，巨木两旗开始将准备好的木头土石堆成一个巨灶。"NOR"\n",me);
				call_out("start_ling",20,me,1);
			}
			else {
				message_vision(HIG"但是厚土，巨木两旗使不在，无法成功的建立炉灶。"NOR"\n",me);
		...
		case 1:
			if (present("xin ran",room) && present("tang yang",room)) {
				message_vision(HIR"烈火旗升起了雄雄大火，洪水旗在外围围了一圈，准备应付紧急状况。"NOR"\n",me);
				call_out("start_ling",20,me,2);
			}
			else {
				message_vision(HIR"但是烈火，洪水两旗使不在，无法起火。"NOR"\n",me);
				me->delete_temp("mj/ling");
				this_object()->delete("making");
			}
			break;
		case 2:
			...
			if (present("zhuang zheng",room)) {
				message_vision(HIW"庄铮赤裸著上身，把刀剑放入炉中，开始熔铸。"NOR"\n",me);
				destruct(ob1);
				destruct(ob2);
				call_out("start_ling",20,me,3);
			}
			else {
				message_vision(HIW"但是锐金旗使不在，无法铸造。"NOR"\n",me);
				me->delete_temp("mj/ling");
				this_object()->delete("making");
			}
			break;
		case 3:
			message_vision(HIY"在场的明教弟子皆上前，使用圣火神功的传功心法帮忙。"NOR"\n",me);
			...
			if (neili < 30000) {
				message_vision(HIY"但是大家集合的内力还是不够强，众人内力耗尽，昏了过去。"NOR"\n",me);
				for (i=0;i<sizeof(obs);i++) {
					if (obs[i]->query("family/family_name") == "明教")
						obs[i]->unconcious();
					obs[i]->set("neili",0); 
				...
				message_vision(HIM"集合了众人之力，两把神兵利器终于开始慢慢的熔解！"NOR"\n",me);
				message_vision(HIM"两把兵器中似乎有什么纸片，但是立刻被烧成灰了。"NOR"\n",me);
				for (i=0;i<sizeof(obs);i++) {
					if (obs[i]->query("family/family_name") == "明教")
						obs[i]->set("neili",0); 
				}
			...
			break;
		case 4:
			message_vision(HIC"玄铁以及白金砂的熔浆慢慢的流入了事先刻好形状的土坑。"NOR"\n",me);
			neili = this_object()->query("total_neili");
			neili = neili/10000 + random(3);
			message_vision(HIC"$N待炉灶冷却，上前取出了"+(neili+10)+"把玄铁令！"NOR"\n",this_object());
			command("party 感谢各位的帮忙，玄铁令铸造成功了。");
			command("party 现在我与各位旗使必须休息一下，有需要玄铁令的请向我询问。");
			message_vision(HIW"张无忌以及五行旗使离开了此地。"NOR"\n",me);
			me->delete_temp("mj/ling");
