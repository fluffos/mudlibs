...
// AGENTS.md §15p/§15ab: dns_master is proactively excluded from preload
// (an intermud-boot-server dependency unreachable in this environment) --
// guard every direct caller against it never having loaded, degrading to
// the mud's own name instead of crashing on `0->query_mud_name()`.
#define Mud_name() (string)(find_object(DNS_MASTER) ? DNS_MASTER->query_mud_name() : CHINESE_MUD_NAME)
