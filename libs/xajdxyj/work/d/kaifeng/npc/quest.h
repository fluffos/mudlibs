...
// daoxing :     type     name     id            object         amount   where
//-----------------------------------------------------------------------------
//  1000   : ({ "find",  "秘笈",   "mi ji" ,     "",             "1",    "长安" }),
//  5000   : ({ "give",  "灾民",   "zai min" ,   "白银(silver)", "5",    "长安" }),
// 12000   : ({ "kill",  "白骨精", "baigu jing", "",             "3",    "长安" });
...
int quest_random_index(int *quest_keys, int i, object who, string cache_name)
{
  ...
  // lower = upper/4;  // removed by FoF (10/15/1999)
  if( upper - lower < INDEXDELTA )
    lower = 0;

//  removed by FoF  (10/14/1999)
