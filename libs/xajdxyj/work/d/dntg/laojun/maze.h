...
void init() {
  ...
  int r;
  if(this_player()->query_temp("laojunmaze/go")){
  	if (ridee = this_player()->ride()) {
     ridemsg = ridee->query("ride/msg")+"着"+ridee->name();
     ridee->move("/d/dntg/laojun/gate");
  }  
    else  
      ridemsg = "";
   tell_room(environment(this_player()),this_player()->name()+ridemsg+
          "一头栽进云端里不见了。\n",this_player()); 
   this_player()->move("/d/dntg/laojun/gate");
   tell_room(environment(this_player()),this_player()->name()+ridemsg+"走了过来。\n", this_player());
   this_player()->delete_temp("laojunmaze");
...
int valid_leave(object me,string dir) {
  ...
  if (map[dir]=="齐天大圣府") steps=steps-1;
    else steps=steps+1;
  if (steps>10) steps=10;
  me->set_temp("laojunmaze/steps",steps);
  if (steps) return 1;
  
  me->set_temp("laojunmaze/go",1);
