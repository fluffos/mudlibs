...
int do_du(string arg)
{
      ...
      me = this_player();
      ob = this_object(); 
      i = (int)me->query_skill("medical", 1);
      ...
      if(!(arg==ob->query("id")||arg=="medicine book")) return 0;
       ...
       if (!random(5))
        ...
        if((int)me->query("sen") < ob->query("jing_cost"))
       ...
       me->receive_damage("sen", ob->query("jing_cost")); 
       me->improve_skill("medical", me->query_int()/2);
       write("你仔细研读着"CYN"【"HIG+ob->name()+NOR+CYN"】"NOR"。\n");
       return 1;
}
