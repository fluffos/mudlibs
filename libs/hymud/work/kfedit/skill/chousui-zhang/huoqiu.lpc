...
int check_pfm(object me, object target)
{
        ...
        if( i < 180 ){
                tell_object(me, "你发现自己的星宿毒掌还不够娴熟，无法使用「火球」进行攻击。\n");
                return 1;
                }
...
if (!inv[i]->query("systemmake") && !inv[i]->query("ownmake"))
{
                         inv[i]->unequip();
...
target->apply_condition("fire_poison",60);
                ...
                damage = damage*2 + random(damage)+100;
