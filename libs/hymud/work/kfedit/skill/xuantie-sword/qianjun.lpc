...
int perform(object me, object target)
{
        ...
        //        return notify_fail("你的精力不够施展千均一击！\n");
         ...
         dp=target->query("combat_exp")/1000+target->query_skill("dodge");
               damage=me->query_skill("force")+me->query_skill("sword")+me->query_skill("xuantie-sword", 1); 
                   damage *=2; 
            if ( ap/3 + random(ap*5/3) > dp/3 + random(dp*4/3) || random(3)==0) 
        {
msg+=HIB"$N力道使得极猛,挺剑送出。$n早有提防，却万想不到来势竟是如此猛烈，眼见闪避不及，
只听砰的一声猛响，登时连退了五六步，才勉强拿椿站定，等时脸如金纸。\n"NOR;
           target->receive_damage("qi", damage+100);
          target->receive_wound("qi", random(damage/2)+damage/2+100);
           me->start_busy(1+random(3));
           status_flag=1;
        }


        else if ( me->query("int") > random(target->query("int")*2)  || random(3)==0)
        {
msg+=
