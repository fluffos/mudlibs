...
int perform(object me, object target)
{
	...
	if(me->is_busy())
        ...
        if( (int)me->query_skill("huanhe-dao",1) < 50)
	...
	extra = me->query_skill("huanhe-dao",1) / 2;
	me->add_temp("apply/attack", extra);	
	me->add_temp("apply/damage", extra);
	msg = HIY  "$N手中的"+ weapon->name() +"幻出漫天刀云，"+weapon->name() +"划破刀云，千军万马般的击向$n！" NOR;
if( (int)me->query_skill("huanhe-dao",1) > 300)
{	
if (!target->is_busy() && random(3)==0) target->start_busy(1);
