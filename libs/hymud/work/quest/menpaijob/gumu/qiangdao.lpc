...
if (!present(me->query("id")+"yuanshuai", room))
{
	obt = new("/quest/menpaijob/gumu/guan");
	    obt->set_name(HIR"蒙古元帅"NOR, ({ me->query("id")+"yuanshuai"}));
	obt->set_temp("jobname",me->query("id"));
	obt->move(room);
	...
	obt->set("startroom", base_name(room));
      ...
      && obn[i]->query("id")==me->query("id")+"yuanshuai" )
{
