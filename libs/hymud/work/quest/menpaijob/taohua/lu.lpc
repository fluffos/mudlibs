...
//            }
   ...
   if ((int)me->query_condition("guojob2_busy"))
        ...
        if (me->query("potential") < 200 || me->query("combat_exp") < 300) {
                message_vision("$N对着$n摇了摇头说：你的潜能太少。\n", this_object(), me);
                return 1;
        }

        if (me->query("taohuajob") == 1) {
                command("kick " + me->query("id"));
                message_vision("$N对着$n摇了摇头说：我不是给了你任务吗?\n", this_object(), me);
                return 1;
                }
        else {
...
command("say 据报，外面有一大队金兵坐战船杀向这里了。\n");
             if (!ob->query_temp("book561"))
               {
               command("say 你帮我对付了欧阳克了吗?\n");
                        return 1;
                }
              if (!ob->query_temp("book562") )
               {
               command("say 你帮我对付了欧阳锋了吗?\n");
                        return 1;
                }
                
              if (!ob->query_temp("book563") )
               {
               command("say 你帮我对付了完颜康了吗?\n");
                        return 1;
                }

              if (!ob->query_temp("book564") )
               {
               command("say 你帮我对付了完颜洪烈了吗?\n");
                        return 1;
                }                
             if ((int)ob->query("jinyong/book5")!=5 )
               {
               command("say 家师没让你来帮忙吧?\n");
                        return 1;
                } 
 ...
 if ((int)ob->query("jinyong/book5")==5 )                        
		...
		write("陆乘风说道：听说一灯大师遇到了点麻烦 你去找找他吧。\n");
		who->save();
               }
return 1;
}