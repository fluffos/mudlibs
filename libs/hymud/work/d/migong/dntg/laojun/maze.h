...
LONG );
  exits=(["southwest":"/d/migong/dntg/sky/yunlue3"]);
  for (int i=0;i<4;i++)
...
// int limit = me->query("con")*6+(int)me->query("max_neili")/50;
 int limit = me->query("per")*6+(int)me->query("max_neili")/50;
...
int valid_leave(object me,string dir) {
  ...
  string ridemsg;
  string loc="/d/migong/dntg/laojun/gate";  
  object ridee;
  me->add("qi",-60+me->query_kar());
