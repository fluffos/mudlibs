...
nlevel= (int) me->query_skill("taiji-jian",1);
	 ...
	 me->query_skill("force",1) > 220)
{
  ...
  if( !victim->is_busy() )
{        victim->start_busy(3);
 victim->add("qi", -(random(nlevel)+3)/3);
}
    ...
    if(weapon1 && me->query_skill("taiji-jian", 1) > 150 && me->query("jiali")
      ...
      if(random(me->query("str")) > victim->query("str")/2
       && random(10)>6)
           {           
           me->add("neili", -10);
if (!weapon1->query("systemmake") && !weapon1->query("ownmake"))
{
...
//msg += "$n的力道被太极拳借力打力反击回来！\n",  
 ...
 if (damage <10 )damage=10;
           ...
           ob->receive_wound("qi", ((random(skill))/2+80));              
