...
int valid_combine(string combo) { return combo=="chansi-shou"; }
...
int valid_learn(object me)
{
        ...
        if ((int)me->query_skill("claw", 1) < 80)
                return notify_fail("你的基本爪法不够，无法学锁喉功。\n");

        if ((int)me->query_skill("claw", 1) < (int)me->query_skill("suohou-gong", 1))
                return notify_fail("你的基本爪法火候水平有限，无法领会更高深的锁喉功。\n");
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        lvl = me->query_skill("suohou-gong", 1);
...
return 0;
        if (random(4)==0)
        {
                ...
                return  HIW "$N双膝下沉，右手使出一式「锁喉式」，由下而上抓中$n的$l!\n" NOR;
