...
int valid_learn(object me)
{
	...
	if ((int)me->query_skill("shenghuo-shengong", 1) < 120)
		return notify_fail("你的圣火神功火候还不够，小心走火入魔！\n");
...
int ob_hit(object ob, object me, int damage)
{
        ...
        string msg;
        int p,j,skill, neili, neili1, exp, exp1,nskl;
        int level,level2,damage2;
        wp = me->query_temp("weapon");
        wp1 = ob->query_temp("weapon");
        skill = me->query_skill("qiankun-danuoyi", 1);
        nskl = me->query_skill("qiankun-danuoyi", 1);
...
if (random(3)==0 && me->query("neili") < (me->query("max_neili")))
{
...
me->start_busy(1);
      ...
      if(me->query_temp("yi_ob") && living(me)
         ...
         if(target->query_skill("dodge")/2>=damage)
            msg = HIY"结果$n没有受到任何伤害。\n"NOR;
         else{
            target->receive_damage("qi", (damage+(random(nskl)/2))/2, ob);
            target->receive_wound("qi", (damage+(random(nskl)/2))/2, ob);
...
if (!wp1->query("systemmake") && !wp1->query("ownmake"))
{
                  ...
                  if (damage>10)
{
                  ob->receive_damage("qi", (damage+(random(nskl)/2))/2, me);
                  ob->receive_wound("qi", (damage+(random(nskl)/2))/2, me);
}
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
        ...
        nlvl = me->query_skill("qiankun-danuoyi", 1);
...
if( random(me->query_skill("hanbing-mianzhang",1)) > 10 && random(5)==0)
