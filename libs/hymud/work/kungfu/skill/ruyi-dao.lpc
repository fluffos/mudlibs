...
me->add("neili",(random(lvl)+50+nlevel));
...
return HIC "$n" HIC "全身功力如流水般源源不绝地流入$N的身体！\n" NOR;
        ...
        if(random(ap)>dp/4 && random(skill) > 120) {
	        if ( random(4) == 1 ) {
                ...
                victim->receive_damage("qi", (skill+50),me);
                victim->receive_wound("qi", (skill)/2+50,me);
  if( !victim->is_busy() )
{
        	        victim->start_busy(3);
}
