...
int perform(object me, object target)
{
	...
	if ((int)me->query_skill("shentuo-zhang", 1) < 120)
		return notify_fail("你的神驼雪山掌不够娴熟，现在还无法使用「柔字诀」。\n");
...
message_vision(HIR "$N气起丹田，突然发出一声长啸！\n" NOR, me);
if ((int) me->query("combat_exp",1)/3 >=  random((int) target->query("combat_exp",1)))
{
if (damage >=1200) damage=1200;
target->add("jing",-random(damage));
target->add("eff_jing",-random(damage));
target->start_busy(3);

message_vision(HIB"$N觉得头痛欲裂，差点儿晕了过去。\n"NOR,target);
}
	return 1;
}
