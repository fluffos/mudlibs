...
string damage_msg(int damage, string type)
{
	...
	case "割伤":
		...
		if( damage < 50 ) return "结果在$p身上划出一道细长的血痕。\n";
		...
		return "结果只听见$n一声惨嚎，$w已在$p身上划出一道深及见骨的可怕伤口！！\n";
                break;
	case "刺伤":
		if( damage < 10 ) return "结果只是轻轻地刺破$n的皮肉。\n";
		if( damage < 50 ) return "结果在$n身上刺出一个创口。\n";
		if( damage < 100 ) return "结果「噗」地一声刺入了$n身上寸许！\n";
		if( damage < 170 ) return "结果「噗」地一声刺进$n的身上，使$p不由自主地退了几步！\n";
		if( damage < 280 ) return "结果「噗嗤」地一声，$w已在$n身上刺出一个血肉模糊的血窟窿！\n";
		return "结果只听见$n一声惨嚎，$w已在$p的身上对穿而出，鲜血溅得满地！！\n";
                break;
        case "砸伤":
		if( damage < 20 ) return "结果只是轻轻地碰撞到了$n一下，没有造成什么伤害。\n";
		if( damage < 60 ) return "结果$w只将$n的身上砸出一块瘀红。\n";
		if( damage < 120 ) return "结果「啪」地一声，$w打中$n的身上，登时肿了一块老高！\n";
		if( damage < 240 ) return "结果$w重重得击中$n的身上，打得$p倒退数步，「哇」地吐出一大口鲜血！\n";
        ...
        case "瘀伤":
                ...
                if( damage < 20 ) return "结果在$p的身上造成一处瘀青。\n";
                if( damage < 40 ) return "结果一击命中，$n的身上登时肿了一块老高！\n";
        ...
        case "内伤":
                ...
                if( damage < 20 ) return "结果$n痛哼一声，在$p的身上造成一处瘀伤。\n";
        ...
        case "震伤":                
                ...
                break;   
        case "抓伤":     
                if( damage < 10 ) return "结果只是在$n身上处轻轻刮过，没有什么伤害。\n";
                if( damage < 25 ) return "结果$n皱了皱眉，$p的身上已经被拉出了一道浅浅的血痕。\n";
                if( damage < 50 ) return "结果拉下来$n身上的一点皮肉，疼得$p「哇哇」怪叫了起来！\n";
                if( damage < 100 ) return "结果「唰」地一声，$n的身上上顿时被抓出五道血痕，鲜血流了出来！\n";
                if( damage < 200 ) return "结果$n疼得大叫一声，$p身上上被抓出了几道深深的血沟，鲜血直流！\n";
                if( damage < 300 ) return "结果只听见$n一声惨嚎，$N的手抓已在$p的身上处刺出了五个血肉模糊的窟窿！\n";
                if( damage < 400 ) return "结果「啊」地一声惨叫，$n身上$p处被$N抓下了一大片皮肉，鲜血横飞满地！！\n";
                return "结果伴随着$n一声凄惨的嚎叫，直透$p处，连皮带肉扯下一大块，露出了血淋淋的骨头！！\n";
                break;       
        default:
                if( !type ) type = "伤害";
                str =  "结果造成非常可怕的严重";
