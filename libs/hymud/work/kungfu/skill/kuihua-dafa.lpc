...
"$N左手一圈，蓦地刺出，刷刷刷急攻三式，招招尽是指向$n的要害"
});
...
isok=0;
if (level>=100 && (me->query_skill_mapped("force") == "tmdafa" || me->query_skill_mapped("force") == "kuihua-xinfa"))
{
isok=1;	
}
if (weapon)
{
   if( random(2)==0 &&  (random(me->query_str()) > victim->query_str()/3 || isok=1)){
	victim->receive_wound("qi", random(lvl)/2+random(100));
