...
void die(object killer)
{
        ...
        {
                // 如果为自己的猎物，则奖励之
                me->set_temp("isdalinb",1);
                me->add_temp("nbjob29",1);
                if ((random(2)==0 || me->query("combat_exp",1) <= 300000)
                   && me->query("combat_exp",1) <= 1500000)
                {
                        // 根据猎物设定的参数来给予奖励
        exp = 35+random(30);
        
power=ob->query("power",1);
if (power)
{
if (power>1) exp=exp+(power*4);
}	
pot=exp*3/4;
