...
//	npc=data[random(sizeof(data))];
   set_name(order[random(13)]+"剑  心"NOR, ({ "keinxin"}));
set("magicgift",1+random(6));
       set("title",HIB"暗之创造神"NOR);
	set("magicgift",1+random(6));
	set("zhuanbest",1);
	set("magicset",1);
               set("szj/passed",1);
...
"这个世界的创造神之一。\n");
	...
	set_skill("parry",2690+random(300));
      ...
      map_skill("claw", "jiuyin-shenzhua");
        set("no_get",1);
b=8558000+random(2568000);
a=8588000+random(2568000);
d=200+random(60);
set("str",d);
set("dex",130+d);
set("int",d);
...
{
        message_vision(HIY"$N"HIY"身上五彩圣光一闪，又恢复了行动！\n",ob,me);
        ob->interrupt_me();
        ob->start_busy(1);
        ob->start_busy(0);
       ...
       message_vision(HIY "$N" HIY "打出一个巨大的五彩冲击波．．．．．．虚神的力量遍布大地！！！\n" NOR,
        ...
        for (i=sizeof(inv)-1; i>=0; i--)
        {
                if (living(inv[i]) && inv[i] != this_object() && userp(inv[i]))
                {
                       ...
                       inv[i]->start_busy(2);
if (this_object()->is_busy()) this_object()->start_busy(1);
	this_object()->apply_condition("new_poison7", 8);
	this_object()->clear_condition();
