...
int perform(object me, object target)
{
        ...
        if (userp(me) && ! me->query("can_perform/qiankun-danuoyi/yi"))
                return notify_fail("你还没有受过高人指点，无法施展「大挪移」。\n"); 
       ...
       if (! living(target))
        ...
        ap = level * 3 / 2 + me->query_skill("force") * 2 / 3 + me->query("level") * 20 + 
             me->query_skill("martial-cognize", 1);
        dp = target->query_skill("force") * 2 / 3 + target->query("level") * 20 + 
