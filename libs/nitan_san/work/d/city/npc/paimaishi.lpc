...
int do_paimai(string arg)
{
    ...
    if(money<0||money>1000000)
        ...
        command("chat "+"有人提交"+NOR+ob->query("name")+HIC+"进行拍卖，请使用"+HIW"〖"HIG" 拍  卖 "HIW"〗"NOR+HIC"频道"HIY"（ＢＩＬＬ）"HIC"了解具体情况。"NOR);
...
int begin_paimai(string name,int money,int num,int m)
{
        ...
        command("bill "+name+HIG"是本店第『"+chinese_number(num)+"』号拍卖品，有意者请使用ＢＩＤ命令进行竞买。\n"NOR);
...
int report_paimai(object buyer,int money,int num
        ...
        if(query("pnum")) // pnum是判断是否有人投标的标志　
                ...
                pay_back(query("ipaimai/" + sprintf("%d", num)),query("vpaimai/" + sprintf("%d", num))*100);//归还前一个竞买者的钱                add("pnum",1)
        ...
        if(!buyer->query_temp("invisibility_buyer")
           ...
           if (!query("pnum")
                   add("pnum",1)
                command("bill  "+buyer->query("id")+" 出价 "+mon+"购入"+NOR+query("npaimai/" + sprintf("%d", num))+HIG"的所有权。")
