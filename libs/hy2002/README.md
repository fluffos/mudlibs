...
- 是 `hy2000` 的后续/扩充版本，地图和玩法基本一致（结婚系统
  `d/marry/`、`d/happy/` 场景等），额外新增约 3000 个文件的新地图
  和门派内容。
- 管理员引导比 `hy2000` 更棘手：硬编码的后门 id `hxsd` 已经是档案
  里一个真实存在的旧玩家账号（密码未知，无法直接认领），本次修复
  只能在旁边追加一个新账号授权，而不能直接复用这个 id（详见下方管
  理员账号说明，新增 AGENTS.md §1.5 bug 类型）。
- 真实存在的反滥用限制：同一 IP 60 秒内只能注册一次新角色，是有意
  的游戏设计。
- 修复过程中发现的最隐蔽的 bug 出现在选完编码之后：跨服互联精灵
  编译失败会让 `logind.lpc` 的 `gb_big5()` 函数提前中断，连英文名
  字提示都不会出现，且没有任何可见报错（详见下方 bug 修复说明）。
- 深度功能测试完整走通了注册→新手引导→平安城探索→出城→战斗→死亡
  →鬼门关复活的全流程，修复了 §8.9 食物/饮水年龄检查错对象、两处
  `d/death/npc/{b,w}gargoyle.lpc` 的 §7.68 复活软锁；还发现并修复了
  一个此前完全没暴露过的 bug：`securd.lpc` 的自定义 ACL 没有放行
  `file_size` 这个编译期探测操作，导致每局游戏第一次有人引用某项
  技能（如新手武馆的"dodge"）时该 NPC 的 `create()` 会被静默中断
  （AGENTS.md §7.5 新增变体，详见 NOTES.md）。
