...
@ `1aded62f49811cf62a8ab960c39ec04f4b9a9f32` (`main`; scheduled safe
rebase 2026-09-24 from `7d562094`, +1 unused-param cleanup in domain files, 4 catalog patches
...
## Leftover 678 (2026-09-12) — upstream lib/ backport

`michaelprograms/nightmare-residuum` +17 since `d9336db`: mostly
`D_TEST` coverage, one harvestable early-return drop, driver submodule
bumps. Ported `lib/` tests + `harvestable.lpc` / `npc.mock.lpc` to
`abb51fa5c1b`. Ignored `driver/` (this project uses `~/src/fluffos`).
No new rooms.
...
Re-ran a full native + WASM §10.7 pass against the cutover tree:
`work/` = `michaelprograms/nightmare-residuum` @ `dee36ca2` with
`mudlib_subdir=lib`, catalog patches `0001`–`0004` applied live via
`scripts/apply_lib_patches.py` (upstream still ships `.c`; no catalog
rename). Port 40237. Fresh world (cutover wiped the old vendored
`fluffos` seed). Driver PID confirmed via `/proc/<pid>/cwd` →
`libs/residuum/work/lib`; live `debug.log` is
`libs/residuum/log/debug.log` (fd 3).

### Native (`scripts/tmux_mud.sh`, continuous session)

- **Register** `deepres` / `Mud@2026` → first-ever character auto-immortal
  → intro story → tank `download warrior` / `done` → Origin Point.
- **`look` / `score` / `say hello museum`** OK (Warrior L1).
- **Planet surface**: `southwest` → `north` → `pad` →
  `/domain/Planet/virtual/room/surface/Terra/250.250` (tropical rainforest).
  Expected caught `NR_NOISE_SO` FFI fallback in `/log/catch` + immortal
  console (same §10.3 shape; LPC fallback terrain works).
- **Combat / defeat**: `clone /domain/Planet/npc/curious_raccoon.c` →
  `attack raccoon` → HP went visibly negative (`-3`) then
  `"You have been defeated!"` (Defeats 0→1) — same author-TODO off-by-one
  as §10.4, left untouched. Defeat room → `recall` → Origin Point.
- **Advance**: immortal `eval` `add_experience(1000)` → `advance` →
  Level 2, XP 896.
- **Shop / economy (Origin nanite vendor — not `domain/Start`)**:
  `northwest` → R-1001001. `list` / `buy syringe` / `inject syringe` /
  `sell syringe` all live-correct (copper debit/credit, HP tick from
  damaging nanites). `domain/Start` remains unwired (still §10.2).
  Gotcha: `present()` matching is by id/adjective (`buy syringe`), not
  the list short `"syringe of healing nanites"` — adjective-qualified
  strings like `buy healing syringe` returned "don't have any"; first
  matching `syringe` wins. Not fixed (parser/id design, not a crash).
- **Quit + ~90s wall-clock gap + reconnect**: account menu showed
  Level 2 Human / Origin Point; join restored score (L2 / 896 XP /
  Defeats 1 / full vitals).
- **Logs this boot**: `/log/character/enter` has enter+exit (patch
  `0004` `/log/`+`COMMAND` still good); `/log/character/advance` has
  `deepres to 2`; `/log/catch` only the expected noise FFI catch; no
  `/log/runtime`. Driver stdout clean beyond boot domain/author_stats
  warnings.

### WASM

`node scripts/wasm_client.js ~/src/fluffos/build-wasm/src libs/residuum
--timeout 45 --idle 1.0` with seeded `deepres`/`Mud@2026` → join →
`look` / `score` (L2 Warrior, Defeats 1, Origin northwest) / `quit` /
`exit`. Clean. MEMFS — no host save churn.
