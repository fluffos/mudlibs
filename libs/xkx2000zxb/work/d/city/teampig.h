...
void pig_init()
{
	...
	for (i = 0; i < 4; i++) {
		tscore[seat[i]] = hscore[seat[i]] = hscore2[seat[i]] = 0;
...
void init()
{
	...
	add_action("do_claim", "claim");
    	add_action("do_pigscore", "pigscore");
...
"拱猪成绩表(Scoreboard)
－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
｜　队名　｜　姓名（得分）｜　姓名（得分）｜　盘分　｜　总分　｜
－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－\n";
	if ((objectp(pl[seat[3]]))&&(objectp(pl[seat[1]])))
	  r = sprintf("%s%s%-8s%6d%s%-8s%6d%s%8d%s%8d%s\n", r,
		      "｜　东西　｜",
		      (string)pl[seat[3]]->query("name"),(int)hscore2[seat[3]],
		      "｜",
		      (string)pl[seat[1]]->query("name"),(int)hscore2[seat[1]],
 		      "｜", (int)hscore[seat[1]], 
		      "｜", (int)tscore[seat[1]],"｜");
	if ((objectp(pl[seat[2]]))&&(objectp(pl[seat[0]])))
	  r = sprintf("%s%s%-8s%6d%s%-8s%6d%s%8d%s%8d%s\n", r,
		      "｜　南北　｜",
		      (string)pl[seat[2]]->query("name"),(int)hscore2[seat[2]],
		      "｜",
		      (string)pl[seat[0]]->query("name"),(int)hscore2[seat[0]],
 		      "｜", (int)hscore[seat[0]], 
		      "｜", (int)tscore[seat[0]],"｜");	
	return r + "－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－\n\n";
}
...
void after_hand()
{
	...
	int temp1,temp2;
		...
		check_collect = check_collect + hscore[seat[i]];
		if (hscore[seat[i]] == 1000) {
		    collect_all = seat[i];
		    collect_all2 = seat[pd[i]];
		}	
		hscore2[seat[i]] = hscore[seat[i]];
	}
	temp1 = hscore[seat[0]]+hscore[seat[2]];
	temp2 = hscore[seat[1]]+hscore[seat[3]];
	hscore[seat[0]]=temp1;
	hscore[seat[2]]=temp1;
	hscore[seat[1]]=temp2;
	hscore[seat[3]]=temp2;
       	for (i = 0; i < 4; i++) {
		pl[seat[i]]->add("pig_new/played", 1);
		pl[seat[i]]->add("pig_new/score", hscore[seat[i]]);
	}
	if (collect_all == "" || check_collect != 1000)
		for (i = 0; i < 4; i++) {
			tscore[seat[i]] += hscore[seat[i]];
			if ( tscore[seat[i]] <= -1000
			|| tscore[seat[i]] >= 1000 ) {
				pl[seat[i]]->add("pig_new/head", 1);
				for (j = 0; j < 4; j++)
					if (j != i)	
					pl[seat[j]]->add("pig_new/pig_made", 1);
			}
		}
	else
                for (i = 0; i < 4; i++)
			    ...
			    &&(seat[i] != collect_all2)) {
                                ...
                                pl[seat[i]]->add("pig_new/pig_made", 2);
	for (i = 0; i < 4; i++) {
		if (tscore[seat[i]] <= -1000) {
			message_vision("$N是一头猪！\n", pl[seat[i]]);
			score_reset();
		...
		else if (tscore[seat[i]] >= 1000) {
			message_vision("$N是一头肥猪！\n", pl[seat[i]]);
			score_reset();
			break;
		}
		pl[seat[i]]->add("water", 20);
		pl[seat[i]]->add("food", 20);
	}
	deal_init();
}
...
int do_pigscore()
{
    int pigplayed = (int)this_player()->query("pig_new/played");
    int pigmade = (int)this_player()->query("pig_new/pig_made");
    int pighead = (int)this_player()->query("pig_new/head");
    int score = (int)this_player()->query("pig_new/score");

    write("你拱过 " + pigplayed + " 盘猪，拱出 " 
	+ pigmade + " 个猪头，你得了 " + pighead + " 个，总分 " + 
	score + " 。\n");
    return 1;
}
