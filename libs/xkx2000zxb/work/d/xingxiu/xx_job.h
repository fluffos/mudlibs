...
void init()
{
	...
	if (this_player()->query_temp("fresh")==1){
		add_action("do_search1","findbug");
	}
}	
...
int do_search1(){
	...
	if (random(20)<1){
		...
		me->start_busy(random(3)+2);
		damage=me->query_skill("force",1);
		me->receive_wound("qi", damage/10,"被石头绊了一交，摔死了");
	}
...
void setup_npc(object me, object ob){
		...
		ob->set("max_qi", me->query("max_qi"));
			...
			ob->carry_object("/clone/weapon/gangjian.lpc")->wield();
