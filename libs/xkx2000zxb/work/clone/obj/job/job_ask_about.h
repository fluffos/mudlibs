...
void tell_player_family_job_opposepkerplace(object player,object master)
{
	...
	if(!mapp(job_map)
		||undefinedp(job_map["job_oppose_pker_place_chinses"]))
	{
		message_vision(CYN "$n对$N说道：我还没有任务分配给你，为这个做甚？\n"NOR, player, master);
		return;
	}

	message_vision(CYN "$n对$N说道：我已派你去"+job_map["job_oppose_pker_place_chinses"]+"附近完成重要任务，赶快去执行吧。\n"NOR, player, master);
