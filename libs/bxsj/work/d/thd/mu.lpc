...
void create()
{
	set("short", HIC"木"NOR);
	set("long", @LONG
这里是五行层的木，你一走下来，往上的铁梯就被撤掉了。五行层的构局
...
void init()
{
	...
	int *month_map = ({1,2,2,2,2,-1,-1,-1,-1,1,1,1});
	...
	if (me->query_temp("wuxing_error") == 6) {
		...
		if (me->query_temp("sixiang") < 0) tempstr += "乱入四象";
		if (me->query_temp("wuxing") < 0) tempstr += "错踏五行";
		write(tempstr + "，被一股大力甩出了阵，正好摔到山脚下的一块岩石上！\n"); 
...
int valid_leave(object me, string dir)
{
	...
	if (dir == "shui" || dir == "huo" || dir == "tu" || dir == "jin") {
		if (wuxing < 0) {
