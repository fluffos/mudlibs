...
void create()
{
	set("short", "瘦西湖酒馆");
	set("long", @LONG
方圆数百里内提起瘦西湖酒馆可以说是无人不知，无人不晓。当年文学泰
斗顾炎武云游到此，对这儿的扬州炒饭赞不绝口，欣然为其题匾，流下一段传
遍海内的佳话，从此瘦西湖酒馆名声大震。大厅内一溜摆着几排桃木做的红漆
桌子(table)，桌上整齐地摆放着碗筷，四周放着些椅子(chair)。扬州贩私盐
的经常也来这里喝酒闹事，你别去招惹他们为妙。一个酒保正在忙个不停。柜
台上立着一个价目表(sign)。
LONG
	);
	...
	for(i = 0; i < sizeof(ob); i++) {
		if (userp(ob[i])) {
			if (pay = ob[i]->query_temp("total_pay")) {
				...
				if (!living(ob[i])) message_vision("酒保把$N扔出了酒楼。\n", ob[i]);
				else message_vision("酒保把$N推出了酒楼。\n", ob[i]);
				ob[i]->delete_temp("marks/sit");
				...
				ob[i]->move(KICK_TO);
				if (!living(ob[i])) tell_room(KICK_TO, ob[i]->name() + "被扔出了酒楼。\n", ({ob[i]}));
				else tell_room(KICK_TO, ob[i]->name() + "被推出了酒楼。\n", ({ob[i]}));
			}
		}
	}
}
	...
	for(i = 0; i < sizeof(ob); i++) {
		if (paytime = ob[i]->query_temp("to_pay")) {
			...
			if (paytime == 6 && ob[i]->query_temp("total_pay")) {
				...
				ob[i]->move(KICK_TO);
				tell_room(KICK_TO, "只见酒楼大门一开，几个大汉将一个昏迷不醒的家伙丢了出来。\n", ({ob[i]}));
			}
...
int do_sit(string arg)
{
	...
	if ( !arg || (arg != "chair" && arg != "yizi") )
		return notify_fail("你要坐在地板上？有个性！不过这样的话恐怕你会被轰出酒楼了。\n");	
