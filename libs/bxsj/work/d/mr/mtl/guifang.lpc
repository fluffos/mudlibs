...
在闺房的一角摆放着一张梳妆台，台上摆着一些女人用的胭脂粉和用来涂抹嘴
唇的香粉，以及一瓶玉露。房屋的另一角摆放着一张闺床，一缕青纱低垂下来。
墙上有一个窗户，一扇窗子(window)大敞着。
long);
	set("no_fight",1); 
	set("item_desc",([
...
int do_jump(string arg)
{
	...
	if( arg=="window") {
		write("你一纵身，从窗户跳了下去。\n");
		message("vision", me->name() + "从窗户跳了下去。\n", environment(me), ({me}) );
		me->move(__DIR__"ytyuan1");
		message("vision", me->name() + "从墙上的窗户中跳了下来。\n", environment(me), ({me}) );
	}
	return 1;
}
