int do_study(string arg)
{
	...
	for (i = 1; i<=times; i++)
	{
		...
		if (item == 4)
		{
                      if ((string)me->query("gender") != "无性"||(int)me->query_skill("pixie-jian",1)<450 )
			{
                             write("可是壁上是辟邪剑法图谱，欲练神功，必先自宫。而且你的pixie-jian不能低于450级!\n");
				return 1;
			}
		}
		else
		...
		if((int)(me->query_skill(skill_name[item],1) * 
			me->query_skill(skill_name[item],1) * 
                     60 ) >= me->query("combat_exp")&&(string)skill_name[item]!="dodge")
		{
		...
		else if(me->query("combat_exp") < 150000000)
		{
               ...
               me->improve_skill(skill_name[item], random((int)me->query("kar"))+(10 + random((int)me->query_int() - 9))*2);
			me->add("qi",-random(me->query("int")));
			success=1;
		}
		else if(me->query("combat_exp") > 150000000)
       ...
       me->improve_skill(skill_name[item], random((int)me->query("kar"))+(10 + random((int)me->query_int() - 9))*4);
			me->add("qi",-random(me->query("int")));
			success=1;
		}
