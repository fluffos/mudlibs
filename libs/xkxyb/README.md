...
厦门热线一百分支，MudOS V22pre11 制作的武侠 MUD。这份档案和本项目里的 `xkx100` 标题相同（都叫"侠客行一百"），但内容是真正不同的、更小的一份快照（6456 对 14255 个 `.lpc` 文件，目录结构也不一样，有 `ftp`/`mudos.etc`/`www` 这些 `xkx100` 没有的目录），不是重复压缩包；地图与门派配置更接近"标准版"：华山、武当、少林、丐帮、峨嵋、明教等门派与长安周边城池，规模比 `xkx100` 那种跨境大地图更为精简。自带 HTTP 精灵的注释里写明"Modify to ES2 mudlib base by xiaobao@XAJH"，另一处标注版本为"MudOS v22.1b20, XAJH mudlib 2.0b"，说明这份档案的真实血统是更早的"XAJH mudlib"分支，而非直接对应 `xkx100` 的核心代码。
