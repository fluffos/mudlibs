...
nosave int	    num_living_names;
nosave int	    num_searches = 1;
nosave int	    search_length = 1;
nosave int	    illegal_sentence_action;
nosave char *	    last_verb;
nosave object_t *   illegal_sentence_ob;

nosave void notify_no_command PROT((void))
{
...
nosave void set_living_name P2(object_t *, ob, char *, str)
{
 ...
 */
nosave void enable_commands P1(int, num)
{
...
nosave int user_parser P1(char *, buff)
{
	...
	*p = ' ';
    ...
    /*
     * copy user_verb into a nosave character buffer to be pointed to by
     * last_verb.
     */
	...
	/*
	 * If the function is nosave and not defined by current object, then
 ...
 * the shadowed object.
 */
nosave void add_action P3(svalue_t *, str, char *, cmd, int, flag)
{
    sentence_t *p;
    object_t *ob;
    ...
    /* don't allow add_actions of a nosave function from a shadowing object */
 ...
 */
nosave int remove_action P2(char *, act, char *, verb)
{
    object_t *ob;
    sentence_t **s;
 ...
 * If the object is not current object, nosave functions will not
