/* dead_neiting.h  the headfile of dead neiting include 2,4,6,8
   write by xli
   version:1.0
   last modified:xli
   1998.10.
...
#define SI_MEN __DIR__"neiting"+sprintf("%d",2*random(4)+2 ) // 死门  
#define SHENG_MEN __DIR__"neiting"+sprintf("%d",2*random(4)+1 ) // 生门  
void init()
{
    object me;
    me=this_player();
...
int welcome_visitor(object me)
{
    ...
    {   
        ...
        if(me->query("xli_marks/defeat_huisi"))  //  打败过慧思了
        {
            ...
            {
                npc2 ->move(here);
                write("你只见眼前人影一闪，"+GRN"伏虎罗汉"NOR+"欺身至你面前。\n");
                npc2->bihua_ob(me);
                me->bihua_ob(npc2);
                call_out("BackDating",150,me); 
                return 1;
            }
            return 1;
        }
        else
        {
            if(me->query("xli_marks/defeat_huitong"))  //  打败过慧通了
            {
                ...
                {
                    npc1 ->move(here);
                    write("你只见眼前人影一闪，"+RED"降龙罗汉"NOR+"欺身至你面前。\n");
                    npc1->bihua_ob(me);
                    me->bihua_ob(npc1);
                    call_out("BackDating",150,me);
                    return 1;
            ...
            else
            {
                ...
                {
                    write("你只见眼前人影一闪，"
                    +RED"降龙"NOR+GRN"伏虎"NOR+"两位罗汉欺身至你面前。\n");
                    npc1 ->move(here);
                    npc2 ->move(here);
                    npc1 ->bihua_ob(me);
                    npc2 ->bihua_ob(me);
