...
void create() {
    ...
    seteuid(getuid());
    set("bashed", ({}));
    /* Removed by Chronos cuz we never use virtual rooms.
...
void reset() {
	...
	i = sizeof(objects); while(i--){
		...
		if(!present(objects[i],this_object())){
		    ...
		    // Cyanide added, March 2000
		    if (living(ob))
			"/adm/daemons/catalog_d"->catalog_monster( ob );
...
/* Chronos doesn't think so. :)  Removed without prejudice.
...
varargs void set_outside (string zone) {
    /* Chronos removed all the weather code to save on cpu and mem usage.
	    if (!zone) zone="A" ;
	    set ("outside", zone) ;
...
/* Removed cuz it's only called in weather rooms.
...
void set_items( mixed items ) {
    ...
    if (mapp(items)) {
	set("item_desc", (mapping)items);
	return;
    }

    if (pointerp(items)) {
	string *descs = (string *)items;

	// Ensures the array is on the form ({ item, desc, item, desc })
	if (sizeof(descs) % 2) return; 

	for (i=0; i<sizeof(descs); i+=2)
	    add("item_desc", ([ descs[i] : descs[i+1] ]) );
}

return;
}

void set_exits( mixed exits ) {
    int i;

    if (mapp(exits)) {
	set("exits", (mapping)exits);
	return;
    }

    if (pointerp(exits)) {
	string *descs = (string *)exits;

	// Ensures the array is on the form ({ item, desc, item, desc })
	if (sizeof(descs) % 2) return; 

	for (i=0; i<sizeof(descs); i+=2)
	    add("exits", ([ descs[i] : descs[i+1] ]) );
}
