...
//              in order to bash the door down.
// (04-04-98) Chronos ports the code to this mud's standards.
...
varargs void // dir, linked_exit, door_desc, status, locked, bash
create_door(string dir, string linked_exit, string door_desc, string door_stat, string lock, int bash)
{
   set("doors/"+dir+"/linked", linked_exit);
   set("doors/"+dir+"/lock", lock) ;
   if (!door_desc) door_desc = "A wooden door.  There is nothing special about it.";
   set("doors/"+dir+"/desc", door_desc);
   set("doors/"+dir+"/status", door_stat) ;
   set("doors/"+dir+"/bash", bash);
   set_desc(dir) ;
   initialize_link(dir) ;
   return;
}
...
void update_link (string dir) {
  ...
  if (link && link != "none") {
    if (ob = find_object((string)query("exits/"+dir))) {
      ...
      switch (door_stat) {
        case "open" : tell_room (ob, "The "+ob_dir+" door swings open.\n") ;
                      break ;
        case "locked" : tell_room(ob,"You hear a clicking sound from the "+ob_dir+" door.\n") ;
                        break ;
        case "closed" : if (old_stat=="locked") 
          tell_room (ob, "You hear a clicking sound from the "+ob_dir+" door.\n") ;
          else tell_room (ob, "The "+ob_dir+" door swings shut.\n") ;
          break ;
      }
    }
  }
