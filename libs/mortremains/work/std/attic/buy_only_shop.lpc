...
ENDLONG
  ...
  printf("%-30s %d coin", capitalize(ob->query("short")),
  value) ; 
  if (value != 1) write("s.") ;
  else write(".") ;
...
int buy_func(string str) {
  ...
  switch(cost) {
   ...
   case 1: write("You toss a single coin to "+capitalize(TO->query("shopkeeper"))+".\n");  break ;
   default: write("You hand "+cost+" coins to "+capitalize(TO->query("shopkeeper"))+".\n"); break ;
  }
  say(TPN+ " hands some coins to "+capitalize(TO->query("shopkeeper"))+".\n");
  TP->add("wealth", -1 * cost) ; // take the coins.
