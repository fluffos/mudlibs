...
int cmd_force(string str) {
    ...
    if (userp(ob) && !adminp(geteuid(this_player(1)))) {
	notify_fail("Force: You aren't authorized to force anyone.\n");
	return 0; }

    if (member_group(ob->query("name"), "root")) {
	write ("You cannot force Root!\n");
	tell_object(ob,this_player()->query("cap_name")+ " just tried to"+
	  " force you to \""+str+"\".\n");
	return 1;
    }
