...
// 99-02-09 Cyanide added support for ambassadors.
// 99-28-09 Added different curleys for different levels of invis.
//        ( = Player invis
//        { = Wiz invis
//        < = Admin invis
// 00-04-01 Cyanide made it so dewizzed players no longer 
//        show up as domainless wizards.
...
int cmd_people(string arg) {
    ...
    */
	...
	else if (list[i]->query("invisible"))
	    switch ( (int)list[i]->query("invisible") ) {
	case 0 : break;
	case 1 : field = "(" + field + ")"; break;
	case 2 : field = "{" + field + "}"; break;
	case 3..100 : field = "<" + field + ">"; break;
	}
	...
	if (wizardp(list[i])) {
	    if (member_group(list[i]->query("name"), "ambassador")) {
		line += sprintf("%-14s", "Ambassador");
		line += sprintf("%-13s", "-");
	    } else {     
  ...
  if (!adminp(TP) && adminp(list[i]) && list[i]->query("prevent_scry")) 
              field = "[Cannot Sense Location]" ;
...
string shorten(string file) {
  ...
  if (file[0..28]=="/d/Planar/Abyss/Nazi/library/")
    file = "[Nazi Library]-" + file[29..<1] ;
  if (file[0..20]=="/d/Planar/Abyss/Nazi/")
    file = "[Nazi]-"+ file[21..<1];
  if (file[0..9]=="/d/Planar/") 
      file = "[Planar]-" + file[10..<1] ;
    if (file[0..21]=="/d/Prime/Central/room/") 
      file = "[Central]-" + file[22..<1] ;
    if (file[0..18] == "/d/Prime/Macrosoft/") 
	file = "[MacSft]-Floor "+ file[19..<1] ;
    if (file[0..17]=="/d/Prime/Baator/")
	file = "[Baator]-" + file[17..<1] ;
    if (file[0..17] == "/d/Prime/Sewers/") 
	file = "[Sewers]-" + file[17..<1] ;
    if (file[0..8]=="/d/Ocean/")
	file = "[Ocean]-" + file[9..<1];
