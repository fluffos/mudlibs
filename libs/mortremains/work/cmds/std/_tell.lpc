...
// Cyanide took out backlog stuff for now. 28-May 97
// Cyanide fixed idlemsg's carriage return.  9-Sep-97
// Cyanide cleaned up a few things. Made it prettier.  29-Oct-98
...
#define NH " is not currently on "+MUD_NAME+"."
...
int cmd_tell(string str) {
	...
	if (sscanf(who,"%s@%s",target,mud) == 2) {
	    ...
	    "/adm/daemons/network/I3/tell"->send_tell(TP->query("name"),
	      TPN, mud, target, msg);
	    return 1;  }
    } else {
	// Perrin 22Mar96
	...
	{
	    write(MUD_NAME+" does not log tells anymore. Sorry.\n");
	    return 1;
	}
    ...
    if(ob == this_player())  {
	message("tell",
	  iwrap("You tell yourself: "+msg), this_player() );
    ...
    if (wizardp(ob) && in_edit(ob))
	write(ob->query("cap_name")+" is currently in edit mode.\n");

    // Do we want an extra idle msg to be added ?

    idlem = query_idle(ob) / 60 ;

    // This was kinda hackey, so Cyanide cleaned it up.
    // 28 Oct 98
    if(idlem)
    {
	string s = ob->query("cap_name")+" has been idle for "+idlem+
	" minutes and "+idles+ " seconds, ";
	idles = query_idle(ob) - (idlem * 60);
	idlemsg = (string)ob->query("idlemsg");
	if (!idlemsg) s += "but "+subjective(ob->query("gender"))+
	    " left you no message at all.";
	else s += possessive(ob->query("gender"))+
	    " idle message is : "+idlemsg;
	message("tell",iwrap(s), this_player() );
    }
	...
	{
	    message("tell",iwrap(identify(ob)+" "+identify(this_player()) ),
	      this_player() ) ;
	    ...
	    message("tell",
	      iwrap("You tell "+capitalize(who)+": "+msg) , 
	      this_player() );
	...
	message("tell",
	  iwrap("You tell "+ob->query("cap_name")+": "+msg), 
	  this_player() );
...
other living creature, no matter where they are located on the mud.
