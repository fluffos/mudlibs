...
**   03 Mar 00  Cyanide added support for ammunition (about damn time, too)
*/
...
int give_ammo (int, string, object);
...
int cmd_give (string strMain) {
    ...
    // Format: "give 100 12mm rounds to bomber"
    if ((sscanf(strMain, "%d %s rounds to %s", num, strA, strGetter)==3) ||
        (sscanf(strMain, "%d %s round to %s", num, strA, strGetter)==3)) {    
    	obGetter = check_valid_receiver(strGetter);

        DEBUG("num: "+num+", strA: "+strA+", strB: "+strB+"\n"); 

    	if (!obGetter) return 1;
   	    give_ammo(num, strA, obGetter);
...
// This handles the giving of ammunition.
int give_ammo (int ammo, string type, object ob) {
    string str;
    ...
    // Do they have enough?
    if (!this_player()->ammo_debit(type, ammo) ) {
	write("You don't have that much "+type+" ammo.\n");
	return 0;
    }

    // Can ob for whatever reason not take the ammo?
    if (!ob->ammo_credit(type, ammo)) {    
    	write(ob->query("cap_name") + " cannot take that.\n");
    	message("command", TPN + " tried to give you "+ammo+
    	  " "+type+" rounds.\n", ob);
    	// Give back the money that wasn't given...
    	TP->ammo_credit(type, ammo);
    	return 0;
    }

    if (ammo>1) {    
    	write("You give "+ammo+" "+type+" rounds to "+
    	  ob->query("cap_name")+".\n");
    	message("command", TPN+" gives you "+ammo+
    	  " "+type+" rounds.\n", ob);
    } else {    
    	write("You give a "+type+" round to "+
    	  ob->query("cap_name")+".\n");
    	message("command", TPN+" gives you a "+type+
    	  " round.\n", ob);
    }

    say(wrap(TPN+" gives some ammunition to "+ob->query("cap_name")
	+"."), ({ ob }) );
