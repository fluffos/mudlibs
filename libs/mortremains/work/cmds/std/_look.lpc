/* File:  /cmds/std/_look.c
   ...
   Updated:	Help added by Brian (1/28/92)
		...
		Cyanide 5/8/97  Added support for ",".

    This code allows a user to "look" in a room, or at a specific
    object in the user's inventory or environment.
// (04-11-98) Chronos makes it possible to exa door as an object.
// (04-18-98) Chronos adds a height description to looks.
// (06-23-00) Cyanide made support for several items with the same
//            'look_desc', seperated by #'s.
*/
...
*/
#define LONG_WITH_SHORT 1
#define FLAG 1

// prototypes...
string look_in_room (object room, int flag);
varargs string look_at_item (string item, object user, string In);
...
string what_hand (int);
string get_condition (object, object);
...
int cmd_look (string str) {
    ...
    */
	...
	write (LANGUAGE_D->garble_string(look_in_room (env, 
	   (int)viewingOb->query_temp ("force_to_look")),"orcish")+"\n") ;
	*/
	...
	if (ob) {
	    /*
		  names = (string *)ob->query("id");
		  tmp = names[0];
	    */
	    write(sprintf("%-=75s", living_description( ob )));
	    return 1; 
	} else {
	    write ("You are not in combat at the moment.\n");
	    return 1;
	}
    }  //Cyanide
...
int can_see (object who, object what) {
	...
	return 1;
    if (invis == 3 && member_group(who->query("name"), "root"))
	return 1;
    return 0;
}
...
string living_description (object ob) {
    ...
    if (ob->query("attrib1")) {
	raceat = ob->query("attrib1");
	if (ob->query("attrib2")) {
	    raceat += ", "+ob->query("attrib2");
	}
	raceat = article(raceat) + " " + raceat + " " + race;
    } else {
    ...
    //reg+= genat + " approximately "+ob->query("size")+" feet tall.\n";
    reg+= get_condition(ob, viewingOb);

    if (ob->query("last_attacker"))
	reg+=(ob->query("cap_name")+" is being attacked by "+
	  (ob->query("last_attacker"))->query("cap_name")+".\n");
	...
	if (i) {
	    ...
	    while (i--) {
		short = capitalize((string)inv[i]->query ("short"));
		if (short)
		    reg = sprintf ("%s  %s%s%s.\n", reg, short,
		      (inv[i]->query ("equipped") ? " (equipped)" : ""),
		      what_hand((int)inv[i]->query("wielded")));
...
string item_description (string str, object user, int infra, string In) {
    ...
    if ((str=="credits" || str=="credit") &&
      !present("credits", environment(TP))) {
	str = wrap("It is a green paper bill with the face of Cyanide "+
	  "on one side and the esteemed Magicka University on the other.");
	return str;
    }

    if (str == "door" || (str && sscanf(str, "%s door", dir))) {
#ifndef BLOCK_ATTACK
	if (Attacking)
	...
	if ((!doors || !sizeof(doors)) && !present("door", TP) && 
	  !present("door", environment(TP)) ) 
	...
	if (env) {
	    ...
	    // Cyanide did the # support, 6-23-2000

	    int tmpv;
	    string *tmpk, *tmpval, *tmpexploded;

	    if (!undefinedp(env->query("item_desc")) && 
	      mapp(env->query("item_desc"))) {
		tmpk = keys(env->query("item_desc"));
		tmpval = values(env->query("item_desc"));

		for (tmpv = 0; tmpv < sizeof(tmpk); tmpv++) {
		    tmpexploded = explode(tmpk[tmpv], "#");
		    if (member_array(str, tmpexploded) != -1) {
			tmp = tmpval[ tmpv ];
			break;
		    }
	    ...
	    if (strlen(ret) > 2 && ret[0..1]=="@@") {
		ret = call_other(env, tmp[2..<1]);
	    }
...
varargs string look_at_item (string str, object user, string In) {
	...
	if (!undefinedp (In))
	    return item_description (str, user, 0, In);
	else
	...
	if (!undefinedp (In))
	    return item_description (str, user, 1, In);
	else
...
string look_in_item (string tmp, object user) {
    ...
    // THIS CLEARS UP THE look in foo PROBLEM   --  Moby
    if (!it) {
	return "I don't see that here.\n" ;
    }
...
string process_contents (mixed *contents, int flag) {
	...
	if (flag == 1)
	    if(stringp(tmp)) ret = sprintf("%s  %s\n", ret,capitalize(tmp));
	    else ret = sprintf("%s  A person with a number for a short\n",ret);
	else
...
See also: brief, glance
Help
    );
}

string what_hand(int hand) {
    string str;

    switch(hand) {
    case 1 : str = "right hand"; break;
    case 2 : str = "left hand"; break;
    case 3 : str = "both hands"; break;
    }
...
string get_condition (object ob, object viewer) {
    int hp, max_hp;
    string str;

    if (ob->query("hide_condition") && (ob!=viewer)) return "";

    hp = ob->query("hit_points");
    max_hp = ob->query("max_hp");
    if (!max_hp) max_hp = 1;
    hp = (hp*100)/max_hp;

    switch(hp) {
    case -15..0 : str = " is the picture of death."; break;
    case 1..4 : str=" is knocking on death's door."; break;
    case 5..20 : str=" is just a bloody mess."; break;
    case 21..35 : str=" seems mortally wounded."; break;
    case 36..50 : str=" is pretty fucked up."; break;
    case 51..65 : str=" is rather hurt."; break;
    case 66..80 : str=" has a few bumps and bruises."; break;
    case 81..95 : str=" is lightly wounded."; break;
    case 96..100 : str=" is in pristine condition."; break;
    default : str=" is overflowing with vitality."; break;
    }

    str = capitalize(subjective(ob->query("gender"))) + str + "\n";
    return str;
}    
