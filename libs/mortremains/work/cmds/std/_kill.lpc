...
int cmd_kill(string str) {
    ...
    if (!TP->query("vision") &&
      !TP->query_skill("Blind Fighting") && 
      !TP->query("infravision") ) {
    ...
    if (((int)victim->query("prevent_attack")==1) ||
      TP->query("prevent_attack")) {
	notify_fail ("You seem to not be able to attack that.\n");
	return 0;
    }
...
string get_monster() {
    ...
    if(!env || !sizeof(env))  return "monster";
