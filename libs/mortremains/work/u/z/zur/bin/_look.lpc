/* File:  /cmds/std/_look.c
   ...
   Updated:	Help added by Brian (1/28/92)
        ...
        Modified item_description to support arrays as keys in item_desc.
           Zur @ Nightfall's Keep. May 1997.
...
string shorten_dirs(string *str)
{
   string  *tmp, *retval;
   string  *temp;
   int i;
   ...
   {
    if(member_array(str[i], tmp) != -1)
    {
       retval += ({ str[i][0] });
       str   -=  ({ str[i] });
    }
   }
   tmp = ({ "northwest", "northeast", "southeast", "southwest" });
   for ( i = 0; i < sizeof(str) ; i++)
   {
    if(member_array(str[i], tmp) != -1)
      {
        temp = explode(str[i], "");
        retval += ({ temp[0]+temp[5] });
        str   -=  ({ str[i] });
      }
    }
    if(!str) str = ({ });
    return implode(retval+str, " ");
  ...
  if (!suppress || pointerp (suppress)) {
	...
	long = sprintf ("%s\t%sbvious exits %sare %s, and %s.\n", long,
		...
		implode (dirs[0..(i-2)], ", "), dirs[i-1]);
      	...
      	if(viewingOb->query("brief"))
          long = sprintf("%s [%s]", room->query("short"), shorten_dirs(dirs));
  }
