...
varargs int cmd_invis(string str) {
  ...
  if (str && str == "0") str = "none" ; // Chronos cludges away!
   ...
   if(!str || str == "") {
	...
	if(invis == 2)
	  write("You are presently invisible to wizards.\n");
	else if(invis == 1) 
	  write("You are presently invisible to players.\n");
	else write("You are not presently invisible.\n");
   ...
   if(str == "players" || str == "1") {
	...
	if((int)act_ob->query("invisible") < 1) {
	  ...
	  say((string)act_ob->query_minvis() + "\n", ({ act_ob }));
	}
   ...
   if(str == "administrators" || str == "admin" || str == "3") {
        if (!member_group(getuid(this_player()), "admin")) return 1; 
        ...
        act_ob->setenv("prompt", (olprompt ? ("AI " + olprompt) : "AI>"));
        act_ob->do_new();

        write("You are now invisible to administrators.\n");
   ...
   if(str == "root" || str == "4") {
        ...
        act_ob->set("invisible", 4);
        act_ob->setenv("prompt", (olprompt ? ("RI " + olprompt) : "RI>"));
        act_ob->do_new();

        write("You are now invisible to root.\n");
