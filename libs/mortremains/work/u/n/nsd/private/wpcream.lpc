...
void create() {
    set("author", "nsd");
    set("long", @EndText
A bowl of whip cream. Excellent to 'dip' an
object in it or 'spread' it on your <partner>.
You can spread it on yourself too by typing:
'spreadme'.
EndText);
    set("short", "a whip cream bowl");
    set("id", ({ "bowl", "whip cream", "cream", 
      "bowl of whip cream" }) );
    set("mass", 0);
}
...
int spreadme(string str) {
    write(wrap(
	"You spread the whip cream all over yourself.\n"+
...
int spread(string str) {
    ...
    write(wrap(
	"You spread the whip cream all over "+ob->query("cap_name")+
	"'s body.\n"+
	"Be naughty and lick "+objective(ob->query("gender"))+".\n"));
    say(
      TPN+" spreads whip cream all over "+ob->query("cap_name")+
      "'s body.\n"+
      TPN+" gets ready to lick "+objective(ob->query("gender"))+
      ".\n", ({ ob }) );
    tell_object(ob, 
      TPN+" spreads whip cream all over your body.\n"+
