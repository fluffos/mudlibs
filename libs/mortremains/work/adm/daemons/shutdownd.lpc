...
varargs
int start_shutdown( int how, int delay, string reasoning, int rebt ) {
    // Called by the shutdown command to start the process off.
    if ( ( geteuid( previous_object() ) != ROOT_UID ) &&
      file_name(previous_object()) != "/adm/daemons/rebootd" )
	return s_sec;
    if( delay < 0 ) return s_sec;
    if ( this_player() && (string)this_player()->query("name") != "logon"
      && (!rebt) ) {
	...
	message("shutdown", 
	  "The end of this cycle has been reached - "+
...
void count_down( int how )
{
    ...
    if ((delay==1)||(delay==2)||(delay==5)||(delay==10)
      ||(delay==15)||(delay==30)||(delay==45)||(delay==60)) {
