...
//
// 11 Nov 97 - Cyanide re-added experience point sharing for
//    parties.

#define FIND find_player("cyanide")
#define DEBUG(x) tell_object(FIND, x)
...
varargs void share_exp(object leader, int xpv, object *rec) {
   object *Members = ({});
   mapping mem_map = ([]);
   int i, val_sum, num_members = 0;

   if (!leader) 
      { DEBUG ("No leader passed.\n"); return; }

// Members = query_members(leader);
// Members = ({ Members }) + ({ leader });
Members = ({ find_player("arcane"), find_player("tester") });
...
for(i=0;i<sizeof(Members);i++)
 DEBUG(Members[i]->query("cap_name"));

   // If "rec" isn't passed, everybody is assumed to have helped.
   if (!rec || !sizeof(rec)) rec = Members;

   num_members = sizeof(Members);
   if (!num_members)
      { DEBUG("Zero member size\n"); return; }

   
   // This loop filters out non people who didn't aid in the 
   // kill.  Kudos Ilzarion and Nightmask for the idea.
   for (i=num_members - 1; i>=0; i--) {
      int counter = member_array(Members[i], rec);
      if (counter==-1) {
         Members -= ({ Members[i] });
         tell_object(Members[i], "Your party just made a kill, "+
          "but you didn't help.\n"+
          "You earned no experience.\n");
      }
   }
   
   // This loops assigns everybody who helped to a mapping and
   // assigns each an integer value depending on level.
   val_sum = 0;
   for (i=0;i<sizeof(Members);i++) {
      mapping tmp_map;
      int tmp_int;

      tmp_int = Members[i]->query_level();

      switch (tmp_int) {
         case 0..9: tmp_int = pow(2, ( tmp_int - 1 )); break;
         default : tmp_int = 256 + ((tmp_int - 9)*128); break;
      }
      
      tmp_map = ([ Members[i] : tmp_int ]);
      mem_map = mem_map + tmp_map;
      val_sum += tmp_int;
   }

   // THIS loop actually does the xp rewarding.
   for (i=0;i<num_members;i++) {
      int val = values(mem_map)[i];
      object ob = keys(mem_map)[i];
      
DEBUG(ob->query("cap_name")+" earned "+val+"% of a "+
 xpv+" xp kill");

      val = (xpv*val)/val_sum;
DEBUG(" = "+val+"\n");

      tell_object(ob, "Your party just made a kill worth "+xpv+"\n"+
        "Your share is "+val+" experience points.\n");
      ob->add("experience", val);
   }
