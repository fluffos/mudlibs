...
int balance() {
  if (!TP->query("bank_balance")) {
    write("You do not have an account with us here.\n");
    return 1;
  }
...
int help(string str) {
  if (!str) return 0 ;
  if (str != "bank" && str != "here") return 0 ;
  write(@ENDHELP

There is a minimum balance of 2,000 coins required to open an account.
Your account can never go below this amount.  You cannot close your
bank account.
ENDHELP
  );
  return 1;
