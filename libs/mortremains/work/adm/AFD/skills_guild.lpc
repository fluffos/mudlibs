...
// (09-20-96) Chronos mades this compatible with the coin system of the mud.
...
// but if you do, dike this out completely.
  ...
  if (can_train(skill)== -1) {
    write("You must develop that skill further on your own.\n") ;
   return 1;
  }
	if (!can_train(skill)) {
   ...
   wealth = TP->query_coins() ;
	if (wealth<cost) {
  notify_fail("The training would cost you "+cost+" coins, which you don't have.\n") ;
		return 0 ;
	}
   TP->debit(cost) ;
...
int list_costs() {
   ...
   int i, s, Work ;
	...
	s = sizeof(names) ;
	for (i=0;i<s;i++) {
    Work = can_train(s_names[i]) ;
   if (!Work) continue ;
   if (Work == -1) {
  printf("%-25s You must develop this skill further on your own.\n",
     s_names[i]) ;
  continue ;
  }
                 write(sprintf("%-25s %d coins\n", 
