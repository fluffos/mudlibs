...
// (09-12-96) Chronos added set_items to support assignment of arrays to
//            individial item descriptions.  Woot.
...
// Chronos added this function to allow support for setting arrays to
// individual item descriptions.
// ie-
// set_items(([ 
//  ({ "rock", "stone", "boulder", "huge boulder", "huge stone", "huge rock" })
//  : "It's a huge boulder blocking the path west.\n",
//  ])) ;
// Neat, huh?

void set_items(mapping work) {
  mixed *temp ;
  string *temp2;
  int i,x;
  if (!work) return ; // make sure we have something to work with.
  if (!mapp(work)) return ;  // if it isn't a mapping, we're screwed.
  temp=keys(work);
  temp2= values(work);
  for(i=0;i<sizeof(temp);i++) {
    if (!pointerp(temp[i])) {  // it's not an array this time.
      set("item_desc/"+temp[i], temp2[i]);
    }
    else {
      for(x=0;x<sizeof(temp[i]);x++) { // start processing the array
        set("item_desc/"+temp[i][x], temp2[i]);
      }
    }
  }
  // we're finished.
