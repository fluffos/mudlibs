...
/*  Removed cuz it sucks.  Maybe I'll put ansi in someday.
   msg = parse_chat(msg);
*/
...
/* Terminal stuff added by Descartes of Borg 940520 */
    else if(class == "system" || class == "help" || strsrch(msg,"%^") == -1)
      receive(msg);
    else {
        string *words;
        int i;
...
void receive_snoop (string str) {
   ...
   if(shell && previous_object() != shell &&
      base_name(shell) + ".c" == (string) this_object()->query("shell"))
 	shell->receive_message("snoop", "% " + str[0..strlen(str)-2]);
 
   else receive("% " + str);
}

void net_dead() {
   if (userp(this_object())) { write("Body.c netdead() called save_data\n");
				this_object()->save_data() ;} 
  say((string)this_object()->query("cap_name") + 
 " suddenly turns into a solid marble statue!n");
   ANNOUNCE->announce_user(this_object(), 3);
   CMWHO_D->remove_user(this_object());
   set_heart_beat(0);
 
#ifdef NETDEAD_LOG
   log_file(NETDEAD_LOG, this_object()->query("cap_name") + ": net-dead\t\t" + 
