...
void start_renshen()
{
	...
	set_name("镇元大仙", ({"zhenyuan_daxian", "zhenyuan daxian", "zhenyuan", "daxian"}));
	setup();

	announce (me, "五庄观人参果品尝大会即将开始！\n");
	announce (me, "诸位仙贤如愿观礼，使accept zhenyuan_daxian便可！\n");
	...
	while  (i--)
	{
		...
		if ((string)list[i]->query_temp("accept")!=me->query("id"))
			continue;
                ...
                if ((int)list[i]->query("rsg_eaten")>35)
                        continue; // If eat 36 rsg can't accept. beeby 7/23/2000
