// array.c by fire on Nov 1998
// inherited by troop_d.c
// this is used to handle array part of an army
int get_troop_attack_area_rate(int p_id);
int get_general_kills(int p_id);
int get_leadrate(int p_id);
int get_array_kills(int p_id) // not real number
{
    mixed p_soldiers;
    int p_kills;
    int p_m,p_t,p_n;
    int p_r,p_k;
    p_soldiers=TROOP_D->get_soldier(p_id,"bowman");
    if(!p_soldiers) // no bowman
        return 0;  
    if(p_soldiers["energy"]<ARRAY_ATTACK_CONSUME)
        return -1; // two tired to attack
    p_r=WARAI_D->get_array_attack_rate();
...
//    p_k=p_n*p_r*(p_t/160)*(p_t/160)*(p_m/160)/10/5;
    p_kills+=p_k+1;
    return p_kills;
}
int get_array_attack_kills(int p_id) // real number of one array attack
{
   int p_area_rate;
   int p_kills,p_leadrate;
   int p_general_kills;
   p_area_rate=get_troop_attack_area_rate( p_id);
   p_kills=get_array_kills(p_id);
