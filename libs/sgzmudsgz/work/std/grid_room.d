...
string* get_long(string p_type, string pos, string soldier, string s_num, string s_color)
{
  ...
  switch(p_type)
  {
    ...
    if ( soldier == "Ａ" || soldier == "Ｄ" || soldier == "Ｍ" ) {
	has_soldier = 1;
	tmp[0] = repeat_string(soldier, 4) ;
        tmp[1] = soldier + s_num + s_num + soldier ;
        tmp[2] = soldier + s_num + s_num + soldier ;
	tmp[3] = repeat_string(soldier, 4) ;
    }
