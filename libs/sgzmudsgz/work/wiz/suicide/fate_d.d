...
mapping create_tz_table(int year)  //得到每年的天灾表
{
 ...
 int datalen,i,seek,rad,tgdz;
 create_year= year;
 tgdz = (year-184)%60;
 data = omen[tgdz];
 if (!(datalen=sizeof(data))) return ([]);
   ...
   if (seek!=rad)
      { 
        ...
        type = tz_type[data[0]][i%4]; //由i决定四季
      	table[season[i%4]] += ({({areas[seek],type[random(sizeof(type))]})});
      }
 }
