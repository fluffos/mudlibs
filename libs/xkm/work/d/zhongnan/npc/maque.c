...
//              return notify_fail("这儿不是练抓麻雀的地方。\n");
	...
	if( me->query("family/family_name") != "古墓派")
        ...
        if ( (int)me->query("qi") < 60 && (int)me->query("jing") < 40 )
         ...
         me->start_busy(3);
	message("\n$N突然向$n扑去。\n", me, ob);
       me->set_temp("oldzhua",2+random(2));
         remove_call_out("checking");
         call_out("checking", 1, me);

        me->receive_damage("jingli", 10 + random(10), "精力透支过度死了");
       me->receive_damage("qi", 5 + random(5), "体力透支过度死了");
...
{
       if (me->query_temp("oldzhua") <= 0) {
               me->delete_temp("oldzhua");
               return ;
          }
        me->add_temp("oldzhua", -1);
        call_out("checking", 1, me);
