...
int do_tong(string arg)
{
        ...
        if( !me->query_temp("gumu/caimi" ) )
                return notify_fail("还是先问问龙姑姑让你做什么吧！\n");
...
*/
           if ( me->query_temp("gm_fengmi") < 10 + random(5)){
            i= me->query("combat_exp")/100000*50;
           if ( random(10) > i || me->query("combat_exp") < 20000 ){
             write("你用力捅着蜂巢，从中挖出了一点蜂蜜。\n");
             me->start_busy(1);            
             me->receive_damage("jingli",5+random(5));
             me->receive_damage("qi",3+random(2));
