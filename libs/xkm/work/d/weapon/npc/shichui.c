...
//		case "寒丝羽竹": 
	...
	message("vision","侍锤低头检视了一下手中的"+(string)ob->query("name")+
			"点了点头说：\n", who);
...
int do_zhu(string arg)
{
	...
	if(!w_name||!id)
		return notify_fail("什么?(使用zhu 锤的名字 英文代号\n)");
	...
	message_vision(HIR "只听咯的一声轻响，一柄大锤从炉中倏然跃起。化作一道青电猛的向$N的前胸刺来！！\n" NOR,me );
	say("侍锤见状大叫：神锤初成，人血以祭！！闪开！\n");
	message_vision("$N只觉得眼前一花，一条白影迅捷无比的挡在了$N的身前。\n",me );	
	message_vision("大锤透胸穿出，侍锤惨号一声，鲜血溅得你满脸都是！\n",me );
	message_vision(RED "侍锤脚下一个不稳，倒在了地上。侍锤已经奄奄一息了。\n" NOR,me);
	this_object()->set("max_qi",10);
	message_vision("大锤又飞了起来，飞到半空，当的一声落回到地上。\n炉中的火灭了。一室的劲气化于无形，一切又归于沉寂。\n",me );
	...
	hammer=new("/d/weapon/weapon/m_hammer",1);	//生成新锤
	hammer->move(this_player());
	message_vision("侍锤摸起地上，带着斑斑血迹还有些烫手的大锤，说：\n锤。。已。。成。。，侍锤的任务。。。也就完成了。。。。\n",me );
	say("侍锤艰难的说：锤。。您。。收好，我该走了。。。\n");
	message_vision("侍锤说完。倏的便不见了。\n",me);
	this_object()->die();
	return 1;
}
