...
void create()
{
        set_name("信用卡", ({ "credit card","credit","card","ka" }) );
        set("unit", "张");
        set("long",@LONG
这是梦巫师为了大家出门方便所设计的新式信用卡，上面记录着你在银行中的
存款。望妥善保管，遗失后果自负。
...
int do_save(string arg)
{
        int amount,check1;
        string money;
        object money_ob,me,coin,silver,gold,cash,tencash,yuanbao;

        if (!arg)
         return notify_fail("你要存什么呀。\n");
        me=this_player();
        ...
        return notify_fail("你那来的这么多的钱，不会是偷的吧。\n");		   		
	   ...
	   silver=present("silver_money",me);
           ...
           yuanbao=present("yuanbao_money",me);
        ...
        else
        {
            ...
            if (arg=="all")
            return notify_fail("你存了所有的钱。\n");
            else		
    	   return notify_fail("你存了"+amount+""+money_ob->query("name")+"。\n");
}
...
\n");
	...
	return notify_fail("你取了"+amount+""+money_ob->query("name")+"。\n");
}
...
{ 
   return HIB+"你看到一张蓝色卡片掉了下来，钻入土中不见了．．．\n"+NOR;
}
