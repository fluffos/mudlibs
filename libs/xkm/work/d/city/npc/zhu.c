...
//add by wzfeng  
	...
	&& who->query_skill("literate", 1) < 90
        && ob->query("money_id") && ob->value() >= 1000) {
                message_vision("朱熹同意指点$N一些读书写字的问题。\n", who);
                who->add_temp("mark/朱", ob->value() / 150);
                if (who->query("连城诀/朱熹")) 
                who->add_temp("mark/朱", ob->value() / 300);
                return 1;
        }
	...
	&& who->query_skill("literate", 1) < 120
        && ob->query("money_id") && ob->value() >= 2000) {
                ...
                who->add_temp("mark/朱", ob->value() / 300);
                if (who->query("连城诀/朱熹"))
                who->add_temp("mark/朱", ob->value() / 600);
                return 1;
        }
	if (who->query_skill("literate", 1) >= 120
        && ob->query("money_id") && ob->value() >= 2000) {
                ...
                who->add_temp("mark/朱", ob->value() / 600);
                if (who->query("连城诀/朱熹"))
                who->add_temp("mark/朱", ob->value() / 1200);
                return 1;
        }
	return 0;
}
...
string ask_book()
{
	...
	if (present("shu", this_player()))
		return "你身上不是有书吗？怎么又向我要？";
		
	if (present("shu", environment(me)))
		return "地上不是有本书吗？你拿去读吧。";	
	
	
        if (me->query_skill("literate",1) >= 30 && me->query_skill("literate",1) < 101  )
	{ me->set_temp("na/zhu",1);
         me->add_temp("mark1/朱",1);               //program add
	 return("老朽看你这"+RANK_D->query_rude(this_player())+"挺聪明的,到楼上的书架里拿一本吧。\n");
	}
	else
	
        if (me->query_skill("literate",1) >= 101 )
	return("你的文化都这么高了，读书死书没有用的，还是多向老朽学点吧。\n");
	else
	add_action("do_ketou", "ketou");
	return("老朽看你这"+RANK_D->query_rude(this_player())+"挺聪明的，怎么一点礼貌都不懂？磕头总会吧？\n");
...
int do_ketou()
{
	object bk,me;
	me = this_player();
	remove_action("do_ketou", "ketou");
	
	if ( me->query("jingli") < 30 )
	{
		message_vision("$N觉得双腿无力，累得跌倒在地。\n",me);
		return 1;
	}
	
	if ( present("shu",me)) {
		command("kick "+me->query("id"));
		tell_object(me,me->query("name")+"，"+RANK_D->query_rude(this_player())+"別殷勤了，你不是有书了吗？\n");
		tell_object(me,HIR"当心你下次要不到书。\n"NOR);
		me->add("ketou_zhu",-1);
		return 1;
		}
	else
	{
		message_vision("$N对著朱熹磕了个响头。\n",me);
		me->add("jingli",-20);
        me->add_temp("mark1/朱",1);           //program add
		command("pat "+me->query("id"));
		{
			bk=new("/d/city/npc/obj/sanzijing.c");
			bk->move(this_player());
			message_vision(me->query("name")+"真是好孩子,這本三字经就送給$N吧。\n",me);
		}
		return 1;
	}
	return 1;
