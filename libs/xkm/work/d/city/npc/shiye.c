...
//#define PRICE 888000      // price for name change (unit: coin).
#define MINVALUE 2220000   //minimal payment each time (unit: coin).
...
string do_ask(object me)
{
	...
	if ( me->query("name_changed")>5)
		//command("kick" + this_player()); 
		return "你真是个不知好歹的人，名子怎可改的这样频繁。\n";
		
	if ( me->query("name_changed")){
		PRICE=PRICE*me->query("name_changed")*2;
		add_action("do_da", "da");
		me->set_temp("processing", 1);
		return "你已经换过"+me->query("name_changed")+"次名字了，再换就收你"+chinese_number(PRICE/10000)+"两金子吧。你愿意吗？(da 愿意)\n";	
 }
 else
{	add_action("do_da", "da");
	me->set_temp("processing", 1);
