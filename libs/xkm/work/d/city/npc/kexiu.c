...
int ask_jiehun()
{
      ...
      if ( me->query_temp("guanfu/jh",1) > 0 ) {
                message_vision("$N大喝：大胆刁民！本官已同意你结婚，还不快去！再敢如此，定不轻饶！\n", this_object(), me);
         me->start_busy(4);
    call_out("zou",1,me);
           return 1;
          }

      if ( me->query("spouse/married",1) > 0 ) {
                 message_vision("$N端详了一阵$n说道：你太多情了吧？有了对象还要？！\n", this_object(), me);
                 return 1;
        }

        if( me->query("class") == "lama" && me->query_skill("lamaism", 1) >= 120 ) {
            me->set_temp("guanfu/jh", 1);
                message_vision("$N对$n说道：根据藏典，活佛可以娶妻生子。本官准许！\n", this_object(), me);
                return 1;
       }


        if( me->query("balance",1) < 1500000 ) {
                message_vision("$N对$n说道：就你那俩钱，能养的活老婆孩子吗？！\n", this_object(), me);
                return 1;
       }

       if ( ( me->query("age") >= 18 && me->query("gender") == "男性" ) || ( me->query("age") >= 16 && me->query("gender") == "女性" ) ) {
             me->set_temp("guanfu/jh", 1);
                 message_vision("$N端详了一阵$n说道：好，本府允许你结婚，去找民妇，让她带你去见媒婆吧！\n$n刚要走，$N一把抓住你道：结婚了别忘了给本官喜糖哦！\n", this_object(), me);

                  return 1;
         }
       else {
                 message_vision("$N对着你皱了皱眉，道：“你不符合结婚条件！左右！给我轰出去！”！\n", this_object(), me);
       me->move("/d/city/yamen");
       return 1;
      }
}
