...
string tell_job(object player) {
    ...
    if (player->query("combat_exp") > 10000000) {
        exp_rate = exp_rate * 9000000 / player->query("combat_exp");
    }
    ...
    if (random(exp_rate) > 180) {
	player->add("max_neili", 1);
	player->add("eff_jingli", 1);
    }
       player->set_temp("ftbjob_done", 1);
...
#include "/u/bright/npc/checkheal.c"
