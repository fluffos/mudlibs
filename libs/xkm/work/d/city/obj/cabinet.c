...
{ 
    ...
    check=year+month+day;            
     if(me->query("weekend_gift")==check)
       return notify_fail("你好象拿过了耶！\n");
        ...
        if ((int)me->query("combat_exp") >200001 ) 
        {
         return notify_fail("你以是高手了还要这新人的玩意 ?\n");
	}
    ...
    if ((int)me->query("combat_exp") <=100000 ) 
     ...
     return notify_fail("你从礼品柜里拿出一大袋"+HIW"qq糖"NOR+"。\n");
	  } 
     ...
     if ((int)me->query("combat_exp") <=200000 ) 
         {
     		...
     		ob->move(me);
     		me->set("weekend_gift",check);
     		return notify_fail("你从礼品柜里拿出一小袋"+HIW"qq糖"NOR+"。\n");
  	   }
