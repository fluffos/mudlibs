...
void create()
{
            set("short", HIW"白鹿藏书阁"NOR);
              set("long",
"这是一个非常宽畅的楼阁，一排又一排的"HIY"书架(shujia)"NOR"上整整齐齐地摆满了\n"
"各种文献书，令人目不暇给。一张不大的"HIM"书桌"NOR"(shuzhuo)放在大堂中央，樟木香\n"
"味夹着墨香扑鼻而来，这里就是白鹿书院的藏书阁，收藏着许多武学经典，武林\n"
"中人，千万不可错过在此一睹为快的良机。\n"
	);
        set("item_desc", ([
                "shujia" : (: look_shujia :),
                "shuzhuo" : "上面放着几本百家姓和千字文,想要先向朱熹师付说一声。\n", 
        ]));

	set("exits", ([
		"down" : __DIR__"shuyuan",
		"enter" : __DIR__"dc_caipiao",
	]));
...
{ 
    ...
    me = this_player();
     ...
     if ( !arg )
          return notify_fail("命令格式： na book from shuzhuo \n");     
    ...
    if (!me->query_temp("na/zhu",1)) 
     ...
     if (me->query_skill("literate",1) >= 60 )
       {  ob = new("/d/city/obj/qzw");
         ob->move(me);
         me->delete_temp("na");
         return notify_fail("你从书桌上拿出一本千字文书。\n");
       } else
	{ ob = new("/d/city/obj/bjx");
     	ob->move(me);
     	me->delete_temp("na");
     	return notify_fail("你从书桌上拿出一本百家姓书。\n");
         }
}
