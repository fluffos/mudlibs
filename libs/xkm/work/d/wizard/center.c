...
void create()
{
	...
	set("long", HIC
"这里是武林中各个门派任务系统的控制中心，在这里可以控制整个侠客行\n"
"的强制性任务系统。此地乃虚空之地，不存在于江湖实境中。四周一片虚无，\n"
"言语无法描述其万一。乃浪翻云穷其一生精力，开辟而成。\n"+NOR
...
string show_word()
{
	return HIW"武林幻境控制大法：\n"NOR
		...
		HIY"\tclose_system\t\t"NOR+HIG"关闭任务系统\n"NOR
...
int can_used()
{
		...
		me=this_player();
                ...
                if(this_player()->query("id")!="xkxmud"&&
                        this_player()->query("id")!="jhy"&&
                        this_player()->query("id")!="edit"
			)
