...
int PRODUCE_TEMP=0;
//////////////////////////////
string copyright=HBBLU+HIY+"侠客行主动性任务察看器1.0版     Wzfeng 2000年7月     \n"NOR;
string show_word();
...
void create()
{
	...
	set("long", HIC
"这里是武林中各个门派任务系统的控制中心，在这里可以控制整个侠客\n"
"行的强制性任务系统。此地乃虚空之地，不存在于江湖实境中。四周一片虚\n"
"无，言语无法描述其万一。乃浪翻云穷其一生精力，开辟而成。\n"+NOR
...
int can_used()
{
		...
		if(this_player()->query("id")!="wzfeng"&&
			this_player()->query("id")!="qfy"&&
			this_player()->query("id")!="ryu"&&
			this_player()->query("id")!="sdong"&&
			this_player()->query("id")!="xuy"&&
			this_player()->query("id")!="cleansowrd"&&
			this_player()->query("id")!="xqin"&&
			this_player()->query("id")!="fear"&&
			this_player()->query("id")!="sage"
			)
...
int do_check_menpai_job(string arg)
{
	...
	object job_data;
	int wudang,xingxiu,huashan,taohua,gaibang,emei,baituo,quanzhen,xueshan,dali,shaolin,mingjiao,i;
	if(!arg)
	...
	if( arg ) {
		...
		if(mingjiao)
		{
			msg+=job_data->query_family_jobdata("明教");
			member=job_data->choose_of_player("明教","good");
			if(!sizeof(member))
				msg+=HIR"这个门派没有完成任务的完家。\n"NOR;
			else
				if(sizeof(member)!=1)
			...
			member=job_data->choose_of_player("明教","bad");
			if(!sizeof(member))
				msg+=HIR"这个门派没有完成任务的完家。\n"NOR;
			else
				if(sizeof(member)!=1)
...
int do_setorg_default(string arg)
{
	...
	obj_temp->add_orgdata("明教",strategy_mj,luck_mj,money_mj,luck_mjrate,power_mj,assess_mj);
...
int do_check(string arg)
{
	...
	int i,menpai,help,luck,money,strategy,wudang,xingxiu,huashan,taohua,gaibang,emei,baituo,
		quanzhen,xueshan,dali,shaolin,mingjiao,rate;
	me=this_player();
	...
	if( arg ) {
		...
		while( i-- )
			...
			case "-dali": dali = 1; break;
			case "-shaolin": shaolin = 1; break;
			case "-mingjiao": mingjiao = 1; break;
	...
	if(!obj_temp->restore())
		return notify_fail("初始化job_menpai错误，请重新设置。\n");
	if(!obj_temp->have_menpai("mingjiao"))
		return notify_fail("没有"+"mingjiao"+"这个门派。\n");
	obj_temp->query_full_menpaidata("mingjiao");
	}
