...
void init()
{
        ...
        if ( !me->query_temp("said") ) {
             switch( random(3) )
             {
                case 0:
                        command( "rob" );
                        command( "say 栽在老子手下的刺客不止你一个人，拿命来吧！" );
                        break;
                case 1:
                        command( "grin" );
                        command( "say 留下你项上人头，滚吧！" );
                        break;
                case 2:
                        command( "hehe" );
                        command( "say 让老子看看你有什么能耐，居然行刺军中大帅！" );
                        break;
             }
             me->set_temp("said", 1);
        }
...
*/
    ...
    switch (menpai){
    case "huashan":
        set_skill("huashan-jianfa", max-random(max)*4/5);
        set_skill("huashan-shenfa", max-random(max)*4/5);
        set_skill("liangyi-dao", max-random(max)*4/5);
        set_skill("zixia-gong", max-random(max)*4/5);
        set_skill("pishi-poyu", max-random(max)*4/5);
        set_skill("hunyuan-zhang", max-random(max)*4/5);
