...
void create()
{
	...
	set("inquiry", ([
		"武器"  : "师傅命我帮助别人铸造武器，但到现在也不见一个人影。谁要铸造武器呢？\n",
		"原料"  : "铸造武器可用寒丝羽竹，海底金母，千年神木。但效果各不一样。\n",
	]) );
...
{	string wqname;
	...
	if(!(who->query_temp("myweapon") ) ){
		message_vision(HIC"师傅没有同意为你铸武器，您请回吧！\n"NOR, who);
		return 0;
	}
	if((who->query_temp("myarmor") ) ){
		message_vision(HIC"我不会造甲，您还是找待甲吧！\n"NOR, who);
		return 0;
	}
	
	
	if((who->query("myweapon/make")) ){
		say("侍剑一脸茫然：您已经有一把自造的武器了，还来干什么？难到不可手么？\n");
		return 1;
	}
	
	wqname=who->query_temp("myweapon/wqname");
	...
	message("vision","侍剑低头检视了一下手中的"+(string)ob->query("name")+
			"点了点头说：\n", who);
	message("vision","。。。好我们现在就开始铸"+wqname+"\n", who);
	write("请这位"+RANK_D->query_respect(who)+"给你的武器起名字吧！(zhu "+wqname+"的名字 英文代号)\n");
	return 2;
}

int do_zhu(string arg)
{
	string o_name,id,w_name,mywq;
	object sword,make_time;   //生成的武器句柄
	object me;
	me = this_player();
	
	mywq=me->query_temp("myweapon/wqname");
	...
	if( !arg )
		return notify_fail("侍剑认真的说：想好"+mywq+"的名称及代号后在对我说。\n");

	sscanf(arg ,"%s %s" ,w_name ,id);
	if(!w_name||!id)
		return notify_fail("什么?(使用zhu "+mywq+"的名字 英文代号\n)");
	...
	message_vision(me->name()+"沉吟了一会，对侍剑悄声说了几句话。侍剑点了点头。说：好吧！\n",me );
	...
	me->set("myweapon/make",1);		//武器制造后防止再造的标志		
	me->set("myweapon/name",w_name +NOR);	//武器的名称
	me->set("myweapon/id",id);		//武器的代号
	me->set("myweapon/lv",1);    	//武器等级
	me->set("myweapon/or",o_name);	//记录制造原料
	me->set("myweapon/value",0);	      	//记录武器升级已有的点数	
	me->set("myweapon/type",mywq);
	me->set("myweapon/time",make_time);
	//生成新剑
	if(me->query_temp("myweapon/stick") ) sword=new("/d/m_weapon/weapon/m_stick",1);
	if(me->query_temp("myweapon/sword") ) sword=new("/d/m_weapon/weapon/m_sword",1);
	if(me->query_temp("myweapon/blade") ) sword=new("/d/m_weapon/weapon/m_blade",1);
	if(me->query_temp("myweapon/club") ) sword=new("/d/m_weapon/weapon/m_club",1);
	if(me->query_temp("myweapon/staff") ) sword=new("/d/m_weapon/weapon/m_staff",1);
	if(me->query_temp("myweapon/hammer") ) sword=new("/d/m_weapon/weapon/m_hammer",1);
	if(me->query_temp("myweapon/whip") ) sword=new("/d/m_weapon/weapon/m_whip",1);
	if(me->query_temp("myweapon/xiao") ) sword=new("/d/m_weapon/weapon/m_xiao",1);
	sword->move(this_player());
	message_vision("侍剑摸起地上，带着斑斑血迹还有些烫手的巨"+mywq+"，说：\n"+mywq+"。。已。。成。。，侍剑的任务。。。也就完成了。。。。\n",me );
	say("侍剑艰难的说："+mywq+"。。您。。收好，我该走了。。。\n");
	message_vision("侍剑说完。倏的便不见了。\n",me);
	me->delete_temp("m_sword");
	me->delete_temp("get_orc");
	me->delete_temp("or");
	me->delete_temp("myweapon");
	//this_object()->die();
	return 1;
}
