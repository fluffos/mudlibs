...
int do_put(string arg)
{
        ...
        if( item=="book"){
                write("你不能把这样东西放进神木王鼎,小心(bright)删除你的档案:)。\n");
                return 1;
        }
        if(query("opened") < 1)
...
int do_hua(string arg)
{
        ...
        if ( me->query_skill("huagong-dafa",1) <150 )
                return notify_fail("你化功大法功力不够，不能通过化毒来提升修为！\n");
        
        if (me->query("neili") < 30 || me->query("jingli") < 30
        || me->query("qi") < 30 || me->query("jing") < 30)
                return notify_fail("你的真气不够练功所需。\n");

        me->add("jing", (-1)*random(45)-20);
        me->add("qi", (-1)*random(45)-20);
        me->add("jingli", (-1)*random(45)-20);
        me->add("neili", (-1)*random(100));

        message_vision(RED "$N伸手将血液接住，盘膝运功将手中毒血化入筋脉,运内力一个周天。\n"NOR, me);
        add("sucked", -random(2));
        me->improve_skill("huagong-dafa", (me->query_skill("poison",1)/10)+(me->query_skill("force",1)/5));
//      me->set_temp("muding_learned",1);
        ...
        if (random(50)==1){
        	message_vision(HIR"$N正运气中，突然觉得胸口一疼，似乎体内各种内力反击，身受内伤。\n"NOR, me);
        	me->receive_wound("qi", random(100), "练功走火入魔死了");
        	if(random(2)==1)
        		me->apply_condition("huadu_poison",random(100));
        }
        ...
        if(query("sucked") < 1){
	        message_vision(RED "鼎中毒血渐渐滴干了。\n"NOR, me);
        	//tell_object(me,HIR"你体内毒性靠化功大法压制，一月之内必须再来化毒，否则毒力反攻，性命悠关！\n"NOR);
        	tell_object(me,HIG"你慢慢站起，觉得精神抖擞，内力一丝丝地又聚回了丹田。\n"NOR);
        	set("sucked", 500);
        	set("suckable", 0);
        	set("canblood", 0);

        	me->add("combat_exp", random(me->query("force"))*2);
        	me->set("maxsuck", 600);        
