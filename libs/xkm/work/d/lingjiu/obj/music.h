...
int do_lianxi(string arg)
{
	...
	if( (int)me->query_skill("music", 1) < 10 ) {
		...
		if(ob->query("shape") =="flute")
              message_vision("$N无法静心，结果$n发出一阵噪音。\n"NOR, me, ob);
		return 1;
	}
...
/*
       if( me->query("family/family_name") == "桃花岛" || if( me->query("family/family_name") == "灵鹫宫" && (int)me->query_skill("music", 1) < 333 )
*/
		me->improve_skill("music", me->query_int()*2 + 1);
		}
		else if(ob->query("shape") =="flute") {
		msg = flute_msg[random(sizeof(flute_msg))] ;
message_vision("$N手拂$n，轻轻拂弹，专心练习著「"+msg+"」的技巧。\n"NOR, me, ob);
/*
     if( me->query("family/family_name") == "桃花岛" || if( me->query("family/family_name") == "灵鹫宫" && (int)me->query_skill("music", 1) < 333)
*/
		me->improve_skill("music", me->query_int()*2 + 1);
