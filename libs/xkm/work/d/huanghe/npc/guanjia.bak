...
string ask_job()
{
	...
	if(present("silk", this_player())|| present("shoushi",this_player()) || present("ciqi", this_player()) || present("renshen",this_player()) || present("zihua",this_player()))
              return RANK_D->query_rude(me) + "不是刚问过我吗？还不快去。";	      

       if( time() < (int)me->query("bangs/asktime") + 25 ||query("job"))
...
int accept_object(object who, object ob)
{
        ...
        case "寻":
                ...
                if( chname != job["name"] )
                        return notify_fail(name() + "大怒道：没有用的东西，连自己的帮务都记不住？！\n");
                bonus = job["bonus"] * job["max"] / ((int)who->query("combat_exp") + 500);
        ...
        case "截镖":
                ...
                record =100+(int)ob->query("combat_exp")/200+random(240);
               // record = 100 + (int)who->query("combat_exp")/1000+random(240));
       ...
       // obj->add("score", bonus);
        message_vision(HIY"你这次工作共得到 "+(int)bonus +" 点功劳点，"+(int)record +" 点经验值。\n"NOR, who);
