...
int do_collect(string arg)
{
        ...
        myexp = (int)me->query("combat_exp")*4/5;
        ...
        if ((int)this_object()->query("job/is_kill")==1) 
        {
          ...
          if (me->query("job/killer")=="me")
          {
            ...
            message_vision("$N接着说道：" + RANK_D->query_respect(me) + "请放心，我马上派人将钱送到贵帮！！！\n", victim);
          	...
          	bonus = 300+((int)me->query("combat_exp") / 10000)+ random(200);;
        	record = bonus + random(bonus);
        	me->add("combat_exp", record);
             bonus /= 4;
            log_file("test/BangJob", sprintf("%s于%s时因收摊费得%s经验点\n", me->query("name"), ctime(time()), chinese_number(record)));
            tell_object(me,HIY"你这次工作共得到"+bonus+"点功劳点，" +record+ "点经验值。\n"NOR);
            me->add("clbscore",bonus);
            delete("job");
            return 1;
          }
        ...
        if ((random(me->query("combat_exp"))/1000)>10)
        {
           ...
           obnpc->set("title", HIR"拿人钱财"HIY"替人消灾"NOR);
        ...
        message_vision(HIR"\n"+ obnpc->name() + HIM"对$N喝道：" + RANK_D->query_rude(me) + "想收钱问问大爷手里的家伙答不答应！！！\n"NOR, me);         
        obnpc->kill_ob(me);
        me->kill_ob(obnpc);
        ...
        bonus = 100+((int)me->query("combat_exp") / 10000)+ random(200);;
        record = bonus + random(bonus);
        me->add("combat_exp", record);
        bonus /= 4;
        log_file("test/BangJob", sprintf("%s于%s时因收摊费得%s经验点\n", me->query("name"), ctime(time()), chinese_number(record)));
