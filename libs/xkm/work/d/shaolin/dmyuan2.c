...
int do_understand(string arg)
{
  ...
  // modified by aln here to show the importance of max jing
	...
	if(random(20) == 0) {
		me->add("potential", -2);
		me->improve_skill(arg, skill_special + random(skill_special*4));
		write( "只觉脑中豁然开朗，你对" + to_chinese(arg) + "的领悟又进了一层！\n" );
	}
	else if(random(10) == 0) {
		me->add("potential", -1);
		me->improve_skill(arg, skill_special + random(skill_special/1));
		write( "你冥思苦想，对" + to_chinese(arg) + "渐渐加深一层体会！\n" );
	}
	else if(random(5) == 0) {
		me->add("potential", 0);
		me->improve_skill(arg, skill_special/2 + random(skill_special/2));
		write( "你拿手比划着，似乎对" + to_chinese(arg) + "中的几个招数有所体会。\n" );
	}
	else if(random(2) == 0) {
		me->improve_skill(arg, skill_special/4 + random(skill_special/4));
