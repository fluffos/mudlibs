...
void create()
{
         set_name("桃花童", ({"taohua tong","kid","tong"}));
        set("gender", "男性");
        set("age", 9);
        set("long",
...
int do_apply(string arg)
{
	...
	RANK_D->query_respect(me) +"不是已经有了房子吗？还想再骗一个？\n",officer,me);
        ...
        partner = me->query_temp("marriage/id");
        if(file_size("/data/playerhomes/h_"+partner+".o") != -1)
        {
                message_vision(laotou+"调皮地刮了刮$n的脸，嘻笑道：" +
	RANK_D->query_respect(me) +"，你们家不是已经有了房子吗？还想再骗一个？\n",officer,me);
        ...
        message_vision(laotou+"笑眯眯地对$N道：买房、娶妻、生子，人生三大事耶！\n", me);
        me->set_temp("is_applying_house",1);
;
        room=new("/clone/taohuaobj/home");
