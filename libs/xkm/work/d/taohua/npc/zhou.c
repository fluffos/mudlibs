...
int ask_jieyi()
{
	object ob=this_player();
     ...
     if( ob->query("family/family_name") != "全真教" && ob->query("family/family_name") != "古墓派" ) {
        command ("say " + RANK_D->query_respect(this_player()) + "跟老顽童好像没有什么渊源吧！");
         return 1;
 }
...
int do_fangyuan()
{
	...
	if (hubo == 22) {
        if (ob->query("kar") > 26 && ob->query_int() < 23 || ob->query_skill("yunu-jue",1) > 200 && ob->query("gender") == "女性") {
