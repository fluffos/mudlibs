// 谁与争锋·NPC: /d/ruzhou/npc/xi-laifu.c  办喜宴出现的喜来福老板
...
int greeting(object me)
{
	...
	else if(me->query_temp("kick"))//被踢的人
...
int accept_object(object me, object ob)
{
	...
	i = ob->value();
//       me->add("balance",i);//pro add
	if(!me->query_temp("songli"))//防止玩家反复送礼骚扰频道
	{
...
//		message_vision("$N对$n不好意思的说：我这正开着一席呢，要不您明儿再来吧！\n",ob,me);
	if( (string)environment(ob)->query("short")!="醉仙楼二楼" )
...
int do_serve()
{
	...
	if( !me->query_temp("party_start_already") )
        ...
        message_vision("$N轻声对喜老板说：喜老板，客人等不及了，请上菜吧！\n", me);
...
void finish_party(object me,object ob)
{
	...
	while (i--)
	{
        ...
        ob2->set("name",me->name()+"和"+me->query("spouse/name")+"的喜糖");
    	ob2->move(list[i]);
	}
...
int give_money(object me,object ob)
{
	...
	if( !me || environment(me) != environment(ob) )
	{
		...
		write_file("/u/jhy/marry",sprintf("%s(%s)%s",me->name(),me->query("id"),ob->query("money")));
		ob->delete_temp("ready_to_party");
		ob->move("/u/jhy/workroom");//不要直接dest，以备查
	   return  1;
	}
...
//因此改成进帐户
	message_vision("\n喜老板笑呵呵地走到$N面前，一挥手，旁边的小伙计捧过来一本帐本，喜老板说道：\n"
     ...
     money = ob->query("money");
       me->add("balance",money);
//        me->set("balance",me->query("balance",1)+money);
	ob->delete_temp("ready_to_party");
  	ob->move("/u/jhy/workroom");//不要直接dest，以备查
	return 1;
}
...
int do_steal(string arg)
{
	...
	me->set("jianyu/what","steal");//做牢原因
	me->set("jianyu/where","ruzhou");//被捕地点
	me->apply_condition("jianyu",20);//做牢时间
	me->move("/d/wuguan/jianlao_yz");//进监狱
	return 1;
}
...
int kick(object me)
{
	...
	tell_room("/d/ruzhou/xiaozheng","突然一个人影从西边的迎宾楼二楼飞出来，“蓬”地重重摔在地上，细看却是一个差点摔死的人。\n");
	me->set_temp("kick",1);
	me->move("/d/ruzhou/xiaozheng");//移到楼下的大街房间里即可
