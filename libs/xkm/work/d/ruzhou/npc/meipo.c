// 谁与争锋·NPC: /d/ruzhou/npc/meipo.c 只处理结婚与拜堂
...
void create()
{
	...
	set("inquiry", ([
		"结婚" : "媒婆笑眯眯地点头道：“是啊... 要结婚当然要真心相爱(爱)我才来做媒的呀！”\n",
		"marry" : "媒婆笑眯眯地点头道：“是啊... 要结婚当然要真心相爱(爱)我才来做媒的呀！”\n",
		"登记" : "媒婆笑眯眯地点头道：“是啊... 要结婚当然要真心相爱(爱)我才来做媒的呀！”\n",
		"离婚" : "媒婆脸色一变，有点勉强地笑道：“要...离婚(unmarry)？唉...！我可是看你们成婚的呀！\n"
      		"不过你们要是一定要离的话，那得去扬州老知府处办理了！”\n",
...
{	
	...
	object who,meipo = this_object();
	
	
        if (this_player()->query("gender") == "女性")
                return notify_fail("媒婆说：“男婚女嫁嘛，应该男方出钱！”\n");
	...
	//if(!me->query_temp("ask_money"))//由下面判断交钱的人是否符合结婚条件。
  	...
  	if(meipo->query_temp("marry/money"))//已经有人交了钱了
	...
	meipo->set_temp("marry/money",me->query("id"));
	meipo->set_temp("marry/money2",who);   //记录给过钱的人
       log_file( "money/MARRY", sprintf("%s(%s)结婚交钱 in %s。\n",me->query("name"),
...
int do_name(string target)//请客人
{
	...
	if(ob->query_temp("marry/money")!=me->query("id")&&ob->query_temp("marry/money2")!=me->query("id"))
      		return notify_fail("媒婆眼睛斜了斜你：“不是你结婚你请个啥客的，等你结婚在请吧”\n");
	
	if(target == me->query("id")||target ==me->query_temp("marriage/id"))
		return notify_fail(HIY"媒婆眼睛斜了斜你：“没有人捧场也用不着自己请自己吧”\n"NOR);
	
	if((me->query("name"))==target || (me->query_temp("marriage/name"))==target)
		return notify_fail(HIY"媒婆眼睛斜了斜你：“没有人捧场也用不着自己请自己吧”\n"NOR);
	...
	ob->set("marry/waiting_que",1);//等鹊
	ob->set("marry/me_name", me->name()+"("+me->query("id")+")");//主人名
...
int do_chase(object ob,object dest)//喜鹊送请柬
{
	...
	obj->set("name",HIR+(string)ob->query("marry/me_name")+"的结婚请帖" NOR);
	obj->move(dest);
...
int do_ok(object ob,int flag)//喜鹊送柬成功回来
{
	if(flag)
		...
		message_vision("突然看见花喜鹊从窗外飞进来，落在媒婆身旁，媒婆笑眯眯地抚着花喜鹊的头后\n"
			"说道：“请柬刚才已经都送到了！您还想请谁？要不想请了，我就在问你们一次是不是真的爱对方(爱<你的对象>)！”\n",ob);
...
int do_marry(string arg) //判断结婚
{
	...
	if(ob->query_temp("marry/money")!=me->query("id")
   		&&ob->query_temp("marry/money")!=obj->query("id"))   //只要其中有一个人给过就行
   	{
		me->set_temp("ask_money",1);
    		obj->set_temp("ask_money",1);//表示条件符合，可以付钱，防止条件不符白付钱
	...
	if(!me->query_temp("marry/answer"))
	{
		message_vision(HIG"媒婆摊开姻缘簿，絮絮叨叨地说道：“结婚是一辈子的终身大事，可不能\n"
			...
			"让我看看："HIC"  "+me->name()+"结过"+(int)me->query("spouse/married")+"次婚；  "+obj->name()+"结过"+(int)obj->query("spouse/married")+"次婚；    "
	...
	message_vision(MAG "\n$N对著$n大声说道：我愿意！\n\n"NOR, me,obj);
	message_vision(MAG "媒婆呵呵笑道：“那就好，那就好，等我叫人布置好就拜堂吧！”\n"
		"说完，媒婆就叫人布置大堂。\n" NOR, obj, me);
	me->delete_temp("ask_marry");//玩家取消问结婚的参数
	obj->delete_temp("ask_marry");//玩家取消问结婚的参数

	ob->delete_temp("marry/money");//媒婆取消交过钱的参数
	ob->set("marry/have_couple",1);//媒婆在主持婚礼中
	
	me->set_temp("pending/pro",1);
	obj->set_temp("pending/pro",1);//婚礼中

	me->set("spouse/name",(string)obj->name());
	me->set("spouse/id",obj->query("id"));
	obj->set("spouse/name",(string)me->name());
	obj->set("spouse/id",me->query("id"));//正式结婚参数
	

	if ((string)obj->query("gender") == "女性")
	{
                obj->set("spouse/title", "丈夫");
                me->set("spouse/title", "妻子");
                me->set_temp("apply/short", ({HIR+(string)obj->name()+"丈夫"HIY+me->name()+"("+me->query("id")+")"+NOR}));
               obj->set_temp("apply/short", ({HIR+(string)me->name()+"妻子"HIY+obj->name()+"("+obj->query("id")+")"+NOR}));
			}
	else
	{
		me->set("spouse/title", "妻子");
		obj->set("spouse/title", "丈夫");
                obj->set_temp("apply/short", ({HIR+(string)me->name()+"深爱的丈夫"HIY+obj->name()+"("+obj->query("id")+")"+NOR}));
                 me->set_temp("apply/short", ({HIR+(string)obj->name()+"深爱的妻子"HIY+me->name()+"("+me->query("id")+")"+NOR}));
			}
...
int ready_marry(object ob,object me,object obj)//准备婚礼
{
	...
	if(!( room = find_object("/d/city/datang")) )
	room = load_object("/d/city/datang");
...
int do_bai(string arg)//拜堂
{
	...
	if( !environment()||base_name(environment()) != ("/d/city/datang") )
		return notify_fail("媒婆说道：“你这人真是的，拜堂是要到礼堂才行的呀！”\n");

	if( !me->query_temp("pending/bai"))
		return notify_fail("媒婆笑道：“哈哈，你犯什么傻呀！”\n");

	if( !objectp(obj = present((string)me->query("spouse/id"), environment(me)))
	...
	else if((int)me->query_temp("pending/bai")==3)
	{
		...
		CHANNEL_D->do_channel(this_object(), "chat",
			sprintf( "恭喜%s和%s，一对璧人喜结良缘。\n        各位亲朋好友，可携礼到醉仙楼二楼参加他们的婚宴！\n", me->name(), obj->name()));

		me->add("spouse/married", 1);        //结婚次数
		me->set("spouse/have_couple",1);     //处于已婚状态
		me->set_temp("married_party",1);     //做上记号，喜来福老板看到就会同意开酒席
		obj->set_temp("married_party",1);    //两人都做，但只有男性才有用
		obj->add("spouse/married", 1);       //结婚次数
		obj->set("spouse/have_couple",1);    //已婚状态
...
int last_marry(object ob,object me,object obj)//拜堂结束
{
	...
	moon->move("/u/jhy/workroom");
	ob->delete_temp("wait_time");
	ob->delete_temp("marry/money");
	say("媒婆喊道：“轿夫！快将新人送到醉仙楼举行婚宴，我和月老就不去了，呵呵！”\n说完一扭一扭地向西边走去。\n");
...
int go_lou(object me,object obj)
{
	...
	message_vision("\n    过了好长一会儿，只听轿夫们一声“到啦！”轿子整个儿一震，想是落到了地上，\n"
   	"$N和$n从轿子中下来一看，原来已经到了醉仙楼。两人又被拥扶着一直上了二楼。\n"
      "只听一阵呵呵笑声，迎宾司仪 喜来福笑容满面地迎出来：“恭喜两位新人！恭喜恭喜！”\n",me,obj);
	npc=new("/d/ruzhou/npc/xi-laifu");
	npc->set("name1",me->name());
	npc->set("name2",obj->name());//双方姓名
	npc->move("/d/city/zuixianlou2");
	me->move("/d/city/zuixianlou2");
	obj->move("/d/city/zuixianlou2");
	return 1;
}
...
int return_home(object home)//覆盖标准继承中的该函数，防止婚礼中更新，媒婆回家
{
	...
	if( this_object()->query("marry")) return 0;//有结婚的参数就不回去
