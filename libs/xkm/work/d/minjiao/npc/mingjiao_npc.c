//Cracked by Kafei
#include <ansi.h>
#define OBJ_PATH "/d/minjiao/obj"
#define MINGJIAO "/d/minjiao"
#include "/d/minjiao/obj/time_all.h"
inherit NPC;

string ask_tyling()
{
	object player=this_player();
	object ling;
    mapping fam ;
	int comexp;

    if (!(fam = this_player()->query("family")) || fam["family_name"] != "明教")
		return "这位"+RANK_D->query_respect(player)+"并非我明教弟兄，打听这个干嘛。\n";


	if(objectp(ling = present("tieyan ling", player)) )
		return "铁焰令乃我明教信物，每个教众在入教时都会由我教光明使者发派一块。凭此\n"
		"令，可以向教中兄弟请教武功，凡是功过奖赏，都会记录在这块铁焰令上。这铁令关系重\n"
		"大，"+RANK_D->query_respect(player)+"切莫丢失，令在人在，令亡人亡。\n";
	else
		if(query("id")=="leng qian"&&query("name")=="冷谦")
		{
			comexp=player->query("mingjiao/credit");
			if(comexp)
				comexp=comexp-comexp/50;
			if(comexp<=0)
				comexp=0;
			command("say 丢了铁焰令，该当受罚。\n");
			tell_object(player,HIB"你的明教声望下降了。\n"NOR);
			player->set("mingjiao/credit",comexp);
			
			ling = new("/d/minjiao/obj/tieyanling");
			ling->move(player);

			message_vision("$N从身上拿出一块铁焰令交给$n。\n",this_object(),player);
			return player->query("name")+"！收好，不可再丢。\n";
		}
		
		else
			return "铁焰令乃我明教信物，"+RANK_D->query_respect(player)+"怎可丢失？快去向冷执法禀明过失。\n";


}

int init()
{
	add_action ("do_tiaozhan", "tiaozhan");
}



void attempt_apprentice(object ob)
{
		command("say 我光明圣教向来没有师徒之别，只有等级之分，教众之间可以相互传授武艺。这是我教历来的规矩。" + RANK_D->query_respect(ob) +"想要学武的话，可向教中有名望的兄弟请教。\n");
		
		return;
}

void Set_Inquiry(mapping I_inquiry,mixed Skill_name)
{
	int i;
	string str_skill;
	mapping I_inquiryS;
	if(sizeof(I_inquiry))
		I_inquiryS=I_inquiry;
	str_skill="";

	if(i=sizeof(Skill_name))
	{
		set("teach_skillsname",Skill_name);
		for(i=0;i<sizeof(Skill_name);i++)
		{
			str_skill=str_skill+to_chinese(Skill_name[i])+"("+Skill_name[i]+")"+"\n";
			
		}
	
		I_inquiryS["武功"]="在下不才，能传授给阁下的武功有\n"+str_skill+"\n如阁下想学，只需向我请教( qingjiao <某人> <技能> [次数])，就可以了。\n";
		I_inquiryS["skills"]="在下不才，能传授给阁下的武功有\n"+str_skill+"\n如阁下想学，只需向我请教( qingjiao <某人> <技能> [次数])，就可以了。\n";
	}
	else
		I_inquiryS["武功"]="在下武功低微，恐怕不能传授阁下什么武功，阁下请另请高明。\n";
		I_inquiryS["skills"]="在下武功低微，恐怕不能传授阁下什么武功，阁下请另请高明。\n";

	I_inquiryS["铁焰令"]=(: ask_tyling :);
	set("inquiry",I_inquiryS);

	
	return;
}

int Set_Action()
{
	object ob=this_object();
	if(ob->query("teach_skillsname"))
	{
		add_action ("do_qingjiao", "qingjiao");

	}
}


void Set_Npcattrib(int Basic_Attrib,int Random_var,
				   int Maxqi,int Random_Maxqi,
				   int Maxjing,int Random_Maxjing,
				   int Maxjingli,int Random_Maxjingli,
				   int Maxneili,int Random_Maxneili,
				   int Exp,int Random_Exp)
				  				  
{
	int Mneili=Maxneili+random(Random_Maxneili);
	set("str", Basic_Attrib+random(Random_var));
	set("int", Basic_Attrib+random(Random_var));
	set("con", Basic_Attrib+random(Random_var));
	set("dex", Basic_Attrib+random(Random_var));
	set("max_qi", Maxqi+random(Random_Maxqi));
	set("max_jing", Maxjing+random(Random_Maxjing));
	set("eff_jingli", Maxjingli+random(Random_Maxjingli));
	set("max_neili", Mneili);
	set("neili",Mneili);
	set("combat_exp", Exp+random(Random_Exp));
	return;
}

void Set_Npcskills(int Basic_skill,int Random_skill,mapping S_skills)
{
	int i;
	mixed S_name;
	
	if(i=sizeof(S_skills))
	{
		S_name=keys(S_skills);
		for(i=0;i<sizeof(S_name);i++)
		{
			set_skill(S_name[i], Basic_skill+random(Random_skill) );
			set_skill(S_skills[S_name[i]], Basic_skill+random(Random_skill) );
			map_skill(S_name[i],S_skills[S_name[i]]);
		}
	}

	set("jiali",(Basic_skill+Basic_skill/2)/2);
	return;
}


int prevent_learn(object me, string skill)
{
	return 0;
}

void set_title(object ob1)
{
	string title;
	int gen=ob1->query("family/generation");
	//switch(generation)
	switch(gen)
	{
		case 33:
			title = "教主";
			break;
		case 34:
			title = "光明使者";
			break;
		case 35:
			title = "护教法王";
			break;
		case 36:
			title = "散人";
			break;
		case 37:
			title = "掌旗使";
			break;
		case 38:
			title = "教众";
			break;
		//default:
		//	title = "教众";
	}
	ob1->set("family/title",title);
        ob1->set("title","明教"+title);
	message_vision("恭喜您了成明教 \n");
}

string ask_jijie()

{
	object player=this_player();
	mapping fam ;
	

	if (!(fam = this_player()->query("family")) || fam["family_name"] != "明教")
		 
		return "这位"+RANK_D->query_respect(player)+"并非我明教弟兄，想晋级先从教众做起吧。\n";
	
	if(!player->query("mingjiao/cc"))
		return "这位"+RANK_D->query_respect(player)+"对明教还没有做贡献,就想晋级不会是做梦吧。\n";
			
	if(player->query_temp("mj/tz"))
		return "这位"+RANK_D->query_respect(player)+"快点找"+(player->query_temp("mj/tz"))+"挑战去吧。\n";
	
	if(player->query("combat_exp")>100000 && player->query("mingjiao/cc")>20000 && player->query("family/generation") ==38 )
		{player->set_temp("mj/tz","掌旗使");
		return "这位"+RANK_D->query_respect(player)+"的条件以够，快找五位掌旗挑战吧。\n";		
	}
	else
	if(player->query("combat_exp")>500000 && player->query("mingjiao/cc")>100000 && player->query("family/generation") ==37 )
		{player->set_temp("mj/tz","散人");
		return "这位"+RANK_D->query_respect(player)+"的条件以够，快找散人挑战吧。\n";		
	}
	else
	if(player->query("combat_exp")>1000000 && player->query("mingjiao/cc")>500000 && player->query("family/generation") ==36 )
		{player->set_temp("mj/tz","法王");
		return "这位"+RANK_D->query_respect(player)+"的条件以够，快找护教法王挑战吧。\n";		
	}else
	if(player->query("combat_exp")>20000000 && player->query("mingjiao/cc")>1000000 && player->query("family/generation") ==35 )
		{player->set_temp("mj/tz","使者");
		return "这位"+RANK_D->query_respect(player)+"的条件以够，快找光明使者挑战吧。\n";		
	}else
	if(player->query("combat_exp")>4000000 && player->query("mingjiao/cc")>2000000 && player->query("family/generation") ==34 )
		{player->set_temp("mj/tz","教主");
		return "这位"+RANK_D->query_respect(player)+"的条件以够，快找教主挑战吧。\n";		
	}else
		{return "这位"+RANK_D->query_respect(player)+"条件还不够呀。\n";
}}

int do_tiaozhan(string arg)
{	object ob1, ob2;
        ob2  = this_object();
        ob1  = this_player();
        
       if(!arg || arg=="")
       {
       return notify_fail("你想挑战谁。\n");
       }
       if( !objectp( ob2 = present(arg, environment(ob1)) ) ) 
       return notify_fail("这里没有你挑战的人。\n");
        
       if ( !ob1->query_temp("mj/tz") ) 
       { 
       	return notify_fail("你想做什么，还是先到执法处的冷谦打听有关晋级的事宜吧。\n");
       	}
       
       if (ob2->query("family/title")!=ob1->query_temp("mj/tz"))
       { 
       return notify_fail("你找错人了,你快去找"+(ob1->query_temp("mj/tz"))+"挑战吧。\n");
	}
	
	
       {ob1->delete_temp("mj/tz");
        command("say 好吧我就接受你的挑战吧。\n");
        remove_call_out("fighting");
	call_out("fighting", 2,ob1,ob2);
        return 1;
	}
}
void fighting(object ob1, object ob2)
{
	message_vision("\n" + ob1->name() + "与" + ob2->name() + "开始比武！\n\n", ob1);

	remove_call_out("checking");
	call_out("checking", 0, ob1, ob2);

	this_object()->set_temp("busy");

	ob1->fight_ob(ob2);
	ob2->fight_ob(ob1);
}

int checking(object ob1, object ob2)
{
	object ob; 
	int gen=ob2->query("family/generation");
	
	
	if(ob1->is_fighting()) 
	{
		call_out("checking", 1, ob1, ob2);
		return 1;
	}

	ob1->delete_temp("pending/fight");
	ob2->delete_temp("pending/fight");

	if ( !present(ob1, environment(ob2)) ) return 1;

	if ( !present(ob2, environment(ob1)) ) return 1;
           
	if (ob1->query_temp("halted"))
	{
		ob1->delete_temp("halted");
		return 1;
	}

	if (ob2->query_temp("halted"))
	{
		ob2->delete_temp("halted");
		return 1;
	}

	if ( ob1->query("qi")*2 > ob1->query("max_qi")) 
		{
		
		
		command("say "+RANK_D->query_respect(ob1)+"武功卓绝，令为兄汗颜之至，惭愧，惭愧！\n");
		//remove_call_out("title"); 
		ob1->set("family/generation",gen);
		call_out("set_title",1,ob1);
		}
	
	else 
	{
		command("say 看来这位"+RANK_D->query_respect(ob1)+"还得多加练习，方能在我明教中出人头地！\n");

	}

	this_object()->delete_temp("busy");

	return 1;

}








