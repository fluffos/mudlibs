...
LONG);        
//让跟随你的贴身武士暂时离开<wavefighter>
  //       呼唤暂时离开的贴身武士<shoutfighter>       
                set("value", 0);
                set("material", "iron");
				set("no_get",1);
...
{	
  ...
  write("	┌───────"+YEL"【铁焰令】"NOR+"─────────┐\n" ); 
  printf("	│明教弟子 ""%30s   │\n",me->query("name") );   
  printf("	│明教功绩 ""%30d   │\n" ,(int)me->query("mingjiao/cc")); 
  printf("	│明教任务 ""%30s   │\n" ,((string)job_flg)); 
  printf("	│明教声望 ""%30s   │\n" ,chinese_number((int)me->query("mingjiao/credit"))); 
 write("	└─────────────"+HIR"【明教】"NOR+"────┘\n" );    
