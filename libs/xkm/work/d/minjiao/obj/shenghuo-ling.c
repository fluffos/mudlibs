...
{
    unequip();
        set("weapon_prop/damage", 60*count);
    ::set_amount(count);
}
...
int do_du(string arg)
{
        ...
        if(me->query_skill("shenghuo-xuanming",1) < 100){
                write("你的圣火玄冥功火侯太浅，不能领会令上的高深武功。n");
                return 1;
        }
        if(me->query_skill("shenghuo-ling",1)<101){
        ...
        qklevel = me->query_skill("shenghuo-ling", 1);
        ...
        if (me->query_skill("shenghuo-ling",1)>120){
         ...
         neili_lost=(int)(me->query_skill("shenghuo-xuanming",1)/10);
        me->receive_damage("jing", 15);
        me->set("neili",(int)me->query("neili")-neili_lost);
        me->improve_skill("shenghuo-ling", (int)me->query_skill("literate", 1)/3+1);
...
int wield()
{
        ...
        if (living(ob) && !ob->query_skill("shenghuo-ling", 1)){
                message_vision("$N拿出"+name()+"看看了，却搞不懂怎么使用这个东西。\n", ob);
                return 0;
                }   
        if (living(ob) && query_amount() >= 2 &&
            (ob->query_skill("shenghuo-ling", 1) < 200 ||
             ob->query_int() < 38)){
