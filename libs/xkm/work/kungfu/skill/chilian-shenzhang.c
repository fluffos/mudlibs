...
mapping query_action(object me, object weapon)
{
	...
	level1   = (int) me->query_skill("chilian-shenzhang",1);
	level2   = (int) me->query_skill("wudu-xinfa",1);
	level3   = (int) me->query_skill("throwing",1);
	target = me->select_opponent();
	weapon = me->query_temp("weapon");

	if ( level1 >= 120 && level3 >= 150 && random(level3) > level3 * 1/2
	&& me->query("neili") > 10 && me->query("jingli") > 5) {
		if ( objectp(anqi=present("bingpo yinzhen", me)) ) {
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
	...
	if(random(me->query_skill("wudu-xinfa")) > 30 && me->query_skill("chilian-shenzhang") > 50) 
			victim->apply_condition("chilian_poison", random(me->query_skill("wudu-xinfa", 1)*2) + victim->query_condition("chilian_poison"));
}