...
int perform(object me, object target)
{
        ...
        if( me->query_temp("yield") )
                return notify_fail("你不出手，如何使出「三无三不手」？\n");
        ...
        me->set_temp("sanwu", 1);
                ...
                {
                        case 1:
				//可以再减小攻击力，目前威力测试较少，可以实验 skill/15
                                amount = skill/8;
                                break;
                        case 2:
				//可以再减小攻击力，目前威力测试较少，可以实验 skill/12
                                amount = skill/6;
                                break;
                        case 3:
				//可以再减小攻击力，目前威力测试较少，可以实验 skill/10
                                amount = skill/4;
                                break;
                }
				if (wizardp(me))
...
void remove_effect(object me)
{
        ...
        me->start_busy( 1 + random(sanwu_busy) );
        call_out("receive_heal", (int)amount / 2, me);
}
