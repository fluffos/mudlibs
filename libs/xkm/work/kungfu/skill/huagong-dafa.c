...
int valid_learn(object me)
{
        ...
        || me->query_skill("xiantian-gong", 1) || me->query_skill("yunu-xinjing", 1)
        || me->query_skill("beiming-shengong", 1) || me->query_skill("bahuang-gong", 1)
...
mixed hit_by(object me, object victim, int damage, int damage_bonus, int factor)
{
	...
	dp = (int)victim->query_skill("huagong-dafa",1);
        ...
        if ( random(ap+dp) > ap ) {
				...
				result = HIB"只见$n手中杖发出淡淡蓝光，$N心中一惊，不敢妄进，将兵刃收了回去。\n"NOR;
				damage_bonus=-random(4000);
			}
                }
                        ...
                        if (damage + damage_bonus > 0)
			{
				...
				result = HIB"只见$n嘴角微微冷笑，似乎成竹在兄，$N心中一惊，不敢妄进，将进招改为守招。\n"NOR;
				damage_bonus=-random(4000);
			}
