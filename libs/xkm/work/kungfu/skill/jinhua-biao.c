...
int valid_learn(object me)
{
	if ((int)me->query_skill("shenghuo-xuanming", 1) < 25)
		return notify_fail("你的圣火玄冥功火候不够，无法练金花镖。\n");
        if ((int)me->query("max_neili") < 100)
                return notify_fail("你的内力太弱，无法练金花镖。");
        return 1;
}
...
{
	int i, level;
	level   = (int) me->query_skill("jinhua-biao",1);
...
{
	return __DIR__"jinhua-biao/" + action;
}
