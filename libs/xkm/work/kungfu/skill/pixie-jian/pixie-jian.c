...
mixed hit_by(object me, object victim, int damage)
{
        ...
        object weapon_me=victim->query_temp("weapon");
        mapping prepare;
        string result, attack_skill,type;
        if(objectp(weapon_me=victim->query_temp("weapon")))
	type=weapon_me->query("skill_type");
	else
	type="none";
        if (!living(victim)
			...
			||type!="sword"
			||(int)victim->query_temp("pixie/guimei")<=1
			||victim->query("jingli")<300
		...
		if(objectp(weapon)&&me->query_skill_mapped("sword") == "dugu-jiujian" )
		{
			result = HIR "$n身法陡变，，形若幽灵，影似鬼魅，似要从剑招中躲过。\n"NOR;
			result += HIC "但见$N心念一动，招随意转，剑招所指，正是$n身法不及之处。\n"NOR;
