...
//        message_vision(msg,me,victim);
        ...
        if ( random(ap+dp) > ap ) {
                result = HIG "\n只听「嗒」的一声，$n手背上被棒端戳了一下，急忙缩手，已感又疼又麻。\n" NOR;
...
//                message_vision((string)weapon->query("weapon_prop/damage") + "\n",me,victim);
                ...
                result = HIG "\n突然青光闪动，棒招来势神妙无方，"+(string)weapon->query("name")+HIG"击中了$n。\n" NOR;
                message_vision(result,me,victim);
//                message_vision((string)weapon->query("weapon_prop/damage") + "\n",me,victim);
                ...
                result = HIG "\n$N猛地递出一招，这一棒迅捷无伦，直取$n眉心，$p「啊」的一声，连退出数尺！\n" NOR;
                message_vision(result,me,victim);
//                message_vision((string)weapon->query("weapon_prop/damage") + "\n",me,victim);
        ...
        {
                result = weapon->name()+HIG"\n幌动，霎眼之间竟已转到$n身后，拍的一声，在$p臀上抽了一下。！\n" NOR;
                message_vision(result,me,victim);
//                message_vision((string)weapon->query("weapon_prop/damage") + "\n",me,victim);
