...
//                      default: msg = WHT"突然间$N掌力破空，拍向$n"+limb+"，劲风扑面，锋利如刀，转而斜砍$p$l"NOR;
                        default: msg = WHT"突然间$N掌力破空，拍向$n"+limb+"，不料$N左掌一带，掌力绕过$N的身子，直拍向$p$l"NOR;
                                break;
                }
        ...
        if ( objectp(victim) && me->query_temp("lingjiu/baihong") ) {
		jiali=me->query("jiali");
		if (jiali < 50)
		jiali = 50;
		if (victim->query("neili") > 0) {
		message_vision(WHT "\n$n摸不透$N掌力虚实，只好大耗真力，把全身都布满真气。\n" NOR, me, victim);
		victim->add("neili", -jiali);
		} else {
		message_vision(HIR "\n$n一口真气提不上来，被$N掌力击中，闷哼一声被震出了丈许。\n" NOR, me, victim);
		victim->receive_damage("qi", jiali, me);
		}
        }
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor)
{
		...
		tell_object(victim,HIC"\n你只觉得一股寒气优似冰箭袭入你的内腑，循着手臂，迅速无伦的扩散至全身！\n"NOR);
	}
}
