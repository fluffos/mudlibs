...
int perform(object me, object target)
{
  ...
  if( !target || !target->is_character()
	...
	if(me->query("int") < 20)
		return notify_fail("你先天悟性太差, 不能使用这项绝技! \n");

	if((int)me->query_int() < 33)
	...
	if( (int)me->query("neili", 1) < 400 )
			return notify_fail("你现在内力不足，不能使用这一绝技！\n");	
...
//    if(!me->query("env/wiz_test")) return notify_fail("你不是试用此招式之人。\n");
  ...
  message_vision(HIY"$N运用斗转星移，对$n使出"+
    to_chinese(skill)+"，俨然是姑苏慕容的“以彼之道，还施其身”！\n"NOR, me, target);
  target->set_temp("perform_target/douzhuan",1);
  me->add("neili", -80);
  me->add("jingli", -20);
...
private void remove_effect(object me, object target)
{
  ...
  {
    message_vision( HIY"$N斗转星移使完，已飞身落地。\n" NOR, me);
    me->delete_temp(DOU_ZHUAN);
	target->delete_temp("perform_target/douzhuan");
    return;
  }
