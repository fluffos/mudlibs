...
int exert(object me, object target)
{
	...
	if( !target )
		return notify_fail("你要用真气为运阳退？\n");

	if( me->is_fighting() || target->is_fighting())
		return notify_fail("战斗中无法运阳退，你找死么！\n");
...
//		return notify_fail("你已经在运功练功了。\n");
	...
	if( (int)me->query_skill("yunu-xinjing", 1) < (int)target->query_skill("yunu-xinjing", 1) / 2 )
		return notify_fail("你的玉女心经火候太低，无法相助对方练功。\n");

        if ( me->query_skill_mapped("force") != "yunu-xinjing")
                return notify_fail("你没有激发玉女心经到内功上！\n"); 

	if ( (int)me->query_skill("taiji-shengong") > 99 || (int)me->query_skill("hunyuan-yiqi") > 99
	|| (int)me->query_skill("huagong-dafa") > 99 || (int)me->query_skill("zixia-gong") > 99 
	|| (int)me->query_skill("linji-zhuang") > 99 || (int)me->query_skill("bitao-xuangong") > 99
	|| (int)me->query_skill("longxiang-banruo") > 99 || (int)me->query_skill("kurong-changong") > 99
	|| (int)me->query_skill("beiming-shengong") > 99 || (int)me->query_skill("bahuang-gong") > 99
	|| (int)me->query_skill("shenghuo-xuanming") > 99 || (int)me->query_skill("jiuyang-shengong") > 99
	|| (int)me->query_skill("xiaowuxiang") > 99 || (int)me->query_skill("douzhuan-xingyi") > 99 )
		return notify_fail("你体内不同内力互相冲撞，难以领会更高深的玉女心经。\n");
	...
	if( userp(target) && (object)target->query_temp("pending/yunu-xinjing")!=me ) {
		...
		tell_object(target, YEL "如果你愿意和对方同时运功修练，请你对" + me->name() + "("+(string)me->query("id")+")"+ "下一次 yun yinjin 指令。\n" NOR);
...
int over3(object me,object obj)
{
		object cloth;
...
的功夫，$N于$n具是燥热无比，气血沸腾，汉如雨下。\n"NOR,me,obj);
		...
		if( me->query_temp("armor/cloth")){
			message_vision(HIR "$N只觉得浑身燥热不安，原来身上还穿有衣衫，热气不能立时发散郁积体内。\n"NOR,me);
			message_vision(HIR "$N突然口吐鲜血，几欲晕倒在地。\n"NOR,me);
			me->receive_damage("qi", 1000, "练功走火入魔死了");
			me->receive_damage("max_qi", random(100), "练功走火入魔死了");
			me->receive_damage("jing", 1000, "练功走火入魔死了");
			me->receive_damage("max_jing", random(100), "练功走火入魔死了");
			return 1;
		}
		if( obj->query_temp("armor/cloth")){
			message_vision(HIR "$N只觉得浑身燥热不安，原来身上还穿有衣衫，热气不能立时发散郁积体内。\n"NOR,obj);
			message_vision(HIR "$N突然口吐鲜血，几欲晕倒在地。\n"NOR,obj);
			obj->receive_damage("qi", 1000, "练功走火入魔死了");
			obj->receive_damage("max_qi", random(100), "练功走火入魔死了");
			obj->receive_damage("jing", 1000, "练功走火入魔死了");
			obj->receive_damage("max_jing", random(100), "练功走火入魔死了");
			return 1;
		}

        if( !environment(me)->query("outdoors") ){
			message_vision(HIR "$N只觉得浑身燥热不安，原来身处室内，热气不能立时发散郁积体内。\n"NOR,me);
			message_vision(HIR "$N突然口吐鲜血，几欲晕倒在地。\n"NOR,me);
			me->receive_damage("qi", 1000, "练功走火入魔死了");
			me->receive_damage("max_qi", random(100), "练功走火入魔死了");
			me->receive_damage("jing", 1000, "练功走火入魔死了");
...
/*
		me->improve_skill("force", me->query_con()+random(me->query_skill("yunu-xinjing", 1))*60);
		me->improve_skill("yunu-xinjing", me->query_con()+random(me->query_skill("force", 1))*60);
		obj->improve_skill("force", obj->query_con()+random(obj->query_skill("yunu-xinjing", 1))*80);
		obj->improve_skill("yunu-xinjing", obj->query_con()+random(obj->query_skill("force", 1))*80);
        obj->add("max_neili",  add);
        obj->add("max_jingli",  add*2);
        obj->add("eff_jingli",  add*2);
*/

		if (obj->query("max_neili") > (int)obj->query_skill("force") * (int)obj->query("con") * 2 / 3)
			obj->set("max_neili", obj->query_skill("force") * (int)obj->query("con") * 2 / 3);
		if (obj->query("max_jingli") > (int)obj->query_skill("force") * (int)obj->query("con")/2)
			obj->set("max_jingli", obj->query_skill("force") * (int)obj->query("con")/2);
