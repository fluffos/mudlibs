...
("你体内不同内力互相冲撞，难以领会更高深的武功。\n");
        ...
        if ( (int)me->query_skill("beiming-shengong",1) > 49 && (int)me->query_skill("huagong-dafa",1) > 49 )
                return notify_fail("你体内不同内力互相冲撞，难以领会更高深的玉女心经。\n");

        if ( me->query("gender") == "无性" && lvl > 49)
                return notify_fail("你无根无性，阴阳不调，难以领会高深的玉女心经。\n");

        if ( me->query("gender") == "男性" && lvl > 200)
...
int practice_skill(object me)
{
        ...
        me->add("neili", -60);
        me->receive_damage("jingli", 70, "精力透支过度死了");
        me->receive_damage("qi", 70, "体力透支过度死了");
        return 1;
}
/*int practice_skill(object me)
{
...
||((string)weapon1->query("skill_type") == "whip" && me->query_skill_mapped("whip") == "duanyun-bian") ) ){
                        ...
                        result = "不料$n忽然一掌按出，无影无踪，手掌竟绕过$N手臂，按到了$N胸口，正是玉女心经上克制全真武功的绝招！\n";
                        if (damage + damage_bonus > 0)
                                me->receive_damage("qi", (damage + damage_bonus)/2, victim);
...
{
        int level = (int)me->query_skill("yunu-xinjing", 1);

        if( level >= 200
        &&  me->query("family/family_name") == "古墓派"
        &&  !me->query("gm_gifts/force") ) {
                me->add("con", 1);
                me->set("gm_gifts/force", 1);
                me->improve_skill("force", (level*level)*2, 1);
				message_vision( HIY"$N突然间眼前似见一片光明，四肢百骸，处处是气，口中不自禁发出一片呼声，这声音犹如龙吟大泽，虎啸深谷，远远传送出去。\n" NOR,me);     
				message_vision( "这片啸声约莫持续了一顿饭时分，方渐渐沉寂。\n" ,me);     
				write("\n你内功竟然进境至斯，不由得惊喜交集。\n");
		}     
}
