...
int valid_learn(object me)
{
	if ((int)me->query_skill("shenghuo-xuanming", 1) < 25)
		return notify_fail("你的圣火玄冥功火候不够，无法练夺命笔法。\n");
        if ((int)me->query("max_neili") < 100)
                return notify_fail("你的内力太弱，无法练夺命笔法。");
        return 1;
}
...
{       object ob;
        ...
        ||      (string)ob->query("skill_type") != "pike" )
                return notify_fail("你必须先找一对判官笔才能学习夺命笔法。\n");
        if( (int)me->query("jingli") < 30
        ||      (int)me->query("neili") < 20 )
                return notify_fail("你的内力或精力不够，没有办法练习夺命笔法。\n");
...
{
        return __DIR__ "duoming-bi/" + action;
}
