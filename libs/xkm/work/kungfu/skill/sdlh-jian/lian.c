        ...
        for(i = 0; i < attack_time; i++)
            if (me->is_fighting(target) && target->is_fighting(me) && target->query("eff_qi")>0){
              me->set_temp("action_msg","紧接着");
                if (!weapon->query("equipped")){ me->start_busy(2); return; }
        if (me->query_skill_mapped("sword")!="sdlh-jian") {me->start_busy(3); return; }
              COMBAT_D->do_attack(me, target, weapon, 0);
              me->start_busy(1);
            }else break;
    me->delete_temp("secondly_weapon");
    if (i) message_vision(HIY " 结果$p被$P攻了个措手不及！\n" NOR, me, target);
}
...
int perform(object me, object target)
{
    ...
    if( (int)me->query_skill("sdlh-jian", 1) < 101 )
   return notify_fail("你的三道轮回剑法极不够娴熟，不会使用「连」字诀。\n");
    ...
    if( me->is_busy()) return notify_fail("你现在正忙着。\n");
        msg = HIY "$N使出三道轮回剑法「连」字诀，招式陡然变快。\n" NOR;
    if( !living(target) || random(me->query("combat_exp")) > (int)target->query("combat_exp")/3 )
    {
        attack_time = (int)me->query_skill("sdlh-jian", 1) / 20;
        if(attack_time < 1)
            attack_time = 1;
    ...
    {
        msg += HIW  "可是$p"+ HIY +"看破了$P的企图，并没有上当。\n" NOR;
          me->start_busy(1);
    }
