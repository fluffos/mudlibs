...
mapping *action = ({
        ([
                ...
                ([
                "action":"$N丝毫不为$n所动，双手交替中，一式“参合无归”已封住了$n的所有退路",
                "force" : 450,
                "dodge" : 40,
                "parry" : 25,
		"damage": 100,
		...
		"damage": 130,
                ...
                ([
                "action":"$N忽然间化指为掌，“参和无形”意味古拙，掌力广被，$n莫辨其方向，难以招架",
                "force" : 550,
                "dodge" : 55, 
		"damage": 170,
                "weapon": "指力",
...
{
	if (this_player()->query_skill("canhe-zhi", 1) > 149)
...
{         
        ...
        if ( bb  >= 60 && aa < 140 && 5*aa <= 4*bb )
                return notify_fail("你的内功不够，无法修练更高深的参合指。\n");
        if ((int)me->query_skill("finger", 1) <50)
                return notify_fail("你的基本指法火候不够，无法学参合指。\n");
        return 1;
}
