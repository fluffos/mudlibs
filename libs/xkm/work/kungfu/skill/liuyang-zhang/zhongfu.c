...
蕴出一缕白线，猛地向$n的胸口射了过去！\n\n" NOR,
	...
	sp = me->query_skill("force") + me->query_skill("dodge") + me->query_kar()+ me->query_dex();
	dp = target->query_skill("force") + target->query_skill("dodge") + me->query_kar()+ me->query_dex();
	...
	if( random(sp) > random(dp) ) 
	{
		...
		tell_object(me, HIG "你觉得指间的白线流入" + target->name() + "的胸口，看来已经种下生死符了。\n" NOR);
                ...
                target->apply_condition("zf_poison",24);
		...
		me->add("neili", -125);
		me->add("jingli", -5);
				log_file("zhongfu", sprintf("%s(%s)于%s向%s(%s)使用了生死符，并且成功。\n",
