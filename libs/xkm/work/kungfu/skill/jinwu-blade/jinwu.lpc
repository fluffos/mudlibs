...
*/
        message_vision(HIY "$N纵身跃起,使出「金乌堕地」，挥舞着从半空中挥刀直劈下来，\n刀锋离地尚有数尺，地下已是尘沙飞扬，败草落叶被刀风激得团团而舞，\n将$n逃遁退避的空隙封的严严实实。\n"NOR, me, target);
        ...
        if( random(ap) > dp )
        {
            if(userp(me))
                 me->add("neili",-250);
        message_vision(HIC "\n$n避无可避,只觉一股刚猛之极的劲风扑面而来，使人直欲窒息。\n$n极力后退隔挡，却只见刀光一闪，毫不停息，一刀正中面门。\n\n" NOR, me, target);
...
msg += HIW "$N力道未尽，手中刀势不绝，连带$n手上"NOR""+inv[equip]->query("name")+HIW"一起劈断。\n只听当的一声，$n手上的"NOR""+inv[equip]->query("name")+HIW"已被劈成两半，掉在了地上！\n"NOR;
        message_combatd(msg, me,target);
*/
        message_vision(HIW "\n$N力道未尽，手中刀势不绝，连带$n手上的武器一起劈断。\n只听当的一声，$n手上的武器已被劈成两半，掉在了地上！\n\n" NOR, me, target);
