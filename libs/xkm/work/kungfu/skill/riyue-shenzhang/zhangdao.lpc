//  /kungfu/skill/riyue-shenzhang/zhangdao.c 掌刀
// Xuanyuan 2001
...
int perform(object me, object target)
{
	...
	if( me->query_temp("zhangdao") )
		return notify_fail("你已在使用掌刀了！\n");

	if( objectp(me->query_temp("weapon")) )
		return notify_fail("空手才能施展掌刀！\n");

	if( me->query_skill_mapped("strike") != "riyue-shenzhang" )
		return notify_fail("你所用的并非日月神掌，不能施展掌刀！\n");

	if( me->query_skill_prepared("strike") != "riyue-shenzhang" )
                return notify_fail("你所备的并非日月神掌，不能施展掌刀！\n");

	if( me->query_skill_mapped("force") != "shenghuo-xuanming" )
                return notify_fail("你所用的并非圣火玄冥功，施展不出掌刀！\n");

	if( me->query_skill("force") < 120 )
		return notify_fail("你的先天功火候未到，无法施展掌刀！\n");

	if( me->query_skill("strike") < 120 )
		return notify_fail("掌刀需要精湛的日月神掌方能有效施展！\n");

	if( me->query("neili") <= 300 )
		return notify_fail("你的内力不够使用掌刀！\n");
	if( me->query("jingli") <= 200 )
		return notify_fail("你的精力不够使用掌刀！\n");	
	if( me->query("jing") <= 200 )
		return notify_fail("你的精不够使用掌刀！\n");


	msg = YEL"$N双掌朝天一拍，发出‘铿锵’一声刺耳的金属声，紧跟着一股凌厉的真气自$N掌力中汹涌而至！\n\n"NOR;
	message_vision(msg, me, target);

	me->set_temp("zhangdao", 1);

	skill =  ( me->query_skill("riyue-shenzhang")
	...
	me->add_temp("apply/damage",  skill/6);
		  ...
		  if( objectp(weapon) )
		{
			...
			+ (int)me->query("riyue-shenzhang")/5
			+ (int)me->query_str()
			+ (int)me->query("jiali");
			...
			if( wap > 3 * wdp ) {
				message_vision(HIY"只见$N身上的$n"+HIY+"已被一掌劈断。\n"NOR, target, weapon);
				weapon->unequip();
