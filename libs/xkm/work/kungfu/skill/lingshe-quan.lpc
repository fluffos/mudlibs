...
// kane 31/8/98
// add pfm lingshe effect by xuanyuan 11/28/2001

#include <ansi.h>
#include <combat.h>
inherit SKILL;

int lingshe(object me, object victim, object weapon, int damage);
...
mapping query_action(object me, object weapon)
{
	...
	msg_lingshe = ({
		HIG"斗然间$N挥拳打出，$n举手挡格，哪知$N的手臂犹似忽然没了骨头，顺势转弯！"NOR,
		HIY"$N拳法精妙，招数未老，右臂疾伸，忽地转弯，打向$n的肩头。！"NOR,
		YEL"这“灵蛇拳”去势极快，一招不中次招接至，倏忽之间已打到$n肩上！"NOR,
		WHT"$N手臂忽然间就如变了一根软鞭，打出后能在空中任意拐弯，$n明明见$N拳头打向左方，蓦地里转弯向右！"NOR,
        });

	if(me->query_temp("baituo/lingshe") && me->query_temp("lingshe_hit") ){
        me->add("neili", -30);
		me->delete_temp("lingshe_hit");
        return ([
        "action":msg_lingshe[random(sizeof(msg_lingshe))] ,
        "force" : 500,
        "dodge" : 100,
        "parry" : 50,
        "damage" : 200,
...
int lingshe(object me, object victim, object weapon, int damage)
{
	if(damage==RESULT_DODGE ) return 1;

	if ( damage==RESULT_PARRY ) {
		me->set_temp("lingshe_hit", 1);
