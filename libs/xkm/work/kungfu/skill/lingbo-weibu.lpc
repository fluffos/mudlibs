...
int valid_learn(object me)
{
	if ( me->query_skill("lingbo-weibu", 1) < 400 )
		return notify_fail("凌波微步只能通过研读《北冥秘籍》来提高。\n");
	return 1;
}
...
int practice_skill(object me)
{
        if ( (int)me->query("jingli") < 150 )
                return notify_fail("你的精力不够练习凌波微步。\n");
        if ( (int)me->query("neili") < 150 )
                return notify_fail("你的内力不够练习凌波微步。\n");
		me->add("neili", -50);
		me->add("jingli", -50, "精力透支过度死了");
		return 1;
}
...
mixed hit_by(object me, object victim, int damage)
{
        ...
        if ( victim->query_skill("lingbo-weibu", 1) < 150
        || victim->query_skill_mapped("force") != "beiming-shengong" 
        || victim->query_skill("beiming-shengong", 1) < 150 
