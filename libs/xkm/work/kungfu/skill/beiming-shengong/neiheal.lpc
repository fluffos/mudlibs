	...
	if( target == me ) mode = 1;
   	...
   	if ( mode == 1 ) {
		...
		if ( (int)me->query_skill("beiming-shengong", 1) < 80 ) 
			return notify_fail("你北冥神功的造诣太浅，不能化解寒梅暖阳内伤！\n");
		...
		message_vision(RED"$N盘膝坐下运动北冥神功，只见头上舞气腾腾，吐出一口殷红的瘀血，脸色显得十分苍白。\n"NOR, me);
		...
		if ( (int)me->query_skill("beiming-shengong", 1) < 100 )
			return notify_fail("你的北冥神功造诣太浅，不能为他人化解寒梅暖阳内伤！\n");
		...
		message_vision(RED"$N坐下运起北冥神功，连连拍按$n各处命门大穴，徐徐地将真气输入$n体内....\n"
		...
		if( (int)target->query_skill("beiming-shengong", 1) > 20 ) {
			...
			duration_hot2 = duration_hot1 - me->query_skill("beiming-shengong", 1) / 10;
			duration_cold1 = target->query_condition("hot_poison");
			duration_cold2 = duration_cold1 - me->query_skill("beiming-shengong", 1) / 10;
			...
			if ( target->query_condition("hot_poison") > 0 && target->query_condition("cold_poison") > 0 ) {
				tell_object(me,"你发觉"+target->name()+"无法完全吸收你北冥神功的真力，看来得分几次治疗！\n");
				if( (int)target->query_skill("beiming-shengong", 1) > 0 )
					tell_object(target,"你的北冥神功修为不够，无法与"+me->name()+"的真力融合，看来得分几次治疗！\n");
				else tell_object(target,"你内功心法与北冥神功大相径庭，无法与"+me->name()+"的真力融合，看来得分几次治疗！\n");
			}
