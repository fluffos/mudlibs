...
int valid_combine(string combo) { return combo=="tianluo-diwang" || combo=="chilian-shenzhang" ; }

int valid_learn(object me)
{
	if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
		return notify_fail("练美女拳法必须空手。\n");
	if ((int)me->query_skill("yunu-xinjing", 1) < 20 &&(int)me->query_skill("wudu-xinfa", 1) < 20)
...
{
    int i,l;
	l = (int)me->query_skill("meinu-quan", 1) / 4;
	i=me->query("per");
	i=i-20;
	if (i=0) i=2;
	if (i<0) return 0; 
...
//  if(victim->is_busy()) return 0;
	if(!random(6) && me->query_skill_mapped("parry") == "meinu-quan" && me->query_skill_mapped("force") == "yunu-xinjing" && (me->query("neili")>200) && me->query_skill("meinu-quan", 1) > 60)
      {
        if (me->query("gender") == "女性") {
	    message_vision(MAG "只见$N衣衫飞舞，长发飘飘，飘忽灵动，犹如仙女下凡，清逸无比。\n" NOR, me,victim );	
		} else {
	    message_vision(HIC "只见$N衣衫飘飘，信手挥舞，一招一式潇洒无比，犹如天上神仙。\n" NOR, me,victim );	
			}
	...
	if((me->query("neili") > 100) && (!victim->query_temp("yunu/beauty")) )
                 {
...
{
        int a = (int)me->query_skill("meinu-quan", 1)/4;
    if (!victim) return 0;
