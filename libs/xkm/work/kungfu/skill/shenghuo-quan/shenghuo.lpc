...
int perform(object me, object target)
{
        ...
        if( (lvl=(int)me->query_skill("shenghuo-quan", 1)) < 100 )
                return notify_fail("你的圣火拳修为不够。\n");

        if( (lvl=(int)me->query_skill("shenghuo-xuanming", 1)) < 100 )
                return notify_fail("你的圣火玄冥功修为不够。\n");
        ...
        message_vision(HIR "$N长吸一口气，猛然大喝一声，一拳挟带着一股热气真向$n胸口击去。\n\n" NOR, me,target);
        lvl = (int)(lvl / 5);
        me->add("neili", -lvl*2); 
        me->add("jingli", -100);  
...
*/
        ...
        ap = COMBAT_D->skill_power(me,"strike", SKILL_USAGE_ATTACK)*3;
        pp = COMBAT_D->skill_power(target, "parry", SKILL_USAGE_DEFENSE);
        dp = COMBAT_D->skill_power(target, "dodge", SKILL_USAGE_DEFENSE);
        ap = (ap+random(ap*2))/3.5;

        if( ap > dp ) {
            tell_object(target, RED"你只觉一股热气扑面而至，还没反应过来已被击中胸口。\n"NOR);
		...
		if( armor->query("id") == "armor")
                     message_vision(HIY"只见$N身上的$n"+HIY+"已被这一拳打成碎片！\n"NOR, target, armor);
		else message_vision(HIY"只见$N身上的$n"+HIY+"已被这一拳打成碎片。\n"NOR, target, armor);
