...
int xuantie(object me, object victim, object weapon, int damage);

string *xuantie_msg = ({
	CYN"$N左手抽出背负的$w，顺手挥出，内力激荡的响声又沉又闷，便如木棍击打败革。"NOR,
	CYN"$N重剑剑尖抖处，已将$n黏住，当下$w不向上扬，反手下压，内力激荡得尘土飞扬"NOR,
	CYN"$N持剑不动，内劲传到剑上，只听得噗的一声闷响击向$n的$l"NOR,
	CYN"$N$w伸出，左击一剑，右击一剑，欲意将$n的双臂一齐折"NOR,
	CYN"$N全身劲力都贯到了臂之上，剑尖颤动，当当当三剑挥剑下击"NOR,
	CYN"$N迳不招架，$w当胸疾刺，去势之急，剑头已距$n$l不到半尺"NOR,
	CYN"$n挥剑向$n的$l斩落$w，这$w一剑斩下来力道奇猛，力过千钧"NOR,
	CYN"$N提起$w，左臂前送，剑上威力雄浑，眼见这一剑$n万难招架得住"
	CYN"$n提剑凝立，目光如炬，突然举起$w向$n当头劈下，这一剑不单力道沉雄绝伦，其中更蕴蓄着无限伤心悲愤"NOR,
	CYN"$N左臂向外挥出，$w加上$N浑厚内力，$n便如腾云驾雾般飞上半空"NOR,
	CYN"$n只觉一股疾风吹来，$N身随风至，挺起手中$w向$N刺去"NOR,
	CYN"$N剑尖直刺$n前胸，这剑虽不锋利，力道却是奇大，只压得$n肋骨向内剧缩，只能呼气出外，不能吸进半口气来"NOR,
});
...
mapping query_action(object me, object weapon)
{
	...
	if (level > 350 && me->query("max_neili") > 6000 && me->query("neili") > 1000 && me->query("jiali") > 99 &&
		weapon->query("id") == "xuantie jian" && me->query("gender")== "男性" )
	{
		return ([
		"action": xuantie_msg[random(sizeof(xuantie_msg))],
		"force" : 500+random(me->query_skill("force")/3),
		"dodge" : -50,
        "parry" : 50,
		"damage": 220+random(me->query("neili")/30),
		"damage_type" : random(2)?"刺伤":"割伤",
		"post_action" : (: xuantie :) ]);
	}
	else if (random(level) > 350 && random(me->query("neili")) > 1000 )
	return ([
		"action":HIW"$N急退急趋，纵上连刺三剑，$w"+HIW"飘忽来去，东刺西击，这一路玉女剑法更见使得英气爽朗，顾盼生姿！"NOR,
        "force" : 450 + me->query_skill("yunu-jianfa"),
		"dodge" : 40,
        "parry" : 10,
        "damage": 160,
		"post_action": (: ke_qz :),	
	...
	if (me->query("gender")== "男性") {
		return ([      
		"action":HIG"$N剑走轻灵，招断意连，绵绵不绝，当真是闲雅潇洒，翰逸神飞，大有晋人乌衣子弟裙屐风流之态！"NOR,
        "force" : 450 + me->query_skill("yunu-jianfa"),
		"dodge" : 40,
        "parry" : 10,
        "damage": 140,
...
int ke_qz(object me, object victim, object weapon, int damage)
{
	...
	skill = me->query_skill("yunu-jianfa");
...
|| (victim->query_skill_mapped("parry")== "yuanyang-tui")
