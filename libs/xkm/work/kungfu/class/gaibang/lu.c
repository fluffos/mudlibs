...
string ask_job()
{
	...
	else if ( ob->query_condition("gb_job") > 5 ) 
	...
	ob->apply_condition("gb_job", 5+random(5));
...
int check_job(object ob)
{
	...
	if ( ob->query("gb/job_status") == 2 ) {
		...
		ob->add("combat_exp", 100+random(130));
		ob->add("potential", 50+random(80));

		if ( (string)ob->query("gb/job_kill") ) {
			message_vision("鲁有脚对$N道：哈哈哈！江湖上传闻你将"+ob->query("gb/job_kill")+"这个叛贼除去，实在是太好了！\n", ob);

			ob->add("combat_exp", 250+random(180));
			ob->add("potential", 60+random(80));
