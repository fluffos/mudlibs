...
{
	seteuid(getuid());
	set_name(HIR"假日老人"NOR, ({ "jiar laoren","laoren"}) );
	set("gender", "男性" );
	set("age", 63);
   ...
   set("env/yield","yes");
	...
	set("chat_msg", ({
		"假日老人笑呵呵说道: 假日快乐！快乐！~~\n",
		"假日老人快活地唱道: 叮叮铛，叮叮铛，铃儿响叮铛~~\n",
		"假日老人笑呵呵说道: 想要礼物吗？我就是神派来送假日礼物的。 \n",
		"假日老人笑道: 只要是乖孩子，就有假日礼物。 \n",
		(: random_move :),
		(: greeting :),
	}) );
...
/*	if (random(20) == 1) {
	...
	ppl->apply_condition( "santa",20+random(20) );
	CHANNEL_D->do_channel(santa, "rumor",
               ppl->query("name")+"得到"HIR+ob->query("name")+HIM"啦！\n");
...
void greeting()
{
	...
	if( strsrch(ctime(time()), "Sun") == -1 )
	{
		message_vision(HIY"$N"+HIY"凄婉地说：假日节结束了，我也该回天堂了.\n"NOR,me);
		command("goodbye");
		message_vision(HIY"$N"+HIY"乘坐风车凌空飞去，转眼就不见了.\n"NOR,me);
