...
string ask_long()
{
	...
	if (query("shunvjian_count") > 2)
	return "你来晚了，淑女剑我已找人带给姑姑了。";
	
	 
 	obs = filter_array(children("/kungfu/class/gumu/obj/shunvjian.c"), (: clonep :));
 	if (sizeof(obs) > 1)
	return "你来晚了耶，淑女剑我已找人带给姑姑了。";

	ob=new("/kungfu/class/gumu/obj/shunvjian");
	ob->move(this_player());
	add("shunvjian_count", -1);
	command ("rumor "+this_player()->query("name")+"得到一把淑女剑。");
	return "你要是见到她，把这把淑女剑给我姑姑，说我好想她。";
}
