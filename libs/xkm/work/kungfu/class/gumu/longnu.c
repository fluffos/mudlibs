...
void attempt_apprentice(object ob)
{
	...
	if(ob->query("family/master_name") == "李莫愁") {
		command("say 你是我师姐"+ob->query("family/master_name")+"的徒弟，我不能收你。");
		return;
 		}       
       if( ob->query("gender") != "女性" ) {
		command("say 我古墓只收女弟子，我不能收你。");
		return;
	}
...
string ask_job()
{
          ...
          job_stat = ob->query_temp("gumu/caimi");
          ...
          command("nod");
          command("say 你去果园里采些蜂蜜回来，然后去药房做成蜂王浆。");
          ob->set_temp("gumu/caimi",1);
}
