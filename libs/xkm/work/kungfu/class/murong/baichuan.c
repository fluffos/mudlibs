...
#define MR_JOB_MOVED ("/d/murong/mrjob/mrnpc/mr_inherit/mrjobd.c")
int auto_perform();
void create()
{
	...
	set("inquiry", ([
		...
		"公孙乾" : "公孙乾是我二弟。\n",
		"包不同" : "包不同是我三弟，他总是喜欢说“非也，非也”。\n",
		"风波恶" : "风波恶是我四弟，他就是喜欢跟别人打架。\n",
		"慕容复" : "我们家公子忙于大事，没时间接待你，你还是请回吧！",
		"姑苏慕容" : "嘿嘿，你是不是觊觎我家公子的“以彼之道，还施彼身”的功夫了？ \n",
...
int ask_job()
{
	...
	object me = this_player();
        ...
        {
 			command("say "+RANK_D->query_respect(me)+"非我姑苏慕容家臣，这庄中之事岂敢劳烦。");
        	return 1;
        } 
...
int get_job(string arg, object me,mapping *jobs)
{
	...
	msg = JOB_G->msg(jobs[i]["content"]);
        switch(i)
        {
               case 0:
                      if(objectp(obj = present("shui hu", me))&&
                         obj->query("mrjob") )
                      break;	
		      obj=new("/d/murong/mrjob/mrnpc/mr_obj/mr_shuihu.c");
		      obj->move(me);
		      command("say 这个水壶你收好。");
                      break;
               case 1:
                      ...
                      me->set_temp("Mrjob_patrol","start");
               	      obj=new("/d/murong/mrjob/mrnpc/mr_obj/mr_lingpai.c");
		      obj->move(me);
		      command("say 这块令牌你拿好。");
		      break;
               case 2:
                      if(!objectp(obj = present("shijie zhang", me))||
                         !obj->query("mrjob") )
                     {
                      obj=new("/d/murong/mrjob/mrnpc/mr_obj/mr_shuxin.c");
		      obj->move(me);
        ...
        if((amu==2||amu==3))
        {
        	...
        	{
        		map=new("/d/murong/mrjob/mrnpc/mr_obj/mr_map.c");
        		map->move(me);
        	}
        	if(!query("jobmove"))
        	{
