...
inherit F_SKILL;
inherit F_TEAM;
inherit F_TRIGGER;   // add for server-side trigger

#define SCHEME_CMD      "/cmds/usr/scheme"   //增加实现离线练功
...
// xuy, die only if falling unconcious (sleeping will have living() return 0 as well)
	...
	if (my["doing"] == "scheme")   //增加实现离线练功
                // executing schedule now
                SCHEME_CMD->execute_schedule(this_object());
