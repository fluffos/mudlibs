...
int check_student(object ob)
{
	...
	if ( ob->query("shen") < -100 && ob->query("family/master_id") == "yue buqun" ) {
		...
		return 1;
          ...
          if (interactive(ob=this_player()) && ob->query("family/family_name") == "华山派" && (!present("xiayi jian", this_player()))){ 
            ob1=new("/d/huashan/obj/hs_weapon.c");
            ob1->move(this_player());
            command("say 身为华山弟子要以“侠义”二字为重。拿好这柄剑好好勉励自己！");
          }
