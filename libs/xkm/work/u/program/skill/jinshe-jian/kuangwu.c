...
int perform(object me, object target)
{
        ...
        z = me->query_skill("youshen-zhang", 1)/8;
        ...
        if (me->query_skill_prepared("strike") != "youshen-zhang"
        ...
        if( (int)me->query("neili") < 500 )
         ...
         msg = HIY"\n$N绕着$n快速奔走，猛然剑掌齐出，齐齐指向$n！\n\n"NOR;

        me->add_temp("apply/strength", z);
        me->add_temp("apply/attack", j); 

        if(weapon->query("id") == "jinshe sword"){
          me->set_temp("jsj", 1);
        ...
        if(me->query_temp("jsj"))
          message_vision(HIY" $N看都不看$n一眼，剑招迅猛地递出！\n"NOR,me);
        ...
        me->add("jingli", -10);
        if(me->query_temp("jsj"))
          message_vision(HIY" 冷哼一声，第二剑悄无声息的攻出！\n"NOR,me);
        ...
        me->start_busy(1);target->apply_condition("man_poison",        random(me->query_skill("jinshe-jianfa",1)/10) + 1 +
		target->query_condition("man_poison"));  
        target->start_busy(1);
        return 1;
}
