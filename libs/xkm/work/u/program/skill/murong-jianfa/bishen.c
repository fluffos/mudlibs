...
int perform(object me, object target)
{
        ...
        if((string)fam["family_name"] == (string)perform[i]["menpai"])
        {
                ...
                switch( perform[i]["perform_type"])
                        {
                        case "1":   {perform_case_1(me,target);break;}
                        case "2":   {perform_case_2(me,target);break;}
                        case "3":   {perform_case_3(me,target);break;}
                        case "4":   {perform_case_4(me,target);break;}
                        case "5":   {perform_case_5(me,target);break;}
                        }
        if( (int)me->query("neili") > 500 ) {me->add("neili", -500); }
...
int perform_case_1(object me,object target)
{
        ...
        me->add_temp("apply/dexerity",  count_s*3/4 );
        me->add_temp("apply/strength",  count_d*3/4 );
        COMBAT_D->do_attack(me, target, me->query_temp("weapon"));
        if ( target_weapon && use_weapon) use_weapon->unequip(); 
        COMBAT_D->do_attack(me, target, me->query_temp("weapon"));
        if ( !target_weapon && use_weapon) use_weapon->equip(); 
        COMBAT_D->do_attack(me, target, me->query_temp("weapon"));
        me->add_temp("apply/dexerity",  -count_s*3/4 );
        me->add_temp("apply/strength",  -count_d*3/4 );
        return 1;
...
int perform_case_5(object me,object target)
{
        ...
        count_d = (int)me->query_dex();
        me->add_temp("apply/dexerity",  count_s*2 );
        me->add_temp("apply/strength",  count_d*2 );
        if ( !target_weapon && use_weapon) use_weapon->unequip(); 
        COMBAT_D->do_attack(me, target, me->query_temp("weapon"));
        me->add_temp("apply/strength", -count_s*2 );
        me->add_temp("apply/dexerity", -count_d*2);
        return 1;
}
