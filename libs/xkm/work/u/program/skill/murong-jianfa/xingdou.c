...
int perform(object me,object target)
{
    ...
    if( !target || !me->is_fighting(target) || !living(target)
        || environment(target)!=environment(me))
                return notify_fail("满天星斗只能对战斗中的对手使用。\n");
if( me->query_temp("canshang") )
                return notify_fail("你已经在使用参商指法了！\n");

    if(me->query_skill_mapped("force") != "shenyuan-gong")
                return notify_fail("你的内功不是神元功，无法使用满天星斗！\n");
    if (!objectp(weapon = me->query_temp("weapon")) || weapon->query("skill_type") != "sword"
        || me->query_skill_mapped("sword") != "murong-jianfa")
                return notify_fail("你手里没有剑，无法使用满天星斗！\n");
                
    if(me->query_skill_mapped("parry") != "douzhuan-xingyi")
                return notify_fail("你的招架功夫不对，无法使用满天星斗！\n");
    if((int)me->query("max_neili") < 3000 )
                return notify_fail("你的内力修为不够，不能使用满天星斗！\n");
    if((int)me->query("neili") < 1200 )
                return notify_fail("你现在真气不足，不能使用满天星斗！\n");
    if((int)me->query("jingli") < 500 )
                return notify_fail("你现在精力不足，不能使用满天星斗！\n");
    if((int)me->query_temp("lianhuan"))
                return notify_fail("你正在使用七剑连环指！\n");
    if((int)me->query_skill("sword", 1) < 230 )
                return notify_fail("你的基本剑法不够娴熟，不能在剑招中使用满天星斗。\n");
    if( (int)me->query_skill("xingyi-zhang", 1) < 250 )
                return notify_fail("你的星移掌不够娴熟，无法施展出满天星斗。\n");
    if( (int)me->query_skill("canhe-zhi", 1) < 250 )
                return notify_fail("你的参合指不够娴熟，无法施展出满天星斗。\n");
    if( (int)me->query_skill("douzhuan-xingyi", 1) < 200 )
                return notify_fail("你的斗转星移不够娴熟，无法施展出满天星斗。\n");
    if( (int)me->query_skill("shenyuan-gong", 1) < 200 )
                return notify_fail("你的内功还未练成，不能使用满天星斗！\n");
    if( (int)me->query_skill("murong-jianfa", 1) < 250 )
                return notify_fail("你的剑法还未练成，不能使用满天星斗！\n");
       if(me->query_skill_mapped("finger") != "canhe-zhi" ||
           me->query_skill_prepared("finger") != "canhe-zhi")
                return notify_fail("你没有用指，无法使用「满天星斗」！\n");  
         if(me->query_skill_mapped("strike") != "xingyi-zhang" )
          
                return notify_fail("你没有用掌，无法使用「满天星斗」！\n");       
	me->set_temp("mr/xingdou",1);
    message_vision(HIM"\n$N想起复国大业不禁激起壮志豪情长啸一声使出慕容绝技满天星斗！\n"NOR, me,target);
	me->add("neili", -100);
...
"幽门穴","章门穴","大横穴","紫宫穴","冷渊穴","天井穴","极泉穴","清灵穴","至阳穴",
	...
	if( !objectp(target) || !me->is_fighting(target) || (now >= 6) || (me->query("neili")<200) ) {
		me->set_temp("mr/xingdou",0);
		me->start_perform(5,"满天星斗");
        	weapon->wield();me->prepare_skill("strike");
	...
	switch( now ) {   
       case 0:
		message_vision(HIR"\n$N微笑不语，伸指向剑尖一弹，"NOR+weaname+HIR"抖起五朵洁白的剑花，有急有缓，分别刺向$n的"NOR+name+HIR"。\n"NOR, me,target);
        break;
	case 2:
		...
		message_vision(MAG"\n$N收剑还鞘,双掌一搓，在双掌之间形成一个耀眼的「"HIW"气旋"MAG"」！\n
"HIY"$N移形换步，双掌收在胸前，闪电般将「"HIR"日旋"HIY"」击向$n！\n"NOR, me,target);
        break;
	case 3:
		message_vision(HIC"\n$N空中一个回转，携「"HIG"月旋"HIC"」, 双掌按向$n的丹田！\n"NOR, me,target);
        break;
	case 4:
         me->prepare_skill("strike");
         me->prepare_skill("finger","canhe-zhi");
		message_vision(HIG"\n$N化掌为指，指风轻飘飘地点向$n周身大穴！\n"NOR, me,target);
        break;
	case 5:
		message_vision(HIB"\n$N双手拇指一阵虚点，两道真气如剑一般刺向$n！\n"NOR, me,target);
        break;
	}
	...
	call_out("xingdou",2,me,target,weapon,now);
}
