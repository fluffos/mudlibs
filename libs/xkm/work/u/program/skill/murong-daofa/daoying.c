...
{
     int d=me->query_skill("murong-shenfa",1)/2;
        int j=me->query_skill("murong-daofa", 1)/4;
     int i=me->query_skill("xingyi-zhang", 1)/3;
        ...
        message_vision(HIR"\n$N大吼一声使出慕容绝学「刀影掌」，掌随刀走，刀光中夹杂着阵阵掌风一起奔向$n！\n"NOR, me,target);
       me->add("neili", -250); 
     me->add("jingli", -100);
  ...
  me->add_temp("apply/attack", (i+j)/2); 
     ...
     me->add_temp("apply/strike", -i/2); 
      weapon->unequip(); 
        COMBAT_D->do_attack(me, target, me->query_temp("weapon"),1);weapon->wield(); 
  me->add_temp("apply/attack", -((i+j)/2)); 
   me->add_temp("apply/dodge", -d); 
...
me->add("neili", -100);    
        ...
        me->start_busy(2);me->start_perform( 4, "「刀影掌」");
