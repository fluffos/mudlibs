...
*/
       ...
       me->start_busy(2+random(1));
        ...
        {
             if (me->query_skill("strike")>200 && me->query_skill("force")>180) {
                 call_out("fail_hit",1,me,target);
               }
          else
                message_vision(HIY "$P感到$p的内力稳固如山，第二招不便攻出，忙撤掌后退。\n" NOR, me, target);
  ...
  int fail_hit(object me, object target){
    int damage;
        ...
        if (present("huo yan", environment(me)) && me->is_fighting()){

           me->add("neili", -100);
                   me->add("jingli", -50);

          message_vision(HIY "\n\n$N潜运内力撩起火焰，激起的火焰向$n激射而去......。\n"NOR, me, target);
                  if((me->query("combat_exp")/3*2)>random(target->query("combat_exp"))){
                          message_vision(HIR "\n$n正要乘机进攻，忽见团团火焰逼向自己，$n躲闪不及，被火焰烧个正着。\n"NOR,me,target);
                           ...
                           target->add("eff_qi", -random(damage)/2);
                   } else {
                  if((me->query_neili()/5*3)<(target->query_neili())){
