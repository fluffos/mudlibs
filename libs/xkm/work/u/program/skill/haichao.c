...
int perform(object me, object target)
{
        ...
        if( me->query("gender") =="男性"){
           ...
           i = me->query_skill("xuantie-jianfa", 1)/3;
           j = me->query_skill("sword")/3;
        }
        ...
        if( me->query_skill_mapped("parry") != "xuantie-jianfa" && userp(me))
                  return notify_fail("你并没有完全使用玄铁剑法，无法使用「海潮汹涌」！\n");
         ...
         || !living(target)){
            ...
            if((int)me->query_skill("xuantie-jianfa", 1) > 150
             ...
             && me->is_fighting(target)){
               message_vision(HIB"\n$N面色凝重，趁势将手中的"+weapon->query("name")+HIB"从轻而响，从响而轻，忽寂然无声，忽轰轰之声，向$n席卷而去。\n"NOR,me,target);
               me->add_temp("apply/strength", i);
               me->add_temp("apply/attack", j);
               me->set_temp("xtjf_pfm", 1);
               COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 1);
               if (userp(me)) me->start_busy(1);
               if (present(target,environment(me)) && me->is_fighting(target))
               COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 1);
               me->add_temp("apply/strength", -i);
               me->add_temp("apply/attack", -j);
