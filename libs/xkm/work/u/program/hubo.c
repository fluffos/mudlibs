...
int perform(object me, object target)
{
	...
	if( !objectp(weapon = me->query_temp("weapon"))
		...
		return notify_fail("你双手正在使用不同的兵器，不能再进行双剑和璧左右互搏！\n");
	...
	message_vision(HIC"\n只见$N双剑一交，左右手玉女剑法与全真剑法配合得天衣无缝，攻守之势猛然大增！\n" NOR, me);

	me->add_temp("apply/attack", skill/6);
	me->add_temp("apply/damage", skill/4);
	me->add_temp("apply/dodge",  skill/2);
	me->add_temp("apply/parry",  skill/2);
	me->add_temp("apply/speed",  skill/6);

	me->set_temp("hubo", 1);

	me->add("jingli",skill*100  );
	...
	me->add("jingli", -100);
	me->add("jing", -100);
	me->add("neili", -(int)me->query_skill("yunu-xinjing",1) - 300);
	me->start_busy(1+random(2));

	me->start_call_out( (: call_other, __FILE__, "remove_effect", me, skill/2,weapon2 :), 10);
...
private int remove_effect(object me, int amount,object weapon)
{
	...
	me->add_temp("apply/speed", -amount/3);
	me->delete_temp("hubo");
	message_vision(HIY "\n$N吸了一口气，收回左右互搏绝技！\n\n" NOR, me,weapon);
