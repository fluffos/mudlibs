...
int main(object me, string arg)
{
	object ob;
	mapping my;
	string line, str, skill_type, swornfriend;
	object weapon;
...
//added by Seashore for display info of sworn friend 2002/5/14
	swornfriend = " ";
	if( sizeof(my["swornfriend"])){
		for (i=0 ; i<sizeof(my["swornfriend"]) ; i++ )
		swornfriend += sprintf(my["swornfriend"][i]["name"])+"  ";			
		line += sprintf(" ÄãµÄ"+HIR"½áÒå½ğÀ¼"NOR+"ÓĞ£º" + swornfriend + "\n\n");
	}
//add money   2002/10/1
if (ob->query("balance") <= 0) 
  line += sprintf( YEL "©§" HIW "¡¾Ç®×¯»ıĞî¡¿" HIY "Ã»ÓĞ»ıĞî                                          " NOR+YEL "©§\n" NOR ); 
else 
{
      line += sprintf ( YEL "©§"  HIW "¡¾Ç®×¯»ıĞî¡¿" NOR ); 
      line += sprintf ("%-50.50s", 
