...
int exert(object me, object target)
{
        ...
        if( (int)me->query("neili") < 1000  ) 
                return notify_fail("你的内力不足使用化功大法护住周身大穴。\n");
...
/*
        if( (int)me->query("canhua") != 1)
                return notify_fail("你不得化功大法精髓要领，不会使用化功绝技！\n");

		if ( me->query("gender") == "无性" )
				return notify_fail("你无根无性，阴阳不调，难以运用化功大法护住周身大穴。\n");
*/
...
void remove_effect(object me, int skill)
{
        ...
        me->delete_temp("apply/parry",  skill/3);                       
        me->delete_temp("apply/dodge",  skill/3);
