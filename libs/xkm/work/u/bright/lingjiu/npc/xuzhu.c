...
//              "小院" : (: ask_me, "lingjiu" :),
                ...
                "易经" : (: ask_jing:), //bright add
                "jing" : (: ask_jing:), //bright add
        ]));
        set("ling_count", 1);
        set("lit_count", 3);
...
string ask_jing()
{
        ...
        if (!(fam = this_player()->query("family")) ||fam["family_name"]!="灵鹫宫")
                   return RANK_D->query_respect(this_player()) +
           "与本派素无来往，不知此话从何谈起？";
        if (query("lit_count") < 1)
                   return "你来晚了，都被你师姐们拿走了。";
        if ( me->query_temp("askjing/done") )
                   return "不是刚给你了吗？怎么又来要了？";
        add("lit_count", -1);
        ob = new("/d/lingjiu/obj/lit_book");  
        ob->move(this_player());
        me->set_temp("askjing/done",1);   
        write(HIW"虚竹给你一本易经文学。\n"NOR);
        return "好吧，拿去好好研习吧！";
}
