...
// 修改 by naihe 2002-10-06 晚7时，系统将会自动开启程序。
	...
	if(now=="stop" && wiz_id=="系统" && biaoji=="none" && !wizardp(this_player()))
	{
		...
		//temp_time=wait_time /60;//正式用
		temp_time=wait_time /6;//测试用
		temp_time_ms1="彩票销售";
		temp_time_ms2="开奖";
...
static int random_tmp=0;
int do_mai(mixed arg)
{
	object me,qian,caipiao;
	int nb1,nb2,nb3,nb4,random_times=3;
	string temp,random_yes;
	...
	else if((sscanf(arg, "%s %d",random_yes,random_times)==2&&random_yes=="random")||(string)arg=="random")
	{
		if(random_tmp=0)random_tmp=random_times;
			random_number_1();
			random_number_2();
			random_number_3();
			random_number_4();
	...
	else if( (string)arg!="random"&&sscanf(arg, "%d %d %d %d", nb1,nb2,nb3,nb4)!=4 )	
		return notify_fail("你想买什么号码（"+max_number+" 选 4）或　随机（random）？\n");
...
// this_game_gxd,本游戏推广期间，奖品里有额外的“贡献度”奖励。
