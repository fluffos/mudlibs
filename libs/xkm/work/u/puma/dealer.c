...
//added the function "query_inventory()",puma@XKM,12/14/2002

#include <dbase.h>

/////////////////////////////////////////////////////////////////////////
#define DEALER_D "/adm/daemons/dealer_d"
void query_inventory()
{       
        int i,j,yes;
        string *goods,*items_dealer_file;
        object item,*inv;
        ...
        items_dealer_file=DEALER_D->query_items_dealer_file();
        if (items_dealer_file!=({})&& !arrayp(goods = query("vendor_goods")))
        {
                
                for(j=0;j<sizeof(items_dealer_file);j++)
                {
                        yes=0;
                        inv = all_inventory(this_object());
                        item=new(items_dealer_file[j]);
                        for (i = 0; i < sizeof(inv); i++)
                        if (inv[i]->query("name") == item->query("name"))
                                {
...
{
        object ob, *inv;
        int value, i,quantity;   
        string goods, ob_file;
        ...
        else
        {
                ...
                //////////////////
                inv = all_inventory(this_object());
                for (i = 0,quantity=0; i < sizeof(inv); i++)
                {
                        quantity+=inv[i]->query("quantity");
                        if (quantity>=30)
                                return notify_fail("对不起，本店库房已满，暂停收购。\n");
                }
...
int do_list(string arg)
{
        ...
        else  if (arrayp(goods = query("vendor_goods"))){
                printf("你只可以向%s购买下列东西：\n", query("name"));
                for (i = 0; i < sizeof(goods); i++)
                        printf("%30-s：%s\n", goods[i]->short(),
                        MONEY_D->price_str(goods[i]->query("value")));
        return 1;
...
int do_buy(string arg)
{
        ...
        string goods, ob_file, file,*all_goods;
        object ob, obj;
...
//              return notify_fail("你是个穷叫化，买什麽东西！\n");
        ...
        if (arrayp(all_goods = query("vendor_goods")))
                if( (ob_file = is_vendor_good(goods)) == "" )
                        return notify_fail("看清楚点，我们这里不卖这种货。\n");
        ...
        default:
                ...
                if( ob->query("base_unit") )
                {
                        if( ob->query_amount() > amount )
                        {
                                ...
                                //////////////////////////////////////
                                if( (ob_file = is_vendor_good(goods)) == "" )
                                DEALER_D->buy_items_dealer(ob,amount);
                                //////////////////////////////////////
                        }
                        else
                        {
                                ...
                                //////////////////////////////
                                if( (ob_file = is_vendor_good(goods)) == "" )
                                DEALER_D->buy_items_dealer(ob,1);
                                //////////////////////////////
