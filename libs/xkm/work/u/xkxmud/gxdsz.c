...
string part2= HIC"＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊　　\n" +
	...
	"  先天容貌(rongmao)	300	点贡献度	\n" +
	"  先天福缘(fuyuan)	350	点贡献度	\n" +
	"  先天臂力(bili)	800	点贡献度	\n" +
	"  先天悟性(wuxing)	800	点贡献度	\n" +
	"  先天根骨(gengu)	1000	点贡献度	\n" +
        "  先天身法(shenfa) 1000	点贡献度        \n" +
        HIR"  其中天赋只能兑换一次！        \n"HIC +
...
void create()
{
	...
	set("title",HIG"贡献度兑换"NOR);
	set("gender","男性");
	set("age",15);
	set("attitude", "friendly");
	set("long",HIR"他是梦巫师为了奖励对梦有贡献的玩家而提拔的兑换特使，您可以用（ask teshi about change）来得知具体兑换细节！\n"NOR);
	set("shen_type", 2);
...
int do_retitle()
{
	...
	int gxd = 0 ;
	gxd = me->query("meng/pts");
	if( gxd < 5 )	return temp2();
	else
	{
		me->add("meng/pts",-500);
		me->set("title",me->query("title_bak"));
		//write(HIG"你成功的恢复了头衔" + me->query("title_bak") + HIG"，恭喜你，继续努力吧！\n"NOR);
		shout( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的恢复了头衔" + me->query("title_bak") + HIG"，恭喜你，继续努力吧！\n" NOR ); 
          	write( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的恢复了头衔" + me->query("title_bak") + HIG"，恭喜你，继续努力吧！\n" NOR );             
		return 1;
	}
}
...
int do_change(string arg)
{
	...
	else if( arg == "容貌" || arg == "rongmao" )
	{
		...
		else
		{
			...
			me->add("meng/per",1);
			me->add("per",1);
			shout( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天容貌！继续努力吧！\n" NOR ); 
          		write( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天容貌！继续努力吧！\n" NOR );             
			return 1;
		}
	}
	else if( arg == "福缘" || arg == "fuyuan" )
	{
		if( me->query("meng/kar") !=0 )
			return temp3();
		else if( gxd < 350 )
			return temp2();
		...
		else
		{
			...
			me->add("meng/kar",1);
			me->add("kar",1);
			shout( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天福缘！继续努力吧！\n" NOR ); 
          		write( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天福缘！继续努力吧！\n" NOR );             
			return 1;
		}
	}
	else if( arg == "臂力" || arg == "bili" )
	{
		if( me->query("meng/str") !=0 )
			return temp3();
		else if( gxd < 800 )
			return temp2();
		else
		{
			...
			me->add("meng/str",1);
			me->add("str",1);
			shout( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天臂力！继续努力吧！\n" NOR ); 
          		write( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天臂力！继续努力吧！\n" NOR );             	return 1;
		}
	}
	else if( arg == "悟性" || arg == "wuxing" )
	{
		if( me->query("meng/int") !=0 )
			return temp3();
		else if( gxd < 800 )
			return temp2();
		else
		{
			...
			me->add("meng/int",1);
			me->add("int",1);
			shout( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天悟性！继续努力吧！\n" NOR ); 
          		write( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天悟性！继续努力吧！\n" NOR );             
			return 1;
		}
	}
	else if( arg == "根骨" || arg == "gengu" )
	{
		if( me->query("meng/con") !=0 )
			return temp3();
		else if( gxd < 1000 )
			return temp2();
		else
		{
			me->add("meng/pts",-1000);
			me->add("meng/con",1);
			me->add("con",1);
			shout( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天根骨！继续努力吧！\n" NOR ); 
          		write( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天根骨！继续努力吧！\n" NOR );             
			return 1;
		}
	}
	else if( arg == "身法" || arg == "shenfa" )
	{
		if( me->query("meng/dex") !=0 )
			return temp3();
		else if( gxd < 1000 )
			return temp2();
		else
		{
			...
			me->add("meng/dex",1);
			me->add("dex",1);
			shout( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天身法！继续努力吧！\n" NOR ); 
          		write( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换了一点先天身法！继续努力吧！\n" NOR );             
			return 1;
		}
	}
	else if( arg == "九阴" || arg == "9yin" || arg == "jiuyin" || arg == "九阴权限" )
	{
		...
		{
			me->add("meng/pts",-1800);
			me->add("meng/9yin",1);
			me->set("9yin","granted");
			shout( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换到了学习九阴真经的权限！继续努力吧！\n" NOR ); 
          		write( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的兑换到了学习九阴真经的权限！继续努力吧！\n" NOR );             
			return 1;
		}
	}
	else if( sscanf(arg, "title %s", str)==1 || sscanf(arg, "头衔 %s", str)==1 )
	{	
		sum =(me->query("meng/title")*500);
		if( gxd < sum )
			return temp2();
		else
		{ 
			...
			shout( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的换取了头衔" + str + HIG"，恭喜你，继续努力吧\n" NOR ); 
          		write( HIM "【谣言】某人：听说"HIC + me->query( "name" ) + HIM"成功的换取了头衔" + str + HIG"，恭喜你，继续努力吧\n" NOR );             
			return 1;
		}
	}
