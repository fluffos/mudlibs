...
write(HIY"             　　请大家光临我们的主页xkm.my163.com！\n"NOR);
write(HIG"             　　请大家用域名xkmbbs.my163.com登陆梦bbs！\n"NOR);
write(HIR"             　　请大家用域名meng.d2g.com 5555登陆梦开发站mud！\n"NOR);
...
private void select_gift(string yn, object ob)
{
        ...
        if (i)
        {
		write("\n请输入您想要的数值,格式为(膂力 悟性 根骨 身法)四项值总和为90：\n");
...
private void set_giftnew(string arg, object ob, mapping my)
{
	...
	{
		write("\n输入错误，格式为(膂力 悟性 根骨 身法)四项值总和为90：");
		input_to("set_giftnew",ob);
		return;
	}
	else if(str1>30||int1>30||con1>30||dex1>30||(str1+int1+con1+dex1)!=90)
	{
		write("\n各单项值不能大于 30 ，总和为 90");
...
varargs void enter_world(object ob, object user, int silent)
{
	...
	a = user->query("str");
        ...
        d = user->query("con");
        num = a + b + c + d;
	write(sprintf( HIR "\n\n  您的天赋分别是：臂力：%3d 悟性：%3d 身法：%3d 根骨：%3d 总和：%3d\n\n如果您的四项基本天赋和大于90，我们会在2002-10-20日统一予以调整！\n\n\n"NOR,a,b,c,d,num
				) );        
        if (num>1000 && !wizardp(user))
        {
...
user->set("channels", ({ "chat","jiaoyi","xkx", "rumor", "gchat" ,"mengpai","wiz","sys"}) );
write(HIY"\n");
cat("/doc/gonggao/wiz");
...
{
user->set("channels", ({ "chat","jiaoyi","xkx", "rumor", "gchat" ,"mengpai"}) );
write(HIG"\n");
