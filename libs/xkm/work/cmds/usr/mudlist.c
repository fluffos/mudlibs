...
int main(object me, string arg)
{
        ...
        mixed *muds;
        string output;
        int loop, size, usernum=0, total=0, uptime,j;
        object *list;
        ...
        for(loop = 0, size = sizeof(muds); loop<size; loop++) {
                if(muds[loop] == lower_case(Mud_name())) {
                        ...
                        list = filter_array(children(USER_OB),(: userp :));                        
                        j = sizeof(list);
		        while( j-- )
		        {
                ...
                output += sprintf(((mud_list[muds[loop]]["CNAME"]==INTERMUD_MUD_CNAME)?BBLU+HIY:NOR)+"%-16s%-16s%5s\t%22s \t%10s位\n"NOR, mud_list[muds[loop]]["CNAME"]+"("+mud_list[muds[loop]]["NAME"]+")", 
