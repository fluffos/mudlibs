...
#define O_MURONG         0x4000;
#define O_OTHER          0x8000;
#define O_LINGXIAO      0x10000;
...
mixed main(object me, string arg, int remote)
{
	string str, *option, *sites;
		...
		case "-murong":  family |= O_MURONG;	break;
		...
		case "-lingxiao":family |= O_LINGXIAO;  break;
		default:
			if( remote ) break;
		    if( option[i][0]=='@' ) {
	...
	ppl_cnt = 0;
		...
		while( j-- ) {
		 ...
		 if(list[j]->query_temp("netdead")&&!list[j]->query("doing") )continue;	 
...
int filter_user(object user, int family)
{
	string fam = user->query("family/family_name");	
        switch (fam) {
		 ...
		 case "姑苏慕容": return family & O_MURONG;  break;
		 ...
		 case "凌霄城":	  return family & O_LINGXIAO;break;
...
-menpai 只列出此门派的玩家。menpai 可以是以下任意一个：
  shaolin, wudang, gaibang, murong, emei, huashan, dali, taohua, quanzhen,
  shenlong, xingxiu, lingjiu, xueshan, xuedao, baituo, gumu, other.
@site 显示其他「玄剑录」站点的信息。
