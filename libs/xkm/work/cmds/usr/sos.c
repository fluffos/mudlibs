    ...
    for( i = 0; i<sizeof(notes); i++){
        str = sprintf("%d) %-50s %10s %s %s \n",
                           i+1, notes[i]["title"], notes[i]["solve"]==""?"No solved":"By "+notes[i]["solve"],
                                               notes[i]["re"]==""?"":HIM"回复"NOR,
                          CHINESE_D->chinese_time(6,ctime(notes[i]["time"])));
        s = s + str;
    }
    me->start_more(s); 
    write(    "==------------------------------------------------------------------==\n");
    write("指令:  sos sos list sos pack <删除有标志的> solve <标志> re 回复\n");                  
}
...
void do_read(object me, int i) 
{
    ...
    s = sprintf("%d) %-50s %10s %s\n"
              "==----------------------------------------------------------wlcq----==\n"
              "%s",
               i, notes[i-1]["title"], notes[i-1]["solve"]==""?"还没有解决":notes[i-1]["solve"]+"以为您解决",
               CHINESE_D->chinese_time(6,ctime(notes[i-1]["time"])), notes[i-1]["msg"] );
    me->start_more(s);
}
void do_re(object me, int i,string text)
{
    if( i > sizeof(notes) || i < 1) {
        write("无此编号!!\n");
        return;
...
int main(object me, string arg)
{
    ...
    if (!arg) {
        if(!wiz_level(me)){
                ...
                if( i > sizeof(notes) || i < 1) {
        	write("您还没有报bug,要报请用sos post来报bug。!!\n");
        	return 1;}
 ...
 str = sprintf(HIW"________________________________________________________________________"NOR"\n"
    ...
    if( sscanf(arg, "%s %d", s, i) == 2 ) {
        ...
        if ( s != "solve" && s!= "re") return notify_fail("格式不对，具体使用方法请参阅 help sos \n");
 ...
 if( sscanf(arg, "%s %s", s, arg1) == 2 ) {
    ...
    switch(arg){
    	...
    	if(!wiz_level(me)){
            ...
            {tell_object(me, HIM "您是巫师还是回复人家的求救信息吧！！！\n" NOR);}
            break;
     	case "list":
            if( !wiz_level(me) ) return notify_fail("格式不对，具体使用方法请参阅 help sos \n");
            list_bug(me);
        ...
        case "pack":
            ...
            if( !(i = sizeof(notes)) ) return notify_fail("没人发出 sos 求救信号。\n"); 
             for( ; i > 0; i-- ) 
                 if( "" != notes[i-1]["solve"] && notes[i-1]["time"] < time() - 150000) { 
                  if (notes[i-1]["re"]==""){
	...
	else if (notes[i-1]["re"]!="" && me->query("id")=="hap" || wizhood(me) == "(arch)" || wizhood(me) == "(admin)")
                                  {
                    printf("%s的 sos 求救已被删除！\n", notes[i-1]["author"]);
                    if( sizeof(notes) == 1 ) notes = ({});
                    else if(i==1) notes = notes[1..sizeof(notes)-1];
...
输入sos post该命令后，系统将提示你输入对 bug 的描述。玩家在这里应该将 bug
...
sos re用来具体回复玩家的bug信息。
sos pack 将删除所有已经标志为解决了的 bug。但要过一定的时间才可以删除。
HELP
    );
    return 1;
}