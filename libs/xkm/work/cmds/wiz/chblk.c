...
int main(object me, string arg)
{
	...
	if (who == "jiaoyi")
	{
		CHANNEL_D->set_block("jiaoyi", 1);
		CHANNEL_D->do_channel(me, "sys", "交易频道关闭了。\n");
		return 1;
	}
	if (who == "menpai")
	{
		CHANNEL_D->set_block("menpai", 1);
	...
	if (ch_name == "" || ch_name == " ")
	{
		if ( ob->query("chblk_on") || ob->query("chblk_menpai") )
		{
			tell_object(me, (string)ob->query("name")+"门派频道是关闭的。\n");
		}
		if ( ob->query("chblk_on") || ob->query("chblk_jiaoyi") )
		{
			tell_object(me, (string)ob->query("name")+"的交易频道是关闭的。\n");
	...
	else if (ch_name == "jiaoyi")
	{
		...
		{
			if ( ob->query("chblk_jiaoyi") )
			{
				ob->delete("chblk_jiaoyi");
				ob->set("chblk_on", 1);
			} else {
				ob->set("chblk_jiaoyi", 1);
			}
			tell_object(me, (string)ob->query("name")+"的聊天频道被关闭了。\n");
		}
	}
