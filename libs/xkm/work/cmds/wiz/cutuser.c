...
*/
    if (type == 1)
        tell_object(me, "You cut "+user->name()+"("+id+")"+" combat exp for "+amount+"%\n");    
    if (type == 0)
        tell_object(me, "You cut "+user->name()+"("+id+")"+" "+skill_name+" for "+amount+"%\n");  

    if (cloned)	destruct(user);
