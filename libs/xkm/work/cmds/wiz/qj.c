...
int main(object me, string arg)
{
    ...
    if 
(environment(me)->query("no_steal")||environment(me)->query("no_fight"))
    ...
    if (me->query("age") < 18)
         return notify_fail("你连十八岁都没到，就学坏啊！\n");
...
");
    ...
    else 
    {
      ...
      message_vision("$N只觉下身一阵疼痛，接着就什么都不知道了。。。\n",obj,me);
      message_vision("悲惨的事终于发生在$N的身上了。\n",obj,me);
 
CHANNEL_D->do_channel(this_object(),"rumor","据说"HIR+me->name()+HIM"把"
HIY+obj->name()+HIM"给强奸了。");
