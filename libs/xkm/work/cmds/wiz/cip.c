...
/*
   ...
   Bugs Fixed by cambrian Dec 17 2001
*/
...
//                                tmp += sprintf("%-25s ", obs[i]->query("name") 
                                tmp += sprintf("%-25s ", obs[i]->query("name") +
 "("+obs[i]->query("id")+")" + "["+query_ip_name(obs[i])+"]");
                                count++;
                                if( count % 3 == 0 ){
                                        tmp += "\n\t";
                                        count = 0;
...
;
        ...
        for(i = 0; i < sizeof(groups); i++) {
                ...
                for( j = 0; j < sizeof(obs); j++ ){
                        if( j > 0 && j % 6 == 0 ) tmp += "\n\t\t    ";
                        tmp += sprintf("%-10s", obs[j]->query("id"));
                }
                
                str += tmp;
                lob = sort_array(lob, (:strcmp($1->query("password"), $2->query(
"password")):));
                count = sizeof(lob);
                ip = j = 0;
                tmp = arg = "";
...
例：
        checkip 61.137.155.126   -> 列出从此ip连入的玩家。
        checkip 61.137.          -> 列出从此ip范围内连入的玩家。
        checkip snowman          -> 列出和次玩家相同ip的其它玩家。
        checkip                  -> 列出所有相同ip的列表，附带相同 email， passwd
列表。
