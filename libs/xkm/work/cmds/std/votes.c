...
/*数据板*/
object data=load_object("/clone/misc/vote_ban");
...
int main(object me, string arg)
{
    ...
    if(!data) data = load_object("/clone/misc/vote_ban");
    if (sscanf(arg,"%d %s",i,choice)==2)
    {
        if (((int)me->query("combat_exp")<100000) && !wizardp(me))
...
{
    object me=this_player();
    message("channel:chat",HIG+"【表决】"+HIC+this_player()->name(1)+"向"+MUD_NAME+"第"+j+"号动议提交了一票。\n"NOR,users());

        write("你感觉你又为侠客梦作了一份贡献。\n");
        this_player()->add("meng/pts", 5);
...
最新的vote系统支持以下命令：
玩家：
votes list                      察看当前的动议列表
votes <动议编号> <选项>		向一条动议投票
votes <动议编号>          	察看动议的具体情况
HELP
    );
        else 
write(@HELP
