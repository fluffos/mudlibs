...
int main(object me, string arg)
{
	...
	if(arg=="all") {
		...
		write("好了，你能扔的全扔了。\n");
		if(wizardp(me)){write(HIR"身为巫师，要想到你丢掉的每一样东西都可能被玩家捡到，为了游戏的公平，请务必慎重。\n"NOR);}
		return 1;
	}
...
int do_drop(object me, object obj)
{
	...
	if( stringp(martial = me->query_condition("perform")) && obj->query("equipped")
	  )
		return notify_fail("你正忙着呢。\n");
	...
	if (obj->move(environment(me))) {
		if( obj->is_character() )
			...
			//added by seashore to avoid being abused.
			else 
				{
			//end
			if(wizardp(me))
			write_file("/u/jhy/old/drop",sprintf("%s  巫师 %s(%s)丢%s%s%s\n",
