...
int main(object me, string arg)
{
	...
	if( sscanf(arg, "%s from %s", arg, from)==2 ) {
		...
		if(wizardp(me) && userp(env) && (wiz_level(me) <= wiz_level(env))
	...
	if( SECURITY_D->get_status(this_player())!= "(admin)"&&!obj->query("ownmake"))
	{
		if( no_get=obj->query("no_get") )
			return notify_fail( stringp(no_get) ? no_get : "这样东西拿不起来。\n");
	}	
...
int do_get(object me, object obj)
{
	...
	{
		if( obj->query("no_get")&& SECURITY_D->get_status(this_player())!= "(admin)"&&!obj->query("ownmake"))	return 0;
	}
