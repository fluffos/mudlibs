...
int mail(object me)
{
      ...
      return notify_fail("你还不会「双手互博之术」。\n");
      if ( !wizardp(this_player()) && (me->query("family/family_name") != "全真教"))
      return notify_fail("非全真嫡传弟子不能使用双手互博术！\n");
      if ( !me->is_fighting() )
      return notify_fail("「双手互博之术」只能在战斗中使用。\n");
      if (me->query_temp("hubo"))
      return notify_fail("你正用着双手互博！\n");
      if ( (int)me->query("jingli") < (int)me->query("max_jingli") )
      return notify_fail("你的精力不足！\n");
      if ( (int)me->query("jingli") < 100 )
      return notify_fail("你现在的精力不够！\n");
      msg =  HIY "$N平心静气，灵台空明，使出「双手互博之术」，双手同时出招！\n" NOR;
      message_vision(msg, me);
      count = (30 - (int)me->query_int()) / 5 + (int)me->query_skill("hubo") / 20 ;
          if( count < 1 ) count = 1 ;
          if( count > 5 ) count = 5 ;
      while( count-- ) {
        object ob;
      ...
      ob = me->select_opponent();
       ...
       {
            message_vision(CYN "$N突然心中一乱，顿时手足无措，已不能双手进行互博！\n" NOR, me);
            break;
       }
