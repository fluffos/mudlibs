...
int main(object me, string arg)
{
        ...
        if(!arg) return notify_fail("指令格式 : skill [武功名称].[?|特殊技能]\n");
	...
	write(sprintf("\n %s\n", BGRN+WHT" "+to_chinese(arg)+"功能表 "NOR));
	write("┌─────────────────────────────────┐");
	// 前面的一些skill状态输出，简单就不用说了吧。
	str = "\n";
	str = replace_string(str,"  ", "─");
	str += sprintf("│"+"等级"+"：  %s %3d/%6d\t      "+"类别"+"：  %-15s%9s  \n",skill_level(SKILL_D(arg)->type(), 
	...
	if(j && j > 0){ // 输出结果了。snowman爱画框框啊。
		if( i )
			str = str + "├─────────────────────────────────┤\n";
		str += "│"HIR"内功方面(exert + )"NOR"：\t\t\t\t\t\t    ";
		for(i=0; i < j; i++){
			if (i % 4 == 0) str += "│\n│\t";  // 一行四个
			str += sprintf("%-28s ",HIG+replace_string(file[i], ".c", "")+NOR);  // 当然输出要把文件名的.c去掉。
			if ( i > j-2 ){
	...
	if(j && j > 0){
		if( i )
			str = str + "├─────────────────────────────────┤\n";
		str += "│"HIR"外功方面(perform + )"NOR"：\t\t\t\t\t    ";
		for(i=0; i < j; i++){
			if (i % 4 == 0) str += "│\n│\t";
			str += sprintf("%-28s ",HIG+replace_string(file[i], ".c", "")+NOR);
			if ( i > j-2 ){
				switch(i%4){
				case 0: str +="\t\t\t\t\t            "; break;
				case 1: str +="\t\t\t            "; break;
...
string skill_enables(object sk) // 也是前面用到的function用以判断所能激发的基本计能。
{
	...
	for(i=0; i < j; i++) {
		if (i % 4 == 0) str += "│\n│\t";  
		str += sprintf("%-28s ", HIG+valid_type[skills[i]]+"("+skills[i]+")"NOR); 
		if ( i > j-2 ){  
			switch(i%4){
				case 0: str +="\t\t\t\t\t            "; break;
				case 1: str +="\t\t\t            "; break;
...
如： 	skill taiji-quan.dongjing  查看动静决
	skill yinyun-ziqi.yinyun   查看氤氲内聚
	skill taiji-quan.?	    查看太极拳
