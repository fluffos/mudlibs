...
mapping min_limit = ([	//某一排行榜能入榜的最低数值
	]); 
...
{
	if(file_size(query_top_file(top_id))==-1)
		return 0;
	min_limit[top_id]=data;
	return 1;
}
int unset_min_limit(string top_id)
{
	if(file_size(query_top_file(top_id))==-1)
		return 0;
	map_delete(min_limit,top_id);
	return 1;
}
string query_top_file(string top_id)
{
...
int is_banned(string top_id,object ob)
{
	...
	for(i=0;i<sizeof(banned_id);i++)
		if(banned_id[i]==(string)ob->query("id"))
			return 1;
	return 0;
}
...
string query_top(string top_id, int op)
{
	...
	int i;
	names=props=ids="";
	if(strlen(msg=read_file(TOPTEN_FILE))<1)	return "";
	msg_a=explode(msg,"\n");
...
string topten_query(string type)
{
	...
	str=HIR+CHINESE_MUD_NAME+"百大"+query_top(type,TOP_NAME)+"排行榜\n"NOR;
