...
－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－\n");
		...
		write(sprintf( HIR"上次连线地址：\t%s( %s )\n\n"NOR,
			...
			if( user->query("wiz_check/PURGE_PLAYER") != file_size("/log/static/PURGE") ) 
			{
				...
				write(HIW"有人使用过PURGE错误，立即查看 /log/static/PURGE 。\n"NOR);
			}
			if( user->query("wiz_check/promotion_PLAYER") != file_size("/log/static/PURGE") ) 
			{
				...
				write(HIW"有人使用过权限命令，立即查看 /log/static/promotion 。\n"NOR);
			}
		}
...
string check_legal_name(string name,object ob)
{
	...
	if( (strlen(name) < 4) || (strlen(name) > 8 ) || i % 2) {
