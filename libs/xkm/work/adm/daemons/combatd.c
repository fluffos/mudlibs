...
varargs int skill_power(object ob, string skill, int usage)
{
        ...
        jingli_bonus = 50 + (int)ob->query("jingli") / (int)(ob->query("max_jingli") +1) * 50;
        if (jingli_bonus > 150) jingli_bonus = 150;

        if( level<1 ) return (int)(ob->query("combat_exp")/20 * (jingli_bonus/10) );
        ...
        if (usage == SKILL_USAGE_ATTACK)
        {
                if( (ob->query_str()/10) *(jingli_bonus/10) >  300 ) 
                        return (power + ob->query("combat_exp")) /30 *300;
                else
                        return (power + ob->query("combat_exp")) /30 * (ob->query_str()/10)
                                *(jingli_bonus/10);
        }
        else
        {
                if( (ob->query_dex()/10) *(jingli_bonus/10) >  300 ) 
                        ...
                        return (power + ob->query("combat_exp")) /30 * (ob->query_dex()/10)
                                *(jingli_bonus/10);
        }
}
...
//      if( dp < 1 || victim->query_temp("yield") ) dp = 1;
        ...
        if( random(ap + dp) < dp ) {    // Does the victim dodge this hit?
                ...
                &&      random(your["jingli"]*100/(your["max_jingli"] + 1) + your["int"]) > 50 ) {
                        your["combat_exp"] += 1;
