...
#define RANDOM_PWD_LEN		5                        /* 随机密码长度 */
#define REGFILE			"/adm/daemons/mail_reg.c"//电子邮件注册信息存放文件
#define REG_ROOM		"/d/xiakedao/register"   //调用send_mail()的room
#define PUBLIC_MUD_MAIL         "virtual1998@etang.com"  //就是MUD发信用的电子邮件帐号
#define MUD_WEB                 "xkm.my163.com"          //这个MUD的主页了
...
/*mailname就是MUD发信用的电子邮件帐号，mailpasswd就是它的密码了*/
protected string mailname = "virtual1998",mailpasswd = "708402";

protected mixed content = ([]);

/* 这是MUD发信用的电子邮件帐号的 SMTP 服务器的域名和 IP 地址 */
protected string domain_name = "smtp.etang.com",address = "61.152.250.207";
...
void send_mail(object user,string mail)
{
	...
	//if(mail_have_reg(user,mail)) return;  //注释掉后允许用同一个email注册多个id
...
请您"HIG"五分钟"NOR"后检查您的邮箱。请用您的新密码登陆!如果您在"HIC"24"NOR"小时后还未能收到我们
给您发出的邮件，请您向 "HIY"%s"NOR" 发信说明详细情况，
我们会尽快为您解决。祝您在%s玩的愉快，再见！\n",mailmsg->rcpt,PUBLIC_MUD_MAIL,MUD_NAME));
	...
	link->set("registered", "yes");    //add for xkx
	usr->set("email",mailmsg->rcpt);
	usr->set("registered", "yes");    //add for xkx
	usr->set("have_reg",1);
	...
	tell_room(environment(usr),"你只觉得眼前一花，"+usr->query("name")+"不见了。\n",({usr}));	
	{
                ...
                sprintf("%s(%s)邮件发出退出游戏。\n"NOR,usr->name(),geteuid(usr)));
	}
...
protected void read_callback(int fd,string message)
{
	...
	if(mailmsg->status == 1) // server ready
	{
		...
		//if(sscanf(message,"%*sAUTH=%s\n%*s",mechanism) == 3)  //改为默认进行smtp认证
		{

			socket_write(fd,sprintf("AUTH %s\r\n",PUBLIC_MUD_MAIL));  //auth xxxx@xxxx.com格式
			mailmsg->status = 2;
			return;
		}
...
protected int mail_have_reg(object user,string mail)
{
	...
	if(member_array(mail,mail_reg)!=-1)
	{
		tell_object(user,"这个邮件地址已经注册过了，本游戏不允许同一地址多重注册。\n对不起！\n");		
		{
			set("channel_id", "注册精灵" );
                        CHANNEL_D->do_channel(this_object(), "sys",
                        sprintf("%s(%s)注册请求被拒绝，退出游戏。\n"NOR,user->name(),geteuid(user)));
		}
		destruct(user);
		destruct(link);
...
// 下面就是对player这个array的处理了
	if(!player || !sizeof(player)){			
		message("system", YEL "无符合条件的玩家" ,filter_array(children(USER_OB), (: userp($1) && wizardp($1) :)));
		return;
	}
