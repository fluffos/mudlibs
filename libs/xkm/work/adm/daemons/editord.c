...
// bugs entirely fixed by seashore 2002/6
...
//		filename = XKX_ARCHIVE + "Unknown";
		filename = XKX_ARCHIVE + "Unknown.pst";//we need a postfix too.
	}
	else
	{
	if (fname)
		...
		filename = XKX_EDITOR + "Unknown.pst";
	}
...
/*	Articles = ({article});//???
	...
	return ( write_file(filename, Articles[sizeof(Articles)-1] + "\n", 0) );  */
//	we needn't write_file with the flag 1 because we have fixed the bug existed in /inherit/misc/bboard.
	
	if (file_size(filename) <= 0)
		return ( write_file(filename, article + "\n", 1	) );
	return ( write_file(filename, article + "\n", 0	) );
}
...
int get_file_num(int arc, string year, string month)
{
	...
	if (arc) {
		base = XKX_ARCHIVE+"/"+year+"/";
        	if (file_size(base) != -2)
        		mkdir(base);

		base = XKX_ARCHIVE+"/"+year+"/"+month+"/";
        	if (file_size(base) != -2)
			mkdir(base);
	}
