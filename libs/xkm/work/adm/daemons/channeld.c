...
mapping channels = ([
	...
	"jiaoyi": ([      "msg_speak": HIM "【交易】%s："HIC"%s\n" NOR,
...
varargs int do_channel(object me, string verb, string arg, int emote)
{
	...
	//add by puma when 2002-09-06
	for(i=0,j=sizeof(banned_msg);i<j;i++)
	if( strsrch(arg,banned_msg[i]) != -1 || regexp(arg,banned_msg[i]) ) {
		write_ob(me,"对不起，这种用语会造成其他人的困扰。");
		return notify_fail("\n");
	}
	...
	//jhy add bangpai chat 02.12.10
	if (channels[verb]["bangpai_only"]) {
		if(channels[verb]["bangpai_only"] && !me->query("club")){
			if ( wizardp(me) ){
				menpai = "帮派";
				wizmp = 1;
			} else return notify_fail("你需要先加入一个帮派！\n");
		} else {
			if ( wizardp(me) ) wizmp = 1;
			menpai = me->query("club");
		}
                channels[verb]["msg_speak"] = HIW "【"+(wizardp(me) ? HIW : "")+menpai+ NOR+HIW "】%s：%s\n" NOR;
                channels[verb]["msg_emote"] = HIW "【"+(wizardp(me) ? HIW : "")+menpai+ NOR+HIW "】%s" NOR;
        } 

        //wiz chat color is $HIG$
        if( userp(me) && wizardp(me) && verb == "chat" ){
            channels[verb]["msg_speak"] = HIG "【闲聊】"HIC"%s：%s\n" NOR;
            channels[verb]["msg_emote"] = HIG "【闲聊】"HIC"%s" NOR;
        }else if( !wizardp(me) && verb == "chat" ){
            channels[verb]["msg_speak"] = HIC "【闲聊】%s：%s\n" NOR;
            channels[verb]["msg_emote"] = HIC "【闲聊】%s" NOR;
        }//else if( !player (me) && verb == "chat" ){
           // channels[verb]["msg_speak"]= HIW "【闲聊】%s：%s\n" NOR;
           // }
	...
	if ((me->query("chblk_jiaoyi") && (verb == "jiaoyi"))||
		((verb == "jiaoyi") && BLOCK_RUMOR))
		return notify_fail("你的不可以用交易频道,想交易请用paimai 物品 add n gold！\n");
    ...
    if ((me->query("chblk_xkx") && (verb == "xkx"))||
        ((verb == "xkx") && BLOCK_CHAT))
		return notify_fail("ｏｏｐｓ！你的侠客行频道被关闭了！\n");
