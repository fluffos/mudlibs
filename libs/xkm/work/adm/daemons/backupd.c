// BackUpD.c  Write By DoingLu
// Install for SNOW MUD 1999/12/22 By JackyBoy
...
#define BACKUP_DATE     20               // the oldest's backup (days)
...
static int *tlist = ({ 0,450,459,500});
...
private void change_state(int new_state)
{
        ...
        case GET_READY_2:
                ...
                {
                        message("vision", sprintf("现在是 %d 点 %d 分，系统将在 %d 点 %d 分"
                                       "自动备份所有玩家的数据，期间游戏会有停滞。\n",
                                       lt[LT_HOUR], lt[LT_MIN],
                                       (tlist[BACKUPING] / 100) % 100,
                                       tlist[BACKUPING] % 100), users());
                        break;
                } else
                        new_state = BACKUPING;
        ...
        case BACKUPING:
                state = new_state;
                message("vision", sprintf(HIY "现在是 %d 点 %d 分，系统开始"
                                       "自动备份所有玩家数据，请稍候...\n" NOR,
                                       lt[LT_HOUR], lt[LT_MIN]), users());

                backup_data();

                message("vision", sprintf(HIC "系统已经处理完备份工作。\n" NOR,
                                       lt[LT_HOUR], lt[LT_MIN]), users());
