...
// modified by sdong for adding qi to shaolin,emei,dali,xueshan,baituo, 7/4/1999
// maco added music to change taohua's jing 12/28/1999
...
void setup_human(object ob)
{
	...
	int lvl, skill,skill1, skill2, skill3, skill4;
	...
	if( userp(ob) || undefinedp(my["max_jing"]) || (!userp(ob) && !ob->query("special_npc")) ) {
		...
		&& (jing_age =(int)ob->query_skill("yunu-jue", 1)) > 39 )
		{
			...
			skill1 = (int)ob->query_skill("yunu-xinjing",1);

			if ( jing_age > 0 )
				my["max_jing"] += jing_age * (skill1/8);
		}

                // 凌霄城冰雪诀：３０岁前补精，３０岁后长精
                if( mapp(my["family"]) && my["family"]["family_name"] == "凌霄城"
                && (jing_age =(int)ob->query_skill("bingxue-jue", 1)) > 39 )
                {
                        jing_age = jing_age/2;
                        if (my["age"] <= 30) jing_age -= my["age"];
                        else jing_age -= 30;

                        skill1 = (int)ob->query_skill("bingxue-xinfa",1);
                ...
                // 姑苏慕容：３０岁前补精，３０岁后长精
                if( mapp(my["family"]) && my["family"]["family_name"] == "姑苏慕容"
                && (jing_age =(int)ob->query_skill("murong-xinfa", 1)) > 29 )
                {
                        jing_age = jing_age/2;
                        if (my["age"] <= 30) jing_age -= my["age"];
                        else jing_age -= 30;

                        skill1 = (int)ob->query_skill("douzhuan-xingyi",1);

                        if ( jing_age > 0 )
                                my["max_jing"] += jing_age * (skill1/6);
                }


                // 灵鹫宫五音十二律：３０岁前补精，３０岁后长精，但效用小
			...
			skill1 = (int)ob->query_skill("bahuang-gong",1);
      ...
      if( !userp(ob) && ob->query("max_jing") < 1) my["max_jing"] = 100;
	}
	//}

	//if( userp(ob) || undefinedp(my["max_qi"]) ) {
	if( userp(ob) || undefinedp(my["max_qi"]) || (!userp(ob) &&!ob->query("special_npc")) ) {
		...
		// 佛家保气：
			if ( mapp(my["family"]) ) {
				...
				lvl = (int)ob->query_skill("buddhism", 1);
                                 else if ( my["family"]["family_name"] == "古墓派" )
		...
		&& (xism_age=(int)ob->query_skill("taoism", 1)) > 39 )
		{
		  ...
		  && (xism_age=(int)ob->query_skill("poison", 1)) > 39 )
		  {
			  ...
			  skill1 = (int)ob->query_skill("hamagong",1);
                ...
                if( mapp(my["family"]) && my["family"]["family_name"] == "丐帮" )
		{
		    ...
		    skill1 = (int)ob->query_skill("huntian-qigong",1); // added by sdong, 3/20/2000
		...
		&& (qi_age =(int)ob->query_skill("zhengqi-jue", 1)) > 39 )
		{
			...
			my["max_qi"] += xism_age * (skill1/16);
                ...
                // 凌霄城：３０岁前补气，３０岁后长气 
                        if ( mapp(my["family"]) && my["family"]["family_name"] == "凌霄城"
                        && (xism_age=(int)ob->query_skill("bingxue-xinfa", 1)) > 39 )
                        {
                                xism_age = xism_age/2;
                                if (my["age"] <= 30) xism_age -= my["age"];
                                else xism_age -= 30;
                ...
                // 姑苏慕容：３０岁前补气，３０岁后长气 
                        if ( mapp(my["family"]) && my["family"]["family_name"] == "姑苏慕容"
                        && (xism_age=(int)ob->query_skill("douzhuan-xingyi", 1)) > 29 )
                        {
                                xism_age = xism_age/2;
                                if (my["age"] <= 30) xism_age -= my["age"];
                                else xism_age -= 30;
			...
			skill1 = (int)ob->query_skill("shenghuo-xuanming",1);

			if (xism_age > 0 )
			my["max_qi"] += xism_age * (skill1/10);
			  }
		if( my["max_neili"] > 0 ) my["max_qi"] += my["max_neili"] / 4;
      if( !userp(ob) && ob->query("max_qi") < 1) my["max_qi"] = 100;
	}
	//}
