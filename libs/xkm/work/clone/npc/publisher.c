...
{	object ling;
	...
	ling = present("bang ling", me);
	
	if(me->query_temp("bangs/fam")!="长乐帮") 
        return ("唉，司徒帮主规定只能给帮众工作。\n");
	if (!present("ling", this_player())&&!ling->query("job"))
	return 0;
	if (me->query_temp("copy")==1) 
	return ("要你抄你就抄吧。\n");
	 
	
	if (ling->query("job/name")=="伙计"){
	me->set_temp("copy",1);
	ling->detele("job");
         return ("好吧，你就在这里抄抄书稿吧。\n");
	 }
	else
         {return ("啊呀，小的现在还顾不起伙计。\n");
	}
}	
