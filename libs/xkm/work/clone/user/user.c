...
//	this_object()->remove_interactive();   //add
...
//	remove_all_enemy();
	...
	//    set("tmp_neili",query("max_neili") );
	  //  set("tmp_jingli",query("max_jingli") );
	    //save();*/
	// Stop fighting, we'll be back soon.     //move
	    remove_all_enemy();
	    call_out("user_dump", NET_DEAD_TIMEOUT, DUMP_NET_DEAD);
	    tell_room(environment(), query("name") + HIG"断线了。\n"NOR, this_object());
	    CHANNEL_D->do_channel(this_object(), "sys", "断线了。");	
	}
	else if( query("doing") )    //add
	{
		if (environment())
                        message("vision", name() + HIB"离线了。\n"NOR,
