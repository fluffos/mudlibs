...
void restore_stuff(object player)
{
	...
	player->delete("bwlt/admitted");
	player->delete("bwlt/fighting");
...
void init()
{
	...
	if(objectp(player)&&player->query("bwlt/fighting"))
	{
...
string long_desc()
{
	...
	desc=HIY"
	  ...
	  |__________________|_|________|_|__________________|"+HIG"
             ||                                          ||
             ||  ________ 比 武 大 赛 龙 虎 板 ________  ||
             ||  |                                    |  ||\n";
...
int valid_leave(object me, string dir)
{
	...
	if(dir=="out")
	{
		if( wizardp(me)||me->query("judge")||me->query("bwlt/defeated")>=16 ||!me->query("bwdh")||!query("start") )	return 1;
		else
		{
...
int do_stop(string arg)
{
	...
	if(!wizardp(me)&&!me->query("judge"))
	{
		tell_object(me,"你不是巫师，不能终止比武擂台！\n");
		return 1;
	}
	if(!query("start"))
	{
		tell_object(me,"比武还没开始呢。如要开始，请用 START 命令。\n");
		return 1;
	}
	if((host=query("host")))
		ob1=find_player(query("host"));
	if(ob1&&ob1->query("bwlt/fighting"))
	{
		ob1->move(KANTAI);
	}
	if((challenger=query("challenger")))
	{
		...
		if(ob2&&ob2->query("bwlt/fighting"))
		{
			ob2->move(KANTAI);
		}
	}
...
int do_bet(string arg)
{
	...
	string name,unit;
	int amount;
	return notify_fail("测试期间暂不开放赌博业务！\n");	//目前没有处理同 ip 或 exp　相差很多的两个 id 不允许下注！
	...
	message_vision(sprintf(HIG"$N掏出%s%s%s往桌子上一摔，说：我押在$n身上。\n"NOR,chinese_number(amount),money->query("base_unit"),money->query("name")),me,obj);
...
int start(string host,string challenger)
{
	...
	ob2=find_player(challenger);
	if( ob1 && ob2 && ob1->query("bwlt/fighting") && ob2->query("bwlt/fighting") )
	{
		message_vision(HIR"一声锣响，$N和$n比赛开始。\n"NOR,ob1,ob2);
		ob1->kill_ob(ob2);
		ob2->kill_ob(ob1);
		ob1->set("bwlt/fighting",1);
		ob2->set("bwlt/fighting",1);
		set("fighting",1);
...
int auto_check()
{
	...
	if(!query("start"))	return 0;
	if(!(host=query("host")))
	{
		tell_room(this_object(),"主持人高声说，比武大赛正在进行，赶紧报名(baoming)呀！\n",this_object());
		delete("fighting");
		delete("challenger");
		delete_temp("no_challenger");
		remove_call_out("auto_check");
		call_out("auto_check",50);
		return 0;
	}
	old_host=query("host");
	old_challenger=query("challenger");
	ob1=find_player(host);
	if(ob1&&living(ob1)&&ob1->query("qi")>0&&ob1->query("bwlt/fighting"))
		message_vision("主持人高声说，$N是现在的擂主.\n",ob1);
	else
	{
		message_vision(HIR"$N守擂失败，被抬下擂台。\n"NOR,ob1);
		ob1->restore();
		ob1->set("bwlt/defeated",1+ob1->query("bwlt/defeated"));
		ob1->delete("bwlt/fighting");
		ob1->set("bwlt/"+old_challenger+"/time",ob1->query("bwlt/"+old_challenger+"/time")+1);
		ob1->set("bwlt/"+old_challenger+"/lose",ob1->query("bwlt/"+old_challenger+"/lose")+1);
		sort_rank(ob1,-1);
		delete("fighting");
		full_all(ob1);
		ob1->save();
	...
	if(challenger=query("challenger"))
	{
		...
		if(ob2)
		{
			...
			{
				ob2->restore();
				ob2->set("bwlt/win",1+ob2->query("bwlt/win") );
				ob2->set("bwlt/"+old_host+"/time",ob2->query("bwlt/"+old_host+"/time")+1 );
				ob2->set("bwlt/"+old_host+"/win",ob2->query("bwlt/"+old_host+"/win")+1 );
				ob2->set("bwlt/once",1);
				ob2->set("bwlt/fighting",1);
				set("host",challenger);
				delete("challenger");
			...
			if(!(living(ob2))||ob2->query("qi")<=0)
			{
				...
				ob2->restore();
				ob2->set("bwlt/defeated",1+ob2->query("bwlt/defeated"));
				ob2->set("bwlt/"+old_host+"/time",ob2->query("bwlt/"+old_host+"/time")+1);
				ob2->set("bwlt/"+old_host+"/lose",ob2->query("bwlt/"+old_host+"/lose")+1);
				ob2->delete("bwlt/fighting");
				sort_rank(ob2,-1);
				full_all(ob2);
				ob2->save();
				//ob2->move(KANTAI);
				...
				{
					ob1->restore();
					ob1->set("bwlt/win",1+ob1->query("bwlt/win"));
					ob1->set("bwlt/"+old_challenger+"/time",ob1->query("bwlt/"+old_challenger+"/time")+1);
					ob1->set("bwlt/"+old_challenger+"/win",ob1->query("bwlt/"+old_challenger+"/win")+1);
					ob1->set("bwlt/fighting",1);
					sort_rank(ob1,1);
					message_vision("主持人高声说： $N打败$n，守擂成功！\n", ob1,ob2);
					if(ob1->query("bwlt/once"))
					{
						message_vision("主持人高声说： $N真是神勇，竟然连胜两场。请下场休息一会！\n", ob1);
						delete("host");
						delete_temp("no_challenger");
						message_vision(HIM"$N神彩飞扬地跳下擂台休息去了.\n"NOR, ob1);
						ob1->delete("bwlt/once");
						set("fighting",0);
						full_all(ob1);
						ob1->save();
						//ob1->move(KANTAI);
...
// count host win once
		ob1->restore();
		ob1->set("bwlt/win",1+ob1->query("bwlt/win"));
		//ob1->set("bwlt/"+challenger,ob1->query("bwlt/"+challenger)+1);
		//ob1->set("bwlt/"+challenger+"/win",ob1->query("bwlt/"+challenger+"/win")+1);
		ob1->delete("bwlt/fighting");
		sort_rank(ob1,1);
		ob1->save();
		//ob1->move(KANTAI);
...
int let(string arg)
{
	...
	full_all(ob);
	message_vision("$N进入擂台。\n",ob);
	tell_object(ob,"主持人告诉你"+RANK_D->query_respect(ob)+"，请从武器架上取武器(look jia; get armor from jia and get weapon from jia).\n");
	delete_temp("no_challenger");
	ob->set("bwlt/fighting",1);
	update_player(ob);
	return 1;
