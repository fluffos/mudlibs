...
string chinese_period(int t)
{
	...
	if(n) time += chinese_number(n) + "个月"	+ (strlen(time)&&d?"":"又");
	if(d) time += chinese_number(d) + "天"		+ (strlen(time)&&h?"":"又");
	if(h) time += chinese_number(h) + "小时"	+ (strlen(time)&&m?"":"又");
	if(m) time += chinese_number(m) + "分"		+ (strlen(time)&&s?"":"又");
