...
int do_search(string arg)
{
                  ...
                  if( !present("yao chu", me) )
                                         return notify_fail("你没有工具，山路硬没法寻找草药！\n");
                  ...
                  if( query("count") < 1 && (int)me->query_temp("herb/times") > 30 )
                                         return notify_fail("你把四周的灌木杂草里都翻遍了，可是没发现什么，看来这里没指望了！\n");

                  message_vision("$N用药锄拨动着四周的灌木杂草，仔细地看有没有草药！\n", me);

                  power = (int)me->query_temp("herb/times") * ( me->query("kar")+me->query("int") + me->query("str") )/2;
                  power *= query("count");

                  if( power/1 + random(power/1) > 200 ) {
                                         me->set_temp("herb/found", 1);
                                         message_vision(HIG"\n$N用药锄不断的拨动着四周的灌木杂草，忽然发现杂草之中有一株特别的草！\n"NOR, me);
                                        return 1;
                  }
