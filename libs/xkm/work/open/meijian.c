...
string ask_job()
{
	...
	if((ob->query("combat_exp"))<1000)                             //      exp<10k  can not do meijian's job; 
	...
	if((ob->query("job_status"))==1)
	{
	  ...
	  return "你来晚了，守门的位置已经分派完了。过一会再来看看。\n";
		...
		ob->set_temp("apply/short",
               ({ HIW"大门守卫 "NOR+ob->query("name")+"("+capitalize(ob->query("id"))+")"}));
		add("job_count",1);
	 }
	...
	call_out("doing_job",200,ob); //use call out and set time 10 minitues.
	return "打起精神，不要偷懒。\n";
}
