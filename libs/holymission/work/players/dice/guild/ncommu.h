...
_shout( str )
{
    ...
    if ( !(TP->query_immortal()) && TP->query_sp() < SHOUT_COST )
...
*/
	...
	if ( !TP->query_immortal() )
	    TP->restore_spell_points( -1 * SHOUT_COST );
    }
...
_tell( str )
{
	...
	write( "You try to tell " + who->query_name() + " in nihonese:\n  " + what + "\n" );
...
_emote (str){
    ...
    "sys/chatd.lpc"->do_chat(8, "[*Ninja*] "+this_player()->query_name()+" "+str+"\n");
    return 1;
}
