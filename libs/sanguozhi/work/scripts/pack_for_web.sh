...
#
# Like fluffos/xkx100 and fluffos/nt7, this repo's mudlib root IS the repo
# root itself (config.cfg: "mudlib directory : ./"), so staging is just
...
STAGE=$(mktemp -d "${TMPDIR:-/tmp}/sgzpack.XXXXXX")
...
# strings instead of real paths, and every cp on one fails.
...
#        in-image path here) --------------------------------------------
python3 - "$REPO_ROOT/config.cfg" "$STAGE/mudlib/mudlib.cfg" <<'PYEOF'
import re, sys
...
echo "packed sanguozhi -> $OUT ($SIZE)"
