...
源码：<https://github.com/fluffos/sanguozhi>。**本馆只保留说明和启动脚本。** `work/` 是 `fluffos/sanguozhi` submodule（mudlib 根目录就是仓库根）。LPC 在那个仓库里是 `.lpc` + §9 格式化——修复请提交到那个仓库，不要在本馆再改一份。管理员试玩号在 `overlay/`。
