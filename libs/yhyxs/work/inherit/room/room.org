...
void reset()
{
        ...
        mapping guards;
        object *obs;
        string mem_want_kill;
        int meminfo, memcost1, memcost2;
        ...
        if (this_object()->is_chat_room()) return;
        // clear object
        memcost1 = memory_info();
        meminfo = 0;
        obs = all_inventory();
        /*
        ...
        */
        for (i = sizeof(obs) - 1; i >= 0; i--)
        {
                if (obs[i]->is_character() ||
                    ...
                    obs[i]->is_corpse()) 
                        continue;
                meminfo += memory_info(obs[i]);
                destruct(obs[i]);
        }
        mem_want_kill = memory_expression(meminfo);
        memcost2 = memory_info();

        log_file("room_clean_up",  sprintf("\n欲清除内存空间：%s   实际清除内存空间：%s\n",
                                     mem_want_kill, memory_expression(memcost1 - memcost2) ));
