...
void build_armor(object ob,object base)
{
	...
	if(base->query("old_long")) fabao_long = HIY""+base->query("old_long")+HIY"上面隐约金色闪现"NOR;
	else fabao_long = HIY""+base->query("long")+HIY"上面隐约金色闪现"NOR;
	...
	newob->set("armor_prop/armor", 10+base->query("armor_prop/armor"));		
	newob->set("armor_prop/dodge", 10+base->query("armor_prop/dodge"));		
	newob->set("armor_prop/spells",10+base->query("armor_prop/spells"));	
	newob->set("armor_prop/force",10+base->query("armor_prop/force"));	
	newob->set("armor_prop/parry",10+base->query("armor_prop/parry"));
	newob->set("armor_prop/unarmed",10+base->query("armor_prop/unarmed"));	
	newob->set("armor_prop/sub_max_jin",365+random(1080));	
	newob->set("armor_prop/sub_max_shui",365+random(1080));	
	newob->set("armor_prop/sub_max_mu",365+random(1080));	
	newob->set("armor_prop/sub_max_huo",365+random(1080));	
	newob->set("armor_prop/sub_max_phy",365+random(1080));	
	newob->set("armor_prop/sub_max_spells_all",365+random(1080));	
	...
	ob->add("force", -200);
	ob->add("mana", -200);
	 ob->add("maximum_force",-36);
	    ob->add("max_force",-36);
	    ob->add("maximum_mana",-36);
	    ob->add("max_mana",-36);
...
void build_weapon(object ob,object base)
{
 ...
 if(weapon_type=="dagger") ob_file = weapon_dir + "dagger";
		if(ob_file=="none") 
	        {
	          write("这样东西不能生成黄金装备。\n");
	          return;
...
{
 newob->set("weapon_prop/damage", 64+random(36));          
newob->set("flag",2);
}
else
	
	newob->set("weapon_prop/damage", 108+random(48));		
newob->set("weapon_prop/attack", 12+random(24));  
	newob->set("weapon_prop/add_max_jin",1080+random(365));	
	newob->set("weapon_prop/add_max_shui",1080+random(365));	
	newob->set("weapon_prop/add_max_mu",1080+random(365));	
	newob->set("weapon_prop/add_max_huo",1080+random(365));	
	newob->set("weapon_prop/add_max_phy",365+random(1080));
	newob->set("weight", 10000);
	// anything else need to be set?
	...
	newob->set("default_file", ob_file);
        newob->set("series_no",series_no);
        newob->set("spi",12+random(36));
        newob->set("skill_type",weapon_type);
        newob->set("actions",actions);
        newob->setup();
        newob->save();	
        ob->set("gold/gold_num",gold_num);
	ob->set("gold/series_no",series_no);
...
int check_baoshi(object ob)
{
   ...
   if(!ob->query("inset_c")) gold_armor = 0;
