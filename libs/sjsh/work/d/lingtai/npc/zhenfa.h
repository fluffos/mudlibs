...
int waiting(object me) {
  ...
  if ((me->query("assigned_sixiang") && !objectp(fighter = present(
    me->query("assigned_sixiang"),
    environment(me)
  ))) ||
    (me->query("assigned_bagua") && !objectp(fighter = present(
      me->query("assigned_bagua"),
      environment(me)
    )))) {
    wait_time++;
    call_out("waiting", 1, me);
...
int bfighting(object me, object fighter, int count) {
  ...
  if (count < 4) {
    ...
    message(
      "vision",
      HIW "只见" + daoshi1->query("name") + "吟道：" + saywords[count] + "\n" NOR,
      room2
    );
    daoshi1->move(room2);
