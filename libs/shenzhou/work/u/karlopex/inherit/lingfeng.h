...
int do_check(string str)
{
	...
	mixed i,j;
        ...
        if( !str ) return notify_fail("你要看谁的状态？\n");
	ob = find_player(str);
	if( !ob ) ob = find_living(str);
	if (!ob) return notify_fail("现在没这个人。\n");
	if (!wizardp(ob)) return notify_fail("你只能查看巫师的状态。\n");
	if (ob->query("env/immortal")){
		i = ob->query("env/immortal");
		if (intp(i)) i = chinese_number(i)+"级";
		else if(i == "YES") i = "允许";
	}
	if (ob->query("env/invisibility")){
		j = ob->query("env/invisibility");
		if (intp(i)) j = chinese_number(i)+"级";
		else j = "允许";
	}
	printf("%s(%s)现在的状态是 隐身：%s，不死：%s。\n",
		(string)ob->name(),
		(string)ob->query("id"),
		j?j:"关闭",
		i?i:"关闭");
