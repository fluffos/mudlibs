...
int do_tan(string arg)
{
	...
	if( ob != weapon )
        ...
        if( (int)me->query_skill("qinqi-shuhua", 1) < 10 ) {
...
//                 message_vision("$N将$n放在唇边一吹，结果发出一阵五音不全的声响。\n"NOR, me, ob);
	...
	if( (int)me->query("jing") > cost ) {
		...
		message_vision("$N手抚$n，反覆练习著「"+msg+"」的技巧。\n"NOR, me, ob);
                if( me->query("family/family_name") == "昆仑派" && (int)me->query_skill("music", 1) < 300 )
...
//                 if( me->query("family/family_name") == "桃花岛" && (int)me->query_skill("music", 1) < 333)
                me->improve_skill("qinqi-shuhua", me->query_int()*2 + 1);
