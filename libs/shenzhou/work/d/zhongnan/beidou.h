...
int beidouzhen(object me)
{
		  ...
		  if ( me->query("neili") < 60 || me->query("jingli")<50 )
		  {
				  if(!objectp(weapon = me->query_temp("weapon"))
					...
					if (userp(me))
					log_file("job/qz-lianzhen",sprintf("[%s] %s(%s) 在 %d 秒内 Lianzhen 得到 %d 经验和 %d 潜能。\n", ctime(time()) ,me->name(), getuid(me), time()-me->query_temp("qz/jobtime"), me->query_temp("qz/jobexp"), me->query_temp("qz/jobpot") )); //add log file.
					me->remove_all_enemy();
					me->delete_temp("pending/beidou");
					me->delete_temp("beidou_time");
					me->delete_temp("bd_pos");
