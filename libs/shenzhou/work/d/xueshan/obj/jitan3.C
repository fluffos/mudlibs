...
void create()
{
        set_name(HIY"无念法坛"NOR, ({ "fa tan", "altar", "tan" }) );
        set_weight(300000);
        ...
        if( clonep() )
                ...
                set("unit", "个");
                set("long", "这是一口新铸的铜制祭坛，高约两丈，方圆约有三丈。\n");
                set("value", 1000);
                set("material", "wood");
                set("no_get",1);
        }
...
int do_light(string arg)
{
        ...
        set("in_use", 0);
        log_file("job/xueshan",me->query("name")+" (" + 
            me->query("id") + ") 开始烧人  "+ctime(time())+"\n");
...
/************************************/
        ...
        me->add("potential", 180 + random(40));
...
void rise(object me)
{
        ...
        if (me->query_temp("award_pending")*10/me->query("combat_exp") >= 16) {
        	if ((int)me->query("max_neili") < ((int)me->query_skill("force") * (int)me->query("con") * 2 / 3)) 
        	{
...
//************************************
	...
	me->add("combat_exp", 380 + random(40));
	me->set("mp_job_time",time() );
	log_file("job/xueshan",me->query("name") +" (" + me->query("id") + ") 完成一次法事"+ctime(time())+"\n");
