...
- 有一处真实存在、未修复的内容缺口：`d/city/chatroom.lpc` 引用了
  ...
  不凭空补内容"的处理方式。
- 登录横幅、BBS 链接通篇写的是"雄霸天下"，和标题"鹿鼎天下II"完全
  对不上——但全库搜索确认游戏内容（任务线、门派设定）确实是《鹿鼎
  记》题材，韦小宝/神龙教/康熙等命中 48 个档案；横幅只是从"雄霸天
  下"引擎模板直接抄来、作者重新包装时没换掉的通用文案，不影响标题
  的准确性。
- 死亡有完整的昏迷→送医复活机制：被打倒后短暂昏迷（此时所有指令
  都会被挡下，容易误以为指令系统坏了），随后自动复活于"武庙"（岳
  飞庙），死因和死亡次数会记录在个人档案里。

## 深度功能测试新发现的 bug（详见 NOTES.md）

`logind.lpc` 里有两处和姊妹档案 `ldtx` 完全相同、行号都一样的祖传
bug（说明是共享血统继承下来、从未修过的问题，不是这份档案自己引入
的）：一处是新玩家中文名字确认之后紧跟着的调试残留
`printf("%O\n", ob)`（AGENTS.md §7.34）；另一处是食物/饮水满值初
始化判断误读了 `ob->query("age")` 而不是 `user->query("age")`，导
致新角色食物/饮水槽永远是空的（AGENTS.md §8.9 第四个确认实例）。
两处均已修复；`ldtx` 里完全相同的两行还没修，留待下次处理该档案时
一并移植过去。
...
份档案本身没有预先存在的管理员账号或 README。修正（round-two 深度
测试发现）：此前记录说"已在游戏内确认"，但存档从未真正提交到仓
库——`git log` 确认此前从未有过对应的 commit，本地也不存在。已重
新走真实注册流程创建并提交存档，`score` 确认"目前权限：(admin)"
显示正确，`update` 指令确认真正的写权限生效。
