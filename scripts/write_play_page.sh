...
# zjProtocol: does this lib implement the "指间MUD" custom mobile-client
# login handshake (scripts/web_shell_override/zj-protocol.js)? Detected
# from the lib's own source rather than a hand-maintained slug list --
# work/include/zjmud.h (the protocol's own header, defining ZJKEY and the
# ESA/ZJSEP/... message-framing macros) only exists in libs that actually
# use it, so this self-maintains if another one ever turns up.
if [ -f "$LIB/work/include/zjmud.h" ] || [ -f "$LIB/work/lib/include/zjmud.h" ]; then ZJ_PROTOCOL=true; else ZJ_PROTOCOL=false; fi

MUDLIB_SUBDIR=$(python3 -c '
import json, sys
...
if p.is_file():
    try:
        ...
        sub = ((m.get("upstream") or {}).get("mudlib_subdir") or "").strip().strip("/")
...
window.FLUFFOS_ZIP_BOOT = {
  ...
  zjProtocol: $ZJ_PROTOCOL,
  mudlibSubdir: $(python3 -c 'import json,sys; print(json.dumps(sys.argv[1]))' "$MUDLIB_SUBDIR"),
};
EOF
