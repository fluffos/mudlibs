...
#   play.html                 the web terminal, patched so that fluffos.js /
...
#                             -- named play.html (not index.html) because
#                             scripts/gen_site_index.py generates a separate,
#                             crawlable, server-rendered index.html (full
#                             description + README/NOTES.md) for this same
#                             slug dir; build_site.sh's assembly step copies
#                             both into the final per-lib output directory.
#                             This is what actually loads/boots the WASM
#                             driver, so it's reached only via that landing
#                             page's "Play Now" link, never on first load.
#
...
// Generated by scripts/pack_lib_for_web.sh -- consumed by play.html.
window.FLUFFOS_BOOT = {
  mount: "/mudlib",
...
# --- 4b. README.md / NOTES.md, verbatim, for play.html's own Info tab -------
# play.html fetches these two by their plain filename (relative to the lib's
...
# (The landing index.html gen_site_index.py generates for this same slug dir
# shows the same content server-rendered, read straight from libs/<slug>/ --
# this copy is only for a visitor who is already in the terminal.)
[ -f "$LIB/README.md" ] && cp "$LIB/README.md" "$OUT/README.md"
[ -f "$LIB/NOTES.md" ] && cp "$LIB/NOTES.md" "$OUT/NOTES.md"

# --- 5. per-lib play page: patched copy of the web terminal ------------------
